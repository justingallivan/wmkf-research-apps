...
- **v4 process record (2026-08-06, owner-directed rescue):** **PART A answer — this was an inconsistency, not new evidence.** The post-build review applied a stricter shipping judgment to the same verify→dispatch window that v3 had explicitly accepted and instructed reviewers not to re-report. No new source evidence appeared between the v3 authorship and that review; Codex failed to honor its own recorded residual. The owner adjudicated by directing the fix. **Fix:** preview rendering no longer mints or rotates live token authority; it emits a JWT-shaped, deliberately non-live URL placeholder. After all recipient gates, `send-emails-service` retains defense-in-depth verification for any real legacy/edited JWT, computes the unchanged per-recipient expiry, mints/stores the final authoritative token, and substitutes only JWT path segments in the final edited subject/body immediately before dispatch. Research-only preview rows no longer expose a live manual-copy link and fail closed with guidance to verify the address or use the separate regeneration workflow. Per-row `email_failed {code,error}` behavior is retained, including `external_link_mint_failed`; the adversarial test rotates the shared durable hash after verification reads it but before send resumes and proves neither the stale nor intervening JWT reaches Dynamics.
- **Bounded render abort (2026-08-06, Sonnet build, `8b66beb3`):** Codex post-build finding 2 closed — every render-emails fetch in both modals carries an AbortController with `PREVIEW_RENDER_TIMEOUT_MS` (45s); the manage-panel close transition aborts any outstanding render so a hung fetch releases `renderTailRef` instead of wedging later sessions; +4 stash-verified tests.
- **Final adjudication (2026-08-06, owner):** the final `/codex:adversarial-review` re-reported the mint→dispatch non-atomicity as a no-ship HIGH — the SAME latest-link-wins residual this plan's non-goals accepted and Codex's own v4 delivery restated ("a separate explicit regeneration/reminder/send after this send's final mint can supersede it; closing that broader post-mint distributed case would require Option 2's atomic/versioned coordination"), and the third instance of the pattern Codex itself named "an inconsistency, not new evidence." Closing only the mint→dispatch window would not change the user-visible outcome (a later rotation kills a delivered link identically); the complete fix is the token-lifecycle redesign (per-suggestion lease/generation or multiple concurrently-valid tokens). **Owner decision: ship as-is with the residual documented; the token-lifecycle redesign is a separate, unscheduled follow-up whose founding requirement is that review's finding and test list.** The branch is strictly safer than production at every point (production has latest-link-wins PLUS preview-time rotation and no send-time verification).
