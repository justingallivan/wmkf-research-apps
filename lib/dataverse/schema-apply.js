...
async function ensureAttribute(client, entityLogicalName, attr, { bindAttempts = 8 } = {}) {
  ...
  const post = () => client.post(`/EntityDefinitions(LogicalName='${entityLogicalName}')/Attributes`, body);
  let result = await withMetadataRetry(post);
  // Binding to a just-created global option set can be refused with
  // 0x80048403 ("IsGlobal is not specified") until the metadata cache sees it.
  for (let i = 1; globalOptionSetId && i < bindAttempts && !result.ok && /0x80048403/.test(result.text || ''); i++) {
    await sleep(Math.min(2000 * i, 10000));
    result = await withMetadataRetry(post);
  }
