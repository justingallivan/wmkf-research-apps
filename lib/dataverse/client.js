...
// exist only on akoya_request; no deployed caller may write them.
const TEST_REQUEST_MARKER_FIELDS = new Set(['wmkf_istestrequest', 'wmkf_testcreationrunid']);
...
function createClient({ resourceUrl, token, solutionUniqueName, dryRun = false, allowTestRequestMarkerWrites = false }) {
  ...
  async function call(method, pathOrUrl, body, extraHeaders = {}, requestOptions = {}) {
    const urlNamesMarker = String(pathOrUrl).split('?')[0].split('/')
      .some((segment) => TEST_REQUEST_MARKER_FIELDS.has(segment.split('(')[0].toLowerCase()));
    if (method !== 'GET' && !allowTestRequestMarkerWrites && (urlNamesMarker || bodyNamesTestRequestMarker(body))) {
