...
function targetEntitySet(pathOrUrl) {
  // Same canonicalization as the marker path check (Fable final review of
  // 6c-i): the URL parser resolves dot segments, fragments and `\`, so
  // `/akoya_requests/../accounts` is classified as `accounts`, exactly as the
  // server will resolve it; the segments are decoded the same way.
  const segments = pathSegmentsForMarkerCheck(pathOrUrl)
    .filter((segment) => segment && segment !== 'api' && segment !== 'data' && segment !== 'v9.2');
  return (segments[0] || '').split('(')[0].toLowerCase();
}
...
// Canonicalize with the WHATWG URL parser FIRST (Codex 6c-i round 7): it
// drops the query and the fragment (a fragment is never transmitted, so
// `.../wmkf_istestrequest#x` reaches the server as the bare property path)
// and normalizes `\` to `/`. A relative path is resolved against a dummy
// origin; an unparsable value is treated as naming a marker (fail closed).
function canonicalPathname(pathOrUrl) {
  try {
    return new URL(String(pathOrUrl), 'https://marker-check.invalid/').pathname;
  } catch {
    return `/${[...TEST_REQUEST_MARKER_FIELDS][0]}`;
  }
}
function pathSegmentsForMarkerCheck(pathOrUrl) {
  return canonicalPathname(pathOrUrl).split(PATH_SEPARATORS)
