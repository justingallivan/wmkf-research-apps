...
const KNOWN_ENTITY_SETS = new Set([
  ...
  // Wave 23 Final Writeup reviewer acknowledgements. Production metadata and
  // the composite Final Document + Reviewer key were exact/Active 2026-08-31.
  'wmkf_finalwriteupreviewacknowledgements',
  'accounts',
  'wmkf_ai_runs',
  'wmkf_appuserpreferences',
  'wmkf_appgrantcycles',
  // Broad grant-program definitions used to configure program-specific Final
  // Writeup matrix audiences. Confirmed live set/fields in the Dataverse
  // taxonomy probe and request Atlas.
  'wmkf_grantprograms',
  'wmkf_portalmemberships',
  'wmkf_proposalbudgetlines',
  'wmkf_reviewquestions',
  // Standard activity sets used by the guarded Dynamics email transport. The
  // distribution recovery path also reads them by stable activity/attachment id.
  'emails',
  'activitymimeattachments',
  // Custom Site Visit Activity and its ActivityParty children. Live metadata
  // parity was re-probed in Production and sandbox on 2026-08-24.
  'wmkf_sitevisits',
  'activityparties',
  // Wave 28 Meeting Tracker. Sandbox metadata was read back exact on
  // 2026-09-09; runtime remains guarded by MEETING_TRACKER_SCHEMA_READY.
  'wmkf_deliberationsessions',
...
const SELECT = {
  ...
  wmkf_appreviewersuggestions: [
    ...
    'wmkf_remindersentat',
    'wmkf_remindercount',
    // Optional per-engagement operational deadline. Null falls back to the
    // request-level wmkf_reviewduedate; see lib/external/reviewer-due-date.js.
    ...
    // Postgres `reviewer_suggestions.summary_blob_url` 2026-05-12. Historical
    // only since 2026-09-06: analyze no longer extracts/uploads summaries and
    // generate-emails (its reader) is retired; kept selected so the
    // maintenance orphan-blob sweep keeps old blobs alive.
    'wmkf_summarybloburl',
    ...
    'wmkf_completedat',
    // Lead-PD reviewer closeout disposition. This does not authorize payment;
    // Operations/Finance owns the separate request-level remit flag.
