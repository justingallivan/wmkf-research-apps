...
/**
 * Whether a rehearsal grant covers this specific write. `$batch` is not
 * parsed in v1: a changeset write is allowed only when the grant's `ops`
 * explicitly includes `"BATCH"` (plan §3.3.2, coarse-but-never-silently-wider).
 * Creates (no URL-addressed record id) are constrained by entity set only —
 * there is no id yet to allowlist.
 */
function grantCoversWrite(grant, url, method) {
  const upperMethod = String(method || '').toUpperCase();
  const isBatch = /\/\$batch\/?$/.test((() => {
    try {
  ...
  if (isBatch) {
    return grant.ops.includes('BATCH');
  }
  ...
  if (upperMethod === 'POST') {
    // Creates: no id exists yet, constrained by entity set only.
    return true;
  }
...
function evaluatePolicy({ deployment, target, operation, url, method }) {
  ...
  if (target === 'production') {
    ...
    if (ack) {
      return { allowed: true, reason: `prod write ack: ${ack.purpose}` };
    }

    const grant = resolveRehearsalGrant();
    if (grant && grantCoversWrite(grant, url, method)) {
      return { allowed: true, reason: `rehearsal grant: ${grant.purpose}` };
    }
...
export function assertDataverseOperationAllowed({ url, method, callerLabel }) {
  ...
  const { allowed, reason } = evaluatePolicy({ deployment, target, operation, url, method });
  if (allowed) return;
