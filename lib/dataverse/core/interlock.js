 ...
 * other (plan §3.5).
 *
 * Stages 1–2 are shipped: runtime Dataverse HTTP seams call
 * `assertDataverseOperationAllowed`; the environment-controlled mode decides
 * whether those calls observe or enforce the policy (plan §3.5/§5).
 *
...
// Dataverse Search is a read API exposed as POST. Keep this exact pathname
// allowlisted so future search endpoints, actions, and path variants remain
// writes until they receive an explicit policy review.
const SEMANTIC_READ_PATH = '/api/search/v1.0/query';

/**
 * GET/HEAD are reads; the exact Dataverse Search query POST is also a read.
 * Every other method/path combination is a write.
 *
 * URL.pathname is the normalized path from the URL parser: query strings and
 * fragments are excluded, while encoded and nonexact path variants remain
 * distinct and therefore stay writes.
 */
function classifyOperation(url, method) {
  const m = String(method || 'GET').toUpperCase();
  if (m === 'GET' || m === 'HEAD') return 'read';
  if (m !== 'POST') return 'write';

  try {
    if (new URL(url).pathname === SEMANTIC_READ_PATH) return 'read';
  } catch {
    // Malformed URLs remain writes and will be denied by target classification.
  }
...
// Once-per-process observation gates. Module-level by design: the activation
// record proves that a non-off interlock mode is actually running without
// flooding logs on every Dataverse call; the ack announces itself once per
// running process. Reset only from tests.
let interlockActivationLogged = false;
...
export function _resetInterlockStateForTests() {
  interlockActivationLogged = false;
  prodWriteAckLogged = false;
}

/**
 * Emit one non-secret activation record per process after URL scoping. This
 * distinguishes a healthy zero-warning observation window from an interlock
 * that is effectively off. Deliberately excludes the URL, env value, request
 * data, and record identifiers.
 */
function logInterlockActivation({ mode, deployment, target, callerLabel }) {
  if (interlockActivationLogged) return;
  interlockActivationLogged = true;
  console.info(
    `[dataverse-interlock] active: mode=${mode} deployment=${deployment} ` +
    `target=${target} caller=${callerLabel || 'unknown'}`
  );
}
 ...
 * `warn` and `on`, the first inspected Dataverse call emits one non-secret
 * activation record per process; a write allowed via a §3.3 exception is
 * audit-logged (plan §3.3): the prod-write ack once per process, every
 * rehearsal-grant write every time.
 *
 * @param {Object} params
...
export function assertDataverseOperationAllowed({ url, method, callerLabel }) {
  ...
  const operation = classifyOperation(url, method);

  logInterlockActivation({ mode, deployment, target, callerLabel });
