 ...
 *
 * Stage 7 addendum: `runChangeset` also requires a trusted DAL context
 * (`assertTrustedDalContext`, same ALS-backed check `DynamicsService` entity
 * CRUD enforces) before composing/executing the batch. It is asserted HERE,
 * not only inside `DynamicsService.executeChangeset`, so a missing context is
 * attributed to the changeset call site with its own message — the
 * transport-level check still fires independently for any caller that
 * bypasses this helper and calls `executeChangeset` directly.
 */

import { DynamicsService } from '../../services/dynamics-service.js';
import { assertTrustedDalContext } from '../../services/dynamics-context.js';
import { entitySet } from './entity-registry.js';

/** @typedef {import('../../utils/guid.js').Guid} Guid */

/**
 * One DAL operation descriptor. `key` must be a branded {@link Guid}
 * (server-sourced, or client input narrowed through `isGuid`) — the transport
 * does NOT GUID-validate, so the type is the guard here. `keyPredicate` stays
 * a plain string: alternate-key escaping/allowlisting is the owning adapter's
 * responsibility (see header).
 *
 * @typedef {object} ChangesetOperation
 * @property {'POST'|'PATCH'|'DELETE'} method
 * @property {string} entitySet
 * @property {Guid} [key]
 * @property {string} [keyPredicate]
 ...
 *
 * @param {Array<ChangesetOperation>} operations
 * @returns {Array<{method:'POST'|'PATCH'|'DELETE', url:string, body?:object, ifMatch?:string}>}
 */
export function buildOperations(operations) {
  ...
  return operations.map((op) => {
    /** @type {{method:'POST'|'PATCH'|'DELETE', url:string, body?:object, ifMatch?:string}} */
 ...
 *
 * @param {Array<ChangesetOperation>} operations - DAL operation descriptors (see buildOperations).
 ...
 * @returns {Promise<{ok:true, operations:Array<object>}>}
 */
export function runChangeset(operations, options = {}) {
  assertTrustedDalContext('changeset.runChangeset');
