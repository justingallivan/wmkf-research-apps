 ...
 */
export async function aggregateMeetingDateCycles({ programIds } = {}) {
  DynamicsService.checkRestriction(
    DynamicsService.resolveLogicalName(ENTITY_SET),
    'akoya_requestid,wmkf_meetingdate,akoya_programid',
  );
  if (!Array.isArray(programIds) || !programIds.length || !programIds.every(isGuid)) {
    throw new Error('Meeting-date cycle aggregation requires valid program IDs');
  }
  const programValues = programIds.map((id) => `<value>${id}</value>`).join('');
  const fetchXml = '<fetch aggregate="true" count="5000" page="1"><entity name="akoya_request">'
    ...
    + `<condition attribute="akoya_programid" operator="in">${programValues}</condition></filter>`
    + '</entity></fetch>';
