...
import { API_TIMEOUT, MAX_EXPORT_RECORDS } from '../../services/dynamics/constants.js';
import { fetchWithTimeout } from '../../services/dynamics/http.js';
...
/** Minimal, concurrency-bearing projection for reviewer honorarium cancellation. */
export async function getHonorariumCancellationState(requestId) {
  return DynamicsService.getRecord(ENTITY_SET, requestId, {
    select: 'akoya_requestid,akoya_requeststatus,wmkf_authorizationtoremitpaymentflag,akoya_paid,wmkf_datewithdrawalreceived',
  });
}
 ...
 * @param {string} [opts.filter] caller-owned OData scope, ANDed with the whole ID group.
 ...
 *
 * Callers (2026-09-06): `lib/services/reviewer-request-authorization.js`,
 * `lib/services/workbench/request-search-service.js`,
 * `lib/services/initial-assessment/artifact-service.js`.
 */
export async function findByIds(requestIds, { select, top, filter } = {}) {
  if (!requestIds?.length) {
    return { records: [], count: 0, totalCount: 0, hasMore: false };
  }
  const idsFilter = odata.or(requestIds.map((id) => odata.eqRaw('akoya_requestid', id)));
  return DynamicsService.queryRecords(ENTITY_SET, {
    select: odata.select(select),
    filter: filter ? `(${idsFilter}) and (${filter})` : idsFilter,
...
/**
 * Record ceiling of `queryAllRequests`: past it the result is `capped` and the
 * tail is dropped. Exposed so services can warn as a scan approaches the cap
 * without importing the transport constants directly.
 */
export const QUERY_ALL_REQUESTS_CAP = MAX_EXPORT_RECORDS;
...
/**
 * Status options scoped by the broad Grant Program lookup. AggregateRecords
 * validates its aggregate/group fields, but the caller-owned filter field is
 * also a security boundary; check it explicitly before transport.
 */
export async function aggregateStatusesByGrantProgram(programId, { filter: extraFilter } = {}) {
  if (!isGuid(programId)) throw new Error('Grant Program status aggregation requires a valid program ID');
  DynamicsService.checkRestriction(
    DynamicsService.resolveLogicalName(ENTITY_SET),
    'akoya_requestid,akoya_requeststatus,wmkf_grantprogram',
  );
  return aggregateRequests({
    ...
    filter: extraFilter
      ? `(_wmkf_grantprogram_value eq ${programId}) and ${extraFilter}`
      : `_wmkf_grantprogram_value eq ${programId}`,
  });
}

/**
 * Aggregate meeting-date year/month groups in UTC for caller-owned program IDs.
 * The 5,000-group ceiling bounds the dropdown without inheriting the ordinary
 * 100-record read page size. Never return a successful but truncated timeline.
 */
export async function aggregateMeetingDateCycles({
  programIds, grantProgramIds, extraFetchXmlFilter = '', extraRestrictionFields = [],
} = {}) {
  const ids = Array.isArray(grantProgramIds) && grantProgramIds.length ? grantProgramIds : programIds;
  const field = Array.isArray(grantProgramIds) && grantProgramIds.length
    ? 'wmkf_grantprogram'
    : 'akoya_programid';
  DynamicsService.checkRestriction(
    DynamicsService.resolveLogicalName(ENTITY_SET),
    [`akoya_requestid,wmkf_meetingdate,${field}`, ...extraRestrictionFields].join(','),
  );
  if (!Array.isArray(ids) || !ids.length || !ids.every(isGuid)) {
    throw new Error('Meeting-date cycle aggregation requires valid program IDs');
  }
  const programValues = ids.map((id) => `<value>${id}</value>`).join('');
  const fetchXml = '<fetch aggregate="true" count="5000" page="1"><entity name="akoya_request">'
    + '<attribute name="wmkf_meetingdate" alias="year" groupby="true" dategrouping="year" usertimezone="false"/>'
    + '<attribute name="wmkf_meetingdate" alias="month" groupby="true" dategrouping="month" usertimezone="false"/>'
    + '<attribute name="akoya_requestid" alias="count" aggregate="count"/>'
    + '<order alias="year" descending="true"/><order alias="month" descending="true"/>'
    + '<filter><condition attribute="wmkf_meetingdate" operator="not-null"/>'
    + `<condition attribute="${field}" operator="in">${programValues}</condition></filter>`
    + extraFetchXmlFilter
    + '</entity></fetch>';
  const token = await DynamicsService.getAccessToken();
  const url = `${process.env.DYNAMICS_URL}/api/data/v9.2/${ENTITY_SET}?fetchXml=${encodeURIComponent(fetchXml)}`;
  const response = await fetchWithTimeout(url, {
    headers: {
      ...DynamicsService.buildHeaders(token),
      Prefer: 'odata.include-annotations="*",odata.maxpagesize=5000',
    },
  }, API_TIMEOUT);
  if (!response.ok) throw new Error(`Meeting-date cycle aggregation failed (${response.status})`);
  const body = await response.json();
  if (!Array.isArray(body.value)) throw new Error('Invalid meeting-date cycle aggregation response');
  if (body.value.length >= 5000 || body['@Microsoft.Dynamics.CRM.morerecords']
    || body['@odata.nextLink'] || body['@Microsoft.Dynamics.CRM.fetchxmlpagingcookie']) {
    throw new Error('Meeting-date cycle aggregation is incomplete; refusing partial cycle options');
  }
  return body.value.map((row) => DynamicsService.processAnnotations(row));
}
