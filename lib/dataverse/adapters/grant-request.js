...
import { MAX_EXPORT_RECORDS } from '../../services/dynamics/constants.js';
import * as odata from '../core/odata.js';
import { entitySet } from '../core/entity-registry.js';
import { assertDataverseOperationAllowed } from '../core/interlock.js';
...
/** Aggregate request counts by canonical meeting-date year/month. */
export async function aggregateMeetingDateCycles() {
  const fetchXml = '<fetch aggregate="true"><entity name="akoya_request">'
  ...
  assertDataverseOperationAllowed({ url, method: 'GET', callerLabel: 'grant-request/aggregateMeetingDateCycles' });
  const response = await fetch(url, { headers: DynamicsService.buildHeaders(token) });
