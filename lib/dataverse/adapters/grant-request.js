 ...
 * `lib/services/test-requests/spend-isolation.js`,
 * `lib/services/dynamics-explorer/tools/composite.js`.
 */
 ...
 */
export async function aggregateStatusesByGrantProgram(programId) {
  ...
  return aggregateRequests({
    ...
    filter: `_wmkf_grantprogram_value eq ${programId}`,
  });
}
 ...
 */
export async function aggregateMeetingDateCycles({ programIds, grantProgramIds } = {}) {
  ...
  DynamicsService.checkRestriction(
    DynamicsService.resolveLogicalName(ENTITY_SET),
    `akoya_requestid,wmkf_meetingdate,${field}`,
  );
