...
/**
 * Aggregate meeting-date year/month groups in UTC, complete or error.
...
export async function aggregateMeetingDateCycles() {
  DynamicsService.checkRestriction(
    ...
    'akoya_requestid,wmkf_meetingdate',
  );
  const fetchXml = '<fetch aggregate="true" count="5000" page="1"><entity name="akoya_request">'
    ...
    + '<filter><condition attribute="wmkf_meetingdate" operator="not-null"/></filter>'
    + '</entity></fetch>';
