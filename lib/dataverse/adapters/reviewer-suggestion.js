...
// addition over bulkUpdateByRequest (see docblock below).
 ...
 * call to the generic bulkUpdateByRequest (Stage 3K,
 ...
 * That whitelist is the one intentional addition over bulkUpdateByRequest;
 * everything else is EXACTLY what bulkUpdateByRequest does today, moved
 * verbatim by delegating to it: findByRequest(requestId, { selectedOnly:
 * true }) then a sequential, unconditional (no ifMatch) updateLifecycle per
 * row, with NO try/catch — a middle-row failure throws out of the loop, so
 * earlier rows stay written and the caller cannot tell which rows
 * committed or retry selectively. This is preserved behavior, not made
 * safer: decision D4 is an open owner decision ((a) per-row results
 * returned to the picker, (b) an atomic changeset, (c) per-row ifMatch, or
 * preserve as-is) that this slice deliberately does not resolve.
 * `bulkUpdateByRequest` stays a compatibility export until the census shows
 * no importer, then Stage 7 removes it.
 */
export async function setRequestMetadata(requestId, updates, { actingUserSystemId } = {}) {
  ...
  return bulkUpdateByRequest(requestId, updates, { actingUserSystemId });
}
