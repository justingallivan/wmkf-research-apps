...
const CLOSED_REVIEW_STATUS_VALUE_SET = new Set([
  REVIEW_STATUS_MAP.complete,
  ...TERMINAL_REVIEW_STATUS_VALUE_SET,
]);
...
const INVITATION_RESPONSE_SOURCE_STATUSES = new Set([
  null,
...
export async function updateLifecycle(id, updates, { actingUserSystemId, ifMatch } = {}) {
  ...
  if (writesInvitationResponse) {
    if (existing?.wmkf_completedat || CLOSED_REVIEW_STATUS_VALUE_SET.has(existing?.wmkf_reviewstatus)) {
    ...
    if (!INVITATION_RESPONSE_SOURCE_STATUSES.has(existing?.wmkf_reviewstatus)) {
  ...
  if (CLOSED_REVIEW_STATUS_VALUE_SET.has(existing?.wmkf_reviewstatus)
