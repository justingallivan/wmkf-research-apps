...
const ENGAGEMENT_STAMP_RESET_ENTRIES = Object.freeze([
  ...
  // softDelete preserves these columns, so without them a restore + re-send
  // leaves the prior engagement's `atSend` in place; the send path reads it as
  // already recorded and never stamps the new engagement's first deadline,
  // silently scoring the new engagement against an old deadline.
  ['wmkf_reviewduedateatsend', 'reviewDueDateAtSend', null],
  ['wmkf_reviewduedatelastsent', 'reviewDueDateLastSent', null],
]);
...
export async function updateLifecycle(id, updates, { actingUserSystemId, ifMatch } = {}) {
  ...
  const map = {
    ...
    reviewDueDateAtSend: 'wmkf_reviewduedateatsend',
    reviewDueDateLastSent: 'wmkf_reviewduedatelastsent',
