...
const ROLLUP_SELECT = '_wmkf_request_value,wmkf_invited,wmkf_accepted,wmkf_declined,wmkf_emailsentat,wmkf_responsetype,wmkf_reviewstatus';

/**
 * Selected, not-excluded suggestion rows across a chunk of request GUIDs, for
 * the lightweight reviewer-count rollup (no person/researcher fan-out). The
...
/**
 * Fields that make a suggestion row read as "invited" in the workbench. Cleared
 * when a SOFT-DELETED candidate is re-selected (manual re-add or the Removed-list
 * Restore) so a re-add is a genuine fresh start: the row returns "Not invited",
 * and a subsequent invitation mints a NEW live token (ensureToken re-mints because
 * softDelete revoked the old one; setExternalToken clears the revoke). Without this
 * the row resurrects showing the STALE emailSentAt as "Invited — awaiting response"
 * over a dead/revoked link, and the duplicate-invitation guard (wmkf_invited===true)
 * blocks any re-invite — so staff can never re-test/re-engage on the same request.
 * Applied ONLY to rows that were removed (wmkf_selected===false); re-adding an
 * already-active candidate must NOT wipe a legitimate live invitation. (S343)
 */
const INVITE_STAMP_RESET = {
  wmkf_invited: false,
...
export async function ensureStaffManualCandidate({
  ...
  if (existing) {
    const payload = {
      wmkf_sources: mergedSources,
      wmkf_selected: true,
      // Re-adding a REMOVED row is a fresh start — clear the stale invite stamps
      // so it returns invitable (see INVITE_STAMP_RESET). Never touch these when
      // re-adding an already-active row (would wipe a live invitation).
      ...(existing.wmkf_selected === false ? INVITE_STAMP_RESET : {}),
    };
    if (!existing.wmkf_suggestionlabel && suggestionLabel) payload.wmkf_suggestionlabel = suggestionLabel;
    if (!existing.wmkf_grantcyclecode && grantCycleCode) payload.wmkf_grantcyclecode = grantCycleCode;
    if (!existing.wmkf_programarea && programArea) payload.wmkf_programarea = normalizeSuggestionProgramArea(programArea);
  ...
  try {
    ...
    await DynamicsService.updateRecord(ENTITY_SET, now.wmkf_appreviewersuggestionid, {
      wmkf_sources: reMerged,
      wmkf_selected: true,
      // Fresh-start reset for a re-added removed row (matches the main path above).
      ...(now.wmkf_selected === false ? INVITE_STAMP_RESET : {}),
    }, { actingUserSystemId });
 ...
 *
 * Scope is deliberately `wmkf_selected=false AND wmkf_applicantdisposition=null`:
 ...
 *    excluded rows (the notExcluded concern) for free.
 *
...
export async function findRemovedByRequest(requestId) {
  ...
  const { records } = await DynamicsService.queryRecords(ENTITY_SET, {
    ...
    filter: `_wmkf_request_value eq ${requestId} and wmkf_selected eq false and wmkf_applicantdisposition eq null`,
...
/**
 * Reverse a softDelete — re-select a removed candidate so it returns to the
 * request's candidate list as a FRESH start. Flips `wmkf_selected` back and clears
 * the stale invite stamps (INVITE_STAMP_RESET) so the row reads "Not invited" and
 * can be invited again — a subsequent invitation mints a NEW live token (it does
 * NOT directly un-revoke the old link; setExternalToken clears the revoke at mint
 * time). Does not touch disposition. (S343: previously left `wmkf_invited` +
 * `wmkf_emailsentat` in place, so a re-added row resurfaced as "Invited — awaiting
 * response" over a dead link and could not be re-invited.)
 *
 * Scope-guarded to EXACTLY the rows findRemovedByRequest surfaces — `wmkf_selected
 * eq false AND wmkf_applicantdisposition eq null` — so an arbitrary suggestionId
 * can't be used to (a) re-select a row that was never X-removed or (b) promote an
 * applicant-recommended row (disposition=recommended, born selected=false) into the
 * pool, bypassing the explicit promote-applicant-reviewer path. A row that's already
 * selected is a harmless idempotent no-op; a non-null disposition throws.
 * Goes through updateLifecycle so the applicant-excluded fail-closed guard also holds.
 */
export async function restore(id, { actingUserSystemId } = {}) {
  ...
  const row = await DynamicsService.getRecord(ENTITY_SET, id, {
    select: 'wmkf_selected,wmkf_applicantdisposition',
  });
  if (row.wmkf_selected !== false) return; // already in the pool — nothing to restore
  if (row.wmkf_applicantdisposition !== null && row.wmkf_applicantdisposition !== undefined) {
  ...
  await updateLifecycle(id, {
    selected: true,
    // Fresh start — clear the stale invite stamps (see INVITE_STAMP_RESET). This
    // row is always a removed row here (the selected!==false guard above), so the
