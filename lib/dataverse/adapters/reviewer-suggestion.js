 ...
 * Full eligibility is deliberately re-checked from a fresh single-row read by
 * the filing service; this query only keeps the scheduled scan narrow.
 */
export async function findReviewDocxFilingCandidates({ cycleCode }) {
  return DynamicsService.queryAllRecords(ENTITY_SET, {
    ...
    filter: [
      ...
      `(wmkf_grantcyclecode eq null or ${odata.eq('wmkf_grantcyclecode', cycleCode)})`,
    ].join(' and '),
