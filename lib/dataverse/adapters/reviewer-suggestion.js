...
import { REVIEWER_ACTION_POLICY_SUGGESTION_FIELDS } from '../../utils/reviewer-action-policy-fields.js';

const ENTITY_SET = entitySet('wmkf_appreviewersuggestions');

const FIELD_SELECT = selectFields(ENTITY_SET);

const ACTION_POLICY_SELECT = Object.freeze([
...
/**
 * Engagement fields that survive softDelete and must be cleared when a removed
 * candidate is re-selected (manual re-add or Removed-list Restore). Applied ONLY
 * to rows that were removed (wmkf_selected===false); re-adding an already-active
 * candidate must NOT wipe a legitimate live invitation or submitted-review state.
 */
...
const ENGAGEMENT_STAMP_LIFECYCLE_RESET = Object.freeze(Object.fromEntries(
  ENGAGEMENT_STAMP_RESET_ENTRIES.map(([, lifecycleKey, value]) => [lifecycleKey, value]),
));
...
/**
 * Inert C0.4 Stage-A projection for the future action-policy reader. It keeps
 * the same applicant-excluded refusal as the existing action chokepoint while
 * avoiding a premature widening of every primary suggestion read.
 */
export async function findActionPolicyById(id) {
  if (!id) throw new Error('reviewer-suggestion.findActionPolicyById: id required');
  const row = await DynamicsService.getRecord(ENTITY_SET, id, { select: odata.select(ACTION_POLICY_SELECT) });
  if (isExcluded(row)) {
    throw new Error(`reviewer-suggestion.findActionPolicyById: refusing to act on an applicant-excluded suggestion (${id})`);
  }
  return row;
}
 ...
 * stale engagement stamps (ENGAGEMENT_STAMP_RESET) so the row reads as a new
...
export async function restore(id, { actingUserSystemId } = {}) {
  ...
  await updateLifecycle(id, {
    selected: true,
    // Fresh start. This row is always a removed row here (the selected!==false
    // guard above), so the reset is unconditional.
    ...ENGAGEMENT_STAMP_LIFECYCLE_RESET,
