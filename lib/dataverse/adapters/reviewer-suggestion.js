...
// `wmkf_programarea` is capped at 100 chars in Dataverse. The value flows from the
// LLM-extracted `analysis.proposalInfo.programArea` (free text the model is asked to
// keep to a short label but doesn't always — e.g. req 1002916 produced a >100-char
// string, hard-failing the save with a 400). Clamp defensively at the write boundary
// so an over-long value degrades to a truncated label instead of blocking the save.
export const PROGRAM_AREA_MAX_LENGTH = 100;
export function clampProgramArea(value) {
  if (typeof value !== 'string') return value;
  const trimmed = value.trim();
  return trimmed.length > PROGRAM_AREA_MAX_LENGTH ? trimmed.slice(0, PROGRAM_AREA_MAX_LENGTH) : trimmed;
}
...
export async function findAllByPotentialReviewer(potentialReviewerId) {
  ...
  const { records } = await DynamicsService.queryAllRecords(ENTITY_SET, {
    select: MERGE_PREDICATE_SELECT.join(','),
...
export async function upsert({
  ...
  const incoming = pruneEmpty({
    ...
    wmkf_programarea: clampProgramArea(programArea),
...
export async function ensureApplicantRecommended({
  ...
  if (existing) {
    ...
    if (!existing.wmkf_programarea && programArea) payload.wmkf_programarea = clampProgramArea(programArea);
...
export async function findRemovedByRequest(requestId) {
  ...
  const { records } = await DynamicsService.queryRecords(ENTITY_SET, {
    select: `${FIELD_SELECT.join(',')},modifiedon`,
 ...
 * request's candidate list. The inverse of softDelete's `wmkf_selected` flip and
 * nothing more: it does NOT un-revoke a previously-revoked magic link (a fresh
 * invite mints a new token) and does not touch disposition or invite stamps.
 *
...
export async function restore(id, { actingUserSystemId } = {}) {
  ...
  await updateLifecycle(id, { selected: true }, { actingUserSystemId, ifMatch: row._etag });
}
