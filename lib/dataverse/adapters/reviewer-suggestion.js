...
import { isPersonSynthetic } from './potential-reviewer.js';
...
/**
 * Binding-seam fence (6c-ii Stage A, plan item (b)): every suggestion-creating
 * op reads the target person's marker and refuses a marker-true person BEFORE
 * any write, a second, independent fence alongside the read-side exclusions in
 * potential-reviewer.js. `isPersonSynthetic` returns false unconditionally
 * while the isolation switch is off (no host can have a synthetic person
 * without the wave30 column), so this is a no-op read until the schema apply.
 */
async function assertPersonBindable(potentialReviewerId) {
  if (await isPersonSynthetic(potentialReviewerId)) {
    throw adapterError('This potential reviewer is a synthetic (factory-created) test record and cannot be bound to a suggestion.', {
      code: 'synthetic_reviewer_not_bindable',
      status: 409,
      details: { potentialReviewerId },
    });
  }
}
...
const MERGE_PREDICATE_SELECT = [
  ...
  'wmkf_reviewduedateextensiongrantedat',
...
export async function repointToPotentialReviewer(suggestionId, keeperId, { actingUserSystemId, ifMatch } = {}) {
  ...
  await assertPersonBindable(keeperId);
  await DynamicsService.updateRecord(
    ENTITY_SET,
    suggestionId,
...
export async function upsert({
  ...
  await assertPersonBindable(potentialReviewerId);
...
const ENGAGEMENT_STAMP_RESET_ENTRIES = Object.freeze([
  ...
  ['wmkf_reviewduedateextensiongrantedat', 'reviewDueDateExtensionGrantedAt', null],
 ...
 * reviewers complete-stamp, my-candidates token mint), so it
 ...
 * the linked request, check the applicant-excluded chokepoint, and enforce
 * lifecycle eligibility before a direct mintAndStore call. Byte-mirrors
 * regenerate-token.js's inline getRecord call; no bypassDynamicsRestrictions
 * here, that wrapper stays at the route (Stage 7 removes it later). The
 * Dynamics read supplies the concurrency-critical `@odata.etag` as `_etag`;
 * it is intentionally not included in `$select` because it is an OData
 * annotation. Callers must require that concrete value before minting.
 */
...
// The former `patchFields` alias of `patchReviewReceipt` (generic PATCH
// passthrough) was removed 2026-09-06 under owner decision D2 together with
// its last caller, the retired generate-emails generation mark. The Stage 7
// boundary gate still lists the name in GENERIC_WRITERS so a re-added export
// or importer fails CI.
 ...
 * patchReviewReceipt — same DynamicsService.updateRecord shape,
 ...
 * Byte-identical transport call to patchReviewReceipt; the sweep
...
/**
 * Narrow, ETag-required PATCH for the pending-invitation expiry sweep
 * (Stage 7 census row 6, `reviewer-engagement/expire-invitation.js`).
 * Byte-identical transport call to patchReviewReceipt (the same
 * `DynamicsService.updateRecord` shape with the same two fields the sweep's
 * former inline generic-passthrough call wrote). Requiring a concrete ETag here is
 * CODIFICATION, not tightening: the sweep already fails closed before this
 * call when the row has no usable ETag (see expire-invitation.js's
 * `_etag`/`isConcreteEtag`-shaped guards), so this op never actually
 * receives a wildcard or missing ifMatch in production — it only makes that
 * existing guarantee an enforced contract of the op itself.
 */
export async function expireInvitationResponse(suggestionId, nowIso, { ifMatch, ...opts } = {}) {
  requireIfMatch(ifMatch, 'expireInvitationResponse');
  return DynamicsService.updateRecord(ENTITY_SET, suggestionId, {
 ...
 * (picklist mapping likewise n/a). D3 (owner decision 2026-09-06, Stage 7
 * plan): a concrete ETag is REQUIRED, matching the other narrow ops above —
 * a missing/wildcard version throws `missing_version` before any Dataverse
 * call instead of falling through to updateLifecycle's unconditional write
 * for this non-status payload.
 */
export async function deselectLegacyDeclinedSuggestion(suggestionId, { ifMatch, ...opts } = {}) {
  requireIfMatch(ifMatch, 'deselectLegacyDeclinedSuggestion');
...
async function findAccepted({ systemuserid = null, cycleCode, programId } = {}) {
  const { meetingDateToCycleCode, cycleCodeToOdataFilter } = await import('../../utils/cycle-code.js');

  const requestFilters = [];
  if (systemuserid) requestFilters.push(`_wmkf_programdirector_value eq ${systemuserid}`);
  if (programId) {
    if (!isGuid(programId)) throw new Error('reviewer-suggestion.findAccepted: invalid programId');
    requestFilters.push(`_wmkf_grantprogram_value eq ${String(programId).trim().toLowerCase()}`);
  }
  if (cycleCode) {
 ...
 */
export async function findAcceptedByPD(systemuserid, { cycleCode, programId } = {}) {
  if (!systemuserid) return { suggestions: [], requestById: {} };
  return findAccepted({ systemuserid, cycleCode, programId });
}
 ...
 * all-history scan.
 */
export async function findAcceptedByCycle(cycleCode, { programId } = {}) {
  if (!cycleCode) return { suggestions: [], requestById: {} };
  return findAccepted({ cycleCode, programId });
}
...
export async function updateLifecycle(id, updates, { actingUserSystemId, ifMatch } = {}) {
  ...
  const map = {
    ...
    reviewDueDateExtensionGrantedAt: 'wmkf_reviewduedateextensiongrantedat',
  ...
  for (const [k, v] of Object.entries(updates || {})) {
    ...
    if (k === 'responseType') payload[map[k]] = mapPicklist(RESPONSE_TYPE_MAP, v, 'responseType');
      ...
      payload[map[k]] = v;
    } else if (k === 'reviewDueDateExtensionGrantedAt') {
      if (v !== null && (typeof v !== 'string' || Number.isNaN(new Date(v).getTime()))) {
        throw new Error('reviewer-suggestion: reviewDueDateExtensionGrantedAt must be an ISO timestamp or null');
      }
      payload[map[k]] = v;
  ...
  if (writesInvitationResponse && !isConcreteEtag(effectiveIfMatch)) {
...
/**
 * Release an accepted reviewer because the Foundation has enough reviews.
 *
 * The reviewer ledger and any still-open linked honorarium request move in one
 * ETag-guarded changeset. The honorarium is retained for audit history and
 * marked Withdrawn; it is never deleted by this command.
 */
export async function applyStaffReviewerRelease(
  id,
  {
    ifMatch,
    actingUserSystemId,
  ...
  if (!id) throw new Error('applyStaffReviewerRelease: id required');
  if (!ifMatch) throw new Error('applyStaffReviewerRelease: ifMatch required');

  const now = releasedAt || new Date().toISOString();
  const payload = {
    wmkf_reviewstatus: REVIEW_STATUS_MAP.released,
    wmkf_externaltokenrevoked: true,
    wmkf_withdrawnsufficientat: now,
  };
  if (notes !== undefined) payload.wmkf_notes = notes || null;
  ...
  if (cancelHonorarium) {
    if (!cancelHonorarium.id || !cancelHonorarium.ifMatch) {
      throw new Error('applyStaffReviewerRelease: cancelHonorarium id and ifMatch required');
    }
    operations.push({
      method: 'PATCH',
      entitySet: 'akoya_requests',
      key: cancelHonorarium.id,
      body: {
        akoya_requeststatus: 'Withdrawn',
        wmkf_datewithdrawalreceived: now,
      },
      ifMatch: cancelHonorarium.ifMatch,
    });
  }
...
/**
 * Race compensation for an acceptance worker that linked an honorarium after
 * a Foundation release committed. The retained request is marked Withdrawn in
 * the same changeset as an ETag-guarded no-op assertion of the released row.
 */
export async function withdrawLinkedHonorariumForReleasedSuggestion(
  id,
  honorariumRequestId,
  { ifMatch, honorariumIfMatch, actingUserSystemId, withdrawnAt } = {},
) {
  if (!id) throw new Error('withdrawLinkedHonorariumForReleasedSuggestion: id required');
  if (!honorariumRequestId) {
    throw new Error('withdrawLinkedHonorariumForReleasedSuggestion: honorariumRequestId required');
  }
  if (!ifMatch || !honorariumIfMatch) {
    throw new Error('withdrawLinkedHonorariumForReleasedSuggestion: both ETags required');
  }
  await runChangeset([
    {
      method: 'PATCH',
    ...
    },
    {
      ...
      body: {
        akoya_requeststatus: 'Withdrawn',
        wmkf_datewithdrawalreceived: withdrawnAt || new Date().toISOString(),
      },
      ifMatch: honorariumIfMatch,
    },
  ], { actingUserSystemId });
}
...
export async function softDelete(id, { actingUserSystemId, alsoRevokeToken = false } = {}) {
  ...
  //
  // D0 (owner decision 2026-09-06, Stage 7 plan): the guard is row-aware, not
  // status-only — a completion-stamped row (`wmkf_completedat`) whose status
  // was never advanced to a closed value is still a closed engagement and
  // must not be un-picked either. Legacy/hand-edited rows are the only ones
  // this widening can refuse; every in-app closeout sets both fields together.
  const existing = await DynamicsService.getRecord(ENTITY_SET, id, {
    select: 'wmkf_reviewstatus,wmkf_completedat',
  });
  if (isClosedEngagementRow(existing)) {
    throw new Error(
      `reviewer-suggestion.softDelete: refusing to remove suggestion ${id} in a closed review status `
      + `(status ${existing.wmkf_reviewstatus ?? 'null'}, completedAt ${existing.wmkf_completedat ?? 'null'}); `
      + 'a closed engagement is durable history, not a candidate to un-pick',
    );
  }
  const payload = {
    wmkf_selected: false,
...
// `setRequestMetadata` (Stage 3K's whitelisted proposal-wide cycle/program-area
// fan-out, itself the renamed generic per-request bulk op) was removed 2026-09-06 under
// owner decision D4: no client had sent the proposal-wide PATCH since the
// standalone Reviewer Finder page was retired 2026-06-16, and its sequential
// unguarded per-row loop could leave a request half-updated with no way to tell
// which rows committed. Cycle/program area are set at save time
// (save-candidates → createSuggestion) and have no post-save correction path.
