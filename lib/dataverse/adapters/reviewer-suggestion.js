 ...
 * `disposition=recommended` and `applicant` in its sources (and `selected=true`
 * on first creation), on equal footing with Claude-discovered candidates for
 * later enrichment.
 *
 * Behavior:
 ...
 *  - **Never resurrects a staff removal:** `wmkf_selected=true` is set only on
 *    CREATE. On an existing row the update LEAVES `wmkf_selected` untouched, so a
 *    staff soft-delete (`softDelete` → `selected=false`, e.g. my-candidates DELETE)
 *    is NOT flipped back to selected by the next lazy ingestion. Staff curation
 *    wins (Codex stop-time review, S210).
 ...
 *   `selected` reflects the row's live curation state (false ⇒ staff removed it).
 */
export async function ensureApplicantRecommended({
  ...
  try {
    ...
    // Only a duplicate / alternate-key CONFLICT means a concurrent run won the
    // (person,request) race — converge to an update so the call stays
    // idempotent. Any other create failure (validation, privilege, throttling,
    // transient) is a real error and must surface, not be masked as success
    // (Codex post-impl review, S210).
    const isConflict = err?.status === 412 || err?.status === 409
      || /duplicate|already exists|alternate key/i.test(err?.message || '');
