 ...
 * sources are unioned with `staff_manual`; applicant recommendation state is
 * preserved when present so a row can carry both origins.
 *
 * @returns {Promise<{ id: string, created: boolean, selected: boolean, skippedExcluded?: boolean }>}
 */
export async function ensureStaffManualCandidate({
  ...
  const mergedSources = Array.from(new Set([...splitSources(existing?.wmkf_sources), 'staff_manual'])).join(',');
