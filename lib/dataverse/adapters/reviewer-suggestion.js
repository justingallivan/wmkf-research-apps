 ...
 *
 * Reviewer Lifecycle Stage 7 (7C) named the expiry sweep's use as
 * `expireInvitationResponse` instead. The sole remaining caller is
 * `reviewer-engagement/record-invitation.js`'s `markInvitationGenerated`
 * (the legacy generation mark-as-sent, raw fields, no `ifMatch`) — preserved
 * verbatim pending owner decision D2
 * (docs/REVIEWER_LIFECYCLE_STAGE7_BUILD_PLAN.md). This alias stays until
 * that call site is itself named or the decision resolves otherwise.
 */
 ...
 * Byte-identical transport call to patchReviewReceipt/patchFields (the same
 * `DynamicsService.updateRecord` shape with the same two fields the sweep's
 * inline `patchFields` call wrote). Requiring a concrete ETag here is
