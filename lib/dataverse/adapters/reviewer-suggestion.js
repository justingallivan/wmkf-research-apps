 ...
 * `disposition=recommended`, `selected=true`, and `applicant` in its sources,
 * on equal footing with Claude-discovered candidates for later enrichment.
 *
 * Behavior:
 ...
 *
 * @returns {Promise<{ id: string, created: boolean, skippedExcluded?: boolean }>}
 */
export async function ensureApplicantRecommended({
  ...
  try {
    ...
    if (isExcluded(now)) {
      return { id: now.wmkf_appreviewersuggestionid, created: false, skippedExcluded: true };
    }
    ...
    return { id: now.wmkf_appreviewersuggestionid, created: false };
  }
}
