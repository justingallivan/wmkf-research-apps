 ...
 * payload so the guards (excluded-row refusal, closed-status checks),
 * picklist mapping (n/a here) and optimistic-lock fallback stay byte-identical
 * to the caller's pre-3J behavior — this op does not reimplement the
 * transport call. D3: optional version preserved pending owner decision —
 * `ifMatch` is forwarded as supplied (including undefined), not required;
 * this is not a statement that it should stay optional forever.
 */
