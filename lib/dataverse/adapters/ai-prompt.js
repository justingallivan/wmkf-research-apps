 ...
 * leaf does not wrap `withDalContext(...)`, the caller must
...
export function create(fields, opts) {
  // Two-arg call when no opts supplied — byte-mirror of both live callers
  // (admin publish and seeder pass no third arg; Codex S329 wave review).
  if (opts === undefined) {
    return DynamicsService.createRecord(ENTITY_SET, fields);
  }
