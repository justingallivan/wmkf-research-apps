...
import {
  createRecord as writeCoreCreateRecord,
  updateRecord as writeCoreUpdateRecord,
} from '../../services/dynamics/write-core.js';
...
export const POST_PRESENTATION_SELECT_FIELDS = Object.freeze([
  'wmkf_externalurl',
  'wmkf_slotversion',
]);
...
export function requestDocumentSelect({
  ...
  const includePresentation = includePostPresentation ?? isPostPresentationMaterialsSchemaReady();
  return [
    ...
    ...(includePresentation ? POST_PRESENTATION_SELECT_FIELDS : []),
  ].join(',');
}
...
export const REQUEST_DOCUMENT_SELECT = requestDocumentSelect({
  ...
  includePostPresentation: false,
});

const IMMUTABLE_EXPLICIT_ORIGIN_FIELDS = new Set([
  'wmkf_initiatedat',
  'wmkf_initiatedby@odata.bind',
  '_wmkf_initiatedby_value',
]);

function assertNoExplicitOriginFields(value, operation) {
  const forbidden = Object.keys(value || {}).filter((key) => (
    IMMUTABLE_EXPLICIT_ORIGIN_FIELDS.has(String(key).toLowerCase())
  ));
  if (forbidden.length) {
    throw new Error(`request-document.${operation}: explicit origin actor fields are server-owned and immutable`);
  }
}
...
export async function create(payload, options = {}) {
  assertNoExplicitOriginFields(payload, 'create');
  const {
    actorPolicy,
    actorContext,
    // Optional sandbox-bound Dataverse service (the Test Request Factory
    // ia-sandbox-deps shape: getAccessToken/buildHeaders/_withCallerId/
    // _writeFetch/processAnnotations + an explicit `baseUrl`). Production
    // callers never pass this, so `svc` is undefined and the call falls
    // through to the module-singleton DynamicsService exactly as before —
    // default-preserving, same seam style as Stage A's `dependencies` param.
    svc,
    ...writeOptions
  } = options || {};
  ...
  try {
    created = svc
      ? await writeCoreCreateRecord(svc, ENTITY_SET, body, writeOptions)
      : await DynamicsService.createRecord(ENTITY_SET, body, writeOptions);
  } catch (error) {
    // A create may commit while its response is lost. For the approved
    // availability-first paths, repair the operational evidence from the
    // generation-key row before preserving the original error for the caller's
    // existing duplicate/recovery logic.
    if (isActorNotCaptured(resolution) && payload?.wmkf_generationkey) {
      try {
        const observed = await findByGenerationKey(payload.wmkf_generationkey);
        const row = observed?.records?.length === 1 ? observed.records[0] : null;
        if (row?.wmkf_requestdocumentid
          && !row.wmkf_initiatedat
          && !row._wmkf_initiatedby_value) {
        ...
        console.warn(
          '[request-document] lost-response actor evidence recovery failed:',
  ...
  if (isActorNotCaptured(resolution)) {
    await recordRequestDocumentActorNotCaptured({
      payload,
      created,
      context: actorContext,
...
export async function update(id, patch, options) {
  assertNoExplicitOriginFields(patch, 'update');
  const { svc, ...writeOptions } = options || {};
  return svc
    ? writeCoreUpdateRecord(svc, ENTITY_SET, id, patch, writeOptions)
    : DynamicsService.updateRecord(ENTITY_SET, id, patch, writeOptions);
}
