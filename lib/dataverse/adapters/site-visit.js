 ...
 * cycle view only needs the scheduled date to derive stage/visit, not attendees.
 * Chunks the OR filter at 25 ids to stay well under Dataverse URL/filter limits.
 */
export async function findActiveByRequests(requestIds) {
  ...
  for (let index = 0; index < ids.length; index += MULTI_REQUEST_CHUNK) {
    ...
    const result = await DynamicsService.queryAllRecords(ENTITY_SET, {
      select: MULTI_REQUEST_SELECT,
      filter: odata.and([`(${regardingFilter})`, `(${activeFilter})`]),
    });
    rows.push(...(result.records || result || []));
  }
  return rows;
}
