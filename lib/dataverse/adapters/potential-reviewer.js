...
import {
  ...
  withSyntheticReviewerMarkerSelect,
  withExcludeSyntheticReviewerODataFilter,
  isSyntheticReviewerRow,
} from '../../services/test-requests/isolation.js';
...
/**
 * Narrow marker-only read for the binding-seam fence (6c-ii Stage A, plan item
 * (b)): every suggestion-creating adapter op reads the target person's marker
 * before any write and refuses a marker-true person. While the switch is off
 * the column cannot exist on any host, so every person is ordinary and this
 * never reads the column, matching the read-side fences in this module. A
 * missing person is left for the caller's own write to surface (not found),
 * not classified here.
 */
export async function isPersonSynthetic(id) {
  if (!id || !syntheticReviewerIsolationEnabled()) return false;
  // M8 (Opus round 1): fail closed on EVERY read failure, including a 404.
  // A transient error or a missing-column 400 must not be swallowed into
  // "not synthetic" (that would silently pass a person whose marker could
  // not actually be read); a 404 must also refuse the bind rather than
  // pass, since a person that doesn't exist is not a safe "ordinary" default
  // either -- the caller's write would fail anyway, but less clearly, and a
  // missing person must never be treated as equivalent to a verified
  // ordinary one. No error is caught or downgraded here.
  const row = await DynamicsService.getRecord(ENTITY_SET, id, {
    select: odata.select(['wmkf_potentialreviewersid', SYNTHETIC_REVIEWER_MARKER_FIELDS.marker]),
  });
  return isSyntheticReviewerRow(row);
}
...
// Read-side isolation fence (6c-ii Stage A, plan "Read-side fan-out of the
// marker" (a)): ordinary identity resolution must never see a synthetic
// person. Both the OData filter and a post-fetch JS filter exclude
// marker-true rows so the two paths agree even if a caller post-filters the
// same result set again downstream.
function excludeSyntheticReviewerRows(rows) {
  if (!syntheticReviewerIsolationEnabled()) return rows || [];
  return (rows || []).filter((row) => !isSyntheticReviewerRow(row));
}
...
export async function findByEmailCandidates(email) {
  ...
  const { records } = await DynamicsService.queryRecords(ENTITY_SET, {
    select: odata.select(withSyntheticReviewerMarkerSelect(FIELD_SELECT)),
    filter: withExcludeSyntheticReviewerODataFilter(odata.eq('wmkf_emailaddress', norm)),
    top: 2,
  });
  const matches = excludeSyntheticReviewerRows(
    (records || []).filter((r) => normalizeEmail(r.wmkf_emailaddress) === norm),
  );
...
export async function findByOrcidCandidates(orcid) {
  ...
  const { records } = await DynamicsService.queryRecords(ENTITY_SET, {
    select: odata.select(withSyntheticReviewerMarkerSelect(FIELD_SELECT)),
    filter: withExcludeSyntheticReviewerODataFilter(odata.eq('wmkf_orcid', norm.id)),
    top: 2,
  });
  const matches = excludeSyntheticReviewerRows(
    (records || []).filter((r) => normalizeOrcid(r.wmkf_orcid).id === norm.id),
  );
  return candidateResult(matches, 'wmkf_potentialreviewersid');
}
...
/**
 * Factory-only lookup (6c-ii Stage A, plan item (c)): a marker/active/
 * Contact-link-FILTERED exact alternate-key read on the normalized email --
 * an inactive, Contact-linked, or non-synthetic row at this address never
 * satisfies the filter and is not returned.
 *
 * As of Stage B round 2 (P2-4, Opus round 1), this is NOT what the reviews
 * recipe's reservation-time resolver calls. That resolver needs to
 * distinguish "no row at this address" from "a row exists but is not an
 * eligible synthetic person" (a REAL reviewer's address must refuse
 * reservation, not silently read as "not found" and be handed a fresh
 * preallocated GUID) -- a filtered lookup collapses both cases to null and
 * cannot make that distinction. So `lib/services/test-requests/
 * reviews-sandbox-deps.js` implements its own UNFILTERED
 * `findAnyPersonByEmail` (returns any matching row, filter applied by the
 * caller) instead of calling this function, and `scripts/
 * rehearse-test-request-sandbox.mjs` applies the marker/active/Contact-link
 * check itself against that unfiltered result.
 *
 * This function currently has no production importer (kept for a future
 * caller that genuinely wants the filtered semantics, e.g. a read-only
 * display path). No ordinary path may import it; see
 * `tests/unit/potential-reviewer-synthetic-lookup-boundary.test.js`, which
 * greps `lib/`, `pages/`, and `scripts/` for importers and pins the
 * allowlist to the definition file itself.
 */
export async function findSyntheticByEmail(email) {
  // P3 (Opus round 1): this factory-only lookup has no carve-out from the
  // isolation switch -- with it off, no host can have a synthetic person, so
  // a call here is a caller defect, not a legitimate "nobody found" miss.
  // Fail closed with a clear message rather than silently querying (and
  // filtering to null) or, worse, selecting a column that may not exist.
  if (!syntheticReviewerIsolationEnabled()) {
    throw adapterError('findSyntheticByEmail requires SYNTHETIC_REVIEWER_ISOLATION to be on.', {
  ...
  if (!norm) return null;
  const { records } = await DynamicsService.queryRecords(ENTITY_SET, {
    select: odata.select([...FIELD_SELECT, SYNTHETIC_REVIEWER_MARKER_FIELDS.marker]),
    filter: odata.eq('wmkf_emailaddress', norm),
    top: 2,
  });
  const matches = (records || []).filter((r) =>
    normalizeEmail(r.wmkf_emailaddress) === norm
    && isSyntheticReviewerRow(r)
    && (r.statecode === undefined || r.statecode === 0)
    && !r._wmkf_contact_value);
...
export async function searchByName(name, { top = 5 } = {}) {
  ...
  const select = odata.select(withSyntheticReviewerMarkerSelect(FIELD_SELECT));
  ...
  for (const rawFilter of filters) {
    const filter = withExcludeSyntheticReviewerODataFilter(rawFilter);
    const { records } = await DynamicsService.queryRecords(ENTITY_SET, { select, filter, top: cap });
    for (const row of excludeSyntheticReviewerRows(records)) {
  ...
  const fallback = await DynamicsService.queryRecords(ENTITY_SET, {
    select,
    filter: withExcludeSyntheticReviewerODataFilter(
      odata.contains('wmkf_name', ContactParser.stripHonorifics(name || '').trim()),
    ),
    top: cap,
  });
  ranked = rankNameRows(excludeSyntheticReviewerRows(fallback.records), name, cap);
  return ranked;
}
...
export async function upsertByEmail({ name, email, emailSource, addressTrustStateJson, affiliation, expertise, whyChosen }, options = {}) {
  ...
  async function reuse(row) {
    // (d) Read-side isolation fence: an ordinary save can never merge into a
    // synthetic person, so a Reviewer Finder save using a throwaway address
    // cannot silently adopt a factory-created row. Refused before any PATCH.
    if (isSyntheticReviewerRow(row)) {
      throw adapterError('This email address belongs to a synthetic (factory-created) reviewer and cannot be reused.', {
...
export async function getByIdForMerge(id) {
  ...
  // Marker added to the read only while the switch is on (see
  // withSyntheticReviewerMarkerSelect): MERGE_FIELD_SELECT itself is left
  // unchanged (a static export a test pins with arrayContaining) because
  // production has no wave30 column yet and an unconditional $select would
  // 400 every merge read. reviewer-merge.js's fence reads this projection.
  return DynamicsService.getRecord(ENTITY_SET, id, {
    select: odata.select(withSyntheticReviewerMarkerSelect(MERGE_FIELD_SELECT)),
  });
}
