...
import { rankNameRows as sharedRankNameRows } from '../utils/rank-name-rows.js';
...
export async function findByContactId(contactId) {
  // GUID-guard the unquoted lookup filter (OData injection defense) — a non-GUID
  // can't match a real link anyway, so return null rather than build the query.
  if (!contactId || !GUID_RE.test(String(contactId))) return null;
  const { records } = await DynamicsService.queryRecords(ENTITY_SET, {
    select: FIELD_SELECT.join(','),
  ...
  const rows = records || [];
  if (rows.length > 1) {
    // A contact should map to at most one reviewer row. >1 is a data-integrity
    // violation the collision check downstream assumes away — surface it loudly
    // rather than silently drop the extras.
    console.warn(`findByContactId: contact ${contactId} is linked to ${rows.length} reviewer rows (expected <=1)`);
  }
  return rows[0] || null;
}
...
function rankNameRows(rows, name, top) {
  return sharedRankNameRows(rows, name, top, displayName);
}
...
export async function setContactLink(potentialReviewerId, contactId, { actingUserSystemId } = {}) {
  ...
  if (!GUID_RE.test(String(contactId))) throw new Error('potential-reviewer.setContactLink: contactId must be a GUID');
