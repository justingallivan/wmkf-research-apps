...
/**
 * Factory-only lookup (6c-ii Stage A, plan item (c)): the reservation-time
 * reuse check for a previously-seeded synthetic person. Exact alternate-key
 * read on the normalized email that REQUIRES marker true, active statecode,
 * and no Contact link -- an inactive, Contact-linked, or non-synthetic row at
 * this address never satisfies the reservation reuse condition and is not
 * returned. No ordinary path may import this function; see
 * `tests/unit/potential-reviewer-synthetic-lookup-boundary.test.js`, which
 * greps `lib/` and `pages/` for importers and pins the allowlist to
 * `lib/services/reviewer-engagement/seed-synthetic-review.js` (the Stage
 * 6c-ii-ii seeder module).
 */
