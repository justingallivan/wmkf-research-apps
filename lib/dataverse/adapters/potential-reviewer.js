...
import { REVIEWER_ACTION_POLICY_PERSON_FIELDS } from '../../utils/reviewer-action-policy-fields.js';
...
/**
 * Inert C0.4 Stage-A projection for the future action-policy reader.
 * No runtime caller uses this method yet; keeping it specialized avoids making
 * non-authoritative Wave 13 fields part of every potential-reviewer read.
 */
export async function getActionPolicyById(id) {
  if (!id) throw new Error('potential-reviewer.getActionPolicyById: id required');
  return DynamicsService.getRecord(ENTITY_SET, id, { select: odata.select(ACTION_POLICY_SELECT) });
}
