...
/**
 * Resolve one server-derived staff identity with its Dataverse team
 * memberships. Persona consumers match only pinned team GUIDs; team names are
 * returned for diagnostics and never grant a persona by themselves.
 */
export async function getByIdWithTeams(id) {
  return DynamicsService.getRecord(ENTITY_SET, id, {
...
/**
 * List the enabled users holding one exact security role. The role-backed
 * roster is the complete Final Writeup acknowledgement audience, independent
 * of persona-team rollout.
 */
export async function listEnabledBySecurityRoleName(roleName, { top = 50 } = {}) {
  const safeTop = Math.max(1, Math.min(Number(top) || 50, 100));
  return DynamicsService.queryRecords(ENTITY_SET, {
    select: 'systemuserid,fullname,isdisabled',
    filter: 'isdisabled eq false and '
      + `systemuserroles_association/any(role:role/name eq '${odata.escape(roleName)}')`,
