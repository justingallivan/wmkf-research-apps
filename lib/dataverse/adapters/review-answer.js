...
export async function fetchAnswersBySuggestion(suggestionIds) {
  ...
  for (const chunk of chunked(suggestionIds, CHUNK)) {
    ...
    for (const a of records) {
      ...
      if (!sid) continue;
      const parsedValues = parseStoredAnswerValues(a.wmkf_answervalues);
      const answerValuesUnreadable = parsedValues.answerValuesUnreadable
        || (a.wmkf_questiontype === 'multiselect' && parsedValues.answerValues === null);
      (out[sid] ||= []).push({
