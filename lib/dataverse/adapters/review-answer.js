 ...
 * Post-Phase-D this snapshot is the system of record for reviewer ratings,
 * categorical multiselect answers, and narrative answers.
 *
 * This adapter owns the current read/write projection while retaining the
 * alternate-key and rating-read semantics established by two earlier modules:
...
// The canonical snapshot rating keys, in display order. Mirrors
...
/**
 * Parse historical multiselect snapshot JSON without allowing one corrupt row
 * to fail the complete review DTO.
 */
export function parseStoredAnswerValues(raw) {
  if (raw === null || raw === undefined || raw === '') {
    return { answerValues: null, answerValuesUnreadable: false };
  }
  let parsed;
  try {
    parsed = typeof raw === 'string' ? JSON.parse(raw) : raw;
  } catch {
    return { answerValues: null, answerValuesUnreadable: true };
  }
  ...
  return {
    answerValues: parsed.map((pair) => ({ value: pair.value, label: pair.label })),
    answerValuesUnreadable: false,
  };
}

/**
 * Parse the complete categorical option snapshot. Null is a valid legacy row
 * (the column did not exist before wave 25); malformed non-null JSON is kept
 * distinct so document renderers never present a partial option list as exact.
 */
export function parseStoredQuestionOptions(raw) {
  if (raw === null || raw === undefined || raw === '') {
    return { questionOptions: null, questionOptionsUnreadable: false };
  }
  let parsed;
  try {
    parsed = typeof raw === 'string' ? JSON.parse(raw) : raw;
  } catch {
    return { questionOptions: null, questionOptionsUnreadable: true };
  }
  ...
  return {
    questionOptions: parsed.map((pair) => ({ value: pair.value, label: pair.label })),
...
export async function fetchAnswersBySuggestion(suggestionIds) {
  ...
  for (const chunk of chunked(suggestionIds, CHUNK)) {
    ...
    for (const a of records) {
      ...
      if (!sid) continue;
      // `wmkf_answervalues` is meaningful only for multiselect snapshots. Ignore
      // stray/manual values on every other row type so corrupt categorical
      // storage cannot suppress an otherwise valid numeric rating or narrative.
      ...
      const isCategorical = a.wmkf_questiontype === 'picklist' || isMultiselect;
      const parsedOptions = isCategorical
        ? parseStoredQuestionOptions(a.wmkf_questionoptions)
      ...
      (out[sid] ||= []).push({
        answerId: a.wmkf_appreviewanswerid || null,
        ...
        answerText: answerValuesUnreadable ? 'Unreadable answer' : (a.wmkf_answertext || ''),
        answerValue: a.wmkf_answervalue ?? null,
        answerValues: parsedValues.answerValues,
        answerValuesUnreadable,
        questionOptions: parsedOptions.questionOptions,
        questionOptionsUnreadable: parsedOptions.questionOptionsUnreadable,
      });
    }
  }
 ...
 * @returns {Promise<{riskLevel:number|null, overallAssessment:number|null}>}
 */
...
export function answerRowBody(row) {
  return {
    ...
    wmkf_questionoptions: row.questionOptions === null || row.questionOptions === undefined
      ? null
      : JSON.stringify(row.questionOptions),
  };
}
