...
const DIRECTORY_SEARCH_MAX_ROWS = CONTACT_BATCH_MAX_IDS + 1;
...
/**
 * Search Contacts for a user-facing directory picker.
 *
 * Unlike searchByName, this intentionally accepts partial first/last names and
 * does not apply identity-resolution matching after Dataverse returns rows.
 * The query is bounded and always completes in one collection request.
 */
export async function searchDirectoryByName(name, { top = 10 } = {}) {
  const cleanName = ContactParser.stripHonorifics(name || '').trim().replace(/\s+/g, ' ');
  const { firstName, lastName } = splitName(cleanName);
  if (!firstName && !lastName) return [];

  // Permit one row beyond the curated-directory cap so callers can disclose
  // truncation without making a second Dataverse request.
  const cap = Math.min(Math.max(Number(top) || 10, 1), DIRECTORY_SEARCH_MAX_ROWS);
  const nameFilters = firstName && lastName
    ? [
      odata.startsWith('fullname', cleanName),
      `(${odata.and([
        odata.startsWith('firstname', firstName),
        odata.startsWith('lastname', lastName),
      ])})`,
    ]
    : [
      odata.startsWith('lastname', lastName),
      odata.startsWith('firstname', lastName),
    ];

  const { records } = await DynamicsService.queryRecords(ENTITY_SET, {
    select: odata.select(FIELD_SELECT),
    filter: `(${odata.or(nameFilters)})`,
