...
function rankNameRows(rows, name, top) {
  return sharedRankNameRows(rows, name, top, contactName);
}
...
export async function searchByName(name, { top = 5 } = {}) {
  ...
  if (lastName && firstName) {
    ...
    // Contacts (unlike reviewers) carry a maintained `fullname`, so an exact
    // fullname match is a cheap extra structured variant before the broad
    // contains() fallback. The reviewer adapter has no equivalent maintained
    // composite field, hence it omits this variant.
