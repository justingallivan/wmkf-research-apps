...
export async function upsertByPotentialReviewer(potentialReviewerId, {
  ...
  // Update an already-existing sidecar: metrics always overwrite (snapshots);
  // other fields fill empty only (preserve staff edits).
  ...
  // Race-safe: the sidecar is 1:1 with the potential reviewer (alternate key on
  // wmkf_potentialreviewer). A concurrent create (double-click / second session)
  // loses the read-then-create gap above; catch the duplicate, re-query, and
  // update instead of leaving a duplicate sidecar. Mirrors the documented race
  // handling in reviewer-suggestion.js ensureApplicantRecommended.
  try {
    ...
    const dup = status === 412 || status === 409 ||
      /duplicate|already exists|0x80040237|0x80060892/i.test(err?.message || '');
    if (!dup) throw err;
    const raced = await getByPotentialReviewer(potentialReviewerId);
    if (!raced) throw err; // genuinely couldn't create and nothing to update — surface it
