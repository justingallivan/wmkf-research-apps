...
export async function upsertByPotentialReviewer(potentialReviewerId, {
  ...
  // address was downgraded to confirm-before-send could still read HIGH from an
...
export async function writeIdentityDecision(potentialReviewerId, decision, { actingUserSystemId } = {}) {
  ...
  // Sticky-'confirmed' guard: a non-'confirmed' verdict must NOT silently
  // downgrade a stored 'confirmed' (which would also trip clearIdentityFields and
  // wipe an attested ORCID). 'confirmed' was DESIGNED as a reserved human-attestation
  // sentinel. NOTE (S235, still-open discrepancy): the automated OpenAlex/ORCID spine
  // now ALSO emits 'confirmed' (reviewer-identity-resolver.js `classifySpineEvidence`,
  // ~L261 & L279), so a stored 'confirmed' is no longer provably human. The sticky-skip
  // below still protects a stored 'confirmed' regardless of source; whether the spine's
  // 'confirmed' should be downgraded to 'probable' (or the sentinel model changed) is an
  // unresolved design question — see docs/REVIEWER_ORCID_BACKPROPAGATION_DESIGN.md §14 and
  // .claude-memory/project-reviewer-self-report-orcid-sticky-confirmed.md before relying on
  // "automated = never confirmed". Skip the overwrite unless the incoming decision is itself
  // 'confirmed' (NB: since the spine can now emit it, an automated 'confirmed' also takes
  // this branch and overwrites a stored 'confirmed' — part of the same open question).
...
export async function clearIdentityFields(potentialReviewerId, fields, { actingUserSystemId } = {}) {
  ...
  // Never clear a 'confirmed' record — same sticky guard as writeIdentityDecision.
  // ('confirmed' was designed as a human-attestation sentinel; NB the automated spine
  // now also emits it — see writeIdentityDecision above and
  // .claude-memory/project-reviewer-self-report-orcid-sticky-confirmed.md for the open
  // reconciliation question.) Fail CLOSED (Codex S217 #1): let a read error propagate
  // rather than clear a record whose status we couldn't verify.
  const cur = await DynamicsService.getRecord(ENTITY_SET, potentialReviewerId, {
    select: 'wmkf_potentialreviewersid,wmkf_identitystatus',
  });
  if (cur?.wmkf_identitystatus === 'confirmed') return;
