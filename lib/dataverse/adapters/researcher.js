...
// wmkf_primaryaffiliation has a hard 500-char Dynamics cap; a longer value 400s
// the WHOLE write (observed prod: req 1002833, "Hongjun Song" — an enrichment/
// OpenAlex affiliation can be a long multi-institution string). Clamp at the write
// boundary so a messy affiliation never blocks the candidate save. Mirrors the
// wmkf_primaryaffiliation:500 entry in potential-reviewer.js FIELD_MAX so BOTH
// writers to this column stay under cap. Only this column has a known cap here —
// no speculative caps (they would silently truncate legitimate values).
const PRIMARY_AFFILIATION_MAX = 500;
function clampAffiliation(v) {
  if (typeof v === 'string' && v.length > PRIMARY_AFFILIATION_MAX) {
    return v.slice(0, PRIMARY_AFFILIATION_MAX - 1).trimEnd() + '…';
  }
  return v;
}
...
export async function upsertByPotentialReviewer(potentialReviewerId, {
  ...
  const fillIfEmpty = pruneEmpty({
    ...
    wmkf_primaryaffiliation: clampAffiliation(affiliation),
  ...
  // A manually staff-entered/promoted email source is an explicit assertion and
  // must be AUTHORITATIVE — overwrite any stale value (the other descriptive
  // fields stay fill-only to preserve staff edits). Without this, promoting a
  // contact lead onto a person whose row already carried a high-confidence source
  // would leave that stale source in place, and the invite gate would read the
  // address as high-confidence instead of `manual` (low → confirm-before-send).
  // Only manual paths (save-candidates promotion, manual-reviewer) ever pass
  // 'manual'; enrichment never does. (Codex contact-leads Slice-4 MED; spec §7.)
  if (emailSource === 'manual') merge.wmkf_emailsource = 'manual';
...
export async function updateById(id, updates, { actingUserSystemId } = {}) {
  ...
  for (const [k, v] of Object.entries(updates || {})) {
    if (k in map && v !== undefined) {
      payload[map[k]] = map[k] === 'wmkf_primaryaffiliation' ? clampAffiliation(v) : v;
...
export async function writeIdentityDecision(potentialReviewerId, decision, { actingUserSystemId } = {}) {
  ...
  // 'confirmed' is RESERVED for human attestation — the resolver
  // (reviewer-identity-resolver) only ever emits probable/unresolved/ambiguous,
  // never confirmed. So a stored 'confirmed' means a person self-reported or staff
  // manually attested their ORCID, and an automated resolver verdict must NOT
  // silently downgrade it (which would also trip clearIdentityFields and wipe the
  // attested ORCID). Skip the overwrite unless the incoming decision is itself
  // 'confirmed' (a fresh human attestation legitimately replaces the prior one).
  if (decision.status !== 'confirmed') {
    // Fail CLOSED (Codex S217 #1): the read MUST succeed to safely downgrade.
    // If it throws (transient/403), let it propagate — callers handle identity
    // writes per-row — rather than fall through and risk wiping a 'confirmed'
    // attestation we couldn't verify. (Self-reports take the incoming-confirmed
    // branch above and skip this read entirely.)
...
export async function clearIdentityFields(potentialReviewerId, fields, { actingUserSystemId } = {}) {
  ...
  // Never clear a human-attested ('confirmed') record — same sticky invariant as
  // writeIdentityDecision: the resolver never emits 'confirmed', so a confirmed
  // status is a self-report/manual attestation the resolver must not wipe. Fail
  // CLOSED (Codex S217 #1): let a read error propagate rather than clear a record
  // whose status we couldn't verify.
  const cur = await DynamicsService.getRecord(ENTITY_SET, potentialReviewerId, {
    select: 'wmkf_potentialreviewersid,wmkf_identitystatus',
  });
  if (cur?.wmkf_identitystatus === 'confirmed') return;
