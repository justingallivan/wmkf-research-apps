...
export async function upsertByPotentialReviewer(potentialReviewerId, {
  ...
  const scholarlySourceUpgrade = (
    emailSource === 'scholarly_multi'
    && sameStoredEmail
    && !!existing?._etag
    && !['orcid', 'institution_page', 'scholarly_multi'].includes(storedSource)
  );
  if (scholarlySourceUpgrade) {
    merge.wmkf_emailsource = emailSource;
  }

  await DynamicsService.updateRecord(ENTITY_SET, potentialReviewerId, merge, {
    actingUserSystemId,
    ...(scholarlySourceUpgrade ? { ifMatch: existing._etag } : {}),
  });
