...
export function validateReviewSubmission(input, questionSet = reviewFormSchema.fields) {
  ...
  for (const field of questionSet) {
    ...
    if (field.type === 'string') {
      ...
      try {
        normalized[field.key] = canonicalizeMultiselectSelection(field, isMissing ? [] : raw);
      } catch (error) {
 ...
 *       questionOptions: Array<{value:number,label:string}>|null }.
...
export function buildReviewSubmission(normalized, { receivedAt, questionSet = reviewFormSchema.fields } = {}) {
  ...
  for (const field of questions) {
    const row = {
      ...
      answerValue: null,
      answerValues: null,
      questionOptions: (field.type === 'picklist' || field.type === 'multiselect')
