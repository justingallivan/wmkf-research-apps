...
import { reviewFormSchema, reviewParentColumnByKey, labelForOption } from './review-form-schema';
import { isEffectivelyEmptyHtml, htmlToPlainText } from './sanitize-review-html';

// Parent-bound ratings for a given question set: picklist fields whose key has a
// parent column in the code-side dual-write map. In the canonical set this is
// the 3 ratings; a future staff-added rating (no parent column) is snapshot-only
// and is NOT asserted against a parent column.
function ratingKeysFor(fields) {
  return fields
    .filter((f) => f.type === 'picklist' && reviewParentColumnByKey[f.key])
    .map((f) => f.key);
}
...
export function buildReviewSubmission(normalized, { receivedAt, questionSet = reviewFormSchema.fields } = {}) {
  ...
  // Snapshot rows are the ANSWER questions (picklist + richtext), in display
  ...
  const questions = questionSet
    .filter((f) => f.type === 'picklist' || f.type === 'richtext')
  ...
  for (const field of questions) {
    ...
    if (field.type === 'picklist') {
      ...
      // only for canonical, parent-bound ratings; snapshot-only ratings skip it.
      const column = reviewParentColumnByKey[field.key];
      if (column) parentPatch[column] = value;
    } else if (field.type === 'richtext') {
  ...
  assertRatingInvariants(parentPatch, answerRows, questionSet);

  return { parentPatch, answerRows };
}

/**
 * Enforce the parent/child rating contract (Codex P1-N4 / P1-R3) for the
 * canonical, parent-bound ratings of the given question set:
 *   - every parent-bound rating row is non-null,
 *   - each rating value is in the live picklist domain,
 *   - each rating row's answerValue equals its parent column.
 * (A snapshot-only rating — picklist with no parent column — is not asserted
 * against a parent column; that path is exercised once staff add ratings.)
 */
function assertRatingInvariants(parentPatch, answerRows, fields) {
  ...
  if (present.length !== ratingKeys.length) {
    throw new Error(
      `buildReviewSubmission: a complete submit needs exactly ${ratingKeys.length} parent-bound rating rows, got ${present.length}.`,
    );
  }
  ...
  for (const row of ratingRows) {
    ...
    const column = reviewParentColumnByKey[row.questionKey];
    if (parentPatch[column] !== row.answerValue) {
      throw new Error(
        `buildReviewSubmission: parent/child rating drift for "${row.questionKey}" `
        + `(parent ${parentPatch[column]} vs child ${row.answerValue}).`,
      );
    }
  }
}
