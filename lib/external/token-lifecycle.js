 ...
 * @param {string} [args.ifMatch] - Optional ETag binding for the token write
 * @param {Object} [args.writeFields] - Fields to persist atomically with the token
 ...
 *   into an email body.
 */
export async function mintAndStore({ suggestionId, requestId, expiresAt, ops = DEFAULT_OPS, actingUserSystemId, ifMatch, writeFields } = {}) {
  ...
  await setExternalToken(suggestionId, { hash, expiresAt }, { actingUserSystemId, ifMatch, writeFields });
...
export async function ensureToken(suggestionId, opts = {}) {
  ...
  if (opts.expiresAt instanceof Date) {
    ...
    // The request date refines expiry but is not required to mint. Preserve the
    // established now+90-day fallback when this supplemental read is transiently
    // unavailable so an accepted reviewer never loses auto-mint altogether.
    const request = await getRequestById(requestId, { select: 'wmkf_reviewduedate' })
      .catch((error) => {
        console.error(`[token-lifecycle] request due-date read failed for ${requestId}; using fallback TTL: ${error.message}`);
        return null;
      });
