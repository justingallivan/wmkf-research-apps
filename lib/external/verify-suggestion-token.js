...
const SUGGESTION_SELECT = [
  ...
  // Rating columns are no longer SELECTed — context.js reads the
...
export async function verifySuggestionToken(jwt) {
  ...
  // S333 Stage 4b: trust-model tightening — all six real callers (the
  // external/review/[token]/{context,proposal,upload,draft,respond,submit}.js
  // routes) now establish the trusted context themselves around the whole
  // verifySuggestionToken() call (label 'external-token-verify' relocated,
  // byte-identical).
  let suggestion;
  try {
