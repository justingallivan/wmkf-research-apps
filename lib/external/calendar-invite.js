...
/**
 * @param {object} p
 * @param {string} [p.reviewDueDate] - effective 'YYYY-MM-DD' review due date
 ...
 */
function buildReviewDueIcs({ reviewDueDate, suggestionId, nowIso } = {}) {
  ...
  const description = escapeText(
    'Review due date for the Keck Foundation proposal. Please submit your review by this date.',
  );
...
/**
 * Build the exact informational Site Visit add-to-calendar attachment.
 *
 * METHOD:PUBLISH intentionally carries no ATTENDEE properties: it is a view-
 * only event attachment, not an RFC 5546 scheduling request and not an RSVP or
 * update/cancellation channel.
 */
function buildSiteVisitIcs({
  activityId,
  startIso,
  endIso,
  subject,
  ...
  const uidToken = String(activityId || '').trim().toLowerCase();
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(uidToken)) {
    throw new Error('Site Visit calendar activityId must be a GUID.');
  }
  const start = toIcsUtc(startIso);
  const end = toIcsUtc(endIso);
  if (!start || !end || Date.parse(endIso) <= Date.parse(startIso)) {
    throw new Error('Site Visit calendar requires an exact increasing UTC range.');
  }
  const organizer = String(organizerEmail || '').trim().toLowerCase();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(organizer)) {
    throw new Error('Site Visit calendar organizer must have a valid email address.');
  }
  const summary = String(subject || '').trim();
  if (!summary) throw new Error('Site Visit calendar subject is required.');
  const stamp = toIcsUtc(nowIso || new Date().toISOString());
  const content = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//W. M. Keck Foundation//Site Visit//EN',
    'METHOD:PUBLISH',
    'CALSCALE:GREGORIAN',
    'BEGIN:VEVENT',
    `UID:wmkf-site-visit-${uidToken}@wmkf`,
    `DTSTAMP:${stamp}`,
    `DTSTART:${start}`,
    `DTEND:${end}`,
    'SEQUENCE:0',
  ...
  return {
    filename: 'wmkf-site-visit.ics',
    contentType: 'text/calendar; method=PUBLISH; charset=utf-8',
    content: Buffer.from(content, 'utf-8'),
  };
}

module.exports = { buildReviewDueIcs, buildSiteVisitIcs, toIcsDate };
