...
// Degraded-limiter alerting. When Postgres is unreachable the limiter fails
// open (requests are allowed un-limited) — which is the right availability
// tradeoff but silently disables rate limiting. Track consecutive limiter DB
// failures in process memory and raise a deduplicated alert once they cross a
// threshold, so a sustained outage becomes visible rather than silent.
const DB_FAILURE_ALERT_THRESHOLD = 5;
const DB_FAILURE_ALERT_KEY = 'external-rate-limit-db-degraded';
...
/**
 * Record a limiter Postgres failure. Increments the consecutive-failure
 * counter; on crossing the threshold, raises a deduplicated `system_alerts`
 * entry (best-effort — the alert write itself may fail if Postgres is the
 * thing that is down, but the structured log line always survives) and resets
 * the counter so the alert re-arms after each further run of failures.
 */
async function noteLimiterDbFailure(context, err) {
  consecutiveDbFailures += 1;
  console.error(`[external rate-limit] ${context} error:`, err.message);
  ...
  try {
    console.error(JSON.stringify({
      level: 'error',
      event: 'external_rate_limit.db_degraded',
    ...
    await AlertService.createAlert({
      type: 'security',
      severity: 'warning',
      title: 'External-reviewer rate limiter degraded',
      message:
        `The external-reviewer rate limiter has failed ${failureCount}+ ` +
        `consecutive Postgres operations and is failing open — requests to ` +
        `/api/external/review/[token]/* are currently NOT rate-limited.`,
    ...
    console.error('[external rate-limit] degraded-limiter alert raise failed:', alertErr.message);
  }
}
...
export async function checkRateLimit(req, token) {
  ...
  try {
    ...
    // but track the failure so a sustained outage raises a visible alert.
    await noteLimiterDbFailure('checkRateLimit', err);
    return { ok: true };
  }
}
...
export async function recordTokenOutcome(req, token, valid) {
  ...
  try {
    ...
    await noteLimiterDbFailure('recordTokenOutcome', err);
    return;
  }
