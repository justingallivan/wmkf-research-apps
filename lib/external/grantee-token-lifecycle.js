 ...
 * `akoya_request`, with lifecycle/image metadata on a related
 * `wmkf_granteedeliverable` row, so the token's `sub` IS the akoya_request GUID.
 * The compensating guard for "no revocation" lives on the submit route: it
 * refuses to write once the related deliverable status is Complete, so a leaked
 * or stale link cannot overwrite a finalized package.
 *
