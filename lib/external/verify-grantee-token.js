...
export async function verifyGranteeToken(jwt) {
  ...
  // S333 Stage 4b: trust-model tightening — both real callers
  // (external/grantee/[token]/context.js and .../submit.js) now establish
  // the trusted context themselves around the whole verifyGranteeToken()
  // call (label 'grantee-token-verify' relocated, byte-identical).
  let request;
  let deliverable;
  try {
    request = await grantRequestAdapter.getById(requestId, { select: REQUEST_SELECT });
    deliverable = await getDeliverableForRequest(requestId);
  } catch (e) {
