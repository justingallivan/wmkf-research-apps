...
function assertNoReviewerLinkContent(value, field = 'content') {
  if (REVIEWER_LINK_CONTENT.test(String(value || ''))) {
    throw new Error(`Review-due reminder ${field} cannot contain a reviewer URL or {{externalLink}}.`);
  }
}

function withoutReviewLinkDirections(bodyText) {
  const text = String(bodyText || '');
  assertNoReviewerLinkContent(text, 'body');
...
/**
 * Honorarium line in the thank-you body. The stored template carries the
 * `{{honorariumNote}}` token; it resolves to this sentence unless the reviewer
 * explicitly declined the honorarium at Stage 2a accept (`wmkf_honorariumoptout`
 * strictly `true`). `false`, `null`, and an unselected field all keep the line —
 * the same "not opted out unless true" rule the acceptance drain applies when it
 * decides whether to mint the honorarium. An omitted line leaves no blank
 * paragraph because the surrounding blank lines collapse in
 * stripLegacyAutomationMarker.
 */
export const HONORARIUM_NOTE_TEXT = 'We will be in touch regarding the processing of your honorarium.';

export function resolveHonorariumNote(honorariumOptOut) {
  return honorariumOptOut === true ? '' : HONORARIUM_NOTE_TEXT;
}
 ...
 * {{greeting}} / {{proposalTitle}} / {{honorariumNote}} / {{signature}} (plus
 * legacy [..] variants).
 */
export function buildThankYouBodyText({ bodyTemplate, reviewerName, title, signatureBlock, honorariumOptOut }) {
  const name = ContactParser.normalizeDisplayName(reviewerName) || 'Reviewer';
  const greeting = buildReviewerGreeting(reviewerName);
  ...
  return stripLegacyAutomationMarker(applyPlaceholders(bodyTemplate, {
    ...
    '{{honorariumNote}}': resolveHonorariumNote(honorariumOptOut),
    '[Program Director signature]': signature,
    '{{signature}}': signature,
  }));
}
 ...
 */
export function renderThankYou({ subjectTemplate, bodyTemplate, reviewerName, title, signatureBlock, senderName, senderEmail, honorariumOptOut }) {
  const proposalTitle = title || 'your assigned proposal';
  ...
  return {
    ...
    html: renderAutomatedEmailNoticeHtml({
      ...
      kind: 'message',
    }) + renderBodyParagraphsHtml(buildThankYouBodyText({ bodyTemplate, reviewerName, title, signatureBlock, honorariumOptOut })),
  };
}
...
export function renderReviewDueReminder(args) {
  assertNoReviewerLinkContent(args.subjectTemplate, 'subject');
  return {
    subject: args.subjectTemplate,
