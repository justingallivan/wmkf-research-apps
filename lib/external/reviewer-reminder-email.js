...
/**
 * Backward-compatible removal for the retired `{{honorariumNote}}` token. Live
 * Dataverse templates can retain the old token until an administrator republishes
 * them; resolving it to blank prevents thank-you delivery from implying that the
 * human closeout or payment-eligibility decision has occurred.
 */
export function resolveHonorariumNote() {
  return '';
}
 ...
 * The retired {{honorariumNote}} token is still stripped for stored-template
 * compatibility.
 */
export function buildThankYouBodyText({ bodyTemplate, reviewerName, title, signatureBlock }) {
  ...
  return stripLegacyAutomationMarker(applyPlaceholders(bodyTemplate, {
    ...
    '{{honorariumNote}}': resolveHonorariumNote(),
    '[Program Director signature]': signature,
    '{{signature}}': signature,
  }));
}
