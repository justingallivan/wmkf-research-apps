 ...
 */
export function renderThankYou({ subjectTemplate, bodyTemplate, reviewerName, title, signatureBlock }) {
  const proposalTitle = title || 'your assigned proposal';
  ...
  return {
    subject,
    html: renderBodyParagraphsHtml(buildThankYouBodyText({ bodyTemplate, reviewerName, title, signatureBlock })),
  };
}
 ...
 */
export function renderRespondReminderFromBodyText({ subject, bodyText, url }) {
  return {
    subject,
    html: renderReviewerReminderHtml({
      bodyText,
