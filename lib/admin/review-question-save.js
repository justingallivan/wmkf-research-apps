...
import { reviewParentColumnByKey } from '../external/review-form-schema';
...
// The four parent-bound keys (affiliation + the 3 ratings) dual-write to columns
// on wmkf_appreviewersuggestion via reviewParentColumnByKey. Until Phase E
// retires those columns, removing one of these keys silently stops its
// dual-write while readers still expect it — so the set must always contain all
// four (Codex Phase C P1-3). Derived from the binding map, not hardcoded.
export const PARENT_BOUND_KEYS = Object.freeze(Object.keys(reviewParentColumnByKey));
