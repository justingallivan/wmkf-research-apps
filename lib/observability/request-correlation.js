 ...
 *
 * Browser-import safety: `node:async_hooks` is never required at the top
 * level — only lazily, inside a try/catch, via a variable module-name path,
 * so a runtime without async_hooks (or one where it's disallowed) degrades
 * gracefully instead of crashing at import time. This does NOT hide the
 * require from Turbopack: Turbopack statically resolves even variable-path
 * `require(modName)` calls to a bundled module id (verified in .next/server
 * chunk output for this repo), so browser-bundle safety rests on
 * REACHABILITY, not on the require being invisible to a static tracer — no
 * client bundle currently reaches this module (verified by a scan of
 * .next/static for its markers). See lib/dataverse/client.js:22-29 for the
 * same pattern and the same correction applied to fs/path.
...
function classifyOutcome({ response, error }) {
  ...
  if (error) {
    ...
    // Raw (untransformed) errors from lib/dataverse/client.js never get
    // service-error.js's causeKind tagging, so mirror its name/code
    // classification exactly (lib/utils/service-error.js:88-89) here — a
    // structured causeKind of 'socket'/'dns'/'unknown' still wins over a
    // coincidentally-matching name/code, since this branch only runs when
    // causeKind is absent.
    if (error.causeKind == null) {
      if (error.name === 'AbortError') return { outcome: 'timeout' };
      // Node's fetch rejects network failures as TypeError('fetch failed')
      // with the real undici error (carrying the code) on .cause — the code
      // is NOT top-level (probed 2026-08-15, Opus delta review NEW-3). One
      // level of cause unwrap covers that real shape; structured errors never
      // reach this branch (buildNoResponseError always sets causeKind), so
      // the plan's structured mapping ('unknown' ⇒ network_error) is intact.
...
function emitDependencyEvent({ url, method, ms, response, error } = {}) {
  try {
    ...
    // Plan export validator requires finite ms >= 0; a clock-stepped negative
    // diff is meaningless — clamp at the envelope owner, not the call sites.
    // Non-numbers (e.g. a BigInt, which the pinned test expects to still
    // swallow the whole event via JSON.stringify throwing) pass through
    // unchanged rather than being coerced.
    const msValue = typeof ms === 'number' ? (Number.isFinite(ms) ? Math.max(0, ms) : 0) : ms;
    eventObject.ms = msValue;
