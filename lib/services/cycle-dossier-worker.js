...
// smoke: rId13–rId15 added), and its in-place rewrite leaves System.IO.Packaging
// garbage slots ([trash]/0000.dat …) that hold no document content (observed on
// the restored 2026-09-12 upload). PDFs are stored verbatim. Compare OOXML by its
// decompressed document parts, with relationship parts normalised to an
// order-independent set that ignores customXml links.
const SHAREPOINT_OWNED_PART = /^(docProps\/|customXml\/|\[trash\]\/|_rels\/\.rels$|\[Content_Types\]\.xml$)/;
...
/**
 * Abort an in-flight paid call or Graph write when the operator stop fires
 * WHILE a single call is in flight (429/529 retry backoff, or a multi-segment
 * Graph folder create) — `assertDossierWorkerOpen()` is otherwise only
 * checked between steps. This is the operator stop ONLY: a run's own Pause/
 * Cancel must not abort an in-flight paid call (see `checkpointResult`'s
 * comment on retaining in-flight results after Pause), so this never polls
 * run status, only `assertDossierWorkerOpen()`.
 */
function operatorStopSignal({ intervalMs = 10000 } = {}) {
  ...
  let timer = setInterval(async () => {
    try {
      ...
      // Only a real operator stop aborts the in-flight call. A transient
      // control-row read failure must not discard a paid call; the next
      // between-step check still observes the stop.
...
async function publishConstituent(run, item, format, { signal = null } = {}) {
  ...
  const createdFolder = await GraphService.ensureFolderPath(current.library, folder, { siteId: current.siteId, driveId: current.driveId, signal });
...
// Plain-language failure copy for the run item. Names the stage that was in
// flight and the cause when it is one we can state safely; anything else
// stays generic so raw provider or stack text never reaches the page.
const STAGE_LABELS = { research: 'literature research', entry: 'briefing', render: 'document rendering', publish: 'SharePoint publishing' };
function stageInFlight(item) {
  if (!item.researchRef) return 'research';
  if (!item.payloadRef) return 'entry';
  if (!item.files) return 'render';
  return 'publish';
}
export function describeEntryFailure(error, item) {
  const stage = STAGE_LABELS[stageInFlight(item)];
  const message = String(error?.message || '');
  const timeout = message.match(/timeout after (\d+)ms/i);
  let cause;
  if (timeout) cause = `The ${stage} stage's AI call timed out after ${Math.round(Number(timeout[1]) / 1000)} seconds.`;
  else if (error?.code === 'executor_deadline_exhausted') cause = `The ${stage} stage ran out of time before the worker's turn ended.`;
  else if (error?.code === 'claude_output_refused') cause = `The AI declined to write the ${stage} output.`;
  else if (error?.code === 'claude_output_truncated') cause = `The ${stage} output ran past its length limit and was not saved.`;
  else if (error?.status === 429 || error?.status === 529) cause = `The AI service was busy (HTTP ${error.status}) during the ${stage} stage.`;
  else if (Number.isInteger(error?.status)) cause = `The AI service returned an error (HTTP ${error.status}) during the ${stage} stage.`;
  else if (typeof error?.code === 'string' && error.code.startsWith('cycle_dossier_')) cause = `The ${stage} stage stopped: ${message.replace(/\.$/, '')}.`;
  else if (error?.httpStatus) cause = message;
  else cause = `The ${stage} stage hit an unexpected problem.`;
  const tail = item.paidInFlight
    ? ' A possible charge remains reserved; retry reruns only this stage.'
    : ' Saved work is kept; retry resumes from the last checkpoint. If it keeps failing, contact an administrator.';
  return cause + tail;
}
...
async function processEntry(run, original) {
  ...
  try {
    ...
    // api_usage_log rows are attributed to the run owner (the paying superuser).
    userProfileId: run.owner_profile_id,
    ...
    // Re-checked on a 10s poll so an operator stop during 429/529 retry backoff
    // does not have to wait for the whole call to return (see operatorStopSignal).
    signal: operatorStop.signal };
    if (!item.researchRef) {
    ...
    for (const format of ['docx','pdf']) if (!item.sharepoint?.[format]) {
      const saved = await publishConstituent(run, item, format, { signal: operatorStop.signal });
    ...
    await updateItem(run, item.requestId, i => {
      // An operator-stop abort can cut a call the provider already accepted;
      // that charge is ambiguous, so retain its reservation like any other
      // unresolved paid attempt instead of releasing it on the retry.
      if (error.interrupted && i.paidInFlight) i.unknownCost = true;
      i.status = error.interrupted ? 'queued' : 'failed';
      i.error = error.interrupted ? null : describeEntryFailure(error, i);
    });
  } finally {
    operatorStop.stop();
  }
...
async function assembleCut(run) {
  ...
  const row = await store.mutateDossierRun(run.id, async (current, client) => {
    ...
    for (const item of current.data.items) {
      ...
      entries.push({ id, revision: entry.request_revision, request: { requestId: item.requestId, requestNumber: item.requestNumber, title: item.title,
      ...
      if (item.status !== 'ready') fallback.push({ requestId: item.requestId, requestNumber: item.requestNumber, revision: entry.request_revision, error: item.error || current.data.pauseReason });
    }
