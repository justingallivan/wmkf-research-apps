...
// touches docProps/, customXml/, the package rels, and content types), so a
// published DOCX never matches the frozen bytes. PDFs are stored verbatim.
// Compare OOXML by its decompressed document parts instead of by package bytes.
const SHAREPOINT_OWNED_PART = /^(docProps\/|customXml\/|_rels\/\.rels$|\[Content_Types\]\.xml$)/;
async function ooxmlContentDigest(bytes) {
  const zip = await JSZip.loadAsync(bytes);
  const hash = createHash('sha256');
  for (const name of Object.keys(zip.files).filter(n => !zip.files[n].dir && !SHAREPOINT_OWNED_PART.test(n)).sort()) {
    hash.update(name).update('\0').update(await zip.files[name].async('nodebuffer')).update('\0');
  }
  return hash.digest('hex');
}
async function assertPublishedMatchesFrozen(format, frozen, observed) {
  let same;
  try {
    same = format === 'docx'
      ? (await ooxmlContentDigest(observed)) === (await ooxmlContentDigest(frozen))
      : dossierDigest(observed) === dossierDigest(frozen);
  } catch { same = false; }
  if (!same) throw store.dossierError('The published file differs from the frozen entry.');
}
...
async function publishConstituent(run, item, format) {
  ...
  await assertPublishedMatchesFrozen(format, bytes, observed.buffer);
