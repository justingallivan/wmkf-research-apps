...
async function publishConstituent(run, item, format) {
  ...
  if (dossierDigest(current) !== dossierDigest(item.destination)) throw store.dossierError('The request SharePoint destination changed. Preview again.');
  const folder = `${current.folder}/AI Artifacts/Cycle Dossier/D26/${item.revisionId}`;
  const createdFolder = await GraphService.ensureFolderPath(current.library, folder);
  ...
  if (dossierDigest(observed.buffer) !== ref.sha256) throw store.dossierError('The published file differs from the frozen entry.');
  return { ...file, sha256: ref.sha256, size: bytes.length };
}
...
async function processEntry(run, original) {
  ...
  try {
    ...
    deadlineMs: new Date(run.locked_until).getTime() - 20000 };
    if (!item.researchRef) {
