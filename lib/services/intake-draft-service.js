...
class IntakeDraftService {
  ...
  static async upsertDraftJson({
    ...
    //
    //   COALESCE on the existing row's idempotency_key (if any) keeps
    //   the first-written value sticky; the EXCLUDED value is used only
    //   when the existing row has no key (true INSERT path, or pre-fix
    //   legacy rows). One SQL statement → atomic → race-free.
    const result = await sql`
      INSERT INTO intake_drafts (
        contact_oid, account_id, request_id, form_key,
        draft_json, attachments
      ...
      DO UPDATE SET
        draft_json = jsonb_set(
          ...
          '{idempotency_key}',
          COALESCE(
            intake_drafts.draft_json->'idempotency_key',
            EXCLUDED.draft_json->'idempotency_key'
          )
        ),
        updated_at = now()
      RETURNING *
