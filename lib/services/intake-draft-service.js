...
class IntakeDraftService {
   ...
   * @returns {Promise<{promoted: true} | {promoted: false, reason: 'race_already_promoted' | 'draft_submitted' | 'cap_exceeded_race' | 'pending_not_found' | 'draft_not_found'}>}
   */
  static async promoteToClean(draftId, attachmentId, cleanRow, fieldCardinality) {
    ...
    // 0 rows: disambiguate the reasons via a follow-up read. The probe also
    // reads request_id (so a draft frozen by /submit is distinguished from a
    // cap-gate block) and counts current attachments for the fieldKey (so we
    // can distinguish `cap_exceeded_race` from `pending_not_found`).
    const probe = await sql`
      SELECT
        (request_id IS NOT NULL) AS is_submitted,
        EXISTS (
          SELECT 1 FROM jsonb_array_elements(attachments) a
          WHERE a->>'attachmentId' = ${attachmentId}
        ) AS in_attachments,
    ...
    // Entry already promoted (won the race before any freeze) — benign,
    // checked first so a submitted-but-fully-promoted draft reports the
    // idempotent outcome, not draft_submitted.
    if (probeRow.in_attachments) return { promoted: false, reason: 'race_already_promoted' };
    // Draft was frozen by /submit (request_id set) before we could promote.
    // The new `request_id IS NULL` guard blocked the UPDATE; refuse rather
    // than append bytes into an already-submitted draft (orphan risk).
    if (probeRow.is_submitted) return { promoted: false, reason: 'draft_submitted' };
