...
class IntakeDraftService {
   ...
   * `attachments`. The UPDATE's WHERE clause gates on BOTH (a) entry
   * present in `pending_attachments` AND (b) entry NOT already in
   * `attachments` — so two racing concurrent calls for the same
   * `(draftId, attachmentId)` cannot double-append. Postgres
   * EvalPlanQual re-evaluates the WHERE after row-lock wait
   * (READ COMMITTED docs § "Read Committed Isolation Level").
   *
   * @param {number} draftId
  ...
  static async promoteToClean(draftId, attachmentId, cleanRow) {
    ...
    // 0 rows: disambiguate the three reasons via a follow-up read.
