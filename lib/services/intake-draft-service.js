...
class IntakeDraftService {
  ...
  static async upsertDraftJson({
    ...
    // Service-level pre-condition (Codex S183-round-9 LOW): require the
    // caller to supply an idempotency_key in draftJson. The endpoint already
    // does this, but the service should validate so a future caller (or a
    // regression in the endpoint) fails loud rather than passing NULL to
    // jsonb_set below.
    const idemKey = draftJson?.idempotency_key;
    if (typeof idemKey !== 'string' || idemKey.length === 0) {
      throw new Error('upsertDraftJson: draftJson.idempotency_key must be a non-empty string');
    }
