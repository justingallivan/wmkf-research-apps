...
// Version-history pagination bounds. `isCurrent` is resolved from the drive
// item's own `publication.versionId`, so paging exists only to order the
// HISTORICAL entries behind it — a much cheaper thing to be slightly wrong about
// than the current editor. Three pages at `$top = limit + 1` covers ~3x the
// displayed rows, enough slack for out-of-order responses without letting a
// heavily-edited document turn one lazy disclosure into dozens of round-trips.
...
export class GraphService {
   ...
   * every page is observed before the complete set is sorted and capped. The
   * drive item's `publication.versionId` is the authoritative current identity;
   * response position never determines `isCurrent`.
   *
  ...
  static async listFileVersions(
    ...
    while (pageUrl) {
      ...
      // every page already fetched plus the authoritative current version, so
    ...
    const hasMore = usable.length > boundedLimit || sawNextLink || stoppedEarly;
    return {
      siteId,
      driveId,
      itemId,
