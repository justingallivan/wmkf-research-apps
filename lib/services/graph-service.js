...
export class GraphService {
   ...
   * @param {'fail'|'replace'} [options.conflictBehavior='replace'] - Existing-path behavior
  ...
  static async uploadFile(
    ...
    if (!['fail', 'replace'].includes(conflictBehavior)) {
      throw new Error('uploadFile: conflictBehavior must be "fail" or "replace"');
    }
    validatePath(folderPath);
  ...
  static async uploadFileLarge(
    ...
    if (!['fail', 'replace'].includes(conflictBehavior)) {
      throw new Error('uploadFileLarge: conflictBehavior must be "fail" or "replace"');
    }
