...
// level (`TenantRequestThrottled`), and the Dynamics Explorer fans several
// scoped searches out in parallel per question, so a burst of 429s is the
// normal failure shape — not a fault worth an operational-events row each.
...
const SEARCH_RETRY_STATUSES = new Set([429, 502, 503, 504]);
...
/** Backoff for search attempt `attempt` (1-based): Retry-After if sent, else exponential; both capped. */
function searchBackoffMs(resp, attempt) {
  const fromHeader = parseRetryAfterMs(resp?.headers?.get?.('retry-after'));
  if (fromHeader != null) return Math.min(fromHeader, SEARCH_BACKOFF_CAP_MS);
  return Math.min(500 * 2 ** (attempt - 1), SEARCH_BACKOFF_CAP_MS);
}
...
export class GraphService {
  ...
  static async searchFiles(query, { libraryName, folderPath } = {}) {
    ...
    for (let attempt = 1; ; attempt += 1) {
      ...
      const retryable = SEARCH_RETRY_STATUSES.has(resp.status);
      if (!retryable || attempt >= SEARCH_MAX_ATTEMPTS) {
        const text = await resp.text();
        console.error(
        ...
        // callers can tell throttling from a bad query.
      ...
      await new Promise(resolve => setTimeout(resolve, searchBackoffMs(resp, attempt)));
    }
...
// structured error is built. Module-level token/site/drive caches
// (tokenCache/tokenPromise/siteCache/driveCache) sit above this helper, so a
// cache hit performs no fetch and emits no event.
