...
async function fetchWithTimeout(url, options, timeout) {
  ...
  try {
    ...
    emitDependencyEvent({ url, method: options?.method, ms: Date.now() - startedAt, response });
    return response;
  } catch (err) {
    ...
    emitDependencyEvent({ url, method: options?.method, ms: Date.now() - startedAt, error: structured });
    throw structured;
  } finally {
    clearTimeout(timer);
  }
