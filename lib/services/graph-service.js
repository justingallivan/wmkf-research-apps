...
export class GraphService {
   ...
   * rather than surfaced half-populated. `limit` is applied as `$top`; the caller
   * is told whether more may exist so a truncated list is never displayed as
   * complete.
   *
  ...
  static async listFileVersions(
    ...
    // Ask for one extra row so "is there more" is observed, never inferred.
    const url = `${GRAPH_BASE}/drives/${encodeURIComponent(driveId)}`
    ...
    // Graph does not contractually guarantee version ordering, so sort rather
    // than trust the response order. Entries without a parseable timestamp keep
    // their relative position instead of sorting to an arbitrary end.
    ...
    const hasMore = usable.length > boundedLimit || Boolean(body?.['@odata.nextLink']);
    return {
      siteId,
      driveId,
      itemId,
      versions: usable.slice(0, boundedLimit),
      hasMore,
      limit: boundedLimit,
    };
  }
