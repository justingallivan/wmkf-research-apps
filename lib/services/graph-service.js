...
export class GraphService {
   ...
   * response position never determines `isCurrent`, and an unobserved current
   * version is fetched directly after the bounded scan.
   *
  ...
  static async listFileVersions(
    ...
    while (pageUrl) {
      ...
      if (resp.status === 404) return null;
      if (!resp.ok) {
        if (pagesFetched > 0 && (resp.status === 429 || resp.status >= 500)) {
          stoppedEarly = true;
          break;
        }
        const text = await resp.text();
      ...
      for (const entry of entries) {
        ...
        if (version) usable.push(version);
      }
    ...
    if (currentVersionId != null
      && !usable.some((version) => version.versionId === currentVersionId)) {
      const encodedVersionId = encodeURIComponent(currentVersionId);
      const currentUrl = `${GRAPH_BASE}/drives/${encodedDriveId}`
        + `/items/${encodedItemId}/versions/${encodedVersionId}`;
      ...
      const currentEntry = await currentResp.json();
      const currentVersion = mapVersionEntry(currentEntry, usable.length);
