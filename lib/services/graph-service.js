...
// Explorer's parallel scoped searches do not retry as one synchronized burst.
...
export class GraphService {
  ...
  static async searchFiles(query, { libraryName, folderPath } = {}) {
    ...
    if (cooldownRemainingMs > 0) {
      const error = new Error(`SharePoint search failed (429): search cooling down for ${Math.ceil(cooldownRemainingMs / 1000)}s after a tenant throttle`);
    ...
    for (let attempt = 1; ; attempt += 1) {
      ...
      if (!plan || plan.waitMs == null) {
        ...
        if (retryable && error.retryAfterMs > 0) {
          searchCooldownUntil = Math.max(searchCooldownUntil, Date.now() + error.retryAfterMs);
        }
        throw error;
      }
