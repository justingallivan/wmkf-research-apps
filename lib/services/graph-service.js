...
import {
  cancelBrowserUploadSession,
  createBrowserUploadSession,
  getBrowserUploadSessionStatus,
  uploadFileLarge,
} from './graph/upload-session.js';
import { readMediaRange, resolveMediaDownloadUrl } from './graph/media.js';
...
export class GraphService {
   ...
   *   returning a partial inventory when Graph pagination or recursion exceeds
   *   `maxFiles`.
  ...
  static async getFileMetadataByPath(
    ...
    folderPath,
    filename,
    { siteId: assertedSiteId = null, driveId: assertedDriveId = null, timeoutMs } = {},
  ) {
  ...
  static async uploadFile(
    ...
    contentType = 'application/octet-stream',
    {
      ...
      siteId: assertedSiteId = null,
      driveId: assertedDriveId = null,
      onItemCreated = null,
    } = {},
  ) {
  ...
  /** Create a browser-direct, preauthenticated upload session. */
  static async createBrowserUploadSession(libraryName, folderPath, filename, options = {}) {
    return createBrowserUploadSession(this, libraryName, folderPath, filename, options);
  }

  /** Read browser-direct upload progress without proxying a fragment. */
  static async getBrowserUploadSessionStatus(uploadUrl, options = {}) {
    return getBrowserUploadSessionStatus(this, uploadUrl, options);
  }

  /** Cancel a browser-direct upload session without an Authorization header. */
  static async cancelBrowserUploadSession(uploadUrl) {
    return cancelBrowserUploadSession(this, uploadUrl);
  }

  /** Resolve a short-lived Microsoft media URL without fetching media bytes. */
  static async resolveMediaDownloadUrl(driveId, itemId) {
    return resolveMediaDownloadUrl(this, driveId, itemId);
  }

  /** Read at most 4096 media bytes for signature validation. */
  static async readMediaRange(driveId, itemId, options = {}) {
    return readMediaRange(this, driveId, itemId, options);
  }
  ...
  /** Delete one exact drive item only when its observed ETag still matches. */
  static async deleteFileWithEtag(driveId, itemId, eTag) {
    return deleteFileWithEtag(this, driveId, itemId, eTag);
  }
