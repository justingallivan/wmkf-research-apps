...
// ────── alt-key Active guard (Codex round-13 Q3 BLOCKER) ──────────────
//
// The wmkf_portaloid alt-key (deployed S179) transitions Pending → Active
// asynchronously. While Pending, Dataverse does NOT enforce uniqueness,
// so the bridge's step-3 duplicate-PK recovery silently fails: two
// parallel first-time bridge calls can BOTH create contacts with the
// same OID, and the subsequent findByOid(top:2) call throws on
// multi-match — locking the applicant out until staff cleanup.
//
// Mitigation: probe the alt-key state on first create-path call. Cache
// "Active" permanently (the state is monotonic — never reverts). On
// "Pending" or probe failure, throw an `altKeyNotActive` structured
// error → submit returns 503 with retry semantic. Each subsequent call
// re-probes until Active, so when the index finishes building the next
// bridge call succeeds without process restart.

const ALT_KEY_ENTITY = 'contact';
const ALT_KEY_NAME = 'wmkf_portaloid';

let _altKeyActive = false;

async function ensureAltKeyActive() {
  if (_altKeyActive) return;
  let keyRow;
  try {
    keyRow = await contactAdapter.getEntityKeyStatus(ALT_KEY_ENTITY, ALT_KEY_NAME);
  } catch (err) {
    const wrapped = new Error(`bridge alt-key probe failed: ${err.message ?? err}`);
    wrapped.altKeyNotActive = true;
    wrapped.serviceName = 'bridge';
    wrapped.isTransient = true;
    wrapped.cause = err;
    throw wrapped;
  }
  if (!keyRow) {
    const err = new Error(`bridge: ${ALT_KEY_ENTITY}.${ALT_KEY_NAME} alt-key not found`);
    err.altKeyNotActive = true;
    err.serviceName = 'bridge';
    err.isTransient = false;
    throw err;
  }
  if (keyRow.EntityKeyIndexStatus !== 'Active') {
    const err = new Error(`bridge: ${ALT_KEY_ENTITY}.${ALT_KEY_NAME} alt-key not Active (status=${keyRow.EntityKeyIndexStatus})`);
    err.altKeyNotActive = true;
    err.altKeyStatus = keyRow.EntityKeyIndexStatus;
    err.serviceName = 'bridge';
    err.isTransient = true; // applicant retry will re-probe; status is monotonic
    throw err;
  }
  _altKeyActive = true;
}

// Test-only reset for the cache.
export function _resetAltKeyCacheForTests() {
  _altKeyActive = false;
}
...
async function findByOid(oid) {
  const records = await contactAdapter.findManyByPortalOid(oid);
...
async function findByEmail(email) {
  return contactAdapter.findManyByEmailLowercased(email);
}
...
async function attachOidToContact(contactId, oid) {
  await contactAdapter.setPortalOid(contactId, oid);
}
...
async function createContact({ oid, email, name }) {
  // Gate create on alt-key Active (Codex round-13 Q3). Without this,
  // parallel first-time bridge calls could each create a duplicate
  // contact for the same OID while the alt-key index is still building.
  await ensureAltKeyActive();
  ...
  const created = await contactAdapter.createPortalContact(body);
  return created;
}
