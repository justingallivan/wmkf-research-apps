...
export async function executePrompt({
  ...
  try {
    ...
    // Step 7: parse output
    const parsedResult = parseClaudeOutput(claudeResp, outputSchema);
    ...
    try {
      runId = await writeRunRow({
        promptRow, requestId, runSource,
        status: 'failed',
        rawOutput: buildFailureRawOutput(err, claudeResp, outputSchema),
        notes: buildFailureNotes(err, claudeResp, { semanticAttempt, retryOfRunId }),
        overrideVariables,
        variableDecls,
