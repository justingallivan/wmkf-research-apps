...
export async function executePrompt({
  ...
  try {
    ...
    claudeResp = await callClaude(promptRow, composed, modelInfo, outputSchema, maxTokensOverride, timeoutMsOverride);
...
async function callClaude(promptRow, { system, body }, modelInfo, outputSchema, maxTokensOverride = null, timeoutMsOverride = null) {
  const apiKey = process.env.CLAUDE_API_KEY;
  if (!apiKey) throw new Error('CLAUDE_API_KEY not set');

  const model = modelInfo.model;
  const configuredMaxTokens = promptRow.wmkf_ai_maxtokens || BASE_CONFIG.MODEL_PARAMS.DEFAULT_MAX_TOKENS;
  const maxTokens = maxTokensOverride ?? configuredMaxTokens;
  if (!Number.isInteger(maxTokens) || maxTokens <= 0) {
    throw new Error(`executePrompt: maxTokensOverride must be a positive integer (got ${maxTokens})`);
  }
  if (modelInfo.capabilities.maxOutputTokens && maxTokens > modelInfo.capabilities.maxOutputTokens) {
    throw new Error(
      `executePrompt: maxTokens ${maxTokens} exceeds reviewed limit ` +
      `${modelInfo.capabilities.maxOutputTokens} for "${model}"`,
    );
  }
  const temperature =
