...
import { resolveModel, loadAvailableModels } from './model-resolver.js';
...
export async function executePrompt({
  ...
  return bypassDynamicsRestrictions('execute-prompt', async () => {
  ...
  try {
    ...
    if (conflicts.length > 0 && !forceOverwrite) {
      const runId = await writeRunRow({
        ...
        overrideVariables,
        variableDecls,
        modelUsed: resolveModel(promptRow.wmkf_ai_model) || promptRow.wmkf_ai_model,
        actingUserSystemId,
      });
    ...
    const meta = {
      ...
      modelUsed: claudeResp.model || resolveModel(promptRow.wmkf_ai_model) || promptRow.wmkf_ai_model,
...
async function writeRunRow({ promptRow, requestId, runSource, status, rawOutput, notes, overrideVariables, variableDecls = [], modelUsed, actingUserSystemId = null }) {
  ...
  const created = await DynamicsService.createRecord(RUNS_ENTITY, payload, { actingUserSystemId });
