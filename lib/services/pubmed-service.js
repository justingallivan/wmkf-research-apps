...
class PubMedService {
  ...
  /**
   * Contact address sent to NCBI as the `email` parameter so they can reach a
   * human if our usage causes them a problem.
   *
   * Prefers `SCHOLARLY_POLITE_MAILTO` and falls back to
   * `NOTIFICATION_EMAIL_FROM`, which historically served double duty here. The
   * fallback keeps behavior identical where the new var is unset; set it
   * explicitly once `NOTIFICATION_EMAIL_FROM` becomes an unmonitored system
   * mailbox, or NCBI's contact path becomes a dead end. Read at call time (not
   * hoisted) so tests can set it per-case.
   */
  static contactEmail() {
    return process.env.SCHOLARLY_POLITE_MAILTO || process.env.NOTIFICATION_EMAIL_FROM || null;
  }
  ...
  /**
   * Search PubMed while retaining the provider's total result count. Identity
   * evaluators need this shape so a capped response cannot masquerade as a
   * complete author corpus.
   *
   * @returns {Promise<{totalCount: number|null, records: Array}>}
   */
  static async searchWithCount(query, maxResults = 100, options = {}) {
    const { signal, throwOnError = false } = options;
    try {
      const { totalCount, pmids } = await this.searchPMIDsWithCount(query, maxResults, { signal });
      if (pmids.length === 0) return { totalCount, records: [] };
      const records = await this.fetchArticles(pmids, { signal });
      return { totalCount, records };
    } catch (error) {
      if (signal?.aborted || error?.name === 'AbortError' || throwOnError) throw error;
      console.error('PubMed search error:', error.message);
      return { totalCount: null, records: [] };
    }
  }
  ...
  static async searchPMIDs(query, maxResults, { signal } = {}) {
    const result = await this.searchPMIDsWithCount(query, maxResults, { signal });
    return result.pmids;
  }

  /** Return both the provider-reported total and the bounded PMID page. */
  static async searchPMIDsWithCount(query, maxResults, { signal } = {}) {
    ...
    const pmids = Array.isArray(data.esearchresult?.idlist) ? data.esearchresult.idlist : [];
    const rawCount = data.esearchresult?.count;
    const totalCount = rawCount === null || rawCount === undefined || rawCount === ''
      ? Number.NaN
      : Number(rawCount);
    return {
      totalCount: Number.isSafeInteger(totalCount) && totalCount >= pmids.length
        ? totalCount
        : null,
      pmids,
    };
