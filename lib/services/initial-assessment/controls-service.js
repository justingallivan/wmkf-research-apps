 ...
 * copies the exact current bytes into a distinct retained Request Document row.
 */
...
async function verifyReadySnapshotFile(row, source, sourceBytes, dependencies) {
  ...
  if (!retained.buffer.equals(sourceBytes)) {
    throw controlError(
      'The retained Board snapshot no longer matches its exact source bytes.',
      'initial_assessment_snapshot_file_invalid',
    );
  }
}

/** Copy the exact current Initial Assessment bytes into one retained Board row/item. */
export async function createInitialAssessmentBoardSnapshot(
  ...
  if (snapshot?.wmkf_operationstatus === REQUEST_DOCUMENT_OPERATION_STATUS.READY) {
    ...
    await verifyReadySnapshotFile(snapshot, source, downloaded.buffer, dependencies);
  ...
  try {
    ...
    if (uploaded) {
      ...
      if (!existing.buffer.equals(downloaded.buffer)) {
        throw controlError(
          'A different file already occupies the deterministic Board snapshot path.',
    ...
    if (!copied.buffer.equals(downloaded.buffer)) {
      throw controlError(
        'The retained Board snapshot bytes do not match the selected source version.',
