 ...
 *
 * Keyed on `requestId`, not the registry row id, to match the public contract
 * the Workbench already uses — the caller asks about the artifact it is
 * displaying, and this service picks the same Ready, non-superseded row that
 * supplies that view's `file`. **The drive and item identity always come from
 * that row, never from the caller**, so a client cannot point this read at an
 * arbitrary SharePoint item.
 *
 ...
 */
export async function listInitialAssessmentArtifactVersions({ requestId, limit = 20 }) {
  if (!isGuid(requestId)) {
  ...
  try {
    ...
    return {
      ...
      status: 'current',
      checkedAt,
      // Newest first, so the first entry is the version staff are editing now.
