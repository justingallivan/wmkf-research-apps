...
/** Resolve a relationship target against its source part's directory, OPC style. */
function resolvePartName(baseDirectory, target) {
...
async function assertPackageOpensGovernedDocument(archive) {
  const rootRels = archive.file('_rels/.rels');
  ...
  if (officeDocuments.length !== 1
    || resolvePartName('', officeDocuments[0].Target) !== 'word/document.xml') {
    throw new Error('Governed DOCX package root does not open word/document.xml.');
  }
  const documentRels = archive.file('word/_rels/document.xml.rels');
  if (!documentRels) return;
  for (const relationship of parseRelationships(await documentRels.async('string'), 'document')) {
    if (/\/customXml$/i.test(relationship.Type)) continue;
    if (String(relationship.TargetMode || '').toLowerCase() === 'external') continue;
    const partName = resolvePartName('word/', relationship.Target);
    if (!partName || !partName.startsWith('word/')) {
      throw new Error('Governed DOCX references a document part outside word/.');
    }
  }
}
...
/** Resolve one request's canonical editable Initial Assessment row by pointer. */
