...
function isTruncationValidation(validation) {
  return (validation?.issues || []).some(issue => (
    issue.code === 'truncated_response' || issue.code === 'truncated_or_missing_queries'
  ));
}
...
function buildAnalyzeRepairInstructions(validation, reviewerCount) {
  ...
  return [
    truncation
      ? 'Your previous response appears truncated or omitted the database-search query section. Return a concise complete response in the same delimiter format.'
    ...
    'Every reviewer must include NAME, INSTITUTION, EXPERTISE, SENIORITY, REASONING, POTENTIAL_CONCERNS, and SOURCE.',
    'Do not include placeholders, bracketed replacement text, duplicate people, or excluded names.',
    'Include at least one source-aware database search query section.',
  ].join('\n');
}
