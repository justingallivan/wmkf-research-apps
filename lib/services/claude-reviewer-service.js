...
function buildTopUpPrompt({ needed, keptNames, droppedNames, exclusionsBlock, proposalContext }) {
  const repeatNames = uniqueText([...keptNames, ...droppedNames]);
  return [
    proposalContext ? `Use this proposal context to suggest relevant reviewers:\n${proposalContext}` : '',
    '',
...
class ClaudeReviewerService {
  ...
  static async _topUpReviewerSuggestions({
    ...
    const proposalContext = createProposalSummary(result?.proposalInfo || {});
