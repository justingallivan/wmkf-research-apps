...
function canonicalTimestamp(value, label) {
  if (typeof value !== 'string') fail('invalid_event', `${label} must be a canonical ISO UTC timestamp`);
...
function buildDecisionPatch(decision, source) {
  ...
  canonicalTimestamp(decision.resolvedAt, 'decision.resolvedAt');
  ...
  return {
    ...
    wmkf_identityresolvedat: persisted.resolvedAt,
...
function normalizeEvent(event) {
  ...
  canonicalTimestamp(boundAt, 'boundAt');
...
function decisionMap(row) {
  return Object.fromEntries(DECISION_FIELDS.map((field) => [field, row?.[field] ?? null]));
}
...
function bindingMap(row) {
  return {
    ...
    boundAt: row?.wmkf_identityboundat ?? null,
...
function determineTransition(current, event) {
  ...
  if (sameSource && event.source === 'automated') {
    const currentResolvedAt = current.decision.wmkf_identityresolvedat;
    const currentResolvedMs = typeof currentResolvedAt === 'string' ? Date.parse(currentResolvedAt) : NaN;
    if (!Number.isFinite(currentResolvedMs) || new Date(currentResolvedMs).toISOString() !== currentResolvedAt) {
...
function prepareTarget(current, event, transition) {
  ...
  const decision = transition.preserveDecision ? current.decision : event.decisionPatch;
  const patch = {
