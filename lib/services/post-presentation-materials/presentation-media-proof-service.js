...
export async function deletePresentationMediaProofItemWithEtag(driveId, itemId, eTag) {
  ...
  const status = await GraphService.deleteFileWithEtag(driveId, itemId, eTag);
  if (status !== 204) {
    throw proofError(
      `Microsoft did not confirm the ETag-guarded deletion (${status}); retain the cleanup permit.`,
