 ...
 * profile and expires 72 hours from mint independently of Graph's moving
 * session expiry. Resume checks exact item identity and live Graph state rather
 * than trusting the sealed initial expiry; cleanup preserves retry authority
 * unless cancellation or exact item deletion is confirmed. The public playback
 * token is a five-minute, audience-scoped JWT.
 */
...
import { API_TIMEOUT, GRAPH_BASE } from '../graph/constants.js';
import { fetchWithTimeout } from '../graph/http.js';
...
export const PROOF_CHUNK_BYTES = GRAPH_UPLOAD_DEFAULT_CHUNK_BYTES;
export const PROOF_MIN_BYTES = 50 * 1024 * 1024 + 1;
export const PROOF_MAX_BYTES = 2_000_000_000;
const PROOF_TOKEN_MS = 5 * 60 * 1000;
const PROOF_PERMIT_MS = 72 * 60 * 60 * 1000;
const CLOSED_SESSION_VISIBILITY_DELAYS_MS = Object.freeze([2_000, 8_000]);
...
const DEFAULT_DEPENDENCIES = Object.freeze({
  ...
  deleteFile: (...args) => deletePresentationMediaProofItemWithEtag(...args),
...
/** Delete one exact-path proof item only while its observed Graph ETag still matches. */
export async function deletePresentationMediaProofItemWithEtag(driveId, itemId, eTag) {
  if (typeof eTag !== 'string' || !eTag.trim()) {
    throw proofError('The exact SharePoint item has no ETag; retain the cleanup permit.', 'presentation_media_proof_cleanup_uncertain', 502);
  }
  ...
  if (response.status !== 204) {
    throw proofError(
      `Microsoft did not confirm the ETag-guarded deletion (${response.status}); retain the cleanup permit.`,
...
export async function beginPresentationMediaProofUpload(input, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  // One governed request container plus a collision-resistant create-only filename
  // avoids leaving an empty per-attempt folder when an upload is cancelled.
  const folderPath = `${bucket.folder}/Post Site Visit Materials`;
  ...
  const permit = sealPermit({
    v: 1,
    // Browser proof authority is bounded from mint time, independently of
    // Graph's moving upload-session expiry. Successful fragments can extend
    // the Graph expiry without extending this absolute permit lifetime.
    exp: dependencies.now().getTime() + PROOF_PERMIT_MS,
...
export async function getPresentationMediaProofUploadStatus({ permit: value, profileId }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  // file: only live upload-session ranges are authoritative for resume. The
  // permit's sealed initial expiry is advisory because successful fragments
  // extend Graph's real expiry.
  let status;
  try {
    status = await dependencies.getSessionStatus(permit.uploadUrl);
  } catch (error) {
    if ([404, 410].includes(error?.status)) {
      // A closed session can be the final-commit outcome. Re-read only the
      // exact server-owned path after bounded SharePoint visibility delays.
      for (const delayMs of CLOSED_SESSION_VISIBILITY_DELAYS_MS) {
        await dependencies.sleep(delayMs);
        const visible = await dependencies.getByPath(
          permit.library,
          permit.folderPath,
          permit.physicalFilename,
          { siteId: permit.siteId, driveId: permit.driveId },
        );
        if (visible && assertPathItemIdentity(permit, visible) === permit.size) {
          return {
            complete: true,
            canFinalize: true,
      ...
      if (error.status === 410) {
        throw proofError('Microsoft confirmed that the Graph upload session expired before the file committed.', 'presentation_media_proof_session_expired', 410);
      }
      throw proofError(
        'Microsoft closed the Graph upload session, but the final item outcome is unresolved.',
        'presentation_media_proof_session_closed',
        409,
      );
    }
    throw error;
...
export async function cleanupPresentationMediaProofUpload({ permit: value, profileId }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  if (item && itemSize === permit.size) {
    if (typeof item.eTag !== 'string' || !item.eTag.trim()) {
      throw proofError('The exact SharePoint item has no ETag; retain the cleanup permit.', 'presentation_media_proof_cleanup_uncertain', 502);
    }
    await dependencies.deleteFile(item.driveId, item.id, item.eTag);
