 ...
 * profile; the public playback token is a five-minute, audience-scoped JWT.
 */
...
export const PROOF_CHUNK_BYTES = 10 * 1024 * 1024;
...
async function resolveCommittedItem(permit, dependencies) {
  ...
  if (!item) return null;
  if (item.driveId !== permit.driveId || item.name !== permit.physicalFilename || Number(item.size) !== permit.size) {
...
export async function beginPresentationMediaProofUpload(input, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const folderPath = `${bucket.folder}/Artifacts/Presentation Media Proof/${proofId}`;
...
export async function getPresentationMediaProofUploadStatus({ permit: value, profileId }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  if (item) return { complete: true, canFinalize: true, file: { name: permit.displayName, size: permit.size, lastModified: permit.lastModified } };
...
export async function cleanupPresentationMediaProofUpload({ permit: value, profileId }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const permit = requirePermit(value, profileId, dependencies);
  try {
    await dependencies.cancelSession(permit.uploadUrl);
  } catch {
    // A committed session is already closed; exact-item cleanup below remains authoritative.
  }
  const item = await resolveCommittedItem(permit, dependencies);
  if (item) await dependencies.deleteFile(item.driveId, item.id);
  return { cleaned: true, deletedItem: Boolean(item) };
}
