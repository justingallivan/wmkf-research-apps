 ...
 * Public email links use DELIBERATION_BRIEFING_PUBLIC_BASE_URL when configured,
 * falling back to NEXTAUTH_URL for existing deployments. The override must be
...
export const DELIBERATION_BRIEFING_PUBLIC_BASE_URL = 'DELIBERATION_BRIEFING_PUBLIC_BASE_URL';
...
function validateBriefingBaseUrl(raw, { override }) {
  ...
  try {
    ...
    throw new Error('Briefing public base URL must be an absolute http(s) URL.');
  }
  if (!['http:', 'https:'].includes(parsed.protocol)
    || parsed.username || parsed.password || parsed.search || parsed.hash
    || (parsed.pathname !== '' && parsed.pathname !== '/')) {
    throw new Error('Briefing public base URL must be an origin without credentials, path, query, or fragment.');
  }
  if (override && parsed.protocol !== 'https:') {
    throw new Error('DELIBERATION_BRIEFING_PUBLIC_BASE_URL must use https.');
  }
  return parsed.origin;
}

export function getBriefingBaseUrl(env = process.env) {
  const override = String(env?.[DELIBERATION_BRIEFING_PUBLIC_BASE_URL] || '').trim();
  const configured = override || String(env?.NEXTAUTH_URL || '').trim();
  const base = validateBriefingBaseUrl(configured, { override: Boolean(override) });
  if (!base) throw new Error('A public briefing base URL is required to mint a briefing link.');
  return base;
}

export function buildBriefingUrl(jwt, env = process.env) {
  return `${getBriefingBaseUrl(env)}/external/briefing/${jwt}`;
}
