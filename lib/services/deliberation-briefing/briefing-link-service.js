 ...
 * still in the future wins: site-visit end + 7 days, then meeting date + 7
 * days. Only when neither authoritative date yields a usable window (both
 * absent, or both already past — a post-visit re-share still has to reach the
 * deliberation meeting) does the 60-day default apply. Callers must pass
 * dates from successful reads; a failed read is not "absent".
 */
export function computeBriefingExpiry({ siteVisitEnd, meetingDate, now }) {
  ...
  const usable = candidates.find((candidate) => candidate.getTime() - base.getTime() >= MIN_TTL_MS);
