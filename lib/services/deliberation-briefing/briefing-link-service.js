 ...
 * replacement targets the row staff were looking at (`expectedLinkId`, when
 * the tab supplies it); if another action replaced it first the call returns
 * 409 `briefing_link_superseded` and the tab refreshes rather than revoking
 * the newer link.
 */
export async function reissueBriefingLink({ requestId, actorId, expectedLinkId }, dependencies = DEFAULT_DEPENDENCIES) {
  assertInputs(requestId, actorId, dependencies);
  const { replacement, url } = await mintReplacement(requestId, actorId, dependencies);
  const options = { revokedBy: actorId };
  if (expectedLinkId !== undefined && expectedLinkId !== null) {
    if (!isGuid(expectedLinkId)) throw briefingError('expectedLinkId must be a GUID.', 'briefing_request_invalid', 400);
    options.expectedLiveId = expectedLinkId;
  }
  const row = await dependencies.replaceLiveLink(requestId, replacement, options);
  return { link: projectBriefingLink(row, { url }), reused: false };
}
