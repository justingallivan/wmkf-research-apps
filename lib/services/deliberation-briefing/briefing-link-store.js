...
export async function replaceLiveLink(requestId, replacement, { revokedBy, expectedLiveId = undefined }) {
  // Excluded transaction shape (early ROLLBACK-then-throw twice below, plus
  // a catch-all ROLLBACK): kept verbatim on a withClient client rather than
  // withTransaction. Recorded behaviour difference: withClient destroys the
  // connection on any throw out of this callback (including after the
  // catch-all ROLLBACK) instead of this function's former unconditional
  // `finally { client.release(); }`, which always returned it to the pool.
