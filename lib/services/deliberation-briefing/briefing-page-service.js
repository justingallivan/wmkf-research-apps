...
/**
 * The Board-facing staff brief is verified against the retained snapshot's
 * governed Word content hash (the snapshot registry row's `wmkf_contenthash`,
 * written by prepare from the same `hashGovernedDocxContent` producer), never
 * against package bytes. SharePoint rewrites a generated .docx after upload
 * (property promotion adds customXml/docProps and repacks the container) and
 * that rewrite lands after prepare's post-upload metadata read, so the pinned
 * `docx_byte_hash` legitimately stops matching the bytes Graph serves; a
 * Word-saved package already carries those parts and round-trips byte-identical,
 * which is why only raw generated briefs failed (plan §12 Finding A, 2026-09-17).
 * The registry row must belong to this request and be Ready. Attempts with no
 ...
 */
async function assertWriteupSnapshotIntact({ requestId, attempt, buffer }, dependencies) {
  const mismatch = () => new ServiceHttpError('snapshot content differs from the pinned identity', {
    httpStatus: 409,
    body: { ok: false, reason: 'snapshot_mismatch' },
  });
  const snapshotId = attempt.docx_snapshot_document_id;
  if (isGuid(snapshotId)) {
    const { records } = await dependencies.findDocumentById(snapshotId);
    const row = records?.length === 1 ? records[0] : null;
    const expected = String(row?.wmkf_contenthash || '');
    if (!row
    ...
    if (!actual || actual !== expected) throw mismatch();
    return;
  }
  const expectedHash = String(attempt.docx_byte_hash || '');
  const actualHash = createHash('sha256').update(buffer).digest('hex');
  if (!/^[0-9a-f]{64}$/.test(expectedHash) || actualHash !== expectedHash) throw mismatch();
}
...
export async function resolveBriefingMember({ requestId, member }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  if (member === BRIEFING_MEMBER.WRITEUP_DOCX) {
    ...
    await assertWriteupSnapshotIntact({ requestId, attempt, buffer: file.buffer }, dependencies);
