...
async function loadLiveReviewsForBundle(requestId, dependencies) {
  const suggestionRows = await dependencies.findSuggestions(requestId);
  return (suggestionRows || [])
    .filter((row) => Boolean(row.wmkf_reviewreceivedat))
    .map((row) => ({
      suggestionId: row.wmkf_appreviewersuggestionid,
      reviewSharePointFolder: row.wmkf_reviewsharepointfolder || null,
      reviewFilename: row.wmkf_reviewfilename || null,
      name: personName(row, null),
      affiliation: personAffiliation(row, null),
      reviewReceivedAt: isoOrNull(row.wmkf_reviewreceivedat),
    }));
}
