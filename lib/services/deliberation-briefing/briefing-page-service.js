...
const DEFAULT_DEPENDENCIES = Object.freeze({
  ...
  // its own real DEFAULT_DEPENDENCIES (live GraphService/adapters); an
  // external Board reader's token carries no staff Dynamics identity, so
  // this always uses ALLOW_UNATTRIBUTED, never REQUIRED.
  retainReviewBundle: (spec) => retainReviewBundle(spec, undefined, null, REQUEST_DOCUMENT_ACTOR_POLICY.ALLOW_UNATTRIBUTED),
...
export async function resolveBriefingMember({ requestId, member }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  if (member === BRIEFING_MEMBER.REVIEW_BUNDLE) {
    ...
    if (liveFingerprint !== attempt.review_bundle_set_fingerprint) {
      ...
      // an error state — only this one member fails (503).
      let rebuilt;
      let sourceRow;
      try {
