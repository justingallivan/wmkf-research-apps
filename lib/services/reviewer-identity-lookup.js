...
function referencedReviewersFromCandidates(candidates = []) {
  ...
  for (const candidate of candidates || []) {
    const ref = referencedReviewer(candidate?.reviewerId, candidate?.context?.affiliation);
    if (!ref) continue;
...
async function lookup({ name, email, orcid }) {
  ...
  return { outcome: 'none', referencedReviewers: [] };
}
