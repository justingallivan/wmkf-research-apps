...
export async function previewCycleDossier(owner, body) {
  await store.assertDossierActor(owner); assertDossierStorageConfigured();
...
export async function cycleDossierAction(owner, body) {
  ...
  if (['pause','resume','cancel','retry'].includes(body.action)) return controlCycleDossier(owner, body);
