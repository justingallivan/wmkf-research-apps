...
import { prepareRequestInput, snapshotConfiguration, estimateGenerationCost, resolveEntryTimeoutMs } from './cycle-dossier-generation';
...
import { TEST_REQUEST_ORDINARY_OData_FILTER, testRequestIsolationEnabled } from './test-requests/isolation.js';
...
export async function loadDossierRoster() {
  ...
  const result = await requests.queryAllRequests({
    ...
    // A cycle dossier is a cycle-wide report, so test requests are excluded
    // from its roster (Test Request Factory Stage 1c, owner decision).
    filter: testRequestIsolationEnabled()
      ? `${buildDossierRosterFilter(programScope.programId)} and ${TEST_REQUEST_ORDINARY_OData_FILTER}`
      : buildDossierRosterFilter(programScope.programId),
...
export async function getCycleDossierPage(owner) {
  ...
  return { dossier: projectDossier(dossier), candidates: roster.map(c => {
    ...
    return { ...c, latestRevision: e ? { id: e.id, revision: e.request_revision, createdAt: e.created_at, createdBy: e.created_by } : null };
  }), runs: runs.map(projectDossierRun), editions: editions.map(e => ({ id: e.id, createdAt: e.created_at,
    status: e.data.status, missing: e.data.missing || [], fallback: e.data.fallback || [],
    requestNumbers: (e.data.entries || []).map(entry => entry?.request?.requestNumber).filter(Boolean) })), configuration,
...
export async function previewCycleDossier(owner, body) {
  ...
  const items = await dossierPool(selected, async requestId => {
    ...
    if (previous && !generate.includes(requestId)) return { ...info, status: 'ready', reuseId: previous.id, revision: previous.request_revision, stage: 'reuse' };
    try {
      ...
      // Persisted objects come back from JSONB with reordered keys, so freeze the
      // destination digest here as a string exactly like inputHash.
      return { ...info, status: 'queued', stage: 'prepared', inputRef, inputHash, estimate, destination, destinationHash: dossierDigest(destination),
...
export async function launchCycleDossier(owner, body) {
  ...
  await dossierPool(preview.data.items.filter(i => i.inputRef), async item => {
    ...
    if (dossierDigest(await resolveDossierDestination(item.requestId, item.requestNumber)) !== item.destinationHash) throw store.dossierError(`SharePoint destination changed for ${item.requestNumber}. Preview again.`);
  });
  const run = await store.withDossierTransaction(async client => {
    ...
    for (const item of data.items) if (item.revisionId) {
      ...
      item.revision = entry.request_revision;
    }
...
export async function controlCycleDossier(owner, body) {
  ...
  if (body.action === 'retry') {
    ...
    return store.withDossierTransaction(async client => {
      ...
      // Prompts stay pinned to the source run, but the entry-stage timeout is an
      // admin-tunable budget: a retry after an admin raised it must use the new
      // value, or a timed-out entry would time out again (1002874, 2026-09-12).
      if (data.config?.budget) data.config.budget.entryTimeoutMs = await resolveEntryTimeoutMs();
  ...
  const row = await store.mutateDossierRun(runId, run => {
    if (body.action === 'pause' || body.action === 'cancel') {
      ...
      if (body.budgetUsd != null) {
        const cap = budget(body.budgetUsd);
        const committed = Number(run.data.spentUsd || 0) + Number(run.data.reservedUsd || 0);
        if (cap < committed) throw store.dossierError(`The cap cannot be below the ${committed.toFixed(2)} USD already spent or reserved on this run.`, 400);
        run.data.budgetUsd = cap;
      }
...
export async function cycleDossierAction(owner, body) {
  ...
  if (body.action === 'selection') {
    ...
    assertDossierCohortConfigured();
    // The allowlist may name request numbers or GUIDs; the roster is already
    // filtered by it, so the roster's GUIDs are the canonical cohort. Comparing
    // the selected GUID to raw allowlist strings rejected a number-based smoke.
    const cohort = new Set((await loadDossierRoster()).map(item => item.requestId));
    if ((process.env.CYCLE_DOSSIER_ROLLOUT_MODE || 'pilot') === 'smoke' && selection.length !== 1) {
      throw store.dossierError('Smoke mode requires exactly one allowlisted request.', 400);
    }
    if (selection.some(id => !cohort.has(id))) {
      throw store.dossierError('The selection includes a request outside the dossier cohort.', 400);
    }
...
export async function downloadCycleDossier(owner, query) {
  ...
  if (query.editionId) {
    ...
    files = row.data.files; name = `D26-${row.data.request.requestNumber}-v${row.request_revision}`;
  }
