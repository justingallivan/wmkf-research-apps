...
import { cycleCodeToOdataFilter } from '../utils/cycle-code';
import { buildVisibilityFilter } from './workbench/dashboard-service';
...
import { assertDossierPilotEnabled, assertDossierCohortConfigured, assertDossierRequestAllowed } from './cycle-dossier-rollout';
...
export async function loadDossierRoster() {
  ...
  const result = await requests.queryAllRequests({
    ...
    filter: `${cycleCodeToOdataFilter(CYCLE)} and ${buildVisibilityFilter(false)}`,
...
export async function getCycleDossierPage(owner) {
  ...
  assertDossierPilotEnabled();
  const [dossier, roster, entries, runs, editions] = await Promise.all([
    store.getDossier(owner), loadDossierRoster(), store.listDossierEntries(), store.listDossierRuns(owner), store.listDossierEditions(owner),
  ]);
  let configuration;
  try {
...
export async function previewCycleDossier(owner, body) {
  await store.assertDossierActor(owner); assertDossierPilotEnabled(); assertDossierStorageConfigured();
