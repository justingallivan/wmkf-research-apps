...
function mapRow(row, registryMap = null, unavailableIds = null) {
  ...
  const isUnavailable = Boolean(docKey && unavailableIds?.has(docKey));
  return {
    id: String(row.id),
    ...
    // that succeeded but returned no matching row falls back to a bare id.
...
export async function listConsultantFeedback({ requestId }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  return rows.map((row) => mapRow(row, registryMap, unavailableIds));
}
...
export async function downloadConsultantFeedbackAttachment({ requestId, entryId }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const sameDocument = String(row?.wmkf_requestdocumentid || '').toLowerCase()
    === String(feedback.requestdocument_id).toLowerCase();
  const sameRequest = String(row?._wmkf_request_value || '').toLowerCase() === requestId.toLowerCase();
  const eligible = row
    && sameDocument
    && sameRequest
  ...
  if (!eligible) throw notFound('Consultant feedback attachment not found.');
