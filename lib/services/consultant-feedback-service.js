...
import { REQUEST_DOCUMENT_OPERATION_STATUS, REQUEST_DOCUMENT_LIFECYCLE_STATE } from '../../shared/config/requestDocument.js';
...
/** Best-effort batch registry read keyed by lowercased id; never throws. */
async function loadRegistryMap(requestdocumentIds, dependencies) {
  const ids = [...new Set(requestdocumentIds.filter(Boolean))];
  if (!ids.length) return new Map();
  try {
    const { records } = await dependencies.findDocumentsByIds(ids);
    return new Map((records || []).map((row) => [String(row.wmkf_requestdocumentid).toLowerCase(), row]));
  } catch (error) {
    console.error('[consultant-feedback] registry batch read failed', { message: error?.message });
    return new Map();
  }
}
...
function mapRow(row, registryMap = null) {
  const registryRow = registryMap && row.requestdocument_id
    ? registryMap.get(String(row.requestdocument_id).toLowerCase()) || null
    : null;
  return {
    id: String(row.id),
    ...
    // batch read failure is swallowed — §3.3 "unavailable is not empty" for
    // the briefing page; the staff list simply omits attachment detail).
    attachment: row.requestdocument_id
      ? (attachmentFromRegistryRow(registryRow) || { requestdocumentId: row.requestdocument_id, filename: null, contentType: null, size: null })
      : null,
...
export async function listConsultantFeedback({ requestId }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const registryMap = await loadRegistryMap(rows.map((row) => row.requestdocument_id), dependencies);
  return rows.map((row) => mapRow(row, registryMap));
}
...
export async function updateFeedbackEntry({ id, requestId, actorProfileId, patch = {} }) {
  ...
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(patch, 'requestdocumentId')) {
      ...
      if (existing.requestdocument_id != null) {
...
export async function loadSharedConsultantFeedbackForBriefing(requestId, dependencies = DEFAULT_DEPENDENCIES) {
  try {
    ...
    return {
      status: 'ok',
      items: rows.map((row) => {
        const registryRow = row.requestdocument_id ? registryMap.get(String(row.requestdocument_id).toLowerCase()) || null : null;
