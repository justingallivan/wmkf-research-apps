...
// Returns { kind, extensions, anyAbstained }. `anyAbstained` is true only when
// kind === 'decoration' AND at least one extension abstained rather than
// proving same-identity decoration — i.e. the 'admit' policy is the reason
...
// allowing genuinely proven decoration/standalone fragments full evidence.
async function classifyFragment(fragment, ownOperandSegments, resolve, {
  signal,
  fragmentIdentity,
  unprovenExtensionPolicy = 'demote',
} = {}) {
  const extensions = fragmentExtensions(fragment, ownOperandSegments);
...
function createInstitutionConsistencyChecker({
  ...
  async function segmentWiseConsistent(left, right, { signal } = {}) {
    ...
    //
    // CLOSURE ARGUMENT (Wave 3c, falsifiable — adversarial review target):
    // after this change, a pairing may draw on associated-link evidence
    // (`institutionsConsistent`'s one-hop `associatedIdentityMatches`) ONLY
    // when BOTH crossing entries are PROVEN: either a whole operand, or a
    // fragment whose every extension resolved to its own identity
    // (`admittedUnproven === false`). An `admittedUnproven` entry — a
    // fragment admitted to the pool despite an abstaining extension — is
    // never disproven to be a bare parent, so it participates in
    // identity-EQUALITY evidence only (`institutionDirectMatch`), in either
    // direction, composing with (not replacing) the existing whole-parent
    // `directOnly` restriction and the `sharedFragmentSelfPair` exclusion.
    // This closes the sibling-campus leak where an admitted-unproven bare
    // fragment (e.g. "University of California", identity SYSTEM) crossed a
    // sibling campus's WHOLE operand via SYSTEM's presence in that campus's
    // `associatedInstitutions` — SYSTEM was never proven to represent the
    // fragment's own operand exclusively, so it must not spend
    // associated-link credit on the other operand's behalf. A residual
    // remains where the SAME bare string resolves identically on both sides
    // (identity EQUALITY, not an associated link) — see the accepted-residual
    // test in tests/unit/institution-pair-segment-comparison.test.js.
