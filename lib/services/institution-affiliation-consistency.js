...
function createInstitutionConsistencyChecker({
  ...
  async function segmentWiseConsistent(left, right, { signal } = {}) {
    ...
    // shape), but an extension that merely abstains does not: step 2 is about
    // to test this fragment's independently-resolved identity against the
    // OTHER operand's independently-resolved identity via
    // `institutionsConsistent`, so a missing decoration lookup shouldn't
    // block a fragment whose own name resolved cleanly (live evidence: real
    // decoration extensions — city/state/zip — routinely abstain). Whole
    // operands themselves ALWAYS stay in the pool.
    ...
    return leftPool.some((resolvedLeft) => (
      rightPool.some((resolvedRight) => {
        ...
        const directOnly = (rightIsWhole && rightWholeIsParentOfLeft)
