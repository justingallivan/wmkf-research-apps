...
function createInstitutionConsistencyChecker({
  ...
  async function segmentWiseConsistent(left, right, { signal } = {}) {
    ...
    // 1) Direct segment-vs-other-operand match (string identity, no resolve).
    if (leftSegments.some((segment) => DeduplicationService.institutionDirectMatch(segment, right))) {
      return true;
    }
    if (rightSegments.some((segment) => DeduplicationService.institutionDirectMatch(segment, left))) {
      return true;
    }
