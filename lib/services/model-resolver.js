 ...
 * Model tier resolver.
 *
 * Maps abstract tiers (fable/opus/sonnet/haiku) to the latest concrete Anthropic
 ...
 * `resolveModelWithCapabilities()` couples that concrete id with the reviewed
 * request-shaping capabilities callers should use for the actual API request.
 *
 * Failure modes:
...
const { requestCapabilitiesForModel } = require('./model-capabilities');
...
const TIERS = {
  fable: { family: 'fable', anthropic: 'Fable', tier: 'extra high', order: 0 },
  opus: { family: 'opus', anthropic: 'Opus', tier: 'high', order: 1 },
  sonnet: { family: 'sonnet', anthropic: 'Sonnet', tier: 'medium', order: 2 },
  haiku: { family: 'haiku', anthropic: 'Haiku', tier: 'low', order: 3 },
};
...
/**
 * Resolve a stored value and return the reviewed request-shaping capabilities for
 * the concrete id that callers will actually send to Anthropic.
 */
function resolveModelWithCapabilities(value) {
  const rawModel = value || null;
  const resolvedId = resolveModel(value) || rawModel;
  return {
    rawModel,
    model: resolvedId,
    resolvedId,
    isTier: isTier(value),
    capabilities: requestCapabilitiesForModel(resolvedId),
  };
}
