 ...
 *   skipped:number, invalid:number, droppedByCap:number}>}
 */
async function ingestDrainEntries(entries) {
  const counts = {
    considered: 0, stored: 0, duplicates: 0, skipped: 0, invalid: 0, droppedByCap: 0,
  };
  ...
  for (const entry of considered) {
    ...
    if (counts.stored >= MAX_ENTRIES_STORED) {
      counts.droppedByCap += 1;
      continue;
    }
    const structured = classified.structured;
    const recorded = await OperationalEventService.recordEvent({
      source: 'vercel-drain',
      ...
      // adversarial finding, 2026-08-19). 'info' is reserved for genuine
      // non-failures, which this ingester never stores.
      status: 'open',
      occurredAt: Number.isFinite(entry.timestamp) ? entry.timestamp : undefined,
      environment: shortString(entry.environment, 20),
    });
    if (recorded) counts.stored += 1;
    else counts.duplicates += 1; // duplicate delivery or best-effort insert failure
  }
