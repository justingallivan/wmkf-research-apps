...
async function findRow(client, systemuserid, key) {
  ...
  // non-string key BEFORE the escape to preserve the fail-closed throw the
  // hand-rolled `.replace` produced, rather than let odata.escape silently
  // coerce it via String().
  if (typeof key !== 'string') throw new TypeError('key must be a string');
  const filter = `_ownerid_value eq ${systemuserid} and wmkf_preferencekey eq '${odata.escape(key)}'`;
...
async function getUserPreferences(profileId, includeDecrypted = false) {
  try {
    assertDataverseAccess('prefs:read');
...
async function setUserPreference(profileId, key, value, isEncrypted = null) {
  assertDataverseAccess('prefs:write');
  try {
    const user = await resolveProfileToSystemUser(profileId);
