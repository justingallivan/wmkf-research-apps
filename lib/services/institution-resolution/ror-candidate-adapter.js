...
function createRorCandidateUnionAdapter({
  ...
  async function fetchOnce(url, signal, budget) {
    ...
    const headers = {
      ...
      'User-Agent': 'WMKF-Institution-Resolver/1.0',
    };
  ...
  async function lookup({ url, strategy, resolution, parse }) {
    ...
    // A 2xx response is not cacheable until its strategy-specific shape and
    // every returned candidate have passed validation. Otherwise one malformed
    // provider response would poison all later request-scoped retries.
    cache.set(requestKey(url), response.body);
    return {
      candidates,
      response,
      strategy,
