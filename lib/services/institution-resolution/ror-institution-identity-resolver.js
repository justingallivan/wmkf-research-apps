 ...
 * hydration consumes the caller's unified OpenAlex request budget when one is
 * supplied. Provider failures or budget exhaustion degrade to null without
 * poisoning the request cache; caller cancellation propagates.
 */
...
function createRorInstitutionIdentityResolver({
  ...
  async function resolve(affiliation, {
    ...
    let hydrated;
    try {
      const requestScope = openAlexRequestBudget?.begin('institution_hydration');
      counters.providerHydrations += 1;
      hydrated = await openAlexService.getInstitution(selectedRor, { signal, requestScope });
      throwIfAborted(signal);
    } catch (error) {
