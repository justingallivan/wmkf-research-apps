...
const WEB_RESULTS_MAX_CHARS = 20_000;
...
class WebDiscoveryService {
  ...
  static async search(input = {}, deps = {}) {
    ...
    try {
      ...
      if (results.length === 0) return { webLeads: [], queriesRun, fromCache };
  ...
  static async _perplexitySearch(query, { apiKey, signal, _fetch }) {
    ...
    const body = {
      ...
      // Bias toward recent work. Format pending the live contract test (the plan
      // requires it before enabling in prod); M/D/YYYY is Perplexity's documented
      // date-filter format.
