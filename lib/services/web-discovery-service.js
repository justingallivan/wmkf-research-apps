 ...
 *    rationale, date, source:'web'}` for a SEPARATE UI panel. It does NOT touch
 *    `/discover`, the candidate pipeline, ranking, roster, or save. It DOES drop
 *    leads matching `excludeNames` (the proposal's own PI/Co-Is + caller
 *    exclusions) so a conflict is never shown, and caps one lead per source URL.
...
// At most N leads per source URL: a single faculty/directory page must not
// contribute a pile of names (S230 — the old behavior scraped a whole roster).
// 2 (not 1) so a genuine two-author paper page isn't truncated to one.
const MAX_LEADS_PER_URL = 2;

// COI matching for the web path is deliberately LOOSER than the candidate
// pipeline's exact normalized-name match: a display-only lead is harmless to
// over-filter (hiding a non-conflict) but must never UNDER-filter and surface a
// real conflict (S230: a Co-PI surfaced). Beyond exact match we also match on
// "<surname> <first-initial>", which catches initials ("A. Leung"), middle
// initials ("Anthony C. Leung"), and hyphen-spacing ("Smith-Jones" vs
// "Smith Jones") — variants the exact matcher misses (Codex review).
function coiNormalize(name) {
  // Pre-map hyphens/slashes to spaces (the canonical normalizer strips them to
  // nothing, fusing "Smith-Jones" into "smithjones"), then reuse it for
  // diacritic/honorific/lowercase handling.
  return normalizeReviewerName(String(name || '').replace(/[/-]/g, ' '));
}
function coiKey(normalized) {
  const parts = String(normalized || '').split(' ').filter(Boolean);
  if (parts.length < 2) return null; // need a given name + surname to key
...
class WebDiscoveryService {
   ...
   * @param {string[]} [input.excludeNames]    - names to drop from leads (proposal PI/Co-Is + caller exclusions)
  ...
  static async search(input = {}, deps = {}) {
    ...
    try {
      ...
      // 3) Parse `NAME: x | SOURCE: n | WHY: …` → map the number to the REAL
      //    results[].url and attach the per-person rationale.
      const extracted = this._parseExtraction(text, results);

      // 4) COI/exclude filter — read-only leads must never surface a conflict
      //    (S230: a Co-PI was surfaced). Exact normalized match PLUS a looser
      //    surname+first-initial match (see coiKey) so name variants don't slip
      //    a proposal PI/Co-I through.
      const exactSet = new Set();
      const keySet = new Set();
      for (const n of (Array.isArray(excludeNames) ? excludeNames : [])) {
        const norm = coiNormalize(n);
        if (!norm) continue;
        exactSet.add(norm);
        const k = coiKey(norm);
        if (k) keySet.add(k);
      ...
      for (const lead of extracted) {
        if (isConflict(lead.name)) { conflictedCount += 1; continue; }
        notConflicted.push(lead);
      }

      // 5) One lead per source URL — a directory/roster page can't contribute
      //    multiple names. Best-match order is preserved (extraction lists best first).
      const perUrl = new Map();
      const webLeads = [];
      for (const lead of notConflicted) {
        const n = perUrl.get(lead.provenanceUrl) || 0;
        if (n >= MAX_LEADS_PER_URL) continue;
        perUrl.set(lead.provenanceUrl, n + 1);
        webLeads.push(lead);
      }

      // Funnel trace (counts only): raw results → extracted → after COI filter →
      // after per-URL cap. Makes recall + the filter's effect observable in logs.
      console.log('[WebDiscoveryService] funnel', JSON.stringify({
        queriesRun, fromCache, rawResults: results.length,
        extracted: extracted.length, conflictsFiltered: conflictedCount, webLeads: webLeads.length,
      }));
      return { webLeads, queriesRun, fromCache };
    } catch (err) {
  ...
  /**
   * Parse `NAME: <name> | SOURCE: <n> | WHY: <rationale>` lines (WHY optional for
   * tolerance). The url/date come ONLY from `results[n-1]` — never from anything
   * the model wrote — so a hallucinated URL can never become a provenanceUrl. The
   * rationale IS model-authored (a short relevance note, shown instead of the raw
   * page snippet); it carries no provenance weight. Dedupes by name. The name and
   * WHY captures exclude `|` so a stray pipe can't bleed across fields.
   */
  static _parseExtraction(text, results) {
    const out = [];
    const seenNames = new Set();
    if (!text) return out;
    const re = /NAME:\s*([^|]+?)\s*\|\s*SOURCE:\s*(\d+)\s*(?:\|\s*WHY:\s*([^|\n]*))?/gi;
    let m;
    while ((m = re.exec(text)) !== null) {
      ...
      seenNames.add(key);
      const rationale = (m[3] || '').trim().slice(0, 200) || null;
      const r = results[idx];
      out.push({ name, provenanceUrl: r.url, rationale, date: r.date, source: 'web' });
    }
    return out;
  }
