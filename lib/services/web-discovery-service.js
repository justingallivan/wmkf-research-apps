...
import { partitionByExcluded } from '../utils/reviewer-name-match.js';
...
// One lead per source URL: a single faculty/directory page must not contribute
// multiple names (S230 — the old behavior scraped several names off one roster).
const MAX_LEADS_PER_URL = 1;
...
class WebDiscoveryService {
  ...
  static async search(input = {}, deps = {}) {
    ...
    try {
      ...
      //    (S230: a Co-PI was surfaced). Reuses the SAME matcher the candidate
      //    pipeline uses (normalized-name exact match, never fuzzy).
      const { kept: notConflicted, removed: conflicted } = partitionByExcluded(
        extracted, Array.isArray(excludeNames) ? excludeNames : [], (l) => l.name,
      );
      ...
      console.log('[WebDiscoveryService] funnel', JSON.stringify({
        ...
        extracted: extracted.length, conflictsFiltered: conflicted.length, webLeads: webLeads.length,
      }));
      return { webLeads, queriesRun, fromCache };
    } catch (err) {
