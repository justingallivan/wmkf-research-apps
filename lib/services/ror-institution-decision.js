...
function explicitLocationConflict(candidate, input, features) {
  ...
  const possibleLocations = parts.filter((part) => {
    ...
    if (/\b(department|dept|division|faculty|program|school|university|college|institute|hospital|laboratory|lab|center|centre|office of the president)\b/.test(part)) {
      return false;
    }
...
function canonicalParent(best, evaluations, input) {
  ...
  const parentEvaluation = evaluations.find((evaluation) => evaluation.ror_id === parents[0]);
  return parentEvaluation?.vetoes.length === 0 ? parentEvaluation.candidate : null;
}
...
function decideSingle(input, candidateSet) {
  ...
  const canonicalParentCandidate = canonicalParent(best, evaluations, input);
