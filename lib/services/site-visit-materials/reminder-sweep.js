 ...
 * readable link, the optional site-visit read) is resolved BEFORE the claim;
 * only the transport and the receipt write follow it. A stamped reminder with
 * no email id therefore means either a transport failure (`sendFailed`) or a
 * delivered email whose id could not be attached (`receiptFailed`, the id is
 * in the log for repair); the two are counted separately.
 *
...
export const DEFAULT_DEPENDENCIES = Object.freeze({
  ...
  readEmailDefaults: COLLECTION_DEPENDENCIES.readEmailDefaults,
  prepareReminder: (args, readEmailDefaults = COLLECTION_DEPENDENCIES.readEmailDefaults) =>
    prepareMaterialsReminderEmail(args, { ...COLLECTION_DEPENDENCIES, readEmailDefaults }),
...
function emptyResult(dryRun) {
  return { dryRun, eligibilityIsProvisional: dryRun, scanned: 0, eligible: 0, sent: 0, skippedNothingMissing: 0, skippedNoRecipient: 0, skippedNoSender: 0, skippedLinkUnreadable: 0, skippedTestRequest: 0, claimLost: 0, sendFailed: 0, sendUnconfirmed: 0, receiptFailed: 0, errors: [] };
}
...
export async function sweepMaterialsReminders({ maxBatch = 100, dryRun = false } = {}, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  // The batch budget counts only rows past the test-request check, so skipped
  // test collections (which stay due) can never crowd out ordinary ones.
  const rows = await dependencies.listDue(now);
  let defaultsPromise;
  const readEmailDefaultsOnce = (...args) => {
    defaultsPromise ??= dependencies.readEmailDefaults(...args);
    return defaultsPromise;
  };

  for (const row of rows) {
    if (result.scanned >= maxBatch) break;
    const label = row.id;
    try {
      // Test requests get no materials reminders (Stage 1b): skip before any
      // read, preparation or claim. An unreadable marker skips this run only.
      if (dependencies.isolationEnabled?.()) {
        const testState = await dependencies.resolveTestState(row.request_id);
        if (testState.kind !== 'ordinary') {
          result.skippedTestRequest += 1;
          if (testState.kind === 'unknown') result.errors.push({ id: label, error: 'request could not be confirmed as ordinary' });
          continue;
        }
      }
      result.scanned += 1;
      ...
      if (!['pi', 'liaison'].every((role) => String(row.contacts?.[role]?.email || '').trim())) { result.skippedNoRecipient += 1; result.errors.push({ id: label, error: 'collection needs both project leader and primary contact emails' }); continue; }
      ...
      // The visit only decorates the email body (date and zone); the reminder
      // sends without it, so a failed read must not consume the claim.
      const visit = await dependencies.findActiveSiteVisit(row.request_id).catch((visitError) => {
        console.error('[site-visit-materials] automatic reminder: site visit read failed, sending without it:', label, visitError?.message || visitError);
        return null;
      });
      // Dry runs avoid the settings read (which can notify ops) and signature
      // resolution. Their eligibility count is therefore only a preflight.
      if (dryRun) { result.eligible += 1; continue; }

      // Required defaults, link, and optional PC signature must resolve before
      // the at-most-once claim; a configuration failure leaves the row retryable.
      const prepared = await dependencies.prepareReminder({ row, request, visit, missing, actorId: sender.systemUserId }, readEmailDefaultsOnce);
      result.eligible += 1;
      ...
      let emailId;
      try {
        emailId = await dependencies.sendReminder({
          row: claimed, prepared,
        ...
        const classification = classifyEmailDispatchError(sendError);
        if (classification.outcome === 'uncertain') result.sendUnconfirmed += 1;
        else result.sendFailed += 1;
        result.errors.push({ id: label, outcome: classification.outcome, error: sendError?.message || String(sendError) });
      ...
      try {
        ...
        // Delivered; only the receipt is missing. The id is logged for repair.
        result.receiptFailed += 1;
        result.errors.push({ id: label, error: `sent as ${emailId}; receipt not attached: ${receiptError?.message || String(receiptError)}` });
        console.error('[site-visit-materials] automatic reminder sent but email id not attached:', label, emailId, receiptError?.message || receiptError);
      }
    } catch (error) {
