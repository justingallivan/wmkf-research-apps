...
export const DEFAULT_DEPENDENCIES = Object.freeze({
  ...
  prepareReminder: (args) => prepareMaterialsReminderEmail(args, COLLECTION_DEPENDENCIES),
  sendReminder: (args) => sendReminderEmail(args, COLLECTION_DEPENDENCIES),
  now: () => new Date(),
});

function emptyResult(dryRun) {
  return { dryRun, scanned: 0, eligible: 0, sent: 0, skippedNothingMissing: 0, skippedNoRecipient: 0, skippedNoSender: 0, skippedLinkUnreadable: 0, claimLost: 0, sendFailed: 0, receiptFailed: 0, errors: [] };
}
...
export async function sweepMaterialsReminders({ maxBatch = 100, dryRun = false } = {}, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      const prepared = await dependencies.prepareReminder({ row, request, visit, missing, actorId: sender.systemUserId });
      result.eligible += 1;
