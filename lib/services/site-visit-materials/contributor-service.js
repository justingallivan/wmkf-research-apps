...
export async function finalizeMaterialUpload({ collection, slotKey, file }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const generationKey = crypto.createHash('sha256').update(`site-visit-materials:${request.akoya_requestid}:${slot}:${file.stagingId || sha256}`).digest('hex');
  ...
  if (existingRow?.wmkf_requestdocumentid) {
    await supersedePrevious(previous, existingRow.wmkf_requestdocumentid, dependencies);
    return { ok: true, slot, filename: existingRow.wmkf_filename, receivedAt: existingRow.wmkf_sharepointlastmodified || existingRow.modifiedon || null, artifactId: existingRow.wmkf_requestdocumentid, sha256, replayed: true };
  }
