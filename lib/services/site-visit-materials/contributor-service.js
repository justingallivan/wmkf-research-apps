...
import { SITE_VISIT_MATERIALS_FOLDERS, SITE_VISIT_MATERIALS_OTHER_UPLOADS_ENABLED } from '../../../shared/config/siteVisitMaterials.js';
...
/** Best-effort staff signal; a logging failure never changes the finalize outcome. */
async function recordReplayAmbiguous(dependencies, event) {
  try {
    if (typeof dependencies.recordEvent === 'function') await dependencies.recordEvent(event);
  } catch (error) {
    console.error('[site-visit-materials] replay_ambiguous event not recorded:', error?.message || error);
  }
}
...
export async function finalizeMaterialUpload({
  ...
  const slot = slotKey === 'other'
    ? (dependencies.otherUploadsEnabled?.() ? 'other' : null)
    : (collection.checklist || []).find((item) => item.key === slotKey && !item.waived)?.key;
  ...
  try {
    ...
    if (existing?.records?.length) {
      if (!replayIsBound({ row: existingRow, candidate: candidateResult, requestId: request.akoya_requestid, slot, generationKey, current })) {
        // Staff signal (plan §16.3, PR 3): the staging row stays held and the
        // applicant sees "needs staff attention"; without a durable event
        // nobody on staff would know. Best-effort, keyed on the staging id
        // so retries fold into one open event.
        await recordReplayAmbiguous(dependencies, {
          source: 'app',
          eventType: 'site_visit_material_replay_ambiguous',
          severity: 'error',
          transient: false,
          subsystem: 'site-visit-materials',
          stage: 'finalize_replay',
          summary: `Applicant upload for request ${request.akoya_requestnum} (${slot}) is held: the prior finalize left a registry row that does not bind to this operation. Reconcile the registry row and the staged upload by hand.`,
          requestNumber: request.akoya_requestnum,
          entityRefs: { requestId: request.akoya_requestid, collectionId: collection.id, stagingId, slot, registryRowId: existingRow?.wmkf_requestdocumentid || null },
          dedupeKey: `site_visit_material_replay_ambiguous:${stagingId}`,
        });
    ...
    }, { actorPolicy: REQUEST_DOCUMENT_ACTOR_POLICY.EXTERNAL_CONTRIBUTOR, actorContext: { operation: 'site-visit-materials-upload', requestId: request.akoya_requestid, slot } });
