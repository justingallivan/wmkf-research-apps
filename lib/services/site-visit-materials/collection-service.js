...
export async function remindMaterialsContributors({ requestId, actorId, fromEmail }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const emailId = await sendReminderEmail({ row: claimed, request, visit, missing, fromEmail, actorId, sequence: Number(claimed.reminder_count || 0) }, dependencies);
  await dependencies.attachReminderEmailId(row.id, emailId);
  return getMaterialsCollection({ requestId }, dependencies);
}

/**
 * The reminder email itself (PC action and the automatic sweep share it).
 * `sequence` is the reminder's ordinal, which keys the Dynamics correlation.
 * Throws when the sealed link cannot be read.
 */
export async function sendReminderEmail({ row, request, visit, missing, fromEmail, actorId, sequence }, dependencies = DEFAULT_DEPENDENCIES) {
  const url = unsealUrl(row, dependencies);
  if (!url) throw materialsError('The contributor link can no longer be read. Start a new collection.', 'site_visit_materials_link_unreadable');
