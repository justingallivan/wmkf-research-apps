...
async function sendInvitation({ row, request, visit, timeZone, url, recipients, fromEmail, actorId }, dependencies, { propagateConfigError = false } = {}) {
  try {
    const defaults = await requiredDefaults([INVITE_SUBJECT_KEY, INVITE_BODY_KEY], 'site-visit-materials:invitation', dependencies);
    const bodyText = invitationBodyText({
      bodyTemplate: defaults[INVITE_BODY_KEY],
      ...
      signature: await signatureForTemplate(defaults[INVITE_BODY_KEY], actorId, dependencies),
    });
    const emailId = await dependencies.sendEmail({
      subject: fillEmailTemplate(defaults[INVITE_SUBJECT_KEY], { proposalTitle: request.akoya_title || request.akoya_requestnum }),
      bodyText,
      url,
...
export async function inviteMaterialsContributors({ requestId, actorId, fromEmail }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const invited = await sendInvitation({ row, request, visit, timeZone: visit.wmkf_ianatimezone || 'America/Los_Angeles', url, recipients, fromEmail, actorId }, dependencies, { propagateConfigError: true });
...
export async function remindMaterialsContributors({ requestId, actorId, fromEmail }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const prepared = await prepareMaterialsReminderEmail({ row, request, visit, missing, actorId }, dependencies);
  ...
  const emailId = await sendReminderEmail({ row: claimed, prepared, fromEmail, actorId, sequence: Number(claimed.reminder_count || 0) }, dependencies);
 ...
 */
export async function prepareMaterialsReminderEmail({ row, request, visit, missing, actorId }, dependencies = DEFAULT_DEPENDENCIES) {
  const url = unsealUrl(row, dependencies);
  if (!url) throw materialsError('The contributor link can no longer be read. Start a new collection.', 'site_visit_materials_link_unreadable');
  const defaults = await requiredDefaults([REMINDER_SUBJECT_KEY, REMINDER_BODY_KEY], 'site-visit-materials:reminder', dependencies);
  return {
    subject: fillEmailTemplate(defaults[REMINDER_SUBJECT_KEY], { proposalTitle: request.akoya_title || request.akoya_requestnum }),
    bodyText: reminderBodyText({
      bodyTemplate: defaults[REMINDER_BODY_KEY],
      institution: institutionOf(request), title: request.akoya_title || 'your proposal',
      ...
      signature: await signatureForTemplate(defaults[REMINDER_BODY_KEY], actorId, dependencies),
    }),
    url,
  };
}
