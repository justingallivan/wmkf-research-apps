...
function requireMaterialRecipients(contacts) {
  ...
  if (missing.length) {
    throw materialsError(
      `Add an email address for the ${missing.map((role) => role === 'pi' ? 'project leader' : 'primary contact').join(' and ')} in AkoyaGo, then refresh the preview. Both recipients are required.`,
