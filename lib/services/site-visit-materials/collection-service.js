...
export async function createMaterialsCollection({ requestId, actorId, fromEmail }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const invitation = await sendInvitation({ row, request, visit, timeZone, url, recipients, fromEmail, actorId }, dependencies);
...
export async function inviteMaterialsContributors({ requestId, actorId, fromEmail }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const invitation = await sendInvitation({ row, request, visit, timeZone: visit.wmkf_ianatimezone || 'America/Los_Angeles', url, recipients, fromEmail, actorId }, dependencies, { propagateConfigError: true });
  if (!invitation.row) {
