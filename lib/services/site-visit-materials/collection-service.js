...
export async function sendReminderEmail({ row, prepared, fromEmail, actorId, sequence }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  // Materials emails carry no regarding record, so the delivery seam cannot
  // see them; refuse test requests here for both the manual remind and the
  // automatic reminder sweep (Stage 1b).
  await assertRequestEmailAllowed(row?.request_id);
