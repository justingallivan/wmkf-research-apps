...
function requireMaterialRecipients(contacts) {
  const missing = ['pi', 'liaison'].filter((role) => !String(contacts?.[role]?.email || '').trim());
  if (missing.length) {
    throw materialsError(
      `Add an email address for the ${missing.map((role) => role === 'pi' ? 'project leader' : 'primary contact').join(' and ')} on the request, then refresh the preview. Both recipients are required.`,
...
function snapshotContacts(resolved) {
  return Object.fromEntries(['pi', 'liaison'].map((role) => [role, resolved?.[role]
    ? { role, name: resolved[role].name || null, email: resolved[role].email || null }
    : null]));
}

async function currentContacts(requestId, dependencies) {
  const contacts = snapshotContacts(await dependencies.resolveRecipients(requestId));
...
export async function createMaterialsCollection({ requestId, actorId, profileId, fromEmail, emailTemplate, preparedEmail }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const contacts = await currentContacts(requestId, dependencies);
...
export async function remindMaterialsContributors({ requestId, actorId, profileId, fromEmail, emailTemplate, preparedEmail }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  if (prepared.names && preparedTemplate) assertPreparedNames(prepared, await namesForTemplate(preparedTemplate.body, request, dependencies, contactList(contacts)));
