...
export const DEFAULT_DEPENDENCIES = Object.freeze({
  ...
  now: () => new Date(),
  sendEmail: async ({ subject, bodyText, from, to, cc, correlationKey, actingUserSystemId }) => {
    const emailId = await DynamicsService.createEmailActivity({
      subject, body: renderPlainTextEmailHtml(bodyText), from, to, cc, correlationKey, actingUserSystemId, noFallback: true,
    });
    await DynamicsService.sendEmail(emailId, { actingUserSystemId });
    return emailId;
  },
...
export function invitationBodyText({ institution, title, visitStartIso, timeZone, dueAt, checklist, url, closesAt }) {
  ...
  return [
    ...
    items,
    '',
    'Upload here (no login needed; you may forward this link to a colleague who is helping):',
    url,
    '',
    `The link stays open until ${formatWhen(closesAt, timeZone)}. You can replace a file at any time before then.`,
    '',
    'Thank you.',
  ].join('\n');
}

export function reminderBodyText({ institution, title, visitStartIso, timeZone, dueAt, missing, url }) {
...
async function sendInvitation({ row, request, visit, timeZone, url, recipients, fromEmail, actorId }, dependencies) {
  const bodyText = invitationBodyText({
    ...
    visitStartIso: visit.scheduledstart, timeZone, dueAt: row.due_at, closesAt: row.closes_at,
    checklist: row.checklist, url,
  });
  try {
