...
/** Save only the contact set reviewed against the live request before a send. */
export async function updateContacts(id, expectedContacts, contacts) {
  const result = await sql`
    UPDATE site_visit_material_collections
       SET contacts = ${JSON.stringify(contacts)}::jsonb, updated_at = NOW()
     WHERE id = ${id} AND status <> 'closed'
       AND contacts = ${JSON.stringify(expectedContacts)}::jsonb
     RETURNING *
  `;
  return result.rows[0] || null;
}
...
/**
 * Claim-before-send for the PC's manual "Send reminder" (S507). Allowed at
 * any time on an open collection, any number of times — unlike the automatic
 * reminder's due-date predicate, this only serializes against a reminder
 * (manual or automatic) stamped in the last 60 seconds, so a PC click during
 * the daily cron run can never double-send. Null when the claim is lost.
 */
export async function claimManualReminder(id, at = new Date(), expectedContacts, contacts) {
  const atIso = at.toISOString();
  const result = await sql`
    UPDATE site_visit_material_collections
       SET last_reminder_at = NOW(), updated_at = NOW(), reminder_count = reminder_count + 1,
           contacts = ${JSON.stringify(contacts)}::jsonb
     WHERE id = ${id}
       AND status = 'open'
       AND contacts = ${JSON.stringify(expectedContacts)}::jsonb
       AND (last_reminder_at IS NULL OR last_reminder_at < ${atIso}::timestamptz - interval '60 seconds')
     RETURNING *
  `;
  return result.rows[0] || null;
}
