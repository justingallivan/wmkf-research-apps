...
/**
 * Verdict-free ROR candidate-set contract for production institution resolution.
 *
 * The provider boundary may return retrieval evidence only. Selection, vetoes,
 * and decision authority belong to the local claim resolver.
 */
...
function assertCandidateSet(value) {
  ...
  for (const candidate of value.candidates) {
    ...
    for (const source of candidate.retrieval) {
      for (const key of FORBIDDEN_DECISION_KEYS) {
        if (source && Object.prototype.hasOwnProperty.call(source, key)) {
          throw new Error(`candidate retrieval must not contain decision field ${key}`);
        }
      }
    }
  }
  ...
  for (const key of FORBIDDEN_DECISION_KEYS) {
    if (Object.prototype.hasOwnProperty.call(value.provenance, key)) {
      throw new Error(`candidate set provenance must not contain decision field ${key}`);
    }
  }
...
function createCandidateSet({ provider, candidates, provenance = {} }) {
  ...
  for (const candidate of Array.isArray(candidates) ? candidates : []) {
    const normalized = candidate?.ror_id
      ? candidate
      : normalizeCandidate(candidate.organization, candidate.retrieval);
