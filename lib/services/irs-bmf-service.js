...
const { getPool } = require('../postgres/client');
...
async function verifyEin(rawEin) {
  ...
  // Lifecycle change (recorded, plan §5 Stage 3 item 4): getPool() is a
  // process-lifetime singleton with no `.end()` on its facade, so this no
  // longer ends the pool per call — it borrows the shared seam pool for
  // the warm instance's lifetime instead of building/ending its own.
  const pool = getPool();
...
async function refresh({ dryRun = false, strict = false } = {}) {
  ...
  // Lifecycle change (recorded, plan §5 Stage 3 item 4): getPool()'s
  // facade has no `.end()` (Codex Stage 2 finding — a raw Pool export
  // would let any importer poison the process-wide singleton for every
  // other caller), so this no longer ends the pool per call — it holds
  // the COPY client from the shared seam pool instead of a per-call Pool.
