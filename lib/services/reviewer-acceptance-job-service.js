...
function buildPayload({
  ...
  return {
    ...
    reviewerPortalTokenEncrypted: reviewerPortalToken ? encrypt(reviewerPortalToken) : null,
    body: {
...
export async function enqueueReviewerAcceptanceJob({
  ...
  const result = await observeReviewerFindWarmEffect({
    ...
    operation: 'reviewer_acceptance_job_enqueue',
  }, () => observeReviewerFindWarmEffect({
    effectClass: 'postgres_write',
    operation: 'reviewer_acceptance_job_insert',
  }, () => sql`
...
export async function cancelReviewerAcceptanceJobsForSuggestion(suggestionId, reason) {
  const error = reason ? String(reason).slice(0, MAX_ERROR_LENGTH) : null;
  const result = await sql`
    UPDATE reviewer_acceptance_jobs
       SET status = 'cancelled',
     ...
     WHERE suggestion_id = ${suggestionId}
       AND status = ANY(${REVIEWER_ACCEPTANCE_JOB_ACTIVE_STATUSES})
       AND (lease_token IS NULL OR locked_until < NOW())
     RETURNING *
  `;
  return result.rows;
}
