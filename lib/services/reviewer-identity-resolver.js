...
function classifySpineEvidence(hypothesis, evidence, anchors) {
  ...
  // S236: forename-gate the affiliation/topic promotions. Without this, a fabricated
  // wrong-forename suggestion ("John Smith" for the real "Jane Smith") that lands an
  // affiliation_match + topic_match on a same-surname namesake was promoted to
  // confirmed/probable → verified → selectable (the fail-dangerous hazard in
  // project-reviewer-verify-fail-dangerous, widened once non-biomedical Track-A routes
  // here). The Track-A spine always computes `forenameAgrees` (true/false), so a
  // wrong-forename record fails the gate and falls through to `unresolved`. Other
  // resolveIdentity callers (reviewer-work-author-resolver, contact-enrichment) leave
  // `forenameAgrees` UNDEFINED — so the gate is `!== false` (block only an explicit
  // disagreement), never `=== true`, to avoid demoting those callers. Mirrors the
  // existing employment-path gate at the orcid_employment_corroborated promotion below.
  if (strongAffiliation && employment && topic && spine.forenameAgrees !== false) {
    return { status: 'confirmed', competitors: [] };
  }
  if (((anyAffiliation && topic) || strongAffiliation) && spine.forenameAgrees !== false) {
    return { status: 'probable', competitors: [] };
  }
