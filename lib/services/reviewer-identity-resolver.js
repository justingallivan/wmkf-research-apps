...
function evidenceFromEnrichment(contactEnrichment = {}, hypothesis = {}) {
  ...
  // writes this key ONLY for an author it accepted — ORCID hard-key, or a spine
  // verdict that already cleared mayPersistIdentity + the forename gate. Shape:
  // { openAlexId, displayName, lastKnownInstitution, ror, acceptPath:'orcid'|'spine',
  //   identityStatus, forenameContradicts, hIndex, i10Index, citedByCount }.
  // The resolver reads only the identity fields; metrics ride along for 1b's use.
...
//   - acceptPath 'orcid' → the author was resolved via OpenAlexService.getAuthorByOrcid:
//     ORCID is the hard identity key (the record's OWN ORCID), so there is no namesake
//     question — STRONG, never forename- or status-gated here.
//   - acceptPath 'spine' → resolved via the OpenAlex/ORCID identity spine, which already
//     reached mayPersistIdentity (probable/confirmed) with its own forename gate. Reflect
//     it STRONG, but FAIL CLOSED (a rejected anchor → no support → unresolved) if a
//     non-persist status or a full-forename contradiction somehow slips through. The
//     producer only writes accepted authors, so the fail path is defense-in-depth.
// A failing anchor lands in rejectedAnchors and does NOT count toward classification,
// so a borderline author yields `unresolved` (abstain) — the safe outcome.
function openAlexAuthorAnchor(a) {
  if (!a || !a.openAlexId) return null;
  const shortId = String(a.openAlexId).replace(/^https?:\/\/openalex\.org\//i, '');
  const orcidPath = a.acceptPath === 'orcid';
  const gateFail = orcidPath
    ? null
    : a.forenameContradicts === true
      ? 'forename_contradiction'
      : (a.identityStatus && !mayPersistIdentity(a.identityStatus))
        ? `identity_${a.identityStatus}`
        : null;
  const anchor = {
    ...
    parserOutput: {
      ...
      acceptPath: orcidPath ? 'orcid' : 'spine',
