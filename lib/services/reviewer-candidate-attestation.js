 ...
 * projection was accepted from a valid automated-resolver attestation. The
 * roster route strips client copies before creating or preserving this value;
 * consumers must still compare the embedded projection to the current row.
 */
export function createServerIdentityDecisionReceipt(candidate = {}) {
  return {
    version: 1,
    source: 'automated_resolver',
    projection: identityAttestationProjection(candidate),
  };
}
...
export function hasServerIdentityDecisionReceipt(candidate = {}) {
  ...
  return JSON.stringify(receipt.projection) === JSON.stringify(identityAttestationProjection(candidate));
}
