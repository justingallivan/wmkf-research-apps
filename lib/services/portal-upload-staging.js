 ...
 * generation key exists (conservative — any row, per plan §4). Unbound
 * (no row) discards the candidate directly.
 */
async function reconcileSiteVisitMaterialCandidate(row, candidate, dependencies) {
  ...
  if (registry?.records?.length > 0) return { action: 'bound' };
  return (await safeDiscard(candidate, row, dependencies)) ? { action: 'cleared' } : { action: 'retain' };
}
 ...
 * `grantee_image`/`staff_grantee_image` candidates carry an image ref, not a
 * generation key; no safe proof is wired for them yet, so they are always
 * retained rather than risk discarding a bound image. Any shape this
 * function does not recognise is retained and logged, never discarded.
 */
async function reconcileCandidate(row, dependencies) {
  ...
  if (row.scope === PORTAL_UPLOAD_SCOPES.GRANTEE_IMAGE || row.scope === PORTAL_UPLOAD_SCOPES.STAFF_GRANTEE_IMAGE) {
    console.warn('[portal-upload-staging] grantee-image candidate retained (no binding proof wired yet)', { id: row.id, scope: row.scope });
    return { action: 'retain' };
  }
  console.warn('[portal-upload-staging] unrecognised candidate retained', { id: row.id, scope: row.scope });
  return { action: 'retain' };
...
export async function cleanupExpiredPortalUploads({ retentionDays = 7 } = {}, dependencies = DEFAULT_CLEANUP_DEPENDENCIES) {
  ...
  const eligible = await sql`
    SELECT id, pathname, scope, status, candidate_result
