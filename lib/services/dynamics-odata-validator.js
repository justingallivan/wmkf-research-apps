...
// $expand segments are `nav`, `nav($select=...;$filter=...)` or the path form
// `nav/child`. Only the ROOT navigation property belongs to THIS table;
// everything after the first `/`, and everything inside the parentheses, names
// the RELATED table and must never be existence-checked here.
//
// The root is kept for the path form too: appending a path cannot turn a scalar,
// PartyList or computed-value property into a navigation property, so a root
// that is provably wrong bare is still provably wrong with a suffix attached.
