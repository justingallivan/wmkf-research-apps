 ...
 *
 * Verification always writes a server-owned Postgres roster receipt. When an
 * exact stable person has a pending conflict, the same action also resolves the
 * person-scoped Dataverse state before clearing the roster block. Repair
 * requests are durable system_alerts rows and never unblock a candidate alone.
 */
...
import { reconcileReviewerContacts } from './reviewer-contact-reconciliation';
...
async function resolveConflictPerson(requestId, candidate) {
  ...
  if (!orcid || !candidate?.name) return null;
  const outcome = await lookupReviewerIdentity({
    name: candidate.name,
    email: null,
    orcid,
  }, { allowNameFallback: false });
  if (outcome?.outcome !== 'confident' || outcome.match?.matchKey !== 'orcid'
    || outcome.match?.nameConsistent !== true || !outcome.match?.reviewerId) return null;
  const person = await potentialReviewerAdapter.getById(outcome.match.reviewerId);
  return person ? { personId: outcome.match.reviewerId, person } : null;
}
...
export async function getAddressConflict({ requestId, candidateKey }) {
  ...
  if (!parsed?.valid || parsed.state.status !== 'conflict_pending') {
    return withRemediation({
      ...
      decision: 'blocked',
      code: 'candidate_stale',
      message: 'The address conflict is no longer current. Retry the check or reload.',
    });
  }
  return {
    success: true,
...
export async function verifyPersonAndAddress({
  ...
  const rosterCandidate = await requireRosterCandidate(requestId, candidateKey);
  if (!rosterCandidate) {
    return withRemediation({
      success: false,
      decision: 'blocked',
...
export async function retryAddressCheck({
  ...
  if (resolved?.person) {
    ...
    const foundEmail = normalizeAddress(
    ...
    const projectResolved = () => {
      ...
      if (refreshed.applicantKnownReviewer) {
        refreshed.applicantKnownReviewer = {
          ...refreshed.applicantKnownReviewer,
          email: storedEmail,
          emailSource: resolved.person.wmkf_emailsource || 'staff_verified',
          addressConflictPending: false,
    ...
    if (currentTrust.valid && currentTrust.state.status === 'conflict_pending') {
      ...
      exactRetryHandled = true;
    } else if (canRecordContradiction) {
  ...
  if (!exactRetryHandled) {
    await reconcileReviewerContacts([refreshed], {
      requestId,
      actingUserSystemId: actorSystemUserId,
    });
  }
  if (refreshed.conflictRecordUnavailable === true) {
    return withRemediation({
      success: false,
      decision: 'blocked',
      code: 'conflict_record_unavailable',
