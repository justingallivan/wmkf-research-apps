...
const REPAIR_ALERT_TYPE = 'reviewer_address_repair_requested';
const STRUCTURAL_RETRY_CODES = new Set([
  'person_inactive',
  'email_conflict',
...
function repairKey(requestId, candidateKey) {
...
export async function listOpenAddressRepairRequests({ requestId, candidateKeys }) {
  const allowedKeys = new Set((Array.isArray(candidateKeys) ? candidateKeys : [])
    .filter((key) => typeof key === 'string' && key.length > 0));
  if (!requestId || allowedKeys.size === 0) return [];

  const alerts = await AlertService.getOpenAlertsByTypeAndRequestId(REPAIR_ALERT_TYPE, requestId);
  const seen = new Set();
  const requests = [];
  for (const alert of alerts) {
    const candidateKey = alert?.metadata?.candidateKey;
    if (!allowedKeys.has(candidateKey) || seen.has(candidateKey)) continue;
...
function matchesRepairCorrelation(alert, {
  candidateKeys,
  suggestionIds,
  potentialReviewerIds,
}) {
  const metadata = alert?.metadata || {};
  return candidateKeys.has(metadata.candidateKey)
    || suggestionIds.has(String(metadata.suggestionId || '').toLowerCase())
    || potentialReviewerIds.has(String(metadata.potentialReviewerId || '').toLowerCase());
}

async function closeResolvedAddressRepairAlerts({ requestId, candidateKey, candidate }) {
  const candidateKeys = new Set([candidateKey, candidate?.candidateKey].filter(Boolean));
  const suggestionIds = new Set();
  const potentialReviewerIds = new Set();
  if (candidate?.suggestionId) {
    suggestionIds.add(String(candidate.suggestionId).toLowerCase());
    candidateKeys.add(reviewerSuggestionCandidateKey(candidate.suggestionId));
  }
  if (candidate?.potentialReviewerId) {
    potentialReviewerIds.add(String(candidate.potentialReviewerId).toLowerCase());
  }

  let alerts;
  try {
    alerts = await AlertService.getOpenAlertsByTypeAndRequestId(REPAIR_ALERT_TYPE, requestId);
  } catch (error) {
    return {
      expectedCount: 0,
      resolvedCount: 0,
  ...
  const matched = alerts.filter((alert) => matchesRepairCorrelation(alert, {
    candidateKeys,
    suggestionIds,
    potentialReviewerIds,
  }));
  if (matched.length === 0) {
    return { expectedCount: 0, resolvedCount: 0, status: 'not_applicable' };
  }

  let resolvedCount = 0;
  const resolvedKeys = new Set();
  for (const alert of matched) {
    const key = typeof alert?.auto_resolve_key === 'string' ? alert.auto_resolve_key : '';
    if (!key || resolvedKeys.has(key)) continue;
    resolvedKeys.add(key);
    try {
      const count = await AlertService.autoResolve(key);
      if (Number.isInteger(count) && count > 0) resolvedCount += count;
    } catch {
      // The person and roster are already durably resolved. Alert cleanup is
      // best-effort metadata and must never turn that success into a retry that
      // could duplicate the adjudication.
    }
  }
  return {
    expectedCount: matched.length,
    resolvedCount,
    status: resolvedCount >= matched.length ? 'resolved' : 'incomplete',
  };
}
...
export async function verifyPersonAndAddress({
  ...
  if (suggestionId && !candidateKey) {
    ...
    const pendingConflict = currentTrust.valid && currentTrust.state.status === 'conflict_pending'
      ...
      : null;
    if (evidenceType === 'staff_address_choice' && !pendingConflict) {
      return withRemediation({
        success: false,
        decision: 'blocked',
        code: 'invalid_address_attestation',
        message: 'A stored-versus-found address choice requires a current address conflict. Reload before choosing.',
      });
    }
    ...
    const addressTrustStateJson = JSON.stringify(verifiedState);
    try {
      ...
      if (translateDuplicateKeyError(error)) {
        return withRemediation({
          ...
          decision: 'blocked',
          code: 'email_conflict',
          message: 'That address belongs to another reviewer record. Fix the duplicate email owner in AkoyaGO, then use Retry record check.',
        });
      }
    ...
    const alertCloseout = pendingConflict
      ? await closeResolvedAddressRepairAlerts({
          requestId,
          candidateKey: stableCandidateKey,
          candidate: { suggestionId },
        })
      : { expectedCount: 0, resolvedCount: 0, status: 'not_applicable' };
    return {
      success: true,
      ...
      suggestionId,
      selectedDecision: resolution?.decision || null,
      personUpdated: true,
      alertCloseout,
      remediation: [],
    };
  ...
  if (pendingConflict) {
    ...
    const cleared = await clearAddressTrustBlocks(requestId, candidateKey, {
      ...
      addressChoice: staffAddressChoiceProjection(verifiedState, {
        email: exactEmail,
        requestId,
        candidateKey,
    ...
    const alertCloseout = await closeResolvedAddressRepairAlerts({
      requestId,
      candidateKey,
      candidate: cleared,
    });
    return {
      ...
      candidateKey,
      candidate: cleared,
      selectedDecision: resolution.decision,
      personUpdated: true,
      rosterCleared: true,
      alertCloseout,
      remediation: [],
...
export async function retryAddressCheck({
  ...
  const structuralCode = STRUCTURAL_RETRY_CODES.has(code) ? code : null;
  if (
    candidate.conflictRecordUnavailable !== true
    && candidate.addressConflictPending !== true
    && !structuralCode
  ...
  if (structuralCode) {
    let trusted = null;
    try {
      trusted = await resolveTrustedReviewerPerson(candidate, { allowInactive: true });
    } catch {
      return withRemediation({
        success: false,
        decision: 'blocked',
        code: 'identity_unavailable',
        message: 'The reviewer record could not be rechecked. Retry after confirming the AkoyaGO change was saved.',
      });
    }
    const resolved = trusted || await resolveConflictPerson(requestId, candidate);
    if (!resolved) {
      return withRemediation({
        success: false,
        decision: 'blocked',
        code: structuralCode,
        message: 'The exact reviewer record is still not available. Confirm the record in AkoyaGO, then retry.',
      });
    }
    if (!isActivePerson(resolved.person)) {
      return withRemediation({
        ...
        decision: 'blocked',
        code: 'person_inactive',
        message: 'The reviewer record is still inactive in AkoyaGO.',
      });
    }
    if (structuralCode === 'contact_linked_elsewhere' && !trusted) {
      return withRemediation({
        success: false,
        decision: 'blocked',
        code: structuralCode,
        message: 'The reviewer/contact linkage still conflicts in AkoyaGO.',
      });
    }
    if (new Set(['email_conflict', 'ambiguous_email_owner', 'inactive_email_owner']).has(structuralCode)) {
      const exactEmail = normalizeAddress(
        candidate.email
          || candidate.contactEnrichment?.email
          || resolved.person.wmkf_emailaddress,
      );
      let owner;
      try {
        owner = await potentialReviewerAdapter.findByEmailCandidates(exactEmail);
      } catch {
        return withRemediation({
          success: false,
          decision: 'blocked',
          code: 'identity_unavailable',
          message: 'Email ownership could not be rechecked. Retry after confirming the AkoyaGO change was saved.',
        });
      }
      const ownerId = owner?.id || owner?.row?.wmkf_potentialreviewersid;
      const ownerActive = owner?.one === true && isActivePerson(owner?.row);
      if (!ownerActive || !sameId(ownerId, resolved.personId)) {
        const unresolvedCode = owner?.ambiguous
          ? 'ambiguous_email_owner'
          : owner?.one === true && !isActivePerson(owner?.row)
            ? 'inactive_email_owner'
            : 'email_conflict';
        return withRemediation({
          success: false,
          decision: 'blocked',
          code: unresolvedCode,
          message: 'The email address still belongs to a different or inactive reviewer record in AkoyaGO.',
        });
      }
    }
    ...
    const refreshed = {
      ...candidate,
      potentialReviewerId: resolved.personId,
      ...
      contactEnrichment: {
        ...
        ...(structuralCode === 'contact_linked_elsewhere' ? {
    ...
    if (!recorded) {
      return withRemediation({
        ...
        decision: 'blocked',
        code: 'candidate_stale',
        message: 'The reviewer card changed while the record was being rechecked. Reload and try again.',
      });
    }
    ...
    return {
      ...
      message: 'The AkoyaGO record now passes this check. Add the reviewer to Invite again.',
      candidateKey,
      candidate: current,
      remediation: [],
    };
  ...
  const projectResolved = (person = resolved.person) => {
    ...
    const addressChoice = staffAddressChoiceProjection(parsedTrust.state, {
      email: storedEmail,
      requestId,
      candidateKey,
      receipt,
    ...
    refreshed.addressChoice = addressChoice;
  ...
  if (
    ...
    if (allowed.has(receiptEmail) && receiptCanResolveConflict(receipt, conflict)) {
      ...
      return {
        ...
        candidateKey,
        candidate: cleared,
        selectedDecision: verifiedState.resolution.decision,
        personUpdated: true,
        rosterCleared: true,
        alertCloseout,
        remediation: [],
...
export async function createAddressRepairRequest({
  ...
  const [existingRequest] = await listOpenAddressRepairRequests({
    requestId,
    candidateKeys: [repairCandidateKey],
  });
  if (existingRequest) {
    return {
      success: true,
      decision: 'repair_requested',
      code: typeof code === 'string' && code.trim() ? code.trim().slice(0, 100) : 'unknown',
      repairReference: existingRequest.alertId,
      repairRequest: existingRequest,
      message: 'A repair request is already pending. The reviewer remains blocked until it is resolved.',
      remediation: [],
    };
  }
  ...
  const alert = await NotificationService.notify({
    ...
    category: 'reviewers',
    autoResolveKey: repairKey(requestId, repairCandidateKey),
  });
  let repairRequest = alert ? {
    alertId: alert.id,
    candidateKey: repairCandidateKey,
    status: 'active',
  } : null;
  if (!repairRequest) {
    [repairRequest] = await listOpenAddressRepairRequests({
      requestId,
      candidateKeys: [repairCandidateKey],
    });
  }
  return {
    ...
    decision: 'repair_requested',
    code: normalizedCode,
    repairReference: repairRequest?.alertId || repairKey(requestId, repairCandidateKey),
    repairRequest,
    message: alert
