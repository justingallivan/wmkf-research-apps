...
export async function retryAddressCheck({
  ...
  if (receipt?.personConfirmed === true && receiptEmail) {
    ...
    if (currentTrust.valid && currentTrust.state.status === 'conflict_pending') {
      ...
      // The original conflict write failed before a person bundle existed. Retry
      // now has both exact sides plus the staff receipt, so persist the adjudicated
      // A/B pair on the stable person before clearing either roster block.
      if (!resolved.person._etag) {
        return withRemediation({
          success: false,
          decision: 'blocked',
          code: 'candidate_stale',
          message: 'The reviewer version could not be confirmed. Reload before applying the recorded evidence.',
        });
      }
      const conflict = {
        ...
        requestId,
        candidateKey,
        // The durable conflict write failed, so the retry observation and staff
        // adjudication share the receipt timestamp; no earlier durable timestamp
        // exists to claim.
        detectedAt: receipt.attestedAt,
      };
      const verifiedState = createStaffVerifiedState({
        email: receiptEmail,
        actorProfileId: receipt.actorProfileId,
        ...
        resolution: {
          conflict,
          decision: receiptEmail === storedEmail ? 'keep_stored' : 'use_found',
          actorProfileId: receipt.actorProfileId || null,
          actorSystemUserId: receipt.actorSystemUserId || null,
          resolvedAt: receipt.attestedAt,
        },
      ...
      try {
        ...
        if (isPreconditionFailure(error)) {
          return withRemediation({
            ...
            decision: 'blocked',
            code: 'candidate_stale',
            message: 'The reviewer changed while the recorded evidence was being applied. Reload and review the current address.',
          });
        }
        return withRemediation({
          success: false,
