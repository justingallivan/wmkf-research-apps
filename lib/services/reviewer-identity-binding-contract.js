...
// Mirrors RESOLVER_SOURCED_FIELDS. Kept explicit here because this contract is
// the future persistence authority; a focused test locks the two lists together
// until the writer migrates the resolver to import this constant directly.
export const IDENTITY_LINEAGE_FIELDS = Object.freeze([
...
export function validateIdentityBindingTuple({
  ...
  const timestamp = typeof boundAt === 'string' ? Date.parse(boundAt) : NaN;
  if (!Number.isFinite(timestamp) || new Date(timestamp).toISOString() !== boundAt) {
