...
const {
  normalizeRequestId,
  normalizeSessionId,
} = require('./dynamics-explorer-request-telemetry');

class FeedbackService {
  /**
   * Create a feedback record
   */
  static async createFeedback({ userProfileId, sessionId, requestId, feedbackType, category, userNote, queryText, conversationContext, autoDetected = false }) {
    try {
      const normalizedRequestId = normalizeRequestId(requestId);
      const normalizedSessionId = normalizeSessionId(sessionId);
      let verifiedRequestId = null;

      // Client correlation is evidence, never authority. Both session values
      // must be present and the request must belong to the authenticated user.
      // A missing migration/telemetry row or lookup outage leaves the feedback
      // uncorrelated without discarding it.
      if (normalizedRequestId && normalizedSessionId) {
        try {
          const verified = await sql`
            ...
            LIMIT 1
          `;
          verifiedRequestId = verified.rows[0]?.request_id || null;
        } catch (error) {
          console.warn('FeedbackService request correlation unavailable:', error.message);
        }
      }
      ...
      try {
        result = await sql`
          INSERT INTO dynamics_feedback (
            ...
            ${userProfileId}, ${normalizedSessionId}, ${feedbackType}, ${category || null},
            ${userNote || null}, ${queryText || null},
            ${conversationContext ? JSON.stringify(conversationContext) : null},
            ${autoDetected}, ${verifiedRequestId}
          )
          RETURNING *
        `;
      } catch (error) {
        if (error?.code !== '42703') throw error;
        // Deployment compatibility: if runtime reaches Production before the
        // additive migration, preserve the pre-Phase-B feedback write.
        result = await sql`
          INSERT INTO dynamics_feedback (
            user_profile_id, session_id, feedback_type, category,
            user_note, query_text, conversation_context, auto_detected
