...
class SerpContactService {
  ...
  /**
   * Run one plain Google query and return the bounded organic-result DTO used by
   * evidence classifiers. Callers own query construction and interpretation.
   */
  static async searchOrganicResults(query, apiKey = null, { signal, limit = 10 } = {}) {
    const key = apiKey || this.apiKey;
    if (!key || !query) return [];
    const cap = Math.min(10, Math.max(1, Number(limit) || 10));
    ...
    const response = await fetch(`${this.baseUrl}?${params}`, { signal });
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`SerpAPI ${response.status}: ${errorText.slice(0, 300)}`);
    }
   ...
   * @returns {Promise<Object|null>} { email, emailEvidence, facultyPageUrl, website } or null
   */
  static async findContact(candidate, apiKey = null) {
    ...
    try {
      ...
      const absorbResults = (items, sourceQuery) => {
        for (const item of items) {
          // Keep the evidence on the SAME organic result as the address. A
          // separately selected profile URL is not proof for a snippet email.
          if (!result.email && item.snippet) {
            ...
            if (email) {
              ...
              result.emailEvidence = {
                ...
                sourceTitle: typeof item.title === 'string' ? item.title.slice(0, 300) : null,
                citedText: item.snippet.slice(0, 800),
                query: sourceQuery,
          ...
          if (!result.facultyPageUrl && item.link) {
            ...
            } else if (!result.website && ContactParser.isUsefulWebsiteUrl(item.link, candidate.name)) {
      ...
      absorbResults(organicResults, query);

      // In evidence-experiment mode, an email-less profile page is a lead, not
      // a terminal result: continue until an address or the query budget ends.
      const evidenceExperiment = process.env.REVIEWER_EMAIL_EVIDENCE_EXPERIMENT_ENABLED === 'true';
      const shouldTryFallbacks = evidenceExperiment
        ? !result.email
        : (!result.email && !result.facultyPageUrl && !result.website);
      if (shouldTryFallbacks) {
        const fallbackQueries = [];

        if (institution) {
          fallbackQueries.push(`"${cleanName}" "${institution}" "email"`);
          fallbackQueries.push(`"${cleanName}" "${institution}" contact`);
        ...
        for (const fallbackQuery of fallbackQueries) {
          ...
          try {
            ...
            if (fallbackResponse.ok) {
              ...
              absorbResults(fallbackResults, fallbackQuery);
            }
          } catch (fallbackError) {
  ...
  static isFacultyPageUrl(url, candidateName) {
    ...
    // A document/media file is never a faculty page. The resolved-page email tier
    // fetches the captured facultyPageUrl, so a PDF here would mean a wasted/bad
    // fetch — share the same document gate as the website validator.
    if (ContactParser.isDocumentUrl(url)) return false;
   ...
   *
   * ⚠ RETIRED from the live enrichment path (Slice 1b, SerpAPI→free-stack migration):
   * ContactEnrichmentService now sources bibliometrics + the verified-email domain from
   * OpenAlex (`_attachOpenAlexMetrics`), NOT this method. `findScholarProfile` /
   * `findScholarProfileViaGoogle` / `fetchScholarMetrics` (and the helpers
   * institutionConflicts / extractScholarDisplayName / scholarNameMismatch) are KEPT only
   * because the dormant S215/S219 measurement+backfill scripts
   * (scripts/measure-scholar-orcid-crosstab.js, scripts/backfill-lone-orcid-scholar.js)
   * still reference them. `findContact` (Tier-4 #1) stays live. Do not re-wire these into
   * enrichment. See docs/REVIEWER_FINDER_SERPAPI_MIGRATION_PLAN.md.
   *
