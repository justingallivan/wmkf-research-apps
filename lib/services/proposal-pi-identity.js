...
async function fetchOrcidRegistryName(orcid, { signal } = {}) {
...
async function resolveProposalPI(requestId, { signal } = {}) {
  ...
  const registryName = await fetchOrcidRegistryName(orcid, { signal }); // throws on abort
...
module.exports = {
  ...
  excludePiIdentity,
  appendPiName,
  _internals: { evaluateNameGuard, appendPiName, shortOpenAlexId, fetchOrcidRegistryName, GUID_RE },
};
