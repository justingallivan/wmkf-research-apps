...
const contactAdapter = require('../dataverse/adapters/contact');
...
async function fetchOrcidRegistryProfile(orcid, { signal } = {}) {
  const empty = { registryName: null, currentAffiliation: null, currentAffiliationRor: null };
  ...
  return {
    ...
    currentAffiliationRor: profile.currentAffiliationDisambiguatedOrganizationId || null,
  };
}
...
async function resolveProposalPI(requestId, { signal } = {}) {
  ...
  let contact;
  try {
    contact = await contactAdapter.getPIIdentityById(plId);
  } catch (err) {
    if (isAbort(err)) throw err;
    throwIfAborted(signal); // budget may have fired alongside a non-abort read error — don't mask it
  ...
  const { registryName, currentAffiliation, currentAffiliationRor } = await fetchOrcidRegistryProfile(orcid, { signal }); // throws on abort
  throwIfAborted(signal);
  ...
  return {
    ...
    currentAffiliationRor: currentAffiliationRor || null,
    lastKnownInstitution,
    lastKnownInstitutionId: author.lastKnownInstitutionId || null,
    lastKnownInstitutionRor: author.lastKnownInstitutionRor || null,
...
function piInstitutions(pi, authorInstitution) {
  const raw = [
    pi?.resolved && pi.currentAffiliation ? {
      name: pi.currentAffiliation,
      ror: pi.currentAffiliationRor || null,
    } : null,
    pi?.resolved && pi.lastKnownInstitution ? {
      name: pi.lastKnownInstitution,
      openAlexId: pi.lastKnownInstitutionId || null,
      ror: pi.lastKnownInstitutionRor || null,
    } : null,
    authorInstitution,
