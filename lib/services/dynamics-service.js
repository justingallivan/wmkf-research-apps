...
export class DynamicsService {
  ...
  /**
   * @param {string} search
   * @param {{entities?:string[], top?:number, skip?:number, orderby?:string[], filter?:string}} [options]
   */
  ...
  // ───────── AI Run Logging ─────────
  //
  // AI_RUN_TASK_TYPES / AI_RUN_STATUSES / logAiRun / _truncateForMemo moved to
  // ./dynamics/ai-run.js — Stage 9 (Checkpoint F). The frozen picklist maps are
  // RE-EXPOSED here as facade statics (not just delegating wrappers) because
  // logAiRun reads them as `svc.AI_RUN_TASK_TYPES`/`svc.AI_RUN_STATUSES`
  // property accesses (the C1 "known trap" — see ai-run.js) — dropping this
  // re-exposure would silently break every logAiRun call. Thin delegating
  // wrappers for logAiRun/_truncateForMemo pass `this` as the svc receiver so
  // internal `this.` calls, external `DynamicsService.` calls, and test spies
  // are unchanged.
   ...
   */
  static AI_RUN_TASK_TYPES = AI_RUN_TASK_TYPES;

  static AI_RUN_STATUSES = AI_RUN_STATUSES;

  /**
   * Write a row to the wmkf_ai_run audit/replay table. Non-critical — callers
   * should treat failures as logged warnings, not user-visible errors.
   * @param {object} opts
   ...
   */
  static logAiRun(opts) {
    return logAiRun(this, opts);
  }
  ...
  static _truncateForMemo(s, maxChars) {
    return _truncateForMemo(s, maxChars);
  }
