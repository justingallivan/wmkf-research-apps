...
export class DynamicsService {
   ...
   *   `wmkf_appreviewanswers(wmkf_AppReviewerSuggestion=<g>,wmkf_questionkey='q2')`
   ...
   *   changeset is rejected — because it is atomic, a throw means NOTHING was
   *   committed. `.status` is the failing op's HTTP status, so callers branch on
   *   412 (If-Match conflict) exactly as they do for updateRecord/deleteRecord.
   */
...
/**
 * Parse one embedded `application/http` response body into { status, headers,
 * body, rawBody }. Shape: "HTTP/1.1 <status> <reason>\r\n<headers>\r\n\r\n<body>".
 */
function parseEmbeddedHttp(text) {
  const statusMatch = text.match(/^HTTP\/1\.1\s+(\d{3})/);
  const status = statusMatch ? Number(statusMatch[1]) : 0;
  const gap = text.indexOf(`${BATCH_CRLF}${BATCH_CRLF}`);
  const rawBody = gap === -1 ? '' : text.slice(gap + 4).trim();
  let body = null;
  if (rawBody) {
...
function collectHttpParts(body, boundary, results) {
  for (const part of splitMultipart(body, boundary)) {
    const headerEnd = part.indexOf(`${BATCH_CRLF}${BATCH_CRLF}`);
    ...
    if (ct.startsWith('multipart/mixed')) {
      const inner = extractBoundary(ct);
      if (inner) collectHttpParts(partBody, inner, results);
    } else if (ct.startsWith('application/http')) {
