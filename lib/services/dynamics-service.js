...
// Legacy module-level restrictions state (deprecated).
// Migrating to AsyncLocalStorage via `dynamics-context.js`. While both paths
// coexist, the ALS context wins; module state is the fallback for callers
// that haven't been migrated yet. Both will be removed once migration completes.
let activeRestrictions = null;
let _restrictionRequestId = null;

const _warnedLegacyApi = new Set();
function warnLegacyRestrictionApi(name) {
  if (_warnedLegacyApi.has(name)) return;
  _warnedLegacyApi.add(name);
  console.warn(
    `[DynamicsService] ${name} is deprecated and races under concurrent-request reuse. ` +
    `Use withDynamicsContext / bypassDynamicsRestrictions from lib/services/dynamics-context.js.`,
  );
}
...
export class DynamicsService {
  ...
  static async getAccessToken() {
    ...
    if (!DYNAMICS_URL || !DYNAMICS_TENANT_ID || !DYNAMICS_CLIENT_ID || !DYNAMICS_CLIENT_SECRET) {
      // Use the buildServiceError shape so the drain classifier sees a 500-class
      // structured error (not transient — config bug, not retryable).
  ...
  // ───────── Restrictions ─────────
  //
  // DEPRECATED. Use `withDynamicsContext` / `bypassDynamicsRestrictions` from
  // `./dynamics-context.js` instead. The static methods below write to module
  // globals, which races under Fluid Compute's concurrent-request reuse — one
  // request can leak its restrictions into another. They're kept as deprecated
  // shims for scripts that haven't been migrated yet; route handlers and
  // libraries must use the ALS-based context API.

  /**
   * @deprecated Use `withDynamicsContext({ restrictions, requestId }, fn)`.
   */
  static setRestrictions(restrictions, requestId) {
    warnLegacyRestrictionApi('setRestrictions');
    activeRestrictions = restrictions || [];
    _restrictionRequestId = requestId || null;
  }

  /**
   * @deprecated Use `bypassDynamicsRestrictions(requestId, fn)`.
   */
  static bypassRestrictions(requestId) {
    warnLegacyRestrictionApi('bypassRestrictions');
    activeRestrictions = [];
    _restrictionRequestId = requestId || null;
  }
  ...
  static checkRestriction(tableName, selectFields, expandParam, requestId) {
    // Prefer the per-request ALS context; fall back to module globals during
    // the incremental migration. Once all callers use withDynamicsContext,
    // the fallback (and the static set/bypassRestrictions methods that feed
    // it) will be deleted.
    ...
    } else if (activeRestrictions !== null) {
      restrictions = activeRestrictions;
      activeRequestId = _restrictionRequestId;
    } else {
      // Fail-closed: nothing initialized this request.
   ...
   */
  static async createRecord(entitySet, data, { actingUserSystemId } = {}) {
    const token = await this.getAccessToken();
    const baseUrl = process.env.DYNAMICS_URL;
   ...
   * @returns {Promise<void>}
   */
  static async deleteRecord(entitySet, recordId, { actingUserSystemId } = {}) {
    const token = await this.getAccessToken();
    const baseUrl = process.env.DYNAMICS_URL;
    const url = `${baseUrl}/api/data/v9.2/${entitySet}(${recordId})`;
    ...
    if (!resp.ok) {
      const errorBody = await resp.text();
      throw new Error(`Delete ${entitySet}(${recordId}) failed (${resp.status}): ${errorBody}`);
    }
  }
  ...
  static async createEmailActivity({ subject, body, from, to, cc, regardingId, regardingType, actingUserSystemId }) {
    const token = await this.getAccessToken();
    const baseUrl = process.env.DYNAMICS_URL;
   ...
   */
  static async addEmailAttachment(emailId, { filename, contentType, content, actingUserSystemId }) {
    const token = await this.getAccessToken();
    const baseUrl = process.env.DYNAMICS_URL;
   ...
   */
  static async sendEmail(emailId, { actingUserSystemId } = {}) {
    const token = await this.getAccessToken();
    const baseUrl = process.env.DYNAMICS_URL;
   ...
   */
  static async createAndSendEmail({ subject, body, from, to, cc, regardingId, regardingType, attachments = [], actingUserSystemId }) {
    // Step 1: Create the email activity
    const emailId = await this.createEmailActivity({ subject, body, from, to, cc, regardingId, regardingType, actingUserSystemId });

    // Step 2: Add attachments (sequentially to avoid race conditions)
    for (const attachment of attachments) {
      await this.addEmailAttachment(emailId, { ...attachment, actingUserSystemId });
    }

    // Step 3: Send
    await this.sendEmail(emailId, { actingUserSystemId });
