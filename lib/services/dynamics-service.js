...
/**
 * Branded Dataverse record id (see lib/utils/guid.js). Every selector below
 * that interpolates a record id into a key predicate — `${entitySet}(${recordId})`
 * — requires `Guid`, not `string`: a raw request-input string does not
 * type-check until it has been narrowed through `isGuid` at the route edge.
 * Server-derived ids (read back off a fetched row) are cast at the read site.
 *
...
/**
 * @type {{
 *   tables: { data: Array<any>|null, fetchedAt: number },
 *   fields: Map<string, { data: any, fetchedAt: number }>,
 *   fieldPromises: Map<string, Promise<any>>,
 *   relationships: Map<string, { data: any, fetchedAt: number }>,
 *   entitySetMap: Map<string, string>|null,
 *   primaryIdMap: Map<string, string>|null,
 *   entitySetFetchedAt: number,
 * }}
 */
const schemaCache = {
...
export class DynamicsService {
   ...
   *   fetch init; init.headers must already include or
   *   exclude MSCRMCallerID per _withCallerId.
   * @param {string|null|undefined} actingUserSystemId - the systemuserid attempted, used
  ...
  static async getEntityDefinitions(searchTerm) {
    ...
    const entities = /** @type {Array<any>} */ (data.value || []).map(e => ({
    ...
    /** @type {Map<string, string>} */
    const primaryIdMap = new Map();
    schemaCache.entitySetMap = entitySetMap;
    schemaCache.primaryIdMap = primaryIdMap;
    schemaCache.entitySetFetchedAt = now;
    for (const e of entities) {
      entitySetMap.set(e.logicalName, e.entitySetName);
      ...
      if (e.primaryIdAttribute) {
        primaryIdMap.set(e.logicalName, e.primaryIdAttribute);
        if (e.entitySetName) primaryIdMap.set(e.entitySetName, e.primaryIdAttribute);
      }
    }
  ...
  static async getEntityAttributes(tableName) {
    ...
    const promise = (async () => {
      ...
      const data = await resp.json();
      const attrs = /** @type {Array<any>} */ (data.value || [])
  ...
  static async queryRecords(entitySet, { select, filter, orderby, top, expand } = {}) {
    ...
    const data = await resp.json();
    const records = /** @type {Array<any>} */ (data.value || []).map(r => this.processAnnotations(r));
   ...
   *
   * `recordId` is interpolated raw into the key predicate
   * `${entitySet}(${recordId})`, so it requires the branded {@link Guid} —
   ...
   * @param {string} [options.field] - Field to aggregate (required at runtime)
   * @param {string} [options.operation] - sum, average, min, max, or countdistinct (required at runtime)
   ...
   * @returns {Promise<{results:Array<any>, operation:string, field:string|undefined, groupBy?:string, filter?:string}>}
   */
  static async aggregateRecords(entitySet, { field, operation, filter, groupBy } = {}) {
    ...
    const data = await resp.json();
    const results = /** @type {Array<any>} */ (data.value || []).map(r => this.processAnnotations(r));

    /** @type {{results:Array<any>, operation:string, field:string|undefined, groupBy?:string, filter?:string}} */
   ...
   * @returns {Promise<{records:Array<any>, totalCount:number, capped:boolean}>}
   */
   ...
   * @returns {Promise<{results:Array<any>, totalCount:number, queryContext:any}>}
   */
  static async searchRecords(search, { entities, top = 20, filter } = {}) {
    ...
    /** @type {{search:string, top:number, returntotalrecordcount:boolean, entities?:string[], filter?:string}} */
    const body = {
      search,
    ...
    const results = /** @type {Array<any>} */ (data.value || []).map(r => {
   ...
   * @param {boolean} [options.noFallback] - Disable the privilege-intersection
   *   fallback retry (see _writeFetch).
   ...
   *
   * @param {string} entitySet
   * @param {Guid} recordId - branded GUID (narrow client input via `isGuid` first)
   * @param {object} data
   * @param {object} [options]
  ...
  static async deleteRecord(entitySet, recordId, { actingUserSystemId, ifMatch } = {}) {
    ...
    if (!resp.ok) {
      ...
      const err = /** @type {Error & {status?: number}} */ (
        new Error(`Delete ${entitySet}(${recordId}) failed (${resp.status}): ${errorBody}`)
      );
   ...
   *
   * @param {string} entitySet
   * @param {Guid} recordId - branded GUID of the referencing row (narrow client
   *   input via `isGuid` first)
  ...
  static async disassociate(entitySet, recordId, navProperty, { actingUserSystemId } = {}) {
    ...
    if (!resp.ok && resp.status !== 404) {
      const errorBody = await resp.text();
      const err = /** @type {Error & {status?: number}} */ (
        new Error(`Disassociate ${entitySet}(${recordId})/${navProperty} failed (${resp.status}): ${errorBody}`)
      );
      err.status = resp.status;
      throw err;
    }
   ...
   *   actingUserSystemId?:string, noFallback?:boolean}} options - see the
   *   field-by-field JSDoc above.
   ...
   * @param {string} [attachment.actingUserSystemId] - MSCRMCallerID attribution.
   * @param {boolean} [attachment.noFallback] - Disable the privilege-intersection fallback.
   */
   ...
   * @param {{actingUserSystemId?:string, noFallback?:boolean}} [options]
   */
   ...
   *   actingUserSystemId?:string, noFallback?:boolean}} options - Same as
   *   createEmailActivity plus `attachments`.
  ...
  static async resolveEntitySetName(logicalName) {
    ...
    const knownSets = /** @type {Record<string, string>} */ (KNOWN_ENTITY_SETS);
    if (knownSets[logicalName]) {
      return knownSets[logicalName];
    }
 ...
 *
 * @param {{method:string, url:string, body?:object, ifMatch?:string}} op
 ...
 *
 * @param {Array<{method:string, url:string, body?:object, ifMatch?:string}>} operations
 ...
 *
 * @param {string} text
 * @returns {{headers:string, body:string}|null}
 */
function splitHeadersAndBody(text) {
  const m = text.match(/\r?\n\r?\n/);
  if (!m) return null;
  const idx = /** @type {number} */ (m.index); // non-global match always has an index
  return { headers: text.slice(0, idx), body: text.slice(idx + m[0].length) };
}
 ...
 *
 * @param {string} text
 * @returns {{status:number, rawBody:string, body:object|null}}
 */
 ...
 * @param {string} body
 * @param {string} boundary
 * @param {Array<{contentId:number|null, status:number, body:object|null, rawBody:string}>} results
 */
...
function parseBatchResponse(rawText, contentType) {
  ...
  /** @type {Array<{ contentId:number|null, status:number, body:object|null, rawBody:string }>} */
  const results = [];
