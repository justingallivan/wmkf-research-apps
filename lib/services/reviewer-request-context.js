...
function piResolutionForResult(piIdentity) {
  if (piIdentity?.resolved) {
    return { state: 'ok', reason: null };
  }
  if (
    piIdentity?.reason === 'request_read_failed'
    || piIdentity?.reason === 'contact_read_failed'
    || piIdentity?.error
  ) {
    return {
      state: 'failed',
      reason: piIdentity?.reason || 'pi_resolve_failed',
      error: piIdentity?.error || null,
    };
  }
  return {
    state: 'ok',
    reason: piIdentity?.reason || null,
  };
}
...
export function projectReviewerRequestContext(request, coPIs = [], applicantAccount = null, applicantFetchError = null) {
  ...
  return {
    ...
    authorInstitution: firstClean(request._akoya_applicantid_value_formatted),
    ...
    // The board-cycle handle: wmkf_meetingdate is the one canonical temporal
    // field on akoya_request, so the cycle code a write path stamps on a
    // suggestion row is derived here and never accepted from a client.
    meetingDate: request.wmkf_meetingdate || null,
    cycleCode: request.wmkf_meetingdate ? meetingDateToCycleCode(request.wmkf_meetingdate) : null,
  };
}
...
export async function loadCoiContext(requestId, {
  ...
  let piIdentity = null;
  let piResolution = { state: 'ok', reason: 'not_requested' };
  if (resolvePi) {
    try {
      piIdentity = await resolveProposalPI(requestId, { signal });
      piResolution = piResolutionForResult(piIdentity);
    } catch (error) {
