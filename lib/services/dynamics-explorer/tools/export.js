 ...
 *
 * Extracted verbatim from pages/api/dynamics-explorer/chat.js:2311-2468,
 * 2657-2726 (pre-S2 line numbers); characterization tests are the safety
 * net.
 *
...
export async function exportCsv({ table_name, select, filter, orderby, filename, process_instruction, confirmed, include_inactive }, sendEvent, userProfileId, restrictions = []) {
  ...
  const requestTable = ['akoya_request', 'akoya_requests'].includes(String(table_name || '').toLowerCase());
  const effectiveFilter = withOrdinaryTestRequestODataFilter(
    applyActiveOnlyFilter(filter, include_inactive),
    requestTable ? process.env : {},
  );
