...
async function getAccountRequestIds(accountId) {
  const requestResult = await DynamicsService.queryRecords('akoya_requests', {
    ...
    filter: withOrdinaryTestRequestODataFilter(`_akoya_applicantid_value eq ${accountId}`),
...
async function handleAccountRequests(accountId, sourceRecord, buildDateFilter) {
  ...
  const result = await DynamicsService.queryRecords('akoya_requests', {
    select: withTestRequestIsolationSelect('akoya_requestnum,akoya_requeststatus,akoya_submitdate,akoya_fiscalyear,akoya_paid,wmkf_request_type,_akoya_primarycontactid_value,_wmkf_grantprogram_value'),
  ...
  const lines = result.records.map(r => {
    ...
    const testBadge = testRequestVisibilityDto(r).isTestRequest === true ? ' | TEST' : '';
    return `Req ${num}${testBadge} | ${status} | ${date} | FY: ${fy} | ${type} | ${program} | Paid: ${paid}`;
  });
...
async function handleRequestReports(requestId, buildDateFilter) {
  const dateFilter = buildDateFilter('akoya_requirementdue');
  const baseFilter = `_akoya_requestlookup_value eq ${requestId} and akoya_type eq true${dateFilter}`;
  const filter = testRequestIsolationEnabled()
    ? `(${baseFilter}) and ${ordinaryTestRequestODataFilterForNavigation('akoya_requestlookup')}`
    : baseFilter;
  const result = await DynamicsService.queryRecords('akoya_requestpayments', {
...
async function handleContactRequests(contactId, buildDateFilter) {
  ...
  const result = await DynamicsService.queryRecords('akoya_requests', {
    select: withTestRequestIsolationSelect(`akoya_requestnum,akoya_requeststatus,akoya_submitdate,akoya_fiscalyear,akoya_paid,wmkf_request_type,_akoya_applicantid_value,_wmkf_grantprogram_value,${contactRoleFields.join(',')}`),
  ...
  const lines = result.records.map(r => {
    ...
    const testBadge = testRequestVisibilityDto(r).isTestRequest === true ? ' | TEST' : '';
    return `Req ${num}${testBadge} | ${status} | ${date} | ${org} | ${program} | ${roles.join(', ') || 'Contact'} | Paid: ${paid}`;
  });
...
async function handleReviewerRequests(reviewerId, buildDateFilter) {
  ...
  const result = await DynamicsService.queryRecords('akoya_requests', {
    select: withTestRequestIsolationSelect('akoya_requestnum,akoya_requeststatus,akoya_submitdate,akoya_fiscalyear,_akoya_applicantid_value,_wmkf_grantprogram_value'),
    filter: `(${orClauses})${dateFilter}`,
    orderby: 'akoya_submitdate desc',
    top: 100,
  });
  ...
  const lines = result.records.map(r => {
    ...
    const program = r._wmkf_grantprogram_value_formatted || '';
    const testBadge = testRequestVisibilityDto(r).isTestRequest === true ? ' | TEST' : '';
    return `Req ${num}${testBadge} | ${status} | ${date} | ${org} | ${program}`;
  });
