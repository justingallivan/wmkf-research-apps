...
function isRequestTable(value) {
  return ['akoya_request', 'akoya_requests'].includes(String(value || '').toLowerCase());
}
  ...
  switch (name) {
    ...
    case 'query_records': {
      const effectiveInput = {
        ...input,
        select: isRequestTable(input.table_name)
          ? withTestRequestIsolationSelect(sanitizeSelect(input.select))
          : sanitizeSelect(input.select),
      ...
      result.records = result.records.map((record) => {
        ...
        return isRequestTable(input.table_name)
          ? projectTestRequestVisibilityRecord(stripped)
          : stripped;
      });
      return result;
    }
    ...
    case 'count_records': {
      const effectiveInput = {
        ...
        filter: withOrdinaryTestRequestODataFilter(
          ...
          isRequestTable(input.table_name) ? process.env : {},
        ),
      };
      const validation = await validateEffectiveODataCall(name, effectiveInput, restrictions);
      if (validation.reject) return validatorReject(validation.reject);
