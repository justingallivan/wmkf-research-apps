...
async function completeEntryWithReport(run, entry, config, chairAttempt) {
  ...
  try {
    const [allAttempts, current, cost] = await Promise.all([
      ...
      store.listAttemptsForEntry(entry.id), store.readReviewPanelEntry(entry.id), store.sumEntryAttemptCosts(entry.id),
    ]);
    ...
    const report = {
      title: `Review Panel — ${current.data?.input?.requestNumber || current.data?.requestNumber || current.request_id}`,
      institution: current.data?.input?.institution || null,
      chair: chairAttempt.result_json,
      seats,
      cost,
    };
    const rendered = await renderReviewPanelEntryDocuments(report);
    const [docxRef, pdfRef] = await Promise.all([
      storeReviewPanelFile(`review-panel/${entry.id}/report.docx`, rendered.docx, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'),
      storeReviewPanelFile(`review-panel/${entry.id}/report.pdf`, rendered.pdf, 'application/pdf'),
    ]);
    await store.mutateReviewPanelEntry(entry.id, (e) => {
      e.status = 'completed';
      e.data = { ...e.data, chairResult: chairAttempt.result_json, files: { docx: docxRef, pdf: pdfRef } };
    }, run.lease_token);
  } catch (error) {
    await store.mutateReviewPanelEntry(entry.id, (e) => {
      e.status = 'failed';
