...
async function runOneSeat(run, entry, seatKey, seatConfig, operatorStop, existingAttempt) {
  ...
  // Deliberately unguarded: an owner-revoked (403) or storage-not-configured
  // (503) throw here must propagate all the way to drainReviewPanels, never
  // be swallowed — see assertReadyForPaidDispatch.
...
async function processEntry(run, entry, config, operatorStop) {
  ...
  // Deliberately unguarded, same as runOneSeat's call — must propagate to
  // drainReviewPanels, never be swallowed.
  await assertReadyForPaidDispatch(run);
...
export async function drainReviewPanels() {
  ...
  try {
    ...
    // Owner-revoked: fail the run outright (never merely paused/interrupted
    // — restoring the owner's role doesn't retroactively make a mid-flight
    // dispatch decision safe to resume) and release the lease WITHOUT
    // dispatching anything further. Mirrors cycle-dossier-worker.js's
    // drainCycleDossiers `if (error.httpStatus === 403) { await
    // store.stopRevokedDossierRun(...); return ...; }`, but review-panel-
    // store.js's stopRevokedReviewPanelRun fails the RUN (not "paused" —
    // review-panel has no separate paused/resume flow) with plain copy.
    ...
    if (error?.httpStatus === 503) return { claimed: 1, paused: true };
    throw error;
  } finally {
    operatorStop.stop();
