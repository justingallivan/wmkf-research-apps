...
const OWNER_LOST_MESSAGE = 'The run owner no longer has Review Panel access; no further model calls were made.';

/**
 * Re-assert the run owner still has Review Panel access (superuser or app grant) AND the D11 store is
...
async function assertReadyForPaidDispatch(run) {
  await store.assertReviewPanelAccess(run.owner_profile_id);
...
async function processEntry(run, entry, config, operatorStop, revocation) {
  ...
  if (finalChair?.state === 'completed') {
    // Record the chair winner BEFORE completing the entry. Unlike the seat
    // winners (selected above at the top of processEntry, before the chair
    // even exists) and the "chair was already completed on a prior pass"
    // branch above (which runs selectWinners on THIS pass' entry before
    // reaching this decision point), a chair that completes inline within
    // THIS same dispatch+await has no earlier selectWinners call in this
    // pass to pick it up — without this, winners_json.chair is never set on
    // the normal completion path (production defect 2026-09-13: only
    // entries that happened to complete their chair on a LATER pass ever
    // got winners_json.chair, via the selectWinners call at the top of
    // processEntry). requestReviewPanelRerender's eligibility no longer
    // depends on this field (it checks review_panel_seat_attempts directly),
    // but this keeps winners_json accurate for every other reader.
    await store.selectWinners(entry.id, run.lease_token);
...
export async function rerenderCompletedEntries(run, config, entryIds) {
  for (const entryId of entryIds) {
    ...
    // requestReviewPanelRerender's eligibility check (store.js) no longer
    // requires winners_json.chair — a completed chair attempt is enough — so
    // an entry can land here with winners_json.chair still unset (any entry
    // that completed via the normal inline-chair path BEFORE the worker fix
    // below started recording it). Backfill it the same way the normal
    // completion path now does, before trusting winners_json.chair.
    await store.selectWinners(entryId, run.lease_token);
    const refreshedEntry = await store.readReviewPanelEntry(entryId);
    const chairAttemptId = refreshedEntry?.winners_json?.chair;
    ...
    if (!chairAttempt) {
      ...
      // a completed chair attempt, and selectWinners above always records
      // the latest completed chair attempt if one exists, so this should be
      // unreachable. Clear the marker rather than looping on it forever.
      await store.mutateReviewPanelEntry(entryId, (e) => {
