 ...
 */
async function completeEntryWithReport(run, entry, config, chairAttempt, { renderStamp = null } = {}) {
 ...
 *
 * `previousFiles` is NEVER simply deleted along with the marker: it is
 * carried forward to `data.lastRerender` (rather than dropped) so it stays
 * reachable even if the process dies right after the marker clears, and — if
 * `completeEntryWithReport` itself fails (a render/upload error; it always
 * catches its own throw and marks the entry `failed` rather than propagating
 * one) — this function restores the entry to `completed` with the OLD
 * `data.files` still in place. Without this, a failed re-render would leave a
 * previously-completed, downloadable entry as `failed` with `data.files`
 * null and no surviving reference to the working edition anywhere on the
 * row — and the operator's only recourse, "Retry failed entries", would
 * re-render under the ORIGINAL (unstamped) pathname, which `storeReviewPanelFile`
 * (`allowOverwrite:false`) refuses forever because fresh bytes never match
 * the digest already stored there (completeEntryWithReport's own header
 * comment, ~:204-207).
 */
export async function rerenderCompletedEntries(run, config, entryIds) {
  for (const entryId of entryIds) {
    ...
    const previousFiles = entry.data.rerender.previousFiles || null;
    ...
    // Clear the rerender marker BEFORE re-rendering (completeEntryWithReport
    // re-reads the entry from the store itself, and a crash mid-render must
    // never leave this entry re-triggering the same re-render forever), but
    // keep `previousFiles` reachable as `data.lastRerender` rather than
    // dropping it outright.
    await store.mutateReviewPanelEntry(entryId, (e) => {
      const { rerender, ...rest } = e.data || {};
      // error:null clears any STALE error left by a prior failed re-render
      // (mirrors retryFailedEntries' own re-arm below) — otherwise a
      // successful re-render would still show the old "could not be saved"
      // copy under the freshly-saved links.
      e.data = { ...rest, error: null, lastRerender: { ...rerender, consumedAt: new Date().toISOString() } };
    }, run.lease_token);
    const renderStamp = `${Date.now()}-${randomUUID().slice(0, 8)}`;
    await completeEntryWithReport(run, entry, config, chairAttempt, { renderStamp });

    if (previousFiles) {
      const after = await store.readReviewPanelEntry(entryId);
      const missingAFormat = !after?.data?.files?.docx || !after?.data?.files?.pdf;
      if (after?.status === 'failed' && missingAFormat) {
        await store.mutateReviewPanelEntry(entryId, (e) => {
          e.status = 'completed';
          e.data = { ...e.data, files: previousFiles, error: 'The re-rendered report could not be saved; the previous edition is kept.' };
        }, run.lease_token);
      }
    }
  }
