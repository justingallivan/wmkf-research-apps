...
async function processEntry(run, entry, config, operatorStop) {
  ...
  if (latestChair) {
    ...
    if (latestChair.state === 'unknown_outcome') {
      await store.mutateReviewPanelEntry(entry.id, (e) => {
        e.status = 'failed';
        e.data = { ...e.data, error: `${LOST_RESPONSE_TEXT} (the chair synthesis)` };
      }, run.lease_token);
      return;
    }
    if (latestChair.state === 'dispatched' || latestChair.state === 'failed') {
      // Still in flight, or already reflected by the chair's own catch block
      // below on an earlier pass: nothing to do.
      return;
    }
