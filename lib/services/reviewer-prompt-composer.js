 ...
 * The `composeAnalyzePrompt` output is intentionally stricter than the legacy
 * `createAnalysisPrompt`: search prompts require trusted request metadata and
 * prepend code-owned exclusion/integrity blocks that editable prompt bodies
 * cannot remove.
 */
...
/**
 * Server-authoritative exclusions block. Built from the client's excludedNames
 * PLUS the applicant team (PI + Co-PIs) and institution name variants from the
 * trusted Dataverse request context. PREPENDED by the composer (not a
 * `{{placeholder}}` in the editable body) so a staff prompt edit cannot remove
 * it, and so the model never spends a suggestion slot on someone who will be
 * screened out downstream (excludePiIdentity / same-institution COI at save).
 */
const splitList = (value) =>
  (value && value !== 'None') ? String(value).split(',').map((s) => s.trim()).filter(Boolean) : [];

const dedupeText = (values) => {
  const seen = new Set();
  const out = [];
  for (const v of values) {
    const t = (v == null ? '' : String(v)).trim();
    if (!t || t === 'Not specified' || t === 'None') continue;
...
export function buildExclusionSets({ excludedNames = [], requestContext = null } = {}) {
  const people = dedupeText([
    ...(Array.isArray(excludedNames) ? excludedNames : []),
    requestContext?.principalInvestigator,
    ...splitList(requestContext?.coInvestigators),
  ]);
  const institutions = dedupeText(requestContext?.applicantInstitutionNames || []);

  return { people, institutions };
}

export function buildExclusionsBlock({ excludedNames = [], requestContext = null } = {}) {
  const { people, institutions } = buildExclusionSets({ excludedNames, requestContext });

  if (!people.length && !institutions.length) return '';
  const lines = ['**EXCLUSIONS - HARD CONSTRAINTS (do NOT suggest any of these as reviewers, under any circumstances):**'];
  if (people.length) {
    lines.push(`People (the proposal's own investigators and other conflicts): ${people.join(', ')}`);
  }
  if (institutions.length) {
    lines.push(`Institution - do NOT suggest anyone affiliated with the applicant institution, known by any of these names: ${institutions.join('; ')}`);
  }
  return lines.join('\n');
}
 ...
 *   repairInstructions?: string, requestContext?: Object|null,
 *   analysisPurpose?: 'search'|'proposal_info' }} args
 *   `proposalText` is the ALREADY-WRAPPED (nonce-bearing) proposal text.
 * @returns {string} the final prompt
 */
export function composeAnalyzePrompt({ body, proposalText, nonces = [], additionalNotes = '', excludedNames = [], reviewerCount = DEFAULT_REVIEWER_COUNT, repairInstructions = '', requestContext = null, analysisPurpose = 'search' }) {
  if (analysisPurpose !== 'proposal_info' && !requestContext) {
    throw new Error('requestContext is required to compose reviewer-finder search prompts');
  }
  const vars = {
    proposal_text: proposalText,
    ...buildAnalyzeBlockVars({ additionalNotes, excludedNames, reviewerCount }),
  };
  // Exclusions are now authoritative in the prepended block below; neutralize the
  // legacy body placeholder so exclusions can't be double-listed or edited away.
  vars.excluded_names_block = '';
  ...
  const exclusions = buildExclusionsBlock({ excludedNames, requestContext });
  const exclusionsBlock = exclusions ? `${exclusions}\n\n` : '';
  ...
  // overrides) so the anti-fabrication guardrail can't be edited away.
  return `${buildUntrustedContentPreamble(nonces)}\n\n${trustedMetadataBlock}${exclusionsBlock}${interpolate(effectiveBody, vars)}${ANALYZE_INTEGRITY_BLOCK}${repairBlock}`;
}
