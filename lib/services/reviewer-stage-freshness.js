...
/**
 * Last-resort selection marker for a historical signed row after a signing-key
 * rotation. It does not trust invalid JWT claims: the precise verifier outcome
 * only unlocks a server-persisted roster, resolver, Dataverse-ORCID, and contact
 * check. It can never make a stage current or authorize promotion.
 */
function projectLegacyServerRosterSelectionProjection(candidate, { contact, automatedAttestation, now } = {}) {
  ...
  if (!candidateKey
    || !evidenceCheckedAt
    || automatedAttestation?.valid !== false
    || automatedAttestation?.reason !== 'invalid_signature') return null;
  return {
    version: LEGACY_SELECTION_PROJECTION_VERSION,
    state: 'selectable',
    evidenceCheckedAt,
