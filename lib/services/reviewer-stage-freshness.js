...
function candidateIdentity(candidate) {
  ...
  if (/^(suggestion|person|orcid|openalex|scholar|seed):/.test(key)) return key;
  const suggestionId = typeof candidate?.suggestionId === 'string' ? candidate.suggestionId.trim() : '';
  return suggestionId ? reviewerSuggestionCandidateKey(suggestionId) : null;
}
...
function mapLegacyStage(candidate, stage, authoritative = {}) {
  ...
  if (!canonicalIso(legacy.completedAt) || !boundedVersion(legacy.sourceVersion) || !boundedVersion(authoritative?.versions?.[stage]) || legacy.sourceVersion !== authoritative.versions[stage]) return null;
  return { state: 'current', contractVersion: legacy.contractVersion, sourceVersion: legacy.sourceVersion, completedAt: legacy.completedAt, mappedFromLegacy: true };
}
...
function planCandidateFreshness({ candidate, authoritative = {}, now = Date.now(), policy = DEFAULT_AGE_POLICY } = {}) {
  ...
  if (!boundedVersion(candidate?.applicantInputVersion) || !boundedVersion(authoritative.applicantInputVersion) || candidate.applicantInputVersion !== authoritative.applicantInputVersion) {
...
module.exports = { STAGES, STATES, REASONS, CONTRACT_VERSIONS, DOWNSTREAM, DEFAULT_AGE_POLICY, canonicalIso, boundedVersion, candidateIdentity, validReason, mapLegacyStage, planCandidateFreshness, planRosterFreshness };
