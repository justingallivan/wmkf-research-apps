 ...
 *   Since 2026-09-06 (owner decision, acceptance-write ETag tightening) the
 *   accept and decline response writes REQUIRE a concrete version: accept
 *   takes the header only; decline takes the header or the verifier row's
 *   `_etag`. A missing/wildcard version is refused with the same 412
 *   `concurrent_modification` envelope a stale one gets (the client's
 *   "refresh and try again" path), instead of falling through to an
 *   unconditional adapter write.
...
export async function applyReviewerResponse({
  ...
  if (body.action === 'decline') {
    ...
    if (declined) {
      if (suggestion._wmkf_honorariumrequest_value) {
        ...
        // the freshly verified row. D3: the op requires a concrete ETag; a
        // row that arrived without one (`missing_version`) is refused with
        // the same concurrent_modification envelope a stale version gets,
        // since either way the write could not be bound to a known version.
        try {
          await withDalContext(
            'external-respond',
            () => deselectLegacyDeclinedSuggestion(
          ...
          const msg = e.message || '';
          if (e.status === 412 || e.code === 'missing_version' || /\b412\b/.test(msg)) {
            throw envelopeError(412, { ok: false, reason: 'concurrent_modification' });
          }
          throw e;
        }
    ...
    const linkedHonorariumRequestId = accepted
      ...
      : null;
    const declineIfMatch = ifMatch || suggestion._etag || undefined;
    if (!isConcreteEtag(declineIfMatch)) {
      // No version to bind the write to: refuse rather than write blind.
      throw envelopeError(412, { ok: false, reason: 'concurrent_modification' });
    }
    try {
  ...
  if (!isAcceptRepeat && !isConcreteEtag(ifMatch)) {
    // The accept write is bound to the version the reviewer saw (/context
    // round-trips it as If-Match). Without a concrete version we refuse
    // before staging an acceptance job, so nothing is enqueued for a write
    ...
    try {
      await withDalContext('external-respond', () =>
        applyStage2aResponse(suggestion.wmkf_appreviewersuggestionid, {
          ...
          // (concreteness checked above, before the job was enqueued).
          ifMatch,
        }),
      );
    } catch (e) {
