...
function receiptEligibilityError(error) {
  if (error.reason === 'review_received_locked') return reviewReceivedLockedError();
  if (error.reason === 'engagement_ended') {
    return new ServiceHttpError('engagement_ended', {
      httpStatus: 409,
      body: {
        ok: false,
  ...
  return new ServiceHttpError('conflict', {
    httpStatus: 409,
    body: {
      ok: false,
      reason: 'conflict',
...
export async function submitReview({ suggestion, body }) {
  ...
  // Authoritative (uncached) resolve: this is a write boundary, and the
  // set_changed comparison below is only meaningful against the live set.
  ...
  try {
    await withDalContext('external-review-submit', async () => {
      ...
      let authorizingRow = suggestion;
      if (!authorizingRow._etag) {
        ...
        authorizingRow = await getForSubmitFinalityCheck(suggestionId);
      }
      const { ifMatch: parentEtag } = authorizeReviewReceipt(authorizingRow);
    ...
    if (e instanceof ReviewReceiptEligibilityError) {
      throw receiptEligibilityError(e);
    }
    ...
    if (e.status === 412 || e?.response?.status === 412) {
