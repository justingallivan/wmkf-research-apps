/**
 * External Reviewer Portal — landing-page context service
 * (Route→Service Consolidation Plan, Stage 5, fail-closed batch).
 *
 * Holds the post-verification business logic for
 * GET /api/external/review/[token]/context; the route is a thin shell that
 * keeps the ENTIRE token boundary byte-untouched (rate limit → verify →
 * outcome recording → failure envelopes) and calls this with the verified
 * { suggestion, request, reviewer }.
 *
 * Contract (plan Decision 3):
 *   - plain argument object, never req/res;
 *   - returns the exact 200 body ({ ok:true, ... });
 *   - throws ServiceHttpError with the explicit { ok:false, reason } body for
 *     the fail-closed policy path (500 policy_misconfigured); everything else
 *     propagates raw to the shell's { ok:false, reason:'server_error' } 500;
 *   - DAL contexts: the historical route established five narrow,
 *     branch-specific withDalContext scopes (first-access stamp, etag
 *     re-read, file listing, co-PI fetch, contact prefill) with the
 *     lib/external fetchers self-managing their own. Those scopes move here
 *     VERBATIM (labels kept, boundaries identical) — a DECLARED deviation
 *     from "services never establish a context", preserved because widening
 *     one context over the whole assembly would change the historical
 *     branch-specific trust boundaries (P1m caveat). Later scoped reads
 *     (rating snapshot and post-accept Program Director contact) follow the
 *     same narrow-sibling doctrine rather than widening that trust boundary.
 */
...
import { readRatingsBySuggestion } from '../../external/review-answer-snapshot';
import { getForEtagRefresh, stampProposalFirstAccessed } from '../../dataverse/adapters/reviewer-suggestion';
import { getByIdWithSelect as getContactByIdWithSelect } from '../../dataverse/adapters/contact';
import { getByIdWithSelect as getSystemUserByIdWithSelect } from '../../dataverse/adapters/system-user';
import { ServiceHttpError } from '../service-http-error';
import { institutionNameOf, looksLikeByline } from '../../../shared/utils/review-writeup-paragraphs';
import { resolveEffectiveReviewDueDate } from '../../external/reviewer-due-date.js';
import { isGeneratedReviewFile } from '../../../shared/utils/review-file-provenance.js';

// Slots Stage 2a renders. Hardcoded per build plan §4a.
const STAGE_2A_POLICY_SLOTS = ['reviewer-coi', 'reviewer-ai-use'];

/**
 * Assemble the landing-page context payload for a VERIFIED token.
 *
 * @param {Object} args - the verifier's { suggestion, request, reviewer }
 * @returns {Promise<Object>} the historical 200 body
 * @throws {ServiceHttpError} 500 { ok:false, reason:'policy_misconfigured' }
 */
export async function buildReviewContext({ suggestion, request, reviewer }) {
  // Engagement state — drives which view the page renders.
  const engagementState = computeEngagementState(suggestion);

  // Post-accept and post-submit guidance names the assigned, active Program
  // Director: the accepted confirmation page, and the "Review received"
  // notices in stage2b/submitted (stage2b is load-bearing — the immediate
  // post-submit banner renders from client state without refetching context).
  // Keep the lookup best-effort: missing/incomplete staff data must not
  // prevent a reviewer from reopening their page.
  let programDirector = null;
  const programDirectorId = request?._wmkf_programdirector_value || null;
  const viewNamesProgramDirector = ['accepted-pre-materials', 'stage2b', 'submitted']
    .includes(engagementState.view);
  if (viewNamesProgramDirector && programDirectorId) {
    try {
      const staff = await withDalContext('external-context-program-director', () =>
        getSystemUserByIdWithSelect(programDirectorId, [
          'systemuserid', 'fullname', 'internalemailaddress', 'isdisabled',
        ]),
      );
      const name = String(staff?.fullname || '').trim();
      const email = String(staff?.internalemailaddress || '').trim();
      if (staff?.isdisabled === false && name && email) {
        programDirector = { name, email };
      }
    } catch (e) {
      console.error('[external context] program director lookup failed:', e.message);
    }
  }
  ...
  // S333 Stage 4b: trust-model tightening — this is the sole caller of
  // readRatingsBySuggestion, which ran as a SIBLING to (not inside) this
  // file's other branch-specific scopes; the wrap moves here as a sixth
  // narrow scope, matching this file's own declared P1m doctrine (label
  // 'read-ratings-by-suggestion' byte-preserved).
  const ratingPrefill = await withDalContext('read-ratings-by-suggestion', () =>
    readRatingsBySuggestion(suggestion.wmkf_appreviewersuggestionid),
  );
  ...
  if (needStage2aData) {
    ...
    try {
      ...
      throw new ServiceHttpError('policy_misconfigured', {
        httpStatus: 500,
        body: { ok: false, reason: 'policy_misconfigured' },
      });
    }
  ...
  return {
    ...
    programDirector,
    // The communicated operational deadline (per-engagement override, then
    // request campaign date). This is what the page shows as "Submission
    // deadline"; the token expiry below is deliberately later and is NOT shown
    // as a deadline. Accepted materials links retain the intentional 90-day
    // post-due cushion through the Board meeting.
    reviewDeadline: resolveEffectiveReviewDueDate({
      overrideDate: suggestion.wmkf_reviewduedateoverride,
      defaultDate: request.wmkf_reviewduedate,
    }),
    // Technical wall-clock cutoff for self-serve submission (magic-link
    // expiry; review due + grace days per reviewer-token-ttl.js).
    tokenExpiresAt: suggestion.wmkf_externaltokenexpires || null,
    submission: {
      receivedAt: suggestion.wmkf_reviewreceivedat || null,
      filename: isGeneratedReviewFile(suggestion.wmkf_reviewsharepointfolder)
        ? null
        : (suggestion.wmkf_reviewfilename || null),
    },
    // Review-form prefill: affiliation plus the two canonical rating snapshots.
    ...
    prefill: {
      ...
      riskLevel: ratingPrefill.riskLevel,
      overallAssessment: ratingPrefill.overallAssessment,
      ...(needStage2aData
...
function buildStage2aPrefill(suggestion, reviewer, contact) {
  ...
  return {
    ...
    // `wmkf_primaryaffiliation` is a raw PubMed byline for a reviewer enriched
    // from PubMed (request 1002852, 2026-09-21): a reviewer who submits the
    // pre-filled seed unchanged carries the whole byline into the stored,
    // confirmed `wmkf_maininstitution`. Seed with the reduced institution
    // name only when the enrichment value still looks byline-shaped
    // (`looksLikeByline` — an echoed email, a geographic segment, or a
    // department lead); a clean enrichment affiliation ("Weill Cornell
    // Medicine, Cornell University") is seeded unchanged, since it may
    // legitimately be a multi-part institution name of its own (Codex round
    // 4 finding, 2026-09-21). A prior confirmed `wmkf_maininstitution` is
    // never touched here — it is the reviewer's own value, used as-is (the
    // composer gates its own reduction the same way for display).
    ...
    mainInstitution: firstNonEmpty(
      reviewer?.wmkf_maininstitution,
      (() => {
        const seed = firstNonEmpty(reviewer?.wmkf_primaryaffiliation, reviewer?.wmkf_organizationname);
        return seed && looksLikeByline(seed) ? institutionNameOf(seed) : seed;
      })(),
    ),
