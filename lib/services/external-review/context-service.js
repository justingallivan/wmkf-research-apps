...
export async function buildReviewContext({ suggestion, request, reviewer }) {
  ...
  // Post-accept guidance names the assigned, active Program Director. Keep the
  // lookup branch-specific and best-effort: missing/incomplete staff data must
  // not prevent a reviewer from reopening their confirmation page.
  let programDirector = null;
  const programDirectorId = request?._wmkf_programdirector_value || null;
  if (engagementState.view === 'accepted-pre-materials' && programDirectorId) {
    try {
