...
import { institutionNameOf } from '../../../shared/utils/review-writeup-paragraphs';
...
function buildStage2aPrefill(suggestion, reviewer, contact) {
  ...
  return {
    ...
    // name instead; a prior confirmed `wmkf_maininstitution` is the
    // reviewer's own value and is used as-is here (the composer reduces it
    // for display).
    ...
    mainInstitution: firstNonEmpty(
      ...
      institutionNameOf(firstNonEmpty(reviewer?.wmkf_primaryaffiliation, reviewer?.wmkf_organizationname)),
    ),
