...
export async function buildReviewContext({ suggestion, request, reviewer }) {
  ...
  return {
    ...
    programDirector,
    // The communicated review deadline (the request's campaign date — same
    // field the email templates fall back to). This is what the page shows
    // as "Submission deadline"; the token expiry below is deliberately later
    // (grace per lib/external/reviewer-token-ttl.js) and is NOT shown as a
    // deadline.
    reviewDeadline: request.wmkf_reviewduedate || null,
