 ...
 * failure is rethrown before the contact write. Deterministic typed failures
 * are marked non-retryable; bounded optimistic-concurrency exhaustion and
 * untyped transport failures remain retryable. No downstream work starts; the
 * decline caller remains best-effort after its response has already committed.
 */
...
const RETRYABLE_BINDING_ERROR_CODES = new Set(['concurrent_update_exhausted']);

function markDeterministicBindingError(error) {
  if (error instanceof IdentityBindingWriteError
      && !RETRYABLE_BINDING_ERROR_CODES.has(error.code)
      && error.code !== 'legacy_classification_required') {
...
/**
 * Dataverse persists DateTime columns at second precision (live sample:
 * docs/audits/reviewer-identity-binding-prod-preflight-2026-07-13.md), so a
 * millisecond-bearing event timestamp never round-trips equal to the stored
 * wmkf_identityboundat and a job retry would reclassify its own replay as a
 * rebind (version bump) or an out-of-order block. Truncate once here so the
 * durable event identity matches what Dataverse can store; an unparseable
 * value passes through for the writer's fail-closed validation to reject.
 */
function toDataverseSecondPrecision(value) {
...
export async function captureSelfReportedReviewerOrcid(
  ...
  if (bindingEventAt) {
    const eventAt = toDataverseSecondPrecision(bindingEventAt) || bindingEventAt;
    try {
      const binding = await writeBinding({
        potentialReviewerId,
        event: buildSelfReportBindingEvent(self.id, eventAt),
        actingUserSystemId,
      });
      if (!COMMITTED_BINDING_OUTCOMES.has(binding?.outcome)) {
        const blocked = new IdentityBindingWriteError(
      ...
      if (!(error instanceof IdentityBindingWriteError) || error.code !== 'legacy_classification_required') {
        throw markDeterministicBindingError(error);
      }
