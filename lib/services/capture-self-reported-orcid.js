 ...
 *
 * Trust model: a reviewer self-attestation is treated as `confirmed` —
 * `wmkf_identitystatus` the automated resolver NEVER emits (it only produces
 * probable/unresolved/ambiguous), so a stored `confirmed` is a sticky
 * human-attestation sentinel the resolver must not downgrade or clear
 * (enforced in researcher.writeIdentityDecision / clearIdentityFields).
 *
...
export async function captureSelfReportedReviewerOrcid(
  ...
  await researcher.writeIdentityDecision(potentialReviewerId, buildSelfReportDecision(self.id, ts), { actingUserSystemId });
