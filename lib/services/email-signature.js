...
function ensureFoundationLine(signature) {
  ...
  if (lines.length === 0 || normalizeLine(lines[lines.length - 1]) !== normalizeLine(FOUNDATION_LINE)) {
    lines.push(FOUNDATION_LINE);
  }
...
export function appendSignatureBlock(bodyText, signatureBlock) {
  ...
  const signature = ensureFoundationLine(signatureBlock?.signature || signatureBlock?.name || '');
  return `${body}\n\n${signature}`.trim();
}

export const EMAIL_SIGNATURE_FOUNDATION_LINE = FOUNDATION_LINE;
