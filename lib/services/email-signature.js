...
function finalizeSignatureBlock(raw, fallback = {}) {
  ...
  const isCustomSignature = Boolean(normalized.signature);
  ...
  return { signature, name, email, isCustomSignature };
}
...
export async function resolveSignatureForRequest(requestId) {
  try {
    ...
    const pd = await systemUserAdapter.getByIdWithSelect(systemuserId, SYSTEMUSER_SELECT)
      .catch(() => null);
    const fallback = {
      name: pd?.fullname || '',
