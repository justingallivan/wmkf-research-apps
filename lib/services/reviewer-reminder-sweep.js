...
export async function sendOneReminder({ kind, subjectTemplate, bodyTemplate, row, request, pd, signatureBlock, reviewer, actingUserSystemId, authorizeMint, result }) {
  ...
  // Manual sends can supply a fresh, path-specific authorization read after the
  // marker claim. Its ETag binds the token write so a concurrent removal or
  // revocation wins with 412 instead of being cleared by minting. Cron callers do
  // not supply this hook, preserving their current behavior until Phase B.
  let mintIfMatch;
  if (authorizeMint) {
    let authorization;
    try {
