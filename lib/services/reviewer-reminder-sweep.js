...
import { notExcludedFilter, queryAllSuggestions, patchFields } from '../dataverse/adapters/reviewer-suggestion.js';
 ...
 */
export async function sendOneReminder({ kind, subjectTemplate, bodyTemplate, row, request, pd, signatureBlock, reviewer, actingUserSystemId, authorizeMint, result }) {
  ...
  // Manual sends authorize immediately before one atomic marker+token PATCH.
  // Cron callers omit this hook and retain the established claim-then-mint flow.
  let mintIfMatch;
  ...
  if (!authorizeMint) {
    try {
      await patchFields(id, claimPatch, {
  ...
  try {
    ({ url } = await mintAndStore({
      ...
      actingUserSystemId,
      ifMatch: mintIfMatch,
      writeFields: mintIfMatch ? claimPatch : undefined,
    }));
  } catch (e) {
    if (mintIfMatch && e?.status === 412) {
      result.claimFailed++;
      return;
    }
    if (mintIfMatch) {
      // The email has not been attempted. Marker + token are one PATCH; a
      // transport failure may leave that PATCH outcome unknown, but a staff
      // retry cannot duplicate an email from this attempt.
      result.prepareFailed++;
      result.errors.push({ id, message: String(e.message || e).slice(0, 240) });
      return;
    }
    // Cron already claimed its fire-once marker before token preparation.
    result.sendFailed++;
    result.errors.push({ id, message: String(e.message || e).slice(0, 240) });
    return;
  }
