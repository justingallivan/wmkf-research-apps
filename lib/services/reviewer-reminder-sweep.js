...
function emptyResult(dryRun) {
  return { scanned: 0, eligible: 0, sent: 0, skipped: 0, claimFailed: 0, sendFailed: 0, errors: [], dryRun };
}
...
export async function sweepRespondReminders({ maxBatch = 200, dryRun = false, actingUserSystemId = null } = {}) {
  ...
  for (const row of records) {
    if (result.sent >= maxBatch) { result.skipped++; continue; }
 ...
 */
async function sendOneReminder({ kind, row, request, pd, signatureBlock, reviewer, actingUserSystemId, result }) {
  ...
  try {
    await DynamicsService.updateRecord(SUGGESTION_SET, id, claimPatch, {
      ifMatch: row['@odata.etag'] || row._etag,
      actingUserSystemId,
    });
  } catch (e) {
  ...
  try {
    ...
    const { subject, html } = kind === 'respond'
      ? renderRespondReminder({ reviewerName, title: request.akoya_title, signatureBlock, url })
      : renderReviewDueReminder({ reviewerName, title: request.akoya_title, reviewDueDate: request.wmkf_reviewduedate, signatureBlock, url });
