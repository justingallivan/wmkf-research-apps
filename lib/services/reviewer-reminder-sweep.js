...
import { getByIdWithSelect as getSystemUserByIdWithSelect } from '../dataverse/adapters/system-user.js';
...
import { loadSenderReminderTemplate } from './reviewer-reminder-personalization.js';
...
export async function loadRequestContext(requestId, cache) {
  ...
  try {
    ...
    if (request?.akoya_requestid) {
      ...
      if (pdGuid) {
        pd = await getSystemUserByIdWithSelect(pdGuid, 'systemuserid,fullname,internalemailaddress,isdisabled').catch(() => null);
      }
...
export async function sweepRespondReminders({ maxBatch = 200, dryRun = false, actingUserSystemId = null } = {}) {
  ...
  const senderTemplateCache = new Map();
  ...
  for (const row of records) {
    ...
    let senderTemplate = senderTemplateCache.get(pd.systemuserid);
    if (!senderTemplate) {
      senderTemplate = await loadSenderReminderTemplate(pd.systemuserid, 'respond', {
        subject: emailDefaults.values[RESPOND_SUBJECT_KEY],
        body: emailDefaults.values[RESPOND_BODY_KEY],
      });
      senderTemplateCache.set(pd.systemuserid, senderTemplate);
    }
    if (!senderTemplate.ok) {
      result.skipped++;
      result.errors.push({ id: row.wmkf_appreviewersuggestionid, message: senderTemplate.reason });
      continue;
    }
    ...
    await sendOneReminder({
      kind: 'respond',
      subjectTemplate: senderTemplate.template.subject,
      bodyTemplate: senderTemplate.template.body,
...
export async function sweepReviewDueReminders({ maxBatch = 200, dryRun = false, actingUserSystemId = null } = {}) {
  ...
  for (const row of records) {
    ...
    let senderTemplate = senderTemplateCache.get(pd.systemuserid);
    if (!senderTemplate) {
      senderTemplate = await loadSenderReminderTemplate(pd.systemuserid, 'reviewdue', {
        subject: emailDefaults.values[REVIEW_DUE_SUBJECT_KEY],
        body: emailDefaults.values[REVIEW_DUE_BODY_KEY],
      });
      senderTemplateCache.set(pd.systemuserid, senderTemplate);
    }
    if (!senderTemplate.ok) {
  ...
  try {
    ...
    const { subject, html } = kind === 'respond'
      ? reviewedContent
        ...
        : renderRespondReminder({
            subjectTemplate, bodyTemplate, reviewerName, title: request.akoya_title, signatureBlock, url,
            senderName: signatureBlock?.name || pd.fullname,
            senderEmail: signatureBlock?.email || pd.internalemailaddress,
          })
      : preparedReviewDue;
    ...
    if (dispatch.outcome === 'uncertain') result.sendUnconfirmed++;
    else result.sendFailed++;
    result.errors.push({
      id,
