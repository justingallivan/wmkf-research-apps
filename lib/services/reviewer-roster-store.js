...
const { PROVENANCE_KINDS, SEED_ROLES, provenanceKindOf, withReviewerProvenance } = require('../utils/reviewer-provenance');
...
function candidateFromRow(row) {
  ...
  if (candidate.provenance) return withReviewerProvenance(candidate);
...
async function recordSurfaced(requestId, candidates) {
  ...
  let recorded = 0;
  for (const c of list) {
    const candidate = withReviewerProvenance(c);
...
async function setExcluded(requestId, candidate) {
  const candidateWithProvenance = withReviewerProvenance(candidate);
