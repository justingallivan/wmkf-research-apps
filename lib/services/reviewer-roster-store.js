...
async function recordSurfaced(requestId, candidates, options = {}) {
  ...
  for (const c of list) {
    ...
    try {
      // Generic cold replacements preserve upstream receipts but always drop
      // the terminal roster-persistence receipt in both merge branches.
          ...
          SET status = CASE
              ...
              candidate = CASE
                ...
                THEN ${JSON.stringify(candidate)}::jsonb || jsonb_build_object(
                  ...
                  'stageFreshness', COALESCE(reviewer_find_roster.candidate->'stageFreshness', '{}'::jsonb) - 'roster_persistence'
                ))
...
function mergeColdStageEvidence({
  ...
  let partial = false;
  let code = null;
  const hasStaffIdentity = !!existing?.staffIdentityConfirmation || existing?.pdIdentityConfirmed === true;
  ...
  const projectsIdentity = Object.keys(projectedStageFreshness || {}).some((stage) => (
    ...
    try {
      ...
      if (!row) {
        ...
        const inserted = await sql`
          INSERT INTO reviewer_find_roster
            (request_id, candidate_key, normalized_name, display_name, status, candidate, source_kind)
          VALUES (
            ${requestId}, ${candidateKey}, ${normalized}, ${String(prepared.name)}, 'active',
      ...
      const updated = await sql`
        UPDATE reviewer_find_roster
           SET candidate = ${JSON.stringify(prepared)}::jsonb,
    ...
    UPDATE reviewer_find_roster
       SET candidate = candidate || jsonb_build_object(
             ...
             COALESCE(candidate->'stageRefresh', '{}'::jsonb)
               || jsonb_build_object(${stage}, ${lease}::jsonb)
           ),
           updated_at = now()
     WHERE request_id = ${requestId}
       AND candidate_key = ${candidateKey}
