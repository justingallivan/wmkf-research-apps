...
async function attestAddress(requestId, candidateKey, {
  ...
  const priorStoredEmail = normalizeAddress(currentCandidate.email);
  const priorFoundEmail = normalizeAddress(currentCandidate.contactEnrichment?.email);
  if ((currentCandidate.conflictRecordUnavailable === true
      || currentCandidate.addressConflictPending === true)
    && priorStoredEmail && priorFoundEmail && priorStoredEmail !== priorFoundEmail
    && (normalizedEmail === priorStoredEmail || normalizedEmail === priorFoundEmail)) {
    receipt = {
      ...receipt,
      resolvedAddressPair: {
        storedEmail: priorStoredEmail,
        foundEmail: priorFoundEmail,
  ...
  if (addressChanged) manualFields.add('email');
  const updatedCandidate = {
    ...currentCandidate,
    email: normalizedEmail,
    emailSource: addressChanged
      ? 'manual'
...
async function finalizeCandidatePromotion(
  ...
  `;
  return { saved: (res.rowCount || res.rows?.length || 0) > 0, candidateKey: key };
}
