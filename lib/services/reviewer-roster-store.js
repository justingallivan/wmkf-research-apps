...
const REFRESH_STATES = new Set(['refreshing', 'current', 'incomplete', 'failed']);
...
async function completeStageRefresh(requestId, candidateKey, expectedUpdatedAt, stage, attemptId, receipt = {}) {
  return updateStageRefresh(requestId, candidateKey, expectedUpdatedAt, stage, {
    state: 'current', contractVersion: Number.isInteger(receipt.contractVersion) ? receipt.contractVersion : null,
    sourceVersion: boundedRefreshText(receipt.sourceVersion, 160), completedAt: boundedRefreshText(receipt.completedAt || new Date().toISOString(), 80),
