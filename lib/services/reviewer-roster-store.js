...
const { reviewerCandidateKey, withReviewerCandidateKey } = require('../utils/reviewer-candidate-key');
...
/**
 * Bulk-record surfaced candidates as 'active'. Idempotent on (request, candidate):
  ...
  for (const c of list) {
    ...
    try {
      await sql`
        ...
        VALUES (${requestId}, ${candidateKey}, ${normalized}, ${String(name)}, 'active', ${JSON.stringify(candidate)}, ${sourceKindOf(candidate)})
