...
async function startStageRefresh(requestId, candidateKey, expectedUpdatedAt, stage, { attemptId = randomUUID(), reason = 'manual_refresh', startedAt = new Date().toISOString() } = {}) {
  ...
  if (!boundedRefreshText(attemptId) || !canonicalIso(startedAt)) return { outcome: 'rejected' };
...
async function completeStageRefresh(requestId, candidateKey, expectedUpdatedAt, stage, attemptId, receipt = {}) {
  ...
  if (!boundedRefreshText(attemptId) || receipt.contractVersion !== CONTRACT_VERSIONS[stage] || !boundedVersion(receipt.sourceVersion) || !canonicalIso(completedAt)) return { outcome: 'rejected' };
  return updateStageRefresh(requestId, candidateKey, expectedUpdatedAt, stage, {
...
async function failStageRefresh(requestId, candidateKey, expectedUpdatedAt, stage, attemptId, { terminal = false, errorCode = 'retryable_failure' } = {}) {
  ...
  const result = await updateStageRefresh(requestId, candidateKey, expectedUpdatedAt, stage, {
    ...
    reason: terminal ? 'stage_incomplete' : 'prior_write_incomplete', errorCode: boundedRefreshText(errorCode, 80),
  }, attemptId);
