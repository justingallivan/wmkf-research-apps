...
const { recordInstitutionMeasurement } = require('./reviewer-institution-measurement');
const {
  buildAttestation,
  normalizeAddress,
...
async function recordSurfaced(requestId, candidates, options = {}) {
  ...
  const measurementPromises = Array.isArray(options.measurementCollector)
    ? options.measurementCollector : [];
  const ownsMeasurementCollector = !Array.isArray(options.measurementCollector);
  ...
  for (const c of list) {
    ...
    try {
      ...
      recorded += changed;
      if (changed > 0) {
        measurementPromises.push(recordInstitutionMeasurement({
          requestId,
          candidate,
          eventType: 'roster_upsert',
          captureSource: options.measurementSource === 'server_applicant'
            ? 'server_applicant' : 'roster_unverified',
          assessment: options.measurementSource === 'server_applicant'
            ? options.measurementAssessment : null,
          legacyHold: options.measurementSource === 'server_applicant'
            ? candidate.institutionMismatch === true : null,
        }));
      }
    } catch (error) {
  ...
  if (ownsMeasurementCollector) await Promise.allSettled(measurementPromises);
  return recorded;
}
...
async function clearAddressTrustBlocks(requestId, candidateKey, {
  ...
  const receipt = current.addressTrustReceipt;
  const selectedEmail = normalizeAddress(addressChoice?.selectedEmail);
  const currentEmail = normalizeAddress(current.email || current.contactEnrichment?.email);
  const normalizedAddressChoice = addressChoice
    && ['keep_stored', 'use_found'].includes(addressChoice.decision)
    && receipt?.evidenceType === 'staff_address_choice'
    && selectedEmail
    && selectedEmail === normalizeAddress(receipt.email)
    && selectedEmail === currentEmail
    ? { decision: addressChoice.decision, selectedEmail }
    : null;
  if (receipt?.evidenceType === 'staff_address_choice' && !normalizedAddressChoice) return null;
  const readinessReason = normalizedAddressChoice
    ? STAFF_ADDRESS_CHOICE_REASON
  ...
  const applicantKnownReviewer = current.applicantKnownReviewer
    ? {
        ...
        addressChoice: normalizedAddressChoice,
        emailReadiness: {
          level: 'high',
          action: 'ready',
 ...
 */
async function findReconcilableCandidates(limit = 200, { excludeRequestIds = [] } = {}) {
  const cap = Math.min(1000, Math.max(1, Number(limit) || 200));
  // Requests the caller has already ruled out (Test Request Factory Stage 1c)
  // are left out here so they never take up the batch.
  const excluded = excludeRequestIds.map(String);
  const res = await sql`
    ...
    WHERE status IN ('active', 'saved')
      ...
      AND NOT (request_id = ANY(${excluded}::uuid[]))
    ORDER BY updated_at DESC
    LIMIT ${cap}
  `;
