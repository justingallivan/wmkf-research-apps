 ...
 * dedup key. Status: 'active' (surfaced; selectability is also gated by the
 * candidate blob) | 'excluded' (collapsed, recoverable) | 'saved' (graduated to
 * the Dataverse pool; kept for dedup) | 'coi_dropped' (discovery-time COI drop
 * ledger). See docs/atlas/postgres-reviewer-find-roster.md + migrations 020/023.
 *
...
// a follow-up). Excluded rows and COI-drop ledger rows are never evicted.
 ...
 * downgrades active/excluded/saved rows to avoid changing staff-visible state.
 */
  ...
  for (const c of list) {
    ...
    try {
      await sql`
        INSERT INTO reviewer_find_roster
          (request_id, normalized_name, display_name, status, candidate, source_kind)
        VALUES (${requestId}, ${normalized}, ${String(name)}, 'coi_dropped', ${JSON.stringify(candidate)}, ${sourceKindOf(candidate)})
        ON CONFLICT (request_id, normalized_name) DO UPDATE
          SET candidate = ${JSON.stringify(candidate)}, source_kind = ${sourceKindOf(candidate)}, updated_at = now()
...
async function confirmIdentity(requestId, candidate, {
  ...
  const safeCandidate = sanitizeCandidateWebsite(withReviewerProvenance(candidate));
  const contact = canonicalManualConfirmation(safeCandidate);
  if (!requestId || !contact.normalizedName || !contact.email) {
    throw new Error('reviewer-roster.confirmIdentity: request, candidate name, and email required');
  }
  ...
  const res = await sql`
    UPDATE reviewer_find_roster
      SET candidate = candidate || ${JSON.stringify(confirmedCandidate)}::jsonb,
          updated_at = now()
      WHERE request_id = ${requestId}
        AND normalized_name = ${contact.normalizedName}
        AND status = 'active'
    RETURNING candidate
  `;
 ...
 * active Find list but stay deduped. Leaves 'excluded' and 'coi_dropped' rows untouched.
 ...
 * `(request_id, normalized_name)` roster identity used at search time.
 */
async function stampSuggestionAnchor(requestId, name, { suggestionId, potentialReviewerId } = {}) {
  const normalized = normalizeReviewerName(name);
  if (!requestId || !normalized || !suggestionId) return { updated: 0 };
