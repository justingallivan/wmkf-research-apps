...
async function completeStageRefresh(requestId, candidateKey, expectedUpdatedAt, stage, attemptId, receipt = {}) {
  ...
  return updateStageRefresh(requestId, candidateKey, expectedUpdatedAt, stage, {
    ...
    refreshAttemptId: null, refreshStartedAt: null, reason: null, errorCode: null,
  }, attemptId);
}

async function failStageRefresh(requestId, candidateKey, expectedUpdatedAt, stage, attemptId, { terminal = false, errorCode = 'retryable_failure' } = {}) {
  if (!validRefreshText(attemptId)) return { outcome: 'rejected' };
  const outcome = terminal ? 'failed_terminal' : 'failed_retryable';
  const safeErrorCode = ['retryable_failure', 'terminal_failure'].includes(errorCode) ? errorCode : 'retryable_failure';
...
async function recoverExpiredStageRefresh(requestId, candidateKey, expectedUpdatedAt, stage, attemptId, { leaseMs = 15 * 60 * 1000 } = {}) {
  ...
  const res = await sql`
    ...
    WHERE request_id = ${requestId} AND candidate_key = ${candidateKey}
      ...
      AND (candidate->'stageFreshness'->${stage}->>'refreshStartedAt')::timestamptz <= now() - (${leaseMs} * interval '1 millisecond')
    RETURNING candidate, updated_at::text AS updated_at_token
  `;
  if (!(res.rowCount || res.rows?.length)) return { outcome: 'skipped_stale' };
