...
/**
 * Source-aware affiliation relationship and consumer policy.
 *
 * Stage 2 runtime callers may consume only its notification and candidate-card
 * projections behind an explicit rollout flag. Identity, selection, and write
 * consumers remain unauthorized. The module separates organization relationship
 * truth from evidence-time interpretation and consumer action; affiliation never
 * establishes person identity, and unknown high-authority inputs fail closed.
 */
...
function directRelationship(left, right) {
  // Parent/child claims describe the source ROR entities. A canonical id may
  // point at a shared system parent; admitting it here can collapse sibling
  // campuses into a false parent/child match.
  const rightSourceIds = new Set([right.sourceRorId].filter(Boolean));
  const leftSourceIds = new Set([left.sourceRorId].filter(Boolean));
  if (left.organizationScope === 'subunit' && rightSourceIds.has(left.parentRorId)) {
    return 'left_child_of_right';
  }
  if (right.organizationScope === 'subunit' && leftSourceIds.has(right.parentRorId)) {
    return 'left_parent_of_right';
  }
  const leftHas = (type) => [...relationshipIds(left, type)].some((id) => rightSourceIds.has(id));
  const rightHas = (type) => [...relationshipIds(right, type)].some((id) => leftSourceIds.has(id));
  ...
  if (!selected) {
    ...
    selected = hasUnresolved
      ? (comparisons.length === 1
        ? { ...comparisons[0], leftIndex: null, rightIndex: null }
