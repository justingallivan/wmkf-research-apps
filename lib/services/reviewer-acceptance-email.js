...
export async function sendAcceptanceConfirmationEmail({
  ...
  // With Test Request isolation on, the confirmation must name its request so
  // the delivery seam can refuse test requests; never send it unlinked.
  if (!request?.akoya_requestid && testRequestIsolationEnabled()) {
    throw new Error('acceptance confirmation request missing');
  }
  if (!from) {
  ...
  await DynamicsService.createAndSendEmail({
    ...
    attachments: ics ? [ics] : [],
    actingUserSystemId,
    // Never silently downgrade a PD-attributed confirmation to the service
    // principal (outbound-email inventory 2026-08-26). Harmless on the
    // NOTIFICATION_EMAIL_FROM fallback branch, where actingUserSystemId is
    // undefined. The drain records a throw as a non-fatal failed step
    // (at-most-once), not a retry loop.
    noFallback: true,
  });
  return { sent: true };
}
