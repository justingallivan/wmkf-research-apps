...
import { EXECUTOR_BUDGET_DEFAULTS, EXECUTOR_BUDGET_LIMITS } from '../../shared/config/executorBudgets.js';
...
export async function prepareRequestInput(requestId, { requestNumber = null } = {}) {
  ...
  return freezeDeep({
    ...
    // Applicant lookup only: wmkf_organizationname is a Bill.com field (PR #201 decision).
...
function snapshotPrompt(row, expectedName) {
  ...
  // The prompt editor's model dropdown may submit a tier key (e.g. "opus")
  // instead of a concrete id (shared/components/admin/PromptTemplatesSection.js).
  // Resolve it through the same resolver getModelForApp uses before applying
  // the concrete-id check, mirroring review-panel-generation.js's snapshotPrompt.
  const resolvedModel = resolveModel(row.wmkf_ai_model) || row.wmkf_ai_model;
  if (!/^claude-[a-z0-9]+(?:-[a-z0-9]+)+$/.test(String(resolvedModel || ''))) {
    throw new CycleDossierInputError(`Prompt ${expectedName} must pin a concrete model id — got "${row.wmkf_ai_model}"`, 'cycle_dossier_prompt_invalid');
  }
...
// The entry stage's transport timeout is an admin-tunable Executor budget
// (`cycle-dossier.entry`), pinned into the run's captured config at preview time
// like the prompt rows. A settings-store outage falls back to the registry
// default rather than failing the preview.
export async function resolveEntryTimeoutMs() {
  const fallback = EXECUTOR_BUDGET_DEFAULTS[ENTRY_PROMPT_NAME].timeoutMsOverride;
  try {
    const budget = await getExecutorBudget(ENTRY_PROMPT_NAME);
    return Number.isInteger(budget?.timeoutMsOverride) && budget.timeoutMsOverride > 0 ? budget.timeoutMsOverride : fallback;
  } catch (error) {
    console.error('[cycle-dossier] executor budget read failed; using the registry default entry timeout:', error?.message);
    return fallback;
  }
}

export async function snapshotConfiguration({ budget = {}, pricing = null } = {}) {
  const [researchRow, entryRow, entryTimeoutMs] = await Promise.all([
    fetchCurrentPrompt(RESEARCH_PROMPT_NAME),
    fetchCurrentPrompt(ENTRY_PROMPT_NAME),
    Number.isInteger(budget?.entryTimeoutMs) ? budget.entryTimeoutMs : resolveEntryTimeoutMs(),
  ]);
  // Resolve a tier key (e.g. "opus") before pricing lookup — same resolver
  // used in snapshotPrompt's D8 check below, since a tier-keyed row that
  // passes D8 must not silently price out as null.
  const resolvedPricing = pricing || {
    researchPlan: pricingForModel(resolveModel(researchRow?.wmkf_ai_model) || researchRow?.wmkf_ai_model),
    entry: pricingForModel(resolveModel(entryRow?.wmkf_ai_model) || entryRow?.wmkf_ai_model),
  };
  return freezeDeep({
    ...
    budget: { ...(clone(budget) || {}), entryTimeoutMs },
...
// The Executor builds its LLMClient without an appName on purpose (the driver
// owns api_usage_log accounting; see execute-prompt.js), so each paid dossier
// stage logs its own row here, before any checkpoint, on success and failure.
// logUsage is fire-and-forget with its own catch and never throws.
const USAGE_APP_NAME = 'cycle-dossier';
async function loggedExecute(execute, args, { userProfileId = null, model = null } = {}) {
  const startedAt = Date.now();
  try {
    const result = await execute(args);
    const usage = result?.usage || {};
    logUsage({
      userProfileId, appName: USAGE_APP_NAME, model: result?.meta?.modelUsed || model,
      inputTokens: usage.input_tokens || 0, outputTokens: usage.output_tokens || 0,
      cacheCreationTokens: usage.cache_creation_input_tokens || 0, cacheReadTokens: usage.cache_read_input_tokens || 0,
      latencyMs: Date.now() - startedAt, status: 'success', errorMessage: null, stopReason: null, requestId: null, requestRound: null,
    });
    return result;
  } catch (error) {
    logUsage({
      userProfileId, appName: USAGE_APP_NAME, model, inputTokens: 0, outputTokens: 0,
      latencyMs: Date.now() - startedAt, status: 'error', errorMessage: text(error?.message, 500) || 'unknown', stopReason: null, requestId: null, requestRound: null,
    });
    throw error;
  }
}
...
export async function generateResearch(input, config, { beforePaidCall, deadlineMs = null, actingUserSystemId = null, userProfileId = null, signal = null, execute = executePrompt, openAlexSearch = (...args) => OpenAlexService.searchWorks(...args), pubmedSearch = (...args) => PubMedService.search(...args) } = {}) {
  ...
  const planResult = await loggedExecute(execute, {
    ...
    deadlineMs, actingUserSystemId, signal,
    timeoutMsOverride: boundedLlmTimeout(config),
  }, { userProfileId, model: prompt.wmkf_ai_model });
  ...
  for (const planned of queries) {
    ...
    const querySignal = signal ? AbortSignal.any([timeoutSignal, signal]) : timeoutSignal;
    ...
    const [openAlex, pubmed] = await Promise.allSettled([
      openAlexSearch(planned.query, { limit: MAX_RESULTS_PER_SOURCE, signal: querySignal }),
      (async () => {
        await paidCall(beforePaidCall, { stage: 'research-search', query: planned.query, source: 'pubmed' });
        return pubmedSearch(planned.query, MAX_RESULTS_PER_SOURCE, { throwOnError: true, signal: querySignal });
      })(),
    ]);
    // allSettled would otherwise record an operator-stop abort as an adapter
    // failure and checkpoint a degraded research result (or fail the item).
...
export async function generateEntry(input, research, config, { beforePaidCall, deadlineMs = null, actingUserSystemId = null, userProfileId = null, signal = null, execute = executePrompt } = {}) {
  ...
  const result = await loggedExecute(execute, {
    ...
    deadlineMs, actingUserSystemId, signal,
    timeoutMsOverride: boundedEntryTimeout(config),
  }, { userProfileId, model: prompt.wmkf_ai_model });
...
// Research-plan stage: one short structured call; the 85 s default has held.
...
// Entry stage: the pinned Executor budget, clamped to the registry's reviewed
// envelope so a stale or hand-edited snapshot cannot exceed the lease.
function boundedEntryTimeout(config) {
  const { min, max } = EXECUTOR_BUDGET_LIMITS[ENTRY_PROMPT_NAME].timeoutMsOverride;
  const fallback = EXECUTOR_BUDGET_DEFAULTS[ENTRY_PROMPT_NAME].timeoutMsOverride;
  const requested = Number(config?.budget?.entryTimeoutMs);
