 ...
 *
 * Contract: fires only AFTER the atomic write committed, NEVER throws, and NEVER
 * blocks the response for more than NOTIFY_BUDGET_MS. A submission the grantee
 * completed must not fail because a notification did, so every internal failure —
 * recipient resolution, the notify() call itself — is caught and logged here
 * rather than propagating to the route.
 *
 * Why the time budget: this route is not in vercel.json's `functions` map, so it
 * runs on the platform default duration, and by the time we get here the request
 * has already spent time on virus scan + SharePoint upload + the changeset. An
 * unbounded await could let the platform kill the invocation before the 200 is
 * written — the grantee would see a timeout on a submission that DID commit, and
 * their retry would be refused as not_editable. A try/catch cannot catch platform
 * termination, so the wait is bounded instead. On expiry the in-flight send is
 * abandoned (it may still land; notify() is not transactional) and staff still
 * learn about the package from the Awardee/awardees status surfaces.
 *
...
// How long the committed submit will wait on the notification before responding.
export const NOTIFY_BUDGET_MS = 3000;
