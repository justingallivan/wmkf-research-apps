...
import { snapshotConfiguration, estimateReservationCost } from './review-panel-generation';
...
export function projectReviewPanelRun(row, entries = []) {
  return {
    id: row.id,
    status: row.status,
...
async function projectRunsWithEntries(runs) {
  return reviewPanelPool(runs, async (run) => projectReviewPanelRun(run, await store.listReviewPanelEntries(run.id)));
}
...
export async function getReviewPanelPage(owner) {
  ...
  try {
    ...
    configuration = { ready: false, error: error.httpStatus ? error.message : 'Published review panel prompts are not ready. Check Admin configuration.' };
  }
...
export async function launchReviewPanel(owner, body) {
  ...
  if (replay) {
    return { run: projectReviewPanelRun(replay, await store.listReviewPanelEntries(replay.id)) };
  }
  ...
  return { run: projectReviewPanelRun(run, await store.listReviewPanelEntries(run.id)) };
}
...
export async function controlReviewPanel(owner, body) {
  ...
  if (body.action === 'stop') {
    ...
    return { run: projectReviewPanelRun(row, await store.listReviewPanelEntries(row.id)) };
  }
