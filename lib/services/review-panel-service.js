...
export function projectReviewPanelRun(row, entries = [], attempts = []) {
  ...
  return {
    ...
    entries: entries.map((e) => ({
      ...
      // previousFiles is surfaced only as a count — the caller never needs
      // the superseded refs themselves, only that an earlier edition existed.
      rerender: e.data?.rerender
        ? { requestedAt: e.data.rerender.requestedAt, previousFiles: Object.keys(e.data.rerender.previousFiles || {}).length }
        : null,
...
export async function controlReviewPanel(owner, body) {
  ...
  if (body.action === 'rerender') {
    const entryIds = ids(body.entryIds, 'entry selection');
    const run = await store.requestReviewPanelRerender(runId, owner, entryIds);
    return { run: await projectRun(run) };
  }
