...
function buildReviewPanelTimeline(row, entries, attempts) {
  ...
  for (const entry of entries) {
    ...
    // Visible immediately on the operator's own retry click, independent of
    // the worker's next pass (which is the only thing that ever clears it) —
    // the owner-reported gap was exactly this: no retry signal in the
    // timeline until the run itself later changed state.
    if (entry.retry_requested_at) {
      events.push({ at: entry.retry_requested_at, label: 'Retry requested' });
    }
    ...
    // while "Report saved" is the signal an operator watching a RETRY
    // specifically needs. entry.updated_at is the only per-entry write
    // timestamp this projection has for the save (set by
    // completeEntryWithReport's mutateReviewPanelEntry call) — falls back to
    // the run's own updated_at only if that's ever missing, and is omitted
    // entirely (never invented) if neither exists.
    if (hasFiles && entry.status === 'completed') {
      const at = entry.updated_at || row.updated_at;
...
export async function getReviewPanelPage(owner) {
  await store.assertReviewPanelActor(owner);
  assertReviewPanelPilotEnabled();
...
export async function controlReviewPanel(owner, body) {
  ...
  if (body.action === 'retry') {
    ...
    const run = await store.requestReviewPanelRetry(runId, owner, entryIds);
    return { run: await projectRun(run) };
  }
  if (body.action === 'stop') {
...
export async function reviewPanelAction(owner, body) {
  ...
  if (['operator-stop', 'stop', 'retry'].includes(body.action)) return controlReviewPanel(owner, body);
