...
import { testRequestVisibilityDto, withTestRequestIsolationSelect } from './test-requests/isolation.js';
...
export async function loadReviewPanelRoster() {
  ...
  const result = await requests.queryAllRequests({
    select: withTestRequestIsolationSelect('akoya_requestid,akoya_requestnum,akoya_title,akoya_requeststatus,_akoya_applicantid_value,_wmkf_projectleader_value,_wmkf_programdirector_value'),
...
/** Configuration block shared by the standalone page and the per-request Workbench tab. Never throws: a misconfigured rollout must not break a read-only status surface. */
async function loadConfigurationSummary() {
  ...
  try {
    ...
    configuration = {
      ready: true,
      seats: Object.values(config.seats).map((s) => ({ seatKey: s.seatKey, label: getReviewPanelSeatLabel(s.seatKey), provider: s.provider, model: s.model })),
...
export async function getReviewPanelPage(owner) {
  ...
  const configuration = await loadConfigurationSummary();
  return {
...
const UNSETTLED_RUN_STATUSES = new Set(['queued', 'running']);
const ACTIVE_RUN_MESSAGE = (requestNumber) => `A panel is already running for request #${requestNumber}. Wait for it to settle before launching another.`;

/**
 * Server-side guard against a second paid run for a request that already has
 * an unsettled one (the tab's Launch button mirrors this via `launchable`).
 * A request is "running" when an unsettled run either has an entry row for it
 * that is not yet completed/failed, or still parks it in pendingEntries.
 */
async function assertNoActiveRunForRequests(selected, byId) {
  for (const requestId of selected) {
    const runs = await store.listReviewPanelRunsForRequest(requestId);
    for (const run of runs.filter((r) => UNSETTLED_RUN_STATUSES.has(r.status))) {
      const entries = (await store.listReviewPanelEntries(run.id)).filter((e) => String(e.request_id).toLowerCase() === requestId);
      const pending = (run.data?.pendingEntries || []).some((p) => String(p.requestId).toLowerCase() === requestId);
      if (pending || entries.some((e) => !['completed', 'failed'].includes(e.status))) {
        throw store.reviewPanelError(ACTIVE_RUN_MESSAGE(byId.get(requestId)?.requestNumber || requestId), 409);
      }
    }
  }
}
...
/**
 * Per-request read for the Request Workbench tab (T3 shared visibility):
 * every run that touched this request, each projected with ONLY this
 * request's entries/attempts (so the timeline is this request's), plus who
 * launched it and whether that is the viewer (`owner.isMine` — the ONLY
 * signal the tab may use to show retry/re-render/stop, which the store keeps
 * owner-scoped). `launchable` is computed by calling the server's own launch
 * preconditions so the tab never re-derives them.
 */
export async function getReviewPanelForRequest(owner, requestId) {
  await store.assertReviewPanelAccess(owner);
  assertReviewPanelPilotEnabled();
  const id = guid(String(requestId || '').toLowerCase(), 'request');
  const [runs, control, configuration] = await Promise.all([
    store.listReviewPanelRunsForRequest(id), store.readReviewPanelControl(), loadConfigurationSummary(),
  ]);
  let rosterItem = null;
  let rosterError = null;
  try {
    rosterItem = (await loadReviewPanelRoster()).find((r) => r.requestId === id) || null;
  } catch (error) {
    if (!error.httpStatus) throw error;
    rosterError = error.message;
  }
  const projected = await reviewPanelPool(runs, async (run) => {
    const allEntries = await store.listReviewPanelEntries(run.id);
    const entries = allEntries.filter((e) => String(e.request_id).toLowerCase() === id);
    const attempts = await store.listEntryAttempts(entries.map((e) => e.id));
    const view = projectReviewPanelRun(run, entries, attempts);
    // How many requests the WHOLE run spans (materialized entries plus still-parked
    // pendingEntries). Stop is run-scoped on the server (requestReviewPanelCancel),
    // so the tab must tell the user when stopping affects other requests too.
    const requestCount = new Set([
      ...allEntries.map((e) => String(e.request_id).toLowerCase()),
      ...(run.data?.pendingEntries || []).map((p) => String(p.requestId).toLowerCase()),
    ]).size;
    return {
      ...view,
      requestCount,
      failures: (view.failures || []).filter((f) => String(f.requestId || '').toLowerCase() === id),
      pending: !entries.length && UNSETTLED_RUN_STATUSES.has(run.status)
        && (run.data?.pendingEntries || []).some((p) => String(p.requestId).toLowerCase() === id),
      owner: { profileId: run.owner_profile_id, name: run.owner_name || null, isMine: Number(run.owner_profile_id) === Number(owner) },
    };
  });
  const activeRun = projected.find((r) => UNSETTLED_RUN_STATUSES.has(r.status)
    && (r.pending || r.entries.some((e) => !['completed', 'failed'].includes(e.status)))) || null;

  // Mirror launchReviewPanel's preconditions, in its order, as a reason string.
  let launchable = { ok: true, reason: null };
  const block = (reason) => { if (launchable.ok) launchable = { ok: false, reason }; };
  try { assertReviewPanelStorageConfigured(); } catch (error) { block(error.httpStatus ? error.message : 'The review panel document store is not configured.'); }
  if (!configuration.ready) block(configuration.error);
  if (rosterError) block(rosterError);
  else if (!rosterItem) block('This request is outside the review panel roster.');
  if (activeRun) block(ACTIVE_RUN_MESSAGE(rosterItem?.requestNumber || activeRun.entries[0]?.requestNumber || id));

  return {
    request: rosterItem ? { ...rosterItem, inRoster: true } : { requestId: id, inRoster: false },
    launchable,
...
export async function launchReviewPanel(owner, body) {
  ...
  await assertNoActiveRunForRequests(selected, byId);
