...
/** "Source" column — origin label (Applicant-Referred, Literature-retrieved, …). */
export function sourceLabel(c) {
  ...
  return c?.isApplicantRecommended ? 'Applicant-Referred' : 'Candidate';
}
...
export function formatConflicts(c) {
  ...
  if (c?.hasCoauthorCOI) {
    ...
    // Total SHARED PAPERS across proposal authors — sum each entry's paperCount
    // (the card does the same at ReviewerSearchSection.js). `coauthorships.length`
    // would count conflicting AUTHORS, not papers. Fall back to 1/entry for any
    // opaque (non-object) entry so a legacy shape still reads sensibly.
    const list = Array.isArray(c.coauthorships) ? c.coauthorships : [];
    const n = list.reduce((s, co) => s + (co && typeof co === 'object' ? (Number(co.paperCount) || 0) : 1), 0);
...
const CANDIDATE_COLUMNS = [
  ...
  { header: 'Expertise tags', key: 'keywords', width: 36 },
