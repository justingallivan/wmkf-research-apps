 ...
 */
export async function createEmailActivity(svc, { subject, body, from, to, cc, regardingId, regardingType, actingUserSystemId, noFallback = false }) {
  ...
  const resp = await observeReviewerFindWarmEffect({
    effectClass: 'email',
    operation: 'dataverse_email_create',
  }, () => svc._writeFetch(`${baseUrl}/api/data/v9.2/emails`, {
    method: 'POST',
...
export async function sendEmail(svc, emailId, { actingUserSystemId, noFallback = false } = {}) {
  ...
  const resp = await observeReviewerFindWarmEffect({
    ...
    operation: 'dataverse_send_email_action',
  }, () => svc._writeFetch(`${baseUrl}/api/data/v9.2/emails(${emailId})/Microsoft.Dynamics.CRM.SendEmail`, {
    method: 'POST',
    headers: svc._withCallerId(svc.buildHeaders(token), actingUserSystemId),
    body: JSON.stringify({
      IssueSend: true,
