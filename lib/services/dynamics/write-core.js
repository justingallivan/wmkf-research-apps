...
import { assertTestRequestMarkerNotWritten } from '../test-requests/isolation.js';
...
export async function createRecord(svc, entitySet, data, { actingUserSystemId, noFallback = false } = {}) {
  ...
  assertTestRequestMarkerNotWritten(data, entitySet);
  const token = await svc.getAccessToken();
  // A service that supplies its own `baseUrl` (e.g. the Test Request Factory
  // sandbox deps, lib/services/test-requests/ia-sandbox-deps.js) is bound to
  // one explicit Dataverse org and must never fall back to the environment —
  // that fallback is reserved for the module-singleton `DynamicsService`,
  // which has no `baseUrl` property and keeps today's env-driven behavior.
  const baseUrl = typeof svc.baseUrl === 'string' && svc.baseUrl
    ? svc.baseUrl
    : `${process.env.DYNAMICS_URL}/api/data/v9.2`;
...
export async function updateRecord(svc, entitySet, recordId, data, { ifMatch, actingUserSystemId, noFallback = false } = {}) {
  ...
  assertTestRequestMarkerNotWritten(data, `${entitySet}(${recordId})`);
  const token = await svc.getAccessToken();
  // A service that supplies its own `baseUrl` (e.g. the Test Request Factory
  // sandbox deps, lib/services/test-requests/ia-sandbox-deps.js) is bound to
  // one explicit Dataverse org and must never fall back to the environment —
  ...
  const baseUrl = typeof svc.baseUrl === 'string' && svc.baseUrl
    ? svc.baseUrl
    : `${process.env.DYNAMICS_URL}/api/data/v9.2`;
  const url = `${baseUrl}/${entitySet}(${recordId})`;
...
export async function deleteRecord(svc, entitySet, recordId, { actingUserSystemId, ifMatch } = {}) {
  ...
  assertTestRequestMarkerNotWritten(undefined, `${entitySet}(${recordId})`);
  const token = await svc.getAccessToken();
  // A service that supplies its own `baseUrl` (e.g. the Test Request Factory
  // sandbox deps, lib/services/test-requests/ia-sandbox-deps.js) is bound to
  // one explicit Dataverse org and must never fall back to the environment —
...
export async function disassociate(svc, entitySet, recordId, navProperty, { actingUserSystemId } = {}) {
  ...
  const baseUrl = typeof svc.baseUrl === 'string' && svc.baseUrl
    ? svc.baseUrl
    : `${process.env.DYNAMICS_URL}/api/data/v9.2`;
  const url = `${baseUrl}/${entitySet}(${recordId})/${navProperty}/$ref`;
