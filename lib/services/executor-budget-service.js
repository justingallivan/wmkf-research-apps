...
const STANDING_KEYS = ['kind', 'maxTokensOverride', 'timeoutMsOverride'];
const RETRY_KEYS = ['kind', 'floor', 'ceiling'];
const TIMEOUT_KEYS = ['kind', 'timeoutMsOverride'];

function expectedKeysFor(kind) {
  if (kind === 'standing') return STANDING_KEYS;
  if (kind === 'retry') return RETRY_KEYS;
  if (kind === 'timeout') return TIMEOUT_KEYS;
  throw new Error(`Executor budget registry has an unknown kind "${kind}".`);
}

function validateOneBudget(promptName, entry) {
  const kind = EXECUTOR_BUDGET_DEFAULTS[promptName].kind;
  const label = `budgets.${promptName}`;
  assertExactKeys(entry, expectedKeysFor(kind), label);
  if (entry.kind !== kind) throw invalid(`${label}.kind must be "${kind}".`);
  const limits = EXECUTOR_BUDGET_LIMITS[promptName];
  if (kind === 'standing') {
    return {
      kind,
      maxTokensOverride: boundedInteger(entry.maxTokensOverride, limits.maxTokensOverride, `${label}.maxTokensOverride`),
      timeoutMsOverride: boundedInteger(entry.timeoutMsOverride, limits.timeoutMsOverride, `${label}.timeoutMsOverride`),
    };
  }
  if (kind === 'retry') {
    const normalized = {
      kind,
      floor: boundedInteger(entry.floor, limits.floor, `${label}.floor`),
      ceiling: boundedInteger(entry.ceiling, limits.ceiling, `${label}.ceiling`),
    };
    if (normalized.floor > normalized.ceiling) throw invalid(`${label}.floor must not exceed ceiling.`);
    return normalized;
  }
  return {
    kind,
 ...
 *
 * Publication (`fillRegisteredDefaults: false`) requires every registered prompt
 * name. Reads of stored revisions pass `fillRegisteredDefaults: true`: a
 * revision published before a prompt name was registered stays authoritative
 * for the names it carries and the missing names take the code default, so
 * additive registry growth never invalidates durable state. Unknown names are
 * rejected either way.
 */
export function validateExecutorBudgets(value, { fillRegisteredDefaults = false } = {}) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw invalid('budgets must be an object.');
  }
  const unknown = Object.keys(value).filter(name => !EXECUTOR_BUDGET_PROMPT_NAMES.includes(name));
  if (unknown.length) {
    throw invalid(`budgets contains unregistered prompt names: ${unknown.join(', ')}.`);
  }
  if (!fillRegisteredDefaults) {
    assertExactKeys(value, EXECUTOR_BUDGET_PROMPT_NAMES, 'budgets');
  }
  const normalized = {};
  for (const promptName of EXECUTOR_BUDGET_PROMPT_NAMES) {
    const entry = Object.prototype.hasOwnProperty.call(value, promptName)
      ? value[promptName]
      : clone(EXECUTOR_BUDGET_DEFAULTS[promptName]);
    normalized[promptName] = validateOneBudget(promptName, entry);
  }
  return normalized;
}
...
function parsePublication(key, meta) {
  ...
  return {
    ...
    settingKey: key,
    source: 'dataverse',
    budgets: validateExecutorBudgets(envelope.budgets, { fillRegisteredDefaults: true }),
    limits: clone(EXECUTOR_BUDGET_LIMITS),
    descriptions: clone(EXECUTOR_BUDGET_DESCRIPTIONS),
...
async function assertModelCeilings(budgets, dependencies) {
  ...
  const modelLimits = {};
  for (const promptName of EXECUTOR_BUDGET_PROMPT_NAMES) {
    // Timeout-only budgets carry no output tokens, so no model ceiling applies
    // and the prompt row is not read.
    if (budgets[promptName].kind === 'timeout') continue;
    let prompt;
    try {
...
export async function assertExecutorBudgetForPromptModel(
  ...
  if (EXECUTOR_BUDGET_DEFAULTS[promptName].kind === 'timeout') return null;
