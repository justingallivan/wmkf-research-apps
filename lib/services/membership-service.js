...
import * as membershipAdapter from '../dataverse/adapters/membership.js';
...
export async function getLiveMembershipsForContact(contactId) {
  ...
  const { records } = await membershipAdapter.queryMemberships(SELECT_FIELDS, filter, 100);
...
/**
 * Any-role membership guard. Returns true iff the contact has a live
 * (approved + active) membership on the given account with any role —
 * Submitter OR Contributor. Used by the autosave endpoint
 * /api/intake/draft, where Contributors are explicitly allowed to edit
 * drafts (only the final submit is Submitter-gated).
 */
export async function hasLiveMembership(contactId, accountId) {
  if (!contactId || !accountId) return false;
  const live = await getLiveMembershipsForContact(contactId);
  return live.some((m) => m.accountId === accountId);
}
