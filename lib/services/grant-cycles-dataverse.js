...
// OData v4 string-literal escape (apostrophe doubling, sourced from canonical
// odata.escape) + URL encoding. The encodeURIComponent is deliberate: this
// literal sits in a path segment, not a $filter a client later encodes
// (OData Escape Consolidation Plan D1, owner ruling S331 option b — byte output
// unchanged; grant-cycles-dataverse-odata-escape.test.js pins the full URL).
function escapeODataString(s) {
  return encodeURIComponent(odata.escape(s));
}
...
const SELECT_FIELDS = [
  ...
  // wmkf_summarypages is deployed but no longer read or written (2026-09-06):
  // the summary-page extraction it configured was removed from analyze.
...
/**
 * @param {import('../utils/guid.js').Guid} id branded record id; interpolated
 *   into the `wmkf_appgrantcycles(${id})` key predicate. A `@ts-check`'d caller
 *   must narrow client input through `isGuid` before passing it here.
 */
...
// Aggregate counts in 2 Dataverse queries (proposal FetchXML + candidate
// $apply/groupby). Returns:
//   {
//     proposalCountsByFiscalYear: Map<cycleName, number>,
...
const PROPOSAL_REQUEST_TYPE = 100000001;
...
function parseMonthYear(value) {
  ...
  return { month: MONTH_NAMES.findIndex((name) => name.toLowerCase() === match[1].toLowerCase()) + 1, year: Number(match[2]) };
}

function proposalCountFetchXml() {
  return '<fetch aggregate="true"><entity name="akoya_request">'
    + '<attribute name="wmkf_meetingdate" alias="year" groupby="true" dategrouping="year"/>'
    + '<attribute name="wmkf_meetingdate" alias="month" groupby="true" dategrouping="month"/>'
    + '<attribute name="akoya_requestid" alias="count" aggregate="count"/>'
    + `<filter><condition attribute="wmkf_request_type" operator="eq" value="${PROPOSAL_REQUEST_TYPE}"/></filter>`
    + '</entity></fetch>';
}

function cycleNameForRow(row) {
  const parsed = parseMonthYear(row?.name) || parseMonthYear(row?.fiscalYearCode);
  return parsed ? { ...parsed, key: `${parsed.year}-${parsed.month}` } : null;
}

function proposalCountTotalFetchXml() {
  return '<fetch aggregate="true"><entity name="akoya_request">'
    + '<attribute name="akoya_requestid" alias="count" aggregate="count"/>'
    + `<filter><condition attribute="wmkf_request_type" operator="eq" value="${PROPOSAL_REQUEST_TYPE}"/></filter>`
    + '</entity></fetch>';
...
async function fetchCounts(cycles = []) {
  const client = await getClient();

  // Proposal counts are grouped by the persisted meeting date, never the
  // legacy fiscal-year text. FetchXML groups by year/month and scopes to
  // proposal requests so honorarium rows cannot inflate this count.
  const fetchXml = proposalCountFetchXml();
  const reqAgg = await client.get(
    `/akoya_requests?fetchXml=${encodeURIComponent(fetchXml)}`,
  );
  if (!reqAgg.ok) throw new Error(`request counts: ${reqAgg.status} ${reqAgg.text}`);
  const totalAgg = await client.get(
    `/akoya_requests?fetchXml=${encodeURIComponent(proposalCountTotalFetchXml())}`,
  );
  if (!totalAgg.ok) throw new Error(`request total: ${totalAgg.status} ${totalAgg.text}`);
  const proposalCountsByFiscalYear = new Map();
  const proposalCountsByCycleId = new Map();
  const cycleKeys = new Map();
  for (const cycle of cycles) {
    const parsed = cycleNameForRow(cycle);
    const key = parsed?.key;
    const cycleName = cycle.fiscalYearCode || cycle.name;
    if (cycleName && !proposalCountsByFiscalYear.has(cycleName)) {
      proposalCountsByFiscalYear.set(cycleName, 0);
    }
    if (key && !cycleKeys.has(key)) {
      cycleKeys.set(key, { cycleName, cycleId: cycle.id });
    }
  }
  ...
  for (const row of reqAgg.body.value || []) {
    ...
    const key = Number.isInteger(year) && month >= 1 && month <= 12 ? `${year}-${month}` : null;
    const cycle = key ? cycleKeys.get(key) : null;
    if (cycle) {
      proposalCountsByFiscalYear.set(cycle.cycleName, (proposalCountsByFiscalYear.get(cycle.cycleName) || 0) + count);
      if (cycle.cycleId) proposalCountsByCycleId.set(cycle.cycleId, (proposalCountsByCycleId.get(cycle.cycleId) || 0) + count);
      assignedProposalCount += count;
    }
    else if (cycles.length === 0 && key) {
      proposalCountsByFiscalYear.set(`${MONTH_NAMES[month - 1]} ${year}`, count);
    }
  }
  const totalProposalCount = Number(totalAgg.body.value?.[0]?.count) || 0;
  const unassignedProposalCount = Math.max(0, totalProposalCount - assignedProposalCount);
  ...
  return { proposalCountsByFiscalYear, proposalCountsByCycleId, candidateCountsByShortCode, unassignedCandidateCount, unassignedProposalCount };
}
