...
function cycleNameForRow(row) {
  const parsed = parseMonthYear(row?.name || row?.fiscalYearCode);
...
async function fetchCounts(cycles = []) {
  ...
  for (const cycle of cycles) {
    ...
    if (key && !cycleKeys.has(key)) {
      cycleKeys.set(key, cycleName);
    }
  }
  let unassignedProposalCount = 0;
  for (const row of reqAgg.body.value || []) {
    const count = Number(row.count) || 0;
    const year = Number(row.year);
    const month = Number(row.month);
    const key = Number.isInteger(year) && month >= 1 && month <= 12 ? `${year}-${month}` : null;
    const cycleName = key ? cycleKeys.get(key) : null;
    if (cycleName) proposalCountsByFiscalYear.set(cycleName, (proposalCountsByFiscalYear.get(cycleName) || 0) + count);
    else if (cycles.length > 0) unassignedProposalCount += count;
    else if (key) proposalCountsByFiscalYear.set(`${MONTH_NAMES[month - 1]} ${year}`, count);
  }
  ...
  return { proposalCountsByFiscalYear, candidateCountsByShortCode, unassignedCandidateCount, unassignedProposalCount };
}
