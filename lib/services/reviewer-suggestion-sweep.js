...
import { expireInvitation } from './reviewer-engagement/expire-invitation.js';
...
export async function sweepStaleInvites({
  ...
  // their invitations are never expired by this sweep.
  const requestTestState = createRequestTestStateLookup();
  const testSkips = {};
  const ordinaryCandidates = [];
  for (const s of candidates.records) {
    const testState = await requestTestState(s._wmkf_request_value);
    if (testState.kind === 'ordinary') ordinaryCandidates.push(s);
    else recordTestRequestSkip(testSkips, testState);
  }

  const eligible = ordinaryCandidates.filter((s) =>
