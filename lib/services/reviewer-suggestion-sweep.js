...
import { RESPONSE_TYPE_MAP } from '../dataverse/adapters/reviewer-suggestion.js';
...
export async function sweepStaleInvites({
  ...
  const candidates = await DynamicsService.queryAllRecords(SUGGESTION_SET, {
    select: 'wmkf_appreviewersuggestionid,wmkf_emailsentat,_wmkf_request_value,wmkf_accepted,wmkf_declined,wmkf_responsetype',
    filter: 'wmkf_selected eq true and wmkf_emailsentat ne null and (wmkf_accepted eq false or wmkf_accepted eq null) and (wmkf_declined eq false or wmkf_declined eq null) and wmkf_responsetype eq null',
  });
  ...
  for (let i = 0; i < requestIds.length; i += CHUNK) {
    ...
    const { records } = await DynamicsService.queryRecords(REQUEST_SET, {
  ...
  for (const s of toWrite) {
    try {
      await DynamicsService.updateRecord(SUGGESTION_SET, s.wmkf_appreviewersuggestionid, {
