...
class ContactEnrichmentService {
  ...
  static async enrichCandidate(candidate, options = {}) {
    ...
    if (useSerpSearch && credentials.serpApiKey) {
      ...
      // Also try to find Google Scholar profile (uses same API, adds value)
      if (!result.contactEnrichment.googleScholarUrl || result.contactEnrichment.googleScholarUrl.includes('view_op=search')) {
        try {
          const scholarResult = await SerpContactService.findScholarProfile(candidate, credentials.serpApiKey);
          if (scholarResult && scholarResult.scholarProfileUrl) {
            result.contactEnrichment.googleScholarUrl = scholarResult.scholarProfileUrl;
            result.contactEnrichment.tierResults.scholar_profile = scholarResult;
            onProgress({
              tier: 4,
              status: 'found',
              message: `Found Google Scholar profile (${scholarResult.scholarCitedBy || 0} citations)`,
            });
          }
        } catch (scholarError) {
