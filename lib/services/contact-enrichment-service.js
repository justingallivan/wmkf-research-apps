...
class ContactEnrichmentService {
  ...
  static _currentOrcidInstitutionNames(ce = {}) { return domainEvidence.currentOrcidInstitutionNames(ce); }
  ...
  static async enrichCandidate(candidate, options = {}) {
    ...
    if (tiers.applyTier0(candidate, result, { onProgress }) === 'finalize') {
      return this._finalize(candidate, result, {
        persist, onProgress, signal, deadlineAt, useSerpSearch, credentials,
      });
    }
    ...
    return this._finalize(candidate, result, {
      persist,
      onProgress,
      scholarCandidate: searchCandidate,
      signal,
      deadlineAt,
      useSerpSearch,
      credentials,
   ...
   * OpenAlex last-known > PubMed-recency). Stage 9: moved to
   * contact-enrichment/tiers.js; the facade
  ...
  static async enrichCandidates(candidates, options = {}) {
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      let enriched;
      try {
        enriched = await observeReviewerFindWarmEffect({
      ...
      if (ce.tiersUsed.includes('eligibility_evidence')) {
        results.stats.actualCost += COSTS.SERP_GOOGLE_SEARCH;
      }
    }
