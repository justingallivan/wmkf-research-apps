...
class ContactEnrichmentService {
  ...
  static _emailDomainContradictsInstitution(email, anchorInstitution) {
    ...
    // heriot/watt), shares a >=3-char prefix with a word (caltech vs california),
    // or matches the institution initialism. Generously keep-biased.
    const relatesToAnchor = (dt) =>
      anchorWords.some((aw) =>
        dt === aw
        || dt.includes(aw)
        || aw.includes(dt)
        || (aw.length >= 4 && dt.startsWith(aw.slice(0, 4)))
