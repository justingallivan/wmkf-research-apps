...
class ContactEnrichmentService {
  ...
  static async enrichCandidate(candidate, options = {}) {
    ...
    if (candidate.affiliation) {
      ...
      if (affiliationEmail) {
        ...
        return this._finalize(candidate, result, { persist, onProgress, signal });
      }
    }
    ...
    return this._finalize(candidate, result, { persist, onProgress, scholarCandidate: searchCandidate, signal });
  }
  ...
  static async _attachOpenAlexMetrics(candidate, result, { signal, onProgress = () => {} } = {}) {
    ...
    const carriedAuthorId = candidate.openAlexId || null;       // from discovery spine (mapSpineVerificationResult)
    ...
    try {
      ...
      // are best-effort (metrics are optional).
   ...
   */
  static async _finalize(candidate, result, { persist = true, onProgress, scholarCandidate = candidate, signal } = {}) {
