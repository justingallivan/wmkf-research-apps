...
class ContactEnrichmentService {
  ...
  /** True when a text window names the candidate: surname present AND forename (full or initial). */
  static _windowNamesCandidate(window, { forename, surname }) {
    const w = this._normForNameMatch(window);
    if (surname.length < 3 || !w.includes(surname)) return false;
    if (forename.length >= 2 && w.includes(forename)) return true;
    // First-initial form ("P. Bucksbaum" → "p bucksbaum"): a standalone initial token.
    return new RegExp(`(^| )${forename[0]}( |$)`).test(w);
  }
  ...
  static _selectGroundedEmail(name, text, emails, verifiedDomain, { identityText = '', pageUrl = '' } = {}) {
    ...
    for (const { email, index } of emails) {
      ...
      const ownerMatch = !!slug && localPart === slug;
