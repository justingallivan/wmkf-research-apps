...
class ContactEnrichmentService {
  ...
  // Validate the captured contact email against the Google Scholar VERIFIED
  // institutional domain ("Verified email at mbiberlin.de"). This is the precise,
  // signal-grounded replacement for the brittle lexical institution-NAME guard
  // (which wrongly rejected the REAL olga.smirnova@mbi-berlin.de because the
  // domain encodes the MBI acronym + city, not the verbose official name). Runs
  // in _finalize, AFTER Scholar metrics are fetched. Keep-biased: only acts when
  // a Scholar-verified domain is known — a clear MATCH confirms the contact for
  ...
  static _validateEmailAgainstVerifiedDomain(ce) {
    ...
    const verified = this._normalizeDomain(ce.scholarVerifiedEmail);
    if (!verified) return;
    const emailDom = this._emailDomain(ce.email);
    if (!emailDom) return;
    const related = emailDom === verified
      || emailDom.endsWith(`.${verified}`) || verified.endsWith(`.${emailDom}`)
      || emailDom.includes(verified) || verified.includes(emailDom);
    if (related) {
  ...
  static async _finalize(candidate, result, { useSerpSearch, serpApiKey, persist = true, onProgress, scholarCandidate = candidate }) {
    ...
    // Now that the Scholar-verified institutional domain is known, validate the
    // captured contact email against it: a domain MATCH confirms a real contact
    // for persistence (recovers olga.smirnova@mbi-berlin.de that the old lexical
    // guard wrongly rejected), a clear CONTRADICTION drops a likely namesake.
