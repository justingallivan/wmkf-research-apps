...
class ContactEnrichmentService {
  ...
  static _validateEmailAgainstVerifiedDomain(ce) {
    ...
    // Quarantine the wrong-domain contact as rejected leads BEFORE nulling — this
    // is the largest missing-email bucket (Slice 2a), so staff can still see what
    // the search found and judge it instead of it being silently dropped.
    const leadSourceUrl = ce.facultyPageUrl || ce.website || null;
    this._addContactLead(ce, { type: 'email', value: ce.email, source: ce.emailSource, sourceUrl: leadSourceUrl, confidence: 'rejected', rejectedReason: 'verified_domain_contradiction' });
    if (ce.facultyPageUrl) this._addContactLead(ce, { type: 'faculty_page', value: ce.facultyPageUrl, source: ce.websiteSource || ce.emailSource, sourceUrl: ce.facultyPageUrl, confidence: 'rejected', rejectedReason: 'verified_domain_contradiction' });
    if (ce.website) this._addContactLead(ce, { type: 'website', value: ce.website, source: ce.websiteSource || ce.emailSource, sourceUrl: ce.website, confidence: 'rejected', rejectedReason: 'verified_domain_contradiction' });
    ce.email = null;
    ce.emailSource = null;
  ...
  static async enrichCandidates(candidates, options = {}) {
    const { onProgress = () => {}, signal, useClaudeSearch = false, useSerpSearch = false } = options;
