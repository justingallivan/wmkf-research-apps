...
class ContactEnrichmentService {
  ...
  static _resultContradictsAnchor(result = {}, anchor = null) {
    ...
    return this._institutionsContradict(anchor.institution, resultInstitution)
      || this._emailDomainContradictsInstitution(result.email, anchor.institution);
  }

  // Conservative "different institution" check for a search-result email whose
  // result carries no structured institution field (Serp/Claude). A domain only
  // contradicts the anchor when NONE of its tokens relate to the anchor AND at
  // least one token is a substantial (>=4 char) academic-looking word — so a
  // wrong-namesake lab domain (e.g. metalab.ifmo.ru vs Max-Born-Institute) is
  // rejected, while legitimate abbreviation/portmanteau/hyphenated domains
  // (ethz.ch=ETH Zurich, caltech.edu=Caltech, heriot-watt.ac.uk) are NOT — those
  // were false-positives in the first cut and would have suppressed correct
  // reviewer emails (Keller@ethz.ch, Travers@heriot-watt). Generic mailbox
  // providers never contradict. Keep-biased: when in doubt, do NOT reject.
  static _emailDomainContradictsInstitution(email, anchorInstitution) {
    if (!email || !anchorInstitution || !String(email).includes('@')) return false;
    const domain = String(email).split('@').pop().toLowerCase();
    const genericDomains = new Set([
      'gmail.com', 'googlemail.com', 'yahoo.com', 'hotmail.com', 'outlook.com',
      'icloud.com', 'me.com', 'proton.me', 'protonmail.com',
    ]);
    if (genericDomains.has(domain)) return false;
    const STOP = new Set([
      'mail', 'email', 'www', 'edu', 'com', 'org', 'net', 'ac', 'gov',
      'faculty', 'staff', 'people', 'users', 'dept', 'department',
    ]);
    const domainTokens = domain.split(/[^a-z0-9]+/).filter((t) => t.length >= 3 && !STOP.has(t));
    if (!domainTokens.length) return false;
    const anchorWords = ContactParser.normalizeNameForMatch(anchorInstitution || '')
      .split(/\s+/)
      .filter((t) => t.length >= 3 && !['department', 'university', 'institute', 'school', 'college', 'for', 'and', 'the', 'of'].includes(t));
    if (!anchorWords.length) return false;
    const initialism = anchorWords.map((w) => w[0]).join('');
    // A domain token "relates" to the anchor if it matches a word exactly, is a
    // substring of (or contains) a word (catches "eth" in ethz, "heriotwatt" vs
    // heriot/watt), CONTAINS a >=4-char prefix of a word (portmanteaus like
    // caltech / gatech both contain "tech" from "technology"), shares a >=3-char
    // prefix with a word, or matches the institution initialism. Keep-biased.
    const relatesToAnchor = (dt) =>
      anchorWords.some((aw) =>
        dt === aw
        || dt.includes(aw)
        || aw.includes(dt)
        || (aw.length >= 4 && dt.includes(aw.slice(0, 4)))
        || aw.startsWith(dt.slice(0, 3)))
      || (initialism.length >= 2 && (dt.includes(initialism) || initialism.includes(dt)));
    if (domainTokens.some(relatesToAnchor)) return false;
    // Only assert a contradiction on a substantial token, never on short
    // abbreviations (mit, mpl, mpg) that the relation rules can't disambiguate.
    return domainTokens.some((t) => t.length >= 4);
  }
  ...
  static async enrichCandidate(candidate, options = {}) {
    ...
    const result = {
      ...
      contactEnrichment: {
        ...
        // scholar_current / pubmed_recency). The per-source candidates
        // (orcidAffiliation / scholarAffiliations) are COLLECTED during the
        ...
        scholarAffiliations: null,
        scholarVerifiedEmail: null,
        contactStatus: null,
    ...
    return this._finalize(candidate, result, { useSerpSearch, serpApiKey: credentials.serpApiKey, persist, onProgress, scholarCandidate: searchCandidate });
  }
  ...
  static async _attachScholarMetrics(candidate, result, useSerpSearch, serpApiKey, onProgress) {
    ...
    const effectiveInstitution = this._effectiveInstitution(candidate, ce);
    if (!effectiveInstitution && !this._hasOrcidAnchor(candidate, ce)) {
      ce.scholarPersistAllowed = false;
      ce.tierResults.scholar_profile = {
        ...(ce.tierResults.scholar_profile || {}),
        skipped: 'identity_anchor_required',
      };
      return;
    }
      ...
      if (scholarResult && (scholarResult.nameMismatch || scholarResult.institutionMismatch)) {
        ...
        if (scholarResult.scholarId) {
          ...
          if (metrics) {
            ...
            // Collect Scholar's current affiliation as an override CANDIDATE
            // (S224 #15), only on this identity-trusted (non-mismatch) profile
            // branch. Authority 2 (used when ORCID has none). The verified-email
            // domain is a corroborator/COI input, never a sendable address.
            if (typeof metrics.scholarAffiliations === 'string' && metrics.scholarAffiliations.trim()) {
              ce.scholarAffiliations = metrics.scholarAffiliations.trim();
            }
            if (typeof metrics.scholarEmail === 'string' && metrics.scholarEmail.trim()) {
              ce.scholarVerifiedEmail = metrics.scholarEmail.trim();
            }
          }
        }
        onProgress({
   ...
   */
  static async _finalize(candidate, result, { useSerpSearch, serpApiKey, persist = true, onProgress, scholarCandidate = candidate }) {
    await this._attachScholarMetrics(scholarCandidate, result, useSerpSearch, serpApiKey, onProgress);
   ...
   * identity-trusted source collected during the tiers (ORCID > Scholar >
   ...
   * `scholarAffiliations` — they never touch candidate.affiliation — so the
  ...
  static _applyAffiliationOverride(result) {
    ...
    // Authority order: ORCID current > Scholar current. (PubMed-recency is the
    ...
    const pinned =
      (typeof ce.orcidAffiliation === 'string' && ce.orcidAffiliation.trim())
        ...
        : (typeof ce.scholarAffiliations === 'string' && ce.scholarAffiliations.trim())
          ? { value: ce.scholarAffiliations.trim(), source: 'scholar_current' }
          : null;
    if (!pinned) return;
