...
class ContactEnrichmentService {
  ...
  static _domainRelated(domain, verifiedDomain) {
    const a = this._normalizeDomain(domain);
    const b = this._normalizeDomain(verifiedDomain);
    if (!a || !b) return false;
    return a === b || a.endsWith(`.${b}`) || b.endsWith(`.${a}`);
  }

  static _emailDomainRelatedToAny(email, domains = []) {
    const emailDom = this._emailDomain(email);
    if (!emailDom) return false;
    return domains.some((domain) => this._domainRelated(emailDom, domain));
  }

  static _addInstitutionDomain(set, domain) {
    if (!set || !domain) return;
    const value = String(domain).trim().toLowerCase();
    if (value && value.includes('.')) set.add(value);
  }

  static _currentOrcidInstitutionRefs(ce = {}) {
  ...
  static _strongInstitutionDisplayMatch(query, displayName) {
  ...
  static async _buildInstitutionDomainEvidence(candidate, result, { signal } = {}) {
    ...
    if (identityTrusted) {
      this._addInstitutionDomain(anchored, ce.verifiedInstitutionDomain);
      for (const ref of this._currentOrcidInstitutionRefs(ce)) {
        try {
          const inst = await OpenAlexService.getInstitution(ref.id, { signal });
          this._addInstitutionDomain(anchored, inst?.domain);
        } catch (err) {
    ...
    for (const domain of anchored) this._addInstitutionDomain(plausible, domain);
    ...
    for (const name of institutionNames) {
      try {
        ...
        if (top?.domain && this._strongInstitutionDisplayMatch(name, top.displayName)) {
          this._addInstitutionDomain(plausible, top.domain);
        }
      } catch (err) {
        if (signal?.aborted) throw err;
      }
