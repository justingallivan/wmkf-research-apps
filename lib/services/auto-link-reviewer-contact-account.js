 ...
 * leave the existing staff mismatch alert to handle the case.
 */
...
export async function autoLinkReviewerContactAccount(
  ...
  return runWithDalContext('accepted-reviewer-contact-account-auto-link', async () => {
    ...
    if (accountResult?.capped) {
      const error = new Error('active Account scan was capped; refusing a partial-cardinality match');
      error.code = 'reviewer_contact_account_scan_capped';
