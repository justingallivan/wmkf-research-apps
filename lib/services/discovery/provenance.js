...
function orcidAffiliationAssertions(values) {
  return (Array.isArray(values) ? values : []).flatMap((affiliation) => {
    const rawText = typeof affiliation?.organization === 'string'
      ? affiliation.organization.trim()
      : '';
    if (!rawText) return [];
    const startYear = Number(affiliation.startYear) || null;
    const endYear = Number(affiliation.endYear) || null;
    const disambiguationSource = String(affiliation.disambiguationSource || '').toUpperCase();
    const disambiguatedId = typeof affiliation.disambiguatedOrganizationId === 'string'
      ? affiliation.disambiguatedOrganizationId.trim()
      : '';
    return [{
      rawText,
      sourceType: 'orcid_employment',
      sourceReference: affiliation.sourceReference || null,
      observedAt: null,
      // No end date is an unbounded interval, not proof observed as current.
      currentness: endYear ? 'historical' : 'unknown',
      authorSpecific: true,
      publicationYear: null,
      startYear,
      endYear,
      ror: disambiguationSource === 'ROR' && disambiguatedId ? disambiguatedId : null,
      openAlexId: null,
    }];
  });
}
...
function mapSpineVerificationResult(suggestion, spineResult = {}) {
  ...
  if (status === 'confirmed' || status === 'probable') {
    ...
    const candidate = withReviewerProvenance({
      ...
      // Carry legacy names for presentation and source/date-bound records for
      // the dormant Phase 2 additional-affiliation COI screen.
      ...
      affiliationAssertions: orcidAffiliationAssertions(spineResult.orcidAffiliationEvidence),
