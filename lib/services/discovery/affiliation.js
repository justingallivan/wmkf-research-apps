 ...
 * bylines, plus the full affiliation history and dated assertion projection used by
 * the dormant Phase 2 COI contract, and
 ...
 * most-recent-first. This legacy presentation field is COI-inert. The typed
 * assertion projection below carries source/date/currentness into the dormant
 * additional-affiliation screen instead.
 */
...
/**
 * Preserve the source/date binding that the legacy string-only history loses.
 * Publication evidence remains `unknown` currentness until a separately
 * reviewed date policy defines current versus historical.
 */
function collectAffiliationAssertions(articles, nameVariants) {
  const normalizedVariants = (nameVariants || [])
  ...
  for (const article of Array.isArray(articles) ? articles : []) {
    const publicationYear = Number(article?.year) || null;
    for (const author of Array.isArray(article?.authors) ? article.authors : []) {
      const normalizedAuthorName = normalizeNameForMatch(author?.name);
      if (!normalizedVariants.some((variant) => namesMatch(variant, normalizedAuthorName))) continue;
      const rawText = typeof author?.affiliation === 'string' ? author.affiliation.trim() : '';
      if (rawText.length <= 10) continue;
      const key = normalizeAffiliationForComparison(rawText);
      const sourceReference = article?.pmid
        ? `pmid:${String(article.pmid).trim()}`
        : (article?.doi ? `doi:${String(article.doi).trim()}` : null);
      const previous = byInstitution.get(key);
      if (!previous || (publicationYear || 0) >= (previous.publicationYear || 0)) {
  ...
  return [...byInstitution.values()]
    .sort((left, right) => (right.publicationYear || 0) - (left.publicationYear || 0));
}
...
/**
 * Split a raw affiliation/byline string into candidate institution segments,
 * for segment-wise pair comparison (Stage 1 of
 * docs/INSTITUTION_PAIR_CONSISTENCY_RESOLUTION_PLAN.md). Deterministic, pure,
 * no network. This is a SEGMENTER, not a ranker: callers must test each
 * returned candidate against the other comparison operand — a `true` verdict
 * may never be derived from picking a "best" segment out of this list alone.
 *
 * Splits on comma/semicolon boundaries, trims, drops empties, and — for each
 * comma-separated run between semicolons — also includes progressive suffix
 * joins so a segment like "Vanderbilt University Medical Center, Nashville"
 * is available alongside the bare institution name. The whole trimmed input
 * string is always included as the last candidate.
 */
function institutionSegments(affiliationString) {
  if (!affiliationString || typeof affiliationString !== 'string') return [];
  const whole = affiliationString.trim();
  if (!whole) return [];
  ...
  // Semicolons separate independent runs (e.g. two department bylines at the
  // same or different institutions); commas separate parts within a run.
  const semicolonRuns = whole.split(';').map((run) => run.trim()).filter(Boolean);

  for (const run of semicolonRuns) {
    const parts = run.split(',').map((part) => part.trim()).filter(Boolean);
    for (const part of parts) {
      add(part);
    }
    // Progressive suffix joins: "Vanderbilt University Medical Center" and
    // "Vanderbilt University Medical Center, Nashville" from
    // "Dept of X, Vanderbilt University Medical Center, Nashville, USA".
