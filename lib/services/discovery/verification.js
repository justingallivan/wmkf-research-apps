 ...
 * ./publications, ./research-area, ./provenance, ../reviewer-identity-evidence, ../pubmed-service,
...
const { MIN_PUBLICATIONS, VERIFICATION_STATUSES, VERIFICATION_SKIPPED_REASON, DEBUG, PUBMED_DELAY } = require('./constants');
 ...
 * override applies (C1); defaults to the shared constant for any direct import.
 */
async function verifyClaudeSuggestions(suggestions, onProgress = () => {}, options = {}, minPublications = MIN_PUBLICATIONS) {
  const verified = [];
  const unverified = [];
