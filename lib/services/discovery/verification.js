...
async function verifyClaudeSuggestions(suggestions, onProgress = () => {}, options = {}, minPublications) {
  ...
  if (verifierRouting.verifier === 'spine') {
    ...
    const spineResults = await ReviewerIdentityRuntime.evaluateSuggestions(
      ...
      },
      {
        ...
        onComparisonObserved: typeof options.onIdentityComparison === 'function'
          ? options.onIdentityComparison
          : null,
      },
    );
  ...
  for (let i = 0; i < normalizedSuggestions.length; i++) {
    ...
    if (finalArticles.length >= minPublications) {
      ...
      const candidate = withReviewerProvenance({
        ...
        // Legacy string history plus the Phase 2 source/date-bound assertion
        // projection. The latter is what the expanded COI screen consumes when
        // its server-only rollout flag is enabled.
        affiliationHistory: collectAffiliationHistory(finalArticles, nameVariants),
        affiliationAssertions: collectAffiliationAssertions(finalArticles, nameVariants),
      ...
      if (verificationStatus === VERIFICATION_STATUSES.VERIFIED) {
        ...
        unverified.push(unresolvedCandidate);
        pubMedResults[i] = unresolvedCandidate;
      }
    } else {
      ...
      const unresolvedCandidate = withReviewerProvenance({
        ...suggestion,
        verified: false,
  ...
  if (typeof options.onIdentityComparison === 'function') {
    try {
      await ReviewerIdentityRuntime.diagnosePubMedResults(
        normalizedSuggestions,
        pubMedResults,
        {
          proposalInfo: options.proposalInfo,
        ...
        { onComparisonObserved: options.onIdentityComparison },
      );
    } catch (error) {
      console.warn('[Verification] PubMed admin diagnostic failed; PubMed results retained', {
