...
async function checkCoauthorHistory(candidateName, proposalAuthors, { signal } = {}) {
  if (!proposalAuthors || proposalAuthors.length === 0) {
    return {
      ...
      // The shared discovery service, not the browser, establishes this
      // narrow exception. Promotion still requires the matching server-stage
      // receipt and never treats a client-provided N/A as a clean result.
