 ...
 */
async function retractNeedsMerge(suggestionId, reason, result, dryRun) {
  if (dryRun) {
...
export async function reconcileReviewerEmails({ maxBatch = 200, dryRun = false, actingUserSystemId = null } = {}) {
  ...
  for (const { requestId, candidate } of rows) {
    ...
    try {
      ...
      if (!sug) {
        await retractNeedsMerge(suggestionId, 'suggestion_gone', result, dryRun);
        result.skipped++; continue;
      }
      if (!eq(sug._wmkf_request_value, requestId)) { result.skipped++; continue; }
      if (suggestionAdapter.isExcluded(sug)) {
        await retractNeedsMerge(suggestionId, 'applicant_excluded', result, dryRun);
        result.skipped++; continue;
      }
      if (!sug.wmkf_selected) {
        await retractNeedsMerge(suggestionId, 'deselected', result, dryRun);
        result.skipped++; continue;
      }
      ...
      if (person && person.wmkf_emailaddress) {
        await retractNeedsMerge(suggestionId, 'email_present', result, dryRun);
        result.skipped++; continue;
      }
      ...
      if (owner.none) {
        ...
        await retractNeedsMerge(suggestionId, 'written', result, dryRun);
      ...
      if (owner.one && typeof owner.row?.statecode === 'number' && owner.row.statecode === 0) {
        ...
        await retractNeedsMerge(suggestionId, 'repointed', result, dryRun);
