...
export async function reconcileReviewerEmails({ maxBatch = 200, dryRun = false, actingUserSystemId = null } = {}) {
  ...
  for (const { requestId, candidate } of rows) {
    ...
    try {
      ...
      if (owner.none) {
        if (!dryRun) {
          await potentialReviewerAdapter.update(personId, { email: vetted.email }, { actingUserSystemId });
          if (vetted.source) await researcherAdapter.updateById(personId, { emailSource: vetted.source }, { actingUserSystemId });
        }
