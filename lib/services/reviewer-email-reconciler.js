 ...
 * deselected, write, repoint. It deliberately does NOT fire on the ambiguous
 * skips (no vetted email, missing personId, contradictory same-person owner) or
 * on a stale-roster request mismatch — those leave the duplicate intact, and
 * retracting there would destroy the only standing signal that it exists.
 *
 ...
 * the email landed, the suggestion is gone or deselected, or we just
 * wrote/repointed. NEVER from the ambiguous skips (no vetted email, missing
 * personId, contradictory same-person owner) — those leave the duplicate
 * intact, and retracting there would destroy the only standing signal.
 * Best-effort: a retraction failure must not fail the run.
 */
async function retractNeedsMerge(suggestionId, reason, result) {
  try {
...
export async function reconcileReviewerEmails({ maxBatch = 200, dryRun = false, actingUserSystemId = null } = {}) {
  ...
  const result = { scanned: rows.length, written: [], repointed: [], alerted: [], retracted: [], skipped: 0, errors: [] };
  ...
  for (const { requestId, candidate } of rows) {
    ...
    try {
      ...
      if (!sug) {
        if (!dryRun) await retractNeedsMerge(suggestionId, 'suggestion_gone', result);
        result.skipped++; continue;
      }
      if (!eq(sug._wmkf_request_value, requestId)) { result.skipped++; continue; }
      if (!sug.wmkf_selected) {
        if (!dryRun) await retractNeedsMerge(suggestionId, 'deselected', result);
        result.skipped++; continue;
      }
      ...
      if (person && person.wmkf_emailaddress) {
        if (!dryRun) await retractNeedsMerge(suggestionId, 'email_present', result);
        result.skipped++; continue;
      }
      ...
      if (owner.none) {
        ...
        if (!dryRun) await retractNeedsMerge(suggestionId, 'written', result);
      ...
      if (owner.one && typeof owner.row?.statecode === 'number' && owner.row.statecode === 0) {
        ...
        if (!dryRun) await retractNeedsMerge(suggestionId, 'repointed', result);
