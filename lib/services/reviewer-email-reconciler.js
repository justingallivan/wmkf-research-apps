 ...
 * Candidates are selected newest first under a cap, so a request that is not
 * verified ordinary is excluded and the page re-read until it holds only
 * ordinary requests; test rows can therefore never occupy the batch. Each
 * pass rules out at least one more request, so the loop ends.
 */
async function selectOrdinaryCandidates(maxBatch) {
  const requestTestState = createRequestTestStateLookup();
  const excluded = new Set();
  const skips = {};
  for (;;) {
    const rows = await findReconcilableCandidates(maxBatch, { excludeRequestIds: [...excluded] });
    let ruledOut = false;
    for (const requestId of new Set(rows.map((row) => row.requestId))) {
      if (excluded.has(requestId)) continue;
      const testState = await requestTestState(requestId);
      if (testState.kind !== 'ordinary') {
        excluded.add(requestId);
        recordTestRequestSkip(skips, testState);
        ruledOut = true;
      }
    }
    if (!ruledOut) return { rows: rows.filter((row) => !excluded.has(row.requestId)), skips };
  }
}

export async function reconcileReviewerEmails({ maxBatch = 200, dryRun = false, actingUserSystemId = null } = {}) {
  const { rows, skips } = await selectOrdinaryCandidates(maxBatch);
  const result = {
    scanned: rows.length,
    written: [],
    repointed: [],
