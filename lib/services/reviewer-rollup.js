...
// withdrew / released — the post-acceptance terminal review statuses written by
// `terminal-transition`. Browser-safe config import (no Dataverse service graph).
 ...
 * @typedef {{total:number, accepted:number, pending:number, released:number, declined:number, uninvited:number}} ReviewerProgress
...
export async function fetchReviewerRollup(requestIds) {
  ...
  for (const chunk of chunked(requestIds, CHUNK)) {
    ...
    for (const s of records) {
      ...
      // A row whose engagement ENDED without a decline: the PD released it as no
      // longer needed / closed it out as no-response (`wmkf_responsetype`), or it
      // reached a terminal review status post-acceptance (`terminal-transition`).
      // None of those writes archive the row (`wmkf_selected` stays true) and none
      // set `wmkf_declined`, so without this the released row keeps rendering as
      // `pending` — and a post-acceptance withdrawal keeps rendering as `accepted`
      // (S406, owner report on request 1002959). `held` is deliberately NOT here:
      // it is retired and still routes the reviewer to the accept form, so it
      // remains 'awaiting' (see the `held` note below).
      const released = s.wmkf_responsetype === RESPONSE_TYPE_MAP.withdrawn_sufficient
        || s.wmkf_responsetype === RESPONSE_TYPE_MAP.no_response
        || TERMINAL_REVIEW_STATUS_VALUE_SET.has(s.wmkf_reviewstatus);
      ...
      else if (released) o.progress.released += 1;
