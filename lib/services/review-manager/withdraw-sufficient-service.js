 ...
 * @param {Object<string, {subject?: string, bodyText?: string, to?: string}>|null} [args.overrides]
 *   Complete per-suggestion copy and the recipient address staff previewed.
 *   Recipients are always re-derived server-side; `to` is only an expected-value
 *   guard. A complete reviewed entry bypasses live template/signature rendering,
 *   so configuration changes between preview and send cannot alter reviewed copy.
 *   A blank, partial, or absent entry preserves the legacy template-render path.
