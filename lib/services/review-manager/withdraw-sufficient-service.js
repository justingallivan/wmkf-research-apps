 ...
 * reviewer lifecycle Stage 3I the authoritative pending→withdrawn_sufficient
...
export async function withdrawSufficient({ requestId, suggestionIds, actingUserSystemId, overrides = null }) {
  ...
  for (const id of suggestionIds) {
    ...
    try {
      await withdrawPendingInvitation({ id, nowIso, ifMatch: s._etag, actingUserSystemId });
    } catch (e) {
