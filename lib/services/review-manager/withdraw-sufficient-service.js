...
export async function withdrawSufficient({
  ...
  for (const id of suggestionIds) {
    ...
    const shouldSendEmail = reason === RELEASE_REASONS.no_longer_needed
      || (reason === RELEASE_REASONS.no_response && reviewedMode);
    let reviewer = null;
    if (reviewedOverride) {
      if (
        !canEmail
    ...
    } else if (!shouldSendEmail && reason === RELEASE_REASONS.no_response) {
