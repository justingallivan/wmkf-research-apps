/**
 * Review Manager — PD selective release service
 * (Route→Service Consolidation Plan, Stage 1 pilot).
 *
 * Owns the orchestration for POST /api/review-manager/withdraw-sufficient
 * (eligibility, override checks, per-id results, courtesy email); since
 * reviewer lifecycle Stage 3I the authoritative pending→release
 * write lives in reviewer-engagement/withdraw-pending-invitation.js. The
 * route is a thin shell (auth, validation, DAL context, HTTP mapping).
 *
 * Contract (plan Decision 3):
 *   - takes a plain argument object, never req/res;
 *   - returns a plain value: { ok: true, withdrawn: N, results: [{ suggestionId, status, reason }] };
 ...
 *     withdrawn_no_email, withdrawn_no_email_by_reason, withdrawn_no_pd.
 *     write_failed additionally carries a `failure` code (write_interlocked,
 *     dataverse_forbidden, not_found, dataverse_unavailable, unknown)
 *     classified from the write error's shape (status/message pattern) —
 *     the raw upstream message is never forwarded to the client.
 */
...
import { withdrawPendingInvitation } from '../reviewer-engagement/withdraw-pending-invitation';
 ...
 * @returns {Promise<{ ok: true, withdrawn: number, results: Array<{ suggestionId: string, status: string, reason: string }> }>}
...
// Classifies a discarded write error into a fixed, client-safe code — mirrors
// lib/services/reviewer-engagement/close-review.js mapWriteError. The raw
// error message (which can contain interlock hostnames or other upstream
// detail) is never forwarded to the client; only this code is.
function classifyWriteFailure(error) {
  if (/^\[dataverse-interlock\] denied/.test(error?.message || '')) {
    return 'write_interlocked';
  }
  const upstreamStatus = Number(error?.status ?? error?.response?.status);
  if (upstreamStatus === 401 || upstreamStatus === 403) return 'dataverse_forbidden';
  if (upstreamStatus === 404) return 'not_found';
  if (upstreamStatus >= 500) return 'dataverse_unavailable';
  if (!Number.isFinite(upstreamStatus)
    && /network|timeout|ECONNRESET|ECONNREFUSED|ENOTFOUND|ETIMEDOUT/i.test(`${error?.message || ''} ${error?.code || ''}`)) {
    return 'dataverse_unavailable';
  }
  return 'unknown';
}

export async function withdrawSufficient({
  requestId,
  suggestionIds,
  actingUserSystemId,
  overrides = null,
  reason = RELEASE_REASONS.no_longer_needed,
  // Explicit courtesy-email choice. Absent → today's defaults (no_longer_needed
  // emails; no_response emails only with reviewed copy). `false` skips the
  // email for either reason; the state write is unaffected.
  sendEmail = undefined,
}) {
  if (!RELEASE_REASON_VALUES.includes(reason)) {
    throw new WithdrawSufficientError('reason must be no_longer_needed or no_response', 400);
  }
  ...
  for (const id of suggestionIds) {
    ...
    const emailRequested = sendEmail === undefined
      ? (reason === RELEASE_REASONS.no_longer_needed || reviewedMode)
      : sendEmail === true;
    // A no_response courtesy note is only ever sent from reviewed copy.
    const shouldSendEmail = emailRequested
      && (reason === RELEASE_REASONS.no_longer_needed || reviewedMode);
    let reviewer = null;
    if (reviewedOverride) {
      if (
    ...
    try {
      const writeArgs = { id, nowIso, ifMatch: s._etag, actingUserSystemId };
      if (reason !== RELEASE_REASONS.no_longer_needed) writeArgs.reason = reason;
      await withdrawPendingInvitation(writeArgs);
    } catch (e) {
      const is412 = e.status === 412 || /\b412\b/.test(e.message || '');
      results.push(is412
        ? { suggestionId: id, status: 'changed_skipped' }
        : { suggestionId: id, status: 'write_failed', failure: classifyWriteFailure(e) });
      continue;
    }
    withdrawn++;

    if (shouldSendEmail && canEmail) {
      reviewer ||= await getReviewerByIdWithSelect(s._wmkf_potentialreviewer_value, {
        select: 'wmkf_name,wmkf_emailaddress',
      }).catch(() => null);
      if (reviewer?.wmkf_emailaddress) {
        try {
          ...
          const classification = classifyEmailDispatchError(e);
          results.push({
            suggestionId: id,
            status: classification.outcome === 'uncertain' ? 'withdrawn_email_unconfirmed' : 'withdrawn_email_failed',
          });
        }
      } else {
      ...
      results.push({ suggestionId: id, status: 'withdrawn_no_email_by_reason' });
    } else {
  ...
  return {
    ok: true,
    withdrawn,
    // Keep the client contract stable for every branch and strip any internal
    // diagnostics before returning results. The explicit reason lets consumers
    // explain deliberate no-email outcomes without exposing reviewer PII.
    results: results.map(({ suggestionId, status, failure }) => (
      failure === undefined
        ? { suggestionId, status, reason }
        : { suggestionId, status, failure, reason }
    )),
  };
}
