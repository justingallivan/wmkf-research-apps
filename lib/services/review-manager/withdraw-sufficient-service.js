...
export async function withdrawSufficient({
  ...
  return {
    ok: true,
    withdrawn,
    // Preserve the legacy response shape for the default reason. New releases
    // carry their explicit reason alongside the status so consumers can explain
    // deliberate no-email outcomes without exposing reviewer PII.
    results: reason === RELEASE_REASONS.no_longer_needed
      ? results
