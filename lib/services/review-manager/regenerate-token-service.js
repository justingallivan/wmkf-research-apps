 ...
 *       409 excluded (applicant-disposition fail-closed chokepoint) or
 *       not_eligible (terminal/unknown lifecycle state);
 ...
 *   or 409 (body { ok:false, reason:'excluded'|'not_eligible' })
 */
export async function regenerateToken({ suggestionId, actingUserSystemId }) {
  ...
  // Re-read lifecycle state at the server chokepoint. The menu is only a
  // convenience gate: a revoked token does not make a terminal engagement
  // eligible for a replacement link, and a client can always bypass that UI.
  // Declined, no-response, withdrawn-sufficient, and held are terminal
  // response outcomes; accepted is the only response outcome eligible here.
  const responseType = suggestion?.wmkf_responsetype;
  const knownResponseType = responseType == null
    ? null
    : RESPONSE_TYPE_BY_VALUE[responseType];
  const terminalResponseTypes = new Set([
    ...
    RESPONSE_TYPE_MAP.held,
  ]);
  const knownReviewStatuses = new Set(Object.values(REVIEW_STATUS_MAP));
  const terminalReviewStatuses = new Set(Object.values(TERMINAL_REVIEW_STATUS_VALUES));
  const unknownResponseType = responseType != null && !knownResponseType;
  const unknownReviewStatus = suggestion?.wmkf_reviewstatus != null
    && !knownReviewStatuses.has(suggestion.wmkf_reviewstatus);
  if (unknownResponseType
      || unknownReviewStatus
      || terminalResponseTypes.has(responseType)
      || terminalReviewStatuses.has(suggestion?.wmkf_reviewstatus)) {
    throw new RegenerateTokenError('engagement is not eligible for token regeneration', 409, {
      ok: false,
      reason: 'not_eligible',
    });
  }
  ...
  // The lifecycle read and token write must describe the same engagement
  // version. Without a concrete ETag, a concurrent release/revoke could be
  // silently cleared by setExternalToken; fail closed before request lookup
  // or minting instead of issuing an unbound replacement.
  if (!isConcreteEtag(suggestion?._etag)) {
    throw new RegenerateTokenError('engagement version is unavailable', 409, {
      ok: false,
      reason: 'not_eligible',
    });
  }
