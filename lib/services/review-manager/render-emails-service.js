 ...
 *     skipped:'no_email'; every row carries emailConfidence);
...
import { findByShortCode as findCycleByShortCode } from '../grant-cycles-dataverse';
import {
  replacePlaceholders,
  buildTemplateData,
 ...
 *
 * templateType is validated (and defaulted) in the shell; rendering itself is
 * template-type-agnostic — the caller supplies subject/body.
 */
export async function renderEmails({ suggestionIds, template, settings = {}, actingUserSystemId }) {
...
// renderer above consumes (short_code, name, program_name, review_deadline,
// custom_fields) so downstream code is unchanged. Transport errors
// propagate to the shell's catch (matches pre-cutover behavior where a
