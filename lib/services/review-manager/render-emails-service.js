...
export async function renderEmails({
  ...
  const drafts = rows.map(({ suggestionId, sug, person, request }) => {
    ...
    const effectiveDueDate = resolveEffectiveReviewDueDate({
      requestedDueDate: settings.reviewDueDate,
      requestDueDate: request?.wmkf_reviewduedate,
      cycleDueDate: cycle.review_deadline,
    });
    if (effectiveDueDate.error) {
      throw new RenderEmailsError(effectiveDueDate.error, 409);
    }
    ...
    if (!candidateEmail) {
      return {
        ...
        effectiveReviewDueDate: effectiveDueDate.value,
        effectiveReviewDueDateSource: effectiveDueDate.source,
        ...abstractFields,
      };
    }
    ...
    const templateSettings = {
      ...
      reviewDueDate: effectiveDueDate.value,
