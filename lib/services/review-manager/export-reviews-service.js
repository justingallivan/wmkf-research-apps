...
export async function exportCombinedReviews({
  ...
  if (testRequestIsolationEnabled()) {
    const ordinary = await grantRequestAdapter.findByIds([proposalId], {
      select: 'akoya_requestid',
      filter: TEST_REQUEST_ORDINARY_OData_FILTER,
      top: 1,
    });
    if (!ordinary.records?.length) {
  ...
  // Sorted exactly as the Reviews tab sorts `submitted` (compareReviewersByName)
  // so a rating tie among >3 verified quotations (W8) selects the identical
  // three reviewers here as on the tab, rather than depending on Dataverse
  // fetch order (Opus Slice 2 review follow-up 1).
  const submitted = (proposal.reviewers || [])
