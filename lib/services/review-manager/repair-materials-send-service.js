...
export async function repairMaterialsSendStamp({
  ...
  // The supplied date is CLIENT data (req.body). ETag locking stops concurrent
  // writes but not sequential replay, so without a server-side check any
  // authorized caller could walk `lastSent` to an arbitrary date with no email
  // ever sent — and, when `atSend` was still null, permanently fix that
  // supposedly immutable value on the first request (Codex adversarial finding,
  // confirmed). Bind the repair to what the SERVER would render right now: the
  // request's own review due date. A caller cannot invent a deadline the
  // campaign does not hold.
  //
  // Deliberate limitation: a render-time staff override that differs from the
  // request column cannot be repaired through this route and must be re-sent.
  // Failing closed on an unverifiable date is the right trade for a field that
  // feeds reliability scoring — flagged for owner review.
  const request = await getRequestById(requestId, { select: 'akoya_requestid,wmkf_reviewduedate' }).catch(() => null);
  const serverDueDate = request?.wmkf_reviewduedate
    ? String(request.wmkf_reviewduedate).slice(0, 10)
    : null;
  if (!serverDueDate || serverDueDate !== effectiveReviewDueDate) {
    throw new ServiceHttpError(
      'Review due date could not be confirmed against the request; re-send materials instead of repairing',
      { httpStatus: 409 },
    );
  }
