...
function formatAnswerForDigest(a) {
  ...
  if (a.questionType === 'multiselect' && Array.isArray(a.answerValues)) {
    lines.push(`Selected categories: ${a.answerValues.map((pair) => pair.label).join('; ')}`);
  }
  ...
  return reviewers
    .map((r) => {
      ...
      const body = (r.answers || [])
        .filter((a) => !a.answerValuesUnreadable
