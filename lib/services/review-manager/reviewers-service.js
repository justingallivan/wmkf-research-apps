 ...
 *   - patchReviewers returns { success: true, message, savedIds, failedIds: [],
 *     notAttemptedIds: [] }; an attempted adapter failure throws
 *     ReviewerStatusMutationError with the original cause and outcome arrays;
 ...
 *
 * PATCH batches trim/lowercase and deduplicate validated GUIDs in first-occurrence
 * order; single calls preserve their submitted ID and lifecycle object. Sequential
 * awaited adapter operations stop at the first failure, reporting the confirmed
 * prefix, unconfirmed operation and untouched suffix. An adapter failure may
 * precede any write or follow a commit whose response was lost; no rollback or
 * retry occurs. Route/auth checks and service dedicated-target prechecks remain
 * outside this outcome envelope; adapter validation failures are included.
 *
