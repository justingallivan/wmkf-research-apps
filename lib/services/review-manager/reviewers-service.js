/**
 * Review Manager — reviewers listing service
 * (Route→Service Consolidation Plan, Stage 2 wave; Reviewer Lifecycle
 * Stage 3C partial-file extraction).
 *
 * Owns `getReviewers`: the staff-shared reviewer GET DTO ({ success,
 * proposals, totalReviewers, liveQuestions } or the empty/PD-null early
 * shapes) and its projections. The route is a thin shell (auth BEFORE
 * method dispatch — preserved from the original route — input validation,
 * DAL context, HTTP mapping).
 *
 * `patchReviewers` / `ReviewerStatusMutationError` are compatibility
 * re-exports of `lib/services/reviewer-engagement/correct-status.js`
 * (Stage 3C) — the route's import path is unchanged, but the sequential
 * stop-on-first-failure correction contract and its outcome arrays
 * (savedIds/failedIds/notAttemptedIds) are documented there, not here.
 *
 * Contract (plan Decision 3):
 *   - getReviewers takes a plain argument object, never req/res;
 ...
 * before invoking either single or batch PATCH behavior (delegated to
 * `reviewer-engagement/correct-status.js`).
 */
...
import { withSyntheticReviewerMarkerSelect, syntheticReviewerVisibilityDto } from '../test-requests/isolation.js';
...
function isNoResponseHistoryEligible(row) {
  return RESPONSE_TYPE_BY_VALUE[row?.wmkf_responsetype] === 'no_response'
    && row?.wmkf_accepted !== true
    && !row?.wmkf_reviewreceivedat;
}

function hasMappedValue(map, value) {
...
function projectRequest(r) {
  ...
  return {
    ...
    grantProgramId: r._wmkf_grantprogram_value || null,
...
function parseReviewSynthesis(raw) {
  ...
  return {
    ...
    // §4.3): read-boundary pass-through only. Provenance (is this quote
    // actually in a submitted review's answer text?) is verified downstream
    // by `composeWriteupParagraphs`, not here — this function only guards
    // against a non-object/non-string shape reaching the client.
    writeupThemes: typeof s.writeupThemes === 'string' ? s.writeupThemes : '',
    writeupQuotations: (Array.isArray(s.writeupQuotations) ? s.writeupQuotations : [])
      .filter((q) => q && typeof q === 'object' && !Array.isArray(q) && typeof q.quote === 'string')
      .map((q) => ({
        questionKey: typeof q.questionKey === 'string' ? q.questionKey : '',
        quote: q.quote,
      })),
  };
}
 ...
 */
export async function getReviewers({ proposalId, requestNumber, cycleCode, status, scope = 'my', azureEmail, programId, callerSystemId }) {
  let suggestions = [];
  let requestById = {};
  ...
  if (isSingleProposal) {
    ...
    if (programId) {
      ...
      if (!request.grantProgramId
        || String(request.grantProgramId).trim().toLowerCase() !== programScope.programId) {
        return { success: true, proposals: [], totalReviewers: 0 };
      }
    }
    ...
    // The ordinary reviewer roster remains accepted/review-received only.
    // no_response rows are projected separately below for history, so they do
    // not affect status buckets, counts, selection, or synthesis DTOs.
    ...
    if (scope === 'all') {
      ...
      const programScope = await resolveWorkbenchProgramScope({ programId });
      result = await suggestionAdapter.findAcceptedByCycle(cycleCode, { programId: programScope.programId });
    } else {
      ...
      const programScope = await resolveWorkbenchProgramScope({
        callerSystemId: pd.systemuserid,
        programId,
      });
      result = await suggestionAdapter.findAcceptedByPD(pd.systemuserid, {
        cycleCode,
  ...
  // Lifecycle rows include invited-but-unaccepted participants that the
  // reviewer list excludes; their names are needed to say WHO blocks synthesis.
  const historySuggestions = isSingleProposal
    ? (lifecycleByRequest[Object.keys(requestById)[0]] || []).filter(isNoResponseHistoryEligible)
    : [];
  const personIds = [...new Set([
    ...suggestions.map((s) => s._wmkf_potentialreviewer_value),
    ...historySuggestions.map((s) => s._wmkf_potentialreviewer_value),
    ...Object.values(lifecycleByRequest).flat().map((s) => s._wmkf_potentialreviewer_value),
  ].filter(Boolean))];
  ...
  if (isSingleProposal) {
    ...
    byRequest[request.requestId] = {
      ...
      proposalInstitution: request.applicant || null,
  ...
  for (const s of [...suggestions, ...historySuggestions]) {
    const reqId = s._wmkf_request_value;
    const request = requestById[reqId];
    if (!request) continue;

    const responseType = RESPONSE_TYPE_BY_VALUE[s.wmkf_responsetype] || null;
    const mappedReviewStatus = REVIEW_STATUS_BY_VALUE[s.wmkf_reviewstatus] || null;
    // Preserve an explicit, non-PII validity signal instead of allowing an
    // unknown Dataverse option value to look like a pending/accepted row.
    // The UI can then fail closed without exposing the raw value.
    const lifecycleValid = hasMappedValue(RESPONSE_TYPE_BY_VALUE, s.wmkf_responsetype)
      && hasMappedValue(REVIEW_STATUS_BY_VALUE, s.wmkf_reviewstatus);
    const historyOnly = isNoResponseHistoryEligible(s);
    const reviewStatus = historyOnly ? null : (mappedReviewStatus || 'accepted');

    // Optional post-filter by status (single value, not 'all')
    if (!historyOnly && status && status !== 'all' && reviewStatus !== status) continue;
    ...
    const projectedReviewer = {
      ...
      // 6c-ii Stage A, plan item (a) read-side display: derived boolean only,
      // never the raw field; gated exactly like testRequestVisibilityDto, so
      // this key is absent entirely while the isolation switch is off.
      ...syntheticReviewerVisibilityDto(person),
      // Writeup paragraphs (Reviews Tab Phase II Slice 1): board-writeup
      // identity + expertise fields, read-time only — not added to
      // digestReviewers below, so stored syntheses stay current.
      ...
      responseType,
      lifecycleValid,
      // The proposal meeting date is already a trusted request projection.
      // Carry it onto each row so no_response attribution can distinguish a
      // pre-meeting staff release from a post-meeting automated sweep.
      meetingDate: request.meetingDate,
      ...
      reviewDueDateExtensionGrantedAt: s.wmkf_reviewduedateextensiongrantedat || null,
      ...
      // Preserve the source boolean's tri-state so history attribution can
      // distinguish revoked (staff), explicitly live (automated), and absent
      // evidence (neutral) without treating a missing value as false.
      tokenRevoked: s.wmkf_externaltokenrevoked == null
        ? null
        : s.wmkf_externaltokenrevoked === true,
    ...
    (historyOnly ? byRequest[reqId].noResponseHistory : byRequest[reqId].reviewers).push(projectedReviewer);
  }
  ...
  const proposalList = await Promise.all(Object.values(byRequest).map(async (p) => {
    ...
    if (lifecycleByRequest[p.proposalId]) {
      ...
      const lifecycleById = new Map(
        lifecycleByRequest[p.proposalId].map((row) => [row.wmkf_appreviewersuggestionid, row]),
      );
      p.reviewSynthesisState = {
        ...
        // Who is blocking and why, so the Reviews tab can name the open
        // invitation instead of reporting a bare count. Names only — no
        // email, token, or answer content.
        blockers: readiness.blockers.map((blocker) => {
          const row = lifecycleById.get(blocker.suggestionId) || {};
          const person = personById[row._wmkf_potentialreviewer_value] || {};
          return {
            suggestionId: blocker.suggestionId,
            reason: blocker.reason,
            name: person.wmkf_name || null,
            accepted: row.wmkf_accepted === true,
            emailSentAt: row.wmkf_emailsentat || null,
          };
        }),
 ...
 * request-scoped reviewer roster + blocker list for the Pre-Site Visit
 * `[[STAFF:RefereeSection]]` fill. Unlike `getReviewers`, this takes no
 * caller `scope`/`azureEmail` — the Pre-Site Visit route already establishes
 * DAL context and access control (`requireAppAccess('reviewers')` inside
 * `withDalContext`, `pages/api/workbench/pre-site-visit.js:70,75`) before
 * `loadPreSiteVisitInputs` calls this. ASSUMES a trusted DAL context already
 * exists — never establishes one (same contract as `getReviewers`, module
 * header).
 *
 * Reuses the SAME query functions `getReviewers` uses for a single proposal
 * — `suggestionAdapter.findByRequest` (`selectedOnly`+`requireComplete`),
 * `fetchPotentialReviewers` (the shared person select, including the five
 * Slice 1 writeup fields), `fetchAnswersBySuggestion` +
 * `ratingsFromAnswers`, and `evaluateReviewSynthesisReadiness` — so this and
 * `getReviewers` cannot silently diverge on who counts as a submitted
 * reviewer or an outstanding blocker. The per-reviewer projection here is a
 * NARROWER, writeup-only subset (no engagement/token/honorarium tracking
 * fields — the referee-section composer doesn't consume them) rather than
 * the full `getReviewers` DTO, kept as a separate literal so this addition
 * cannot change `getReviewers`'s own projection or behavior.
 *
 * @param {{requestId: string}} args
 * @returns {Promise<{reviewers: Array<Object>, blockers: Array<{suggestionId:string, reason:string, name:(string|null), accepted:boolean}>}>}
 */
export async function getWriteupRoster({ requestId }) {
  if (!requestId) return { reviewers: [], blockers: [] };

  const lifecycleRows = await suggestionAdapter.findByRequest(requestId, {
    selectedOnly: true,
    requireComplete: true,
  });
  // Same accepted-or-submitted roster filter `getReviewers` applies for a
  // single proposal (:219) — no_response history rows are excluded, same as
  // there.
  const suggestions = lifecycleRows.filter((r) => r.wmkf_accepted === true || !!r.wmkf_reviewreceivedat);

  const personIds = [...new Set(
    lifecycleRows.map((r) => r._wmkf_potentialreviewer_value).filter(Boolean),
  )];
  const personById = await fetchPotentialReviewers(personIds);
  ...
  const reviewers = suggestions.map((s) => {
    ...
    return {
      ...
      // `s` already carries these — `wmkf_reviewsharepointfolder`/
      // `wmkf_reviewfilename` are part of the SAME shared entity-registry
      // SELECT (`lib/dataverse/core/entity-registry.js` wmkf_appreviewersuggestions)
      // that `suggestionAdapter.findByRequest` always applies, so no new
      // $select is needed — this was a missing projection, not a missing
      // fetch. `loadPreRpBriefInputs` passes this roster's `reviews` straight
      // into `assembleReviewBundle`, which requires both fields on every
      // received review (fail-closed, no silent skip); without them every
      // Pre-RP Brief share would 409 `review_bundle_incomplete`.
      reviewSharePointFolder: s.wmkf_reviewsharepointfolder || null,
      reviewFilename: s.wmkf_reviewfilename || null,
      reviewerOverallAssessment: null,
      answers: [],
  ...
  // Answer snapshot only for submitted rows — same as `getReviewers` (:441-462).
  const submittedIds = reviewers.filter((r) => r.reviewReceivedAt).map((r) => r.suggestionId);
  const answersBySuggestion = await fetchAnswersBySuggestion([...new Set(submittedIds)]);
  for (const r of reviewers) {
    if (!r.reviewReceivedAt) continue;
    r.answers = answersBySuggestion[r.suggestionId] || [];
    r.reviewerOverallAssessment = ratingsFromAnswers(r.answers).overallAssessment;
  }

  // No `contentHash` needed here (unlike `getReviewers`'s job-currentness
  // fingerprint) — only `.blockers` is consumed.
  const readiness = evaluateReviewSynthesisReadiness(lifecycleRows);
  const lifecycleById = new Map(lifecycleRows.map((row) => [row.wmkf_appreviewersuggestionid, row]));
  ...
  // Sort canonically (Codex adversarial review, wrap-up 2026-09-14): the tab
  // and the export roster both name-sort via `compareReviewersByName` before
  // composing, and the composer keeps CALLER order for equal ratings (see
  // that composer's doc comment) — so an unsorted adapter-fetch-order roster
  // here could tie-break differently from the tab for the SAME reviewers
  // (W6/plan §4.2 parity). Sorting here, once, keeps every downstream
  // composer call (composeRefereeSection, etc.) order-identical to the tab
  // without each one having to re-sort defensively.
  reviewers.sort(compareReviewersByName);
 ...
 * `lib/services/reviewer-engagement/correct-status.js` (Stage 3C). This
 * module re-exports the same objects — `instanceof` and reference identity
 * (`toBe`) hold across both import paths.
 */
export { ReviewerStatusMutationError, patchReviewers } from '../reviewer-engagement/correct-status';
...
// lookups) must remain in this select. A third clause (Reviews Tab Phase II
// Slice 1, docs/plans/REVIEWS_TAB_WRITEUP_PARAGRAPHS_PLAN_2026-09-14.md §4.2)
// adds the board-writeup identity + expertise fields the writeup paragraph
// composer needs: wmkf_lastname, wmkf_academicrank, wmkf_maininstitution,
...
async function fetchPotentialReviewers(ids) {
  ...
  for (const chunk of chunked(ids, CHUNK)) {
    ...
    const { records } = await queryReviewers({
      select: withSyntheticReviewerMarkerSelect(
        'wmkf_potentialreviewersid,wmkf_name,wmkf_emailaddress,wmkf_organizationname,wmkf_primaryaffiliation,wmkf_website,wmkf_hindex,wmkf_totalcitations,wmkf_lastname,wmkf_academicrank,wmkf_maininstitution,wmkf_areaofexpertise,wmkf_keywords',
      ),
      filter: orChain,
      top: 500,
    });
