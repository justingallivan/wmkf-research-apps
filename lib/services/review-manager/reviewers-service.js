 ...
 *   - throws NOTHING typed: this route has no non-500 domain errors beyond
 *     shell-side validation; failures propagate UNTYPED and the shell maps
 *     them to the route's existing per-verb 500 envelopes
 *     ({ error, details(dev-only), timestamp });
 ...
 */
export async function getReviewers({ proposalId, requestNumber, cycleCode, status, azureEmail }) {
  let suggestions = [];
  let requestById = {};
  ...
  for (const s of suggestions) {
    ...
    byRequest[reqId].reviewers.push({
      ...
      reviewDueDateAtSend: s.wmkf_reviewduedateatsend || null,
      reviewDueDateLastSent: s.wmkf_reviewduedatelastsent || null,
 ...
 * path (single, batch, thank-you send) is covered uniformly.
 *
 * @param {Object} args
...
export async function patchReviewers({ suggestionIds, reviewStatus, suggestionId, lifecycle, actingUserSystemId }) {
  ...
  if (isTerminalReviewStatus(requestedStatus)) {
