...
export async function getReviewers({ proposalId, requestNumber, cycleCode, status, scope = 'my', azureEmail, programId, callerSystemId }) {
  ...
  if (isSingleProposal) {
    ...
    // Keep the accepted/review-received population unchanged, but retain a
    // no_response release as a read-only history row. It is deliberately
    // projected as `released` below so it cannot enter accepted-only selection
    // or mutation controls in ReviewerManagePanel.
    suggestions = rows.filter((r) => r.wmkf_accepted === true
      || !!r.wmkf_reviewreceivedat
      || isNoResponseLifecycleRow(r));
  } else {
    let result;
    if (scope === 'all') {
      if (!cycleCode) {
  ...
  for (const s of suggestions) {
    ...
    const reviewStatus = responseType === 'no_response'
      && s.wmkf_accepted !== true
      && !s.wmkf_reviewreceivedat
      ? 'released'
      : mappedReviewStatus || 'accepted';
