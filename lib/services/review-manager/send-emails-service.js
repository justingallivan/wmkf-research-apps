 ...
 * the single service call). Post-send bookkeeping for materials/follow-up/
 * thank-you (`recordDeliveredEmail`, called at the existing post-loop site)
 * is extracted to `reviewer-engagement/record-email-outcome.js` (Reviewer
 * Lifecycle Stage 3E); the invitation stamp (`recordDeliveredInvitation`,
 * called inline in the send loop immediately after each send) is extracted
 * to `reviewer-engagement/record-invitation.js` (Reviewer Lifecycle Stage
 * 3F); scan orchestration, transport, streaming and correlation stay here.
 *
 ...
 *     invitation writes run in-loop via `recordDeliveredInvitation`;
 *     non-invitation bookkeeping re-reads the recipient's row and uses its
 *     exact ETag in the best-effort post-loop pass.
...
import { recordDeliveredEmail } from '../reviewer-engagement/record-email-outcome';
...
import { recordDeliveredInvitation } from '../reviewer-engagement/record-invitation';
import {
...
export async function sendEmails({ requestBody, fromEmail, actingUserSystemId }, onEvent) {
  ...
  try {
    ...
    for (const draft of drafts) {
      ...
      // With Test Request isolation on, a reviewer email must name its request
      // so the delivery seam can refuse test requests; never send it unlinked.
      if (!regardingId && testRequestIsolationEnabled()) {
        failed.push({
          suggestionId: draft.suggestionId,
          candidateName: name,
          candidateEmail: email,
      ...
      // Refuse test requests before attachment work, reviewer-token minting
      // (which invalidates any existing portal link) or lifecycle writes.
      try {
        await assertRequestEmailAllowed(regardingId);
      } catch (refusal) {
        failed.push({
          suggestionId: draft.suggestionId,
          candidateName: name,
          candidateEmail: email,
          code: refusal?.code || 'test_request_email_denied',
          error: refusal?.message || 'Email is disabled for test requests.',
        });
        sendEvent('email_failed', failed[failed.length - 1]);
        sendEvent('progress', {
          stage: 'sending',
      ...
      try {
        ...
        if (templateType === 'invitation' && markAsSent) {
          try {
            await recordDeliveredInvitation({ suggestionId: draft.suggestionId, actingUserSystemId });
            inviteRecorded = true;
          } catch (stampErr) {
        ...
        // A send that threw may have dispatched before failing (e.g.
        // the SendEmail POST reached Dynamics but the response was lost). We cannot
        // prove it did NOT go out, so every template type is recorded as
        // "possibly sent — verify before retry", never plain `failed`, to keep
        // staff from blindly sending a duplicate.
        ...
        if (err?.dispatched !== false) {
          console.error(`Email send to ${name} <${email}> failed after a possible dispatch (verify before retry):`, err.message);
          unconfirmed.push({
            suggestionId: draft.suggestionId,
            candidateName: name,
            candidateEmail: email,
