    ...
    for (const draft of drafts) {
      ...
      if (templateType === 'invitation') {
        ...
        if (invitationLinkValidation.reason === INVITATION_LINK_INVALID_REASON.UNRESOLVED_PLACEHOLDER) {
          ...
          reason = INVALID_SECURE_LINK_SKIP_REASON;
      ...
      try {
        ...
        // Exception: the pre-dispatch impersonation guard (`impersonation_disabled`)
        // provably never sent, so it takes the plain failed[] path.
        if (templateType === 'invitation' && err?.code !== 'impersonation_disabled') {
