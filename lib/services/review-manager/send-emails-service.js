 ...
 *     invitation writes run inline while re-sendable templates use the
 *     best-effort post-loop lifecycle pass;
 ...
 *
 * Send-time token authority gate (v3, Plan: preview-retry + stale-render
 * token authority): a template containing `{{externalLink}}` mints a fresh
 * JWT and overwrites the recipient's durable `wmkf_externaltokenhash` at
 * RENDER time (render-emails-service.js), so an older still-open preview can
 * carry a dead link. Client generation/epoch guards suppress stale client
 * writes but cannot order two durable render calls. This service is
 * therefore the dispatch-time authority: immediately before
 * `DynamicsService.createAndSendEmail`, it extracts the reviewer JWT(s) from
 * the FINAL edited draft subject+body and, whenever `externalLinkExpected`
 * requires one (or one was manually introduced), calls
 * `verifySuggestionToken` and requires a case-insensitive suggestion/request
 * match before dispatching. A stale, ambiguous, missing, or
 * recipient-mismatched link fails only that row via `email_failed` (see
 * `code` above) and the batch continues to healthy siblings.
 */
...
// Matches `/external/review/<jwt>` with a three-base64url-segment JWT,
// ignoring any trailing query string (e.g. `?action=accept`). Global so
// callers can find every occurrence in the final edited subject+body.
const EXTERNAL_REVIEW_JWT_PATTERN = /\/external\/review\/([A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+)/g;

// Extracts the unique reviewer JWTs embedded in the final edited draft.
// Scans subject AND body concatenated: the expectation stamp
// (render-emails-service's `needsExternalLink`) derives from the placeholder
// in either the template subject or body, so a PD-edited template with the
// placeholder in its subject would otherwise stamp `externalLinkExpected:
// true` while body-only extraction found 0 JWTs — a permanent fail-closed
// dead-end that "regenerate the preview" cannot resolve. Repeated identical
// copies of the same JWT dedupe to one (invitation HTML generation already
// tolerates a repeated identical link).
function extractExternalReviewJwts(text) {
  const seen = new Set();
  let match;
  EXTERNAL_REVIEW_JWT_PATTERN.lastIndex = 0;
  while ((match = EXTERNAL_REVIEW_JWT_PATTERN.exec(text)) !== null) {
    seen.add(match[1]);
  }
  return [...seen];
}
...
export async function sendEmails({ requestBody, fromEmail, actingUserSystemId }, onEvent) {
  ...
  try {
    ...
    const reviewButtonLabel = templateType === 'invitation'
      ? ''
    ...
    for (const d of drafts) {
      ...
      const [person, request] = await Promise.all([
        personId ? potentialReviewerAdapter.getByIdWithSelect(personId, {
          select: 'wmkf_potentialreviewersid,wmkf_name,wmkf_emailaddress,wmkf_firstname,wmkf_lastname,_wmkf_contact_value,wmkf_orcid,wmkf_identitystatus,wmkf_emailsource,wmkf_addresstruststatejson',
        }).catch(() => null) : null,
    ...
    for (const draft of drafts) {
      ...
      // Send-time token authority gate (v3): the durable per-recipient
      // wmkf_externaltokenhash is the only cross-request ordering authority for
      // the {{externalLink}} placeholder — client generation/epoch guards
      // suppress stale client writes but cannot order two durable render calls
      // (or two clients) that both minted a token for the same recipient. Verify
      // the FINAL edited draft immediately before dispatch; no application
      // `await` may separate a successful verification from the dispatch call
      // below. See render-emails-service.js `externalLinkExpected` stamp.
      const extractedJwts = extractExternalReviewJwts(`${draft.subject || ''}\n${draft.body || ''}`);
      let tokenFailureCode = null;
      if (typeof draft.externalLinkExpected !== 'boolean') {
        ...
        // externalLinkExpected === false and no JWT: no token expected, don't verify.
      } else {
        let verified;
        try {
          verified = await verifySuggestionToken(extractedJwts[0]);
        } catch (verifyErr) {
        ...
        if (!verified.ok) {
          ...
          // Verifier success is necessary but not sufficient: also require the
          // token belong to THIS recipient/request, case-insensitively — a
          // valid token minted for another suggestion/request fails closed.
      ...
      if (tokenFailureCode) {
        failed.push({
          ...
          error: tokenFailureCode === 'external_link_superseded'
      ...
      try {
        const emailPayload = {
          subject: draft.subject,
          body: plainTextToHtml(draft.body, { programDirectorContact, reviewButtonLabel, templateType }),
