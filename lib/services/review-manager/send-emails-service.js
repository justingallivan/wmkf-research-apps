 ...
 *     invitation and materials writes run inline. A failed materials write is
 *     an actionable sent_but_unrecorded partial result, never a clean success;
 ...
 *   - email_sent_but_unrecorded { suggestionId, candidateName,
 *     candidateEmail, repairReceipt,
 *     repairEndpoint, error } — materials only; dispatch succeeded but the
 *     ETag-guarded lifecycle stamp failed, so repair without re-sending
 *   - result { sent, failed, skipped, unconfirmed, sentButUnrecorded, stats,
 *     orcidBackprop }
...
import { resolveEffectiveReviewDueDate } from './effective-review-due-date';
import {
  assertMaterialsSendRepairReceiptConfigured,
  mintMaterialsSendRepairReceipt,
} from './materials-send-repair-receipt';
...
export async function sendEmails({ requestBody, fromEmail, actingUserSystemId }, onEvent) {
  ...
  try {
    ...
    if (templateType === 'materials' && markAsSent) {
      try {
        assertMaterialsSendRepairReceiptConfigured();
      } catch (receiptError) {
        sendEvent('error', { message: receiptError.message });
        return;
      }
    }
    ...
    const unconfirmed = [];
    // A materials email was dispatched, but its ETag-guarded inline lifecycle
    // write failed. These are not clean successes and must be repaired without
    // blindly re-sending the email.
    const sentButUnrecorded = [];
    ...
    for (const draft of drafts) {
      ...
      if (templateType === 'materials') {
        ...
        const requestedDueDate = draft.effectiveReviewDueDateSource === 'settings'
          ? draft.effectiveReviewDueDate
          : null;
        const currentDueDate = resolveEffectiveReviewDueDate({
          requestedDueDate,
          requestDueDate: request?.wmkf_reviewduedate,
          cycleDueDate: cycle.review_deadline,
        });
        if (currentDueDate.error || !draft.effectiveReviewDueDate) {
          skipped.push({
            suggestionId: draft.suggestionId,
            candidateName: name,
            candidateEmail: email,
            reason: currentDueDate.error ? 'invalid_effective_due_date' : 'missing_effective_due_date',
          });
          continue;
        }
        if (currentDueDate.value !== draft.effectiveReviewDueDate) {
          skipped.push({
            suggestionId: draft.suggestionId,
            candidateName: name,
            candidateEmail: email,
            reason: 'stale_due_date',
            renderedDueDate: draft.effectiveReviewDueDate,
            currentDueDate: currentDueDate.value,
          });
          continue;
        }
      }
      ...
      try {
        ...
        const emailDispatchedAt = new Date().toISOString();

        // Materials: stamp the exact server-rendered due date INLINE with the
        // materials lifecycle fields. The batch-start row is fresh and carries
        // the ETag used for this conditional set-once write. A re-send omits the
        // date when already present, so the first deadline is never rewritten.
        let lifecycleRecorded;
        if (templateType === 'materials' && markAsSent) {
          const currentStatusValue = suggestion?.wmkf_reviewstatus;
          const shouldBump = currentStatusValue == null
            || currentStatusValue === suggestionAdapter.REVIEW_STATUS_MAP.accepted;
          try {
            await suggestionAdapter.updateLifecycle(draft.suggestionId, {
              materialsSentAt: emailDispatchedAt,
              // Two dates, deliberately (owner decision S369). `atSend` is
              // set-once — the deadline first committed to. `lastSent` is
              // overwritten every send — the deadline last communicated. A
              // set-once stamp alone marks a reviewer late whenever WMKF
              // extended the deadline and re-sent materials, which is the same
              // "never penalize a reviewer for WMKF's own scheduling" principle
              // that split withdrew from released. If this write fails after
              // dispatch, the signed repair receipt recovers this exact rendered
              // date without sending the email again.
              ...(draft.effectiveReviewDueDate
                ? {
                  reviewDueDateLastSent: draft.effectiveReviewDueDate,
                  ...(suggestion?.wmkf_reviewduedateatsend
                    ? {}
                    : { reviewDueDateAtSend: draft.effectiveReviewDueDate }),
                }
                : {}),
            ...
            const repairReceipt = mintMaterialsSendRepairReceipt({
              requestId: suggestion._wmkf_request_value,
              suggestionId: draft.suggestionId,
              effectiveReviewDueDate: draft.effectiveReviewDueDate,
              materialsSentAt: emailDispatchedAt,
            ...
            sentButUnrecorded.push(outcome);
            sendEvent('email_sent_but_unrecorded', outcome);
            console.error(`Materials email sent but lifecycle stamp failed for ${draft.suggestionId}:`, stampError.message);
          }
        }
        ...
        // rather than losing that fact to a scrolling warning. Materials also
        // stamp inline above; followup/thankyou remain in the post-loop pass.
        let inviteRecorded;
        ...
        sent.push({
          ...
          ...(lifecycleRecorded === undefined ? {} : { lifecycleRecorded }),
    ...
    // Invitations and materials are stamped inline in the send loop (above);
    // this post-loop pass handles followup/thankyou only.
    if (markAsSent && sent.length > 0 && templateType !== 'invitation' && templateType !== 'materials') {
      ...
      for (const s of sent) {
        try {
          if (templateType === 'followup') {
            ...
            const currentStatusValue = recipientBySuggestion.get(s.suggestionId)?.suggestion?.wmkf_reviewstatus;
            const isTerminal = Object.values(TERMINAL_REVIEW_STATUS_VALUES).includes(currentStatusValue);
            await suggestionAdapter.updateLifecycle(s.suggestionId, {
              thankYouSentAt: now,
              ...(isTerminal ? {} : { reviewStatus: 'complete' }),
            }, { actingUserSystemId });
          }
    ...
    sendEvent('result', {
      ...
      stats: {
        ...
        sentButUnrecorded: sentButUnrecorded.length,
        total: drafts.length,
      },
    ...
    sendEvent('complete', {
      message: `Sent ${sent.length} of ${drafts.length} ${templateType} email(s)`
        ...
        + (sentButUnrecorded.length ? `; ${sentButUnrecorded.length} sent but not recorded (repair required)` : '')
