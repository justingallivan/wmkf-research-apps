 ...
 * ambiguous, invalid, recipient-mismatched, or mint-failed link fails only
 * that row via `email_failed`; healthy siblings continue.
 */
...
// path segment captured by EXTERNAL_REVIEW_JWT_PATTERN. Query strings and all
...
function substituteExternalReviewJwt(text, authoritativeJwt) {
  ...
  return String(text || '').replace(
    ...
    (fullMatch, priorJwt) => fullMatch.replace(priorJwt, authoritativeJwt),
  );
}
