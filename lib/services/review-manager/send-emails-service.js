...
export async function sendEmails({ requestBody, fromEmail, actingUserSystemId }, onEvent) {
  ...
  try {
    ...
    for (const draft of drafts) {
      ...
      // Accepted/null are the only pre-release status shapes. A terminal or
      // unknown status must not fall through to mintAndStore, which would clear
      // a revocation while issuing a materials link.
      if (
        templateType === 'materials'
