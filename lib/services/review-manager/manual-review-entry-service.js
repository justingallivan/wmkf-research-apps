...
export async function submitManualReviewEntry({
  ...
  const suggestion = await loadEligibleSuggestion(suggestionId);
  // Authoritative (uncached) resolve: write boundary — the setVersion check
  // below must compare against the live set, not this process's cached copy.
  ...
  const snapshotKeys = new Set(
    questionSet
      .filter((field) => field.type === 'picklist' || field.type === 'multiselect' || field.type === 'richtext')
      .map((field) => field.key),
  );
