...
function makeStrategy(kind) {
  return {
    async checkEligibility(message, { force = false } = {}) {
      ...
      // A marker with no transport state means someone else (the manual nudge
      // path, or a pre-ledger send) already reminded this reviewer — our queued
      // copy is redundant. Once OUR activity exists we are completing our own
      // send: the marker is ours, stamped by the pre-send claim.
      if (reminderMarker(kind, row) && !message.dynamics_email_id) {
