 ...
 *           superseded us), or the frozen recipient no longer matches the
 *           reviewer's current email. The sweep may later REVIVE a stopped,
...
function makeStrategy(kind) {
  return {
    async checkEligibility(message, { force = false } = {}) {
      ...
      // queued copy is redundant. The marker PLUS claim_committed_at proves
      // our marker/token PATCH landed: a retry after a post-claim crash must
      // resume the send (correlation recovery, then re-mint if no activity
      // exists), not falsely stop with a "reminded" record whose email never
      // went out. claim_committed_at alone is only pre-PATCH intent.
      ...
      if (kind === 'respond') {
        ...
        // EXCEPT when this row's fresh marker proves its pre-activity claim
        // PATCH landed: the current token is our own rotation and we are about
        // to re-mint a fresh one anyway. claim_committed_at without the marker
        // means the PATCH may have failed, so the original token still governs.
        const resumingOwnClaim = Boolean(reminderMarker(kind, row))
          && Boolean(message.claim_committed_at)
          && !message.dynamics_email_id;
      ...
      // Before minting a live token, fail closed if the frozen ledger recipient
      // no longer matches the reviewer's current email. Never rewrite the row
      // here: the next sweep revives a stopped, never-transported row with fresh
      // recipients. An existing Dynamics activity already froze its recipients
      // and content, so correlation/send completion adds no new stop condition.
      if (!message.dynamics_email_id) {
        const reviewer = await loadReminderReviewer(row._wmkf_potentialreviewer_value);
        const currentEmail = reviewer?.wmkf_emailaddress;
        const frozenRecipients = parseRecipientList(message.to_recipients);
        const recipientMatches = typeof currentEmail === 'string'
          && frozenRecipients.length === 1
          && typeof frozenRecipients[0] === 'string'
          && frozenRecipients[0].toLowerCase() === currentEmail.toLowerCase();
        if (!recipientMatches) return { stop: true, reason: 'recipient_changed' };
      }
