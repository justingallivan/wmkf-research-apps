...
function costLines(cost) {
  if (!cost) return ['Cost total: unavailable.'];
  if (cost.unknownCount > 0) {
    return [`Cost total withheld: ${cost.unknownCount} attempt(s) with unknown outcome.`];
  }
  return [`Cost total: $${(Number(cost.totalCents || 0) / 100).toFixed(2)}.`];
}
 ...
 */
export async function renderReviewPanelEntryDocuments(report) {
  if (!report || typeof report !== 'object') throw new Error('renderReviewPanelEntryDocuments requires a report object');
  const [docxRaw, pdfRaw] = await Promise.all([renderReportDocx(report), renderReportPdf(report)]);
  const docx = await assertStructurallyValidDocx(docxRaw);
