...
/**
 * Return a claimed job to the queue without consuming the attempt its claim
 * counted: its request's test-request state could not be read (Stage 1c), so
 * nothing was tried and a transient read failure must never exhaust retries.
 */
export async function releaseReviewSynthesisJob(job, { delaySeconds = 300 } = {}) {
  const result = await sql`
    UPDATE review_synthesis_jobs
       SET status = 'queued',
           attempts = GREATEST(attempts - 1, 0),
           next_attempt_at = NOW() + (${clampSeconds(delaySeconds)} || ' seconds')::INTERVAL,
           locked_until = NULL,
           lease_token = NULL,
           updated_at = NOW()
     WHERE id = ${job.id}
