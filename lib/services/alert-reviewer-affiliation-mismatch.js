...
const CONTACT_INSTITUTION_SELECT = 'contactid,adx_organizationname,_parentcustomerid_value';
...
const defaultContactsAdapter = {
  async getInstitutionById(contactId) {
    if (!contactId) return null;
    return DynamicsService.getRecord('contacts', contactId, {
      select: CONTACT_INSTITUTION_SELECT,
    });
  },
};
...
export async function alertReviewerAffiliationMismatch(
  ...
  const {
    ...
    withDynamicsBypass = bypassDynamicsRestrictions,
    warn = console.warn,
  } = deps;
