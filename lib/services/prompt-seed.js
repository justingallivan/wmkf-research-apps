...
const odataEsc = (s) => String(s).replace(/'/g, "''");

async function loadRows(promptName) {
  const res = await DynamicsService.queryRecords(ENTITY, {
    select: 'wmkf_ai_promptid,wmkf_ai_promptname,wmkf_promptversion,wmkf_ai_iscurrent',
    filter: `wmkf_ai_promptname eq '${odataEsc(promptName)}'`,
...
export async function seedPromptRow({ promptName, recordData, force = false, now } = {}) {
  ...
  const created = await DynamicsService.createRecord(ENTITY, {
    ...recordData,
  ...
  for (const r of plan.current) {
    const fresh = await DynamicsService.getRecord(ENTITY, r.wmkf_ai_promptid, { select: 'wmkf_ai_promptid' });
    await DynamicsService.updateRecord(ENTITY, r.wmkf_ai_promptid, { wmkf_ai_iscurrent: false }, { ifMatch: fresh._etag });
  }
