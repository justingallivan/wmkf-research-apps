...
async function assertReportRequestIncluded(requestGuid) {
  if (!testRequestIsolationEnabled() || !requestGuid) return;
  const result = await withDalContext('grant-reporting-test-request-check', () => (
    grantRequestAdapter.findByIds([requestGuid], {
...
export async function handleFullExtract({ apiKey, reportRef, proposalRef, headerFromDynamics, requestGuid, profileId, actingUserSystemId }) {
  ...
  await assertReportRequestIncluded(requestGuid);
