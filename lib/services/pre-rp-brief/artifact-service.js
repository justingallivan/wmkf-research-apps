...
function attemptIsInFlight(attempt, sourceDocumentId) {
...
async function assertCurrentBriefReplaceable(requestId, currentRow, dependencies) {
  ...
  if (sent || attempts.some((attempt) => attemptIsInFlight(attempt, sourceDocumentId))) {
...
// distribution service). Malformed/missing snapshot JSON reads as zero
// rather than throwing, since this is a display affordance, not the gate.
function receivedReviewCountOf(row) {
  ...
  return reviews.filter((review) => !!review?.reviewReceivedAt).length;
}
...
export async function generatePreRpBrief(
  { requestId, clientOperationId, actingUserSystemId = null },
  dependencies = DEFAULT_DEPENDENCIES,
) {
  if (!isGuid(requestId)) {
    throw new ServiceHttpError('A valid requestId is required.', {
  ...
  await assertCurrentBriefReplaceable(requestId, currentRow, dependencies);
