...
import { briefInputFingerprint, isSupportedBriefSnapshotVersion, renderBrief } from './docx-renderer.js';
import {
...
function receivedReviewCountOf(row) {
  ...
  if (!snapshot || typeof snapshot !== 'object' || Array.isArray(snapshot)
    || !isSupportedBriefSnapshotVersion(snapshot.schemaVersion)
  ...
  // `briefInputFingerprint(snapshot)` uses the snapshot's own schemaVersion,
  // so a legacy (v1) row re-hashes with the 8-field legacy list.
  try {
...
/**
 * The frozen input snapshot a reclaimed generation row was created with,
 * parsed and verified (supported schema version, artifact type, and a
 * re-hash equal to the row's recorded fingerprint). Throws 409 when it does
 * not verify: rendering fresh inputs under a stale snapshot would break the
 * snapshot-is-provenance contract, and rendering an unverifiable snapshot
 * would trust bytes the row cannot vouch for.
 */
function storedBriefEnvelopeForRender(row) {
  let snapshot = row?.wmkf_presiteinputsnapshotjson;
  try {
  ...
  if (valid) {
    try {
      fingerprintMatches = briefInputFingerprint(snapshot) === row.wmkf_inputfingerprint;
    } catch {
      fingerprintMatches = false;
    }
  }
  if (!valid || !fingerprintMatches) {
    throw new ServiceHttpError('The reclaimed brief row\'s stored input snapshot does not verify; it cannot be rendered.', {
      httpStatus: 409,
      code: 'pre_rp_brief_snapshot_invalid',
    });
  }
  return snapshot;
}
...
export async function generatePreRpBrief(
  ...
  try {
    ...
    // Provenance (Codex adversarial review, 2026-09-21): a RECLAIMED row keeps
    // the frozen snapshot it was created with, so it must render from THAT
    // snapshot, not from the freshly loaded inputs. Under phase 1 the two can
    // differ in fields the v1 fingerprint does not cover (expertise), and an
    // exact retry after an expertise edit would otherwise upload text the
    // stored snapshot never recorded. A fresh row renders the envelope it is
    // about to persist. Fails closed if the stored snapshot does not verify.
    const renderEnvelope = reclaimedRow ? storedBriefEnvelopeForRender(row) : inputs.envelope;
    const { docx } = await dependencies.renderDocx(renderEnvelope);
