 ...
 *   { schemaVersion: 1 | 2, artifactType: 'pre-rp-brief', request, reviews }
...
// Role labels for the two people tokens (owner request 2026-09-16): the
// brief prints "PI: <name>" and "PD: <name>" so readers can tell which is
// which. Applied at fill only; the input fingerprint hashes the raw names.
const DV_VALUE_PREFIX = Object.freeze({
  principalInvestigator: 'PI: ',
  programDirector: 'PD: ',
});

function dvPlaceholderValue(key, value) {
  return `${DV_VALUE_PREFIX[key] || ''}${value}`;
}
...
// id, received state, name, academic rank, overall rating, the three
// institution-source fields the composers read, and (2026-09-21, owner
// decision reversing the plan's "two sentences only") lastName/keywords/
// areaOfExpertise, the composeExpertiseSentence inputs, so the brief
// regenerates when a reviewer's expertise data changes.
...
// Snapshot schemaVersion 1's field list, frozen exactly as it was before
// 2026-09-21. A v1 stored row's fingerprint was computed from these 8
// fields only; re-hashing it (or a live envelope, when comparing against a
// v1 stored row) must use this list, never the current one, or every
// existing brief row fails verification the moment the current list grows.
export const LEGACY_REVIEW_FINGERPRINT_FIELDS_V1 = Object.freeze([
  'suggestionId',
  'reviewReceivedAt',
  'name',
  'academicRank',
...
export const SUPPORTED_BRIEF_SNAPSHOT_SCHEMA_VERSIONS = Object.freeze([1, 2]);

export function isSupportedBriefSnapshotVersion(version) {
  return SUPPORTED_BRIEF_SNAPSHOT_SCHEMA_VERSIONS.includes(version);
}

// The review field list a given snapshot schemaVersion was fingerprinted
// with. Fails closed on an unsupported version, same posture as
// `assertEnvelope`.
export function reviewFingerprintFieldsFor(schemaVersion) {
  if (schemaVersion === 1) return LEGACY_REVIEW_FINGERPRINT_FIELDS_V1;
  if (schemaVersion === 2) return REVIEW_FINGERPRINT_FIELDS;
  throw new Error(`Pre-RP Brief snapshot has an unsupported schemaVersion: ${schemaVersion}.`);
}
...
function assertEnvelope(envelope) {
  ...
  if (!isSupportedBriefSnapshotVersion(envelope.schemaVersion)) {
 ...
 * a professor at Institution; ...  Nadell has expertise in microbial
 * ecology, while Bharat has expertise in cryo-electron tomography." — the
 * same deterministic composers the Reviews tab and Pre-Site writeup use,
 * concatenated: score sentence, reviewer sentence, and (owner decision
 * 2026-09-21, reversing the plan's "two sentences only") the expertise
 * sentence (no themes, no quotations).
 */
function composeRefereeRunSegments(reviews) {
  ...
  const expertise = composeExpertiseSentence(submitted);
  const segments = [];
...
function renderDocumentXml(xml, { request, refereeSegments }) {
  ...
  const updated = xml.replace(/<w:p(?:\s[^>]*)?>[\s\S]*?<\/w:p>/g, (paragraphXml) => {
    ...
    for (const [key, placeholder] of Object.entries(DV_PLACEHOLDERS)) {
      const result = replaceAcrossTextNodes(out, placeholder, dvPlaceholderValue(key, request[key]));
...
function canonicalReview(review, fields) {
  const canonical = {};
  for (const field of fields) canonical[field] = review?.[field] ?? null;
  return canonical;
}

/**
 * Canonical representation of every composer input in the snapshot envelope
 ...
 * either), with reviews ordered by `compareReviewersByName` (the same tie
 * order the roster and writeup composers already use). Pure — no I/O.
 *
 * `options.schemaVersion` picks the review field list, defaulting to the
 * envelope's own `schemaVersion`. When comparing a LIVE envelope against a
 * STORED row, pass the stored snapshot's schemaVersion so a legacy brief
 * only drifts on changes it could ever have rendered.
 *
 * @param {{schemaVersion:number, artifactType:string, request:object, reviews:Array}} envelope
 * @param {{schemaVersion?: number}} [options]
 * @returns {object} canonical received-only input state
 */
export function canonicalBriefInputState(envelope, { schemaVersion } = {}) {
  assertEnvelope(envelope);
  const effectiveSchemaVersion = schemaVersion ?? envelope.schemaVersion;
  const fields = reviewFingerprintFieldsFor(effectiveSchemaVersion);
  ...
  return {
    ...
    reviews: orderedReviews.map((review) => canonicalReview(review, fields)),
  };
}
 ...
 */
export function briefInputFingerprint(envelope, { schemaVersion } = {}) {
  return crypto.createHash('sha256')
    .update(stableStringify(canonicalBriefInputState(envelope, { schemaVersion })), 'utf8')
    .digest('hex');
}
