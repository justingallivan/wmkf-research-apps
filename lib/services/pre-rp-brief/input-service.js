...
const REQUEST_SELECT = [
  ...
  'akoya_requestnum',
  'akoya_title',
  // Select the base lookups only. Dataverse rejects `_x_value_formatted` in
  // `$select` (400 0x80060888); the formatted names arrive as
  // `@OData.Community.Display.V1.FormattedValue` annotations that
  // `lib/services/dynamics/annotations.js` rewrites onto `_x_value_formatted`
  // after the read (same pattern as `grantee-document-assembly.js`).
...
export { DEFAULT_DEPENDENCIES, REQUEST_SELECT };
