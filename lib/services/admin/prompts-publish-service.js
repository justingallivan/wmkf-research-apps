...
export async function publishPrompt({
  ...
  const payload = buildPublishPayload(priorRow, { body, systemPrompt, variables, model });
...
function buildPublishPayload(priorRow, { body, systemPrompt, variables, model }) {
  return {
    body,
    ...
    outputSchema: priorRow.wmkf_ai_promptoutputschema || null,
