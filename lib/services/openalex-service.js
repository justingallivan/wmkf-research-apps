...
class OpenAlexService {
  ...
  static async getRichestAuthorByOrcid(orcid, { signal, timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
    ...
    const params = new URLSearchParams({ filter: `orcid:${checked.id}`, 'per-page': '25' });
    if (OPENALEX_POLITE_MAILTO) params.set('mailto', OPENALEX_POLITE_MAILTO);
    ...
    try {
      ...
      if (signal?.aborted) throw err; // abort/timeout must propagate like the other lookups
      records = [];
    }

    if (records.length === 0) {
      // List form returned nothing (or errored, non-abort): fall back to the canonical
