...
function mapAuthorRecord(record = {}) {
  ...
  return {
    ...
    // Public profile link for staff orientation (field-primer expert links only —
    // never a contact/candidate source). OpenAlex returns `ids.wikipedia` as a full
    // article URL; null when the author has no linked Wikipedia page. Never fabricated.
    wikipedia: record.ids?.wikipedia || null,
