...
/**
 * Research topics from the author's `topics` list (the current OpenAlex author
 * object; verified live 2026-09-21). These are far more specific than the
 * deprecated `x_concepts` that `topTopics` reads ("Scientometrics and
 * bibliometrics research" vs "Computer science"), so they are the expertise
 * source for a reviewer whose only provenance is an applicant recommendation.
 * Sorted by work count explicitly rather than trusting API order; capped at five.
 * `topTopics`/`topics` stay unchanged — identity grounding depends on them.
 */
function researchTopics(record) {
  const topics = Array.isArray(record?.topics) ? record.topics : [];
  return topics
    .filter((topic) => typeof topic?.display_name === 'string' && topic.display_name.trim())
...
function mapAuthorRecord(record = {}) {
  ...
  return {
    ...
    researchTopics: researchTopics(record),
  ...
  return {
    ...
    authorships: (Array.isArray(record.authorships) ? record.authorships : []).map((authorship) => {
      ...
      return {
        ...
        rawOrcid: normalizeOrcid(authorship.raw_orcid),
        orcid: normalizeOrcid(authorship.raw_orcid || author.orcid),
