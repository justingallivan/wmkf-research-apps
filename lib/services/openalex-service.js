...
/**
 * Shared server-side OpenAlex client. Methods that participate in the reviewer
 * identity shadow path accept an optional `requestScope`; its callback is
 * invoked by `safeFetch` before every actual request, so retries and redirects
 * consume the caller's per-resolution cap rather than escaping it.
 */
  ...
  for (let attempt = 0; attempt < MAX_RETRY_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      if (err?.name === 'AbortError'
        || err?.code === 'openalex_timeout'
        || err?.code === 'reviewer_identity_openalex_budget_exhausted'
        || err?.status < 500
...
class OpenAlexService {
   ...
   */
  static async getAuthorByOrcid(orcid, {
    signal,
    timeoutMs = DEFAULT_TIMEOUT_MS,
    requestScope = null,
  } = {}) {
    ...
    let data;
    try {
      data = await fetchJsonWithRetry(url, { signal, timeoutMs, requestScope });
    } catch (err) {
   ...
   */
  static async getRichestAuthorByOrcid(orcid, {
    signal,
    timeoutMs = DEFAULT_TIMEOUT_MS,
    requestScope = null,
  } = {}) {
    ...
    try {
      const data = await fetchJsonWithRetry(
        `${OPENALEX_AUTHOR_BASE_URL}?${params.toString()}`,
        { signal, timeoutMs, requestScope },
      );
      ...
      if (err?.status === 404) {
        return this.getAuthorByOrcid(orcid, { signal, timeoutMs, requestScope });
      }
      throw err;
    }
   ...
   */
  static async getInstitution(rorOrId, {
    signal,
    timeoutMs = DEFAULT_TIMEOUT_MS,
    requestScope = null,
  } = {}) {
