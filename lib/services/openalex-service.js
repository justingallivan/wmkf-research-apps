...
// Known second-level-domain tokens that, under a 2-letter ccTLD, form a TWO-LABEL public
// suffix (e.g. `ac.uk`, `edu.au`, `edu.ph`, `co.jp`, `gov.in`). When the host's last two
// labels match this shape, the registrable domain (eTLD+1) is the LAST THREE labels, not
// two. This is a PATTERN, not the curated list it replaces (Codex S251 HIGH): the old
// fixed list silently returned a bare public suffix like `edu.ph` for any suffix it didn't
// enumerate, and the keep-biased email-domain guard then matched `anyone@x.edu.ph` against
// it — over-keeping a namesake. The pattern generalizes to unlisted academic ccTLD suffixes.
const PUBLIC_SUFFIX_SLDS = new Set([
  'ac', 'edu', 'co', 'com', 'org', 'net', 'gov', 'gob', 'go', 'or', 'ne', 'res',
  'sch', 'mil', 'int', 'nic', 'web', 'gen', 'in',
]);

// Extract the registrable domain (eTLD+1) from an institution homepage URL. Strips the
// scheme/userinfo/port/path, lowercases, and reduces to eTLD+1: `https://web.mit.edu` →
// `mit.edu`; `https://www.ox.ac.uk/` → `ox.ac.uk`; `https://www.university.edu.ph` →
// `university.edu.ph`. FAILS CLOSED — returns null — when it cannot determine a safe
// registrable domain (a bare two-label public suffix like `edu.ph`), so the keep-biased
// guard skips rather than over-matches. Returns null when no host is present.
function registrableDomainFromUrl(url) {
  ...
  // Two-label public suffix (known SLD token + 2-letter ccTLD): registrable = last THREE.
  if (tld.length === 2 && PUBLIC_SUFFIX_SLDS.has(sld)) {
    // Only a bare suffix (no institution label) — cannot form a safe eTLD+1 → fail closed.
