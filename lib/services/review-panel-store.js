...
import { sql, withTransaction } from '../postgres/client';
import { ServiceHttpError } from './service-http-error';
import { listAppKeysForUser } from './app-access-service';

/** The admin-panel app grant that (with the superuser role) admits a profile to the panel — owner decision T1, docs/plans/REVIEW_PANEL_WORKBENCH_TAB_PLAN_2026-09-13.md. */
export const REVIEW_PANEL_APP_KEY = 'review-panel';
const ACTOR_REQUIRED_MESSAGE = 'An active profile with Review Panel access is required.';
...
/**
 * Postgres-only actor check: the profile exists and is active. Safe to call
 * inside a `FOR UPDATE` transaction (it is, from mutateReviewPanelRun and the
 * retry/re-render/cancel requests) because it never leaves Postgres. Returns
 * `isSuperuser` so assertReviewPanelAccess can skip the grant lookup.
 */
export async function assertReviewPanelActor(profileId, client = sql) {
  if (!Number.isInteger(Number(profileId)) || Number(profileId) <= 0) throw reviewPanelError(ACTOR_REQUIRED_MESSAGE, 403);
  const result = await client.query(`SELECT p.id, p.dynamics_systemuser_id,
    EXISTS (SELECT 1 FROM dynamics_user_roles r WHERE r.user_profile_id=p.id AND r.role='superuser') AS is_superuser
    FROM user_profiles p WHERE p.id=$1 AND p.is_active=TRUE`, [profileId]);
  if (!result.rows[0]) throw reviewPanelError(ACTOR_REQUIRED_MESSAGE, 403);
  return { profileId: result.rows[0].id, actingUserSystemId: result.rows[0].dynamics_systemuser_id || null, isSuperuser: result.rows[0].is_superuser === true };
}

/**
 * Full access check (T1): active profile AND (superuser role OR the
 * `review-panel` app grant in Dataverse). Makes a Dataverse read for
 * non-superusers, so it is called only OUTSIDE Postgres transactions — the
 * service entry points and the worker's claim-time / pre-paid-dispatch
 * checks (both run inside a DAL context). A grant lookup failure is a 503
 * marked `interrupted` so the worker pauses the pass instead of failing the
 * run, and the route returns retry copy instead of a revocation.
 */
export async function assertReviewPanelAccess(profileId, { listAppKeys = listAppKeysForUser } = {}) {
  const actor = await assertReviewPanelActor(profileId);
  if (actor.isSuperuser) return actor;
  let apps;
  try {
    apps = await listAppKeys(profileId, { throwOnError: true });
  } catch (error) {
    console.error('[review-panel] app grant lookup failed', { profileId, message: error.message });
    const wrapped = reviewPanelError('Review Panel access could not be verified. Please retry.', 503);
    wrapped.interrupted = true;
    throw wrapped;
  }
  if (!Array.isArray(apps) || !apps.includes(REVIEW_PANEL_APP_KEY)) throw reviewPanelError(ACTOR_REQUIRED_MESSAGE, 403);
  return actor;
}
...
/**
 * T3 shared visibility (docs/plans/REVIEW_PANEL_WORKBENCH_TAB_PLAN_2026-09-13.md):
 * every run that touched ONE request, across ALL owners — runs with an entry
 * row for the request, plus unsettled runs whose parked `data.pendingEntries`
 * still name it (the worker has not materialized the entry yet). Bounded to
 * the newest 20. Carries the launcher's display name for the tab's
 * "Launched by" line. Read-only; owner scoping is applied by the caller to
 * MUTATIONS only (retry/re-render/cancel stay owner-scoped in this store).
 */
export async function listReviewPanelRunsForRequest(requestId, client = sql) {
  return (await client.query(
    `SELECT r.*, COALESCE(NULLIF(p.display_name, ''), p.name) AS owner_name
     FROM review_panel_runs r LEFT JOIN user_profiles p ON p.id = r.owner_profile_id
     WHERE r.id IN (SELECT e.run_id FROM review_panel_entries e WHERE e.request_id = $1)
        OR (r.status IN ('queued','running') AND r.data->'pendingEntries' @> $2::jsonb)
     ORDER BY r.created_at DESC LIMIT 20`,
    [requestId, JSON.stringify([{ requestId }])])).rows;
}
...
/**
 * Read-only, batched (one query for every entry, never N+1): the per-seat
 * projection input for the Progress tab. Explicit columns only — never
 * prompt_snapshot_json/usage_json/result_json, which the page's projection
 * must never surface (no token usage or prompt snapshots in the API
 * response). Ordered by attempt_no ASC so a caller folding rows into a
 * `Map` keyed by (entry_id, seat_key) naturally keeps the latest attempt.
 */
export async function listEntryAttempts(entryIds, client = sql) {
  if (!Array.isArray(entryIds) || !entryIds.length) return [];
  return (await client.query(
    `SELECT entry_id, seat_key, attempt_no, state, provider, model, cost_cents, cost_state, error_text, dispatched_at, updated_at
     FROM review_panel_seat_attempts WHERE entry_id = ANY($1::uuid[]) ORDER BY entry_id, seat_key, attempt_no ASC`,
    [entryIds])).rows;
}
...
/**
 * Resolve the ONE run that owns every id in `entryIds`, scoped to `owner`, and
 * lock that run row FOR UPDATE — shared by requestReviewPanelRetry and
 * requestReviewPanelRerender so neither ever trusts a client-supplied runId.
 * (Production defect, 2026-09-13: the route used to take the CALLER's runId
 * — normally the owner's newest run — and filter entries by it; an entry
 * belonging to an older, already-settled run then matched zero rows and the
 * request was refused as "not eligible" even though the entry itself was
 * perfectly eligible. Resolving the run FROM the entries makes the owning
 * run irrelevant to which run happens to be "latest" client-side.)
 *
 * The single JOIN query below folds "do these ids exist" and "does the
 * caller own them" into one predicate (`r.owner_profile_id = $2`): a
 * nonexistent id and an id owned by someone else are indistinguishable to
 * the caller (both simply don't come back), same as every other
 * owner-scoped lookup in this file (e.g. requestReviewPanelCancel's own
 * `WHERE id=$1 AND owner_profile_id=$2`).
 */
async function resolveOwnedRunForEntries(client, owner, entryIds) {
  const rows = (await client.query(
    `SELECT e.id, e.run_id, e.status, e.winners_json, e.data
     FROM review_panel_entries e JOIN review_panel_runs r ON r.id = e.run_id
     WHERE e.id = ANY($1::uuid[]) AND r.owner_profile_id = $2`,
    [entryIds, owner])).rows;
  if (rows.length !== entryIds.length) throw reviewPanelError('One or more selected entries were not found.', 404);
  const runIds = new Set(rows.map((r) => r.run_id));
  if (runIds.size > 1) throw reviewPanelError('Selected entries must all belong to the same run.', 400);
  const runId = rows[0].run_id;
  const run = (await client.query('SELECT * FROM review_panel_runs WHERE id=$1 FOR UPDATE', [runId])).rows[0];
  if (!run) throw reviewPanelError('Run not found.', 404); // defense in depth: the row above just proved it exists
 ...
 *
 * Takes ONLY (owner, entryIds) — the owning run is resolved from the entries
 * themselves (resolveOwnedRunForEntries), never from a client-supplied runId;
 * see that helper's header for why.
 */
export async function requestReviewPanelRetry(owner, entryIds) {
  if (!Array.isArray(entryIds) || !entryIds.length) throw reviewPanelError('At least one entry id is required.', 400);
  return withReviewPanelTransaction(async client => {
    const { run } = await resolveOwnedRunForEntries(client, owner, entryIds);
    ...
    await client.query(
      ...
      [eligible.map((r) => r.id)]);
    return (await client.query(
      `UPDATE review_panel_runs SET status='queued', updated_at=NOW() WHERE id=$1 RETURNING *`, [run.id])).rows[0];
  });
}

/**
 * Operator-initiated re-render request from the route (no worker lease
 * held): re-derive the DOCX/PDF editions of an already-`completed` entry from
 * its saved seat/chair reviews, with NO model calls. Same settle fence as
 * requestReviewPanelRetry (the run must have no live lease and not be
 * queued/running/cancelled). No new hand-off column: this reuses
 * `retry_requested_at` as the route→worker marker exactly like
 * requestReviewPanelRetry does — the worker distinguishes the two cases by
 * entry status (`failed` -> retryFailedEntries re-runs seats/chair;
 * `completed` with `data.rerender` present -> re-render only, no attempts
 * created) and no migration is needed for that.
 *
 * Takes ONLY (owner, entryIds) — see resolveOwnedRunForEntries's header for
 * why a client-supplied runId is never trusted (this was the production
 * defect: the page always sent its OWN idea of "the latest run", which does
 * not have to be the run an already-completed entry belongs to).
 *
 * Eligible entries are those in `entryIds` that are currently `completed`
 * AND have a `completed` attempt in the `chair` seat — checked directly
 * against review_panel_seat_attempts, NOT via `winners_json.chair` (that
 * field is only ever populated by selectWinners, which the NORMAL
 * chair-completes-inline path in review-panel-worker.js's processEntry does
 * not call before marking the entry `completed`; see completeEntryWithReport's
 * call site there for the accompanying fix that starts recording it going
 * forward — this eligibility check must not depend on that field for
 * entries completed before or without that fix).
 *
 * Deliberately does NOT touch `data.files` — the CURRENT edition stays LIVE
 * (downloadable, and `hasReport` stays true) for the entire wait and render
 * window; only the worker's own successful swap ever replaces it, atomically,
 * once a full new DOCX+PDF pair has landed (review-panel-worker.js's
 * completeEntryWithReport, rerender mode). `data.rerender` carries only
 * `{ requestedAt, requestedBy }` — no file refs — because there is nothing to
 * preserve here: the "previous pair" is simply whatever `data.files` already
 * is, read fresh by the worker at swap/rollback time.
 */
export async function requestReviewPanelRerender(owner, entryIds) {
  if (!Array.isArray(entryIds) || !entryIds.length) throw reviewPanelError('At least one entry id is required.', 400);
  return withReviewPanelTransaction(async client => {
    const { run } = await resolveOwnedRunForEntries(client, owner, entryIds);
    await assertReviewPanelActor(owner, client);
    // Same "settled" fence as requestReviewPanelRetry, including the explicit
    // cancelled-run carve-out (an operator stop must never be resurrected by
    // a re-render either).
    if (run.lease_token || ['queued', 'running', 'cancelled'].includes(run.status)) {
      throw reviewPanelError(run.status === 'cancelled' ? 'This run was cancelled by the operator and cannot be re-rendered.' : 'Wait for this run to settle before re-rendering.');
    }
    const candidates = (await client.query(
      `SELECT id, data, winners_json FROM review_panel_entries WHERE run_id=$1 AND id = ANY($2::uuid[]) AND status='completed'`,
      [run.id, entryIds])).rows;
    if (!candidates.length) throw reviewPanelError('None of the selected entries are eligible for re-rendering.', 400);
    const chairRows = (await client.query(
      `SELECT DISTINCT entry_id FROM review_panel_seat_attempts WHERE entry_id = ANY($1::uuid[]) AND seat_key='chair' AND state='completed'`,
      [candidates.map((r) => r.id)])).rows;
    const chairCompletedIds = new Set(chairRows.map((r) => r.entry_id));
    const eligible = candidates.filter((row) => chairCompletedIds.has(row.id));
    if (!eligible.length) throw reviewPanelError('None of the selected entries are eligible for re-rendering.', 400);
    const requestedAt = new Date().toISOString();
    for (const row of eligible) {
      const nextData = { ...row.data, rerender: { requestedAt, requestedBy: owner } };
      await client.query(
        `UPDATE review_panel_entries SET retry_requested_at=NOW(), data=$2::jsonb, updated_at=NOW() WHERE id=$1`,
...
/**
 * Worker-side: the run owner lost Review Panel access mid-drain (checked by
 * assertReviewPanelAccess immediately after claim and again immediately
