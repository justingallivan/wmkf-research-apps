 ...
 *
 * state on review_panel_seat_attempts has exactly five writers: createAttempt's
 * INSERT (the initial 'pending' row), this file's markAttemptDispatched, the
 * two CAS UPDATEs inside finalizeAttempt, and reapExpiredAttempts. dispatch_token
...
/** Sum known attempt costs for a run. unknownCount > 0 means the total is withheld by the caller. A row is counted as unknown when cost_state is not 'known', OR it is 'known' with a NULL cost_cents (should be prevented by the DB CHECK, but never trusted as $0 here either). */
export async function sumAttemptCosts(runId, client = sql) {
  ...
  const isKnown = (row) => row.cost_state === 'known' && row.cost_cents != null;
  const totalCents = rows.reduce((sum, row) => sum + (isKnown(row) ? Number(row.cost_cents) : 0), 0);
  const unknownCount = rows.filter((row) => !isKnown(row)).length;
...
/** Generic run-row mutator, mirrors cycle-dossier-store's mutateDossierRun: locks the row, checks the caller's lease (if supplied), calls `fn(row, client)`, writes back status/data. */
export async function mutateReviewPanelRun(id, fn, { owner, leaseToken } = {}) {
  return withReviewPanelTransaction(async client => {
    ...
    if (leaseToken && (row.lease_token !== leaseToken || !row.locked_until || new Date(row.locked_until).getTime() <= Date.now())) {
      throw reviewPanelError('Worker lease expired.');
    }
    await fn(row, client);
    await client.query(
  ...
  return withReviewPanelTransaction(async client => {
    const row = (await client.query(
      `SELECT e.*, r.lease_token AS run_lease_token, r.locked_until AS run_locked_until
       FROM review_panel_entries e JOIN review_panel_runs r ON r.id = e.run_id WHERE e.id=$1 FOR UPDATE OF e`, [entryId])).rows[0];
    ...
    await fn(entry);
    return (await client.query(
      `UPDATE review_panel_entries SET status=$2,data=$3::jsonb,winners_json=$4::jsonb,updated_at=NOW() WHERE id=$1 RETURNING *`,
      [entryId, entry.status, JSON.stringify(entry.data), JSON.stringify(entry.winners_json)])).rows[0];
  });
}
