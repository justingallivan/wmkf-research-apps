 ...
 * AND already have a chair winner (`winners_json.chair`) — an entry can only
 * be `completed` after that ipso facto (completeEntryWithReport only marks
 * an entry `completed` once winners_json.chair exists), so this is defense
 * in depth rather than a reachable-in-practice filter.
 *
 ...
 */
export async function requestReviewPanelRerender(runId, owner, entryIds) {
  if (!Array.isArray(entryIds) || !entryIds.length) throw reviewPanelError('At least one entry id is required.', 400);
  return withReviewPanelTransaction(async client => {
    const run = (await client.query('SELECT * FROM review_panel_runs WHERE id=$1 AND owner_profile_id=$2 FOR UPDATE', [runId, owner])).rows[0];
    if (!run) throw reviewPanelError('Run not found.', 404);
    ...
    const candidates = (await client.query(
      ...
      [runId, entryIds])).rows;
    const eligible = candidates.filter((row) => row.winners_json?.chair);
