 ...
 * in depth rather than a reachable-in-practice filter. For each eligible
 * entry: `data.rerender` is set (requestedAt, requestedBy, and the CURRENT
 * `data.files` preserved as `previousFiles` — the old edition refs are never
 * deleted, only superseded), and `data.files` itself is cleared to null so
 * the worker's completeEntryWithReport sees both formats as missing and
 * re-renders them under NEW Blob pathnames.
 */
  ...
  return withReviewPanelTransaction(async client => {
    ...
    for (const row of eligible) {
      const nextData = {
        ...row.data,
        files: null,
        rerender: { requestedAt, requestedBy: owner, previousFiles: row.data?.files || null },
      };
      await client.query(
