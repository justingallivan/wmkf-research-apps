...
class MaintenanceService {
   ...
   */
  static async sweepBillOnboarding({ limit = 100, alertAfterAttempts = 5 } = {}) {
    ...
    let failedClosed = 0;
    if (pending.length === 0) {
      return { resumed, scanned: 0, errors, failedClosed };
    }
    ...
    try {
      ...
      return { resumed, scanned: pending.length, errors: pending.length, failedClosed };
    }
    ...
    for (const row of pending) {
      ...
      try {
        await bypassDynamicsRestrictions('bill-onboarding-resume', () =>
          DynamicsService.updateRecord('akoya_requests', id, body),
        );
    ...
    return { resumed, scanned: pending.length, errors, failedClosed };
  }
