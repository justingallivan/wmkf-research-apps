 ...
 * Scheduled-email retention deletes only finalized sends and explicit stops;
 * unresolved recovery rows remain ineligible.
 *
...
const reviewerSuggestionAdapter = require('../dataverse/adapters/reviewer-suggestion');
...
class MaintenanceService {
  ...
  /**
   * Bounded cleanup for reviewer_identity_shadow_log: a retention window plus
   * a hard row cap, so a runaway shadow cohort cannot grow the table without
   * bound between daily runs. The newest maxRows rows survive the cap.
   */
  static async cleanupReviewerIdentityShadowLog(retentionDays = 90, maxRows = 200000) {
    try {
      const aged = await sql`
        DELETE FROM reviewer_identity_shadow_log
        WHERE created_at < NOW() - MAKE_INTERVAL(days => ${retentionDays})
      `;
      const capped = await sql`
        ...
        WHERE id < (
          SELECT COALESCE(MIN(id), 0) FROM (
            SELECT id FROM reviewer_identity_shadow_log
            ORDER BY id DESC
            LIMIT ${maxRows}
          ) newest
        )
      `;
      return (aged.rowCount || 0) + (capped.rowCount || 0);
    } catch (error) {
      // 42P01 = undefined_table: a newly provisioned environment may run the
      // daily cron before migration 026; that ordering must not create noise.
      if (error?.code === '42P01') return 0;
      console.error('MaintenanceService.cleanupReviewerIdentityShadowLog error:', error.message);
      throw error;
    }
  }

  /** Retain only recent prospective institution observations; missing pre-rollout table is harmless. */
  static async cleanupReviewerInstitutionMeasurement(retentionDays = 90, maxRows = 200000) {
    try {
      const aged = await sql`
        DELETE FROM reviewer_institution_measurement_events
        WHERE created_at < NOW() - MAKE_INTERVAL(days => ${retentionDays})
      `;
      const capped = await sql`
        DELETE FROM reviewer_institution_measurement_events
        WHERE id < (
          SELECT COALESCE(MIN(id), 0) FROM (
            SELECT id FROM reviewer_institution_measurement_events
      ...
      if (error?.code === '42P01') return 0;
      console.error('MaintenanceService.cleanupReviewerInstitutionMeasurement error:', error.message);
      throw error;
    }
  }

  /**
   * Bounded cleanup for operational_events (migration 030). Three phases:
   *   1. Settled rows (recovered/resolved/superseded/info) past the retention
   *      window are deleted — the diagnostic value of settled events decays.
   *   2. Open rows are kept twice as long (an unresolved failure should stay
   *      visible past normal retention) but not forever.
   *   3. A hard row cap bounds a runaway drain delivery between daily runs;
   *      the newest maxRows rows survive.
   */
  static async cleanupOperationalEvents(retentionDays = 90, maxRows = 200000) {
    try {
      const settled = await sql`
        DELETE FROM operational_events
        WHERE status <> 'open'
          AND last_occurred_at < NOW() - MAKE_INTERVAL(days => ${retentionDays})
      `;
      const staleOpen = await sql`
        DELETE FROM operational_events
        WHERE status = 'open'
          AND last_occurred_at < NOW() - MAKE_INTERVAL(days => ${retentionDays * 2})
      `;
      // Cap by OPERATIONAL recency, not id: a folded/reopened app event keeps
      // its original low id while recurrence refreshes last_occurred_at in
      // place, so an id-ordered cap could delete a still-current open incident
      // (Codex adversarial finding, cycle 2, 2026-08-19). The append-only
      // shadow-log sibling above keeps its id-based cap because rows there are
      ...
      `;
      return (settled.rowCount || 0) + (staleOpen.rowCount || 0) + (capped.rowCount || 0);
    } catch (error) {
      // 42P01 = undefined_table: a newly provisioned environment may run the
      // daily cron before migration 030; that ordering must not create noise.
      if (error?.code === '42P01') return 0;
      console.error('MaintenanceService.cleanupOperationalEvents error:', error.message);
      throw error;
    }
  }
  ...
  /**
   * Delete Dynamics Explorer request lifecycle rows after the same retention
   * window as their per-tool query rows. Feedback request_id uses ON DELETE
   * SET NULL, so retained user feedback remains valid.
   */
  static async cleanupDynamicsExplorerRequests(retentionDays = 365) {
    try {
      const result = await sql`
        DELETE FROM dynamics_explorer_requests
        WHERE started_at < NOW() - MAKE_INTERVAL(days => ${retentionDays})
      `;
      return result.rowCount || 0;
    } catch (error) {
      // A newly deployed function may run before additive migration 033.
      if (error?.code === '42P01') return 0;
      console.error('MaintenanceService.cleanupDynamicsExplorerRequests error:', error.message);
      throw error;
    }
  }
  ...
  static async sweepBillOnboarding({ limit = 100, alertAfterAttempts = 5, stuckThresholdHours = 24 } = {}) {
    // Both bill modules are ESM; require() of them in the production bundle
    // yields an object without the named exports (daily-maintenance failures:
    // "listPending is not a function"). Dynamic import is the CJS→ESM seam
    // used elsewhere in lib/services (e.g. contact-enrichment/search-tiers.js).
    const onboardingState = await import('../bill/onboarding-state.js');
    const { BILLCOM_ACCOUNT_YES, BILLCOM_ACCOUNT_NO, assertOptionSetValuesConfigured } = await import('../bill/option-set-values.js');
    ...
    for (const row of pending) {
      ...
      try {
        ...
        // (pages/api/cron/maintenance.js) now establishes the trusted
        // context itself around the whole sweepBillOnboarding() call (label
        // 'bill-onboarding-resume' relocated, byte-identical).
        await grantRequestAdapter.updateById(id, body);
  ...
  static async cleanupBillOnboardingState(retentionDays = 30) {
    try {
      // ESM module — see the interop note in sweepBillOnboarding().
      const onboardingState = await import('../bill/onboarding-state.js');
  ...
  static async cleanupBlobs(retentionDays = 90, dryRun = false) {
    ...
    try {
      ...
      // S333 Stage 4b: trust-model tightening — the sole caller
      // (pages/api/cron/maintenance.js) now establishes the trusted context
      // itself around the whole cleanupBlobs() call (label
      // 'maintenance-blob-scan' relocated, byte-identical).
  ...
  /**
   * Reap only exact private Blob pathnames recorded in portal_upload_staging.
   * No prefix/folder listing is used: an unrelated private-store object can
   * never become eligible merely because it looks old.
   */
  static async cleanupPortalUploadStaging({ retentionDays = 7 } = {}) {
    const { cleanupExpiredPortalUploads } = await import('./portal-upload-staging.js');
    return cleanupExpiredPortalUploads({ retentionDays });
  }

  /** Reconcile only exact expired post-presentation MP4 intents. */
  static async cleanupPresentationMaterialUploads({ limit = 25 } = {}) {
    const { cleanupPresentationMaterialUploads } = await import(
      './post-presentation-materials/upload-intent-cleanup.js'
    );
    return cleanupPresentationMaterialUploads({ limit });
  }

  /**
   * Applicant materials auto-close (plan §16.3, PR 3): every open or ready
   * collection past `closes_at` becomes `closed`, which is what frees the
   * request's partial unique index for a later collection. Reads already
   * treat a past `closes_at` as closed, so this only makes it durable. Returns
   * the closed count; 0 when the readiness flag is off or the table is absent.
   */
  static async closeExpiredSiteVisitMaterialCollections() {
    const { isSiteVisitMaterialsSchemaReady } = await import('../utils/site-visit-materials-readiness.js');
    if (!isSiteVisitMaterialsSchemaReady()) return 0;
    const { closeExpiredCollections } = await import('./site-visit-materials/collection-store.js');
    try {
      return await closeExpiredCollections(new Date());
    } catch (error) {
      if (error?.code === '42P01') return 0;
      throw error;
    }
  ...
  /**
   * Retain review/send evidence long enough for operational audit while keeping
   * unresolved work recoverable. Only fully finalized sends and explicit stops
   * are terminal enough to prune; failed, scheduled, sending, and sent-but-not-
   * finalized rows are intentionally ineligible.
   */
  static async cleanupScheduledEmailMessages(retentionDays = 365) {
    try {
      const result = await sql`
        DELETE FROM scheduled_email_messages
        WHERE (status = 'sent'
               AND finalized_at IS NOT NULL
               AND finalized_at < NOW() - MAKE_INTERVAL(days => ${retentionDays}))
           OR (status = 'stopped'
               AND stopped_at IS NOT NULL
               AND stopped_at < NOW() - MAKE_INTERVAL(days => ${retentionDays}))
      `;
      return result.rowCount || 0;
    } catch (error) {
      // A deployment can briefly run the cron before migration 036 lands.
      if (error?.code === '42P01') return 0;
      console.error('MaintenanceService.cleanupScheduledEmailMessages error:', error.message);
      throw error;
    }
  }
  ...
  static async getRetentionConfig() {
    ...
    try {
      ...
      for (const [settingKey, settingValue] of Object.entries(rows)) {
        ...
        // keys in `defaults`. The hasOwnProperty check below is the canonical
        // key gate, so the rule applies to every configured retention key
        // without a hardcoded enumeration that can drift.
