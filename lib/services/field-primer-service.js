...
export async function groundPrimerExperts(experts, { signal } = {}) {
  ...
  const confirm = (e, r) => ({ ...e, grounding: { status: 'confirmed', resolvedName: r.displayName, openAlexId: r.openAlexId, orcid: r.orcid, worksCount: r.worksCount, institution: r.lastKnownInstitution || null, hIndex: Number.isFinite(r.hIndex) ? r.hIndex : null, citedByCount: Number.isFinite(r.citedByCount) ? r.citedByCount : null, wikipedia: r.wikipedia || null, affiliationMatch: affiliationMatches(e?.affiliation, r.lastKnownInstitution) || undefined } });
