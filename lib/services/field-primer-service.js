 ...
 *        overlapping the anchor; if one DOMINATES (only one, or ≥3× runner-up) AND clears
 *        the higher CORRECT_MIN_WORKS floor AND its forename contradicts the model's →
 *        `corrected` (SUGGESTED, `needsVerification:true` — surname+field+dominance is
 *        strong but NOT identity proof; a dominant in-field namesake is still possible);
 *        forename agrees → `confirmed`; else → `unverified`.
 *      * no exact match + no anchor → `unverified` (nothing to disambiguate against).
 *  We NEVER silently bind a namesake; a correction is always surfaced as "verify".
 *
...
export async function groundPrimerExperts(experts, { signal } = {}) {
  ...
  const confirm = (e, r) => ({ ...e, grounding: { status: 'confirmed', resolvedName: r.displayName, openAlexId: r.openAlexId, orcid: r.orcid, worksCount: r.worksCount } });
  ...
  for (const p of prelim) {
    ...
    if (inField.length === 0) {
      ...
      if (dominant && Number(top.worksCount) >= CORRECT_MIN_WORKS && forenamesContradict(name, top.displayName)) {
        // SUGGESTED forename correction — surfaced for verification, never trusted blindly:
        // surname + field + works-dominance is strong but not identity proof.
        out.push({ ...e, grounding: { status: 'corrected', resolvedName: top.displayName, openAlexId: top.openAlexId, orcid: top.orcid, worksCount: top.worksCount, needsVerification: true, note: `suggested forename correction — matched by surname + field (model named "${name}"); verify it is the same person` } });
      } else if (dominant && !forenamesContradict(name, top.displayName)) {
...
export function renderPrimerMarkdown(primer = {}, meta = {}) {
  ...
  if (Array.isArray(primer.experts) && primer.experts.length) {
    ...
    for (const i of primer.experts) {
      ...
      else if (g?.status === 'corrected') { name = `likely ${g.resolvedName || name}`; mark = ` ✎ _(suggested — model named "${i.name}"; verify same person)_`; }
