 ...
 * @param {number|null} [args.timeoutMs=null] - Server-owned transport timeout for
 *   this call (from the `field-primer.generate` Executor budget). Null keeps the
 *   LLM client's default.
 * @param {number|null} [args.deadlineMs=null] - Absolute epoch-ms bound for the
 *   provider call (the caller's lease deadline minus its post-call reserve); the
 *   Executor clamps the timeout to it immediately before calling the model.
 ...
 */
export async function generateFieldPrimer({ proposalText, focus, runSource = 'Vercel User', timeoutMs = null, deadlineMs = null } = {}) {
  ...
  const result = await executePrompt({
    ...
    forceOverwrite: true,
    ...(Number.isInteger(timeoutMs) && timeoutMs > 0 ? { timeoutMsOverride: timeoutMs } : {}),
