...
// then surname) fans out across experts concurrently, but capped so we stay
// inside OpenAlex's polite-pool budget (~10 req/s with a mailto set) even when the
// model returns an unusually long experts list. searchAuthors already retries on
// 429/5xx, so a brief overlap is self-healing.
