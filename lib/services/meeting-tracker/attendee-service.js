 ...
 * generic 409. A directory row that exists but has no email is a data gap
 * the PC can fix (a Board member's preferred email lives on the Expertise
 * Finder roster), so name the person and the remedy before the generic
 * resolver runs. Refs that are not in the directory at all fall through.
 */
export function assertAttendeeEmailsOnFile(attendeeRefs, directory) {
  ...
  for (const ref of attendeeRefs || []) {
    ...
    const remedy = ref.kind === 'roster'
      ? 'Add a preferred email for them on the Expertise Finder roster, then try again.'
