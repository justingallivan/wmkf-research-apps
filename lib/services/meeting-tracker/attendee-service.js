...
/**
 * The shared directory resolver rejects any row without an email with a
 * generic 409. A directory row that exists but has no email is a data gap the
 * PC can fix. Linked rows depend exclusively on their Dataverse Contact;
 * unlinked rows depend on the Expertise Finder preferred email. Name the
 * person and the correct remedy before the generic resolver runs. Refs that
 * are not in the directory at all fall through.
 */
export function assertAttendeeEmailsOnFile(attendeeRefs, directory) {
  ...
  for (const ref of attendeeRefs || []) {
    const row = ref.kind === 'staff' ? staff.get(String(ref.profileId)) : roster.get(String(ref.rosterId));
    if (!row || row.email) continue;
    const name = String(row.name || '').trim() || (ref.kind === 'staff' ? `staff profile ${ref.profileId}` : `roster entry ${ref.rosterId}`);
    const remedy = ref.kind === 'roster'
      ? (row.linked
        ? 'Their linked Dataverse contact is inactive, missing, or has no email; fix or relink the contact, or unlink the roster row.'
        : 'Add a preferred email for them on the Expertise Finder roster.')
      : 'Their staff profile has no email on file.';
    throw attendeeError(
      `${name} has no email on file and cannot be added as an attendee. ${remedy}`,
      'meeting_tracker_attendee_email_missing',
      409,
    );
  }
}
...
export async function resolveMeetingAttendeeRefs(
  ...
  const resolvedDirectory = directory || await dependencies.getRecipientDirectory();
  assertAttendeeEmailsOnFile(normalized.attendees, resolvedDirectory);
