...
const DEFAULT_DEPENDENCIES = Object.freeze({
  ...
  getLatestAgenda: store.getLatestAgendaSend,
...
export function defaultAgendaSubject(session) {
  const { start, timeZone } = requireSessionTime(session);
...
function scheduleSignature(session, slots) {
  return {
    scheduledStartIso: String(session?.scheduledStartIso || ''),
...
export function agendaScheduleChanged(session, slots, snapshot) {
  ...
  const stored = {
    scheduledStartIso: String(snapshot.scheduledStartIso || ''),
...
function exactPreparedRow(row, input) {
  return sameId(row.session_id, input.sessionId)
    && JSON.stringify(parseStoredObject(row.agenda_snapshot)) === JSON.stringify(input.agendaSnapshot)
...
export async function getAgendaStatus({ sessionId }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const row = await dependencies.getLatestAgenda(sessionId);
  const snapshot = parseStoredObject(row?.agenda_snapshot);
  return {
    lastAgenda: projectAgenda(row),
    scheduleChanged: row ? agendaScheduleChanged(current.session, current.slots, snapshot) : false,
  };
}
...
export async function sendAgendaEmail(input, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  if (existing.from_email !== fromEmail
  ...
  try {
    ...
    if (attempt.send_requested_at && attempt.dynamics_email_id) {
      ...
      throw agendaError(
        'Dynamics has not confirmed transport acceptance for this agenda. Check the last agenda before trying again.',
    ...
    if (!SEND_ACCEPTED_STATUS_CODES.has(Number(after?.statuscode))) {
      throw agendaError(
        'Dynamics accepted the send request, but its transport status is not confirmed yet. Check the last agenda before trying again.',
        'agenda_send_unconfirmed',
        202,
      );
    }
