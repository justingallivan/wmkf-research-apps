 ...
 * time zone. Returns the template unchanged when the token is absent, or
 * when the session's time cannot be resolved (falls back to the raw
 * template rather than throwing).
 */
export function resolveAgendaDefault(template, session) {
  ...
  let timeZone;
  try {
    ({ start, timeZone } = requireSessionTime(session));
  } catch {
    return text;
  }
...
async function readAgendaDefault(key, session, dependencies) {
  try {
    const result = await dependencies.getSettingStrict(key);
    const value = result?.found ? String(result.value ?? '') : '';
    if (value.trim() === '') return { value: '', unavailable: false };
    return { value: resolveAgendaDefault(value, session), unavailable: false };
  } catch (error) {
...
export async function getAgendaStatus({ sessionId }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const [current, sentRow, pendingRow] = await Promise.all([
  ...
  const [subjectDefault, bodyDefault] = await Promise.all([
    readAgendaDefault(AGENDA_SUBJECT_DEFAULT_KEY, current.session, dependencies),
    readAgendaDefault(AGENDA_BODY_DEFAULT_KEY, current.session, dependencies),
  ]);
  return {
    ...
    defaults: {
      subject: subjectDefault.value,
      message: bodyDefault.value,
      unavailable: subjectDefault.unavailable || bodyDefault.unavailable,
    },
  };
}
