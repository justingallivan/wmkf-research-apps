...
import { normalizeDistributionRecipients } from '../pre-site-visit/distribution/composition.js';
...
function sessionWhenText(session, start, end, timeZone) {
  ...
  return `Pre-discussion: ${startText}–${endText}.`;
}
...
export function computeAgenda(session, slots, _now = new Date()) {
  ...
  const agendaSlots = orderedSlots(slots).map((slot) => {
    ...
    const institution = String(slot?.institution || '').trim() || null;
...
function agendaSlotText(slot) {
  ...
  const institution = slot.institution ? ` · ${slot.institution}` : '';
  return `${slot.windowText} · #${slot.requestNumber} · ${slot.title}${institution} · Lead PD: ${slot.leadPdName} · ${briefing}`;
}
...
export function renderAgendaEmail(message, snapshot) {
  ...
  // Blank line between agenda items: a single-line run of seven fields is hard to scan.
  ...
  const bodyHtml = [
    ...
    ...snapshot.slots.map((slot) => {
      ...
      const institution = slot.institution ? ` · ${escapeHtml(slot.institution)}` : '';
      return `<li style="margin-bottom:12px"><strong>${escapeHtml(slot.windowText)}</strong> · #${escapeHtml(slot.requestNumber)}`
        + ` · ${escapeHtml(slot.title)}${institution} · Lead PD: ${escapeHtml(slot.leadPdName)} · ${briefing}</li>`;
    }),
    '</ol>',
  ].join('');
  ...
  try {
    ...
    try {
      ...
      if (!SEND_ACCEPTED_STATUS_CODES.has(Number(ambiguous?.statuscode))) {
        throw agendaError(
          'Dynamics has not confirmed this send. Check this exact agenda before trying again.',
          'agenda_send_unconfirmed',
          202,
          { outcome: 'uncertain', pendingSend: projectAgenda(attempt) },
        );
      }
    }
