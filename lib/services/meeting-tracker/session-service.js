...
async function projectSession(row, dependencies, { attendees = null } = {}) {
  if (!row) return null;
  const refs = parseMeetingAttendeeRefs(row.wmkf_attendeerefsjson);
  return {
    ...
    attendeeRefs: refs.attendees,
    attendees: attendees || await dependencies.resolveAttendees(refs),
...
async function normalizedCreate(input, dependencies) {
...
export async function updateDeliberationSession(
  ...
  const currentRefs = parseMeetingAttendeeRefs(current.wmkf_attendeerefsjson);
  const normalized = await normalizedCreate({
    ...
    attendees: input.attendees ?? currentRefs.attendees,
  }, dependencies);
  try {
...
export async function listDeliberationSessions(dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const sessions = [];
  for (const row of result?.records || []) sessions.push(await projectSession(row, dependencies));
  return { sessions };
}
