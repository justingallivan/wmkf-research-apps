...
const DEFAULT_DEPENDENCIES = Object.freeze({
  ...
  getMaterialsSummariesWithAvailability: getMaterialsSummaryByRequestsWithAvailability,
});
...
export async function loadMeetingTrackerDashboard(
  ...
  const documentIds = requests
    ...
    .filter(Boolean);
  const [documents, schedules, activeVisits, materialsResult] = await Promise.all([
    ...
    // Applicant materials cue on the list row (plan §16.3, PR 3): fail-open.
    dependencies.getMaterialsSummariesWithAvailability({
      requestIds,
      requestNumbers: new Map((base.proposals || []).map((row) => [row.requestId, row.requestNumber])),
      cycleCode: args.cycleCode,
    }).then((value) => {
      if (value?.summaries instanceof Map && value?.availability instanceof Map) {
        const complete = requestIds.every((id) => value.summaries.has(id) && value.availability.has(id));
        if (complete) return value;
        return { summaries: new Map(), availability: new Map(requestIds.map((id) => [id, 'unavailable'])) };
      }
      return { summaries: new Map(), availability: new Map(requestIds.map((id) => [id, 'unavailable'])) };
    }).catch(() => ({
      summaries: new Map(),
      availability: new Map(requestIds.map((id) => [id, 'unavailable'])),
    })),
  ]);
  const materialsByRequest = materialsResult.summaries;
  const materialsAvailabilityByRequest = materialsResult.availability;
  ...
  const proposals = (base.proposals || []).map((proposal) => {
    ...
    return {
      ...
      ...(Object.prototype.hasOwnProperty.call(proposal, 'isTestRequest')
        ? { isTestRequest: proposal.isTestRequest }
        : {}),
      ...
      deliberation,
      siteVisit,
      materials: materialsByRequest.get(proposal.requestId) || null,
      materialsAvailability: materialsAvailabilityByRequest.get(proposal.requestId) || 'unavailable',
