...
export async function drainReviewSynthesisJobs({
  ...
  // was on is cancelled before any provider call or request write.
  ...
  for (const job of jobs) {
    try {
      ...
      if (testState.kind === 'unknown') {
        throw new Error(`could not confirm request ${job.request_id} is an ordinary request (${testState.reason})`);
      }
