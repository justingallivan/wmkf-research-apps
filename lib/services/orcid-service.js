...
class ORCIDService {
  ...
  static async searchByName({ name, affiliation, clientId, clientSecret, maxResults = 10, signal }) {
    ...
    // Strip honorifics before splitting: an unstripped "Prof. Ursula Keller" makes
    // firstName="Prof." → `given-names:Prof.* AND family-name:Keller*`, which matches
    // the wrong person or nothing. (The honorific strip in findContact's name-match
    // gate runs too late — it doesn't touch the SOLR query built here.)
    const nameParts = ContactParser.stripHonorifics((name || '').trim())
      .split(/\s+/).filter((p) => /[\p{L}\p{N}]/u.test(p));
  ...
  static async getProfile(orcidId, clientId, clientSecret, opts = {}) {
    ...
    for (const group of employments) {
      ...
      for (const summary of summaries) {
        ...
        if (emp) {
          const disambiguated = emp.organization?.['disambiguated-organization'] || null;
          affiliations.push({
            organization: emp.organization?.name || '',
            disambiguatedOrganizationId: disambiguated?.['disambiguated-organization-identifier'] || null,
            disambiguationSource: disambiguated?.['disambiguation-source'] || null,
    ...
    return {
      ...
      currentAffiliationDisambiguatedOrganizationId: affiliations.find(a => a.current)?.disambiguatedOrganizationId || null,
    };
  }

  /**
   * Fetch a researcher's SELF-ASSERTED works list (titles) from their ORCID record.
   *
   * This is the merge-immune corpus per `project-openalex-merge-use-orcid-works`: an
   * OpenAlex author *cluster* can be a merge of same-name people, but the ORCID record's
   * own works are PI-curated for that exact identity. Used by the identity-evidence rescue
   * path to corroborate (independently of the OpenAlex author cluster) that a candidate
   * works on the proposal's topic. Titles only — enough for a topic-overlap check; we do
   * not resolve DOIs here.
   *
   * Returns `[]` (never throws for a missing record) on 404 so the caller fails safe.
   *
   * @param {string} orcidId - bare ORCID iD (e.g., "0000-0002-1234-5678")
   * @param {string} clientId
   * @param {string} clientSecret
   * @param {{ signal?: AbortSignal, limit?: number }} [opts]
   * @returns {Promise<string[]>} work titles (de-duped, non-empty), newest-first as ORCID returns them
   */
  static async getWorks(orcidId, clientId, clientSecret, opts = {}) {
    ...
    if (!response.ok) {
      ...
      const error = await response.text();
      const err = new Error(`ORCID works fetch failed: ${error}`);
      err.status = response.status;
      throw err;
    }
    ...
    for (const group of groups) {
      ...
      // A group is one work with possibly several source summaries; the first summary may
      // lack a title while a later one carries it — scan all so the veto isn't undercounted.
      const title = summaries.map((s) => s?.title?.title?.value).find((t) => t && typeof t === 'string');
  ...
  /**
   * Return bounded ORCID work identifiers for exact source-work binding.
   * Unlike getWorks(), this preserves DOI/PMID identifiers and is intended for
   * identity proof, not topic/title comparison.
   *
   * @returns {Promise<{totalCount: number, examinedCount: number, records: Array<{title: string|null, doi: string|null, pmid: string|null}>}>}
   */
  static async getWorkReferences(orcidId, clientId, clientSecret, opts = {}) {
    const limit = Number.isFinite(opts?.limit)
      ? Math.max(1, Math.min(Math.floor(opts.limit), 100))
      : 50;
    const token = opts?.signal
      ? await this.getAccessToken(clientId, clientSecret, { signal: opts.signal })
      : await this.getAccessToken(clientId, clientSecret);
    ...
    if (!response.ok) {
      if (response.status === 404) return { totalCount: 0, examinedCount: 0, records: [] };
      const error = await response.text();
      const err = new Error(`ORCID works fetch failed: ${error}`);
      err.status = response.status;
      throw err;
    ...
    const data = await response.json();
    const groups = Array.isArray(data?.group) ? data.group : [];
    const examinedGroups = groups.slice(0, limit);
    const references = [];
    const seen = new Set();
    const scalar = (value) => {
    ...
    for (const group of examinedGroups) {
      const summaries = Array.isArray(group?.['work-summary']) ? group['work-summary'] : [];
      for (const summary of summaries) {
        const title = scalar(summary?.title?.title);
        const identifiers = Array.isArray(summary?.['external-ids']?.['external-id'])
          ? summary['external-ids']['external-id']
          : [];
        let doi = null;
        let pmid = null;
        for (const identifier of identifiers) {
          const type = scalar(identifier?.['external-id-type'])?.toLowerCase();
          const value = scalar(identifier?.['external-id-value']);
          const relationship = scalar(identifier?.['external-id-relationship'])?.toLowerCase();
          if (relationship && relationship !== 'self') continue;
          if (type === 'doi' && value) doi = value;
          if ((type === 'pmid' || type === 'pubmed') && value) pmid = value;
        }
        if (!doi && !pmid) continue;
        const key = doi ? `doi:${doi.toLowerCase()}` : `pmid:${pmid.toLowerCase()}`;
        if (seen.has(key)) continue;
        seen.add(key);
        references.push({ title, doi, pmid });
      }
    }
    return {
      totalCount: groups.length,
      examinedCount: examinedGroups.length,
      records: references,
    };
  }
  ...
  static async findContact({ name, affiliation, clientId, clientSecret, signal, throwOnError = false, strictAmbiguity = false }) {
    try {
      ...
      if (distinctIds.size > 1) {
        // The "single contactable record breaks the tie" heuristic below is
        // load-bearing for the identity spine (contact-enrichment + identity-
        // evidence), where a downstream forename/email gate re-checks the pick.
        // The Workbench manual-lookup path has NO such downstream gate and never
        // passes the staff-entered email in here, so a lone public email would
        // attach a namesake purely for being contactable. strictAmbiguity disables
        // the tie-break for that path → abstain rather than guess. Affiliation
        // corroboration (narrowing above) still resolves when it can.
        const withEmail = pool.filter((r) => r.emails && r.emails.length > 0);
        if (!strictAmbiguity && withEmail.length === 1) {
