...
const MALFORMED_BOOLEAN_FIELDS = Object.freeze([
  'wmkf_invited',
  'wmkf_accepted',
  'wmkf_declined',
...
// Every `reason` string classifyParticipant can emit for a `resolved: false`
// (blocker) classification — i.e. every reason evaluateReviewSynthesisReadiness
// can put into a blocker entry. Kept exported so downstream allowlist logic
// (e.g. composeRefereeSection's outstanding-review sentence) can be tested
// against the live, current set of reasons rather than a hand-copied list
// that silently drifts when a new reason is added here.
export const REVIEW_SYNTHESIS_BLOCKER_REASONS = Object.freeze([
  'unknown_response_type',
  'unknown_review_status',
  ...MALFORMED_BOOLEAN_FIELDS.map((field) => `malformed_${field}`),
...
function classifyParticipant(row, nowMillis) {
  ...
  for (const field of MALFORMED_BOOLEAN_FIELDS) {
