...
export const CURATED_RECIPIENT_MAX_ENTRIES = 50;
...
const DEFAULT_DEPENDENCIES = {
  ...
  getContactById: contactAdapter.getById,
  searchContactsByName: contactAdapter.searchByName,
...
export async function resolveCuratedRecipientConfig(
  ...
  const [staff, contactRows] = await Promise.all([
    ...
    Promise.all(contactEntries.map(async (entry) => {
      try {
        return await dependencies.getContactById(entry.contactId);
      } catch (error) {
  ...
  const contactById = new Map(contactEntries.map((entry, index) => [entry.contactId, contactRows[index]]));
...
export async function getCuratedRecipientOptions(dependencies = DEFAULT_DEPENDENCIES) {
  ...
  return sortResolved(entries.filter((entry) => entry.available).map((entry) => ({
    key: entry.key,
