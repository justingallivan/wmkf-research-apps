...
export const CURATED_RECIPIENT_SEARCH_LIMIT = CURATED_RECIPIENT_MAX_ENTRIES;
const CURATED_RECIPIENT_SEARCH_PROBE_LIMIT = CURATED_RECIPIENT_SEARCH_LIMIT + 1;
...
/**
 * Search existing Dataverse Contacts with one bounded read. The extra probe row
 * lets the caller distinguish a complete result from the capped first 50.
 */
export async function searchCuratedRecipientContacts(query, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const rows = search.includes('@')
    ...
    : await dependencies.searchContactsByName(search, { top: CURATED_RECIPIENT_SEARCH_PROBE_LIMIT });
  const seen = new Set();
  const contacts = [];
  ...
  return {
    contacts: contacts.slice(0, CURATED_RECIPIENT_SEARCH_LIMIT),
    truncated: (rows || []).length > CURATED_RECIPIENT_SEARCH_LIMIT,
