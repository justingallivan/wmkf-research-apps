 ...
 * system users. External roster identity stays expertise_roster.id. Linked
 * roster rows resolve email from Dataverse; unlinked rows retain their
 * maintained preferred address. Email is never inferred from a name.
 */
...
const DEFAULT_DEPENDENCIES = {
  ...
  async listRoster() {
    return (await sql`
      SELECT id, name, role_type, role, affiliation, preferred_email, dataverse_contact_id
  ...
  async listSystemUsers() {
    return systemUserAdapter.queryAllUsers({
  ...
  async getContactsByIds(ids) {
    return contactAdapter.getByIds(ids);
  },
};
...
export async function getActiveStaffRecipientDirectory(dependencies = DEFAULT_DEPENDENCIES) {
  ...
  if (userResult?.capped) {
    throw new ServiceHttpError('The enabled Dataverse user directory exceeds the supported read limit.', {
      httpStatus: 503,
...
export async function getSiteVisitRecipientDirectory(dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const linkedIds = [...new Set((roster || [])
    .map((row) => String(row.dataverse_contact_id || '').trim().toLowerCase())
    .filter(Boolean))];
  const contactChunks = chunksOf(linkedIds, contactAdapter.CONTACT_BATCH_MAX_IDS);
  const contactResults = await Promise.all(
    contactChunks.map((ids) => dependencies.getContactsByIds(ids)),
  );
  const contactsById = new Map(contactResults.flat().map((row) => [
    String(row?.contactid || '').toLowerCase(),
    row,
  ]));
  const external = (roster || []).map((row) => {
    const contactId = String(row.dataverse_contact_id || '').trim().toLowerCase();
    const linked = Boolean(contactId);
    const contact = linked ? contactsById.get(contactId) : null;
    const contactActive = contact
      && (contact.statecode === undefined || contact.statecode === 0);
    return {
      kind: 'roster',
      rosterId: Number(row.id),
      name: String(row.name || ''),
      email: linked
        ? (contactActive ? normalizeSiteVisitEmail(contact.emailaddress1) : null)
        : normalizeSiteVisitEmail(row.preferred_email),
      linked,
