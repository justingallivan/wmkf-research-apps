 ...
 * profile IDs, expertise_roster IDs, and manual recipients. Writes are
 * request-gated (the request must be an advancing request in a cycle with a
 * meeting date, per `isVisibleRequestRow`; owner decision 2026-09-09 — a
 * Program Coordinator schedules the visit weeks before the draft is shared)
 * and request-bound. Ordinary field edits use an ETag-fenced PATCH;
...
import { applicantAttendeeSuggestions, resolveSiteVisitApplicantContacts } from './applicant-contacts.js';
import {
...
const DEFAULT_DEPENDENCIES = Object.freeze({
  ...
  resolveApplicantContacts: (requestId) => resolveSiteVisitApplicantContacts({ requestId }),
...
async function assertSchedulableRequest(requestId, dependencies) {
  const request = await dependencies.getRequest(requestId);
  if (!request?.akoya_requestid) {
    throw logisticsError('Request not found.', 'site_visit_logistics_request_not_found', 404);
  }
  if (!request.wmkf_meetingdate || !isVisibleRequestRow(request, false)) {
    throw logisticsError(
      'Site Visit logistics can be edited only for an advancing request in a cycle.',
      'site_visit_request_not_schedulable',
    );
  }
  return request;
}
...
const PARTY_NAVIGATION_PROPERTY = 'wmkf_SiteVisit_activity_parties';

/**
 * Read-projection fallback for visits scheduled directly in Dynamics (S466):
 * such records never carry the app-owned `wmkf_attendeerefsjson` map, but the
 * expanded ActivityParty rows still name every attendee by email. Deriving
 * manual refs from the parties keeps the read/calendar contract working now
 * that the in-app logistics editor (the map's only producer) is gone. The
 * strict map contract still applies whenever the map is present.
 */
function refsFromParties(row) {
  const parties = Array.isArray(row?.[PARTY_NAVIGATION_PROPERTY])
    ? row[PARTY_NAVIGATION_PROPERTY]
    : null;
  if (!parties || parties.length === 0) return null;
  const byMask = (mask) => parties
    .filter((party) => party?.participationtypemask === mask)
    .map((party) => normalizeSiteVisitEmail(party.addressused))
    .filter(Boolean)
    .map((email) => ({ kind: 'manual', email }));
  const organizers = byMask(SITE_VISIT_PARTICIPATION_MASK.ORGANIZER);
  return {
    organizer: organizers[0] || null,
    requiredAttendees: byMask(SITE_VISIT_PARTICIPATION_MASK.REQUIRED),
    optionalAttendees: byMask(SITE_VISIT_PARTICIPATION_MASK.OPTIONAL),
  };
}
...
function parseRefs(row) {
  try {
    ...
    if (!row?.wmkf_attendeerefsjson) {
      const fallback = refsFromParties(row);
...
// Meeting Tracker opts into applicant suggestions; the Workbench read keeps its existing shape and cost.
export async function getSiteVisitLogistics({ requestId, includeApplicantAttendees = false }, dependencies = DEFAULT_DEPENDENCIES) {
  if (!isGuid(requestId)) {
    throw logisticsError('A valid requestId is required.', 'invalid_request_id', 400);
  }
  assertSchemaReady(dependencies);
  // Read is not gated on schedulability: a visit recorded while the request
  // was advancing stays readable (calendar invite, materials) even if triage
  // later changes. Only writes require an advancing request (owner 2026-09-09).
  const [status, row, documents, applicants] = await Promise.all([
    dependencies.getArtifactStatus({ requestId }),
    findOneActive(requestId, dependencies),
    dependencies.findDocumentsByRequest(requestId),
    includeApplicantAttendees
      ? Promise.resolve().then(() => dependencies.resolveApplicantContacts(requestId))
        .then(applicantAttendeeSuggestions).catch(() => null)
      : null,
  ]);
  ...
  const presentation = presentationEnabled
    ? projectPostPresentationDescriptors(documents?.records || [], requestId, { includeStaffUrls: true })
    : null;
  return {
    siteVisit: projectSiteVisit(row),
    materials: projectMaterials(documents?.records, requestId, status?.currentArtifact?.artifactId),
    presentationMaterialsStatus: presentationEnabled ? 'ready' : 'disabled',
    ...(presentationEnabled ? {
      presentationMaterials: presentation.materials,
      presentationMaterialConflicts: presentation.conflicts,
    } : {}),
    ...(includeApplicantAttendees ? {
      applicantAttendees: applicants || [],
      applicantAttendeesUnavailable: applicants === null,
    } : {}),
  };
}
...
export async function saveSiteVisitLogistics(
  ...
  assertSchemaReady(dependencies);
  await assertSchedulableRequest(requestId, dependencies);
