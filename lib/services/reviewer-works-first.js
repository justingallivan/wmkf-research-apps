...
function buildEvidenceBundle(entries, selectedOrcid, claimedInstitutionRor = null) {
  ...
  return {
    ...
    anchorDois,
    rors: Array.from(rors).sort(),
    openAlexAuthorIds: samePerson.map((entry) => entry.authorId).sort(),
  };
}
...
async function resolveWorksFirst(candidate, {
  ...
  if (plausible.length >= highFragmentationCandidateCount) {
    return {
      ...
      candidateCount: plausible.length,
      evidenceBundle: buildEvidenceBundle(institutionMatched, anchor.slice('orcid:'.length), claimedInstitutionRor),
      candidates: plausible.map((entry) => publicCandidate(entry, claimedInstitutionId)),
    };
  }
  ...
  return {
    ...
    evidenceBundle: buildEvidenceBundle(institutionMatched, selectedOrcid, claimedInstitutionRor),
    candidates: plausible.map((entry) => publicCandidate(entry, claimedInstitutionId)),
  };
}
