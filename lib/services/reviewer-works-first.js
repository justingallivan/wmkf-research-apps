/**
 * Shared works-first reviewer identity resolver and offline evaluation policy.
 *
 * Runtime callers use only `resolveWorksFirst` and `combineIdentityDecisions`.
 * The frozen W2 evaluator additionally uses the scoring/promotion exports.
 * Distinct ORCID clusters always return review; benchmark adjudication is kept
 * outside this module and cannot influence resolver decisions.
 */

const {
  generateNameVariants,
  nameMatchEvidence,
} = require('./discovery/name-matching');

const HIGH_FRAGMENTATION_CANDIDATE_COUNT = 10;
const MAX_EVIDENCE_OPENALEX_AUTHORS = 8;
const MAX_EVIDENCE_RORS = 6;
...
function createAnchorMatcher({
  ...
  async function canonical(anchor) {
    ...
    if (anchor.startsWith('orcid:')) {
      ...
      if (authorId && typeof getAuthorById === 'function') {
        try {
...
function normalizeAuthorship(authorship = {}) {
  const raw = authorship.raw || authorship;
  return {
    rawName: raw.raw_author_name
      || authorship.displayName
      || raw.author?.display_name
      || null,
    authorId: authorship.openAlexAuthorId || raw.author?.id || null,
    orcid: authorship.orcid || raw.author?.orcid || null,
    displayName: authorship.displayName || raw.author?.display_name || null,
...
function collectBylineCandidates(candidateName, works = []) {
  ...
  for (const work of works) {
    for (const authorship of Array.isArray(work?.authorships) ? work.authorships : []) {
      const normalized = normalizeAuthorship(authorship);
      if (!nameConsistent(candidateName, normalized.rawName)) continue;
      const authorId = shortOpenAlexAuthorId(normalized.authorId);
      if (!authorId) continue;
      const entry = byAuthorId.get(authorId) || createCandidate(authorId);
      entry.bylineCount += 1;
      const doi = normalizeDoi(work?.doi || work?.url);
      if (doi) entry.anchorDois.add(doi);
      const orcid = normalizeOrcid(normalized.orcid);
      if (orcid) entry.orcids.add(orcid);
      if (normalized.displayName) {
        entry.displayNames.add(normalized.displayName);
      }
      for (const institution of normalized.institutions) {
        ...
        if (ror) entry.institutionRors.add(ror);
      }
...
function publicCandidate(entry, claimedInstitutionId) {
  return {
    ...
    anchorDois: Array.from(entry.anchorDois).slice(0, 5),
    ...
    institutionRors: Array.from(entry.institutionRors).sort(),
...
function buildEvidenceBundle(
  entries,
  selectedOrcid,
  claimedInstitutionRor = null,
  selectedAuthorId = null,
) {
  const samePerson = entries.filter((entry) => entry.orcids.has(selectedOrcid));
  const anchorDois = [];
  const seenDois = new Set();
  for (const entry of samePerson) {
    for (const doi of entry.anchorDois) {
      if (seenDois.has(doi)) continue;
      seenDois.add(doi);
      anchorDois.push(doi);
      if (anchorDois.length === 5) break;
    }
    if (anchorDois.length === 5) break;
  }
  const rors = new Set(
    samePerson.flatMap((entry) => Array.from(entry.institutionRors)),
  );
  if (claimedInstitutionRor) rors.add(claimedInstitutionRor);
  const authorIds = samePerson.map((entry) => entry.authorId).sort();
  if (selectedAuthorId) {
    const selectedIndex = authorIds.indexOf(selectedAuthorId);
    if (selectedIndex > 0) {
      authorIds.splice(selectedIndex, 1);
      authorIds.unshift(selectedAuthorId);
    }
  }
  return {
    orcids: [selectedOrcid],
    anchorDois,
    rors: Array.from(rors).sort().slice(0, MAX_EVIDENCE_RORS),
    openAlexAuthorIds: authorIds.slice(0, MAX_EVIDENCE_OPENALEX_AUTHORS),
  };
}

/**
 * An OpenAlex author fragment can agree with a works-first ORCID result even
 * when that fragment's author profile is sparse and omits the ORCID. Require
 * the exact author id to appear in the matched byline evidence and require that
 * fragment to carry exactly the selected ORCID. Missing/conflicting evidence
 * fails closed.
 */
function worksEvidenceLinksAnchor(legacyAnchor, worksResult) {
  const legacyAuthorId = String(legacyAnchor || '').startsWith('openalex:')
    ? shortOpenAlexAuthorId(String(legacyAnchor).slice('openalex:'.length))
    : null;
  const selectedOrcid = String(worksResult?.anchor || '').startsWith('orcid:')
    ? normalizeOrcid(String(worksResult.anchor).slice('orcid:'.length))
    : null;
  if (!legacyAuthorId || !selectedOrcid || worksResult?.decision !== 'bind') return false;
  const exactFragment = (worksResult.candidates || []).find(
    (candidate) => shortOpenAlexAuthorId(candidate?.authorId) === legacyAuthorId,
  );
  const fragmentOrcids = Array.from(new Set(
    (exactFragment?.orcids || []).map(normalizeOrcid).filter(Boolean),
  ));
  return fragmentOrcids.length === 1 && fragmentOrcids[0] === selectedOrcid;
}
...
async function resolveWorksFirst(candidate, {
  ...
  let claimedInstitutionRor = null;
  if (candidate?.claimedAffiliation) {
    ...
    claimedInstitutionRor = normalizeRor(institutions?.[0]?.ror);
  }
  ...
  if (plausible.length >= highFragmentationCandidateCount) {
    return {
      ...
      evidenceBundle: buildEvidenceBundle(
        institutionMatched,
        anchor.slice('orcid:'.length),
        claimedInstitutionRor,
        selected.authorId,
      ),
      candidates: plausible.map((entry) => publicCandidate(entry, claimedInstitutionId)),
    };
  ...
  const selectedOrcid = anchor.slice('orcid:'.length);
  return {
    decision: 'bind',
    anchor,
    ...
    candidateCount: plausible.length,
    observedOrcids,
    selectedCandidate: publicCandidate(selected, claimedInstitutionId),
    evidenceBundle: buildEvidenceBundle(
      institutionMatched,
      selectedOrcid,
      claimedInstitutionRor,
...
function evaluatePromotion(rows, {
  ...
  const providerFailures = rows.filter((row) =>
    ['spine', 'works'].some((arm) => {
      const reason = String(row?.[arm]?.reason || '');
      return Boolean(row?.[arm]?.providerFailure)
  ...
  const gates = {
    ...
    providerFailures: {
      actual: providerFailures,
      maximum: maximumProviderFailures,
      pass: providerFailures <= maximumProviderFailures,
    },
  };
  return {
