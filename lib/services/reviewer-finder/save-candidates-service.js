...
export async function saveCandidates({
  ...
  for (let candidateIndex = 0; candidateIndex < candidates.length; candidateIndex += 1) {
    ...
    try {
      ...
      const potentialReviewerResult = validatedSeedAnchor?.potentialReviewerId
        ? { id: validatedSeedAnchor.potentialReviewerId, created: false }
      ...
      if (suggestionResult?.skippedExcluded) {
        ...
        try {
          await markPromotionBlocked(requestId, validation.candidateKey, {
          ...
          try {
            await NotificationService.notify({
              ...
              source: 'reviewer-finder/save-candidates',
              category: 'reviewers',
              autoResolveKey: `reviewer-roster-block:${requestId}:${validation.candidateKey}`,
            });
          } catch (notifyErr) {
      ...
      try {
        ...
        try {
          await NotificationService.notify({
            ...
            source: 'reviewer-finder/save-candidates',
            category: 'reviewers',
            autoResolveKey: `reviewer-roster-promotion:${requestId}:${validation.candidateKey}`,
          });
        } catch (notifyErr) {
      ...
      errors.push({
        ...
        code: candidateError?.code || 'candidate_save_failed',
        outcome: 'failed',
      });
    }
  }
