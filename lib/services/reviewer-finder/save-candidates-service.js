...
export async function saveCandidates({
  ...
  for (let candidateIndex = 0; candidateIndex < candidates.length; candidateIndex += 1) {
    ...
    try {
      ...
      // A promotion click is an explicit staff action, so it may drive the
      // bounded server-side evidence recovery that warm page loads deliberately
      // do not. The coordinator accepts only this already server-read roster
      // key, performs no selection/promotion/email work itself, and leaves an
      // incomplete/manual-required result for the unchanged fail-closed gate.
      try {
        await reconcileReviewerStages({
          requestId,
          candidateKeys: [authoritativeRosterCandidateKey],
        });
        const refreshedRows = await findCandidatesByKeys(requestId, [authoritativeRosterCandidateKey]);
        const refreshedCandidate = refreshedRows.find((row) => row?.candidateKey === authoritativeRosterCandidateKey) || null;
        if (!refreshedCandidate || !['active', 'saved'].includes(refreshedCandidate.rosterStatus)) {
          rejectedUnresolved += 1;
          errors.push(buildPromotionAuthorityError(candidate, {
            candidateKey: validation.candidateKey,
            index: candidateIndex,
        ...
        authoritativeRosterCandidate = refreshedCandidate;
        candidate = withReviewerProvenance(authoritativeRosterCandidate);
      } catch {
        // Do not attempt a promotion against the pre-reconciliation snapshot
        // when an authority read/lease/store failure interrupts recovery.
        rejectedUnresolved += 1;
        errors.push(buildPromotionAuthorityError(candidate, {
          candidateKey: validation.candidateKey,
          index: candidateIndex,
