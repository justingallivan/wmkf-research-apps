...
// `candidates` outcome (the reviewer the write reuses by email may be any of the
// reviewer-source candidates). Reading candidates too closes the fail-open where a
// non-confident match let a same-institution EXISTING reviewer through.
function collectLookupAffiliations(contactMatch) {
  const out = [];
  const push = (aff) => { if (typeof aff === 'string' && aff.trim()) out.push(aff.trim()); };
  if (contactMatch?.outcome === 'confident') {
    ...
    for (const candidate of contactMatch.candidates || []) {
      if (candidate?.source === 'reviewer') push(candidate?.context?.affiliation);
    }
  }
  return out;
}
...
//   1. `collectLookupAffiliations(contactMatch)` — the CRM affiliation of the
//      reviewer the identity lookup matched (confident, incl. an ORCID/name match on
//      an email-less candidate, or a reviewer-source `candidates` row). ALWAYS
//      evaluated, even with no email, so a no-email confident same-institution match
//      cannot be saved as a new/duplicate reviewer unscreened.
...
async function resolveServerReuseAffiliations({ contactMatch, candidateEmail, hasSeedAnchor }) {
  ...
  return { affiliations: lookupAffiliations, existing: null };
}
  ...
  for (const rawCandidate of candidates) {
    try {
      ...
      if (candidate.hasInstitutionCOI || enrichmentInstitutionCOI || recomputedInstitutionCOI) {
        ...
        errors.push(institutionCoiError);
        continue;
      }
