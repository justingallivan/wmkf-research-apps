...
import { findIdentityConfirmation, findEligibilityByCandidateKey, stampSuggestionAnchor } from '../reviewer-roster-store';
...
export async function saveCandidates({
  ...
  for (let candidateIndex = 0; candidateIndex < candidates.length; candidateIndex += 1) {
    ...
    try {
      ...
      // New Find-tab rows are roster-managed and must carry a valid receipt
      // that binds the immutable pre-enrichment roster key. Without it, mutable
      // email/affiliation fields could redirect the durable eligibility lookup.
      ...
      if (
        rosterManaged
        && (!automatedIdentityReceipt.valid || !automatedIdentityReceipt.rosterCandidateKey)
      ) {
      ...
      try {
        const rosterCandidateKey = rosterManaged
      ...
      // forged/stale payload could send emailSource:'orcid' (or another ready source)
      // and bypass the quick-check acknowledgement at send.
      ...
      // client-side → the quick-check acknowledgement still fires at send). Otherwise the normal
      ...
      const potentialReviewerId = validatedSeedAnchor?.potentialReviewerId || (await potentialReviewerAdapter.upsertByEmail({
        ...
        emailSource: candidateEmail ? candidateEmailSource : undefined,
    ...
    if (savedCount === 0 && (rejectedInvalid + rejectedUnresolved + rejectedInstitutionCOI + rejectedIneligible) === candidates.length) {
