...
import {
  ...
  classifiedOutcome,
} from '../reviewer-institution-measurement';
import { recomputeReviewerInstitutionCOI } from '../reviewer-institution-coi-screen';
import {
...
async function screenCandidateInstitutionCOI({
  ...
  const recomputed = await recomputeReviewerInstitutionCOI({
    candidate,
    institutionEntries: coiInstitutionEntries,
    serverAffiliations: affiliations,
    institutionIdentityResolver,
  ...
  return {
    ...
    additionalCoi: recomputed.additionalCoi,
  };
}
...
async function measureSaveOutcomes(requestId, savedResults, errors, measurementKeyByIndex) {
  if (!measurementEnabled()) return;
  try {
    const results = promotionResults(savedResults, errors);
    const keys = [...new Set(results.map((result) => measurementKeyByIndex.get(result.index)).filter(Boolean))];
    if (keys.length === 0) return;
    const stored = await findCandidatesByKeys(requestId, keys);
    const byKey = new Map(stored.map((candidate) => [candidate.candidateKey, candidate]));
    const observations = results.map((result) => {
      const key = measurementKeyByIndex.get(result.index);
      const candidate = byKey.get(key);
      if (!candidate) return null; // No server-owned roster row: never trust the request payload.
      const saved = result.outcome === 'saved';
      return recordInstitutionMeasurement({
        requestId,
        candidate,
        eventType: saved ? 'save_saved' : 'save_rejected',
        captureSource: 'stored_roster',
        outcomeCategory: classifiedOutcome(result.code, saved),
      });
    }).filter(Boolean);
    await Promise.allSettled(observations);
  } catch (error) {
    console.warn('[save-candidates] institution measurement unavailable:', error?.code || error?.name || 'Error');
  }
}
 ...
 *
 * @param {Object} args - proposalTitle, programArea, requestId,
 *   candidates[] (shell validated requestId + non-empty array),
 *   actingUserSystemId. The suggestion row's cycle code is derived here from
 *   the request's meeting date (loadCoiContext.cycleCode); a client-supplied
...
export async function saveCandidates({
  ...
  const savedNames = [];
  const savedKeys = [];
  const measurementKeyByIndex = new Map();
  ...
  const coiInstitutionEntries = Array.isArray(coiContext.institutionEntries)
    ...
    : [];
  const grantCycleCode = coiContext.cycleCode || null;
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          const submittedIdentityReceipt = createServerIdentityDecisionReceipt(candidate);
          ...
          if (identityStillMatches) {
            ...
            const submittedEmail = String(
              candidate.email || candidate.contactEnrichment?.email || '',
            ).trim().toLowerCase();
      ...
      // Only an attestation/staff/server-roster-bound key can attribute a save
      // outcome. Early failures retain their response correlation key but do
      // not get a case event from that client-supplied value.
      if (rosterManaged && authoritativeRosterCandidateKey) {
        measurementKeyByIndex.set(candidateIndex, authoritativeRosterCandidateKey);
      }
      ...
      const phase2EvidenceTrusted = phase2Enabled
        && !!authoritativeRosterCandidate
        && hasServerInstitutionEvidenceReceipt({
          requestId,
          candidate: authoritativeRosterCandidate,
        });
      if (phase2Enabled && !phase2EvidenceTrusted) {
        rejectedInstitutionCOI += 1;
        errors.push({
          name: candidate.name,
          candidateKey: validation.candidateKey,
          index: candidateIndex,
          error: 'Reviewer institution evidence is missing or stale. Rerun discovery before saving.',
          code: 'institution_evidence_required',
          outcome: 'withheld',
          decision: 'institution_evidence_required',
        });
        continue;
      }
      const institutionCandidate = phase2EvidenceTrusted
        ? {
            ...candidate,
            affiliationAssertions: authoritativeRosterCandidate.affiliationAssertions,
            affiliationAssertionsComplete: authoritativeRosterCandidate.affiliationAssertionsComplete,
          }
        : candidate;
      const institutionCoiScreen = await screenCandidateInstitutionCOI({
        ...
        coiInstitutionEntries,
        institutionIdentityResolver,
        includeAdditionalAffiliations: phase2EvidenceTrusted,
      });
      ...
      if (phase2EvidenceTrusted && institutionCoiScreen.additionalCoi === 'incomplete') {
        rejectedInstitutionCOI += 1;
        errors.push({
          name: candidate.name,
          candidateKey: validation.candidateKey,
          index: candidateIndex,
          error: 'An additional reviewer affiliation could not be classified as current or historical. Review the institution evidence before saving.',
          code: 'institution_coi_incomplete',
          outcome: 'withheld',
          decision: 'institution_coi_incomplete',
        });
        continue;
      }
      ...
      if (
        ...
        errors.push({
          ...
          candidateKey: validation.candidateKey,
          index: candidateIndex,
          error: 'The saved verification predates the current address conflict. Review the email choice again.',
          code: 'address_verification_required',
          outcome: 'withheld',
          decision: 'needs_address_verification',
      ...
      const buildAddressTrustBundle = (currentPerson) => {
        ...
        if (
          addressTrustReceipt?.evidenceType === 'staff_address_choice'
          && currentTrust.valid
          && currentTrust.state.status === 'staff_verified'
        ) {
          const preservedChoice = staffAddressChoiceProjection(currentTrust.state, {
            email: normalizedCandidateEmail,
            requestId,
            candidateKey: authoritativeRosterCandidateKey,
            receipt: addressTrustReceipt,
          });
          if (!preservedChoice) {
            throw Object.assign(new Error('The recorded email choice no longer matches this reviewer. Review the current choice again.'), {
              code: 'address_verification_required',
            });
          }
          return JSON.stringify(currentTrust.state);
        }
        if (currentTrust.valid && currentTrust.state.status === 'conflict_pending') {
          ...
          if (!allowed.has(normalizedCandidateEmail)) {
            throw Object.assign(new Error('The address conflict changed; review the current email choice again.'), {
              code: 'address_verification_required',
            });
          }
        ...
        if (addressTrustReceipt?.evidenceType === 'staff_address_choice' && !resolution) {
          throw Object.assign(new Error('The stored-versus-found email choice is no longer available. Reload and review it again.'), {
            code: 'address_verification_required',
          });
        }
      ...
      errors.push({
        ...
        index: candidateIndex,
        error: duplicate
          ? 'That email belongs to another reviewer record. Fix the duplicate email owner in AkoyaGO, then use Retry record check.'
  ...
  await measureSaveOutcomes(requestId, savedResults, errors, measurementKeyByIndex);
