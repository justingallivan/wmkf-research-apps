...
//     (upsertByEmail { existing }). Read that exact reuse target's CRM affiliation
...
async function resolveServerReuseAffiliations({ contactMatch, candidateEmail, hasSeedAnchor }) {
  if (hasSeedAnchor) return { affiliations: collectLookupAffiliations(contactMatch), existing: null };
  if (candidateEmail) {
    ...
    return { affiliations: trimmed ? [trimmed] : [], existing };
  }
  return { affiliations: [], existing: null };
}
