...
} from '../reviewer-candidate-attestation';
...
import { deriveReviewerPromotionAuthoritySnapshot } from '../workbench/reviewer-warm-validation-service';
import { reconcileReviewerStages } from '../workbench/reviewer-stage-reconciliation-service';
...
// Keep promotion recovery within the same bounded request-level coordinator
// contract as the dedicated reviewer-reconcile endpoint.  Candidates beyond
// this selection are deliberately withheld rather than starting another full
// recovery window from inside the per-candidate mutation loop.
const MAX_PROMOTION_PREFLIGHT_CANDIDATES = 24;
...
function buildPromotionAuthorityError(candidate, {
  candidateKey,
  index,
  authority,
}) {
  const messages = {
    promotion_authority_missing: 'Current reviewer authority is missing; refresh this candidate before promoting.',
    promotion_authority_unavailable: 'Current reviewer authority could not be derived; retry after the server refresh completes.',
    roster_snapshot_stale: 'Reviewer evidence changed while promotion was running. Reload before retrying.',
    ...
    eligibility_no_trusted_domains: 'No trusted institutional domain is available to verify this reviewer’s eligibility. Correct the affiliation or create a repair request before promoting.',
    ...
    candidate_ineligible: 'Official institutional evidence reports this candidate is not eligible; not saved.',
  };
  return {
    name: candidate.name,
    candidateKey,
    index,
    code: authority.code || 'promotion_authority_missing',
    stage: authority.stage || null,
    error: messages[authority.code] || messages.promotion_authority_missing,
    outcome: 'withheld',
  };
}
...
function promotionResults(savedResults, errors) {
  const withheldCodes = new Set([
    ...
    'roster_status_not_promotable',
    'stage_authority_missing',
    'stage_authority_stale',
    'stage_authority_incomplete',
    'coauthor_conflict',
    'coauthor_check_incomplete',
    'eligibility_no_trusted_domains',
    'eligibility_check_incomplete',
    'eligibility_result_unrecognized',
  ]);
  return [
    ...savedResults,
 ...
 *
 * @param {Object} args - requestId, candidates[] (shell validated requestId
 *   + non-empty array),
 *   actingUserSystemId
...
export async function saveCandidates({
  ...
  const coiInstitutionEntries = Array.isArray(coiContext.institutionEntries)
    ...
    : [];
  // Request labels are immutable server metadata for this write.  A selected
  // browser row is only a correlation target, so proposalTitle/programArea/
  // cycle values posted beside it must never reach Dataverse.
  const requestContext = coiContext.requestContext || {};
  const serverSuggestionTitle = typeof requestContext.title === 'string'
    && requestContext.title.trim()
    ? requestContext.title.trim()
    : null;
  const serverSuggestionProgramArea = typeof requestContext.programArea === 'string'
    && requestContext.programArea.trim()
    ? requestContext.programArea.trim()
    : null;
  ...
  // A promotion click may request one bounded recovery pass for the submitted
  // server-issued roster handles.  It must happen once per save operation:
  // starting a fresh request/proposal recovery for every candidate would make
  // a multi-select promotion unbounded.  The per-candidate canonical roster
  // re-read and authority gate below remain the mutation authority.
  const promotionPreflightCandidateKeys = [];
  const promotionPreflightCandidateKeySet = new Set();
  for (const submittedCandidate of candidates) {
    const candidateKey = canonicalStoredReviewerCandidateKey(submittedCandidate?.candidateKey);
    if (
      candidateKey
      && !promotionPreflightCandidateKeySet.has(candidateKey)
      && promotionPreflightCandidateKeys.length < MAX_PROMOTION_PREFLIGHT_CANDIDATES
    ) {
      promotionPreflightCandidateKeys.push(candidateKey);
      promotionPreflightCandidateKeySet.add(candidateKey);
    }
  }
  let promotionPreflightAvailable = true;
  if (promotionPreflightCandidateKeys.length > 0) {
    try {
      await reconcileReviewerStages({
        requestId,
        candidateKeys: promotionPreflightCandidateKeys,
      });
    } catch {
      // A preflight failure cannot be replaced with an unbounded per-row
      // retry. The later server re-read recognizes this condition and
      // withholds the affected promotion without aborting the rest of batch.
      promotionPreflightAvailable = false;
    }
  }
  ...
  for (let candidateIndex = 0; candidateIndex < candidates.length; candidateIndex += 1) {
    ...
    try {
      // This initially exists only to verify the signed selection/confirmation
      // correlation. After the canonical roster key is resolved below, this
      // binding is replaced with the server-read roster row before any
      // contact, COI, identity, or Dataverse mutation input is computed.
      let candidate = withReviewerProvenance(validation.candidate);
      ...
      if (!rosterManaged) {
        ...
        errors.push(buildPromotionAuthorityError(candidate, {
          candidateKey: validation.candidateKey,
          index: candidateIndex,
          authority: { code: 'promotion_authority_missing' },
        }));
        continue;
      }
      const automatedRosterCandidateKey = automatedIdentityReceipt.valid
        ...
        : null;
      if (
        (staffConfirmedRosterCandidateKey && submittedRosterCandidateKey
          && staffConfirmedRosterCandidateKey !== submittedRosterCandidateKey)
        || (automatedRosterCandidateKey && submittedRosterCandidateKey
          && automatedRosterCandidateKey !== submittedRosterCandidateKey)
      ) {
        rejectedInvalid += 1;
        errors.push({
          ...
          candidateKey: validation.candidateKey,
          index: candidateIndex,
          error: 'The submitted reviewer selection does not match its server-issued identity receipt.',
          code: 'invalid_candidate',
        });
        continue;
      }
      // Exact-address adjudication is allowed to change contact fields without
      // changing the resolved person. That intentionally invalidates the older
      // v3 automated token's contact digest. Recover only from the current,
      // request-scoped roster row when its server-owned identity receipt still
      // matches the submitted identity and its exact-address receipt matches the
      // submitted email. Client-carried copies grant no authority here.
      let authoritativeRosterCandidate = null;
      let serverReceiptRosterCandidateKey = null;
      let serverReceiptAddressAuthority = false;
      if (
        rosterManaged
        && submittedRosterCandidateKey
        ...
        try {
          ...
          const submittedIdentityReceipt = createServerIdentityDecisionReceipt(authoritativeRosterCandidate);
          const storedIdentityReceipt = authoritativeRosterCandidate?.serverIdentityDecisionReceipt;
          const identityStillMatches = ['active', 'saved'].includes(authoritativeRosterCandidate?.rosterStatus)
            && hasServerIdentityDecisionReceipt(authoritativeRosterCandidate)
            && !!submittedIdentityReceipt?.identityDigest
            && submittedIdentityReceipt.identityDigest === storedIdentityReceipt?.identityDigest;
          if (identityStillMatches) {
            serverReceiptRosterCandidateKey = submittedRosterCandidateKey;
            const submittedEmail = String(
              authoritativeRosterCandidate.email || authoritativeRosterCandidate.contactEnrichment?.email || '',
            ).trim().toLowerCase();
            const storedAddressReceipt = authoritativeRosterCandidate?.addressTrustReceipt;
            serverReceiptAddressAuthority = storedAddressReceipt?.personConfirmed === true
              && !!storedAddressReceipt?.receiptId
              && storedAddressReceipt?.requestId === requestId
              && storedAddressReceipt?.candidateKey === submittedRosterCandidateKey
              && String(storedAddressReceipt.email || '').trim().toLowerCase() === submittedEmail;
          }
        } catch {
          authoritativeRosterCandidate = null;
        }
      }
      if (
        staffConfirmedRosterCandidateKey
      ...
      if (
        ...
        && !staffConfirmedRosterCandidateKey
        && !automatedRosterCandidateKey
        && !serverReceiptRosterCandidateKey
      ) {
        rejectedUnresolved += 1;
        errors.push({
          name: candidate.name,
      ...
      try {
        if (
          !authoritativeRosterCandidate
          || authoritativeRosterCandidate.candidateKey !== authoritativeRosterCandidateKey
        ) {
          [authoritativeRosterCandidate] = await findCandidatesByKeys(
            requestId,
        ...
        errors.push({
          ...
          candidateKey: validation.candidateKey,
          index: candidateIndex,
          error: 'Could not verify the current reviewer authority. Retry the check before promoting.',
      ...
      if (authoritativeRosterCandidate.candidateKey !== authoritativeRosterCandidateKey) {
        rejectedInvalid += 1;
        errors.push({
          name: candidate.name,
          candidateKey: validation.candidateKey,
          index: candidateIndex,
          error: 'The server roster record did not match the selected reviewer. Reload before retrying.',
          code: 'invalid_candidate',
          outcome: 'withheld',
        });
        continue;
      }
      if (!trimmedString(authoritativeRosterCandidate.name)) {
        // A keyed row without a usable server-side display identity cannot be
        // promoted. Do not fall back to the browser name after canonical
        // rebind; that would let a malformed roster read reopen a client-
        // supplied identity path.
        rejectedUnresolved += 1;
        errors.push(buildPromotionAuthorityError(candidate, {
          candidateKey: validation.candidateKey,
          index: candidateIndex,
      ...
      // Browser rows are selection/correlation DTOs only. From this point on
      // every fact that can affect a Dataverse mutation or policy decision
      // comes from the request-scoped server roster row (or a fresh server
      // read performed below). Do not merge browser contact/COI evidence here.
      candidate = withReviewerProvenance(authoritativeRosterCandidate);
      if (
        rosterManaged
        && (
          !promotionPreflightAvailable
          || !promotionPreflightCandidateKeySet.has(authoritativeRosterCandidateKey)
        )
      ) {
        // This exact roster handle was not included in the one bounded
        // request-level recovery. Do not open a new full recovery window from
        // this row; selection must be retried after a fresh page/request.
        rejectedUnresolved += 1;
        errors.push(buildPromotionAuthorityError(candidate, {
          candidateKey: validation.candidateKey,
          index: candidateIndex,
          authority: { code: 'promotion_authority_unavailable' },
        }));
        continue;
      }
      ...
      try {
        promotionSnapshot = await deriveReviewerPromotionAuthoritySnapshot({
          requestId,
          candidate: authoritativeRosterCandidate,
        });
      } catch {
        promotionSnapshot = null;
      }
      const promotionAuthority = getCandidatePromotionAuthority(authoritativeRosterCandidate, {
        serverAuthoritative: true,
        requestId,
        authoritative: promotionSnapshot,
        // The trusted request-context recomputation below remains the
        // authoritative institution-COI check for this mutation.
        checkInstitution: false,
      });
      if (promotionAuthority.decision !== 'ready') {
        if (promotionAuthority.code === 'candidate_ineligible') rejectedIneligible += 1;
      ...
      // This is a server-roster fact after the canonical rebind above.  Do not
      // rely solely on an authority helper's aggregate decision: a deceased
      // reviewer can never be promoted even if an older snapshot/mock omits
      // the eligibility-stage block.
      if (candidate.eligibilityStatus === 'deceased') {
        rejectedIneligible += 1;
        errors.push(buildPromotionAuthorityError(candidate, {
          candidateKey: validation.candidateKey,
          index: candidateIndex,
          authority: { code: 'candidate_ineligible' },
        }));
        continue;
      }
      ...
      const identityReceiptTrusted = automatedIdentityReceipt.valid
        || !!validatedSeedAnchor
        || !!serverReceiptRosterCandidateKey;
      ...
      // Bind the just-completed, server-owned evidence decision to the roster
      // row we originally read. A browser timestamp is never consulted here.
      // Finalization repeats this compare-and-set after the Dataverse writes.
      if (!await promotionSnapshotIsCurrent(
        requestId,
        authoritativeRosterCandidateKey,
        authoritativeRosterCandidate.rosterUpdatedAt,
      )) {
        rejectedUnresolved += 1;
        errors.push(buildPromotionAuthorityError(candidate, {
          candidateKey: validation.candidateKey,
          index: candidateIndex,
          authority: { code: 'roster_snapshot_stale' },
        }));
        continue;
      }
      ...
      const suggestionResult = await reviewerSuggestionAdapter.upsert({
        potentialReviewerId,
        requestId,
        suggestionLabel: serverSuggestionTitle ? `${serverSuggestionTitle} — ${candidate.name}` : null,
        // The generic save endpoint has no server-loaded cycle in its request
        // context.  Leave this optional field blank rather than persisting a
        // browser-supplied grantCycleCode.
        grantCycleCode: null,
        programArea: serverSuggestionProgramArea,
        relevanceScore,
        matchReason,
        sources: sources.join(','),
        selected: true,
        // A client-supplied Blob URL is neither immutable request metadata nor
        // an attested candidate fact.  Do not let a Find-card submission bind
        // arbitrary content to the durable Dataverse suggestion.
        summaryBlobUrl: null,
      }, { actingUserSystemId });
      ...
      try {
        const finalized = await finalizeCandidatePromotion(requestId, candidate, {
          ...
          potentialReviewerId,
          expectedUpdatedAt: authoritativeRosterCandidate.rosterUpdatedAt,
        });
