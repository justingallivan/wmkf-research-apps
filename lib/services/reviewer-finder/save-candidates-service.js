...
// The affiliations to evaluate COI against, matching the write's reuse surface.
// Prefer already-fetched lookup data; fall back to a direct getByEmail ONLY when
// the identity lookup THREW (contactMatch === null), which upsertByEmail would
// otherwise silently reuse by email — closing the transient-lookup-failure
// fail-open without a redundant read on every candidate.
async function resolveServerReuseAffiliations(contactMatch, candidateEmail) {
  const fromLookup = collectLookupAffiliations(contactMatch);
  if (fromLookup.length) return fromLookup;
  if (contactMatch === null && candidateEmail) {
...
export async function saveCandidates({
  ...
  for (const rawCandidate of candidates) {
    try {
      ...
      const serverReuseAffiliations = await resolveServerReuseAffiliations(contactMatch, candidateEmail);
