...
async function resolveMatchedContactInstitution(contactMatch) {
  const affiliations = [];
  if (!shouldLinkMatchedContact(contactMatch)) return { affiliations };

  const contactId = trimmedString(contactMatch.match.contactId);
  try {
    ...
    if (!parentAccountId) return { affiliations };
  ...
  if (piResolution?.state === 'failed') {
    return {
      ...
      screeningFailure: {
        ...
        error: piResolution.error || piResolution.reason || null,
      },
    };
  }
