...
// CRM affiliation(s) of the reviewer identity persistence will REUSE, from the
// already-fetched identity lookup — for BOTH a confident match and the ambiguous
// `candidates` outcome. Reading every candidate source lets reviewer-carrying
// shapes such as `source:'linked'` enforce the same-institution gate without a
// brittle source allowlist; contact-only candidates naturally contribute null.
function collectLookupAffiliations(contactMatch) {
  const out = [];
  const push = (aff) => {
    const trimmed = trimmedString(aff);
    if (trimmed) out.push(trimmed);
  };
  if (contactMatch?.outcome === 'confident') {
    push(contactMatch.match?.context?.affiliation);
  } else if (contactMatch?.outcome === 'candidates') {
    for (const candidate of contactMatch.candidates || []) {
      push(candidate?.context?.affiliation);
    }
  }
  return out;
}

function collectLookupReviewerIdsMissingAffiliation(contactMatch) {
  const out = [];
  const seen = new Set();
  const push = (id) => {
    const trimmed = trimmedString(id);
    if (!trimmed) return;
  ...
  if (contactMatch?.outcome === 'confident') {
    if (contactMatch.match?.reviewerId && !trimmedString(contactMatch.match?.context?.affiliation)) {
      push(contactMatch.match.reviewerId);
    }
  } else if (contactMatch?.outcome === 'candidates') {
    for (const candidate of contactMatch.candidates || []) {
      if (candidate?.reviewerId && !trimmedString(candidate?.context?.affiliation)) {
        push(candidate.reviewerId);
      }
    }
  } else if (contactMatch?.outcome === 'conflict') {
    for (const [key, value] of Object.entries(contactMatch.details || {})) {
      if (/reviewerid$/i.test(key)) push(value);
    }
  }
...
// The affiliation(s) to evaluate COI against, plus the EXACT reviewer identity
// persistence will REUSE. Two independent server signals, both evaluated:
//   1. `collectLookupAffiliations(contactMatch)` — the CRM affiliation of any
//      reviewer/contact candidate the identity lookup surfaced. ALWAYS evaluated,
//      even with no email, so a no-email same-institution match cannot be saved as
//      a new/duplicate reviewer unscreened.
//   2. For the no-seed-anchor EMAIL path, the exact reuse target the write reuses
//      (upsertByEmail { existing } / getByEmail) — threaded back for single-read
//      atomicity. A read failure throws to the per-candidate catch (fail-closed).
//   3. For the no-email/no-seed path, reviewer IDs with no lookup affiliation are
//      read by ID and screened. If that exact-reviewer read fails, the candidate is
//      rejected before writes rather than persisted unscreened.
// Seed anchor: the write reuses the anchor by id; its affiliation rides the
// confident anchor lookup (signal 1).
async function resolveServerReuseAffiliations({ contactMatch, candidateEmail, hasSeedAnchor }) {
  const lookupAffiliations = collectLookupAffiliations(contactMatch);
  if (hasSeedAnchor) return { affiliations: lookupAffiliations, existing: null };
  if (candidateEmail) {
    const existing = await potentialReviewerAdapter.getByEmail(candidateEmail);
    const affiliation = reviewerReuseAffiliation(existing);
    const trimmed = typeof affiliation === 'string' ? affiliation.trim() : '';
    return { affiliations: trimmed ? [...lookupAffiliations, trimmed] : lookupAffiliations, existing };
  }

  const reviewerIds = collectLookupReviewerIdsMissingAffiliation(contactMatch);
  ...
  for (const reviewerId of reviewerIds) {
    ...
    try {
      ...
      return {
        affiliations: lookupAffiliations,
        existing: null,
        screeningFailure: {
    ...
    if (trimmed) reviewerAffiliations.push(trimmed);
  }

  return {
    affiliations: reviewerAffiliations.length > 0
      ? [...lookupAffiliations, ...reviewerAffiliations]
      : lookupAffiliations,
    existing: null,
  };
}
...
export async function saveCandidates({
  ...
  const savedNames = [];
  const coiContext = await loadCoiContext(requestId, {
    includeCoPIs: false,
  ...
  for (const rawCandidate of candidates) {
    try {
      ...
      const noEmailNoSeedAnchor = !validatedSeedAnchor && !candidateEmail;
      if (noEmailNoSeedAnchor && contactMatch === null) {
      ...
      const serverReuse = await resolveServerReuseAffiliations({
        contactMatch,
        candidateEmail,
        hasSeedAnchor: !!validatedSeedAnchor,
      });
      if (serverReuse.screeningFailure) {
        console.warn('[save-candidates] reviewer_identity_affiliation_screening_failed', {
          requestId,
          candidateName: candidate.name || null,
          decisionSource: serverReuse.screeningFailure.decisionSource,
          reviewerId: serverReuse.screeningFailure.reviewerId || null,
          error: serverReuse.screeningFailure.error?.message || serverReuse.screeningFailure.error || null,
        });
        rejectedInstitutionCOI += 1;
        errors.push(buildInstitutionCoiError(candidate, {
          error: 'Candidate reviewer identity could not be screened for institution COI; not saved.',
      ...
      const recomputedInstitutionCOI = recomputeInstitutionCOI(
        candidate,
        coiInstitutionEntries,
        serverReuse.affiliations,
      );
      if (recomputedInstitutionCOI) {
      ...
      if (candidate.hasInstitutionCOI || enrichmentInstitutionCOI || recomputedInstitutionCOI) {
        rejectedInstitutionCOI += 1;
        errors.push(buildInstitutionCoiError(candidate, { recomputedInstitutionCOI }));
        continue;
      }
      ...
      }, { actingUserSystemId, existing: serverReuse.existing })).id;
