...
// The affiliation(s) to evaluate COI against, matching the EXACT reviewer identity
// persistence will REUSE:
//   - seed anchor set  → the write reuses the anchor reviewer by id; its CRM
//     affiliation rides the confident anchor lookup already fetched.
//   - no seed anchor   → the write reuses an existing reviewer by EMAIL
//     (upsertByEmail → getByEmail). Read that exact reuse target's CRM affiliation
//     directly, INDEPENDENT of the identity lookup's confidence or shape
//     (confident / candidates / linked / conflict / none / thrown) — so no
//     non-confident lookup shape can slip a same-institution existing reviewer past
//     the gate. A read failure throws to the per-candidate catch (fail-closed).
async function resolveServerReuseAffiliations({ contactMatch, candidateEmail, hasSeedAnchor }) {
  if (hasSeedAnchor) return collectLookupAffiliations(contactMatch);
  if (candidateEmail) {
    const existing = await potentialReviewerAdapter.getByEmail(candidateEmail);
    const affiliation = existing?.wmkf_primaryaffiliation || existing?.wmkf_organizationname || null;
...
export async function saveCandidates({
  ...
  for (const rawCandidate of candidates) {
    try {
      ...
      // identity lookup above (confident OR ambiguous candidates), matching the write's
      // reuse surface — before any potential-reviewer/researcher/suggestion write.
      const enrichmentInstitutionCOI = candidate.contactEnrichment?.coiRecomputed
        && !!candidate.contactEnrichment?.hasInstitutionCOI;
      const serverReuseAffiliations = await resolveServerReuseAffiliations({
        contactMatch,
        candidateEmail,
