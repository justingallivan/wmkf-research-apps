...
function lookupAffiliation(contactMatch) {
  if (contactMatch?.outcome !== 'confident') return null;
  const affiliation = contactMatch.match?.context?.affiliation;
  return typeof affiliation === 'string' && affiliation.trim() ? affiliation.trim() : null;
}

function recomputeInstitutionCOI(candidate, institutionEntries, serverAffiliation) {
  ...
  if (!serverAffiliation) return null;
  ...
  if (!serverAffiliationDecision) return null;
...
export async function saveCandidates({
  ...
  for (const rawCandidate of candidates) {
    try {
      ...
      // includes CRM affiliation resolved by lookupReviewerIdentity above, before any
      ...
      const recomputedInstitutionCOI = recomputeInstitutionCOI(
        ...
        lookupAffiliation(contactMatch),
      );
