...
async function recomputeInstitutionCOI(
  ...
  const decisions = [];
  const rawPayloadDecision = DeduplicationService.institutionCOIDecision(
    candidate,
    institutionEntries,
  );
  const payloadDecision = await DeduplicationService.institutionCOIDecisionResolved(
    candidate,
    institutionEntries,
    { resolver: institutionIdentityResolver },
  );
  // `institutionCOIDecisionResolved` only runs for a pre-existing lexical
  // match. If that raw match disappears after both sides resolve to distinct
  // institution ids, the server has positively refuted the stale client COI
  // flag. Provider abstention leaves the lexical match intact and therefore
  // does not enter this exception.
  const payloadRefuted = !!rawPayloadDecision && !payloadDecision;
