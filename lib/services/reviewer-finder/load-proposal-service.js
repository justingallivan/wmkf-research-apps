 ...
 */
async function loadProposalImpl({ requestId, fileKey }) {
  ...
  const blob = await observeReviewerFindWarmEffect({
    effectClass: 'blob_write',
    operation: 'reviewer_proposal_blob_put',
  }, () => put(`reviewer-finder/${requestNumber}/${picked.name}`, downloaded.buffer, {
    access: 'public',
...
// Explicit proposal loading is a cold action. This wrapper is intentionally at
// the service boundary so an accidental warm caller becomes visible without
// recording request or file identifiers.
export async function loadProposal(args) {
  return observeReviewerFindWarmEffect({
    effectClass: 'proposal_load',
    operation: 'reviewer_proposal_load',
  }, () => loadProposalImpl(args));
}
