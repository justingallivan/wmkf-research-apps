 ...
 * document at `Reviewer Materials/Proposal_{Request#}.pdf`, downloads it,
 ...
 *   override for deliberate historical/ad-hoc work; when omitted, only the
 *   active request's canonical Reviewer Materials proposal qualifies
 ...
 *   (body + allFiles), 404 canonical-proposal-missing (body + allFiles),
 *   409 duplicate active canonical proposals (body + allFiles)
 */
export async function loadProposal({ requestId, fileKey }) {
  ...
  // 3. Pick the file: explicit staff override, else the exact active
  // Reviewer Materials proposal. Never infer from Phase I / narrative names.
  let picked = null;
  if (fileKey) {
    ...
    const canonical = allFiles.filter((file) => (
      file.source === 'dynamics'
      && String(file.library || '').toLowerCase() === 'akoya_request'
      && isReviewerProposalFile(file.folder, file.name, requestNumber)
    ));
    if (canonical.length === 0) {
      const expectedPath = `Reviewer Materials/Proposal_${requestNumber}.pdf`;
      const message = `Canonical reviewer proposal not found at ${expectedPath}. Pass fileKey to override.`;
