...
export async function loadProposal({ requestId, fileKey }) {
  ...
  try {
    ...
    // Any head() failure (not-found or otherwise) is treated as a cache miss
    // and falls through to download — never fail the request because the
    // cache check itself errored.
    cached = null;
  }
