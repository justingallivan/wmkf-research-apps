 ...
 *
 * Cross-store ordering (the non-atomic seams): audit → required Dataverse
 * changeset (atomic) → optional contact delete changeset → optional SharePoint
 * review-file cleanup → Postgres `ReviewDraftService.deleteBySuggestion` →
 * audit-result update. Any post-Action-A cleanup failure is recorded and
 * returned as a partial warning; it never undoes the committed Action A delete.
 *
...
import { cleanupSharePointItems } from '../sharepoint-cleanup';
...
export async function removeCandidateEntirely({
  ...
  const { suggestion, request, honorarium, answerRows, contactId, potentialReviewer, reviewFile } =
    await loadRemovalContext(suggestionId);
  const scopedSharePointCleanupPlan = normalizedReviewFileDeletionAllowlist
    ? await prepareScopedSharePointCleanup(reviewFile, normalizedReviewFileDeletionAllowlist)
    : null;
  ...
  //    Action A. ──
  sharePointCleanup = await cleanupReviewSharePointFiles(reviewFile, scopedSharePointCleanupPlan);
...
function normalizeReviewFileDeletionAllowlist(allowlist) {
  ...
  if (!Array.isArray(allowlist) || allowlist.length === 0) {
    throw new RemoveCandidateError(
      'reviewFileDeletionAllowlist must be null or a non-empty array',
      400,
    );
  }
...
async function prepareScopedSharePointCleanup(reviewFile, allowlist) {
  ...
  const filesToDelete = allowlist.map((expected) => {
  ...
  return { driveId, filesToDelete, preservedFiles, folderFilesFound: files.length };
}
...
async function cleanupReviewSharePointFiles(reviewFile, scopedPlan = null) {
  ...
  try {
    const driveId = scopedPlan?.driveId || await GraphService.getDriveId(REVIEW_LIBRARY);
    const items = scopedPlan
      ? scopedPlan.filesToDelete
      : await GraphService.listFiles(REVIEW_LIBRARY, reviewFile.folder, { recursive: false });
    const files = (items || []).filter((item) => item?.id);
    ...
    const cleanedUp = await cleanupSharePointItems(driveId, files, 'remove-candidate-review-files');
    return {
      attempted: true,
      filesFound: files.length,
      deleted: cleanedUp,
      deletedCount: cleanedUp ? files.length : null,
