 ...
 *
 * Holds ALL business logic for /api/reviewer-finder/my-candidates as ONE
 ...
 *     hydration helpers;
 *   - patchMyCandidates  — per-suggestion lifecycle/researcher/person edits,
 *     restore, and bulk-by-request updates, incl. the duplicate-email 409
 *     partial-success + savedFields semantics and conflicting-owner lookup;
...
import { buildExternalUrl, ensureToken } from '../../external/token-lifecycle';
import { hashToken } from '../external-token';
import { ContactParser } from '../../utils/contact-parser';
// Same address-confidence classifier the render + send gates use, so this action can
// only attest to an address those gates actually refuse.
...
async function fetchPotentialReviewers(ids) {
  ...
  for (const chunk of chunked(ids, CHUNK)) {
    ...
    const { records } = await potentialReviewerAdapter.queryReviewers({
      select: 'wmkf_potentialreviewersid,wmkf_name,wmkf_emailaddress,wmkf_emailsource,wmkf_organizationname,wmkf_areaofexpertise,wmkf_academicrank,wmkf_primarydepartment,wmkf_maininstitution',
      filter: orChain,
      top: 500,
    });
 ...
 *
 * @param {Object} args - body (the request body), actingUserSystemId
 ...
 */
export async function patchMyCandidates({ body = {}, actingUserSystemId = null }) {
  ...
  try {
    ...
    // ── Record a staff attestation that a search-found address is correct ──
    // The dead-end this exists for: an address whose only provenance is a web search
    // is `research_only`, which the send gate refuses outright and no send-time
    // checkbox can promote. The advertised escape hatch ("verify it, then Edit
    // contact") is a no-op when the verified address is the one already stored,
    // because CandidateEditModal omits an unchanged email — so the reviewer could
    // never be invited in-app (S387).
    //
    // The address is NOT taken from the client: the stored value is re-read and the
    // submitted `verifiedEmail` must match it, so a staffer can only attest to the
    // address actually shown to them, and a value changed by another session since
    // the preview rendered fails closed instead of being silently attested.
    //
    // Only `research_only` is attestable. Anything else is already sendable, and
    // stamping it would DOWNGRADE a stronger provenance (e.g. ORCID-owned `ready`)
    // to a quick-check — the guard is what makes this action non-destructive, and it
    // is also the idempotency mechanism: a second click finds `staff_verified`
    // (quick_check, not research_only) and is refused as already-verified.
    if (body.verifyEmailAddress === true) {
      const submitted = typeof body.verifiedEmail === 'string' ? body.verifiedEmail.trim() : '';
      if (!submitted) {
        throw new MyCandidatesError('The address being verified is required', 400, {
          error: 'The address being verified is required',
          code: 'verified_email_required',
        });
      }
      // The attested address lives on the SHARED person row, so this write reaches every
      // request that uses that person. Bind it to the request the staffer is actually
      // working (Codex adversarial review, finding 1): without this, any suggestionId —
      // another PD's request, a removed row — is a valid target for a send-gate change.
      if (!isGuid(body.requestId || '')) {
        throw new MyCandidatesError('requestId is required to verify an address', 400, {
          error: 'requestId is required to verify an address',
          code: 'request_id_required',
        });
      }
      const suggestion = await suggestionAdapter.findById(suggestionId);
      if (String(suggestion?._wmkf_request_value || '').toLowerCase() !== body.requestId.toLowerCase()) {
        throw new MyCandidatesError('This reviewer is not on the request you are working', 409, {
          error: 'This reviewer is not on the request you are working',
          code: 'wrong_request',
        });
      }
      // Lifecycle: only a reviewer actually queued for a first-contact invitation on this
      // request. A removed/unselected row, or one already invited or already responded,
      // has no pending invitation for this gate to unblock — and stamping provenance for
      // it would be a durable person-level change with no live purpose.
      if (suggestion.wmkf_selected !== true) {
        throw new MyCandidatesError('This reviewer is not selected for invitation', 409, {
          error: 'This reviewer is not selected for invitation',
          code: 'candidate_not_selected',
        });
      }
      if (suggestion.wmkf_invited === true || suggestion.wmkf_emailsentat) {
        throw new MyCandidatesError('This reviewer has already been invited', 409, {
          error: 'This reviewer has already been invited',
          code: 'already_invited',
        });
      }
      if (
        suggestion.wmkf_accepted === true
        || suggestion.wmkf_declined === true
        || suggestion.wmkf_responsetype != null
      ) {
        throw new MyCandidatesError('This reviewer has already responded', 409, {
          error: 'This reviewer has already responded',
          code: 'already_responded',
        });
      }
      const personId = suggestion?._wmkf_potentialreviewer_value;
      if (!personId) {
        throw new MyCandidatesError('This candidate has no linked reviewer record', 409, {
          error: 'This candidate has no linked reviewer record',
          code: 'person_missing',
        });
      }
      const person = await potentialReviewerAdapter.getByIdWithSelect(personId, {
        select: 'wmkf_potentialreviewersid,wmkf_emailaddress,wmkf_emailsource,wmkf_identitystatus',
      });
      const storedEmail = String(person?.wmkf_emailaddress || '').trim();
      if (!storedEmail) {
        throw new MyCandidatesError('There is no address on this reviewer to verify', 409, {
          error: 'There is no address on this reviewer to verify',
          code: 'no_address',
        });
      }
      if (storedEmail.toLowerCase() !== submitted.toLowerCase()) {
        throw new MyCandidatesError(
          'The address changed since this screen was loaded. Refresh and verify the current address.',
          409,
          { error: 'The address changed since this screen was loaded. Refresh and verify the current address.', code: 'address_changed' },
        );
      }
      const confidence = emailConfidence(person);
      if (confidence.action !== 'research_only') {
        throw new MyCandidatesError(
          'Only a search-only address needs this verification; this address is already usable.',
          409,
          {
            error: 'Only a search-only address needs this verification; this address is already usable.',
            code: 'not_research_only',
            emailConfidence: confidence,
          },
        );
      }
      // Distinct from 'manual' on purpose: nobody typed this address, a human
      // confirmed a system-found one. It maps to quick_check (never `ready`), so the
      // per-recipient acknowledgement at send still applies. The actor and old/new
      // value are recorded by the Dataverse audit trail via actingUserSystemId.
      //
      // ETag-CONDITIONAL (Codex adversarial review, finding 2): the address check above
      // is a read, so without this the write could land on an address another session
      // substituted in between — stamping "staff verified" on a string nobody attested,
      // which the send path would then treat as sendable after the quick-check tick.
      // `_etag` is surfaced by processAnnotations even with a $select (researcher.js:384).
      // No ETag ⇒ refuse rather than write unconditionally.
      if (!person?._etag) {
        throw new MyCandidatesError('Could not verify the current reviewer record. Reload and try again.', 409, {
          error: 'Could not verify the current reviewer record. Reload and try again.',
          code: 'person_state_unavailable',
        });
      }
      try {
        await researcherAdapter.updateById(
          personId,
          { emailSource: 'staff_verified' },
          { actingUserSystemId, ifMatch: person._etag },
        );
      } catch (writeError) {
        if (writeError?.status === 412) {
          throw new MyCandidatesError(
            'This reviewer record changed while you were verifying it. Reload and verify the current address.',
            409,
            { error: 'This reviewer record changed while you were verifying it. Reload and verify the current address.', code: 'stale_person_row' },
          );
        }
        throw writeError;
      }
      return {
        success: true,
        message: 'Address recorded as staff-verified',
        updated: { suggestionId, emailSource: 'staff_verified', email: storedEmail },
      };
    }
    ...
    if (body.markManualInviteSent === true) {
      ...
      try {
        ...
        }, { actingUserSystemId, ifMatch: suggestion._etag });
      } catch (error) {
        if (error?.status === 412) {
          throw manualInviteError(
