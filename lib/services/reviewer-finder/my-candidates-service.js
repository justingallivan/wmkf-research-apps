 ...
 *     hydration helpers;
 *   - patchMyCandidates  — dispatches restore,
  ...
  for (const s of suggestions) {
    ...
    if (!byRequest[reqId]) {
      byRequest[reqId] = {
        ...
        // grantCycleCode: the request's meeting-date-derived code is ground
        // truth (owner decision 2026-09-08). With a meeting date the derived
        // code stands even when it is null (an off-month meeting surfaces as
        // no cycle, never as a stored page value); the stored
        // wmkf_grantcyclecode only fills in for the rare request with no
        // meeting date at all. The picker override the stored value once
        // served was removed 2026-09-06 (Stage 7 decision D4).
        grantCycleCode: request.meetingDate ? request.cycleCode : (s.wmkf_grantcyclecode || null),
      ...
      if (!request.meetingDate && !byRequest[reqId].grantCycleCode && s.wmkf_grantcyclecode) {
        // No meeting date on the request and the first row carried no code;
        // pick up a later row's stored code.
    ...
    byRequest[reqId].candidates.push({
      ...
      // never the raw field; gated exactly like testRequestVisibilityDto.
...
/**
 * Per-suggestion lifecycle/researcher/person edits, restore, or explicit
 * manual invitation recording. PI / institution edits are intentionally not
 * supported here — those belong on `akoya_request`. The former proposal-wide
 * (`proposalId`, no `suggestionId`) cycle/program-area fan-out branch was
 * removed 2026-09-06 under owner decision D4: its only client, the standalone
 * Reviewer Finder page's per-proposal dropdowns, was retired 2026-06-16, and
 * the unguarded per-row loop it drove could leave a request half-updated.
 *
 ...
 * @returns {Promise<Object>} the exact 200 payload (restore / update shapes)
...
export async function patchMyCandidates({ body = {}, actingUserSystemId = null, authorizedRequestId }) {
  ...
  try {
    ...
    if (!suggestionId) {
      throw new MyCandidatesError('suggestionId is required', 400);
    }
