...
import { APPLICANT_DISPOSITION_EXCLUDED, REVIEW_STATUS_MAP } from '../../../shared/config/reviewerLifecycle.js';
import { isDataverseRecordNotFound } from '../../dataverse/core/errors.js';

const CORRECTION_SOURCE_STATUSES = new Set([
  null,
...
const CLOSED_CORRECTION_STATUSES = new Set([
  ...
  try {
    ...
    if (hasLifecycle) {
      ...
      try {
        ...
        if (current.wmkf_completedat || CLOSED_CORRECTION_STATUSES.has(current.wmkf_reviewstatus)) {
          throw correctionError('Closed reviewer invitation and response history cannot be changed here.', 'correction_closed');
        }
        if (!CORRECTION_SOURCE_STATUSES.has(current.wmkf_reviewstatus)) {
