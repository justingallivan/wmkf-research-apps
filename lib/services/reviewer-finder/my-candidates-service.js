  ...
  for (const s of suggestions) {
    ...
    if (!byRequest[reqId]) {
      byRequest[reqId] = {
        ...
        // which delegates to bulkUpdateByRequest); fall back to the
...
export async function patchMyCandidates({ body = {}, actingUserSystemId = null, authorizedRequestId }) {
  ...
  try {
    ...
    if (proposalId !== undefined && suggestionId === undefined) {
      // GUID-validate before it reaches setRequestMetadata (which delegates
      // to bulkUpdateByRequest → findByRequest), which interpolates it raw
      // into an OData `_wmkf_request_value eq …` filter.
