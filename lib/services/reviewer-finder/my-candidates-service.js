 ...
 *     manual-invite-sent recording, and person/researcher edits here; the
 *     per-suggestion response-correction path (the approved lifecycle write
 *     plus its nonfatal accepted-token follow-up) is delegated to
 *     `reviewer-engagement/correct-response.js` (Stage 3D), incl. the
 *     duplicate-email 409 partial-success + savedFields semantics and
 *     conflicting-owner lookup;
 *   - deleteMyCandidates — atomic soft-delete (unselect + token revoke).
 *
 * `MyCandidatesError` is a compatibility re-export of
 * `reviewer-engagement/correct-response.js` (Stage 3D) — `instanceof` and
 * reference identity (`toBe`) hold across both import paths.
 *
...
import { correctResponse, MyCandidatesError } from '../reviewer-engagement/correct-response';

/**
 * Compatibility re-export. The implementation moved to
 * `lib/services/reviewer-engagement/correct-response.js` (Stage 3D). This
 * module re-exports the same object — `instanceof` and reference identity
