/**
 * Reviewer Finder — invitation .eml generation service
 * (Route→Service Consolidation Plan, micro-stage 2s — the P1s streaming pilot).
 *
 * Holds ALL business logic for POST /api/reviewer-finder/generate-emails;
 * the route is a thin shell (method dispatch, auth, rate limit, input
 * validation incl. GUID checks, DAL context, and ALL SSE framing — headers,
 * `event:`/`data:` serialization, `res.end()`).
 *
 ...
 *     generic mid-stream failure) are emitted as a terminal `error` event
 *     and the function returns — matching the route's pre-extraction
 *     top-level catch exactly;
 *   - non-terminal per-candidate failures accumulate in `result.errors`
 *     and the loop continues (stream still ends `result` → `complete`);
 *   - mark-sent is BEST-EFFORT PER SUGGESTION: a patch failure is logged
 *     and skipped, never terminal;
 *   - ASSUMES a trusted DAL context already exists — never establishes one
 *     (the shell wraps the single service call in one
 *     `withDalContext('reviewer-finder-generate-emails', …)`, the Decision 4
 *     widening that retired the legacy 'generate-emails-lookup' and
 *     'generate-emails-mark-sent' bypass scopes).
 *
 * Event vocabulary (pinned by tests/unit/generate-emails-route.test.js and
 * tests/unit/generate-emails-service.test.js): `progress` (stages lookup,
 * fetching_attachments, starting, generating, updating_database),
 * `email_generated` (per success), then terminal `result` → `complete` on
 * success or a single terminal `error` on failure (`timeout: true` when the
 * reviewer time budget aborted the run).
 */
...
import { markInvitationGenerated } from '../reviewer-engagement/record-invitation';
import { queryPersons } from '../../dataverse/adapters/app-request-person';
 ...
 * Runs inside the shell-established DAL context.
 */
...
/**
 * Generate invitation .eml files for the (pre-validated) candidate set,
 * emitting every SSE event through `onEvent({ event, data })`.
 *
 * @param {object} args
 * @param {Array}  args.candidates       full original candidate list (stats.total)
 * @param {Array}  args.validCandidates  candidates with an email address
 * @param {number} args.skippedCount     candidates dropped for missing email
 * @param {Array}  args.suggestionIds    GUID-validated suggestion ids (may be empty)
 * @param {object} args.template         { subject, body } (validated by shell)
 * @param {object} args.settings         { senderName?, senderEmail, … } (validated)
 * @param {object} [args.proposalInfo]   fallback proposal info (no suggestionId)
 * @param {object} [args.attachmentConfig] { summaryBlobUrl?, reviewTemplateBlobUrl?, additionalAttachments? }
 * @param {object} [args.options]        { useClaudePersonalization?, markAsSent? }
 * @param {*}      args.userProfileId    for LLM usage attribution
 * @param {function({event: string, data: object}): void} onEvent
 * @returns {Promise<void>} resolves after a terminal event has been emitted
 */
    ...
    if (markAsSent) {
      ...
      if (suggestionIdsToUpdate.length > 0) {
        ...
        for (const suggestionId of suggestionIdsToUpdate) {
          try {
            await markInvitationGenerated({ suggestionId, now });
            markedAsSentCount++;
          } catch (dvError) {
