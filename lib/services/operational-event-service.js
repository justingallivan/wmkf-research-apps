...
class OperationalEventService {
   ...
   * and reopened. When the caller supplies the rendered row's freshness
   * snapshot, the update only applies if it still matches; on mismatch this
   * throws `code: 'stale_state'` with the row's current state attached so the
   * route can 409 and the UI refetch.
   */
  static async setEventStatus(id, action, {
    ...
    // The admin route requires it; only trusted in-process callers may omit it.
    //
    // Precision: `status_changed_at = NOW()` stores microseconds, but the row
    // reaches the client as a JS Date → JSON ISO string, which carries
    // milliseconds only. Both freshness predicates therefore compare the
    // column truncated to milliseconds; exact equality would classify every
    // unchanged row with non-zero microseconds as stale (Codex confirm-pass).
    expectedStatusChangedAt = undefined,
    // Timestamps lose sub-millisecond precision in JSON. occurrence_count is
    // monotonic across folds, so asserting it closes the remaining same-ms gap.
    ...
    if (expectedOccurrenceCount != null) {
      expectedOccurrence = Number(expectedOccurrenceCount);
      if (!Number.isInteger(expectedOccurrence) || expectedOccurrence < 1) {
        const err = new Error('invalid expectedOccurrenceCount');
        err.code = 'invalid_action';
        throw err;
      }
    }
    const result = await sql`
      ...
      WHERE id = ${eventId}
        ...
        AND (${expectedIso}::timestamptz IS NULL
             OR date_trunc('milliseconds', last_occurred_at) = ${expectedIso}::timestamptz)
        AND (${assertStatusChanged}::boolean = false
             OR date_trunc('milliseconds', status_changed_at)
                IS NOT DISTINCT FROM ${expectedStatusChangedIso}::timestamptz)
        AND (${expectedOccurrence}::integer IS NULL
             OR occurrence_count = ${expectedOccurrence})
      RETURNING id, status
    `;
   ...
   *
   * Partial success is the contract, not a failure mode: every row lands in
   * one outcome bucket, including unexpected per-row database failures, and
   * the client refetches the list. Throws only for batch-level caller errors
   * (`invalid_action`, `batch_too_large`).
   *
   * @param {Array<{id:number|string, expectedStatus:string, expectedLastOccurredAt:string, expectedStatusChangedAt:string|null, expectedOccurrenceCount:number}>} items — all four preconditions REQUIRED per item; incomplete items land in `invalid`
   * @param {'resolve'|'reopen'} action
   * @returns {Promise<{updated:number[], stale:number[], notFound:number[], invalid:number[], failed:number[]}>}
   */
  static async setEventStatuses(items, action, { profileId = null, note = null } = {}) {
    ...
    const outcome = { updated: [], stale: [], notFound: [], invalid: [], failed: [] };
    for (const item of list) {
      const id = item && typeof item === 'object' ? item.id : item;
      // A batch item without its full freshness snapshot is refused, not
      // applied unguarded: the whole point of the batch is that an operator
      // resolved what they SAW, and what they saw is the snapshot.
      const complete = item && typeof item === 'object'
        ...
        && Number.isInteger(item.expectedOccurrenceCount)
        && item.expectedOccurrenceCount >= 1;
      if (!complete) {
      ...
      try {
        const row = await this.setEventStatus(id, action, {
          ...
          expectedOccurrenceCount: item.expectedOccurrenceCount,
        });
        ...
        if (error?.code === 'stale_state') outcome.stale.push(Number(id));
          ...
          console.error(`[operational-events] batch status update failed for event ${Number(id)}:`, error?.message || String(error));
          outcome.failed.push(Number(id));
        }
      }
    }
    return outcome;
