...
const SENSITIVE_KEY_FRAGMENTS = [
  ...
  'client_ip', 'useragent', 'user_agent', 'email',
];
...
class OperationalEventService {
  ...
  static async recordEvent(opts = {}) {
    try {
      ...
      const recoveryKey = sanitizeShortField(opts.recoveryKey);
      const dedupeKey = sanitizeShortField(opts.dedupeKey);
      ...
      if (dedupeKey && source === 'vercel-drain') {
        ...
        const result = await sql`
          ...
          RETURNING id
        `;
        if (result.rows.length === 0) return null; // duplicate delivery
  ...
  static async _settleByRecoveryKey(recoveryKey, newStatus, note) {
    try {
      const key = sanitizeShortField(recoveryKey);
      if (!key) return 0;
   ...
   */
  static async setEventStatus(id, action, { profileId = null, note = null } = {}) {
...
module.exports._internal = { sanitizeString, isSensitiveKey };
