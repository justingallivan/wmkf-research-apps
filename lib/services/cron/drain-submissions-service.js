...
async function recoverRequestCreated(client, job, originalError) {
  let existing;
  try {
    // S333 Stage 4a: nested-redundant withDalContext removed. This function is
    // only reached from within a state handler dispatched under processJob's
    // own withDalContext('drain-submissions') (:836), which is what a read
    // like this needs too (DynamicsService.checkRestriction requires SOME
    // established context to read ctx.restrictions, even for reads) — the
    // local wrap was pure nesting inside that already-established context.
    existing = await grantRequestAdapter.getById(job.request_id, { select: 'akoya_requestnum' });
  } catch (err) {
    const cls = classify(err);
    return await recordFailure(client, {
      job,
