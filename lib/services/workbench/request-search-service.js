...
function projectRequest(row) {
  ...
  return {
    ...
    projectLeader: row._wmkf_projectleader_value_formatted
      ...
      || null,
    cycleCode,
    cycleLabel: row.wmkf_meetingdate
      ? meetingDateLabel(new Date(row.wmkf_meetingdate).getUTCFullYear(), new Date(row.wmkf_meetingdate).getUTCMonth() + 1, !cycleCode)
      : row.akoya_fiscalyear || (cycleCode ? cycleCodeToLabel(cycleCode) : null),
...
export async function loadRequestSearchOptions() {
  ...
  const cycles = [...new Map(cycleRows.map((row) => {
    ...
    const offCycle = month !== 6 && month !== 12;
    const label = Number.isInteger(year) && month >= 1 && month <= 12
      ? meetingDateLabel(year, month, offCycle)
      : null;
  ...
  cycles.push({ value: 'Unclassified (no meeting date)', label: 'Unclassified (no meeting date)' });
