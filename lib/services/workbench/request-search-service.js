...
import {
  TEST_REQUEST_ORDINARY_FETCHXML_FILTER,
...
function projectRequest(row) {
  ...
  return {
    ...
    institution: requestInstitution(row)
      || null,
...
function buildFilter({ cycle, status, programId }) {
  ...
  if (cycle) {
    ...
    if (match && month > 0) {
      ...
      // Cycle labels come from loadRequestSearchOptions; anything else is a
      // client error. No fiscal-year fallback: akoya_fiscalyear is only a
      // string encoding of wmkf_meetingdate, never its own filter axis.
      throw new ServiceHttpError('Select a cycle from the list.', { httpStatus: 400 });
    }
  }
...
async function hydrateSearchHits(ids, filter, programId) {
  ...
  for (const idChunk of chunk(ids, SEARCH_HYDRATION_CHUNK_SIZE)) {
    const hydrated = await grantRequestAdapter.findByIds(idChunk, {
      select: withTestRequestIsolationSelect(SEARCH_SELECT),
      top: idChunk.length,
      filter,
    });
...
export async function loadRequestSearchOptions({ callerSystemId, programId } = {}) {
  ...
  const [cycleRows, statusResult] = await Promise.all([
    grantRequestAdapter.aggregateMeetingDateCycles({
      grantProgramIds: [scope.programId],
      ...(testRequestIsolationEnabled() ? {
        extraFetchXmlFilter: TEST_REQUEST_ORDINARY_FETCHXML_FILTER,
    ...
    testRequestIsolationEnabled()
      ? grantRequestAdapter.aggregateStatusesByGrantProgram(scope.programId, {
        filter: TEST_REQUEST_ORDINARY_OData_FILTER,
      })
      : grantRequestAdapter.aggregateStatusesByGrantProgram(scope.programId),
  ]);
