/**
 * Workbench — bounded active/historical Research request discovery.
 *
 ...
 * then apply Research/cycle/status filters during canonical hydration through
 ...
 * statuses reflect Research requests and cycles show June/December meeting dates.
 *
 * Contract:
...
const RESEARCH_FILTER = `(${odata.or(RESEARCH_PROGRAM_IDS.map((id) => odata.eqGuid('_akoya_programid_value', id)))})`;
...
function projectOutsideProgramRequest(row, requestNumber) {
  if (!row || RESEARCH_PROGRAM_IDS.includes(String(row._akoya_programid_value || '').toLowerCase())) {
    return null;
  }
  return {
    requestNumber,
...
async function findOutsideProgramRequest(requestNumber) {
  const found = await grantRequestAdapter.findByRequestNumber(requestNumber, {
    select: OUTSIDE_PROGRAM_SELECT,
    top: 1,
  });
  return projectOutsideProgramRequest(found.records?.[0], requestNumber);
}

function buildFilter({ cycle, status }) {
  const clauses = [RESEARCH_FILTER];
  if (cycle) {
...
async function hydrateSearchHits(ids, filter) {
  const records = [];
  ...
  // before returning indexed hits; absent or other program IDs are unavailable.
  const byId = new Map(records.filter((row) => (
    RESEARCH_PROGRAM_IDS.includes(String(row._akoya_programid_value || '').toLowerCase())
  )).map((row) => [
...
export async function loadRequestSearchOptions() {
  const [cycleRows, statusResult] = await Promise.all([
    grantRequestAdapter.aggregateMeetingDateCycles({ programIds: RESEARCH_PROGRAM_IDS }),
    grantRequestAdapter.aggregateRequests({ ...STATUSES_AGGREGATE, filter: RESEARCH_FILTER }),
  ]);
...
export async function searchWorkbenchRequests({ query, cycle, status, offset = 0 }) {
  ...
  if (q && !/^\d+$/.test(q)) {
    ...
    const hydrated = ids.length
      ? await hydrateSearchHits(ids, filter)
    ...
    if (q && /^\d+$/.test(q) && rows.length === 0 && offset === 0) {
      outsideProgramRequest = await findOutsideProgramRequest(q);
    }
