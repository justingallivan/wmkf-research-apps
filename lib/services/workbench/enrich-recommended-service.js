...
import { normalizeAffiliationForComparison } from '../discovery/affiliation';
...
export async function institutionEvidenceConnectsIdentity({
  ...
  if (await checker.areConsistent(evidenceInstitution, finalAffiliation, { signal })) {
    return true;
  }
  // The evidence string is typically a verbatim PubMed byline ("Department of
  // X, <Institution>, City, ST, USA"), which defeats both the direct matcher
  // and OpenAlex resolution (zero search results), so a same-institution pair
  // still compares false. Before concluding contradiction, retry on the
  // extracted institution cores. A true cross-institution contradiction
  // (different extracted cores) still returns false — this fallback widens
  // what counts as the SAME institution, never what counts as a person match.
  const evidenceCore = normalizeAffiliationForComparison(evidenceInstitution);
  const affiliationCore = normalizeAffiliationForComparison(finalAffiliation);
  if (!evidenceCore || !affiliationCore) return false;
  const unchanged = evidenceCore === String(evidenceInstitution).trim().toLowerCase()
    && affiliationCore === String(finalAffiliation).trim().toLowerCase();
  if (unchanged) return false;
  return checker.areConsistent(evidenceCore, affiliationCore, { signal });
}
...
export function institutionVerdictReason({ contradictionSource, contradictedEvidence, finalAffiliation }) {
  ...
  if (contradictionSource === 'compared' && contradictedEvidence && finalAffiliation) {
    return `the matched publications place them at “${clipInstitutionForCopy(contradictedEvidence)}”, which could not be reconciled with the listed “${clipInstitutionForCopy(finalAffiliation)}”`;
  }
  if (contradictionSource === 'prior_flag') {
    return 'an earlier verification pass flagged an institution mismatch that could not be re-checked this run';
  }
