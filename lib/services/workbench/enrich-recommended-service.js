...
export async function enrichRecommended({
  ...
  try {
    ...
    for (const c of enriched) {
      ...
      // `affiliationHistory` is current-run PubMed evidence carried through
      // enrichment unchanged. Prefer it to stored primary affiliation, which a
      // prior automated match may already have contaminated on a rerun.
      const pubmedInstitution = Array.isArray(c.affiliationHistory) && c.affiliationHistory.length > 0
