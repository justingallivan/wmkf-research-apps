...
import { addressTrustDecision, createConflictPendingState } from '../../utils/reviewer-address-trust';
...
export async function enrichRecommended({
  ...
  try {
    ...
    for (const c of enriched) {
      ...
      const applicantContactMismatch = canonicalContact.decision === 'contact_claim_mismatch';
      ...
      let addressTrustStateJson = null;
        ...
        if (prId && enrichedEmail && storedCanonicalEmail && ce.emailPersistAllowed === true) {
          try {
            ...
            const conflictState = createConflictPendingState({
              currentState: addressTrustDecision(currentPerson).state,
      ...
      out.push(identityNeedsReview ? {
        ...
        contactEnrichment: {
          ...
          email: enrichedEmail,
          emailSource: enrichedEmailSource,
          emailPersistAllowed: ce.emailPersistAllowed === true && !!enrichedEmail,
        },
        website: c.website || ce.website || null,
        orcidUrl,
        googleScholarUrl,
