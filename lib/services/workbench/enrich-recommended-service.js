...
// TEMP S400 verdict-operand trace — remove after one production capture.
...
export async function enrichRecommended({
  ...
  try {
    ...
    // TEMP S400 verdict-operand trace — remove after one production capture.
    console.log('[verdict-trace]', JSON.stringify({
      kind: 'row-disposition',
      sha: process.env.VERCEL_GIT_COMMIT_SHA || null,
      openAlexKeyConfigured: !!process.env.OPENALEX_API_KEY,
      preservedConfirmed: preservedConfirmed.map((p) => p.suggestionId || null),
      rowsToEnrich: rowsToEnrich.map((r) => r.wmkf_appreviewersuggestionid || null),
    }));
    ...
    // TEMP S400 verdict-operand trace — remove after one production capture.
    // Propagating resolver distinguishes provider failure from abstention for
    // the trace only; it must never feed the verdict path.
    const verdictTraceResolver = createInstitutionIdentityResolver({ propagateProviderErrors: true });
    ...
    for (const c of enriched) {
      ...
      const verdictTrace = {
        kind: 'comparison',
        suggestionId: c.suggestionId || null,
        rowSource: 'fresh-enrich',
        priorInstitutionMismatch: c.institutionMismatch === true,
        priorAffiliation: ce.priorAffiliation || null,
        affiliationHistory0: (Array.isArray(c.affiliationHistory) && c.affiliationHistory[0]) || null,
        suggestedInstitution: c.suggestedInstitution || null,
        verificationInstitution: verificationInstitution || null,
        finalAffiliation,
        resolvedInstitutions,
        evidenceInstitutions: null,
        comparisons: null,
        checkerThrew: null,
      };
      try {
        const comparisons = [];
        verdictTrace.comparisons = comparisons; // TEMP S400 — live reference, reflects pushes even on throw
        ...
        verdictTrace.evidenceInstitutions = evidenceInstitutions; // TEMP S400
        ...
        verdictTrace.checkerThrew = { name: err?.name || null, message: err?.message || null }; // TEMP S400
        sendEvent('progress', { message: `Could not reconcile institution affiliations for ${c.name}: ${err.message}` });
      }
      // TEMP S400 verdict-operand trace (continued) — provider-failure vs
      // abstention classification via the propagate-errors resolver. Wrapped so
      // it can never affect the verdict; remove after one production capture.
      try {
        const traceNames = institutionNames([
          ...(verdictTrace.evidenceInstitutions || []),
          finalAffiliation,
        ]);
        const resolverOutcome = {};
        for (const traceName of traceNames) {
          try {
            const traceResolved = await verdictTraceResolver.resolve(traceName);
            resolverOutcome[traceName] = traceResolved
              ? `resolved:${traceResolved.displayName}`
              : 'abstained';
          } catch (traceErr) {
            resolverOutcome[traceName] = `provider-failed:${traceErr?.message || 'unknown'}`;
          }
        }
        verdictTrace.resolverOutcome = resolverOutcome;
        verdictTrace.institutionContradicted = institutionContradicted;
        console.log('[verdict-trace]', JSON.stringify(verdictTrace));
      } catch (traceErr) {
        console.log('[verdict-trace]', JSON.stringify({
          kind: 'trace-error',
          suggestionId: c.suggestionId || null,
          message: traceErr?.message || null,
        }));
      }
