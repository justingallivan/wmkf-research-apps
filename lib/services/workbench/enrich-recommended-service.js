...
export async function enrichRecommended({
  ...
  try {
    ...
    for (const c of enriched) {
      ...
      const scholarlyOwnershipGrounded = ce.emailSource === 'scholarly_multi'
        || ce.emailSource === 'scholarly_single';
      const email = (unconfirmedMatch || (
        rawEmail
        ...
        try {
          ...
          if (identity) {
            ...
            const decisionToWrite = unconfirmedMatch
              ...
              : identity;
            await researcherAdapter.writeIdentityDecision(prId, decisionToWrite, {
              actingUserSystemId,
              identityOrigin: 'automated',
            });
            if (blockByIdentity) {
              await researcherAdapter.clearIdentityFields(prId, RESOLVER_SOURCED_FIELDS, {
                actingUserSystemId,
                identityOrigin: 'automated',
              });
            }
