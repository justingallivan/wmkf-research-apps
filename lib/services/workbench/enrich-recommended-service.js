...
import { createInstitutionStage2Evaluator } from '../institution-affiliation-stage2';
...
import { measurementEnabled } from '../reviewer-institution-measurement';
...
import { evaluateServerCandidateIndependentIdentity } from '../reviewer-independent-identity-runtime';
import {
  createServerInstitutionEvidenceReceipt,
  institutionEvidenceProjection,
  reviewerInstitutionPhase2Enabled,
  TTL_SECONDS as INSTITUTION_EVIDENCE_TTL_SECONDS,
} from '../reviewer-institution-evidence-attestation';
...
export function stage2EvidenceAssertion(candidate, verificationInstitution) {
  const sourceContract = verificationInstitution ? ({
    pubmed: {
      sourceType: 'publication',
      // The compact verifier result does not bind this affiliation to a
      // publication date. Treat it as unknown rather than calling a possibly
      // current byline historical.
      currentness: 'unknown',
      authorSpecific: true,
    },
    orcid: {
      sourceType: 'orcid_employment',
      // The compact runtime history does not preserve employment end dates.
      currentness: 'unknown',
      authorSpecific: true,
    },
    openalex: {
  ...
  return {
    rawText: verificationInstitution || candidate.suggestedInstitution || '',
    ...sourceContract,
    sourceReference: candidate.suggestionId || null,
    observedAt: null,
  };
}

export function stage2RecordedAssertion(candidate, enrichment, finalAffiliation) {
  const source = enrichment.affiliationSource || candidate.affiliationSource || null;
  const sourceContract = {
    orcid_current: { sourceType: 'orcid_employment', currentness: 'current' },
    openalex_current: { sourceType: 'official_profile', currentness: 'unknown' },
    pubmed_recency: { sourceType: 'publication', currentness: 'unknown' },
    staff_confirmed: { sourceType: 'staff_record', currentness: 'current' },
    staff_manual: { sourceType: 'staff_record', currentness: 'current' },
  }[source] || { sourceType: 'staff_record', currentness: 'unknown' };
  return {
    rawText: finalAffiliation || '',
    ...sourceContract,
    sourceReference: candidate.suggestionId || null,
    observedAt: null,
    authorSpecific: true,
...
/**
 * Sentence-case an OpenAlex topic name to the stored-keyword convention the
 * Reviewer Finder already uses ("Microbial ecology"): only a plain
 * Title-Case word (one uppercase letter followed by lowercase letters —
 * `/^[A-Z][a-z]+$/`) is changed at all, and only by lowercasing it (then the
 * FIRST word, if it is now all-lowercase, gets its own first letter
 * capitalized). Every other word — anything mixed-case, all-caps, hyphenated,
 * or containing a digit — is left EXACTLY as the provider supplied it: an
 * OpenAlex topic name can carry a scientific token whose casing is the
 * information (`mTOR`, `scRNA-seq`, `p53`, `CRISPR`, `COVID-19`, `IL-6`), and
 * lowercasing it would destroy that. OpenAlex topic names are mostly Title
 * Case ("Research Data Management Practices"); the Reviews paragraph composer
 * only lowercases the FIRST letter of an area for mid-sentence use, so an
 * unconverted plain-Title-Case name would render as "research Data
 * Management Practices".
 */
function sentenceCaseTopic(topic) {
  const isPlainTitleCase = (word) => /^[A-Z][a-z]+$/.test(word);
  const isAllLowercase = (word) => /^[a-z]+$/.test(word);
  const words = String(topic || '')
    .trim()
    .split(/\s+/)
    .filter(Boolean)
    .map((word) => (isPlainTitleCase(word) ? word.toLowerCase() : word));
  if (words.length === 0) return '';
  if (isAllLowercase(words[0])) words[0] = words[0][0].toUpperCase() + words[0].slice(1);
  return words.join(' ');
}

/**
 * Expertise areas to persist as `wmkf_keywords` (fill-if-empty in the
 * researcher adapter, so a staff-entered value is never overwritten). Claimed
 * areas the verifier checked win; an applicant-recommended reviewer arrives
 * with none, so fall back to the accepted OpenAlex author's research topics.
 * `blockScholar` mirrors the h-index/citation gating: a skipped or unconfirmed
 * OpenAlex match must never write a namesake's topics. Capped at three, the
 * number the expertise sentence renders.
 */
function expertiseKeywordsFor(candidate, enrichment, blockScholar) {
  const claimed = Array.isArray(candidate.expertiseAreas) ? candidate.expertiseAreas.filter(Boolean) : [];
  if (claimed.length > 0) return claimed.join('; ');
  if (blockScholar) return null;
  const topics = Array.isArray(enrichment.openAlexResearchTopics) ? enrichment.openAlexResearchTopics : [];
  const areas = topics.map(sentenceCaseTopic).filter(Boolean).slice(0, 3);
...
function institutionStage2HoldReason(presentation) {
  if (!presentation?.visible) return null;
  if (presentation.kind === 'current_conflict') {
    return `${presentation.detail} Confirm or correct the identity before selecting this reviewer.`;
  }
  return 'The explanatory institution assessment did not confirm a current mismatch, but the existing verification gate still requires identity confirmation before this reviewer can be selected.';
}

function institutionStage2LegacyFallback(legacyHold) {
  return {
...
async function persistRecommendedRoster({
  ...
  const measurementCollector = [];
  throwIfDeadlineAborted(signal);
  try {
    for (const candidate of candidates) {
      throwIfDeadlineAborted(signal);
      try {
        let pruned = pruneCandidateForRoster({
          ...candidate,
          enrichedProposalKey: proposalKey,
          applicantEnrichmentCacheVersion: APPLICANT_ENRICHMENT_CACHE_VERSION,
        });
        if (!pruned?.name) continue;
        if (reviewerInstitutionPhase2Enabled()) {
          const trustedEvidence = institutionEvidenceProjection(pruned);
          pruned = {
            ...pruned,
            independentIdentity: trustedEvidence.independentIdentity,
            affiliationAssertions: trustedEvidence.affiliationAssertions,
            affiliationAssertionsComplete: trustedEvidence.affiliationAssertionsComplete,
          };
          const phase2Receipt = createServerInstitutionEvidenceReceipt({
            requestId,
            candidate: pruned,
            expiresAt: new Date(Date.now() + (INSTITUTION_EVIDENCE_TTL_SECONDS * 1000)).toISOString(),
          });
          if (phase2Receipt) {
            pruned = { ...pruned, serverInstitutionEvidenceReceipt: phase2Receipt };
          }
        } else {
          const {
        ...
        const recorded = await recordSurfaced(
          ...
          [pruned],
          {
            ...
            ...(measurementEnabled() ? {
              measurementSource: 'server_applicant',
              measurementAssessment: snapshotKey ? measurementBySuggestion?.get(snapshotKey) : null,
    ...
    await Promise.allSettled(measurementCollector);
  }
  return outcome;
}
...
export async function enrichRecommended({
  ...
  try {
    ...
    const stage2Evaluator = isInstitutionStage2PresentationEnabled()
    ...
    const measurementBySuggestion = new Map();
    for (const c of enriched) {
      const prId = c.potentialReviewerId;
      const ce = c.contactEnrichment || {};
      const independentIdentity = reviewerInstitutionPhase2Enabled()
        ? await evaluateServerCandidateIndependentIdentity({
            requestId,
            candidate: c,
            authority: 'server_applicant',
            signal: deadlineController.signal,
          })
        : null;
      ...
      let institutionPresentation = null;
      if (stage2Evaluator) {
        try {
          const stage2 = await stage2Evaluator.evaluate({
            evidenceAssertion: stage2EvidenceAssertion(c, verificationInstitution),
            recordedAssertion: stage2RecordedAssertion(c, ce, finalAffiliation),
            consumer: 'candidate_card',
            // Runtime identity evidence is not yet proven independent of the
            // affiliation under assessment. Stage 2 therefore supplies no
            // identity authority and uses the result for copy only.
            independentIdentity: null,
            legacyHold: institutionContradicted,
          }, { signal: deadlineController.signal });
          institutionPresentation = stage2.presentation;
          if (c.suggestionId) {
            measurementBySuggestion.set(suggestionSnapshotKey(c.suggestionId), stage2.assessment);
          }
          console.log('[institution-stage2-card]', JSON.stringify({
            suggestionId: c.suggestionId || null,
            relationship: stage2.assessment?.relationship || 'unresolved',
            evidenceContext: stage2.assessment?.evidenceContext || 'unresolved',
            kind: institutionPresentation?.kind || 'unresolved',
            legacyHold: institutionContradicted,
          }));
        } catch (error) {
          if (deadlineController.signal.aborted) throw error;
          console.warn('[institution-stage2-card] presentation unavailable; using legacy copy:', error?.message || error);
          // Record that this version was attempted so a deterministic Stage 2
          // projection failure cannot invalidate the cache on every page load.
          // `visible: false` keeps the incumbent mismatch presentation active.
          institutionPresentation = institutionStage2LegacyFallback(institutionContradicted);
        }
      }
      ...
      if (prId && !identityNeedsReview) {
        try {
          await researcherAdapter.upsertByPotentialReviewer(prId, {
            ...
            keywords: expertiseKeywordsFor(c, ce, blockScholar),
          }, { actingUserSystemId });
      ...
      out.push(identityNeedsReview ? {
        ...
        reasoning: !identityConfirmed
          ...
          : (institutionStage2HoldReason(institutionPresentation)
        ...
        ...(institutionPresentation ? { institutionPresentation } : {}),
        ...(reviewerInstitutionPhase2Enabled() && independentIdentity
          ? { independentIdentity }
          : {}),
        ...(reviewerInstitutionPhase2Enabled() && Array.isArray(c.affiliationAssertions)
          ? { affiliationAssertions: c.affiliationAssertions }
          : {}),
        ...(reviewerInstitutionPhase2Enabled() && Array.isArray(c.affiliationAssertions)
