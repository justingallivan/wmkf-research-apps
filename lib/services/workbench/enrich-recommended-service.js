 ...
 * Reviewer Finder already uses ("Microbial ecology"): first letter capitalized,
 * the rest lowercased except an all-caps token of two or more letters (an
 * acronym such as "DNA" or "CRISPR"). OpenAlex topic names are mostly Title
 ...
 * unconverted name would render as "research Data Management Practices".
 */
function sentenceCaseTopic(topic) {
  const isAcronym = (word) => /^[A-Z0-9-]{2,}$/.test(word) && /[A-Z]/.test(word);
  ...
  if (!isAcronym(words[0])) words[0] = words[0][0].toUpperCase() + words[0].slice(1);
  return words.join(' ');
}
