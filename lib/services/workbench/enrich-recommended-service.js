...
export async function institutionEvidenceConnectsIdentity({
  ...
  // NOTE (S400 review): raw PubMed byline evidence ("Department of X,
  // <Institution>, City, ST, USA") defeats both the direct matcher and
  // OpenAlex resolution, so same-institution pairs compare false — the known
  // false-mismatch class attributed in the S400 capture. A core-extraction
  // fallback was built and REVERTED: the borrowed aggregation-key extractor
  // collapsed comma-qualified sibling institutions (UC San Diego/San
  // Francisco) into false CONSISTENTs, which would clear identityNeedsReview
  // and enable writes for a wrong identity. False negatives are tolerable at
  // this seam; false positives are not. A conservative segment-whole
  // extractor is the accepted future fix (Codex review, 2026-08-04).
