...
export async function institutionEvidenceConnectsIdentity({
  ...
  // NOTE (S400 review, updated Wave 6 2026-08-09): raw PubMed byline evidence
  // ("Department of X, <Institution>, City, ST, USA") used to defeat both the
  // direct matcher and OpenAlex resolution, so same-institution pairs
  // compared false — the known false-mismatch class attributed in the S400
  // capture (e.g. request 1002912, "Lunenfeld-Tanenbaum Research Institute"
  // byline vs "Lunenfeld-Tanenbaum Research Institute, University of
  // Toronto" recorded affiliation). An earlier core-extraction fallback was
  // built and REVERTED: the borrowed aggregation-key extractor collapsed
  ...
  // false positives are not. The accepted fix (Codex review, 2026-08-04) was
  // a conservative segment-whole extractor — exact segment match plus
  // decoration proof, never free core extraction — and it is now active at
  // this seam (`checker` is constructed with the opt-in comparison mode
  // above). Its sibling-institution false pins (the 148-row uc-sibling live
  // suite) are the evidence that this widening does not reopen the reverted
  // hazard; see docs/INSTITUTION_PAIR_CONSISTENCY_RESOLUTION_PLAN.md Stage 1
  // for the full mechanism and gate history.
...
export async function enrichRecommended({
  ...
  try {
    ...
    const institutionConsistencyChecker = createInstitutionConsistencyChecker({ segmentComparison: true });
