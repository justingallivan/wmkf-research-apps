...
async function persistRecommendedRoster({ requestId, proposalKey, candidates, signal }) {
  if (!proposalKey || !requestId || !Array.isArray(candidates) || candidates.length === 0) return;
...
export async function enrichRecommended({
  ...
  try {
    ...
    if (rowsToEnrich.length === 0) {
      await persistRecommendedRoster({
        requestId,
        proposalKey,
    ...
    for (const c of enriched) {
      ...
      if (institutionContradicted && c.suggestedInstitution && c.affiliation) {
        try {
          // The legacy mismatch detector compares strings. Before making that a
          // hard identity gate, reconcile direct and one-hop OpenAlex institution
          // identities so legitimate co-affiliations do not become needless staff
          // work (for example Broad↔MIT or an affiliated hospital↔university).
          const coAffiliationCompatible = await institutionConsistencyChecker.areConsistent(
            c.suggestedInstitution,
            c.affiliation,
            { signal: deadlineController.signal },
          );
          if (coAffiliationCompatible) institutionContradicted = false;
        } catch (err) {
      ...
      if (identityNeedsReview) {
        const reason = institutionContradicted
      ...
      out.push(identityNeedsReview ? {
        ...
        reasoning: institutionContradicted
