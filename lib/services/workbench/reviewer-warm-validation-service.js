...
import {
  ...
  candidateIdentity,
  canonicalIso,
  LEGACY_SELECTION_PROJECTION_VERSION,
  legacySelectionCandidateKey,
  legacyServerRosterSelectionCandidateKey,
  planRosterFreshness,
  projectLegacySelectionProjection,
  projectLegacyServerRosterSelectionProjection,
} from '../reviewer-stage-freshness.js';
import * as stageSourceVersions from './reviewer-stage-source-versions.js';
import { projectReviewerContact } from '../../utils/reviewer-vetted-email.js';
import { hasCandidateStaffIdentityConfirmation } from '../../utils/reviewer-identity-authority.js';
import { verifyHistoricalAutomatedIdentitySelection } from '../reviewer-candidate-attestation.js';
...
export function buildApplicantAnchorRefreshReceipt({ request, candidate, completedAt } = {}) {
  ...
  return {
    ...
    contractVersion: 1,
    sourceVersion,
    // The applicant-slot fingerprint is both the exact input and the bounded
    // result for this deterministic anchor decision.
...
function serverNotApplicableApplicantAnchor(candidate) {
  ...
  return {
    // This is an expected server-owned dependency value, not a receipt. Warm
    // GET must not mint/persist a `not_applicable` receipt, especially one
    // without the producer's canonical completedAt/resultVersion envelope.
    // A non-applicant anchor becomes current only after cold/manual emission
    // has written a complete matching receipt.
    expectedSourceVersion: sourceVersion,
    applicantAnchorAuthority: 'not_applicable',
  };
}

function plannerInputForCandidate({ candidate, proposal, slotVersions, request, requestCoiContextVersion }) {
  ...
  if (lane === 'non_applicant') {
    ...
    if (notApplicable?.expectedSourceVersion) {
      ...
      return {
        ...
        authoritative: {
          ...
          proposalContentVersion,
          requestCoiContextVersion,
          canonicalPersonVersion: snapshot.canonicalPersonVersion,
          stageInputVersions: snapshot.stageInputVersions,
          versions: snapshot.versions,
          legacyEvidenceDependencies: snapshot.legacyEvidenceDependencies,
        },
        applicantAnchorAuthority: notApplicable.applicantAnchorAuthority,
      };
    }
    return {
      candidate,
  ...
  const candidateWithApplicantInput = { ...candidate, applicantInputVersion: slotVersion };
  const snapshot = stageSourceVersions.buildReviewerStageDependencySnapshot({
    candidate: candidateWithApplicantInput,
    requestId: request?.akoya_requestid,
    rosterUpdatedAt: candidate?.rosterUpdatedAt,
...
async function planWarmRosterCandidates({ roster, request, proposal, state, applicantInput, requestCoiContextVersion }) {
  const candidates = rosterCandidates(roster);
  const slotVersions = applicantSlotVersions(request);
  return Promise.all(candidates.map(async (candidate) => {
    const input = plannerInputForCandidate({
      candidate,
      proposal,
      slotVersions,
      request,
    ...
    // Pre-stage-freshness rows may only receive this display/selection bridge
    // from server-read identity, contact, and exact-address attestation data.
    // It cannot turn the plan into a cache hit or permit promotion; the normal
    // server snapshot still demands current stage receipts before mutation.
    const contact = projectReviewerContact(candidate, {
      staffConfirmed: hasCandidateStaffIdentityConfirmation(candidate),
    });
    // Selection compatibility is bounded signed history only. This is a
    // crypto-only read of the stored receipt; it does not invoke identity,
    // COI, contact, or roster-write providers. Promotion still derives a
    // current ordinary stage snapshot separately.
    const legacyCandidateKey = legacySelectionCandidateKey(candidate, contact);
    const legacyServerRosterCandidateKey = legacyServerRosterSelectionCandidateKey(candidate, contact);
    const automatedAttestation = state === 'current' && (legacyCandidateKey || legacyServerRosterCandidateKey)
      ? await verifyHistoricalAutomatedIdentitySelection(candidate?.automatedIdentityAttestation, {
        requestId: request?.akoya_requestid,
        candidate,
      })
      : null;
    const legacySelection = state === 'current'
      ? projectLegacySelectionProjection(candidate, {
        requestId: request?.akoya_requestid,
        contact,
        automatedAttestation,
      }) || projectLegacyServerRosterSelectionProjection(candidate, {
        contact,
        automatedAttestation,
      })
      : null;
    ...
    return {
      ...
      ...(legacySelection
        ? {
          legacySelection,
          // Keep the status explicit even if a future planner change were to
          // make this incomplete legacy row appear otherwise clean.
...
function projectPlan(plan) {
  ...
  return {
    ...
    currentStages: Array.isArray(plan.currentStages) ? plan.currentStages.slice(0, STAGES.length) : [],
    pendingStages: Array.isArray(plan.pendingStages) ? plan.pendingStages.slice(0, STAGES.length) : [],
    leaseRepairRequired: plan?.leaseRepairRequired === true,
    refreshes: (Array.isArray(plan.refreshes) ? plan.refreshes : []).slice(0, STAGES.length).map((refresh) => ({
      stage: refresh.stage,
      reason: refresh.reason,
      // The client may only execute a server-plan action.  In particular the
      // generic route never performs structured address verification.
      action: refresh.action === 'recover_expired_lease'
        ? 'recover_expired_lease'
        : refresh.action === 'operator_repair_required'
          ? 'operator_repair_required'
        : refresh.stage === 'roster_persistence'
          ? 'finalize_cached_evidence'
          : refresh.stage === 'address_trust'
            ? 'dedicated_address_action'
            : 'refresh_stage',
    })),
    promotionAuthority: plan.promotionAuthority,
    legacySelection: plan?.legacySelection?.version === LEGACY_SELECTION_PROJECTION_VERSION
      && plan.legacySelection.state === 'selectable'
      && canonicalIso(plan.legacySelection.evidenceCheckedAt)
      ? {
...
/**
 * Fresh, server-only promotion snapshot for exactly one roster candidate.
 *
 * This deliberately reuses the warm planner rather than merely comparing
 * receipt source hashes: the planner also applies stage age policy and live
 * refresh-lease handling.  A snapshot is bound to request + canonical key so
 * a valid snapshot cannot be replayed for a sibling candidate/request.
 */
export async function deriveReviewerPromotionAuthoritySnapshot({ requestId, candidate, deps = {} }) {
  const key = candidateIdentity({ candidateKey: candidate?.candidateKey });
  if (!key || !isGuid(requestId)) {
    return { authorityState: 'stale', requestId: null, candidateKey: null, versions: {} };
  }
  ...
  try {
    ...
    return { authorityState: 'stale', requestId, candidateKey: key, versions: {} };
  }
  ...
  const requestInputs = compactRequestInputs(request);
  const requestCoiContextVersion = stageSourceVersions.buildRequestCoiContextVersion({
    requestId: request.akoya_requestid,
    applicantOrganization: requestInputs.applicantOrganization,
    principalInvestigator: requestInputs.pi,
  });
  ...
  const state = applicantInput.state === 'current' && proposal.state === 'current'
    ? 'current'
    : 'stale';
  const input = plannerInputForCandidate({
    candidate,
    proposal,
    slotVersions: applicantSlotVersions(request),
    request,
    requestCoiContextVersion,
  });
  const authoritative = state === 'current'
    ...
    : staleAuthoritativeInput(input, { proposal, applicantInput });
  const [plan] = planRosterFreshness({ candidates: [input.candidate], authoritative });
  const allCurrent = state === 'current'
    && plan?.cacheOutcome === 'hit'
    && Array.isArray(plan?.currentStages)
    && plan.currentStages.length === STAGES.length
    && (!Array.isArray(plan?.pendingStages) || plan.pendingStages.length === 0);
  return {
    ...authoritative,
    authorityState: allCurrent ? 'current' : 'stale',
    requestId,
    candidateKey: key,
...
export async function readReviewerWarmValidation({ requestId, roster, deps = {} }) {
  ...
  const candidatePlans = (await planWarmRosterCandidates({
    roster,
    request,
    proposal,
    state,
  ...
  return {
    state,
    reasonCode,
    // Bounded operational attestation for the read-only smoke preflight. The
    // enum proves which current-cycle binding won without exposing a Graph
    // path, file key, item ID, or request/reviewer data.
    proposalBinding: proposal.state === 'current' ? proposal.binding : null,
...
export {
  ...
  serverNotApplicableApplicantAnchor,
};
