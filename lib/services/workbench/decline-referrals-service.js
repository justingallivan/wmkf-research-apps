 ...
 *   referralText: string, legacy: boolean, dismissible: boolean,
 *   referralVersion: string|null, declinedAt: string|null }> }>}
 */
export async function getDeclineReferrals({ requestId }) {
  ...
  const referrals = declinedWithReferral.flatMap((r) => {
    const referralVersion = declineReferralContentVersion(r.wmkf_declinereferral);
    return parseStoredDeclineReferral(r.wmkf_declinereferral).flatMap((referral, index) => {
      if (referral.resolved === true) return [];
      if (structuredReferralAlreadyAdded(referral, resolvedCandidateRows, personById)) return [];
      const dismissible = referral.structured
        ? referralVersion !== null
        : resolveLegacyDeclineReferral(r.wmkf_declinereferral).ok;
      return [{
...
export async function dismissDeclineReferral({
  ...
  const result = await suggestionAdapter.dismissDeclineReferral(
    { requestId, suggestionId, referralIndex, referralVersion },
    { actingUserSystemId },
  );
