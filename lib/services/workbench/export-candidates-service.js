...
export async function exportCandidates({ requestId, candidates }) {
  const r = testRequestIsolationEnabled()
    ? (await grantRequestAdapter.findByIds([requestId], {
      select: REQUEST_SELECT,
      filter: TEST_REQUEST_ORDINARY_OData_FILTER,
      top: 1,
    }).catch(() => null))?.records?.[0]
    : await grantRequestAdapter.getById(requestId, { select: REQUEST_SELECT }).catch(() => null);
  if (!r) {
