...
export async function ingestApplicantReviewers({ requestId, actingUserSystemId, userProfileId }) {
  ...
  const d0TSlots = Date.now(); // D0 TEMPORARY (S399)
  ...
  const d0THydrate = Date.now(); // D0 TEMPORARY (S399)
  ...
  console.log(
    `[applicant-reviewers timing] service getById=${d0TGetById - d0T0}ms ` +
    `slots=${d0TSlots - d0TGetById}ms(N=${slotsPopulated}) ` +
    `hydrate=${d0THydrate - d0TSlots}ms exclude=${Date.now() - d0THydrate}ms ` +
    `substantive=${excludedSubstantive} requestId=${requestId}`
  );
