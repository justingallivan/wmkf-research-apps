...
import { deriveReviewerPromotionAuthoritySnapshot } from './reviewer-warm-validation-service';
...
async function screenApplicantInstitutionCOI({ requestId, personId, rosterCandidate }) {
  ...
  try {
    ...
    const canonicalAffiliation = String(reviewerAffiliation(canonicalPerson) || '').trim() || null;
    const canonical = canonicalAffiliation
      ...
      : null;
    if (payload?.status === 'candidate_institution_missing' && !canonicalAffiliation) {
      return {
        screeningFailure: {
 ...
 * server-owned roster contact/address evidence, bind an exact verified address
 * when required, verify canonical contact, then select the suggestion.
 *
 * @param {Object} args
 ...
 */
export async function promoteApplicantReviewer({ requestId, suggestionId, actingUserSystemId }) {
  try {
    ...
    if (String(rosterCandidate.suggestionId || '').toLowerCase() !== suggestionId.toLowerCase()) {
      throw promotionError(
        'Reviewer roster evidence did not match the selected suggestion. Reload before retrying.',
        409,
    ...
    // The route body selects the suggestion only. Contact corrections are
    // committed by their dedicated, server-attested action before promotion;
    // a posted email, name, affiliation, or COI blob must never influence the
    // canonical-person mutation below.
    const manualEmailAttempted = false;
