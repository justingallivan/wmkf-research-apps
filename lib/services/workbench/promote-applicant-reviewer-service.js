...
async function measureApplicantPromotion(requestId, candidate, eventType, outcomeCategory) {
  if (!candidate) return;
  try {
    await recordInstitutionMeasurement({
      requestId, candidate, eventType, captureSource: 'stored_roster', outcomeCategory,
    });
  } catch (error) {
    console.warn('[promote-applicant-reviewer] institution measurement unavailable:', error?.code || error?.name || 'Error');
  }
}
...
async function prepareApplicantAddressTrust({
  ...
  if (!manualEmail && storedEmail && rosterCandidate?.applicantContactMismatch === true
    ...
    if (
      ...
      throw promotionError(
        'The saved verification predates the current address conflict. Review the email choice again.',
        409,
    ...
    if (
      ...
      throw promotionError(
        pendingConflict
          ? 'The stored and newly found addresses differ. Review the email choice before promotion.'
          : 'The address conflict could not be restored. Retry the check or create a repair request before promotion.',
        409,
  ...
  if (needsAttestation) {
    ...
    const preservedChoice = receipt?.evidenceType === 'staff_address_choice'
      && currentTrust.valid
      && currentTrust.state.status === 'staff_verified'
      ? staffAddressChoiceProjection(currentTrust.state, {
          email,
          requestId,
      ...
      : null;
    if (receipt?.evidenceType === 'staff_address_choice' && preservedChoice) {
      addressTrustStateJson = JSON.stringify(currentTrust.state);
    } else if (currentTrust.valid && currentTrust.state.status === 'conflict_pending') {
      ...
      if (!allowed.has(email)) {
        throw promotionError(
          'The address conflict changed. Review the current email choice again.',
          409,
          'address_verification_required',
          { decision: 'needs_address_verification' },
        );
      }
      if (!receiptCanResolveConflict(receipt, currentTrust.state.conflict)) {
        throw promotionError(
          'The saved verification predates the current address conflict. Review the email choice again.',
          409,
          'address_verification_required',
    ...
    } else if (receipt?.evidenceType === 'staff_address_choice') {
      throw promotionError(
        'The recorded email choice no longer matches this reviewer. Reload and review the current choice again.',
        409,
        'address_verification_required',
        { decision: 'needs_address_verification' },
      );
...
export async function promoteApplicantReviewer({ requestId, suggestionId, contact, actingUserSystemId }) {
  let measurementRosterCandidate = null;
  try {
    ...
    measurementRosterCandidate = rosterCandidate;
    ...
    // Dormant Phase 2 save-boundary parity. When explicitly enabled, applicant
    // promotion uses the same server-loaded PI/applicant institution context and
    // additional-affiliation signal set as ordinary Find saves. This runs before
    // any contact or suggestion mutation; stale/missing evidence and provider
    // failures hold for retry.
    if (reviewerInstitutionPhase2Enabled()) {
      if (!hasServerInstitutionEvidenceReceipt({ requestId, candidate: rosterCandidate })) {
        throw new ServiceHttpError('Reviewer institution evidence is missing or stale; rerun enrichment before promotion', {
          httpStatus: 422,
          body: {
            error: 'Reviewer institution evidence is missing or stale; rerun enrichment before promotion',
            code: 'institution_evidence_required',
          },
        });
      }
      let coiContext;
      let institutionScreen;
      try {
        ...
        if (coiContext.piResolution?.state === 'failed') {
          throw Object.assign(new Error('PI institution resolution failed'), {
            code: 'pi_institution_lookup_unavailable',
          });
        }
        institutionScreen = await recomputeReviewerInstitutionCOI({
          candidate: rosterCandidate,
          institutionEntries: coiContext.institutionEntries,
          serverAffiliations: [
            rosterCandidate.applicantKnownReviewer?.affiliation,
          ].filter(Boolean),
          institutionIdentityResolver: createInstitutionIdentityResolver(),
          includeAdditionalAffiliations: true,
        });
      } catch (error) {
        console.warn('[promote-applicant-reviewer] institution COI screening unavailable:', error?.message || error);
        throw new ServiceHttpError('Reviewer institution conflicts could not be verified; retry the promotion', {
          httpStatus: 503,
          body: {
            error: 'Reviewer institution conflicts could not be verified; retry the promotion',
            code: error?.code || 'institution_coi_unavailable',
          },
        });
      }
      if (institutionScreen.decision && institutionScreen.decision.dropDecision !== 'exempt') {
        throw new ServiceHttpError('Candidate is at a proposal institution (institution COI); not promoted', {
          httpStatus: 422,
          body: {
            error: 'Candidate is at a proposal institution (institution COI); not promoted',
            code: 'institution_coi',
            institutionCOIDetails: institutionScreen.decision.candidate?.institutionCOIDetails || null,
          },
        });
      }
      if (institutionScreen.additionalCoi === 'incomplete') {
        throw new ServiceHttpError('An additional reviewer affiliation could not be classified as current or historical', {
          httpStatus: 422,
          body: {
            error: 'An additional reviewer affiliation could not be classified as current or historical',
    ...
    if (!contactError && addressPlan.shouldWrite) {
      try {
        ...
        if (addressPlan.attested && addressPlan.receipt) {
          ...
          const preservedChoice = addressPlan.receipt.evidenceType === 'staff_address_choice'
            && currentTrust.valid
            && currentTrust.state.status === 'staff_verified'
            ? staffAddressChoiceProjection(currentTrust.state, {
                ...
                requestId,
                candidateKey: rosterCandidate.candidateKey,
                receipt: addressPlan.receipt,
              })
            : null;
          if (addressPlan.receipt.evidenceType === 'staff_address_choice' && preservedChoice) {
            currentAddressTrustStateJson = JSON.stringify(currentTrust.state);
          } else if (currentTrust.valid && currentTrust.state.status === 'conflict_pending') {
            const allowed = new Set([
              currentTrust.state.conflict.storedEmail,
              currentTrust.state.conflict.foundEmail,
            ...
            if (!receiptCanResolveConflict(addressPlan.receipt, currentTrust.state.conflict)) {
          ...
          } else if (addressPlan.receipt.evidenceType === 'staff_address_choice') {
            throw Object.assign(new Error('The recorded email choice no longer matches this reviewer.'), {
              code: 'address_verification_required',
            });
          } else {
            currentAddressTrustStateJson = JSON.stringify(createStaffVerifiedState({
    ...
    await measureApplicantPromotion(requestId, measurementRosterCandidate, 'save_saved', 'saved');
    return {
      success: true,
      suggestionId,
      candidateKey: rosterCandidate.candidateKey,
    ...
    await measureApplicantPromotion(
      requestId, measurementRosterCandidate, 'save_rejected',
      classifiedOutcome(err?.body?.code || err?.code),
    );
