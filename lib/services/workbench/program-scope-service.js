...
export async function resolveWorkbenchProgramScope({
  ...
  if (callerSystemId) {
    ...
    const latestCycle = records
      .map((row) => {
        const timestamp = Date.parse(row?.wmkf_meetingdate || '');
        const cycle = meetingDateToCycleCode(row?.wmkf_meetingdate);
        return Number.isFinite(timestamp) && cycle ? { timestamp, cycle } : null;
      })
      .filter(Boolean)
      .sort((a, b) => b.timestamp - a.timestamp)[0]?.cycle || null;
