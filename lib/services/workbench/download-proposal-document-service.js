 ...
 * of the Proposal-tab Reviewer Materials, Phase I, Phase II, or canonical AI
 * Materials documents the list service surfaces for this request — not just
 * any file under the request folder. Re-derived from the request's own record
 * (never from raw query params). Library allowlisting is additionally enforced
 * by GraphService.
 *
...
export async function downloadProposalDocument({ requestId, library, folder, filename }) {
  ...
  const {
    ...
    aiMaterials = [],
    otherDocuments,
  } = await listProposalDocuments(
    rec.akoya_requestid,
    rec.akoya_requestnum,
    cycleCode,
  );
  const allowed = new Set([
    ...slots.filter((s) => s.found).map((s) => `${s.library}::${s.folder}::${s.name}`),
    ...reviewerMaterials.map((d) => `${d.library}::${d.folder}::${d.name}`),
    ...phaseIIDocuments.map((d) => `${d.library}::${d.folder}::${d.name}`),
    ...aiMaterials.filter((s) => s.found).map((s) => `${s.library}::${s.folder}::${s.name}`),
