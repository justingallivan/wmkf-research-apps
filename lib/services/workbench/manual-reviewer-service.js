 ...
 * the durable match reason. The referral → identity resolution
...
import { formatReferredByReason } from '../../utils/reviewer-provenance';
...
export async function addManualReviewer({
  ...
  // match reason staff already read on the suggestion/person rows. The clause owns
  // line 1 so a name carrying periods ("Dr. Abby Doyle") round-trips intact; see
  // formatReferredByReason.
  const matchReason = referredBy
    ? formatReferredByReason(referredBy, note)
  ...
  const remedyOutcomes = ['promotion_required', 'restore_required', 'already_handled'];
  if (suggestion.outcome && !remedyOutcomes.includes(suggestion.outcome)) {
    throw new ManualReviewerError(409, {
      error: 'This reviewer is not actionable from Add Reviewer; reload the request and try again.',
      code: 'unsupported_manual_reviewer_outcome',
      suggestionId: suggestion.id,
    });
  }

  if (remedyOutcomes.includes(suggestion.outcome)) {
  ...
  if (suggestion.selected !== true) {
    throw new ManualReviewerError(409, {
      error: 'This reviewer was not added to the candidate pool; reload the request and try again.',
