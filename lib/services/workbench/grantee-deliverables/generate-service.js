...
export async function generateDeliverableAbstract({ requestId, regenerate, actingUserSystemId }) {
  ...
  // Once the grantee has returned the package, a REGENERATE is destructive and
  // pointless: it burns a paid LLM call and overwrites wmkf_abstractformatted —
  // the historical draft, i.e. the record of what was actually sent to the
  // grantee — while the published text is wmkf_abstractapproved, which it does
  // not touch. So the visible editor does not change and the loss is silent: the
  // "what we sent" vs "what they approved" comparison is gone. Mirrors the range
  // guard in send-invite-service.js:85. The REUSE path (regenerate false) stays
  // open — it performs no write here and doubles as the tab's load.
  if (regenerate && currentStatus !== null && !Number.isNaN(currentStatus)
    && currentStatus >= GRANTEE_DELIVERABLE_STATUS.SUBMITTED) {
    throw new ServiceHttpError(
      'The grantee has already returned this package; the draft abstract can no longer be regenerated.',
      { httpStatus: 409 },
    );
  }
