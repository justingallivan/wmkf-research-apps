 ...
 * replace-submission (S412); the route is a thin shell (method, auth, staged
 * image resolution, GUID validation, DAL context, HTTP mapping).
 *
 ...
 *   Optional durability hook called after SharePoint upload and before the
 *   conditional Dataverse PATCH.
  ...
  if (imageFile) {
    ...
    if (onCandidateUploaded) {
      try {
        ...
        console.error('[grantee-deliverables/replace] candidate durability hook failed:', e?.message || e);
        await cleanupSharePointItems(driveId, [uploadedItem], 'grantee-replace-candidate');
        throw new ServiceHttpError('Could not persist upload recovery state.', {
          httpStatus: 503,
          code: 'staging_failed',
          body: { code: 'staging_failed', error: 'Could not prepare the image for saving. Please try again.' },
        });
      }
    }
  }
