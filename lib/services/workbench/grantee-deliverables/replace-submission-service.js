 ...
 *   - the prior-image prune matches /grantee_image_/i.
...
export async function replaceGranteeSubmission({
  ...
  try {
    ...
    // server-side commit is possible. Never delete an image a committed row may
    // now reference — re-read first.
    //
    ...
    // a referenced image. The image ref alone is the evidence here.
    if (uploadedItem) {
      let state = 'not_committed';
      try {
        const after = await getDeliverableForRequest(requestId);
        state = after?.wmkf_imagefileref === newImageRef ? 'committed' : 'not_committed';
      } catch (reReadErr) {
        state = 'unknown';
      ...
      // 'unknown' → leave it. A possible orphan beats deleting a referenced image.
    }
  ...
  // Best-effort: retire the prior image now that the new one is committed. Orphan
  // cleanup, never data loss — logged and swallowed on failure. Matches the portal
  // writer, so a staff replacement leaves the folder in the same shape a grantee
  // resubmission would.
  if (imageFile && hasExistingImage) {
    try {
      const existing = await GraphService.listFiles(GRANTEE_LIBRARY, folder);
      // Exclude the just-uploaded item by ID — the new name is a random nonce, so
      // an id compare is the robust guard.
      ...
      if (stale.length) {
        ...
        await cleanupSharePointItems(driveId, stale, 'grantee-replace-orphan');
      }
    } catch (e) {
