 ...
 * `currentField`/`status`/`statusLabel`) so they set explicit bodies. A
 * non-412/ambiguous write is re-read: an exact committed value returns success,
 * while a mismatch or failed confirmation returns a typed 503.
...
const SAVE_VERIFY_SELECT = 'akoya_requestid,wmkf_abstractformatted,wmkf_abstractapproved';
...
export async function loadGranteeAbstract({ requestId }) {
  ...
  return {
    ...
    // Response-only sanitized HTML initializes the editor and renders the staff
    // preview. Markdown remains authoritative for dirty tracking and writes.
 ...
 *   503 when a non-412 write cannot be confirmed as committed
 */
export async function saveGranteeAbstract({ requestId, text, clientEtag, baseField, actingUserSystemId }) {
  ...
  try {
    ...
    // A timeout/no-response is an ambiguous write: Dataverse may have committed
    // the PATCH before its response was lost. Re-read both abstract fields so an
    // exact committed value can be reported as success without losing the fresh
    // etag, while a draft→approved target flip still fails as a stale edit.
    try {
      confirmedRow = await grantRequestAdapter.getById(requestId, { select: SAVE_VERIFY_SELECT });
    } catch (reReadError) {
      console.error('[grantee-deliverables/abstract] post-error re-read failed:', reReadError?.message || reReadError);
      throw new ServiceHttpError('Abstract save could not be confirmed.', {
        httpStatus: 503,
        code: 'save_unconfirmed',
        body: {
          code: 'save_unconfirmed',
          error: 'Dataverse did not respond, so the save could not be confirmed. Reload the abstract before trying again.',
        },
      });
    }

    const confirmedTarget = resolveTarget(confirmedRow || {});
    if (confirmedTarget.which !== target.which) {
      throw new ServiceHttpError('stale abstract edit after ambiguous write', {
        httpStatus: 409,
        code: 'stale',
        body: {
          code: 'stale',
          error: 'The abstract changed while the save was being confirmed. Reload and re-apply your edit.',
          currentField: confirmedTarget.which,
        },
      });
    }
    if (confirmedTarget.text !== text) {
      throw new ServiceHttpError('Abstract save was not committed.', {
        httpStatus: 503,
        code: 'save_retryable',
        body: {
          code: 'save_retryable',
          error: 'The abstract was not saved because Dataverse did not respond. Try again.',
        },
      });
    }
    console.warn('[grantee-deliverables/abstract] patch threw but write committed (response drop); returning success');
  }

  // Re-read for the new etag so the PD can keep editing without a reload.
  let newEtag = confirmedRow?._etag || null;
  if (!confirmedRow) {
    try {
