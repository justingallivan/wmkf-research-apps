...
import { produceAddressTrustEvidence } from './reviewer-stage-producers/address-trust';
...
const STAGE_PREREQUISITES = Object.freeze({
  ...
  // The address-trust source is explicitly bound to the sealed contact
  // receipt.  Do not let its deterministic projection run against a contact
  // result that has not yet become durable.
  address_trust: ['contact'],
  roster_persistence: [
    'applicant_anchor', 'identity', 'institution_domains', 'institution_coi',
...
// This path creates current promotion evidence, so a missing lifecycle field
// is not equivalent to an active Dataverse row. The adapter's shared select
// includes statecode; fail closed if that contract ever regresses.
function isActivePerson(person) { return !!person && person.statecode === 0; }
function personRecordId(person) {
  const id = person?.wmkf_potentialreviewersid || person?.id || null;
  return guid(id) ? id : null;
}
function canonicalPersonReference(candidate) {
  // These fields are admitted only through the contact-stage projector.  A
  // generic candidate/potentialReviewer ID is not enough to bind address
  // authority to a specific current Dataverse person version.
  const id = candidate?.canonicalPersonId;
  const etag = candidate?.canonicalPersonEtag || candidate?.personEtag;
  return guid(id) && safeToken(etag)
    ? { id: id.toLowerCase(), etag }
    : null;
}
function trustedReconciliationContext(context, requestId) {
  return !!context
    && context.requestId === requestId
    && sameGuid(context?.request?.akoya_requestid, requestId);
}
...
export async function planReviewerCandidateRefresh({ requestId, candidateKey, reconciliationContext = null } = {}) {
  ...
  // Applicant-origin rows without a person link cannot produce the exact
  // suggestion + person + request receipt required for applicant_anchor.  Read
  // the suggestion first so a request-scoped, applicant-provenance row is
  // distinguished from a stale/cross-request handle.  This is a staff
  // identity/link repair, not an evidence-refresh rejection and never grants
  // promotion authority.
  let applicantAnchor;
  try {
    applicantAnchor = await validateApplicantAnchor(requestId, candidate);
  } catch {
    return { outcome: 'failed_retryable', code: 'suggestion_anchor_unavailable', candidateKey: canonicalCandidateKey };
  }
  if (!applicantAnchor.ok) {
    return {
      outcome: applicantAnchor.outcome,
      code: applicantAnchor.code,
...
export async function createReviewerStageRefreshRequestContext(requestId) {
  ...
  return {
    ...
    // A candidate-specific server person read is useful for planning and for
    // emitting a contact receipt.  Each producer completion still force-reads
    // the person before its roster CAS, so this cache cannot make an older
    // person version current.
    canonicalPeople: new Map(),
  };
}

function contactLinkedPersonId(candidate) {
  const id = candidate?.canonicalPersonId
    || candidate?.potentialReviewerId
    || candidate?.applicantKnownReviewer?.potentialReviewerId
    || candidate?.seedResolvedPotentialReviewerId
    || null;
  return guid(id) ? id.toLowerCase() : null;
}

async function serverContactCanonicalPerson(candidate, requestContext = null, forceFresh = false) {
  let id = contactLinkedPersonId(candidate);
  const cache = requestContext?.canonicalPeople instanceof Map ? requestContext.canonicalPeople : null;
  let person = !forceFresh && id ? cache?.get(id) || null : null;

  // Some historical rows predate contact-stage canonical-person receipts. Do
  // not trust their old Dataverse-contact evidence to fill the gap. Instead,
  // make a fresh exact-ORCID lookup, require its unique name-consistent
  // potential-reviewer outcome, then bind the returned row's ID and ETag.
  // This is deliberately narrower than a name/email recovery and does not
  // turn a merely plausible historical match into contact authority.
  if (!id) {
    const normalized = normalizeOrcid(candidate?.contactEnrichment?.orcidId
      || candidate?.contactEnrichment?.orcid
      || candidate?.orcid);
    if (normalized.state !== 'valid' || !candidate?.name) {
      return { candidate, canonicalPerson: null };
    }
    let match;
    try {
      match = await lookupReviewerIdentity({
        name: candidate.name,
        email: null,
        orcid: normalized.id,
    ...
    if (match?.outcome !== 'confident'
      || match?.match?.matchKey !== 'orcid'
      || match?.match?.nameConsistent !== true
      || !guid(match?.match?.reviewerId)) {
      return { candidate, canonicalPerson: null };
    }
    id = match.match.reviewerId.toLowerCase();
  }
  if (!person) {
    try {
      person = await potentialReviewerAdapter.getById(id);
    } catch {
      return { candidate, canonicalPerson: null };
    }
    if (cache && !forceFresh) cache.set(id, person);
  }
  if (!isActivePerson(person) || !sameGuid(personRecordId(person), id) || !safeToken(person?._etag)) {
    return { candidate, canonicalPerson: null };
  }
  ...
  // This is a short-lived server-only source preview. The contact producer is
  // the only persistence path that admits these fields to the roster row.
  return {
    candidate: {
      ...candidate,
      canonicalPersonId: id,
...
async function serverContext(requestId, candidate, {
  ...
  const contactAuthority = await serverContactCanonicalPerson(candidate, requestContext, forceFresh);
  const authoritativeCandidate = contactAuthority.candidate;
  const applicantReceipt = isApplicantCandidate(authoritativeCandidate)
    ? buildApplicantAnchorRefreshReceipt({ request, candidate: authoritativeCandidate, completedAt: new Date().toISOString() })
    : null;
  ...
  const plan = planCandidateFreshness({ candidate: authoritativeCandidate, authoritative });
  return {
    request,
    proposal,
    candidate: authoritativeCandidate,
    ...
    plan,
    contactCanonicalPerson: contactAuthority.canonicalPerson,
  };
}

/**
 * Assess the only address-trust path that request-level reconciliation may
 * execute.  It is deliberately stricter than the structured staff route:
 * neither a browser acknowledgement nor an unversioned potential-reviewer ID
 * can create authority here.  The contact-stage-projected canonical ID/ETag
 * must still name the active Dataverse row, and the existing producer must
 * classify the server-loaded evidence as terminal current/not-applicable.
 */
async function deterministicAddressTrustProjection(context, signal, deadlineAt) {
  const sourceVersion = expectedSource(context?.snapshot, 'address_trust');
  if (!sourceVersion) return { action: 'dedicated_address_action', reason: 'source_unavailable' };

  // A non-authoritative identity is a producer-defined not-applicable result.
  // It needs no person read only when this row has no contact-projected
  // canonical reference to become stale.  If a reference exists, validate it
  // before writing even a not-applicable receipt.
  const withoutPerson = await produceAddressTrustEvidence({
    candidate: context.candidate,
    expectedSourceVersion: sourceVersion,
    signal,
    deadlineAt: boundedDeadlineAt(deadlineAt),
  });
  const reference = canonicalPersonReference(context.candidate);
  if (!reference) {
    return withoutPerson?.outcome === 'not_applicable'
      ? { action: 'refresh_stage', envelope: withoutPerson, person: null }
      : { action: 'dedicated_address_action', reason: 'canonical_person_unavailable' };
  }

  let person;
  try {
    person = await potentialReviewerAdapter.getById(reference.id);
  } catch {
    return { action: 'dedicated_address_action', reason: 'canonical_person_unavailable' };
  }
  if (!isActivePerson(person)
    || !sameGuid(personRecordId(person), reference.id)
    || !sameEtag(person?._etag, reference.etag)) {
    return { action: 'dedicated_address_action', reason: 'canonical_person_changed' };
  }

  const canonicalPerson = {
    authority: 'server_loaded',
    canonicalPersonId: reference.id,
    canonicalPersonEtag: reference.etag,
    personEtag: reference.etag,
  };
  const envelope = await produceAddressTrustEvidence({
    candidate: context.candidate,
    linkedPerson: person,
    canonicalPerson,
    expectedSourceVersion: sourceVersion,
  ...
  return ['current', 'not_applicable'].includes(envelope?.outcome)
    ? { action: 'refresh_stage', envelope, person: { id: reference.id, etag: reference.etag } }
    : { action: 'dedicated_address_action', reason: 'deterministic_projection_incomplete' };
}

/** Re-read the exact person immediately before the roster CAS. */
async function addressTrustPersonStillCurrent(projection) {
  if (!projection?.person) return true;
  try {
    const settled = await potentialReviewerAdapter.getById(projection.person.id);
    return isActivePerson(settled)
      && sameGuid(personRecordId(settled), projection.person.id)
      && sameEtag(settled?._etag, projection.person.etag);
  } catch {
    return false;
  }
}
...
async function validateApplicantAnchor(requestId, candidate) {
  ...
  if (!guid(suggestionId)) return { ok: false, outcome: 'rejected', code: 'canonical_candidate_unavailable' };
  ...
  if (!sameGuid(suggestion?.wmkf_appreviewersuggestionid, suggestionId)
    ...
    return { ok: false, outcome: 'rejected', code: 'suggestion_anchor_mismatch' };
  }
  const personId = candidatePersonId(candidate);
  // The suggestion is proven request-scoped and applicant-origin, but its
  // roster projection has no linked person.  Do not copy a person id from the
  // suggestion or stamp an applicant receipt: a staff member must resolve the
  // exact person/link through the existing identity/edit-add workflow.
  if (!personId) return { ok: false, outcome: 'action_required', code: 'dedicated_identity_action' };
...
async function executeStage(stage, context, signal, requestContext = null, deadlineAt = null, execution = null) {
  ...
  if (stage === 'contact') return produceReviewerContactEvidence({
    candidate,
    canonicalPerson: context.contactCanonicalPerson,
    institutionDomains: candidate.trustedInstitutionDomains,
  ...
  if (stage === 'address_trust') {
    const projection = await deterministicAddressTrustProjection(context, signal, deadlineAt);
    if (projection.action !== 'refresh_stage') return null;
    if (execution) execution.addressTrust = projection;
...
export async function refreshReviewerCandidateStage({
  ...
  const reconciliationAddressTrust = stage === 'address_trust'
    && trustedReconciliationContext(reconciliationContext, requestId);
  // Address trust remains unavailable from the generic staff/browser route.
  // The one exception is the private request-level reconciler, which still
  // executes only the deterministic server-read branch below.
  if (stage === 'address_trust' && !reconciliationAddressTrust) return rejected(args, 'dedicated_address_action');
  if (!EXECUTABLE_REVIEWER_REFRESH_STAGES.includes(stage) && !reconciliationAddressTrust) return rejected(args, 'stage_not_executable');
  ...
  try {
    ...
    if (!anchor.ok) {
      if (anchor.outcome === 'action_required') {
        return response({
          ...args,
      ...
      return rejected(args, anchor.code, plan);
    }
  } catch { return rejected(args, 'suggestion_anchor_unavailable', plan); }
  if (stage === 'address_trust') {
    let projection;
    try {
      projection = await deterministicAddressTrustProjection(context, null, deadlineAt);
    } catch {
      return rejected(args, 'request_authority_unavailable', plan);
    }
    if (projection.action !== 'refresh_stage') {
      return response({
        ...args,
        outcome: 'action_required',
  ...
  try {
    const execution = {};
    const envelope = await executeStage(stage, context, controller.signal, reconciliationContext, deadlineAt, execution);
    ...
    const after = afterCandidate
      ...
      : null;
    const currentAddressTrustPerson = stage !== 'address_trust'
      || await addressTrustPersonStillCurrent(execution.addressTrust);
    if (!currentAnchor?.ok || !after || !currentAddressTrustPerson || !sameStageAuthority(context.snapshot, after.snapshot, stage)) {
      let persisted;
      try {
...
export const _internals = {
  safePlan,
  validateApplicantAnchor,
  serverContext,
  executeStage,
  deterministicAddressTrustProjection,
  addressTrustPersonStillCurrent,
  expectedSource,
