...
function executableRefresh(plan, { allowDeterministicAddressTrust = false } = {}) {
  ...
  const ordinary = refreshes.find((entry) => entry?.action === 'refresh_stage'
    || entry?.action === 'recover_expired_lease'
    || entry?.action === 'finalize_cached_evidence');
  if (ordinary || !allowDeterministicAddressTrust) return ordinary || null;
  ...
  return refreshes.find((entry) => (
    entry?.stage === 'address_trust' && entry?.action === 'dedicated_address_action'
  )) || null;
}
