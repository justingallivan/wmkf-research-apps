...
import { listForRequest } from '../reviewer-roster-store';
...
const MAX_REQUEST_CANDIDATES = 24;
...
function executableRefresh(plan) {
  ...
  return refreshes.find((entry) => entry?.action === 'refresh_stage'
    || entry?.action === 'recover_expired_lease'
    || entry?.action === 'finalize_cached_evidence') || null;
}
...
async function keysForRequest(requestId, candidateKeys) {
  ...
  return {
    ...
    // An operational caller can feed these exact server-listed keys into the
    // next bounded call. Do not invent a cursor whose ordering could drift
    // after stage writes update row timestamps.
    deferredKeys: all.slice(MAX_REQUEST_CANDIDATES, MAX_REQUEST_CANDIDATES * 2),
  };
}
...
async function reconcileOneCandidate({ requestId, candidateKey, context, remainingBudget, deadlineAt }) {
  ...
  while (attempts < remainingBudget) {
    ...
    const next = executableRefresh(lastPlan);
    if (!next) {
...
export async function reconcileReviewerStages({ requestId, candidateKeys } = {}) {
  ...
  const continuationCandidateKeys = Array.from(new Set([
    ...candidates.filter((candidate) => candidate.outcome !== 'current').map((candidate) => candidate.candidateKey),
    ...target.deferredKeys,
  ...
  const aggregateOutcome = complete
    ...
    : currentCount > 0
      ...
      : counts.action_required
        ? 'action_required'
        : counts.failed_retryable || counts.refresh_in_progress || counts.lease_recovery_required
          ? 'failed_retryable'
