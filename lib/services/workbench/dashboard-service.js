 ...
 * predicate and Research program scope as the proposal feed. It still prefers the caller's newest active
...
function buildVisibilityFilter(includeSetAside) {
  ...
  const programs = odata.or(RESEARCH_PROGRAM_IDS.map((id) => odata.eqGuid('_akoya_programid_value', id)));
  return `(${programs}) and ${visibility}`;
}
