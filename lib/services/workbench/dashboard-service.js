 ...
 * The cycle picker derives cycles from the PD's meeting-dated proposals
 * (default = latest), with no allowlist anchor — d26Allowlist.js is retired
 * from live use (§5, S261). my-proposals.js is untouched.
 *
...
import { fetchReviewerRollup, deriveWorkRemaining, REVIEWERS_NEEDED } from '../reviewer-rollup';
 ...
 */
export async function loadDashboard({ azureEmail, profileId, cycleCode, scope, includeSetAside }) {
  ...
  return listProposals(pd, String(cycleCode), scope, includeSetAside, isSuperuser);
}

/**
 * Cycle-picker source: the cycles the PD has meeting-dated proposals in.
 * Scope does not narrow the picker — it narrows the proposal list — which
 * keeps this query cheap (PD-scoped, not a full-table scan).
 */
...
async function listProposals(pd, cycleCode, scope, includeSetAside, isSuperuser) {
