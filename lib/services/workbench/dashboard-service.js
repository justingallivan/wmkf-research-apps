 ...
 * and this organization-wide list — `defaultCycleCode` is the WORKING cycle
 * (earliest meeting on/after today) and `lastDecidedCycleCode` the newest
 * meeting before today. Neither depends on the caller's assignments or on what
 * is visible to them, so two users always open on the same cycle. `myCount`
 * still tells the client whether the caller has anything in it.
 */
