 ...
 * still scheduled/failed, and no send lease is held; a concurrent send or a
 * second rebuild no-ops on the WHERE. Deliberately discards the former PD's
 * edits and approval (approved_at/reviewed_at/edited_at → NULL): the draft is
 * re-personalized and re-enters the NEW PD's posture. scheduled_send_at is
 * deliberately NOT rebuilt — the send time derives from the immutable invite
 * date and a handoff never moves it.
 */
