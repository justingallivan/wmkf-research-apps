 ...
 * the claiming worker can defer its own row. The refusal boundary is
 * send_requested_at, NOT dynamics_email_id: an unsent draft activity is still
 * deferrable (its send never started), while a row whose send was actually
 * requested must complete instead — deferring it could double-send.
 */
...
/**
 * Record that this row is about to stamp its external claim (the reviewer
 * reminded-marker + token-rotation PATCH). Stamped BEFORE the external write
 * so a crash between the two directions is safe: claim recorded but PATCH
 * failed → harmless (the marker is absent, eligibility unaffected); PATCH
 * succeeded → the retry sees claim_committed_at and knows the marker is its
 * own, not a manual sender's. Lease-guarded; COALESCE keeps the first stamp.
 */
export async function recordScheduledEmailClaim(message) {
  const result = await sql`
    UPDATE scheduled_email_messages
       SET claim_committed_at = COALESCE(claim_committed_at, NOW()),
           updated_at = NOW()
     WHERE id = ${message.id}
       AND lease_token = ${message.lease_token}
       AND status = 'sending'
