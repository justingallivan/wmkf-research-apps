 ...
 */
export async function resolveProgramDirectorEmailForRequest(requestId) {
  ...
  try {
    ...
    if (pdGuid) {
      ...
      // this conversion wave and its getById select
      // ('systemuserid,internalemailaddress,isdisabled') doesn't byte-mirror this
      // caller's narrower select — left raw.
      const pd = await DynamicsService.getRecord('systemusers', pdGuid, {
