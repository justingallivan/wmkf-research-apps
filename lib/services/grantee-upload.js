 ...
 *   akoya_requestnum, wmkf_granteeimagefileref, _etag
 ...
 */
export async function writeGranteeDeliverables({ request, editedAbstract, caption, imageFile }) {
  const requestId = request?.akoya_requestid;
  const requestNum = request?.akoya_requestnum;
  if (!requestId || !requestNum) return { ok: false, reason: 'not_found', status: 404 };
  ...
  const hasExistingImage = Boolean(request.wmkf_granteeimagefileref);
  ...
  if (!request._etag) return { ok: false, reason: 'no_etag', status: 503 };
  ...
  const patch = {
    ...
    wmkf_granteeimagecaption: cap,
    wmkf_granteedeliverablestatus: GRANTEE_DELIVERABLE_STATUS.SUBMITTED,
  };
  if (newImageRef) patch.wmkf_granteeimagefileref = newImageRef;
