 ...
 * already verified the token, status-guarded (editable allowlist), and resolved
 * any actor-bound staged Blob into bytes; this service validates + scans + uploads the image, then
...
import { cleanupSharePointItems, cleanupSharePointItemsDetailed } from './sharepoint-cleanup.js';
...
} from '../../shared/config/granteeAbstract.js';

const REQUEST_ENTITY_SET = 'akoya_requests';

export const GRANTEE_LIBRARY = 'akoya_request';
...
function failure(reason, status, diagnostics = undefined) {
  const result = { ok: false, reason, status };
...
/**
 * The SharePoint folder a grantee image lives in, under GRANTEE_LIBRARY.
 *
 * Exported so the staff-side reader
 * (lib/services/workbench/grantee-deliverables/image-service.js) addresses the
 * same folder this writer uploads to. The stored `wmkf_imagefileref` is a
 * webUrl-or-relative-path and carries no drive/item identity, so the reader has
 * to re-derive the path — and it must derive it from THIS definition, not a
 * copy, or the two silently drift and staff get a 404 for a file that exists.
 */
export const granteeUploadFolder = (requestNum, requestId) =>
  `${requestFolderName(requestNum, requestId)}/${GRANTEE_SUBFOLDER}`;
 ...
 * @param {string} [args.waiverBodyHash] - SHA-256 hex of the waiver body the
 *   grantee saw (from the render token). Persisted for audit so a later in-place
 *   body edit of the acknowledged version is detectable. Optional.
 * @param {(candidate: Object) => Promise<void>} [args.onCandidateUploaded]
 *   Optional durability hook. When supplied it MUST persist the exact new
 *   SharePoint identity before this service attempts the Dataverse commit.
 * @returns {Promise<{ ok: true } | { ok: false, reason: string, status: number }>}
 */
export async function writeGranteeDeliverables({
  request,
  deliverable,
  editedAbstract,
  caption,
  ...
  if (abstract.length > MAX_GRANTEE_ABSTRACT_MARKDOWN_LENGTH) {
    return { ok: false, reason: 'abstract_too_long', status: 400 };
  }
  if (!cap) return { ok: false, reason: 'caption_required', status: 400 };
  if (cap.length > MAX_GRANTEE_CAPTION_MARKDOWN_LENGTH) {
    return { ok: false, reason: 'caption_too_long', status: 400 };
  }
  ...
  const folder = granteeUploadFolder(requestNum, requestId);
  ...
  if (imageFile) {
    ...
    if (isVirusScanEnabled()) {
      ...
      try {
        ...
        return e.isTransient
          ? failure('scan_unavailable', 503, {
            stage: 'virus_scan',
            service: e.serviceName || 'cloudmersive',
            transient: true,
            status: e.status || null,
          })
          : failure('scan_misconfigured', 500, {
            stage: 'virus_scan',
            service: e.serviceName || 'cloudmersive',
            transient: false,
            status: e.status || null,
      ...
      if (scan?.scan_result !== 'clean') {
        return failure('infected', 422, {
          stage: 'virus_scan',
          service: scan?.scanner || 'cloudmersive',
          scanResult: scan?.scan_result || 'unknown',
          detectedThreats: Array.isArray(scan?.detectedThreats)
            ? scan.detectedThreats.slice(0, 3)
            : [],
        });
      }
    }
    ...
    try {
      ...
      return failure('sharepoint_failed', 502, {
        stage: 'sharepoint_upload',
        service: e.serviceName || 'graph',
        transient: e.isTransient === true,
        status: e.status || null,
        uploadedItemCreated: Boolean(uploadedItem),
        cleanupAttempted: Boolean(uploadedItem),
      });
    }
    newImageRef = uploadedItem.webUrl || `${folder}/${newFilename}`;
    if (onCandidateUploaded) {
      try {
        await onCandidateUploaded({
          driveId,
          itemId: uploadedItem.id || null,
          imageRef: newImageRef,
          filename: newFilename,
          folder,
        });
      } catch (e) {
        console.error('[grantee-upload] candidate durability hook failed:', e?.message || e);
        await cleanupSharePointItems(driveId, [uploadedItem], 'grantee-upload-candidate');
        return failure('staging_failed', 503, {
          stage: 'candidate_persist',
  ...
  // Audit: persist the hash of the exact body the grantee saw, so a later in-place
  // edit of the acknowledged version's body is detectable (stored hash vs current).
  if (waiverBodyHash) deliverablePatch.wmkf_waiverbodyhash = waiverBodyHash;
  ...
  try {
    ...
    let state = uploadedItem ? 'not_committed' : 'no_upload';
    let cleanupOutcome = uploadedItem ? 'not_attempted' : 'not_applicable';
    if (uploadedItem) {
      try {
      ...
      if (state === 'not_committed') {
        const cleanup = await cleanupSharePointItemsDetailed(driveId, [uploadedItem], 'grantee-upload');
        cleanupOutcome = cleanup.failed.length === 0 ? 'deleted' : 'delete_failed';
    ...
    return failure('dataverse_failed', 502, {
      stage: 'dataverse_changeset',
      service: e.serviceName || 'dataverse',
      transient: e.isTransient === true,
      status: e.status || null,
      uploadedItemCreated: Boolean(uploadedItem),
      postErrorCommitState: state,
      cleanupOutcome,
    });
  }
