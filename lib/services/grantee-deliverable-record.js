...
export async function getDeliverableForRequest(requestId) {
  ...
  const { records } = await DynamicsService.queryRecords(GRANTEE_DELIVERABLE_ENTITY_SET, {
...
export async function ensureDeliverableForRequest(requestId, { requestNumber, actingUserSystemId } = {}) {
  ...
  try {
    return await DynamicsService.createRecord(GRANTEE_DELIVERABLE_ENTITY_SET, payload, {
      actingUserSystemId,
    });
  } catch (err) {
