 ...
 */
async function findApplicantSlotRefs(requests, loserId, keeperId) {
...
export async function executeMerge({ keeperId, loserId, fieldChoices, actingUserSystemId }, deps = {}) {
    ...
    for (const op of [...repointOps, ...clearOps]) {
      ...
      try {
        if (op.action === 'repoint') {
          ...
          await requests.disassociate(ref.requestId, navProp, { actingUserSystemId });
  ...
  if (emailMoves) {
    try {
      ...
      await pr.update(keeperId, { email: updates.email, emailSource: 'manual' }, { actingUserSystemId });
