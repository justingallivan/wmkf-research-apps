...
export async function planMerge({ keeperId, loserId }, deps = {}) {
  ...
  return {
    ...
    loser: { id: loserId, name: loser.wmkf_name, email: loser.wmkf_emailaddress, identityStatus: loser.wmkf_identitystatus ?? null },
    fields,
    repoint,
    collisions,
  };
...
function resolvePersonUpdates(fieldChoices, plan) {
  ...
  for (const f of plan.fields) {
    ...
    if (pick === 'loser' && f.differs) out[f.field] = f.loser;
  }
  return out;
}
