...
// Default (production) `dynamics` shape backed by the DAL adapters where a
// method exists. `disassociate` stays raw DynamicsService: no equivalent
// method exists on the (not-owned) grant-request adapter, so the applicant-slot
// $ref-delete call site cannot convert (data-access-layer migration tail-2B
// skip). The `deps.dynamics` injection seam itself is unchanged, so
// tests/unit/reviewer-merge-service.test.js's full fake override keeps working.
const defaultDynamicsAdapter = {
  queryAllRecords: (entity, options) => grantRequestAdapter.queryAllRequests(options),
  updateRecord: (entity, id, data, options) => (options === undefined
    ? grantRequestAdapter.updateById(id, data)
    : grantRequestAdapter.updateById(id, data, options)),
  // Literal 'akoya_requests' (not the `entity` param) — this file's only
  // disassociate call site is always akoya_requests.
  disassociate: (entity, id, navProp, options) => DynamicsService.disassociate('akoya_requests', id, navProp, options),
};
 ...
 */
async function findApplicantSlotRefs(dyn, loserId, keeperId) {
  ...
  const { records, capped } = await dyn.queryAllRecords(REQUEST_ENTITY, { select, filter });
  if (capped) {
...
export async function planMerge({ keeperId, loserId }, deps = {}) {
  ...
  const dyn = deps.dynamics || defaultDynamicsAdapter;
  ...
  const [loserSug, keeperSug, slotRepoints] = await Promise.all([
    ...
    findApplicantSlotRefs(dyn, loserId, keeperId),
  ]);
...
export async function executeMerge({ keeperId, loserId, fieldChoices, actingUserSystemId }, deps = {}) {
  ...
  for (const ref of plan.slotRepoints) {
    ...
    for (const op of [...repointOps, ...clearOps]) {
      const navProp = `wmkf_PotentialReviewer${op.slot}`;
      try {
        if (op.action === 'repoint') {
          await dyn.updateRecord(
            REQUEST_ENTITY,
            ref.requestId,
            { [`${navProp}@odata.bind`]: `/wmkf_potentialreviewerses(${keeperId})` },
            { ifMatch: ref.etag || undefined, actingUserSystemId },
          );
          summary.slotsRepointed += 1;
        } else {
          await dyn.disassociate(REQUEST_ENTITY, ref.requestId, navProp, { actingUserSystemId });
