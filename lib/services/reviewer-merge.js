...
import { isSyntheticReviewerRow } from './test-requests/isolation.js';
...
const ENGAGEMENT_SIGNAL_FIELDS = [
  ...
  'wmkf_completedat', 'wmkf_honorariumeligibility', 'wmkf_coiackedat', 'wmkf_aiuseackedat',
...
export async function planMerge({ keeperId, loserId }, deps = {}) {
  ...
  const reasons = [];
  // 6c-ii Stage A, plan item (b'): a synthetic keeper OR loser refuses the
  // merge before the first write. getByIdForMerge only selects the marker
  // while SYNTHETIC_REVIEWER_ISOLATION is on (see potential-reviewer.js); off,
  // no host can have a synthetic person, so this never fires.
  if (isSyntheticReviewerRow(keeper)) {
    reasons.push({ code: 'keeper_synthetic_reviewer', detail: 'Keeper is a synthetic (factory-created) test record and cannot be merged.' });
  }
  if (isSyntheticReviewerRow(loser)) {
    reasons.push({ code: 'loser_synthetic_reviewer', detail: 'Loser is a synthetic (factory-created) test record and cannot be merged.' });
  }
  ...
  return {
    blocked,
    reasons,
    keeper: { id: keeperId, name: keeper.wmkf_name, email: keeper.wmkf_emailaddress, identityStatus: keeper.wmkf_identitystatus ?? null, etag: keeper._etag || null, isSynthetic: isSyntheticReviewerRow(keeper) },
    loser: { id: loserId, name: loser.wmkf_name, email: loser.wmkf_emailaddress, identityStatus: loser.wmkf_identitystatus ?? null, statecode: loser.statecode ?? null, etag: loser._etag || null, isSynthetic: isSyntheticReviewerRow(loser) },
    fields,
    repoint,
    collisions,
    slotRepoints,
...
export async function executeMerge({ keeperId, loserId, fieldChoices, actingUserSystemId }, deps = {}) {
  ...
  const requests = deps.requests || grantRequestAdapter;
  // Injectable ONLY for the M7 mutation-kill test proving the synthetic-
  // reviewer re-check below is independent of planMerge's own block
  // predicate (a stubbed planMerge returning blocked:false must still be
  // refused by the standalone check). Production never overrides this.
  const planMergeFn = deps.planMerge || planMerge;

  // Re-validate against live state at execute time (fail-closed).
  const plan = await planMergeFn({ keeperId, loserId }, deps);
  if (plan.blocked) throw blockedError(plan.reasons);

  // Independent re-check (6c-ii Stage A, plan item (b')): deliberately NOT
  // derived from plan.blocked/plan.reasons above -- a second, standalone fence
  // reading the same plan.keeper/plan.loser projection so a defect in
  // planMerge's block-predicate wiring alone cannot let a synthetic keeper or
  // loser reach the first write below.
  if (plan.keeper.isSynthetic || plan.loser.isSynthetic) {
    throw blockedError([{
      code: plan.keeper.isSynthetic ? 'keeper_synthetic_reviewer' : 'loser_synthetic_reviewer',
      detail: 'A synthetic (factory-created) test record cannot be merged.',
    }]);
  }
