...
export async function finalizeAttachmentUpload({
  ...
  if (existingRecords.length === 1) {
    ...
    const uploaded = await dependencies.uploadFile(bucket.library, folderPath, filename, file.buffer, file.mimeType, { conflictBehavior: 'rename' });
    ...
    // between them leaves a discoverable orphan for the staging cleanup
    // sweep. Residual (accepted): a crash in exactly this window followed by
    // a client retry re-uploads under 'rename', producing a second, harmless
    // SharePoint item; the sweep's consultant_feedback proof (no registry row
    // for THIS candidate's generation key) reclaims the earlier orphan as
    // unbound-with-no-registry-row and discards it.
    ...
    createdFilename = uploaded.name || filename;
  }
  ...
  try {
    ...
    const isConflict = error instanceof ServiceHttpError
      && (error.body?.reason === 'attachment_conflict' || error.code === 'attachment_conflict');
    if (!isConflict) {
      // Any OTHER bind failure — eligibility (400), the target entry gone
      // (404), a transient Postgres error — leaves the registry row Ready
      // and unbound. The staging row is released (not rejected) by the
      // caller for these codes, and a retry with the SAME staging id finds
      // this exact row again by generation key and performs only step 7:
      // superseding it here would strand a live SharePoint item behind a
      // Superseded row that a retry would then bind to as dead (the bug this
      // fix closes).
      throw error;
    }
    // Loser-side cleanup (§4 step 7, `attachment_conflict` ONLY): another
    // finalize or a delete won the race — the registry row this finalize
    ...
    // staging row on this error code, so there is no retry to protect here.
    try {
    ...
    throw attachmentError('This entry already has an attachment.', 'attachment_conflict', 409);
  }
}
