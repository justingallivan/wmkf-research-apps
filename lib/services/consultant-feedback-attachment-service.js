 ...
 * `actorPolicy: REQUEST_DOCUMENT_ACTOR_POLICY.ALLOW_UNATTRIBUTED` with the
 * uploading staff member's session `actingUserSystemId`, so the row records
 * its explicit origin actor. A staff member with no linked or enabled
 * Dataverse identity still uploads; that row stays unattributed and records
 * the bounded missing-actor event. Staff identity is also on the Postgres
 * `consultant_feedback` row's `created_by`/`updated_by`.
 */
...
export async function finalizeAttachmentUpload({
  requestId, actorProfileId, actingUserSystemId = null, file, stagingId, entryId = null, newEntry = null,
}, dependencies = DEFAULT_DEPENDENCIES) {
  if (!isGuid(requestId)) throw badRequest('requestId must be a GUID.', 'invalid_request_id');
  ...
  if (existingRecords.length === 1) {
    ...
    const created = await dependencies.createDocument({
      ...
      actingUserSystemId,
      actorContext: { operation: 'consultant-feedback-attachment', requestId: request.akoya_requestid },
    });
