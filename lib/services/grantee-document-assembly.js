 ...
 * This applies to the exported `resolveByline` too, which staff surfaces call
 * on its own to show the PI/Co-PI names before they publish.
 */
...
/**
 * The `$select` fragment a caller needs so `resolveByline` can read the PI. The
 * name arrives on the lookup's `_formatted` annotation, not the raw value.
 */
export const BYLINE_REQUEST_SELECT = '_wmkf_projectleader_value';

/**
 * Derive the published byline — PI plus Co-PIs — for one request.
 *
 * SINGLE PRODUCER. The grantee portal, website HTML, cycle export, and the
 * Awardee-tab pre-send check all render this byline, so they must all derive it
 * here. A staff surface that re-implemented the join or read a different PI
 * field could show a name the published document does not use, which would make
 * a visual check pass while the grantee still receives the wrong one.
 *
 * @param {object} row - an akoya_request row selected with BYLINE_REQUEST_SELECT
 * @param {string} requestId - akoya_request GUID
 * @returns {Promise<{pi: string|null, coPIs: string[], names: string|null}>}
 */
export async function resolveByline(row, requestId) {
  const pi = row?.['_wmkf_projectleader_value_formatted'] || null;
  const coPIs = await fetchCoPIs(requestId);
  return { pi, coPIs, names: joinNames([pi, ...coPIs]) };
}
...
export async function assembleGranteeDocument(requestId, { includeImageRef = false } = {}) {
  ...
  if (applicantId) {
    try {
      const acct = await accountAdapter.getById(applicantId, { select: ACCOUNT_SELECT });
  ...
  const { pi, coPIs, names } = await resolveByline(row, requestId);
  ...
  const caption = typeof deliverable?.wmkf_imagecaption === 'string' && deliverable.wmkf_imagecaption.trim()
    ? deliverable.wmkf_imagecaption
    : null;
  const imageFileRef = deliverable?.wmkf_imagefileref || null;
