...
import { isVirusScanEnabled, VIRUS_DETECTION_ALERT_EMAIL } from '../utils/virus-scan-config.js';
...
export async function writeReviewFiles({ suggestionId, files, structuredData, opts }) {
  ...
  if (isVirusScanEnabled()) {
    ...
    if (scanFailure) {
      // Detection alerts only fire for 'infected'. scan_misconfigured /
      // scan_unavailable are operational issues that surface via existing
      // operator alert channels — they're not events to route to a PD.
      if (scanFailure.reason === 'infected') {
        await fireReviewDetectionAlert({
          suggestionId,
          source: opts?.source || 'unknown',
          files,
          infectedErrors: scanFailure.errors || [],
        }).catch(err => {
 ...
 */
async function fireReviewDetectionAlert({ suggestionId, source, files, infectedErrors }) {
  let suggestion;
  try {
    suggestion = await DynamicsService.getRecord(
      'wmkf_appreviewersuggestions',
  ...
  await NotificationService.notify({
    ...
    // explicitRecipients bypasses category routing. We always include the
    // foundation alerts address; the PD email is added when resolvable.
    // Deduped so a PD whose mailbox is alerts@ doesn't get two copies.
    explicitRecipients: Array.from(new Set(
      [VIRUS_DETECTION_ALERT_EMAIL, pdEmail].filter(Boolean),
    )),
  });
}
