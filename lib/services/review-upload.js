...
export async function writeReviewFiles({ suggestionId, files, structuredData, opts }) {
  ...
  try {
    ...
    if (ratingRows.length === 0) {
      ...
      const parent = { method: 'PATCH', entitySet: SUGGESTION_ENTITY_SET, key: suggestionId, body: dvPatch };
      await runChangeset(atomicParentWithChildren({ parent, children }), actingOpts);
    }
  } catch (e) {
