...
export async function writeReviewFiles({ suggestionId, files, structuredData, opts }) {
  ...
  try {
    ...
    // Roll back only files owned by this unique attempt. On a 412, also compare
    // drive-item ids with the folder now referenced by the winning row. The
    // unique path should make overlap impossible under Graph semantics, but the
    // defensive comparison prevents a shared/replaced identity from being
    // deleted if Graph ever returns one (or a test double exposes the hazard).
    let cleanedUp;
    if (current?.wmkf_reviewsharepointfolder) {
      try {
        ...
        // Fail safe: an orphan losing attempt is preferable to deleting the file
        // referenced by the winning Dataverse receipt.
        cleanedUp = false;
        console.error(
