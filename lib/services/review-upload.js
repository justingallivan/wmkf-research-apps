...
async function runVirusScans(files) {
  ...
  if (misconfiguredErr) {
    return { ok: false, reason: 'scan_misconfigured', infectedErrors: hasInfected ? infectedErrors : [] };
  }
  if (unavailableErr) {
    return { ok: false, reason: 'scan_unavailable', infectedErrors: hasInfected ? infectedErrors : [] };
  }
  if (hasInfected) {
    return { ok: false, reason: 'infected', errors: infectedErrors, infectedErrors };
  }
  return null;
}
