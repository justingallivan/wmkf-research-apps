...
import { buildRatingSnapshotRows, answerRowUrl, answerRowBody } from '../external/review-answer-snapshot.js';
...
export async function writeReviewFiles({ suggestionId, files, structuredData, opts }) {
  ...
  try {
    suggestion = await DynamicsService.getRecord(
      ...
      suggestionId,
      {
        select: 'wmkf_appreviewersuggestionid,_wmkf_request_value,wmkf_reviewstatus',
        expand:
  ...
  // Phase D: dual-write the rating snapshot rows atomically with the parent
  // PATCH so the child table is the complete system of record before the
  // DTO/prefill readers stop reading the parent rating columns. The upload path
  // always carries the three ratings (non-partial validation above), so all
  // three rows are written; the narrative answers live in the uploaded PDF, not
  // the form, so they are never snapshotted here.
  ...
  try {
    const actingOpts = opts?.actingUserSystemId ? { actingUserSystemId: opts.actingUserSystemId } : {};
    if (ratingRows.length === 0) {
      await DynamicsService.updateRecord('wmkf_appreviewersuggestions', suggestionId, dvPatch, actingOpts);
    } else {
      ...
      operations.push({ method: 'PATCH', url: `wmkf_appreviewersuggestions(${suggestionId})`, body: dvPatch });
      await DynamicsService.executeChangeset(operations, actingOpts);
    }
  } catch (e) {
    // Roll back the SharePoint writes — we don't want orphan files when the
    // canonical pointer record never got updated.
    const cleanedUp = await cleanupSharePointItems(driveId, uploaded, 'review-upload');
    return {
      ok: false,
