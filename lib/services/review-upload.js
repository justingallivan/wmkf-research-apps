...
import { getActiveQuestionSet } from '../external/review-question-fetcher.js';
import { buildRatingSnapshotRows } from '../external/review-answer-snapshot.js';
...
export async function writeReviewFiles({ suggestionId, files, structuredData, opts }) {
  ...
  // always carries the three ratings (non-partial validation above), so all three
  // rows are written; narrative answers live in the uploaded PDF, not the form.
  ...
  try {
    ...
    if (is412) {
      try {
        current = await getForExternalVerification(suggestionId, {
          select:
            'wmkf_appreviewersuggestionid,wmkf_reviewreceivedat,wmkf_reviewstatus,'
    ...
    // - 412: a concurrent upload won. Because Graph `conflictBehavior=replace`
    //   can return a SHARED drive-item id for two same-name uploads, deleting
    //   our items could delete the file the winner's receipt now references.
    //   Only delete when we positively identified the winner's folder AND
    //   listed it, excluding any id that appears there. If the winner re-read
    //   failed (current null), the row carries no folder yet, or the listing
    //   throws, we CANNOT prove ownership — orphan the losing attempt rather
    //   than risk deleting the winner (Codex S369 fourth-pass finding).
    let cleanedUp;
    if (!is412) {
      cleanedUp = await cleanupSharePointItems(driveId, uploaded, 'review-upload');
    } else if (current?.wmkf_reviewsharepointfolder) {
      try {
        const winningItems = await GraphService.listFiles(
          REVIEW_LIBRARY,
          current.wmkf_reviewsharepointfolder,
        );
        const winningIds = new Set(winningItems.map((item) => item.id).filter(Boolean));
        const losingItems = uploaded.filter((item) => !winningIds.has(item.id));
        cleanedUp = await cleanupSharePointItems(driveId, losingItems, 'review-upload');
      } catch (cleanupGuardError) {
        cleanedUp = false;
        console.error(
          `[review-upload] could not verify winning SharePoint items before cleanup: ${cleanupGuardError.message}`,
        );
      }
    } else {
      // 412 but winner ownership unverifiable — orphan, never blind-delete.
      cleanedUp = false;
      console.error(
        `[review-upload] 412 with no verifiable winning folder for ${suggestionId}; leaving losing attempt orphaned rather than risk deleting the winner`,
      );
    }
    return {
      ok: false,
