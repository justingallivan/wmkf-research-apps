    ...
    for (let i = 0; i < suggestions.length; i++) {
      ...
      if (finalArticles.length >= this.MIN_PUBLICATIONS) {
        ...
        if (institutionMismatch) {
          demotionReasons.push('Suggested institution does not match PubMed affiliation');
        }
        if (expertiseMismatchResult.hasMismatch) {
          demotionReasons.push('Claimed expertise terms were not found in matched publications');
        }
