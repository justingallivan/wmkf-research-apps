...
// DiscoveryService is a thin FACADE over the per-cluster modules in ./discovery/* (the S335
// decomposition — docs/DISCOVERY_SERVICE_DECOMPOSITION_PLAN.md). All Stage-2 logic lives in those
// modules; this file keeps the `discover` orchestrator, the static constants (re-exposed from
// ./discovery/constants so DiscoveryService.<CONST> reads keep resolving and MIN_PUBLICATIONS stays
// a runtime-overridable own prop — C1), and one delegating static wrapper per moved method so every
// existing DiscoveryService.method() call site (routes, scripts, tests) keeps working unchanged.
// DeduplicationService is the only external service the orchestrator itself still touches.
...
class DiscoveryService {
  ...
  static async discover(analysisResult, options = {}) {
    const {
      ...
      piInstitutions = null,
      // Request-local, privileged diagnostic observer. The route supplies this
      // only after a fresh superuser check; it is never persisted by discovery.
      onIdentityComparison = null,
    } = options;
    ...
    if (reviewerSuggestions && reviewerSuggestions.length > 0) {
      const verificationResults = await this.verifyClaudeSuggestions(
        ...
        { searchPubmed, proposalInfo, signal, onIdentityComparison }
      );
    ...
    const coiInstitutions = (Array.isArray(piInstitutions) && piInstitutions.length)
      ? piInstitutions
      : authorInstitution;
    const conflictPartition = await DeduplicationService.partitionConflictsResolved(
      deduplicated,
      coiInstitutions,
      [],
      { signal },
  ...
  static collectAffiliationAssertions(articles, nameVariants) {
    return affiliation.collectAffiliationAssertions(articles, nameVariants);
  }
   ...
   */
  static async checkCoauthorHistory(candidateName, proposalAuthors, options = {}) {
    return coauthorCoi.checkCoauthorHistory(candidateName, proposalAuthors, options);
  }
  ...
  static async checkCoauthorshipsForCandidates(
    candidates,
    proposalAuthors,
    onProgress = () => {},
    options = {}
  ) {
    return coauthorCoi.checkCoauthorshipsForCandidates(
      candidates,
      proposalAuthors,
      onProgress,
      options
  ...
  static summarizeCoauthorChecks(candidates) {
    return coauthorCoi.summarizeCoauthorChecks(candidates);
  }
