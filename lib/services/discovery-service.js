...
class DiscoveryService {
  ...
  static VERIFICATION_SKIPPED_REASON = 'Verification skipped — PubMed off / no verifier for this field.';
  static TRACK_B_IDENTITY_RESOLUTION_LIMIT = 25;
  ...
  static async discover(analysisResult, options = {}) {
    ...
    const qualifiedBeforeIdentity = filtered.filter(c =>
    ...
    const rankedForIdentity = DeduplicationService.rankByRelevance(qualifiedBeforeIdentity, proposalKeywords);
   ...
   */
  static dedupePublicationsByTitle(publications, { limit } = {}) {
    ...
    for (const pub of (Array.isArray(publications) ? publications : [])) {
      ...
      if (!chosen.has(key)) {
        chosen.set(key, pub);
        order.push(key);
      } else if (this._isPreprintPublication(chosen.get(key)) && !this._isPreprintPublication(pub)) {
  ...
  static OPENALEX_PUB_BACKFILL_LIMIT = 5;
  static OPENALEX_PUB_BACKFILL_CONCURRENCY = 4;
  ...
  static async backfillOpenAlexPublications(candidates, { signal } = {}) {
    const isTrusted = (c) => c?.verified === true
      || c?.identityStatus === 'confirmed' || c?.identityStatus === 'probable'
      || c?.verificationStatus === this.VERIFICATION_STATUSES.VERIFIED
      || c?.verificationStatus === this.VERIFICATION_STATUSES.PROBABLE;
    ...
    const limit = this.OPENALEX_PUB_BACKFILL_LIMIT;
    const concurrency = this.OPENALEX_PUB_BACKFILL_CONCURRENCY;
      ...
      await Promise.all(batch.map(async (c) => {
        ...
        try {
          ...
          const mapped = (Array.isArray(records) ? records : [])
            ...
            .filter((p) => p.title);
          const pubs = this.dedupePublicationsByTitle(mapped, { limit });
          if (pubs.length > 0) {
            c.publications = pubs;
            if (!Number.isFinite(c.publicationCount5yr)) {
              c.publicationCount5yr = this.countRecentPublications(pubs);
            }
          }
        } catch (err) {
          if (!signal?.aborted) {
  ...
  static async verifyClaudeSuggestions(suggestions, onProgress = () => {}, options = {}) {
    ...
    const verificationContract = this.pubMedVerificationContract(options);

    if (!verificationContract.enabled) {
      console.log(`[Verification] Skipping PubMed verification of ${normalizedSuggestions.length} candidates: ${verificationContract.reason}`);
    ...
    for (let i = 0; i < normalizedSuggestions.length; i++) {
      ...
      if (finalArticles.length >= this.MIN_PUBLICATIONS) {
        ...
        // Collapse preprint+published duplicates of the same paper (PubMed indexes some
        // bioRxiv preprints, so a candidate's article list can carry both) before building the
        // display list and the recency count, so the card doesn't show the same paper twice and
        // publicationCount5yr doesn't double-count it.
        const dedupedFinalArticles = this.dedupePublicationsByTitle(finalArticles);
        ...
        const candidate = withReviewerProvenance({
          ...
          publications: dedupedFinalArticles.slice(0, 5).map(a => ({
            title: a.title,
            year: a.year,
            pmid: a.pmid,
            journal: a.journal,
          ...
          publicationCount5yr: this.countRecentPublications(dedupedFinalArticles),
  ...
  static mapSpineVerificationResult(suggestion, spineResult = {}) {
    const status = spineResult.resolverStatus || spineResult.status || this.VERIFICATION_STATUSES.UNRESOLVED;
    const sourceNames = [];
    ...
    if (status === 'confirmed' || status === 'probable') {
      ...
      const candidate = withReviewerProvenance({
        ...
        source: suggestion.source,
      }, this.provenanceOriginForSpineSuggestion(suggestion, sourceNames, true));
      return { verified: true, candidate };
    }

    const reason = spineResult.reason || this.VERIFICATION_SKIPPED_REASON;
    const candidate = withReviewerProvenance({
      ...suggestion,
      verified: false,
      verificationStatus: this.VERIFICATION_STATUSES.UNRESOLVED,
      identityStatus: status === 'ambiguous' ? this.VERIFICATION_STATUSES.AMBIGUOUS : this.VERIFICATION_STATUSES.UNRESOLVED,
      ...
      identityNote: spineResult.identityNote || null,
      source: suggestion.source,
    }, this.provenanceOriginForSpineSuggestion(suggestion, status === 'ambiguous' ? ['openalex'] : [], false));
  ...
  static isCrossFieldDiscoveredContamination(proposalInfo = {}, candidate = {}) {
    ...
    return this.isClearlyNonBiomedicalVerifierArea(proposalInfo?.primaryResearchArea);
  }
  ...
  static isClearlyNonBiomedicalVerifierArea(primaryResearchArea) {
    return this.isPhysicalOrEngineeringResearchArea(primaryResearchArea)
      && !this.isClearlyBiomedicalResearchArea(primaryResearchArea);
  }
  ...
  static evaluateCrossFieldNamesakeGuard({ proposalInfo, articles, expertiseMatch, expertiseMismatchResult }) {
    ...
    const nonBiomedical = primaryResearchArea && !this.isClearlyBiomedicalResearchArea(primaryResearchArea);
    ...
    const crossFieldBiomedicalMatch = this.isPhysicalOrEngineeringResearchArea(primaryResearchArea)
      && this.articlesLookBiomedicalOrClinical(articles);
  ...
  static evaluateVerificationIncoherence({ institutionMismatch, expertiseMismatchResult, verificationStatus }) {
    const reasons = [];
    if (verificationStatus === this.VERIFICATION_STATUSES.VERIFIED && institutionMismatch) {
      reasons.push('institution_mismatch');
    }
    if (verificationStatus === this.VERIFICATION_STATUSES.VERIFIED && expertiseMismatchResult?.hasMismatch) {
