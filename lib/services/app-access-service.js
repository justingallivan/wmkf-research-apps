 ...
 *   listAllGrantsForAdmin(options?)                    → [{ user_profile_id, user_name, azure_email, apps }]
...
async function listAppKeysForUser(profileId, options) {
  return getDataverse().listAppKeysForUser(profileId, options);
}

async function listAllGrantsForAdmin(options) {
  return getDataverse().listAllGrantsForAdmin(options);
}
