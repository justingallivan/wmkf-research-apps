 ...
 *   listAllGrantsForAdmin({ throwOnError? }) → [{ userProfileId, userName, azureEmail, apps: string[] }]
 *   grantApps(profileId, appKeys, grantedBy) → { granted: string[], error? }
...
async function listAllGrantsForAdmin({ throwOnError = false } = {}) {
  try {
    assertDataverseAccess('app-access:read');
