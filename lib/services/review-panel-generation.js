...
import { projectSeatQuestionSet, buildSeatValidationSchema, chairInput } from './review-panel-questions';
...
export async function snapshotConfiguration({ seats = REVIEW_PANEL_SEATS.filter((s) => s.enabled) } = {}) {
  ...
  const [seatRow, chairRow, fullQuestionSet] = await Promise.all([
  ...
  for (const seat of reviewerSeats) {
    ...
    seatConfigs[seat.key] = {
      seatKey: seat.key, provider, model, pricing,
      promptSnapshot: withValidationSchema({ ...seatRow, wmkf_ai_model: model }, validationSchema),
    };
  }
  ...
  const chairConfig = {
    ...
    promptSnapshot: { ...chairRow, wmkf_ai_model: chairModel },
  };
...
export async function runSeat(input, seatConfig, { attemptId, dispatchToken, deadlineMs = null, signal = null, execute = executePrompt } = {}) {
  ...
  const overrideVariables = { proposal_narrative: input.narrative.text };
  let result;
  try {
...
/** Run the chair. Input is the narrative plus each winning seat's structured review with teamCapacity omitted (D9). `seatWinnerReviews` is `{ seatKey: parsedReviewObject }`. */
export async function runChair(input, seatWinnerReviews, chairConfig, { attemptId, dispatchToken, deadlineMs = null, signal = null, execute = executePrompt } = {}) {
  ...
  const overrideVariables = {
    ...
    seat_reviews: JSON.stringify(chairInput(seatWinnerReviews)),
  };
  let result;
  try {
    result = await execute({
