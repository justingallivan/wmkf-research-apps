...
export function estimateReservationCost(config, entryCount = 1) {
  ...
  // Each seat's review is bounded by that SAME seat's own configured
  // wmkf_ai_maxtokens (the actual output-token ceiling used for seatCost
  // above), not a separate guessed constant.
  const chairInTokens = (NARRATIVE_INPUT_CEILING_TOKENS
    + seats.reduce((sum, seat) => sum + Number(seat.promptSnapshot?.wmkf_ai_maxtokens || 0), 0)) / 1000;
