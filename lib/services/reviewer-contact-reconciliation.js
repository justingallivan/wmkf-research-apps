...
function sortPriorRequests(a, b) {
  const aDate = priorRequestSortKey(a);
  const bDate = priorRequestSortKey(b);
...
function sourceCompletedWithinLimit(source, limit) {
  if (!source.ok || source.value?.hasMore === true) return false;
  const records = Array.isArray(source.value?.records) ? source.value.records : [];
  if (Number.isFinite(source.value?.totalCount)) {
    return source.value.totalCount === records.length;
  }
...
export async function loadPriorRequestContext(potentialReviewerId, currentRequestId, {
  ...
  let complete = sourceCompletedWithinLimit(suggestionSource, PRIOR_REQUEST_SOURCE_LIMIT)
    && sourceCompletedWithinLimit(legacySource, PRIOR_REQUEST_SOURCE_LIMIT);
...
export async function reconcileReviewerContacts(candidates, {
  ...
  const checkedAt = now();
  // Share one optional-history budget across the batch, starting only when the
  // first eligible person reaches the history read.
  let priorRequestDeadlineAt = null;
  ...
  for (const candidate of list) {
    ...
    try {
      ...
      if (mayLoadPriorRequestContext) {
        if (priorRequestDeadlineAt === null) {
          priorRequestDeadlineAt = Date.now() + Math.max(0, priorRequestBudgetMs);
        }
        const remainingBudgetMs = Math.max(0, priorRequestDeadlineAt - Date.now());
