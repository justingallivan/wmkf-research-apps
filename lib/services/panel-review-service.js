...
export class PanelReviewService {
   ...
   * dropped, types and lengths bounded (the anti-injection property), with the
   * `parseJSONResponse` `_truncated` marker re-attached if present. On a
  ...
  static _validateStageOutput(parsed, schemaKey, label) {
    ...
    if (parsed._truncated) r.value._truncated = true;
    return r.value;
  }
