...
export async function previewManualReminder({ kind, requestId, suggestionId, actingUserSystemId, template } = {}) {
  ...
  if (!personal.ok) return { ok: false, reason: personal.reason };
  const resolved = template === undefined ? personal.template : template;
  const checked = validateReminderTemplate(kind, resolved);
