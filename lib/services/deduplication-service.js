...
const LOW_TRUST_COI_AFFILIATION_SOURCES = new Set([
  'openalex_current',
  'pubmed_recency',
  'scholar_current',
  'pubmed_additional',
...
const INSTITUTION_COI_MATCH_KIND = Object.freeze({
  NONE: 'none',
  DIRECT: 'direct',
  EXEMPT: 'exempt',
});
const COI_EXEMPT_OPENALEX_IDS = new Map([
  ['I1344073410', 'hhmi'],
  ['I107606265', 'broad'],
]);
const COI_EXEMPT_RORS = new Map([
  ['https://ror.org/006w34k90', 'hhmi'],
  ['https://ror.org/05a0ya142', 'broad'],
]);
const COI_EXEMPT_NAMES = new Map([
  ['hhmi', 'hhmi'],
  ['howard hughes medical institute', 'hhmi'],
  ['broad institute', 'broad'],
  ['the broad institute', 'broad'],
]);

function affiliationAssertionKey(assertion) {
  ...
  const normalize = (value) => String(value || '')
    ...
    .replace(/\s+/g, ' ')
    .toLowerCase();
  const sourceType = normalize(assertion.sourceType);
  const rawText = normalize(assertion.rawText);
  if (!sourceType || !rawText) return null;
  return [sourceType, rawText, normalize(assertion.sourceReference)].join('|');
}

function strongerAffiliationAssertion(existing, incoming) {
  const currentnessRank = { historical: 0, unknown: 1, current: 2 };
  return (currentnessRank[incoming?.currentness] ?? -1)
    > (currentnessRank[existing?.currentness] ?? -1)
    ? incoming
    : existing;
}
...
class DeduplicationService {
  ...
  static async mergeGroup(group) {
    ...
    // Distinct legacy affiliation strings plus source/date-bound Phase 2
    // assertions. The expanded COI consumer remains behind its server flag.
    ...
    const affiliationAssertionsByKey = new Map();
    for (const assertion of group.flatMap((candidate) => (
      Array.isArray(candidate.affiliationAssertions) ? candidate.affiliationAssertions : []
    ))) {
      const key = affiliationAssertionKey(assertion);
      if (!key) continue;
      const existing = affiliationAssertionsByKey.get(key);
      affiliationAssertionsByKey.set(
        key,
        existing ? strongerAffiliationAssertion(existing, assertion) : assertion,
      );
    }
    const affiliationAssertions = Array.from(affiliationAssertionsByKey.values());
    const affiliationAssertionsComplete = group.every(
      (candidate) => candidate.affiliationAssertionsComplete !== false,
    );
  ...
  /**
   * Async W1 counterpart used by production paths. W0 resolution may only
   * confirm or narrow a lexical/direct match; it never creates a new hard drop.
   */
  static async markInstitutionCOIResolved(
    researchers,
    authorInstitution,
    { resolver = createInstitutionIdentityResolver(), signal } = {},
  ) {
    const institutions = (Array.isArray(authorInstitution) ? authorInstitution : [authorInstitution])
      .filter(Boolean);
    if (institutions.length === 0) return researchers;

    const piInstitutions = institutions.map((institution) => ({
      raw: institution,
      display: this.institutionDisplayName(institution),
      identity: this.institutionInput(institution),
    }));
    return Promise.all((Array.isArray(researchers) ? researchers : []).map(async (researcher) => {
      const decision = await this.institutionCOIDecisionResolved(researcher, piInstitutions, {
        resolver,
        signal,
      });
      if (decision) return decision.candidate;
      return {
        ...researcher,
        hasInstitutionCOI: false,
        institutionCOIExempt: false,
  ...
  static partitionConflicts(researchers, authorInstitution, excludeNames = []) {
    ...
    const filtered = list.filter(researcher => {
      ...
      if (conflict) {
        if (conflict.dropDecision === 'flagged' || conflict.dropDecision === 'exempt') {
  ...
  /**
   * W1 production partition. Preserves the synchronous return contract after
   * awaiting W0 identity resolution. Resolution can refute a lexical match or
   * classify an approved exemption, but cannot turn a lexical non-match into a
   * new hard COI.
   */
  static async partitionConflictsResolved(
    researchers,
    authorInstitution,
    excludeNames = [],
    { resolver = createInstitutionIdentityResolver(), signal } = {},
    ...
    const decisions = await Promise.all(list.map((researcher) => (
      piInstitutions.length > 0
        ? this.institutionCOIDecisionResolved(researcher, piInstitutions, { resolver, signal })
        : null
    )));
    ...
    for (let index = 0; index < list.length; index += 1) {
      const researcher = list[index];
      const conflict = decisions[index];
      if (conflict) {
        if (conflict.dropDecision === 'flagged' || conflict.dropDecision === 'exempt') {
          institutionFlagged.push(conflict.candidate);
        } else {
      ...
      filtered.push(conflict?.candidate || researcher);
    }
  ...
  static async resolveInstitutionInputForCOI(
    institution,
    { resolver = createInstitutionIdentityResolver(), signal } = {},
  ) {
    const input = this.institutionInput(institution);
    const ids = this.institutionIds(input);
    if (ids.openAlex || ids.ror || !input.name) return input;

    const resolved = await resolver.resolve(input.name, { signal });
    if (!resolved) return input;
    return this.institutionInput({
      name: input.name || resolved.displayName,
      openAlexId: resolved.openAlexId,
      ror: resolved.ror,
    });
  }

  static async resolveInstitutionEntriesForCOI(
    institutions,
    { resolver = createInstitutionIdentityResolver(), signal } = {},
  ) {
    return Promise.all((Array.isArray(institutions) ? institutions : [])
      .filter(Boolean)
      .map(async (institution) => {
        const wrapped = institution
          && typeof institution === 'object'
          && Object.prototype.hasOwnProperty.call(institution, 'identity')
          ? institution
          : {
            raw: institution,
            display: this.institutionDisplayName(institution),
        ...
        return {
          ...wrapped,
          identity: await this.resolveInstitutionInputForCOI(wrapped.identity, {
            resolver,
            signal,
          }),
        };
  ...
  static institutionSignalsForCandidate(candidate = {}, { includeAdditionalAffiliations = false } = {}) {
    const ce = candidate.contactEnrichment || {};
    const signals = [];
    ...
    if (includeAdditionalAffiliations) {
      for (const assertion of Array.isArray(candidate.affiliationAssertions)
        ? candidate.affiliationAssertions
        : []) {
        if (assertion?.authorSpecific !== true
          || assertion?.currentness !== 'current'
          || !assertion?.rawText) continue;
        add({
          value: assertion.rawText,
          source: assertion.sourceType === 'orcid_employment'
            ? 'orcid_additional'
            : 'pubmed_additional',
          ror: assertion.ror || null,
          openAlexId: assertion.openAlexId || null,
        });
      }
    }
  ...
  static institutionCOIDecision(candidate = {}, piInstitutions = [], options = {}) {
    const signals = this.institutionSignalsForCandidate(candidate, options);
    if (signals.length === 0) return null;
    return this.institutionCOIDecisionFromSignals(candidate, signals, piInstitutions);
  }

  static institutionCOIDecisionFromSignals(candidate, signals, piInstitutions) {
    const matches = [];
    const exemptions = [];
    for (const signal of signals) {
      let exempt = null;
      for (const hit of piInstitutions) {
        const matchKind = this.institutionCOIMatchKind(hit.identity, signal.input);
        if (matchKind === INSTITUTION_COI_MATCH_KIND.DIRECT) {
        ...
        if (matchKind === INSTITUTION_COI_MATCH_KIND.EXEMPT && !exempt) {
          exempt = {
            signal,
            hit,
            exemptInstitution: this.coiExemptInstitutionKey(hit.identity)
              || this.coiExemptInstitutionKey(signal.input),
          };
        }
      }
    ...
    if (matches.length === 0) {
      ...
      return {
        ...
        candidate: {
          ...candidate,
          // Keep the signal visible to staff while the explicit `exempt`
          // decision prevents it from becoming an automatic hard drop.
          ...
          institutionCOIDetails: {
            ...
            contradictoryAffiliationSource: null,
            exemptInstitution: selected.exemptInstitution,
          },
        },
      };
    }
    ...
    const nonMatchingCurrentSignals = signals.filter((signal) =>
      ...
      && piInstitutions.every((inst) =>
        this.institutionCOIMatchKind(inst.identity, signal.input) === INSTITUTION_COI_MATCH_KIND.NONE)
    );
  ...
  static institutionCOILexicalDecision(candidate = {}, piInstitutions = [], options = {}) {
    const signals = this.institutionSignalsForCandidate(candidate, options).map((entry) => ({
      ...entry,
      input: this.institutionInput({ name: this.institutionDisplayName(entry.input) }),
    }));
    const lexicalPiInstitutions = (Array.isArray(piInstitutions) ? piInstitutions : [])
      .filter(Boolean)
      .map((institution) => {
        const wrapped = institution
          && typeof institution === 'object'
        ...
        return {
          ...wrapped,
          identity: this.institutionInput({
            name: this.institutionDisplayName(wrapped.identity),
          }),
        };
      });
    if (signals.length === 0) return null;
    return this.institutionCOIDecisionFromSignals(
      candidate,
      signals,
      lexicalPiInstitutions,
    );
  }

  /**
   * Classify and resolve only a pre-existing lexical/direct COI candidate.
   * The explicit status lets save-time callers distinguish a true lexical
   * non-match from a lexical match positively refuted by institution ids.
   * Provider abstention retains the pre-resolution decision.
   */
  static async institutionCOIResolution(
    candidate = {},
    piInstitutions = [],
    { resolver = createInstitutionIdentityResolver(), signal, includeAdditionalAffiliations = false } = {},
  ) {
    const signalOptions = { includeAdditionalAffiliations };
    const lexicalDecision = this.institutionCOILexicalDecision(candidate, piInstitutions, signalOptions);
    if (!lexicalDecision) {
      return { status: 'lexical_non_match', decision: null };
    }

    const rawDecision = this.institutionCOIDecision(candidate, piInstitutions, signalOptions);
    if (!rawDecision) {
      return { status: 'refuted_by_existing_ids', decision: null };
    }

    const signals = this.institutionSignalsForCandidate(candidate, signalOptions);
    const [resolvedSignals, resolvedPiInstitutions] = await Promise.all([
      Promise.all(signals.map(async (entry) => ({
        ...entry,
        input: await this.resolveInstitutionInputForCOI(entry.input, { resolver, signal }),
      }))),
      this.resolveInstitutionEntriesForCOI(piInstitutions, { resolver, signal }),
    ]);
    const decision = this.institutionCOIDecisionFromSignals(
      candidate,
      resolvedSignals,
      resolvedPiInstitutions,
    );
    return {
      status: decision ? 'matched' : 'refuted_by_resolved_ids',
      decision,
    };
  }

  /**
   * Compatibility wrapper for callers that only consume the final decision.
   * The no-widening firewall remains in institutionCOIResolution.
   */
  static async institutionCOIDecisionResolved(
    candidate = {},
    piInstitutions = [],
    options = {},
  ) {
    const outcome = await this.institutionCOIResolution(candidate, piInstitutions, options);
  ...
  static coiExemptInstitutionKey(input) {
    const ids = this.institutionIds(input);
    const idMatch = (ids.openAlex && COI_EXEMPT_OPENALEX_IDS.get(ids.openAlex))
      || (ids.ror && COI_EXEMPT_RORS.get(ids.ror))
      || null;
    if (idMatch) return idMatch;
    const name = this.normalizeInstitution(this.institutionDisplayName(this.institutionInput(input)));
    return COI_EXEMPT_NAMES.get(name) || null;
  }
  ...
  /**
   * Direct same-institution comparison without the W1 exemption overlay.
   * Associated institutions are deliberately absent: they are consistency
   * evidence only and must never widen the hard-drop set.
   */
  static institutionDirectMatch(inst1, inst2) {
    if (!inst1 || !inst2) return false;
  ...
  /**
   * Three-way COI comparison. HHMI and Broad are explicit owner-approved
   * exemptions when the shared institute is the only institutional overlap.
   */
  static institutionCOIMatchKind(inst1, inst2) {
    if (!inst1 || !inst2) return INSTITUTION_COI_MATCH_KIND.NONE;
    const ids1 = this.institutionIds(inst1);
    const ids2 = this.institutionIds(inst2);
    if (ids1.openAlex && ids2.openAlex && ids1.openAlex !== ids2.openAlex) {
      return INSTITUTION_COI_MATCH_KIND.NONE;
    }
    if (ids1.ror && ids2.ror && ids1.ror !== ids2.ror) {
      return INSTITUTION_COI_MATCH_KIND.NONE;
    }

    const exempt1 = this.coiExemptInstitutionKey(inst1);
    const exempt2 = this.coiExemptInstitutionKey(inst2);
    if (exempt1 && exempt1 === exempt2) return INSTITUTION_COI_MATCH_KIND.EXEMPT;
    return this.institutionDirectMatch(inst1, inst2)
      ? INSTITUTION_COI_MATCH_KIND.DIRECT
      : INSTITUTION_COI_MATCH_KIND.NONE;
  }

  /**
   * Boolean compatibility wrapper for existing callers. Exempt institutions
   * are intentionally false here so they cannot become hard drops.
   */
  static institutionsMatchForCOI(inst1, inst2) {
    return this.institutionCOIMatchKind(inst1, inst2) === INSTITUTION_COI_MATCH_KIND.DIRECT;
  }
