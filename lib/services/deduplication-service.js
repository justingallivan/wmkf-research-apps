...
class DeduplicationService {
  ...
  static markInstitutionCOI(researchers, authorInstitution) {
    ...
    return researchers.map(researcher => {
      ...
      // Current-affiliation match (the original check).
      ...
      // Former-institution match: scan the affiliation HISTORY for the PI's
      // institution. Catches a reviewer who shared an institution with the PI in
      // the past and has since moved (e.g. JHU → Harvard) — a genuine COI the
      // recency-best current affiliation structurally hides. Only checked when
      // the current affiliation didn't already match.
      let historicalMatch = null;
      if (!currentMatch && Array.isArray(researcher.affiliationHistory)) {
        for (const aff of researcher.affiliationHistory) {
          if (aff && this.institutionsMatch(normalizedInstitution, this.normalizeInstitution(aff))) {
            historicalMatch = aff;
            break;
          }
        }
      ...
      const hasInstitutionCOI = currentMatch || !!historicalMatch;
      ...
      return {
        ...
        institutionCOIDetails: hasInstitutionCOI ? {
          ...
          reviewerInstitution: currentMatch ? researcherAffiliation : historicalMatch,
          historical: !currentMatch && !!historicalMatch
        } : null
      };
    });
  }
