...
class DeduplicationService {
  ...
  static institutionCOILexicalDecision(candidate = {}, piInstitutions = []) {
    const signals = this.institutionSignalsForCandidate(candidate).map((entry) => ({
      ...entry,
   ...
   * The explicit status lets callers distinguish absent candidate institution
   * evidence, a true lexical non-match, and a lexical match positively refuted
   * by institution ids.
  ...
  static async institutionCOIResolution(
    ...
    const candidateSignals = this.institutionSignalsForCandidate(candidate);
    if (candidateSignals.length === 0) {
      return { status: 'candidate_institution_missing', decision: null };
    }
    const lexicalDecision = this.institutionCOILexicalDecision(candidate, piInstitutions);
    if (!lexicalDecision) {
      return { status: 'lexical_non_match', decision: null };
    }

    const rawDecision = this.institutionCOIDecision(candidate, piInstitutions);
    if (!rawDecision) {
