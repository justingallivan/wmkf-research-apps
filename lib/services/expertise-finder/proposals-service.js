 ...
 *
 * The cycle compiles to a `wmkf_meetingdate` range: the meeting date is the
 * one canonical temporal field on akoya_request, and `akoya_fiscalyear` is
 * only a string encoding of it (see .claude-memory/akoya-temporal-axis-encodings.md).
 *
 * @param {Object} args
 * @param {string} args.cycleCode - Jyy/Dyy (already validated by the shell; an
 *   unparseable code throws — never a silent empty result)
 * @param {string} [args.program] - "SE" or "MR"
 * @returns {Promise<{ proposals: Array, totalCount: number, cycleCode: string, program: string }>}
 */
export async function queryProposals({ cycleCode, program }) {
  const cycleFilter = cycleCodeToOdataFilter(cycleCode);
  if (!cycleFilter) throw new Error(`Invalid cycle code "${cycleCode}"`);
  const result = await grantRequestAdapter.queryAllRequests({
    ...
    filter: `${cycleFilter} and wmkf_request_type eq 100000001`,
    orderby: 'akoya_requestnum asc',
  });

  const countableRequestIds = new Set(result.records
    .filter((row) => testRequestCountsAsOrdinary(row))
  ...
  return {
    proposals,
    totalCount: proposals.filter((proposal) => countableRequestIds.has(proposal.requestId)).length,
    cycleCode,
    program: program || 'all',
  };
}
