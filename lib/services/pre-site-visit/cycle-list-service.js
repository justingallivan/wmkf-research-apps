 ...
 * among the Pre-Site rows, else the newest active Pre-Site Word row. That
 * same Pre-Site pointer-or-newest resolution also supplies `finalReached`
 * (its lifecycle FINAL) regardless of which rail supplies the stage source,
 * because Final Writeup activation marks the Pre-Site row, not the brief
...
function resolveStageSource({ request, preSiteRows, briefRows }) {
  const preSitePointerRow = resolvePointerRow(preSiteRows, request._wmkf_currentpresitevisit_value || null);
  const preSiteChosen = preSitePointerRow || preSiteRows[0] || null;
  const finalReached = preSiteChosen?.wmkf_lifecyclestate === REQUEST_DOCUMENT_LIFECYCLE_STATE.FINAL;

  if (briefRows.length === 0) {
    // Legacy: no brief rows at all — today's Pre-Site derivation, unchanged.
    return {
      row: preSiteChosen,
      isCurrent: Boolean(preSitePointerRow),
