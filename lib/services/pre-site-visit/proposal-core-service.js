 ...
 * - the only proposal input is the canonical AI Materials narrative;
...
export async function loadPreSiteVisitInputs({ requestId }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const [coPIs, proposal] = await Promise.all([
...
export async function generatePreSiteVisitProposalCore({
  ...
  const result = await dependencies.runPrompt({
    ...
    overrideVariables: {
      ...
      proposal_text: inputs.proposal.text,
    },
    runSource,
    actingUserSystemId,
  ...
  return {
    ...
    source: { filename: inputs.proposal.filename },
