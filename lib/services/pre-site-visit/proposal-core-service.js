 ...
 *   program-grant rollups cross-checked against the live program-grant rows;
 *   lib/services/pre-site-visit/funding-history.js). It is NOT prompt input.
 *   Applicant read failure, query failure/cap, rollup↔live mismatch, or an
 *   undated grant all abort generation (fail closed);
...
export async function loadPreSiteVisitInputs({ requestId }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  let programGrants;
  try {
    [applicant, programGrants] = await Promise.all([
      dependencies.getApplicant(applicantId),
      dependencies.getProgramGrants(applicantId),
    ]);
  } catch (error) {
  ...
  if (programGrants?.capped) {
    throw new ServiceHttpError('The applicant program-grant list was truncated; funding history cannot be determined.', {
      httpStatus: 409,
      code: 'pre_site_visit_funding_history_unavailable',
    });
  }
  const programGrantRows = programGrants?.records || [];
  // The rollups recalculate on a schedule. If they disagree with the live rows,
  // the sentence would be stale (wrong count/total) or false ("has not
  // previously received") — fail closed until the rollup catches up.
  const reconciliation = reconcileProgramGrantRollups({
    records: programGrantRows,
    rollupCount: applicant.wmkf_countofprogramgrants,
    rollupSum: applicant.wmkf_sumofprogramgrants,
  });
  if (!reconciliation.ok) {
    console.warn('[pre-site-visit] program-grant rollups disagree with live rows', {
      requestId,
      reason: reconciliation.reason,
    });
    throw new ServiceHttpError(
      'The applicant account award-history rollups do not match the program grants on record; retry after the rollups refresh.',
  ...
  let newestOverall;
  try {
    // Validates every program grant has a recency date (fail closed otherwise),
    // then cites the newest RESEARCH program grant (owner decision 2026-08-28).
    newestOverall = selectMostRecentProgramGrant(programGrantRows);
    mostRecentProgramGrant = selectMostRecentProgramGrant(researchProgramGrants(programGrantRows));
  } catch (error) {
    console.warn('[pre-site-visit] most recent program grant is ambiguous', {
      requestId,
      error: error?.message || String(error),
    });
    throw new ServiceHttpError('A program grant on record has no decision or meeting date, so the most recent grant cannot be determined.', {
      httpStatus: 409,
      code: 'pre_site_visit_funding_history_unavailable',
    });
  }
      ...
      mostRecentGrant: mostRecentProgramGrant ? {
        awardedIn: clean(mostRecentProgramGrant.akoya_fiscalyear)
          || formatMeetingDate(mostRecentProgramGrant.akoya_decisiondate
            || mostRecentProgramGrant.wmkf_meetingdate),
        description: mostRecentProgramGrant.wmkf_wmkfprojectdescription,
        qualifier: newestOverall?.akoya_requestid !== mostRecentProgramGrant.akoya_requestid
          ? 'research'
          : null,
      } : null,
    })
  ...
  // Slice 4 (plan §4.5): compose the deterministic [[STAFF:RefereeSection]]
  // fill from the reviews submitted at generation time. `refereeSection` is
  // `null` when zero reviews are submitted (a legitimate business state, not
  // a failure — the token stays for staff to fill by hand, plan §4.5). Both
  // a genuine roster READ failure and a composer failure fail closed, same
  // posture as the funding-history read above (:196-212), but with distinct
  // codes (wrap-up item 6) so a Dataverse read outage is never confused with
  // a pure-function defect in composeRefereeSection: the document cannot be
  // composed either way, but the failure surfaces differently on-call.
  let refereeSection = null;
  let refereeSectionDiagnostics = [];
  let roster;
  try {
    roster = await dependencies.getWriteupRoster(requestId);
  } catch (error) {
    console.warn('[pre-site-visit] reviewer roster read failed', {
      requestId,
      error: error?.message || String(error),
    });
    throw new ServiceHttpError('The reviewer roster could not be read from Dataverse; the Reviews paragraph cannot be composed.', {
      httpStatus: 409,
      code: 'pre_site_visit_referee_roster_unavailable',
    });
  }
  try {
    const composed = dependencies.composeRefereeSection({
      reviewers: roster.reviewers,
      blockers: roster.blockers,
    });
    if (composed) {
      refereeSection = { text: composed.text, names: composed.names };
      refereeSectionDiagnostics = composed.diagnostics;
    }
  } catch (error) {
    console.warn('[pre-site-visit] reviewer roster composition failed', {
      requestId,
      error: error?.message || String(error),
    });
    throw new ServiceHttpError('The reviewer roster could not be composed into the Reviews paragraph.', {
      httpStatus: 409,
      code: 'pre_site_visit_referee_compose_failed',
    });
  }
...
export async function generatePreSiteVisitProposalCoreFromInputs({
  ...
  const result = await dependencies.runPrompt({
    ...
    // Output budget: the prompt row's 16 384 is exhausted by Sonnet 5's
    // adaptive thinking on long proposals (production run f8bb1326,
    // 2026-08-28, Request 1002379, stopReason=max_tokens). The Admin editor
    // persists this server-owned budget as an immutable Admin-published
    // revision. Publications remain bounded by code-owned safety limits and
    // the resolved model ceiling; no request body can override either value.
