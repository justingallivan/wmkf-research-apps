...
export async function loadPreSiteVisitInputs({ requestId }, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  // a failure — the token stays for staff to fill by hand, plan §4.5). A
  // genuine roster READ failure fails closed, same posture as the funding-
  // history read above (:196-212): the document cannot be composed if the
  // reviewer roster could not be loaded at all.
  let refereeSection = null;
  let refereeSectionDiagnostics = [];
  try {
    const roster = await dependencies.getWriteupRoster(requestId);
