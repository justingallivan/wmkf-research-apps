...
const PRE_SITE_BUDGET = EXECUTOR_BUDGETS['pre-site-visit.proposal-core.generate'];
...
export async function generatePreSiteVisitProposalCoreFromInputs({
  ...
  const result = await dependencies.runPrompt({
    ...
    // contract as before.
    timeoutMsOverride: PRE_SITE_BUDGET.timeoutMsOverride,
    ...
    // does not expose wmkf_ai_maxtokens, so the budget is server-owned here,
    // like the timeout. 32 768 is a quarter of the model's reviewed
    // 128 000 ceiling; the Executor still rejects anything above that ceiling.
    // Both values come from shared/config/executorBudgets.js so the Admin
    // Prompt templates panel shows the budget this call actually uses.
    maxTokensOverride: PRE_SITE_BUDGET.maxTokensOverride,
  });
