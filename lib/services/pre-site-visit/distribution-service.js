 ...
 * and a renewed pre-transport lease. SharePoint + Request Document own file
 * identity, Postgres owns recovery state, and Dynamics owns transport.
 */
...
function assertReadySnapshotMetadata(row, metadata, expectedSize) {
  if (!metadata
    || !sameId(metadata.driveId, row.wmkf_sharepointdriveid)
    || !sameId(metadata.id, row.wmkf_sharepointitemid)
    || !metadata.versionId
    || metadata.versionId !== row.wmkf_sharepointversionid
    || !metadata.eTag
    || metadata.eTag !== row.wmkf_sharepointetag
    || Number(metadata.size) !== Number(expectedSize)) {
    throw distributionError(
      'The retained snapshot publication identity changed after it was frozen.',
...
function stableUploadedMetadata(provisional, stable, expectedSize) {
  if (!stable
    ...
    || !stable.versionId
    || !stable.eTag
    || (provisional?.eTag && provisional.eTag !== stable.eTag)
...
async function validateReadySnapshot(row, spec, dependencies) {
  ...
  assertReadySnapshotMetadata(row, before, row.wmkf_filesize);
  const downloaded = await dependencies.downloadFile(
    row.wmkf_sharepointdriveid,
    row.wmkf_sharepointitemid,
  );
  ...
  assertReadySnapshotMetadata(row, after, downloaded.buffer.length);
