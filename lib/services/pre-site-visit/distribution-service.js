 ...
 * optionally derives PDF from that retained item, binds the selected bytes and
 * compose fields into an exact preview, and resumes one Dynamics activity
 * through granular attachment/send steps. SharePoint + Request Document own
 * file identity, Postgres owns recovery state, and Dynamics owns transport.
 */
...
const TEMPLATE_VERSION = 'pre-site-frozen-distribution-v1';
const PRODUCER = 'request-workbench-distribution';
...
export function distributionBodyHtml(bodyText, operationId) {
  const paragraphs = String(bodyText || '').trim().split(/\n{2,}/)
    .map((paragraph) => `<p>${escapeHtml(paragraph).replaceAll('\n', '<br>')}</p>`)
    .join('');
  return `${paragraphs}<!-- wmkf-pre-site-distribution:${operationId} -->`;
}
...
function normalizeComposeInput(input) {
  ...
  return { attachmentMode, recipients, subject, bodyText };
}
...
async function validateReadySnapshot(row, spec, dependencies) {
...
async function ensureSnapshot(spec, dependencies, actingUserSystemId) {
  ...
  if (row?.wmkf_operationstatus === REQUEST_DOCUMENT_OPERATION_STATUS.READY) {
    return validateReadySnapshot(row, spec, dependencies);
  }
...
export async function preparePreSiteDistribution(input, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const bodyHtml = distributionBodyHtml(compose.bodyText, input.operationId);
  ...
  attempt = await dependencies.recordPrepared(input.operationId, { docx: word, pdf, previewHash });
  if (!attempt) {
...
async function ensureEmailAttachment(attempt, file, dependencies, actingUserSystemId) {
  ...
  if (sha256(downloaded.buffer) !== file.byteHash) {
    throw distributionError(
  ...
  try {
    ...
    if (!email) {
      ...
      try {
        ...
        if (!email) email = await recoverEmailActivity(attempt, dependencies);
      } catch (error) {
        email = await recoverEmailActivity(attempt, dependencies).catch(() => null);
        if (!email) throw error;
      }
    }
    assertEmailActivityMatches(attempt, email);
    attempt = await dependencies.recordEmailActivity(attempt, email.activityid);
    if (!attempt) throw distributionError('The Dynamics activity ID could not be persisted.', 'distribution_email_persist_failed', 502);

    for (const file of attemptAttachments(attempt)) {
      const alreadyAttached = file.kind === 'docx' ? attempt.docx_attached_at : attempt.pdf_attached_at;
      if (!alreadyAttached) {
