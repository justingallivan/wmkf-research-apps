...
export function sessionLineText(snapshot) {
  if (!snapshot?.scheduledStartIso) return 'Deliberation session: not yet scheduled.';
  const start = new Date(snapshot.scheduledStartIso);
  if (Number.isNaN(start.getTime())) return 'Deliberation session: not yet scheduled.';
  let when;
  try {
...
function sessionHtml(snapshot) {
  ...
  const line = `<strong>Deliberation session:</strong>${escapeHtml(text.slice('Deliberation session:'.length))}`;
...
function briefingLinkHtml(briefingLink) {
  ...
  return `<p><strong>Briefing page:</strong> <a href="${BRIEFING_LINK_PLACEHOLDER}">Open the deliberation briefing</a>`
    + ` — the writeup, every completed review, the proposal narrative, and the site visit materials, no login required.${expiresText}</p>`;
}
