...
export async function readDeliberationShareDefaults(dependencies = DEFAULT_DEPENDENCIES) {
  const [
    subject,
    body,
    briefingHeading,
...
export const BRIEFING_LINK_PLACEHOLDER = 'wmkf-briefing-link://pending';

export function renderBriefingBody(bodyHtml, briefingUrl) {
  if (!briefingUrl) return bodyHtml;
  return String(bodyHtml || '').replaceAll(BRIEFING_LINK_PLACEHOLDER, escapeHtml(briefingUrl));
}
...
export function sessionSnapshotOf(session) {
...
export function sessionSnapshotsMatch(left, right) {
...
export function sessionLineText(snapshot) {
...
export function distributionBodyHtml(
  bodyText,
  operationId,
  materialLinks = [],
  briefingLink = null,
...
export function projectDistributionAttempt(
  attempt,
 ...
 */
function computeStaleInputsDelta(generatedEnvelope, liveEnvelope) {
  const generatedCanonical = canonicalBriefInputState(generatedEnvelope);
  const liveCanonical = canonicalBriefInputState(liveEnvelope);
  ...
  const changedReviewerSuggestionIds = [...liveById.keys()].filter((id) => {
    ...
    return REVIEW_FINGERPRINT_FIELDS.some((field) => (
...
async function assertBriefInputsReady({ sourceRow, requestId, acknowledgeStaleInputs }, dependencies) {
  ...
  try {
    if (!storedSnapshot
      || storedSnapshot.schemaVersion !== PRE_RP_BRIEF_CONTRACT.snapshotSchemaVersion
  ...
  // reviews are reduced to `REVIEW_FINGERPRINT_FIELDS`, which excludes
  // `reviewSharePointFolder`/`reviewFilename` on purpose (extending that
  // list would invalidate every stored brief fingerprint), so the bundle
  // reads the live envelope's reviews directly instead.
  ...
  const delta = computeStaleInputsDelta(storedSnapshot, liveInputs.envelope);
...
export async function preparePreSiteDistribution(input, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  // an invalid part throws out of `assembleReviewBundle`/`ensureSnapshot`
  // before `recordPrepared` is ever called, so no attempt reaches `prepared`
  // without a bundle (fail closed).
  const reviewBundleSetFingerprintValue = reviewSetFingerprint(briefGate.liveReviews);
  const reviewBundleAssembly = await assembleReviewBundle({
    reviews: briefGate.liveReviews,
    requestNumber: request.akoya_requestnum || input.requestId,
    institutionName: briefGate.institutionName,
  }, {
    downloadFileByPath: (folderPath, filename) => dependencies.downloadFileByPath('akoya_request', folderPath, filename),
    getFileMetadataByPath: (folderPath, filename) => dependencies.getFileMetadataByPath('akoya_request', folderPath, filename),
    downloadFileAsPdf: dependencies.downloadFileAsPdf,
  });
  const reviewBundleSnapshot = await ensureSnapshot({
    format: 'review-bundle',
    requestId: input.requestId,
    requestNumber: request.akoya_requestnum || input.requestId,
    cycleCode: sourceRow.wmkf_cyclecode,
    // The bundle sits beside the brief snapshot: same source document as the
    // DOCX/PDF, but its generation key is a pure function of the review set
    // (sourceVersionId/sourceByteHash carry the literal 'review-set' marker
    // and the review-set fingerprint instead of the brief's own version/hash),
    // so the bundle is reused across an unrelated brief re-render and only
    // regenerated when the review set itself changes.
    sourceDocumentId: sourceRow.wmkf_requestdocumentid,
    sourceVersionId: 'review-set',
    sourceContentHash: null,
    sourceByteHash: reviewBundleSetFingerprintValue,
    contentType: PDF,
    contentHash: null,
    folderPath,
    filename: `${slugifyInstitutionName(briefGate.institutionName)} - Reviews - ${reviewBundleAssembly.byteHash.slice(0, 8)}.pdf`,
    buffer: reviewBundleAssembly.buffer,
    expectedByteHash: reviewBundleAssembly.byteHash,
  }, dependencies, input.actingUserSystemId || null);
