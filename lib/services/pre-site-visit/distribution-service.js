 ...
 * exact preview. The link is the carrier for reviews and the proposal narrative;
...
function briefingLinkHtml(briefingLink) {
  ...
  const expiresText = expires && !Number.isNaN(expires.getTime())
    ? ` The link expires on ${escapeHtml(expires.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'America/Los_Angeles' }))}.`
    : '';
  return `<p><strong>Briefing page:</strong> <a href="${BRIEFING_LINK_PLACEHOLDER}">Open the deliberation briefing</a>`
    + ` — the writeup, every completed review, and the proposal narrative, no login required.${expiresText}</p>`;
}

export function distributionBodyHtml(bodyText, operationId, materialLinks = [], briefingLink = null) {
  ...
  const links = materialLinks.length
    ? '<h3>Site Visit materials</h3><ul>'
      ...
      + '</ul>'
    : '';
  return `${paragraphs}${briefingLinkHtml(briefingLink)}${links}<!-- wmkf-pre-site-distribution:${operationId} -->`;
}
...
export async function preparePreSiteDistribution(input, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const bodyHtml = distributionBodyHtml(compose.bodyText, input.operationId, materialLinks, briefingLink);
 ...
 * (docs/DELIBERATION_BRIEFING_PAGE_PLAN.md §2.2). Null when the feature flag
 * is off or the caller's dependency set does not wire the link service, so
 * every existing send path is byte-identical until the owner enables it.
 */
async function resolveBriefingLink(input, dependencies) {
  ...
  if (typeof dependencies.ensureBriefingLink !== 'function') return null;
...
async function resolveBoundBriefingUrl(attempt, dependencies) {
  if (!attempt.briefing_link_id) {
    // Flag cutover: a preview prepared before the briefing page was enabled
    // has no link. Once the flag is on, such a preview must be re-prepared
    // (so the email carries the link) unless its send was already requested,
    // in which case the retry only reconciles Dynamics status.
    const briefingOn = typeof dependencies.briefingReady === 'function' && dependencies.briefingReady();
    if (briefingOn && !attempt.send_requested_at) {
      throw distributionError(
        'The briefing page became available after this preview was prepared. Prepare a new exact preview so the email carries the link.',
...
export async function getPreSiteDistributionHistory(
  ...
  const [attempts, currentSource, briefingLink] = await Promise.all([
