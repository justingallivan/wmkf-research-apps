 ...
 * Distribution history also reads the admin-governed Share-for-deliberation
 * subject/body defaults; blank or unavailable settings retain the code-owned
 * fallback and the response reports that status to the composer.
 */
...
async function readDeliberationShareDefault(key, fallback, dependencies) {
  ...
  try {
    ...
    console.error(`[pre-site distribution history] email default read failed for ${key}:`, error?.message || error);
...
export async function readDeliberationShareDefaults(dependencies = DEFAULT_DEPENDENCIES) {
  ...
  return {
    ...
    configured: subject.configured && body.configured,
...
function briefingLinkHtml(briefingLink) {
  ...
  return `<p><strong>Briefing page:</strong> <a href="${BRIEFING_LINK_PLACEHOLDER}">Open the deliberation briefing</a>`
    + ` — the writeup, every completed review, the proposal, and the research presentation materials, no login required.${expiresText}</p>`;
}

export function distributionBodyHtml(bodyText, operationId, materialLinks = [], briefingLink = null, sessionSnapshot = null) {
  ...
  const links = materialLinks.length
    ? '<h3>Site Visit materials</h3><ul>'
      ...
      + '</ul>'
    : '';
  return `${paragraphs}${sessionHtml(sessionSnapshot)}${briefingLinkHtml(briefingLink)}${links}<!-- wmkf-pre-site-distribution:${operationId} -->`;
}
...
export async function preparePreSiteDistribution(input, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const bodyHtml = distributionBodyHtml(compose.bodyText, input.operationId, materialLinks, briefingLink, sessionSnapshot);
