...
import { ensureLiveBriefingLink, getLiveBriefingLink, isBriefingLinkLive } from '../deliberation-briefing/briefing-link-service';
import {
...
const DEFAULT_DEPENDENCIES = Object.freeze({
  ...
  isBriefingLinkLive: (requestId, briefingLinkId) => isBriefingLinkLive({ requestId, briefingLinkId }),
...
function briefingLinkHtml(briefingLink) {
  ...
  const expiresText = expires && !Number.isNaN(expires.getTime())
    ...
    : '';
  return `<p><strong>Briefing page:</strong> <a href="${escapeHtml(briefingLink.url)}">Open the deliberation briefing</a>`
...
export async function preparePreSiteDistribution(input, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  const draftHash = canonicalHash({
    ...
    briefingLinkId: briefingLink?.id || null,
    to: recipients.to,
    cc: recipients.cc,
...
async function assertBriefingLinkCurrent(attempt, dependencies) {
  if (!attempt.briefing_link_id) return;
  const live = typeof dependencies.isBriefingLinkLive === 'function'
    && await dependencies.isBriefingLinkLive(attempt.request_id, attempt.briefing_link_id);
  if (!live) {
    throw distributionError(
...
export async function sendPreSiteDistribution(input, dependencies = DEFAULT_DEPENDENCIES) {
  ...
  try {
    ...
    await assertBriefingLinkCurrent(attempt, dependencies);
