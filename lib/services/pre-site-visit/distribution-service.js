 ...
 *
 * PLAN-DEVIATION candidate, flagged rather than silently resolved (round-4
 * review instruction): `REVIEW_FINGERPRINT_FIELDS` (slice 2, unchanged
 * here) still includes the raw `reviewReceivedAt` timestamp, so
 * `briefInputFingerprint` still treats a pure timestamp rewrite — with no
 * actual received/not-received flip — as drift. This delta instead
 * compares received state as a boolean, so that same reviewer can show as
 * unchanged here while the fingerprint still triggers `brief_inputs_stale`.
 * Kept this way deliberately: the gate stays on the already-approved,
 * conservative fingerprint (never silently widens what counts as "no
 * drift"); only the human-facing delta narrows what it reports as changed.
 * See docs/plans/PRE_RESEARCH_PRESENTATION_BRIEF_PLAN_2026-09-16.md §8 for
 * the follow-up to reconcile this properly.
 */
function computeStaleInputsDelta(generatedEnvelope, liveEnvelope) {
  const changedRequestFields = REQUEST_FINGERPRINT_FIELDS.filter((field) => (
    (generatedEnvelope?.request?.[field] ?? null) !== (liveEnvelope?.request?.[field] ?? null)
  ));
  const byId = (list) => new Map((list || []).map((review) => [review.suggestionId, review]));
  const generatedById = byId(generatedEnvelope?.reviews);
  const liveById = byId(liveEnvelope?.reviews);
  ...
  const changedReviewerSuggestionIds = [...liveById.keys()].filter((id) => {
    ...
    return REVIEW_FINGERPRINT_FIELDS.some((field) => (
      field === 'reviewReceivedAt'
        ? receivedBoolean(generatedReview) !== receivedBoolean(liveReview)
        : (generatedReview?.[field] ?? null) !== (liveReview?.[field] ?? null)
    ));
  });
  return {
...
async function assertBriefInputsReady({ sourceRow, requestId, acknowledgeStaleInputs }, dependencies) {
  ...
  if (liveFingerprint === generatedFingerprint) {
    return { generatedFingerprint, liveFingerprint, delta: null, acknowledgedFingerprint: null };
  }
  ...
  return { generatedFingerprint, liveFingerprint, delta, acknowledgedFingerprint: acknowledgeStaleInputs };
}
