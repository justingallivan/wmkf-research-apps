...
export const REVIEW_BUNDLE_LINK_PLACEHOLDER = 'wmkf-briefing-link://review-bundle';
 ...
 */
export function reviewBundleDocumentUrl(briefingUrl) {
  ...
  if (!row) {
    try {
      await dependencies.createDocument({
        ...
        actingUserSystemId,
        // Plan §11 (Step C2): the on-demand rebuild runs from an external
        // Board reader's token with no staff Dynamics identity, so it opts
        // into ALLOW_UNATTRIBUTED via `spec.actorPolicy`; every other
        // caller (staff-initiated prepare) keeps the default REQUIRED.
        actorPolicy: spec.actorPolicy || REQUEST_DOCUMENT_ACTOR_POLICY.REQUIRED,
        actorContext: {
 ...
 * snapshot (plan §11). Shared by prepare (Step C1, staff-initiated,
 * `actorPolicy` REQUIRED) and the briefing page's on-demand rebuild (Step
 * C2, external-token-initiated, `actorPolicy` ALLOW_UNATTRIBUTED — there is
 * no staff Dynamics identity behind a Board reader's token). The generation
 * key is a pure function of the review set (`sourceVersionId`/
