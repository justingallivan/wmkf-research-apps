 ...
 * reopen metadata plus source lineage form the durable correction event. The
 * current read model also projects standard created-by/created-on, but Request
 * Document writes fall back to the service principal; those standard fields
 * are not reliable human attribution. The owner-selected explicit-actor repair
 * is tracked in docs/REQUEST_DOCUMENT_EXPLICIT_ACTOR_PLAN.md. Later generated
 * drafts carry the cycle id so they cannot collide
 * with or reactivate a preserved pre-reopen generation. A live competing lease
 * blocks another reopen; an expired reopen claim is failed first, and any exact
 * retained SharePoint identity becomes durable cleanup work on that audit row.
 * When a different operation supersedes a Failed reopen, its retained copy is
 * likewise recorded for cleanup while the original failure evidence remains.
 */
...
function assertNoDownstream(
  ...
  const derived = state.rows.find((row) => (
    !exclusions.has(String(row.wmkf_requestdocumentid || '').toLowerCase())
...
function competingGenerations(state, excludedArtifactId = null) {
...
function parseCleanupQueue(value) {
  ...
  try {
    ...
    return parsed.map((entry) => {
      ...
      if (!normalized.driveId || !normalized.itemId) throw new Error('missing stable identity');
      return normalized;
    });
  } catch {
    throw reopenError(
      'A guarded-reopen row contains unreadable SharePoint cleanup work.',
      'pre_site_reopen_cleanup_invalid',
      500,
    );
  }
}

function cleanupPatch(row, metadata, dependencies, { reason, error }) {
  if (!metadata?.driveId || !metadata?.id) return {};
  const existing = parseCleanupQueue(row.wmkf_orphancleanupjson);
  const overflow = parseCleanupQueue(row.wmkf_orphancleanupoverflowjson);
  if ([...existing, ...overflow].some((entry) => (
    sameId(entry.driveId, metadata.driveId) && sameId(entry.itemId, metadata.id)
  ))) return {};
  const entry = {
    driveId: metadata.driveId,
    itemId: metadata.id,
    name: metadata.name || row.wmkf_filename || null,
    recordedAt: dependencies.now().toISOString(),
    reason,
    error,
  };
  const primary = JSON.stringify([...existing, entry]);
  return primary.length <= 10000 && overflow.length === 0
    ? { wmkf_orphancleanupjson: primary }
    : { wmkf_orphancleanupoverflowjson: JSON.stringify([...overflow, entry]) };
}

async function retireExpiredReopenAttempt(row, source, actingUserSystemId, dependencies) {
  let metadata = null;
  if (row.wmkf_sharepointdriveid && row.wmkf_sharepointitemid) {
  ...
  } else if (row.wmkf_sharepointfolderpath && row.wmkf_filename) {
  ...
  if (metadata && (!metadata.driveId || !metadata.id)) {
    throw reopenError(
      'An expired reopen copy has incomplete SharePoint identity.',
      'pre_site_reopen_cleanup_identity_invalid',
      502,
    );
  }
  if (metadata
    && sameId(metadata.driveId, source.wmkf_sharepointdriveid)
    && sameId(metadata.id, source.wmkf_sharepointitemid)) {
    throw reopenError(
      'An expired reopen row points at the preserved source file.',
      'pre_site_reopen_cleanup_identity_invalid',
      500,
    );
  }
  const patch = {
    ...
    wmkf_lasterrormessage: 'The guarded-reopen claim expired before activation.',
    wmkf_lastfailedat: dependencies.now().toISOString(),
    ...(metadata ? {
      wmkf_sharepointsiteid: metadata.siteId || row.wmkf_sharepointsiteid || null,
      wmkf_sharepointdriveid: metadata.driveId,
      wmkf_sharepointitemid: metadata.id,
      wmkf_sharepointweburl: metadata.webUrl || row.wmkf_sharepointweburl || null,
      wmkf_sharepointversionid: metadata.versionId || row.wmkf_sharepointversionid || null,
      wmkf_sharepointetag: metadata.eTag || row.wmkf_sharepointetag || null,
      wmkf_filename: metadata.name || row.wmkf_filename,
      wmkf_filesize: metadata.size ?? row.wmkf_filesize ?? null,
      wmkf_sharepointlastmodified:
        metadata.lastModified || row.wmkf_sharepointlastmodified || null,
      ...cleanupPatch(row, metadata, dependencies, {
        reason: 'expired_reopen_attempt_retained',
        error: 'The reopen claim expired before activation; exact cleanup requires reconciliation.',
      }),
    } : {}),
  };
  try {
    ...
    if (error?.status === 412) {
      throw reopenError(
        'A Pre-Site generation resumed while the expired attempt was being reconciled.',
        'pre_site_reopen_generation_in_progress',
        409,
        { pendingArtifactId: row.wmkf_requestdocumentid },
      );
    }
    throw error;
  }
...
async function recordAbandonedFailedCopy(row, source, actingUserSystemId, dependencies) {
  let metadata = null;
  if (row.wmkf_sharepointdriveid && row.wmkf_sharepointitemid) {
    metadata = await dependencies.getFileMetadataById(
      row.wmkf_sharepointdriveid,
  ...
  if (!metadata.driveId || !metadata.id) {
    throw reopenError(
      'An abandoned reopen copy has incomplete SharePoint identity.',
      'pre_site_reopen_cleanup_identity_invalid',
      502,
    );
  }
  if (sameId(metadata.driveId, source.wmkf_sharepointdriveid)
    && sameId(metadata.id, source.wmkf_sharepointitemid)) {
    throw reopenError(
      'An abandoned reopen row points at the preserved source file.',
      'pre_site_reopen_cleanup_identity_invalid',
      500,
    );
  }
  const cleanup = cleanupPatch(row, metadata, dependencies, {
    reason: 'abandoned_failed_reopen_copy_retained',
    error: 'A different reopen operation superseded this failed attempt; exact cleanup requires reconciliation.',
  });
  ...
  try {
    ...
    if (updateError?.status === 412) {
      throw reopenError(
        'A failed reopen attempt changed while its retained copy was being reconciled.',
        'pre_site_reopen_attempt_changed',
        409,
        { pendingArtifactId: row.wmkf_requestdocumentid },
      );
    }
...
async function reconcileAbandonedFailedCopies(
  state,
  excludedArtifactId,
  source,
  actingUserSystemId,
  ...
  const failedReopens = state.rows.filter((row) => (
    !sameId(row.wmkf_requestdocumentid, excludedArtifactId)
    && row.wmkf_artifacttype === REQUEST_DOCUMENT_ARTIFACT_TYPE.PRE_SITE_VISIT
    && row.wmkf_contenttype === PRE_SITE_VISIT_CONTRACT.contentType
    && row.wmkf_operationstatus === REQUEST_DOCUMENT_OPERATION_STATUS.FAILED
    && row.wmkf_lifecyclestate !== REQUEST_DOCUMENT_LIFECYCLE_STATE.SUPERSEDED
    && row.wmkf_reopencycleid
    && row.wmkf_reopenreasoncode
  ));
  let reconciled = false;
  for (const failed of failedReopens) {
    reconciled = await recordAbandonedFailedCopy(
      failed,
      source,
      actingUserSystemId,
      dependencies,
...
async function reconcileCompetingGenerations(
  state,
  excludedArtifactId,
  source,
  actingUserSystemId,
  ...
  const pendingRows = competingGenerations(state, excludedArtifactId);
  const active = pendingRows.find((row) => leaseActive(row, dependencies));
  if (active) {
    throw reopenError(
      'A Pre-Site generation is already in progress. Wait for it to finish before reopening.',
      'pre_site_reopen_generation_in_progress',
      409,
      { pendingArtifactId: active.wmkf_requestdocumentid },
    );
  }
  const expiredReopens = pendingRows.filter((row) => (
    row.wmkf_reopencycleid && row.wmkf_reopenreasoncode
  ));
  for (const expired of expiredReopens) {
    await retireExpiredReopenAttempt(expired, source, actingUserSystemId, dependencies);
  }
  return expiredReopens.length > 0;
}
...
export async function reopenPreSiteVisit(
  ...
  const competingRows = competingGenerations(state, row?.wmkf_requestdocumentid || null);
  assertNoDownstream(
    state,
    source,
    row?.wmkf_requestdocumentid || null,
    competingRows.map((candidate) => candidate.wmkf_requestdocumentid),
  );
  const reconciledExpiredAttempt = await reconcileCompetingGenerations(
    state,
    row?.wmkf_requestdocumentid || null,
    source,
    actingUserSystemId,
  ...
  const reconciledFailedCopy = await reconcileAbandonedFailedCopies(
    state,
    row?.wmkf_requestdocumentid || null,
    source,
    actingUserSystemId,
  ...
  if (reconciledExpiredAttempt || reconciledFailedCopy) {
    state = await readState(input.requestId, dependencies);
    source = sourceForExpected(state, input.expectedArtifactId);
    row = findAuditRow(state.rows, input.clientOperationId);
    if (row) assertAuditMatches(row, input, generationKey);
  }
      ...
      try {
        await dependencies.createDocument(
          successorPayload(input, source, generationKey, claimToken),
          {
            ...
            actorContext: {
              operation: 'pre-site-guarded-reopen',
              requestId: input.requestId,
