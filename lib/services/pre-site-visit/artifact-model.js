/**
 * Pre-Site artifact identities, projections, validation, and model constants;
 * its correction-cycle and stale-replay rules remain distinct from Initial Assessment.
 */
...
const WARNING_MESSAGES = Object.freeze({
  ...
  referee_expertise_missing: 'A submitted reviewer has no recorded expertise, so the Reviews paragraph expertise sentence omits them. Add keywords or an area of expertise to their reviewer record and regenerate.',
});
...
function projectWarnings(row) {
  return diagnosticsForRow(row).map((diagnostic) => {
    ...
    } else if (code === 'referee_expertise_missing' && diagnostic.name) {
      message = `${diagnostic.name} has no recorded expertise, so the Reviews paragraph expertise sentence omits them. Add keywords or an area of expertise to their reviewer record and regenerate.`;
    }
    return {
      ...diagnostic,
      code,
