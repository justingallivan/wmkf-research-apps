 ...
 *
 * Render contract v7 uses the v6 OOXML bytes and retains the non-fatal
 * editorial paragraph/name behavior from v5. Template v6 retains v5's fixed
 * first-page columns while explicitly zeroing the left value-column inset.
 * It also retains v3's non-wrapping Recommendation cells and pins the
...
const DV_PLACEHOLDERS = Object.freeze({
  ...
  // Deterministic Dataverse sentence (funding-history.js), not an LLM section;
  // the template token keeps its historical AI: prefix.
  institutionalFundingHistory: '[[AI:InstitutionalFundingHistory]]',
});
...
// Slice 4 (plan §4.5): `[[STAFF:RefereeSection]]` is conditionally filled —
// unlike MANUAL_PLACEHOLDERS (always left for staff), this token is either
// replaced with the composed deterministic referee text (when `refereeSection`
// is supplied) or left exactly as-is (when it is null, e.g. zero submitted
// reviews at generation). Codex AR-1 finding 4: neither the MANUAL_PLACEHOLDERS
// "lost placeholder" loop nor the AI/DV "unfilled required" loop covers this
// class, so both cases get their own explicit assertion below.
const CONDITIONAL_PLACEHOLDERS = Object.freeze({
  refereeSection: '[[STAFF:RefereeSection]]',
});
...
function replacePart(xml, replacements, counts, {
  ...
  const replaced = xml.replace(/<w:p(?:\s[^>]*)?>[\s\S]*?<\/w:p>/g, (paragraphXml) => {
    ...
    // Slice 4: reviewer names underline ONLY the paragraph that contained
    // the RefereeSection token (computed from the PRE-replacement logical
    // text, same pattern as `isPersonnelSection` above) — never joined into
    // `personnelNames`, so `personnel_name_not_matched` never fires for a
    // reviewer name and reviewer names never underline inside a Personnel
    // section (plan §4.5).
    const isRefereeSection = refereeNames.length > 0
      && logicalText.includes(CONDITIONAL_PLACEHOLDERS.refereeSection);
    ...
    if (isRefereeSection) {
      paragraphs = paragraphs.map((paragraph) => {
        const styled = underlineTermsInParagraph(paragraph, refereeNames);
        if (refereeUnderlineCounts) {
          for (const [name, count] of styled.counts) {
            refereeUnderlineCounts.set(name, (refereeUnderlineCounts.get(name) || 0) + count);
          }
        }
        return styled.xml;
      });
...
/**
 * Fill a retained OOXML template. STAFF placeholders intentionally
 * remain visible so the PD can complete those sections in Word.
 *
 * `refereeSection` (Slice 4, plan §4.5) is CONDITIONAL, not manual:
 * `null` leaves `[[STAFF:RefereeSection]]` exactly as-is (asserted to
 * survive exactly once); `{ text, names }` replaces it with `text` (a single
 * paragraph, like a DV field) and underlines each name in `names` — ONLY in
 * the paragraph that held the token, never joined into `personnelNames`.
 *
 * @returns {Promise<{docx: Buffer, diagnostics: Array<{code:string, name:string}>}>}
 *   `diagnostics` carries `{code: 'referee_name_not_matched', name}` for any
 *   `refereeSection.names` entry with zero underlines in the rendered
 *   document (Codex AR-1 finding 6: the renderer is the only place that
 *   knows the underline count, so it must be the one to report it).
 */
export async function renderPreSiteVisitDocx({
  documentFields,
  proposalCore,
  personnelNames,
  refereeSection: rawRefereeSection = null,
  templateBuffer = null,
}) {
  validateProposalCore(proposalCore);
  if (!String(documentFields?.institutionalFundingHistory || '').trim()) {
    throw new Error('Institutional funding history is required to render the Pre-Site Visit document.');
  }
  // Blank/whitespace-only text is treated identically to `null`: the token
  // is preserved for staff rather than replaced with an empty paragraph.
  const refereeSection = rawRefereeSection && String(rawRefereeSection.text || '').trim()
    ? rawRefereeSection
    : null;
  const normalizedPersonnelNames = normalizePersonnelNames(personnelNames);
  const refereeToken = CONDITIONAL_PLACEHOLDERS.refereeSection;
  const refereeNames = refereeSection
    ? [...new Set((refereeSection.names || []).map((name) => String(name || '').trim()).filter(Boolean))]
    : [];
  ...
  if (refereeSection) {
    replacements.push([refereeToken, refereeSection.text || '']);
  }
  const counts = new Map();
  const refereeUnderlineCounts = new Map(refereeNames.map((name) => [name, 0]));
  ...
  if (refereeSection) {
    const count = counts.get(refereeToken) || 0;
    if (count !== 1) {
      throw new Error(`Expected exactly one template occurrence of ${refereeToken}; found ${count}.`);
    }
  }
  ...
  if (refereeSection) {
    if (joined.includes(refereeToken)) {
      throw new Error(`Referee section placeholder was not fully replaced: ${refereeToken}`);
    }
  } else {
    const occurrences = countOccurrences(joined, refereeToken);
    if (occurrences !== 1) {
      throw new Error(`Expected ${refereeToken} to survive exactly once when no referee section is supplied; found ${occurrences}.`);
    }
  }

  const diagnostics = [];
  for (const name of refereeNames) {
    if (!refereeUnderlineCounts.get(name)) {
      diagnostics.push({ code: 'referee_name_not_matched', name });
    }
  }

  return { docx: output, diagnostics };
}
