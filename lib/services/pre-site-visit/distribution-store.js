 ...
 * most recent attempt whose send was requested, which pins the exact frozen
 * Word/PDF snapshots the recipients were sent. Null before the first Share.
 */
export async function getLatestSendRequestedAttempt(requestId) {
  const result = await sql`
    SELECT * FROM pre_site_distribution_attempts
     WHERE request_id = ${requestId}
       AND send_requested_at IS NOT NULL
     ORDER BY send_requested_at DESC
     LIMIT 1
  `;
  return result.rows[0] || null;
}
