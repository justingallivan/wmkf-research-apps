...
function validateTwoInputPrompt(prompt) {
  ...
  const required = ['request_context_json', 'proposal_narrative', 'proposal_bibliography'];
  const missing = required.filter((name) => !names.has(name));
  if (!isGuid(promptId) || !Number.isInteger(promptVersion) || promptVersion < 1 || missing.length) {
    throw new ServiceHttpError(
      'The current Pre-Site prompt is not a published two-input prompt version.',
      {
        httpStatus: 409,
        code: 'pre_site_visit_prompt_not_ready',
        body: {
          error: 'The current Pre-Site prompt is not a published two-input prompt version.',
          code: 'pre_site_visit_prompt_not_ready',
        },
      },
    );
...
function sourceManifest(proposalMaterials) {
  return ['narrative', 'bibliography'].map((key) => {
    const source = proposalMaterials[key];
    const manifest = {
      role: key === 'narrative' ? 'proposalNarrative' : 'proposalBibliography',
...
export function buildPreSiteVisitInputSnapshot(inputs) {
  ...
  return {
    ...
    proposalSources: sourceManifest(inputs.proposalMaterials),
  };
}

export function buildPreSiteVisitIdentity({ requestId, inputSnapshot, promptIdentity }) {
...
function persistedDraft(row, expectedInputSnapshot) {
  ...
  if (coreEnvelope?.schemaVersion !== 2 || inputSnapshot?.schemaVersion !== 1
  ...
  validateGeneratedCore(coreEnvelope.proposalCore);
  ...
  return {
    ...
    personnelNames: (inputSnapshot.request.personnel || []).map((person) => person.name),
  };
}
...
export async function generatePreSiteVisitArtifact(
  ...
  // Both exact files and all authoritative structured inputs are frozen before
  ...
  const promptIdentity = validateTwoInputPrompt(
  ...
  try {
    ...
    if (!draft) {
      ...
      row = await assertOwnedClaim(generationKey, claimToken, dependencies);
      await dependencies.updateDocument(row.wmkf_requestdocumentid, {
        ...proposalCorePatch(generated.proposalCore),
...
export { SECTION_FIELDS, validateTemplateContract, validateTwoInputPrompt };
