 ...
 * DTO and envelope/named-field divergence fails closed.
 */
...
function diagnosticsForRow(row) {
  ...
  const inputSnapshot = row.wmkf_presiteinputsnapshotjson
    ...
    : null;
  if (inputSnapshot?.schemaVersion !== 2) {
  ...
  for (const diagnostic of [...prepared.diagnostics, ...stored]) {
...
export function projectPreSiteVisitArtifact(row) {
  ...
  return {
    ...
    correction: row.wmkf_reopencycleid ? {
      ...
      actorId: row._createdby_value || null,
      actorName:
        row['_createdby_value@OData.Community.Display.V1.FormattedValue'] || null,
  ...
  return rows
    ...
    .map((row) => {
      ...
      return {
        ...
        outcome: artifact.operationStatus === REQUEST_DOCUMENT_OPERATION_STATUS.READY
          ? 'completed'
...
export async function getPreSiteVisitArtifactStatus(
  ...
  const wordRows = activeRows.filter((row) => row.wmkf_contenttype === PRE_SITE_VISIT_CONTRACT.contentType);
...
function persistedDraft(row, expectedInputSnapshot) {
  ...
  if (![2, 3].includes(coreEnvelope?.schemaVersion) || inputSnapshot?.schemaVersion !== 2
