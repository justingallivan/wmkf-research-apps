...
} = require('./institution-affiliation-consistency');
...
// Count of DISTINCT shared tokens between two texts. The work-grounding rescue uses this
// instead of hasTokenOverlap (which fires on a SINGLE shared token): a rescued record's
// only positive signal is work-title overlap (no affiliation/topic anchor), so one generic
// cross-domain token ("cell", "model", "network", "imaging") must not be enough to ground a
// wrong person — require ≥2 distinct shared tokens (Codex review, S249).
function distinctOverlapCount(left, right) {
  const leftTokens = new Set(tokenize(left));
  if (!leftTokens.size) return 0;
  const shared = new Set();
  for (const token of tokenize(right)) {
    if (leftTokens.has(token)) shared.add(token);
  }
  return shared.size;
}
...
function scoreRecord(record, suggestion, fieldText, claimedInstitutionIdentity = null) {
  const affiliationMatched = hasTokenOverlap(
    suggestion.suggestedInstitution,
    record.lastKnownInstitution,
  ) || institutionsConsistent(claimedInstitutionIdentity, {
    displayName: record.lastKnownInstitution,
    openAlexId: record.lastKnownInstitutionId,
    ror: record.lastKnownInstitutionRor,
  });
...
const RESCUE_MIN_TOPIC_TOKENS = 2;  // distinct shared tokens required to call titles on-topic
...
async function rescueByWorkGrounding(scored, suggestion, fieldText, { signal } = {}) {
  ...
  // evidence than an affiliation match, so the namesake gate is strict here. Keep the
  // FULL forename-agreeing set: collision detection below must see every same-forename
  // candidate, not just the probed slice (Codex review, S249).
  const forenameAgreeing = (scored || [])
    .map((item) => item.record)
    .filter((record) => record && record.openAlexId && forenameFullyAgrees(suggestion.name, record.displayName));
  if (!forenameAgreeing.length) return null;
  const probed = forenameAgreeing.slice(0, RESCUE_FETCH_LIMIT);
  ...
  for (const record of probed) {
    ...
    // Require ≥2 distinct shared tokens — a single generic token must not ground a person.
    if (distinctOverlapCount(fieldText, titles.join(' ')) < RESCUE_MIN_TOPIC_TOKENS) continue;
    ...
    if (normalizeOrcid(record.orcid)) {
      ...
      if (orcidTitles.length) {
        const orcidOnTopic = distinctOverlapCount(fieldText, orcidTitles.join(' ')) >= RESCUE_MIN_TOPIC_TOKENS;
        if (orcidOnTopic) {
  ...
  // An aborted probe loop may have left candidates unexamined — never promote on partial
  // evidence; keep the original safe abstain.
  if (signal?.aborted) return null;

  if (grounded.length > 1) return { abstainReason: 'rescue_work_grounding_collision', collision: true };
  if (grounded.length === 1) {
    // Fail-safe single-match: a lone grounded candidate may be promoted ONLY when we have
    // examined EVERY forename-agreeing record. If more share the forename than we probed
    // (RESCUE_FETCH_LIMIT cap), we cannot rule out a second on-topic namesake — abstain
    // rather than silently pick one (Codex review, S249: the slice must not hide a collision).
    if (forenameAgreeing.length > probed.length) {
      return { abstainReason: 'rescue_work_grounding_collision', collision: true };
    }
    return { selected: grounded[0], rescued: true };
  }
  return null;
}
...
function orcidProfileForenameFullyAgrees(suggestionName, orcidProfile) {
  const givenNames = String(orcidProfile?.givenNames || '').trim();
  const firstGiven = givenNames.split(/\s+/)[0] || '';
  const normalizedFirstGiven = firstGiven.toLowerCase().normalize('NFKD').replace(/[^\p{L}]/gu, '');
  if (normalizedFirstGiven.length < 2) return false;
  const familyName = String(orcidProfile?.familyName || '').trim();
  return forenameFullyAgrees(suggestionName, `${givenNames} ${familyName}`.trim());
}
...
function institutionMatchesAny(claimedInstitution, profile, claimedInstitutionIdentity = null) {
  ...
  return candidates.some((inst) =>
    hasTokenOverlap(claimedInstitution, inst)
    || institutionsConsistent(claimedInstitutionIdentity, inst));
}
...
function buildAnchors({
  ...
  directOrcid,
  collision,
  claimedInstitutionIdentity,
}) {
  const anchors = [];
  const record = selected.record;
  const orcid = normalizeOrcid(record.orcid);
  const employmentCorroborated = institutionMatchesAny(
    suggestion.suggestedInstitution,
    orcidProfile,
    claimedInstitutionIdentity,
  );
  ...
  if (directId && orcid && directId === orcid) {
    ...
    if (orcidProfile && orcidProfileForenameFullyAgrees(suggestion.name, orcidProfile)) {
      anchors.push(anchor('orcid_name_confirmed', 'strong', orcid, {
        givenName: orcidProfile.givenNames,
      }));
    }
  }
...
function buildIdentityNote(status, anchors = [], record = null, orcid = null) {
  ...
  if (status === 'confirmed' || status === 'probable') {
    ...
    if (has('orcid_name_confirmed')) corroborated.push('ORCID profile name');
...
class ReviewerIdentityEvidence {
  static async evaluateSuggestion(suggestion = {}, { proposalInfo = {}, signal } = {}) {
    ...
    const institutionConsistency = createInstitutionConsistencyChecker();
    let claimedInstitutionIdentity = null;
    try {
      claimedInstitutionIdentity = await institutionConsistency.resolve(
        suggestion.suggestedInstitution,
        { signal },
      );
    } catch (err) {
      if (signal?.aborted) return abstainResult('openalex_outage', sources);
    }

    const fieldText = fieldTextFor(suggestion, proposalInfo);
    const scored = (openAlexResult.records || []).map((record) =>
      scoreRecord(record, suggestion, fieldText, claimedInstitutionIdentity));
    ...
    let directOrcid = selection.directOrcid || null;
    if (!directOrcid && normalizeOrcid(selection.selected.record.orcid)) {
      try {
        directOrcid = await fetchOrcidDirect(suggestion, { signal });
      } catch (err) {
        sources.orcid = err?.name === 'AbortError' ? 'timeout' : 'error';
        return abstainResult('orcid_outage', sources);
      }
    ...
    return {
      ...
      // Preserve both the legacy organization-name list and the source/date-bound
      // employment records used by the dormant Phase 2 affiliation projection.
      orcidAffiliations: Array.isArray(orcidProfile?.affiliations)
        ? orcidProfile.affiliations.map((a) => a && a.organization).filter(Boolean)
        : [],
      orcidAffiliationEvidence: Array.isArray(orcidProfile?.affiliations)
        ? orcidProfile.affiliations.map((affiliation) => ({
            organization: affiliation?.organization || null,
            disambiguatedOrganizationId: affiliation?.disambiguatedOrganizationId || null,
            disambiguationSource: affiliation?.disambiguationSource || null,
            sourceReference: normalizeOrcid(selection.selected.record.orcid)
              ? `orcid:${normalizeOrcid(selection.selected.record.orcid)}`
              : null,
            startYear: affiliation?.startYear || null,
            endYear: affiliation?.endYear || null,
            current: affiliation?.current === true,
          })).filter((affiliation) => affiliation.organization)
        : [],
      anchors,
      sources,
  ...
  static _internals = {
    ...
    forenameFullyAgrees,
    forenamesContradict,
    orcidProfileForenameFullyAgrees,
  };
}
