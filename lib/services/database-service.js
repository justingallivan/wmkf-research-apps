  ...
  static async archiveUserProfile(id) {
    try {
      const result = await sql`
        ...
        WHERE id = ${id}
      `;
      // A concurrent identity transfer can remove the selected row before
      // this statement acquires its lock. Do not report a successful
      // revocation when Postgres updated nothing; the caller must refresh and
      // retry against the identity's current profile row.
      return result.rowCount > 0;
    } catch (error) {
