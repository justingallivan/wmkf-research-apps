...
export async function deliverScheduledEmail(
  ...
  try {
    ...
    if (check.defer) {
      ...
      // Defer refused: transport already started on a prior attempt
      // (dynamics_email_id set) — complete that send below instead of leaving
      // the row leased in 'sending'. The existing activity carries the
      // original token/body, so no ctx (and no re-mint) is needed.
    ...
    if (!existing) {
      if (check.defer) {
        // Defer was refused (transport recorded) yet the activity cannot be
        // resolved right now — fail retryable rather than re-mint over it.
    ...
    if (!accepted(existing)) {
      ...
      if (recheck.defer) {
        ...
        if (deferred) return { deferred: true, message: projectScheduledEmail(deferred, dependencies) };
        // Defer refused (transport already recorded): fall through and send.
      }
