...
export async function deliverScheduledEmail(
  ...
  // the claim, so a test row's lease, attempts and status are never touched:
  // it is stopped with one terminal write so it leaves the due list and the
  // PD digest. An unreadable marker throws with no write and is retried on a
  // later run. The marker is immutable, so nothing can change between this
  // read and the claim.
  if (dependencies.isolationEnabled?.()) {
    const pending = await dependencies.getMessage(id);
    if (!pending) return { skipped: true };
    if (pdSystemUserId && pending.pd_systemuser_id !== pdSystemUserId) return { skipped: true };
  ...
  try {
    ...
    const classification = message?.send_requested_at
    ...
    if (message?.lease_token && classification.outcome === 'failed') {
