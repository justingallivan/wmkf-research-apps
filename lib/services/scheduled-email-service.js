...
export async function deliverScheduledEmail(
  ...
  try {
    // Test requests never receive grantee reminders (Stage 1b): stop the row
    // before any link mint, activity recovery or send. An unreadable marker
    // takes the ordinary failure path and is retried later.
    if (dependencies.isolationEnabled?.()) {
      const testState = await dependencies.resolveTestState(message.request_id);
