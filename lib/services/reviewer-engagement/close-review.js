...
function mapWriteError(error) {
  ...
  // Deployment policy and upstream rejections used to fall through to the
  // route's generic 500 with no status in the UI. Fixed copy only: upstream
  // text never reaches the client (the detail is in the server log).
  if (/^\[dataverse-interlock\] denied/.test(error?.message || '')) {
    return closeoutError(
      'Closeout writes are blocked in this environment by the Dataverse write interlock. Contact an administrator.',
      'write_interlocked',
      503,
    );
  }
  ...
  if (upstreamStatus === 404) {
    return closeoutError('Reviewer suggestion was not found.', 'not_found', 404);
  }
  if (upstreamStatus === 401 || upstreamStatus === 403) {
    return closeoutError(
      'Dataverse refused the closeout write for this account. Contact an administrator.',
      'dataverse_forbidden',
      502,
    );
  }
  if (upstreamStatus >= 400 && upstreamStatus < 500) {
    return closeoutError(
      'Dataverse rejected the closeout write. Reload and try again.',
      'dataverse_rejected',
      502,
    );
  }
