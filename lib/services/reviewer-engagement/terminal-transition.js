...
export async function renderAcceptedReleasePreviews({ requestId, suggestionIds }) {
  ...
  const emailDefaults = await readRequiredEmailDefaults([RELEASE_SUBJECT_KEY, RELEASE_BODY_KEY], {
    source: 'review-manager/terminal-transition:release-preview',
  });
  const drafts = [];
  for (const suggestionId of suggestionIds) {
    const row = await suggestionAdapter.findById(suggestionId).catch(() => null);
  ...
  for (const suggestionId of suggestionIds) {
    ...
    if (terminalStatus === 'released') {
      ...
      if (row._wmkf_honorariumrequest_value) {
        const honorariumRow = await getHonorariumCancellationState(
          row._wmkf_honorariumrequest_value,
        ).catch(() => null);
        if (!honorariumRow) {
          results.push({ suggestionId, status: 'honorarium_read_failed' });
          continue;
        }
        honorarium = honorariumDisposition(honorariumRow);
        if (!honorarium.ok) {
