 ...
 */
// The legibility prefix lives in `wmkf_firstname`, NOT `wmkf_name`: Dataverse
// rewrites the primary name from first/last on create (live proof run
// 83c5da2e, sandbox Request 1000343, 2026-09-26: a POST carrying
// `wmkf_name: 'TEST · …'` read back as ` Martha Cat `, the platform's own
// ` <first> <last> ` derivation), so the projection never writes or compares
// `wmkf_name`; `isSyntheticNameDerived` checks the derived name instead.
export const SYNTHETIC_NAME_PREFIX = 'TEST · ';

/** True when the platform-derived primary name carries the synthetic prefix. */
export function isSyntheticNameDerived(row) {
  return String(row?.wmkf_name ?? '').trim().startsWith(SYNTHETIC_NAME_PREFIX.trim());
}

export function syntheticPersonProjection(bundleReviewer, address) {
  const person = bundleReviewer?.person || {};
  const projected = {
    wmkf_firstname: `${SYNTHETIC_NAME_PREFIX}${person.wmkf_firstname ?? ''}`,
