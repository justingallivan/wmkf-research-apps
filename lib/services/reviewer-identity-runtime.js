...
function reportShadowComparison(comparison, runContext = {}) {
  console.info('[reviewer-identity-runtime] shadow comparison', comparison);
  // Durable best-effort copy (Vercel runtime logs expire within minutes).
  // Fire-and-forget by contract: recordShadowComparison never throws and is
  // never awaited, so a storage outage cannot delay the legacy result.
  recordShadowComparison({ ...comparison, runId: runContext.runId || null });
}

function reportShadowError(error, runContext = {}) {
...
async function evaluateShadowAgainstLegacy(suggestion, options, legacyResult, {
  ...
  onShadowComparison = (comparison) => reportShadowComparison(comparison, { runId }),
  onShadowError = (error) => reportShadowError(error, { runId }),
} = {}) {
  try {
...
async function evaluateWithRuntimeSeam(suggestion = {}, options = {}, {
  ...
  await evaluateShadowAgainstLegacy(
    ...
    options,
    legacyResult,
    { runId: crypto.randomUUID(), ...shadowDependencies },
  );
  return legacyResult;
}
...
async function evaluateSuggestionsWithRuntimeSeam(suggestions = [], options = {}, {
  ...
  if (normalizeResolverMode(mode) === RESOLVER_MODE.SHADOW) {
    ...
    const runDependencies = { runId: crypto.randomUUID(), ...shadowDependencies };
    for (let index = 0; index < candidates.length; index += 1) {
      await evaluateShadowAgainstLegacy(
        candidates[index],
        options,
