 ...
 * Biomedical PubMed verification may invoke a separate privileged compare-only
 * method after its authoritative results settle. That diagnostic never writes
 * W2 telemetry and returns the exact PubMed result array unchanged.
 *
...
function pubMedDiagnosticDecision(result = {}) {
  return result.verified === true || result.verificationStatus === 'verified'
    ? 'bind'
    : 'abstain';
}

function pubMedDiagnosticReason(baselineDecision, worksDecision) {
  if (baselineDecision === worksDecision) return 'resolver_consensus';
  if (baselineDecision === 'abstain' && worksDecision === 'bind') return 'works_found_identity';
  if (baselineDecision === 'bind' && worksDecision !== 'bind') return 'works_did_not_confirm';
  return 'resolver_disagreement';
}

const PUBMED_DIAGNOSTIC_DECISIONS = new Set(['bind', 'review', 'abstain']);

/**
 * Compare already-computed PubMed decisions with Works-first for the current
 * privileged request only. This is deliberately NOT W2 shadow telemetry: it
 * does not call the durable comparison logger and it never replaces or mutates
 * the PubMed results supplied by the caller.
 */
async function diagnosePubMedResultsWithRuntimeSeam(
  suggestions = [],
  pubMedResults = [],
  options = {},
  {
    evaluateWorksFirst = evaluateWorksFirstSuggestion,
    institutionResolver = null,
    diagnosticTimeoutMs = SHADOW_TIMEOUT_MS,
    onComparisonObserved = null,
    now = Date.now,
  } = {},
) {
  const candidates = Array.isArray(suggestions) ? suggestions : [];
  const baselineResults = Array.isArray(pubMedResults) ? pubMedResults : [];
  if (typeof onComparisonObserved !== 'function') return baselineResults;

  const resolver = institutionResolver
    || (typeof evaluateWorksFirst === 'function' && evaluateWorksFirst !== evaluateWorksFirstSuggestion
      ? null
      : createRorInstitutionIdentityResolver());
  const runId = crypto.randomUUID();
  const count = Math.min(candidates.length, baselineResults.length);
  const boundedTimeoutMs = Math.max(1, Number(diagnosticTimeoutMs) || SHADOW_TIMEOUT_MS);
  const deadlineAt = now() + boundedTimeoutMs;

  for (let index = 0; index < count; index += 1) {
    const suggestion = candidates[index];
    const baselineDecision = pubMedDiagnosticDecision(baselineResults[index]);
    let worksDecision = 'unavailable';
    let comparisonStatus = 'available';
    let differenceReason = 'comparison_failed';
    try {
      const remainingMs = deadlineAt - now();
      if (remainingMs <= 0) throw shadowDeadlineError();
      const worksResult = await runShadowWithDeadline((signal) => evaluateWorksFirst(
        suggestion,
        {
          ...options,
          signal,
          ...(resolver ? { institutionResolver: resolver } : {}),
        },
      ), remainingMs, { signal: options.signal });
      if (!PUBMED_DIAGNOSTIC_DECISIONS.has(worksResult?.decision)) {
        throw new Error('reviewer_identity_diagnostic_invalid_decision');
      }
      worksDecision = worksResult.decision;
      differenceReason = pubMedDiagnosticReason(baselineDecision, worksDecision);
    } catch (error) {
      comparisonStatus = 'failed';
      console.warn('[reviewer-identity-runtime] PubMed diagnostic comparison failed; PubMed result retained', {
        errorName: error?.name || 'Error',
      });
    }

    try {
      await onComparisonObserved({
        runId,
        resolverMode: 'diagnostic',
        baselineKind: 'pubmed',
        candidateKey: shadowCandidateKey(suggestion),
        reviewerName: String(suggestion?.name || '').normalize('NFKC').trim().slice(0, 240) || null,
        claimedInstitution: String(suggestion?.suggestedInstitution || '').normalize('NFKC').trim().slice(0, 240) || null,
        baselineDecision,
        worksDecision,
        comparisonStatus,
        differenceReason,
      ...
      // A request-local admin diagnostic must never alter the PubMed result.
    }
  }
...
class ReviewerIdentityRuntime {
  ...
  static async diagnosePubMedResults(suggestions = [], pubMedResults = [], options = {}, hooks = {}) {
    return diagnosePubMedResultsWithRuntimeSeam(suggestions, pubMedResults, options, hooks);
  }
