 ...
 * @param {boolean} [options.failOnDepthLimit=false] - Throw (same
 *   `graph_file_list_truncated` code) when a folder exists at the `maxDepth`
 *   boundary that the walk will not enter, instead of silently returning an
 *   inventory that omits everything below it. A verifier that must prove
 *   "nothing else is here" turns this on.
  ...
  const walk = async (currentPath, depth) => {
    ...
    while (pageUrl) {
      ...
      if (!resp.ok) {
        ...
        const error = buildServiceError('graph', resp, text);
        // Keep the long-standing message shape for existing diagnostics while
        // adding a narrow machine-readable contract for callers that may ignore
        // a speculative folder miss. Only the requested root folder qualifies:
        // a missing child mid-walk is an incomplete listing, not an absent
        // folder. A drive-resolution 404 never reaches this branch.
        error.message = `Failed to list files in ${libraryName}/${currentPath} (${resp.status}): ${text}`;
        if (error.status === 404 && error.dataverseCode === 'itemNotFound' && currentPath === folderPath) {
    ...
    if (recursive && depth >= maxDepth && failOnDepthLimit && folders.length > 0) {
      const error = new Error(
        `listFiles(${libraryName}/${folderPath}) found ${folders.length} folder(s) below the maxDepth=${maxDepth} boundary at ${currentPath}; the inventory would be incomplete`,
      );
      error.code = 'graph_file_list_truncated';
      throw error;
    }
