  ...
  const walk = async (currentPath, depth) => {
    ...
    while (pageUrl) {
      ...
      if (!resp.ok) {
        ...
        // a speculative folder miss. A drive-resolution 404 never reaches this
        // branch and therefore cannot be mistaken for a missing folder.
        error.message = `Failed to list files in ${libraryName}/${currentPath} (${resp.status}): ${text}`;
        if (error.status === 404 && error.dataverseCode === 'itemNotFound') {
