/**
 * GraphService governed folder and file writes.
 *
 * Write operations receive the facade as `svc` so nested resolution,
 * authentication, headers, receiver dispatch, and cancellation behavior
 * remain unchanged.
 */
...
export async function uploadFile(
  ...
  if (onItemCreated !== null && typeof onItemCreated !== 'function') {
    throw new Error('uploadFile: onItemCreated must be a function');
  }
  ...
  // Journaling hook: the PUT has committed and its stable identity is known
  // here, before the optional metadata read below can time out. A caller that
  // must persist the created item's id before any further network call
  // (create-only copy receipts) awaits it now.
  if (onItemCreated && item?.id) {
    await onItemCreated({
      siteId,
      driveId,
      id: item.id,
...
/**
 * Delete one exact drive item only while its observed ETag still matches.
 * The status is returned so the caller can distinguish a stale observation
 * (412) from an unconfirmed cleanup without importing Graph transport internals.
 *
 * @param {string} driveId
 * @param {string} itemId
 * @param {string} eTag
 ...
 */
export async function deleteFileWithEtag(svc, driveId, itemId, eTag) {
  if (typeof eTag !== 'string' || !eTag.trim()) {
    throw new Error('deleteFileWithEtag: eTag is required');
  }
  const token = await svc.getAccessToken();
  const resp = await fetchWithTimeout(
