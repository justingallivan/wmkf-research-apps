...
class NotificationService {
   ...
   * @param {string[]} [opts.explicitRecipients] - When non-empty, bypasses
   *   category routing and emails exactly this list. Use for per-event routing
   *   (e.g. the program director on a specific akoya_request). When empty/null,
   *   category routing applies. Falsy entries are filtered out — pass `[null]`
   *   from a failed lookup and category fallback still kicks in.
  ...
  static async sendAdminEmail({ subject, htmlBody, category, explicitRecipients }) {
    ...
    // When the caller supplies explicit recipients (e.g. the program director
    // resolved per-event), bypass category routing. Falsy entries are filtered
    // out so a failed lookup yielding [null] falls through to category routing.
    ...
    if (filteredExplicit.length > 0) {
      recipients = filteredExplicit;
      routingTag = 'explicit';
    } else {
      const resolved = await AlertRecipients.resolveRecipients(category);
      recipients = resolved.recipients;
      routingTag = `category=${resolved.category}, source=${resolved.source}`;
