...
const { getRequestCorrelation } = require('../observability/request-correlation');
...
class NotificationService {
   ...
   * @param {Object} [opts.operationalEvent] - Opt-in durable operational-event
   *   enrichment (see OperationalEventService.recordEvent: stage, transient,
   *   requestNumber, entityRefs, metadata, recoveryKey…). Severity
   *   error/critical notifications mirror into `operational_events`
   *   automatically; passing this records the event at ANY severity and
   *   overrides the mirrored defaults. Recording is strictly best-effort and
   *   never affects the alert or email outcome.
  ...
  static async notify({
    ...
    // Durable operational-event mirror. Error/critical notifications are
    // unambiguous operational failures; other severities record only when the
    // caller opts in via `operationalEvent` (not every notification is an
    // operational failure). recordEvent never throws, and this runs even when
    // the alert deduplicated (the event layer folds occurrences itself).
    if (severity === 'error' || severity === 'critical'
        || (operationalEvent && typeof operationalEvent === 'object')) {
      try {
        let correlationId = null;
        try {
          correlationId = getRequestCorrelation()?.correlationId || null;
        ...
        await OperationalEventService.recordEvent({
          ...
          subsystem: source,
          requestNumber: metadata?.requestNumber ?? null,
          metadata,
          dedupeKey: autoResolveKey ? `alert:${autoResolveKey}` : null,
          recoveryKey: autoResolveKey || null,
          correlationId,
          ...(operationalEvent && typeof operationalEvent === 'object' ? operationalEvent : {}),
        });
      } catch (eventError) {
        // recordEvent never throws by contract; this belt-and-suspenders catch
        // guarantees event recording can never change the notify() outcome.
        console.error('NotificationService operational-event mirror failed (non-fatal):', eventError?.message || String(eventError));
      }
    }
  ...
  static async sendAdminEmail({ subject, htmlBody, category, explicitRecipients }) {
    ...
    // Trust-model: this method ASSUMES an ambient trusted DAL context — every
    // entry point that can reach it establishes one at the route/cron seam
    // (docs/NOTIFICATION_TRUST_MODEL_PLAN.md, S334 site-33 push-up). Do NOT
    // re-add an internal withDalContext here: createAndSendEmail's
    // assertTrustedDalContext fails closed if a new caller forgets to wrap,
    // which is the intended signal, not a bug to paper over.
    ...
    console.log(
      `NotificationService email accepted for delivery: "${subject}" from ${sender} to ${recipients.length} recipient(s) [${routingTag}]`,
    );
    return true;
  }
