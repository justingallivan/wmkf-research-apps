/**
 * Dry-run-first operator orchestration for historical individual-review DOCX
 * retention. Manifests contain identities and hashes, never review content.
 */
...
function summarize(candidates, anomalies) {
  ...
  return {
    ...
    alreadyFiled: candidates.filter((candidate) => candidate.eligibility === 'already_filed').length,
...
export async function buildReviewDocxBackfillManifest({
  ...
  const resolvedTarget = await resolveReviewDocxTarget();
  const target = {
  ...
  const candidates = plans.map(manifestCandidate).sort(candidateOrder);
...
export async function executeReviewDocxBackfill(manifest) {
  ...
  if (writeSet.length === 0) {
    const results = fresh.candidates.map((candidate) => ({
      ...
      status: candidate.eligibility === 'already_filed' ? 'already_filed' : 'skipped',
    }));
    const alreadyFiled = results.filter((result) => result.status === 'already_filed').length;
    return {
      status: 'completed',
      ...
      summary: {
        ...
        skipped: results.length - alreadyFiled,
        failed: 0,
      },
    };
  }
  ...
  for (const candidate of fresh.candidates) {
    ...
    try {
      ...
      results.push({
        suggestionId: candidate.suggestionId,
        status: ['created', 'reconciled', 'already_filed'].includes(result.status) ? result.status : 'failed',
  ...
  const summary = { created: 0, reconciled: 0, already_filed: 0, skipped: 0, failed: 0 };
