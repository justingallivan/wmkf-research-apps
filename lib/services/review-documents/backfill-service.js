...
export const REVIEW_DOCX_BACKFILL_SCHEMA_VERSION = 2;

const NON_BLOCKING_STATUSES = new Set([
  'eligible',
  'excluded',
  'excluded_test_request',
  'not_selected',
  'not_structured',
  'wrong_cycle',
]);
...
function manifestCandidate(plan, { excludedTestRequest = false } = {}) {
  return canonicalize({
    ...
    eligibility: excludedTestRequest ? 'excluded_test_request' : plan.status,
    originalEligibility: excludedTestRequest ? plan.status : null,
    exclusion: excludedTestRequest
      ? { kind: 'operator_confirmed_test_request', requestNumber: plan.requestNumber }
      : null,
...
function manifestBody({ cycleCode, requestNumber, excludedTestRequestNumbers, observedAt, target, candidates, anomalies }) {
  const scope = {
    cycleCode,
...
export function validateBackfillScope({
  cycleCode,
  requestNumber = null,
  excludedTestRequestNumbers = [],
}) {
  ...
  if (!Array.isArray(excludedTestRequestNumbers)) {
    throw new Error('--exclude-test-request values must be an array.');
  }
  const normalizedExclusions = excludedTestRequestNumbers.map((value) => String(value).trim());
  if (normalizedExclusions.some((value) => !/^\d+$/.test(value))) {
    throw new Error('--exclude-test-request must contain only digits.');
  }
  if (new Set(normalizedExclusions).size !== normalizedExclusions.length) {
    throw new Error('--exclude-test-request values must be unique.');
  }
  if (normalizedRequest && normalizedExclusions.includes(normalizedRequest)) {
    throw new Error('--request-number cannot also be an excluded test request.');
  }
  normalizedExclusions.sort(compareCodePointKeys);
  return {
    cycleCode: normalizedCycle,
    requestNumber: normalizedRequest,
    excludedTestRequestNumbers: normalizedExclusions,
  };
}
...
export async function buildReviewDocxBackfillManifest({
  ...
  const scope = validateBackfillScope({ cycleCode, requestNumber, excludedTestRequestNumbers });
  ...
  const excludedRequests = new Set(scope.excludedTestRequestNumbers);
  const candidates = plans
    .filter((plan) => plan.status !== 'already_filed')
    .map((plan) => manifestCandidate(plan, {
      excludedTestRequest: excludedRequests.has(String(plan.requestNumber || '')),
    }))
    .sort(candidateOrder);
  ...
  for (const excludedRequestNumber of scope.excludedTestRequestNumbers) {
    if (!candidates.some((candidate) => candidate.requestNumber === excludedRequestNumber)) {
      anomalies.push({
        code: 'excluded_test_request_not_found',
        requestNumber: excludedRequestNumber,
      });
    }
  }
...
export async function executeReviewDocxBackfill(manifest) {
  ...
  const fresh = await buildReviewDocxBackfillManifest({
    ...
    excludedTestRequestNumbers: manifest.scope.excludedTestRequestNumbers,
  });
