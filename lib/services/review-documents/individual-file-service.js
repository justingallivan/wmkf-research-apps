  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      await attachReviewDocumentPointer(
        suggestionId,
        { folder: expected.folder, filename: expected.filename },
...
export async function sweepMissingIndividualReviewFiles({
  ...
  // their reviews are never filed to SharePoint by this sweep.
  ...
  for (const row of discovered.records || []) {
    ...
    if (testState.kind === 'ordinary') population.push(row);
