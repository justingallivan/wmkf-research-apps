/**
 * Immutable, create-only retention of structured individual-review DOCX files.
 *
 ...
 * win; partial pointers and divergent target content fail visibly and are never
...
export function buildGeneratedReviewPath({ requestId, requestNumber, suggestionId }) {
  if (!isGuid(requestId) || !isGuid(suggestionId)) {
    throw new Error('Generated review path requires request and suggestion GUIDs.');
  }
  ...
  const suggestionGuid = suggestionId.replace(/-/g, '').toUpperCase();
  return {
    folder: `${number}_${requestGuid}/Reviewer_Uploads/Generated/${suggestionGuid}`,
    filename: `Review-${number}.docx`,
  };
}
...
async function loadCandidate(suggestionId, { cycleCode }) {
  ...
  if (!isGuid(requestId) || !isGuid(reviewerId)) {
  ...
  if (!request || !reviewer) return { result: { ...base, status: 'unresolved_relationship' } };
  ...
  if (!candidateCycle) return { result: { ...identified, status: 'no_cycle' } };
  if (candidateCycle !== cycleCode) return { result: { ...identified, status: 'wrong_cycle' } };
  ...
  if (!row._etag) {
    return { result: failureResult(identified, 'pointer_write_failed', new Error('Suggestion ETag is unavailable.'), 'missing_etag') };
  }
  return { base: identified, row, request, reviewer, answers, cycleCode: candidateCycle };
}
...
export async function preflightReviewDocxWrite({ executionMode = 'scheduled' } = {}) {
  ...
  if (executionMode !== 'scheduled' || classifyDeployment() !== 'production') {
    const error = new Error('Scheduled review-DOCX filing is allowed only in a Production deployment.');
    error.code = 'scheduled_not_production';
    throw error;
  }
  ...
  const [siteId, driveId] = await (async () => {
    const resolvedSiteId = await GraphService.getSiteId();
    const resolvedDriveId = await GraphService.getDriveId(REVIEW_DOCX_LIBRARY, { siteId: resolvedSiteId });
    return [resolvedSiteId, resolvedDriveId];
  })();
  if (!siteId || !driveId) {
  ...
  return { siteUrl: configuredSite, siteId, driveId, dynamicsBase, [TARGET_PROOF]: true };
}
...
async function commitPointers({ suggestionId, row, expected, base }) {
  const payload = {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (!lastError) {
      return failureResult(base, 'pointer_write_failed', new Error('Pointer PATCH returned without durable readback.'), 'pointer_not_committed');
    }
    if (attempt === 1 || !fresh._etag) {
      return failureResult(base, 'pointer_write_failed', lastError, fresh._etag ? 'pointer_retry_exhausted' : 'missing_etag');
    }
    current = fresh;
    lastError = null;
  }
...
export async function ensureIndividualReviewFile(
  suggestionId,
  { cycleCode, executionMode = 'scheduled', target = null } = {},
) {
  ...
  let loaded;
  try {
    loaded = await loadCandidate(suggestionId, { cycleCode: normalizedCycle });
  } catch (error) {
    return failureResult(baseResult(suggestionId), 'read_failed', error, 'read_failed');
  }
  if (loaded.result) return loaded.result;

  const { base, row, request, reviewer, answers } = loaded;
  let built;
  let semanticHash;
  try {
  ...
  try {
    assertedTarget ||= await preflightReviewDocxWrite({ executionMode });
  ...
  const pointerResult = await commitPointers({ suggestionId, row, expected, base: itemBase });
...
async function recordActionableResult(result) {
  if (!ACTIONABLE_STATUSES.has(result.status)) return;
  await OperationalEventService.recordEvent({
