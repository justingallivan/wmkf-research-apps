 ...
 * repaired or overwritten automatically. Operator backfill additionally binds
 * its Graph target to the tracked Production Dataverse hostname.
 */
...
async function loadCandidate(suggestionId, { cycleCode, fullInspection = false }) {
  ...
  if (!request) return { result: { ...base, status: 'unresolved_relationship' }, source };
  ...
  if (!reviewer) return { result: { ...identified, status: 'unresolved_relationship' }, source };
  ...
  if (initialPointerState === 'complete' && scopedPointerState !== 'exact') {
    return {
  ...
  return {
    ...
    source,
    preclassifiedStatus: scopedPointerState === 'exact' ? 'already_filed' : null,
  };
}
...
export async function planIndividualReviewFileCandidate(
  ...
  let loaded;
  try {
    loaded = await loadCandidate(suggestionId, { cycleCode: normalizedCycle, fullInspection: true });
  } catch (error) {
  ...
  const { base, row, request, reviewer, answers, preclassifiedStatus } = loaded;
  let built;
  let semanticHash;
  try {
...
export async function ensureIndividualReviewFile(
  ...
  const { base, row, request, reviewer, answers, source, preclassifiedStatus } = loaded;
  ...
  if (preclassifiedStatus === 'already_filed') {
    try {
      ...
      return { ...hashedBase, status: 'already_filed', item: safeItem(finalItem) };
    } catch (error) {
