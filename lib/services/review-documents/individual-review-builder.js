 ...
 * immutable review receipt timestamp. `outputFilename` is an internal caller-
 * owned override so SharePoint retention can use its human-readable storage
 * name without changing the existing courtesy-attachment filename.
 */
export async function buildIndividualReviewDocx({
  ...
  const answers = answerSnapshot === undefined
    ? ((await fetchAnswersBySuggestion([suggestionId]))[suggestionId] || [])
    : answerSnapshot;
  ...
  return {
    filename: outputFilename || `Review-${request.akoya_requestnum || 'copy'}.docx`,
