...
function paragraph(runs, {
  ...
  const properties = [
    ...
    numbering ? `<w:numPr><w:ilvl w:val="0"/><w:numId w:val="${numbering}"/></w:numPr>` : '',
  ].join('');
...
function title(text) {
  return textParagraph(text, { bold: true, size: 30 }, { after: 180, align: 'center', keepNext: true });
}
...
function table(headers, rows, widths, { centerColumns = [] } = {}) {
  ...
  const body = rows.map((row) => `<w:tr>${row.map((value, index) => tableCell(value, widths[index], {
    align: centerColumns.includes(index) ? 'center' : 'left',
  })).join('')}</w:tr>`).join('');
  return '<w:tbl><w:tblPr><w:tblW w:w="9360" w:type="dxa"/><w:tblInd w:w="0" w:type="dxa"/><w:tblLayout w:type="fixed"/>'
    + borders + '</w:tblPr><w:tblGrid>' + grid + '</w:tblGrid>' + header + body + '</w:tbl>';
}

function blockParagraph(block, { after = 110 } = {}) {
  const renderedRuns = [];
  ...
  if (block?.type === 'heading2' || block?.type === 'heading3') {
    return paragraph(renderedRuns.map((value) => value.replace('<w:rPr>', '<w:rPr><w:b/>')), {
      before: 80,
      after: 60,
      keepNext: true,
    });
  ...
  if (block?.type === 'list-item') return paragraph(renderedRuns, { left: 360, after: 60, numbering: block.ordered ? 1 : 2 });
...
function individualBody(copy) {
  ...
  for (const section of Array.isArray(copy?.sections) ? copy.sections : []) {
    ...
    if (section.state === 'empty') {
      ...
      for (const block of section.blocks || []) children.push(blockParagraph(block, { after: 150 }));
...
function synthesisBody(synthesis) {
  ...
  for (const [label, items] of groups) {
    ...
    for (const item of items) children.push(paragraph([run(item)], { left: 360, after: 50, numbering: 2 }));
  }
...
function multiselectComparison(section, reviewers) {
  ...
  if (reviewers.length <= 4) {
    ...
    const reviewerWidth = Math.floor((PAGE_WIDTH - 5000) / Math.max(reviewers.length, 1));
    const widths = [4200, ...reviewers.map(() => reviewerWidth), PAGE_WIDTH - 4200 - reviewerWidth * reviewers.length];
    const rows = options.map((option) => {
      let count = 0;
      const marks = reviewers.map((reviewer) => {
        const answer = section.answers.find((candidate) => candidate.suggestionId === reviewer.suggestionId);
        if (selectedPair(answer, option)) { count += 1; return '☒'; }
        if (offeredPair(answer, option)) return '☐';
        return '—';
      });
      return [option.label, ...marks, String(count)];
    });
    return table(['Outcome', ...labels, 'Total'], rows, widths, {
      centerColumns: Array.from({ length: reviewers.length + 1 }, (_, index) => index + 1),
    });
  }
  const rows = options.map((option) => {
    const names = [];
    ...
    return [option.label, String(names.length), names.join(', ') || 'None'];
  });
  return table(['Outcome', 'Total', 'Selected by'], rows, [4400, 900, 4060], { centerColumns: [1] });
}
...
function combinedBody(report) {
  ...
  const children = [
    title('COMBINED EXTERNAL REVIEWS'),
    metadataLine('Applicant Institution', header.institution),
    metadataLine('Project Title', header.requestTitle),
    metadataLine('Request Number', header.requestNumber),
    ...
    if (section.type === 'multiselect') {
      ...
      children.push(table(
        ...
        section.answers.map((answer) => [
          answer.reviewerName || 'Unnamed reviewer',
          answer.state === 'answered' ? (answer.label || '—')
      ...
      for (const answer of section.answers || []) {
        ...
        if (answer.state === 'answered' && Array.isArray(answer.blocks)) {
          for (const block of answer.blocks) children.push(blockParagraph(block, { after: 130 }));
        } else {
          children.push(textParagraph(
            answer.state === 'not-asked' ? 'Not asked' : 'No answer provided',
...
async function loadTemplate(kind, templateBuffer = null) {
  ...
  if (!source) {
    if (!sourceCache.has(kind)) sourceCache.set(kind, fs.readFile(templatePath(kind)));
  ...
  return { zip, documentXml, template };
}
...
async function render(kind, payload, bodyXml, { templateBuffer = null } = {}) {
  const { zip, documentXml, template } = await loadTemplate(kind, templateBuffer);
  ...
  zip.file('docProps/core.xml', outputCoreXml(
    template,
    kind === 'individual' ? `External Review ${requestNumber}`.trim() : `Combined External Reviews ${requestNumber}`.trim(),
    payload?.header?.generatedAtIso,
  ));
