...
export async function loadSenderReminderTemplate(senderSystemId, kind, shared) {
  ...
  if (!row) {
    const checked = validateReminderTemplate(kind, shared);
    if (!checked.valid) return { ok: false, reason: 'misconfigured', errors: checked.errors };
    return { ok: true, configured: false, template: checked.value };
  }
