 ...
 * an automatic reminder claim. Explicit own-profile save/reset uses the existing
 * preference service. Preview and send never persist one-off edits. The five-minute
...
import { findByOwnerAndKey } from '../dataverse/adapters/user-preference.js';
...
export async function saveOwnReminderTemplate(profileId, kind, template) {
  const config = CONFIG[kind];
  const checked = validateReminderTemplate(kind, template);
  if (!config || !profileId || !checked.valid) return { ok: false, reason: 'validation', errors: checked.errors };
  const saved = await DatabaseService.setUserPreference(profileId, config.key, JSON.stringify(checked.value));
  return saved ? { ok: true, template: checked.value } : { ok: false, reason: 'persistence' };
}

export async function clearOwnReminderTemplate(profileId, kind) {
  const config = CONFIG[kind];
  if (!config || !profileId) return false;
  return DatabaseService.deleteUserPreference(profileId, config.key);
}
