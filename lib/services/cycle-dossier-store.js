...
export async function reserveDossierEntry({ id, requestId, owner, data }, client = sql) {
  // request_revision counts within the request (reservations included); the
  // unique (request_id, request_revision) index turns a concurrent launch for
  // the same request into a conflict rather than a duplicate number.
  try {
    return (await client.query(`INSERT INTO cycle_dossier_entries(id,request_id,created_by,data,request_revision)
      VALUES($1,$2,$3,$4::jsonb,(SELECT COALESCE(MAX(request_revision),0)+1 FROM cycle_dossier_entries WHERE request_id=$2)) RETURNING *`,
    [id, requestId, owner, JSON.stringify(data)])).rows[0];
  } catch (error) {
    if (error?.code === '23505') throw dossierError('Another launch just reserved a revision for this request. Preview again.', 409);
    throw error;
  }
}
