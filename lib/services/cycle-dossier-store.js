...
export async function readDossierControl(client = sql) {
  return (await client.query('SELECT stop_requested, reason, updated_at FROM cycle_dossier_control WHERE id=TRUE')).rows[0] || null;
}
  ...
  return withDossierTransaction(async client => {
    ...
    if (stopRequested) {
      await client.query(`UPDATE cycle_dossier_runs SET status='paused',
        data=jsonb_set(jsonb_set(data,'{pauseReason}',to_jsonb($1::text)),'{cutPending}','true'::jsonb), updated_at=NOW()
