...
/**
 * Attach real bibliometrics (h-index / i10 / citations) + a last-known-affiliation
...
async function attachOpenAlexMetrics(candidate, result, { signal, onProgress = () => {} } = {}) {
  ...
  try {
    ...
    // Specific research topics from the accepted author record. Consumed by
    // Enrich recommended as the expertise (`wmkf_keywords`) source for an
    // applicant-recommended reviewer, who otherwise has no expertise data at
    // all (the Reviews paragraph's expertise sentence silently omitted them).
    // Rides on `ce`, not the evidence DTO, and only past the identity gate.
    ce.openAlexResearchTopics = Array.isArray(author.researchTopics) ? author.researchTopics : [];
