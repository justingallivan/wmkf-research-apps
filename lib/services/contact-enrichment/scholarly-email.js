...
const WEAK_AFFILIATION_TOKENS = new Set([
  'cancer', 'center', 'centre', 'clinical', 'clinic', 'foundation', 'health',
  'hospital', 'laboratory', 'medical', 'medicine', 'national', 'research',
  'science', 'sciences', 'state', 'system',
  // A shared place name alone does not identify an institution (for example,
  ...
  // Tokens from multi-word state names.
  'carolina', 'dakota', 'hampshire', 'island', 'jersey', 'mexico', 'new',
...
function affiliationCorroborates(candidateAffiliation, authorAffiliation) {
  if (domainEvidence.strongInstitutionDisplayMatch(candidateAffiliation, authorAffiliation)) {
    return true;
  }
  ...
  const overlap = new Set(authorTokens.filter((token) => candidateTokens.has(token)));
  return [...overlap].some((token) => !WEAK_AFFILIATION_TOKENS.has(token));
}

function emailDomainMatchesAffiliation(email, candidateAffiliation) {
  const domain = String(email || '').split('@')[1]?.toLowerCase();
  if (!domain) return false;
  const domainLabels = domain.split('.').filter(Boolean);
  const rawAffiliation = String(candidateAffiliation || '').trim();
  const explicitAcronym = /^[A-Z0-9]{3,8}$/.test(rawAffiliation)
    ? rawAffiliation.toLowerCase()
    : null;
  const institutionTokens = [
    ...domainEvidence.institutionTokens(candidateAffiliation),
    explicitAcronym,
  ]
    // Exact-label matching admits distinctive abbreviations such as mit.edu
    // without matching the same letters inside an unrelated label like smith.edu.
    .filter((token) => token?.length >= 3 && !WEAK_AFFILIATION_TOKENS.has(token));
  return institutionTokens.some((token) => domainLabels.includes(token));
}
...
async function searchEuropePmc(candidate, { signal } = {}) {
  ...
  // Europe PMC's polite-pool contact. Prefers SCHOLARLY_POLITE_MAILTO and falls
  // back to NOTIFICATION_EMAIL_FROM (its historical double duty) so behavior is
  // unchanged where the new var is unset. Set it explicitly once the
  // notification sender becomes an unmonitored system mailbox.
  const contactEmail = process.env.SCHOLARLY_POLITE_MAILTO || process.env.NOTIFICATION_EMAIL_FROM;
...
function selectEmail(evidenceRows, { candidateAffiliation = null } = {}) {
  ...
  const topPublicationCount = ranked[0].publications.length;
  const tied = ranked.filter((entry) => entry.publications.length === topPublicationCount);
  if (tied.length > 1) {
    const affiliationMatches = candidateAffiliation
      ? tied.filter((entry) => emailDomainMatchesAffiliation(entry.email, candidateAffiliation))
      : [];
    if (affiliationMatches.length === 1) {
      const selected = affiliationMatches[0];
      const providers = [...new Set(
        selected.publications.flatMap((publication) => publication.providers),
      )];
      return {
        status: 'found',
        email: selected.email,
        publicationCount: selected.publications.length,
        latestYear: selected.publications[0]?.year || null,
        providers,
        publications: selected.publications.slice(0, MAX_EVIDENCE_PUBLICATIONS),
        selectionReason: 'unique_affiliation_domain_match',
        alternates: tied
          .filter((entry) => entry.email !== selected.email)
          .map((entry) => ({
            email: entry.email,
...
async function findScholarlyEmail(candidate, { signal } = {}) {
  ...
  if (providerErrors.length === settled.length && publications.length === 0) {
    return {
  ...
  return {
    ...selectEmail(evidenceRows, { candidateAffiliation: candidate.affiliation }),
    providerErrors,
  };
}
