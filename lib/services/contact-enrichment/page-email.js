...
function selectGroundedEmail(name, text, emails, verifiedDomain, { identityText = '', pageUrl = '' } = {}) {
  ...
  for (const { email, index } of emails) {
    ...
    // Association is directional: profile/list markup normally puts the
    // person's name before their address. Looking after the address can cross
    // into the next record and misattribute the preceding email (for example,
    // "Lab manager: Emily <email> David R. Liu is ...").
    ...
    const compactLocalPart = localPart.replace(/[^a-z]/g, '');
    const adjacency = windowNamesCandidate(precedingWindow, parsed);
    const ownerMatch = !!slug && localPart === slug && slugNamesCandidate(slug, parsed);
    // Some authoritative profile pages separate the page owner's email from
    // the heading/name. Accept the narrow bare-surname mailbox only when the
    // page identity itself names the candidate; broader name-shaped addresses
    // still require adjacency or the existing personal-URL owner proof.
    const pageOwnerSurnameMatch = pageIdentifiesCandidate && compactLocalPart === parsed.surname;
    if (adjacency || ownerMatch || pageOwnerSurnameMatch) associated.add(email);
  }
