...
const eligibilityEvidence = require('./eligibility-evidence');
...
async function applyScholarlyTier(candidate, result, {
  ...
  try {
    ...
    ce.emailEvidence = {
      ...
      selectionReason: scholarlyResult.selectionReason || null,
      alternates: scholarlyResult.alternates || [],
 ...
 */
async function finalize(service, candidate, result, {
  persist = true,
  onProgress,
  ...
  try {
    ...
    const legacyIdentity = identityResolver.resolveIdentity(hypothesis, evidence);
    const openAlexAuthor = result.contactEnrichment.tierResults?.openalex_author || null;
    const legacyResult = {
      status: legacyIdentity.status,
      resolverStatus: legacyIdentity.status,
      orcid: result.contactEnrichment.orcidId || openAlexAuthor?.orcid || null,
      selectedRecord: openAlexAuthor?.openAlexId
        ? {
            openAlexId: openAlexAuthor.openAlexId,
            displayName: openAlexAuthor.displayName || null,
            orcid: openAlexAuthor.orcid || null,
            lastKnownInstitution: openAlexAuthor.lastKnownInstitution || null,
            lastKnownInstitutionId: openAlexAuthor.lastKnownInstitutionId || null,
            lastKnownInstitutionRor: openAlexAuthor.ror || null,
          }
        : null,
      anchors: legacyIdentity.anchors,
      reason: legacyIdentity.evidenceSummary,
      identity: legacyIdentity,
    };
    const runtimeResult = await ReviewerIdentityRuntime.evaluateExistingResult({
      ...candidate,
      suggestedInstitution: candidate.suggestedInstitution || candidate.affiliation || null,
    }, legacyResult, { signal });
    result.contactEnrichment.identity = runtimeResult.identity || legacyIdentity;
    if (runtimeResult.status === 'probable' && runtimeResult.orcid) {
      result.contactEnrichment.orcidId = runtimeResult.orcid;
      result.contactEnrichment.orcidUrl = `https://orcid.org/${runtimeResult.orcid}`;
    }
  } catch (idErr) {
  ...
  // Pin the best available affiliation evidence from the highest-authority
  // identity-trusted source NOW — ORCID current, then OpenAlex last-known, then
  // PubMed recent-publication. Run after resolveIdentity so the override cannot corrupt the
  ...
  // Eligibility evidence requires the resolved identity + trusted institution
  // domains assembled above. Unknown/provider failure is deliberately a no-op.
  await eligibilityEvidence.attachEligibilityEvidence(candidate, result, {
    useSerpSearch,
    credentials,
    signal,
    onProgress,
  ...
  if (persist && result.contactEnrichment.eligibilityStatus !== 'deceased') {
    await service.saveToDatabase(candidate, result.contactEnrichment);
  }
  return result;
}

/**
 * Pin the candidate's best available affiliation evidence from the highest-
 * authority identity-trusted source collected during the tiers (ORCID current >
 * OpenAlex last-known > PubMed-recency), and record provenance in
 * `affiliationSource` (S224, Topic
 * #2 piece #15).
 *
