...
function estimateCost(candidates, options = {}) {
  ...
  if (useSerpSearch) {
    ...
    // Conservative upper bound: identity-resolved candidates with a trusted
    // institution domain receive one additional first-party eligibility query.
    // Some candidates abstain before this call, but the pre-run estimate cannot
    // know which ones will resolve safely.
    const eligibilitySearches = candidates.length;
    estimate.breakdown.eligibility_evidence = {
      count: eligibilitySearches,
      cost: eligibilitySearches * COSTS.SERP_GOOGLE_SEARCH,
    };
    estimate.paidOperations += eligibilitySearches;
    estimate.estimatedCost += estimate.breakdown.eligibility_evidence.cost;
  }
