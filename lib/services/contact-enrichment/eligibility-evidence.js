...
function namePattern(parsed) {
  ...
  // Official pages commonly insert a middle initial or a parenthetical
  // nickname even when discovery supplied only first + last (for example,
  // "Patricia A. Thiel" and "Patricia (Pat) Thiel"). Keep the allowance
  // deliberately narrow: initials and one parenthetical nickname only, never
  // arbitrary intervening words that could span two different people.
  const middleVariant = `(?:\\s+(?:[a-z]\\.?|\\([a-z][a-z' -]{0,30}\\)))?`;
  const firstLast = `${escapeRegex(parsed.first)}${middleVariant}\\s+${escapeRegex(parsed.last)}`;
  return parsed.tokens.length > 2 ? `(?:${fullName}|${firstLast})` : firstLast;
}
...
function headingNamesCandidateVariant(text, parsed) {
  const subject = normalizedText(ContactParser.stripHonorifics(text || ''))
    .split(/\s*[|:–—]\s*/)[0]
    .replace(/[^a-z0-9' ()-]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
...
function evidenceSentence(text, parsed, status) {
  // Split on structural blocks, not every period: a sentence splitter treats
  // the middle initial in "Patricia A. Thiel" as a sentence boundary and
  // discards the very evidence that supported classification.
  ...
  const firstNameIndex = trimmed.search(new RegExp(`\\b${escapeRegex(parsed.first)}\\b`, 'i'));
  const start = firstNameIndex > 120 ? firstNameIndex - 120 : 0;
  return trimmed.slice(start, start + 800);
}
...
function classifyEligibilityPage(name, page, sourceUrl, checkedAt = new Date().toISOString()) {
  ...
  const bodyItem = { title: '', snippet: page.text || '' };
  const bodyDirectlyNamesEligibility = directDeceasedEvidence(bodyItem, parsed)
    || directEmeritusEvidence(bodyItem, parsed);
  const identityAnchored = textNamesCandidate(titleText, parsed)
    || (h1Texts.length === 1 && textNamesCandidate(h1Texts[0], parsed))
    // A first-party article may use a nickname in its headline while its body
    // directly states the candidate's full name and status. The same strict
    // candidate-bound sentence matcher below must corroborate a shortened
    // first name + identical surname in the title/sole H1. A generic news page
    // or unrelated obituary/navigation text still cannot establish identity.
    || (bodyDirectlyNamesEligibility && (
      headingNamesCandidateVariant(titleText, parsed)
      || (h1Texts.length === 1 && headingNamesCandidateVariant(h1Texts[0], parsed))
    ));
...
async function attachEligibilityEvidence(candidate, result, {
  ...
  ce.eligibilityCheckStatus = 'incomplete';
  result.eligibilityStatus = 'unknown';
  result.eligibilityReason = null;
  result.eligibilityEvidence = null;
  result.eligibilityCheckStatus = 'incomplete';

  if (!mayPersistIdentity(ce.identity?.status)) {
    // The server cannot safely bind a person-specific eligibility search until
    // identity authority exists. That prerequisite is incomplete, rather than
    // a documented no-source N/A, so it cannot later be promoted as a clean
    // eligibility receipt.
    return result;
  }
  if (!useSerpSearch || !credentials.serpApiKey) return result;
  const domains = trustedInstitutionDomains(ce);
  if (domains.length === 0 || !parsedName(candidate?.name)) {
    // There is no trusted first-party source to query for this resolved person.
    // It may be accepted only when the server persists the matching N/A stage
    // receipt; a browser-provided value does not loosen promotion authority.
    ce.eligibilityCheckStatus = 'not_applicable';
    result.eligibilityCheckStatus = 'not_applicable';
    return result;
  }
  ...
  try {
    ...
    ce.eligibilityCheckStatus = 'complete';
    result.eligibilityCheckStatus = 'complete';
    onProgress({
      tier: 'eligibility',
    ...
    ce.eligibilityCheckStatus = 'error';
    result.eligibilityCheckStatus = 'error';
