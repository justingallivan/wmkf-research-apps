...
export function verifyDossierCronSecret(req, res, env = process.env) {
  const secret = env.CYCLE_DOSSIER_CRON_SECRET;
  if (!secret) {
    console.error('[cycle-dossier] CYCLE_DOSSIER_CRON_SECRET not configured');
    res.status(500).json({ error: 'Dossier cron secret not configured' });
    return false;
  }
