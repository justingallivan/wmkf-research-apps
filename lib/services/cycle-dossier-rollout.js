...
export const DOSSIER_CYCLE = 'D26';

/**
 * The one roster predicate shared by the service and the rollout preflight, so
 * the preflight proves the same list the pilot loads. Program scope is resolved
 * server-side (the Workbench default program, Research today); the page renders
 * no selector. A request tagged into the cycle under another program is omitted.
 */
export function buildDossierRosterFilter(programId) {
  return `${cycleCodeToOdataFilter(DOSSIER_CYCLE)} and ${buildProgramScopeFilter(programId)} and ${buildVisibilityFilter(false)}`;
}
