...
import { buildReviewerSubfolder } from '../review-upload.js';
...
export const REVIEW_FILE_COPY_POLICY = Object.freeze({
  version: 'reviews-copy-2026-09-25',
  ...
  maxFilesPerReview: 5,
  // The ledger's reviewer-assignment ceiling for one run (D-R2's assignment
  // rows are the cross-run provenance authority; a run is not expected to
  // exceed this many reviewers in one bundle).
  maxReviewers: 100,
 ...
 * never raw bytes, so this is the plan-time half of the plan's "magic-byte
 * validation ... before planning" sentence; the byte-level half runs on the
 * downloaded bytes before upload, via `validateReviewFile`
 * (file-magic.js), inside the copy step itself -- see run-runner.js's
 * `copy_review_file` handler).
 */
...
function planOneReviewerFiles(bundleReviewer, review, policy) {
  ...
  return files.map((file, index) => {
    assertReviewFilePolicy(file, policy);
    const ext = extensionOf(file.name);
    return {
      kind: 'reviewerUpload',
      source: { ...file },
