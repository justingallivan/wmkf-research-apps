 ...
 *
 * `findAnyPersonByEmail` (P2-4, Opus round 1) is an UNFILTERED exact-match
 * lookup by normalized address — no marker/active/Contact filter — used ONLY
 * by the CLI's reservation-time resolver (`rehearse-test-request-sandbox.mjs`
 * `resolveReviewerAssignments`). The synthetic-only predicate (marker true,
 * active, no Contact link) that a `findSyntheticByEmail` would apply is
 * deliberately NOT built into this read: silently returning null for a REAL
 * reviewer's address (an existing row that is not synthetic) would let the
 * reservation treat that address as if it were free and preallocate a fresh
 * destination GUID for it, storing a real reviewer's address as the
 * recipient-confinement address for a run — stopping only much later, at
 * seed time, on the `wmkf_emailaddress_unique` alternate-key conflict. The
 * caller (the reservation resolver) therefore reads this row's marker/
 * statecode/Contact fields itself and refuses the reservation
 * (`reviewer_person_not_synthetic`) for any row that fails that check,
 * before ever preallocating or reusing a GUID. `lib/dataverse/adapters/
 * potential-reviewer.js#findSyntheticByEmail` (the production, marker-
 * filtered analogue) has no `options.svc` to bind to the sandbox anyway, so
 * this module never re-implements ITS predicate — only the plain lookup.
 *
