...
export function compileBasicCloneFilePlan(trustedInput = {}, selectionInput = {}) {
  ...
  for (const file of previewFiles) {
    // Before the destination number exists, a numbered PDF has only its
    // template; the template stands for its future pathname, so it collides too.
    const destinationName = file.destination?.filename || file.destination?.filenameTemplate;
    if (!file.destination?.folder || !destinationName) continue;
    const key = `${file.destination.folder}\u0000${destinationName}`.toLowerCase();
    const prior = destinationOwners.get(key);
    if (prior && prior !== file.source.id) {
      blockers.push(blocker(
        'DESTINATION_COLLISION',
        'selectedDocumentIds',
        `Multiple selected documents resolve to ${file.destination.folder}/${destinationName}.`,
      ));
    } else {
