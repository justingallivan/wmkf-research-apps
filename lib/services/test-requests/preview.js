...
export function compileBasicTestRequestPreview(trustedInput = {}, requestedInput = {}) {
  ...
  const rawRequestPlan = compileTestRequestDraft({
    ...
    fiscalYear: safeRequested.fiscalYear === undefined
      ? safeTrusted.sourceRequest?.akoya_fiscalyear
      : safeRequested.fiscalYear,
    meetingDate: safeRequested.meetingDate === undefined
      ? (typeof safeTrusted.sourceRequest?.wmkf_meetingdate === 'string'
        ? safeTrusted.sourceRequest.wmkf_meetingdate.slice(0, 10)
        : safeTrusted.sourceRequest?.wmkf_meetingdate)
      : safeRequested.meetingDate,
    metadata: safeTrusted.metadata,
    requestId: safeTrusted.requestId,
    requestType: safeTrusted.requestType,
  ...
  return {
    ...
    preview: {
      files: planReady
        ? rawFilePlan.previewFiles
        : rawFilePlan.previewFiles.map(file => file.operation === 'would-copy'
          ? { ...file, operation: 'blocked' }
          : file),
      request: {
        authoritative: false,
        body: rawRequestPlan.createBody,
      },
    },
    requestPlan,
  };
