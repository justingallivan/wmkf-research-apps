 ...
 *
 * `failOnTruncation` and `failOnDepthLimit` are both on: a census that
 * silently stopped at `maxFiles`, or silently declined to enter a folder at
 * the depth boundary, would report MISSING files rather than the EXTRA ones
 * it never saw, which is the wrong direction for a verifier (Codex slice
 * review round 2). The caller records the thrown `graph_file_list_truncated`
 * error as a Graph inspection failure. The census still sees files only: an
 * EMPTY rogue folder is invisible at any depth (accepted limit).
 */
