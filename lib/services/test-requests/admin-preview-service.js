...
function assertSandboxTarget(info) {
  if (info?.target !== 'sandbox' || info?.deployment === 'production') {
    throw previewError(
...
async function discoverDocuments(source, dependencies) {
  ...
  const results = await Promise.all(buckets.map(async (bucket) => {
    try {
      ...
      return { bucket, files: [], error: true };
    }
  }));
  ...
  for (const result of results) {
    if (result.error) {
      errors.push({ source: result.bucket.source, code: 'SOURCE_BUCKET_UNAVAILABLE' });
      continue;
    }
...
async function resolveSource({ requestId, requestNumber }, dependencies) {
  ...
  try {
    if (requestId) {
      ...
      source = result.records?.length === 1 ? result.records[0] : null;
    }
  } catch {
    source = null;
  }
...
async function hydrateSelectedDocument(document, dependencies) {
  ...
  const versionId = before?.versionId || before?.eTag || null;
  if (!before || !versionId || before.name !== document.name || before.size !== document.size) {
    throw previewError(
...
export async function loadTestRequestPreviewSource(
  ...
  const environment = dependencies.getTargetInfo();
  assertSandboxTarget(environment);
...
export async function buildTestRequestAdminPreview(
  ...
  const externalBlockers = [
    ...
    ...inventory.errors.map((error) => ({
      ...
      detail: `A ${error.source} SharePoint bucket could not be inventoried.`,
      scope: 'files',
    })),
  ];
  const preview = addBlockingConditions(compiled, externalBlockers);
