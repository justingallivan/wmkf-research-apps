...
const KEY_RULES = Object.freeze({
  ...
  mimeType: MIME, eTag: ETAG, versionId: VERSION_ID,
  ...
  requestIds: GUID, itemIds: IDENTIFIER, locationIds: GUID, resourceIds: DIGITS,
});
...
/**
 * The ledger never stores upstream message text: last_error,
 * needs_attention_reason and resource error hold an allowlisted code plus an
 * HTTP status at most. Callers keep the full message in their private
 * receipt or log. Internal ServiceHttpError codes pass through; foreign
 * errors are classified by status or transport kind.
 */
export function describeLedgerError(error) {
  ...
  let token = code;
  if (!token) {
    if (status !== null) token = 'upstream_http';
    else if (error?.name === 'AbortError' || /timed? ?out/i.test(String(error?.name ?? ''))) token = 'timeout';
    else if (/^E[A-Z]+$/.test(String(error?.code ?? ''))) token = 'network';
    else token = 'unknown_error';
  }
  return status !== null && status >= 100 && status <= 599 ? `${token} (http ${status})` : token;
}

/** Accept a reason token, a stored-reason string, or an Error (described); reject prose. */
export function ledgerReasonOrThrow(reason) {
  ...
  if (typeof reason === 'string' && STORED_REASON.test(reason)) return reason;
  throw new ServiceHttpError('Ledger reasons must be an allowlisted code token or an Error, never message text.', {
    httpStatus: 400,
    code: 'test_request_ledger_unsafe_value',
  });
}
      ...
      const { rows } = await db.query(
        `UPDATE test_request_runs
          ...
          WHERE run_id = $1::uuid
            ...
            AND locked_until > NOW()
          RETURNING *`,
        [runId, leaseToken, leaseGeneration, expectedVersion, storedReason],
      );
      return mapRun(rows[0] || null);
    },
      ...
      return db.transaction(async (tx) => {
        ...
        const { rows } = await tx.query(
          `UPDATE test_request_run_resources
            ...
            WHERE resource_id = $1::bigint AND run_id = $2::uuid
            RETURNING *`,
          [resourceId, runId, outcome, ledgerReasonOrThrow(error)],
        );
        return mapResource(rows[0] || null);
      });
    },
