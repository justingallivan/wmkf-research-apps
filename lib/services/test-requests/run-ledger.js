...
/**
 * Scrub a JSON value before it lands in a JSONB column: keys that name a
 * credential, link, body, or source text are dropped; every string value is
 * passed through the error sanitizer; depth and size are bounded. The ledger
 * holds identities, hashes, sizes and timestamps only (design doc,
 * "Operation contract and recovery").
 */
export function sanitizeLedgerJson(value, depth = 0) {
  if (depth > 8) return '[depth-limited]';
  if (value == null || typeof value === 'number' || typeof value === 'boolean') return value;
  if (typeof value === 'string') return sanitizeErrorMessage(value);
  if (Array.isArray(value)) return value.slice(0, 200).map((item) => sanitizeLedgerJson(item, depth + 1));
  if (typeof value === 'object') {
    const out = {};
    for (const [key, item] of Object.entries(value)) {
      if (SECRET_KEY_PATTERN.test(key) && !/hash|id$|ids$|count|size|status/i.test(key)) {
        out[key] = '[redacted]';
        continue;
      }
      out[key] = sanitizeLedgerJson(item, depth + 1);
    }
    return out;
  }
  return String(value);
...
export function createRunLedger(db) {
  return {
    ...
    async advanceStep({
      ...
      if (destinationRequestNumber !== null && !/^\d{1,10}$/.test(String(destinationRequestNumber))) {
      ...
      const { rows } = await db.query(
        `UPDATE test_request_runs
          ...
          RETURNING *`,
        [runId, leaseToken, leaseGeneration, expectedVersion, nextStep, nextStepIndex, status, destinationRequestNumber == null ? null : String(destinationRequestNumber)],
      );
      return mapRun(rows[0] || null);
    },
      ...
      const { rows } = await db.query(
        `UPDATE test_request_runs
          ...
          WHERE run_id = $1::uuid
            ...
            AND locked_until > NOW()
          RETURNING *`,
        [runId, leaseToken, leaseGeneration, expectedVersion, destinationRequestNumber],
      );
      return mapRun(rows[0] || null);
    },
      ...
      return db.transaction(async (tx) => {
        ...
        const { rows } = await tx.query(
          ...
          [
            ...
            JSON.stringify(sanitizeLedgerJson(plannedIdentity ?? {})),
            sourceProvenance != null ? JSON.stringify(sanitizeLedgerJson(sourceProvenance)) : null,
          ],
        );
      ...
      return db.transaction(async (tx) => {
        ...
        const { rows } = await tx.query(
          `UPDATE test_request_run_resources
            ...
            WHERE resource_id = $1::bigint AND run_id = $2::uuid
            RETURNING *`,
          [resourceId, runId, responseStatus, readback != null ? JSON.stringify(sanitizeLedgerJson(readback)) : null, outcome],
        );
        return mapResource(rows[0] || null);
      });
    },
