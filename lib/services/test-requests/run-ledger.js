...
function assertReviewerAssignments(recipe, assignments) {
  ...
  return list.map((assignment, index) => {
    ...
    // reused: true means "bind to an existing synthetic person," which
    // requires the cross-run provenance check (an existing row's marker,
    // address and prior-assignment source person all agreeing) that only
    // the synthetic-only lookup can perform -- not built until slice 6c-ii.
    // Refuse it here rather than accepting an unverified destination GUID.
    if (assignment.reused === true) {
      throw unsafe(`reviewerAssignments[${index}].reused: true is not supported until slice 6c-ii (the synthetic-only lookup and cross-run provenance check are not built)`);
    }
