...
const IDEMPOTENCY_KEY = /^[A-Za-z0-9._:\-]{1,120}$/;
const REVISION = /^(?:W\/)?"?[0-9A-Za-z\-]{1,80}"?$/;
const POLICY_VERSION = /^[a-z0-9][a-z0-9.\-]{0,59}$/;
const ISO_DATE = /^\d{4}-\d{2}-\d{2}$/;
// Operator-authored label that becomes the TEST: title; bounded charset, no
// URL separators, never source-derived text.
const TEST_LABEL = /^(?!.*\/\/)[A-Za-z0-9 _\-.:()]{1,140}$/;
...
export function assertReservePlan({ actorId, idempotencyKey, plan }) {
  ...
  shapeOrThrow('plan.testLabel', plan.testLabel, TEST_LABEL);
  return plan;
}
