...
const RECIPES = new Set(['basic']);
const ENVIRONMENTS = new Set(['sandbox', 'production']);
const HOSTNAME = /^[a-z0-9][a-z0-9.\-]{1,253}$/;
// Factory-owned principal and key formats: lowercase only, no scheme or
// base64 segments, so neither a URL nor a JWT can be expressed.
//   actor: <cli|admin|user>:<local>[@<domain>]   e.g. cli:gallivan, user:jane-doe@example.org
//          (no dots in the local part, so dot-separated base64 segments cannot form)
//   key:   lowercase tokens joined by single hyphens, 1-7 tokens of 1-40 chars
const ACTOR_ID = /^(?:cli|admin|user):[a-z0-9][a-z0-9_-]{0,63}(?:@[a-z0-9-]+(?:\.[a-z0-9-]+)+)?$/;
const IDEMPOTENCY_KEY = /^[a-z0-9]{1,40}(?:-[a-z0-9]{1,40}){0,6}$/;
...
export function assertReservePlan({ actorId, idempotencyKey, plan }) {
  ...
  shapeOrThrow('idempotencyKey', idempotencyKey, IDEMPOTENCY_KEY);
  ...
  shapeOrThrow('plan.expectedGraphSiteId', plan.expectedGraphSiteId, GRAPH_DRIVE_OR_SITE);
  shapeOrThrow('plan.expectedGraphDriveId', plan.expectedGraphDriveId, GRAPH_DRIVE_OR_SITE);
