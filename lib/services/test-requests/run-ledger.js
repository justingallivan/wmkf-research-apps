...
// Recognized credential prefixes are rejected anywhere a string is accepted,
// at the start of the value or after any non-alphanumeric wrapper character
// (`b!ghp_...`, `"glpat-..."`, `W/"ghp_..."`, `...-sk-...`), since the drive,
// eTag and version grammars could otherwise carry a token behind their syntax.
// Mirrored verbatim in migration 054 (`test_request_receipt_ok` and the
// run-level Graph identifier CHECK).
const CREDENTIAL_SHAPE = /(?:^|[^A-Za-z0-9])(?:gh[pousr]_|github_pat_|sk-|xox[abprs]-|AKIA[0-9A-Z]{16}|eyJ[A-Za-z0-9_-]{8}|glpat-|AIza|Bearer_)/;
