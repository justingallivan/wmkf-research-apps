...
const RECEIPT_VALUE_MAX = 200;
const RECEIPT_SHORT_MAX = 80;

/**
 * Allowlist of receipt keys a resource row may carry in planned_identity,
 * source_provenance or readback. The ledger stores identities, hashes,
 * sizes, statuses and timestamps only; anything else is rejected, never
 * redacted (design doc, "Operation contract and recovery"). Keys ending in
 * "At" must be ISO timestamps; keys ending in "Id"/"Ids" are bounded ids;
 * *Hash keys are SHA-256 hex. Slice 5b's runner must fit its receipts here.
 */
export const LEDGER_RECEIPT_KEYS = Object.freeze([
  'requestId', 'runId', 'requestNumber', 'locationId', 'parentLocationId', 'workflowId', 'workflowName',
  'driveId', 'siteId', 'itemId', 'folderItemId', 'graphItemId', 'sourceGraphItemId', 'sourceDriveId',
  'library', 'folder', 'filename', 'relativeUrl', 'name', 'size', 'mimeType', 'eTag', 'versionId',
  'versionNumber', 'versionNumberBefore', 'versionNumberAfter', 'contentHash', 'sha256Match', 'sizeMatch',
  'ownerId', 'createdById', 'expectedAppUserId', 'applicantId', 'organizationId',
  'status', 'statusCode', 'responseStatus', 'outcome', 'recovered', 'recoveredByExactItem', 'restored',
  'restoreVerified', 'restoreWasAlreadyActive', 'manualRecheckRequired', 'reason', 'field',
  'expectedValue', 'actualValue', 'valueBefore', 'valueAfter', 'fiscalYear', 'meetingDate', 'kind',
  'sequence', 'index', 'count', 'matched', 'exists', 'ok',
  'requestIds', 'itemIds', 'locationIds', 'resourceIds',
]);
const RECEIPT_KEY_SET = new Set(LEDGER_RECEIPT_KEYS);
const SHORT_VALUE_KEYS = new Set(['expectedValue', 'actualValue', 'valueBefore', 'valueAfter', 'reason', 'field']);
...
function assertSafeString(key, value) {
  const max = SHORT_VALUE_KEYS.has(key) ? RECEIPT_SHORT_MAX : RECEIPT_VALUE_MAX;
  if (value.length > max) throw unsafe(`${key} exceeds ${max} characters`);
  if (/\n|\r/.test(value)) throw unsafe(`${key} contains a line break`);
...
function assertReceiptValue(key, value, depth) {
  if (value == null || typeof value === 'boolean') return;
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw unsafe(`${key} is not a finite number`);
    return;
  }
  if (typeof value === 'string') {
    if (/Hash$/i.test(key) && !HEX64.test(value)) throw unsafe(`${key} is not a SHA-256 hex digest`);
    if (/At$/.test(key) && Number.isNaN(Date.parse(value))) throw unsafe(`${key} is not a timestamp`);
    if (key === 'requestNumber' && !REQUEST_NUMBER.test(value)) throw unsafe('requestNumber is not bounded digits');
    if (/Id$/.test(key) && !(GUID.test(value) || /^[A-Za-z0-9!_\-.:,]{1,200}$/.test(value))) {
      throw unsafe(`${key} is not a bounded identifier`);
    }
    assertSafeString(key, value);
    return;
  }
  if (Array.isArray(value)) {
    if (depth > 0) throw unsafe(`${key} nests an array`);
    if (value.length > 100) throw unsafe(`${key} has more than 100 entries`);
    for (const item of value) {
      if (typeof item !== 'string') throw unsafe(`${key} must be an array of strings`);
      assertReceiptValue(key.replace(/s$/, ''), item, depth + 1);
    }
    return;
  }
  throw unsafe(`${key} has an unsupported value type`);
}

/**
 * Validate a receipt object against the allowlist. Unknown keys, nested
 * objects, over-long strings, credentials, links, bodies and source text are
 * rejected with 400 test_request_ledger_unsafe_value. Returns the value
 * unchanged so callers can bind it.
 */
export function assertLedgerReceipt(value, field = 'receipt') {
  ...
  for (const key of keys) {
    const allowed = RECEIPT_KEY_SET.has(key)
      || (/^[a-z][A-Za-z0-9]{0,40}At$/.test(key) && !/body|content|purpose|token|secret|download|narrative|bytes/i.test(key));
    if (!allowed) throw unsafe(`${field}.${key} is not an allowlisted receipt key`);
    assertReceiptValue(key, value[key], 0);
  }
  return value;
}
      ...
      const { rows } = await db.query(
        `UPDATE test_request_runs
          ...
          WHERE run_id = $1::uuid
            ...
            AND version = $4::integer
            AND locked_until > NOW()
            AND COALESCE($5::text, destination_request_number) IS NOT NULL
          RETURNING *`,
