...
// Basic recipe's subfolders (lib/services/test-requests/file-plan.js), plus
// the Initial Assessment recipe's artifact folder and its Board snapshot child
// (shared/config/requestDocument.js relativeFolder values).
const REQUEST_FOLDER = String.raw`\d{1,10}_[0-9A-F]{32}`;
// Reviews recipe (slice 6c-i): the reviewer-upload attempt folder
// (review-upload.js buildReviewerSubfolder / the upload write path), reused
// identically as a request-folder child and as a bare subfolder, exactly how
// the other SUBFOLDER alternatives are composed below.
const REVIEWER_UPLOADS = String.raw`Reviewer_Uploads\/(?:[A-Za-z0-9]{1,30}_)?[0-9a-f]{8}\/attempt_[0-9a-f]{32}`;
const SUBFOLDER = String.raw`(?:Phase I|AI Materials|Reviewer Materials|Artifacts\/Initial Assessment(?:\/Board Milestones)?|${REVIEWER_UPLOADS})`;
...
// plus the IA DOCX (artifact-service.js: "<number> Initial Assessment
// <key8>-<claim8>.docx") and Board snapshot (controls-service.js: "<number>
// Initial Assessment Board v<version> <key8>.docx") families, and the Reviews
// recipe's deterministically renamed copied review files (`Review_<n>.<ext>`,
// n = the file's ordinal within its review, primary file first).
const FILENAME = /^(?:(?:Proposal|ProposalNarrative|ProposalBibliography)_\d{1,10}\.pdf|(?:ProjectDescription|Biosketches|ProjectBudget)\.pdf|Project Budget spreadsheet\.xlsx|\d{1,10} Initial Assessment [0-9a-f]{8}-[0-9a-f]{8}\.docx|\d{1,10} Initial Assessment Board v[0-9A-Za-z._-]{1,40} [0-9a-f]{8}\.docx|Review_[1-5]\.(?:pdf|docx|doc))$/;
// The Reviews recipe's finite review-form classification (source lifecycle
// derived, never free text): whether the source review's file was uploaded,
// received without a file (e.g. markReceivedNoFile / manual entry), or never
// received at all.
const REVIEW_FORM = /^(?:uploaded|received_no_file|unreceived)$/;
...
const KEY_RULES = Object.freeze({
  ...
  // Reviews recipe (slice 6c-i): source/destination person and suggestion identities.
  sourcePersonId: GUID, destinationPersonId: GUID, suggestionId: GUID,
  ...
  // Reviews recipe: the suggestion's eTag immediately before and after the
  // answers-plus-completion changeset, so the verifier can require the
  // journaled post-changeset eTag rather than trusting a fresh read alone.
  eTagBefore: ETAG, eTagAfter: ETAG,
  versionNumber: DIGITS, versionNumberBefore: DIGITS, versionNumberAfter: DIGITS,
  contentHash: HEX64, generationKey: HEX64, claimTokenSha256: HEX64, foundationBaselineSha256: HEX64, bytesSha256: HEX64,
  // Reviews recipe: the run_reviewer_assignments row's address digest (the
  // plaintext address itself never reaches the ledger receipts) and the
  // source-to-destination DOCX package attestation digest (slice 6c-ii).
  addressSha256: HEX64, attestedDigest: HEX64,
...
// `assignmentSequence` (D-R2): the reviewer-assignment row's own `sequence`
// column is the "row id" a 6c-ii receipt may reference; a distinct key from
// the generic resource `sequence` above (a different table, a different
// per-run counter), same small-integer shape.
// `itemSize` (Opus round 1, slice 6c-ii Stage C P1-1): the DESTINATION
// item's post-write size, distinct from `size` (the bundle/source's
// pre-copy size). SharePoint's DOCX property promotion changes the
// destination's byte count, so a DOCX census can never compare a Graph
// folder listing against the source's `size` -- it must compare against
// what the destination actually reads back as, which was previously
// resolved in memory (bundle-file-copy.js's `entry.item.size`) but never
// journaled, so the ledger had nothing to give back to a resumed or
// out-of-process verifier.
const NUMERIC_KEYS = new Set(['size', 'itemSize', 'statusCode', 'responseStatus', 'sequence', 'index', 'count', 'versionNumber', 'answerCount', 'assignmentSequence']);
...
export const LEDGER_REASON_CODES = Object.freeze([
  ...
  // Reviews recipe (slice 6c-i ledger dimension; step bodies land in 6c-ii)
...
/**
 * Finite resource-kind tokens; the SQL CHECK in migration 054 (and its
 * setup-database.js mirror) lists the same values on
 * test_request_run_resources.resource_kind. Exported (P2-a, Opus round 1) so
 * the migration parity test asserts set equality against the SQL CHECK's own
 * parsed IN (...) list, rather than a hand-copied JS array that can drift
 * from both the source of truth and the SQL without either failing.
 */
export const LEDGER_RESOURCE_KINDS = Object.freeze([
  ...
  'foundation_baseline',
  // Reviews recipe: copied review files reuse sharepoint_folder/sharepoint_file.
  'dataverse_potential_reviewer', 'dataverse_reviewer_suggestion', 'dataverse_review_answer_set',
]);
const RESOURCE_KINDS = new Set(LEDGER_RESOURCE_KINDS);
...
/** Finite recipe tokens; the SQL CHECK in migration 054 lists the same values. */
export const LEDGER_RECIPES = Object.freeze(['basic', 'initial_assessment', 'reviews']);
const RECIPES = new Set(LEDGER_RECIPES);
...
// Reviews recipe (slice 6c-i, D-R2 owner decision): the assignments table's
// `address` column is the ONE sanctioned plain-text exception to the
// receipt no-text invariant, since decision 4 makes the per-run address
// assignment the recipient-confinement authority. Every other surface
// (receipts, needs_attention_reason, inspect output) carries only the
// SHA-256 digest below, never the address itself. A loose email shape is
// enforced here only to reject obvious garbage/credential-shaped input --
// this is NOT one of the finite grammars above, by design.
// Case-insensitive twin of CREDENTIAL_SHAPE plus URL shapes, because the
// address is lowercased BEFORE this check (Codex 6c-i round 6: the
// case-sensitive CREDENTIAL_SHAPE could never match AKIA/eyJ/AIza/Bearer_ on
// lowercased input). Mirrors the address_shape CHECK in migration 054 exactly.
const REVIEWER_ADDRESS_EXCLUSIONS = /(?:gh[pousr]_|github_pat_|sk-|xox[abprs]-|akia[0-9a-z]{16}|eyj[a-z0-9_-]{8}|glpat-|aiza|bearer_|https?:\/\/|\/\/)/i;
const REVIEWER_ADDRESS_SHAPE = /^[^\s@]{1,64}@[^\s@]{1,255}\.[^\s@]{1,24}$/;

/**
 * Normalize (trim + lowercase) and validate a reviewer address, returning
 * both the normalized address and its SHA-256 digest. Normalizing before
 * hashing AND before storage/uniqueness means `Alice@x.test` and
 * `alice@x.test` are the same assignment everywhere they are compared.
 */
export function reviewerAddressSha256(address) {
  if (typeof address !== 'string') throw unsafe('reviewer address must be a string');
  const normalized = address.trim().toLowerCase();
  if (
    !normalized || normalized.length > 320
    || /[\s\x00-\x1f]/.test(normalized)
    || REVIEWER_ADDRESS_EXCLUSIONS.test(normalized)
    || !REVIEWER_ADDRESS_SHAPE.test(normalized)
  ) {
    throw unsafe('reviewer address is not a plausible email address');
  }
  return { address: normalized, addressSha256: crypto.createHash('sha256').update(normalized).digest('hex') };
}

/**
 * Validate and normalize the reviewer assignments a `reviews` reservation
 * must carry (D-R2/decision 4): non-empty, no two assignments naming the
 * same source reviewer or the same (normalized) address, `sequence`
 * assigned gapless in the caller's array order (the CLI is responsible for
 * canonical ordering so a same-key retry's digest is order-independent).
 * Any recipe other than `reviews` must carry none. Runs entirely before any
 * SQL, like every other reserve-time assertion in this file.
 */
function assertReviewerAssignments(recipe, assignments) {
  const list = Array.isArray(assignments) ? assignments : [];
  if (recipe !== 'reviews') {
    if (list.length > 0) throw unsafe('reviewerAssignments is valid only for the reviews recipe');
    return [];
  }
  if (list.length === 0) throw unsafe('the reviews recipe requires at least one reviewer assignment');
  if (list.length > 100) throw unsafe('reviewerAssignments has more than 100 entries');
  const seenSource = new Set();
  const seenAddress = new Set();
  return list.map((assignment, index) => {
    if (!assignment || typeof assignment !== 'object') throw unsafe(`reviewerAssignments[${index}] must be an object`);
    shapeOrThrow(`reviewerAssignments[${index}].sourcePersonId`, assignment.sourcePersonId, GUID);
    shapeOrThrow(`reviewerAssignments[${index}].destinationPersonId`, assignment.destinationPersonId, GUID);
    if (typeof assignment.reused !== 'boolean') throw unsafe(`reviewerAssignments[${index}].reused must be a boolean`);
    // reused: true means "bind to an existing synthetic person." The
    // cross-run provenance check (an existing row's marker, address and
    // prior-assignment source person all agreeing) is now performed by the
    // CLI's reservation-time resolution (slice 6c-ii Stage B,
    // rehearse-test-request-sandbox.mjs runReserve), through the
    // synthetic-only lookup and listAssignmentsByDestinationPerson below,
    // BEFORE the CLI ever calls reserveRun. The ledger itself still only
    // requires destinationPersonId to be a GUID -- the CLI is the one writer
    // of this table and is trusted to have done that resolution.
    const { address, addressSha256 } = reviewerAddressSha256(assignment.address);
    const sourceKey = assignment.sourcePersonId.toLowerCase();
    if (seenSource.has(sourceKey)) throw unsafe('two reviewer assignments name the same source reviewer');
    if (seenAddress.has(address)) throw unsafe('two reviewer assignments share the same address');
    seenSource.add(sourceKey);
    seenAddress.add(address);
    return {
      sequence: index + 1,
      sourcePersonId: sourceKey,
      destinationPersonId: assignment.destinationPersonId.toLowerCase(),
...
/**
 * Opus round 1 (2026-09-25): the CLI's plan-digest binding of reviewer
 * assignments is caller-computed and untrustworthy on its own (the CLI's
 * `runId` is minted fresh per invocation rather than derived deterministically,
 * so a genuine retry's digest is unreliable evidence either way). The ledger
 * itself must refuse a same-key retry whose reviewer assignments differ,
 * independently of `plan_digest`. Compares by (sourcePersonId -> addressSha256)
 * pairing and count; `destinationPersonId` is excluded because it is a fresh
 * random GUID on every call and is never meant to match across retries.
 */
function assertReviewerAssignmentsUnchanged(storedRows, proposed) {
  if (storedRows.length !== proposed.length) {
    throw new ServiceHttpError(
      'A test request run already exists for this idempotency key with a different number of reviewer assignments.',
      { httpStatus: 409, code: 'test_request_run_conflict' },
    );
  }
  const storedBySource = new Map(storedRows.map((row) => [String(row.source_person_id).toLowerCase(), row.address_sha256]));
  for (const assignment of proposed) {
    const storedDigest = storedBySource.get(assignment.sourcePersonId);
    if (storedDigest === undefined || storedDigest !== assignment.addressSha256) {
      throw new ServiceHttpError(
        'A test request run already exists for this idempotency key with different reviewer assignments.',
        { httpStatus: 409, code: 'test_request_run_conflict' },
      );
    }
  }
...
export function createRunLedger(db) {
  return {
     ...
     */
    async reserveRun({
      actorId, idempotencyKey, plan, reviewerAssignments = [],
    }) {
      assertReservePlan({ actorId, idempotencyKey, plan }); // before any SQL
      const assignments = assertReviewerAssignments(plan.recipe, reviewerAssignments); // before any SQL
      ...
      return db.transaction(async (tx) => {
        ...
        // Assignments are written ONLY on the very first reservation of this
        // key, in the same transaction as the run insert, and are immutable
        // afterwards (no update path). A same-key retry's assignments are
        // independently verified against the stored rows (Opus round 1: the
        // caller-computed plan_digest is not trustworthy evidence on its
        // own), never re-inserted.
        // Only issue the assignments query when there is something to check
        // (the stored run is `reviews`, or the caller supplied assignments):
        // a basic/initial_assessment retry never touches
        // test_request_run_reviewer_assignments at all (Codex adversarial
        // round 1, defensive: keeps a basic/IA retry working unchanged even
        // on a schema that predates this table).
        if (!created && (existing.recipe === 'reviews' || assignments.length > 0)) {
          const { rows: storedAssignments } = await tx.query(
            `SELECT sequence, source_person_id, address_sha256 FROM test_request_run_reviewer_assignments WHERE run_id = $1::uuid ORDER BY sequence ASC`,
            [existing.run_id],
          );
          assertReviewerAssignmentsUnchanged(storedAssignments, assignments);
        }
        if (created) {
          for (const assignment of assignments) {
            await tx.query(
              `INSERT INTO test_request_run_reviewer_assignments (
                 run_id, sequence, source_person_id, destination_person_id, reused, address, address_sha256
               ) VALUES ($1::uuid, $2::integer, $3::uuid, $4::uuid, $5::boolean, $6::text, $7::text)`,
              [
                plan.runId, assignment.sequence, assignment.sourcePersonId, assignment.destinationPersonId,
                assignment.reused, assignment.address, assignment.addressSha256,
              ],
            );
          }
        }
    ...
    /**
     * Read-only projection for inspect/CLI display: NEVER selects the
     * plaintext `address` column, only its digest (D-R2; the redaction
     * contract's "print only the digest" rule is enforced structurally by
     * never fetching the plaintext here, not by scrubbing it afterwards).
     */
    async listRunReviewerAssignments(runId) {
      const { rows } = await db.query(
        `SELECT sequence, source_person_id, destination_person_id, reused, address_sha256, created_at
           FROM test_request_run_reviewer_assignments WHERE run_id = $1::uuid ORDER BY sequence ASC`,
        [runId],
      );
      return rows.map((row) => ({
        sequence: row.sequence,
        sourcePersonId: row.source_person_id,
        destinationPersonId: row.destination_person_id,
        reused: row.reused,
        addressSha256: row.address_sha256,
        createdAt: row.created_at,
      }));
    },

    /**
     * Cross-run provenance authority (slice 6c-ii Stage B, plan "Reserve"/
     * "Identity contract"): every source person GUID that has EVER been
     * assigned to `destinationPersonId`, across every run and every status
     * (a run's own success/failure never matters here -- the assignment row
     * itself is immutable once written). The CLI's reservation-time
     * resolution requires every returned row to name the SAME source person
     * before it will bind a destination GUID with `reused: true`; a single
     * disagreeing row refuses the reservation (`reviewer_person_provenance_
     * mismatch`). Read-only; no lease/version fencing needed (no run row is
     * touched). Returns source GUIDs only -- never the address or its digest,
     * matching listRunReviewerAssignments' "no plaintext" posture.
     */
    async listAssignmentsByDestinationPerson(destinationPersonId) {
      shapeOrThrow('destinationPersonId', String(destinationPersonId ?? ''), GUID);
      const { rows } = await db.query(
        `SELECT DISTINCT source_person_id FROM test_request_run_reviewer_assignments
          WHERE destination_person_id = $1::uuid`,
        [String(destinationPersonId).toLowerCase()],
      );
      return rows.map((row) => row.source_person_id);
    },

    /**
     * Runner-internal read (slice 6c-ii Stage B): the ONE place the seeder
     * reads back the plaintext address it needs to write onto a freshly
     * created synthetic person, or to re-verify a reused one. Unlike
     * listRunReviewerAssignments (CLI --run-inspect display), this DOES
     * select the plaintext `address` column -- it is used only in-memory to
     * build a Dataverse write body / comparison, never persisted to a
     * receipt or printed (the no-text invariant governs what the LEDGER
     * stores and displays, not this internal read).
     */
    async getRunReviewerAssignment(runId, sequence) {
      shapeOrThrow('runId', String(runId ?? ''), GUID);
      if (!Number.isInteger(sequence) || sequence < 1) throw unsafe('sequence must be a positive integer');
      const { rows } = await db.query(
        `SELECT sequence, source_person_id, destination_person_id, reused, address, address_sha256
           FROM test_request_run_reviewer_assignments WHERE run_id = $1::uuid AND sequence = $2::integer`,
        [runId, sequence],
      );
      const row = rows[0];
      if (!row) return null;
