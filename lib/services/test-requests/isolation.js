...
/**
 * Write guard. The marker and run ID are set only when the factory creates a
 * request (POST). Any update of an `akoya_requests` row that names either
 * field, whatever the value, is refused, so no app path can turn a test
 * request ordinary or an ordinary request into a test one. A Dataverse PATCH
 * can also upsert, so every PATCH body is checked, not only updates by ID.
 */
export function assertTestRequestMarkerNotUpdated(entitySet, data) {
  if (String(entitySet ?? '').replace(/^\//, '').toLowerCase() !== TEST_REQUEST_ENTITY_SET) return;
  if (!data || typeof data !== 'object') return;
  const named = Object.keys(data).filter(key => MARKER_FIELD_NAMES.has(key.toLowerCase()));
  if (named.length === 0) return;
  throw Object.assign(
    new Error(`Test Request marker fields (${named.join(', ')}) are set only at creation and cannot be updated.`),
