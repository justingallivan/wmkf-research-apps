...
/**
 * Rollout switch for the synthetic-reviewer READ-side fences (slice 6c-ii Stage
 * A). `wmkf_issyntheticreviewer` ships under a new schema wave
 * (`lib/dataverse/schema/wave30-synthetic-reviewer-marker/`) that is not yet
 * applied to any host (D-R1: "[ASSUMED pending] until recorded here"). A
 * `$select`/`$filter` naming a column Dataverse does not have returns 400, so
 * every read-side fence below must select/filter the marker ONLY once this
 * switch is on for the target host, mirroring `testRequestIsolationEnabled`
 * (same literal-`'on'` shape as `DATAVERSE_DAL_ENFORCEMENT`). While off, a
 * person can never legitimately carry the marker (the column doesn't exist),
 * so treating every row as ordinary is correct, not merely a fallback -- this
 * is the same "no configuration flag claiming success" posture Stage 1 used
 * for `TEST_REQUEST_ISOLATION`. The marker WRITE guard above
 * (`assertTestRequestMarkerNotWritten`) is independent of this switch and
 * always active, exactly as the request-marker write guard is.
 */
export function syntheticReviewerIsolationEnabled(env = process.env) {
  return env?.SYNTHETIC_REVIEWER_ISOLATION === 'on';
}

export const SYNTHETIC_REVIEWER_EXCLUDE_ODATA_FILTER =
  `(${SYNTHETIC_REVIEWER_MARKER_FIELDS.marker} eq false or ${SYNTHETIC_REVIEWER_MARKER_FIELDS.marker} eq null)`;

/** Add the marker to a $select array/string only while the switch is on. */
export function withSyntheticReviewerMarkerSelect(select, env = process.env) {
  if (!syntheticReviewerIsolationEnabled(env)) return select;
  const fields = typeof select === 'string'
    ? select.split(',').map((field) => field.trim()).filter(Boolean)
    : [...(select || [])];
  if (!fields.includes(SYNTHETIC_REVIEWER_MARKER_FIELDS.marker)) fields.push(SYNTHETIC_REVIEWER_MARKER_FIELDS.marker);
  return typeof select === 'string' ? fields.join(',') : fields;
}

/** Compose the marker-exclusion predicate onto an existing filter, switch-gated. */
export function withExcludeSyntheticReviewerODataFilter(filter, env = process.env) {
  if (!syntheticReviewerIsolationEnabled(env)) return filter;
  return filter
    ? `(${filter}) and ${SYNTHETIC_REVIEWER_EXCLUDE_ODATA_FILTER}`
    : SYNTHETIC_REVIEWER_EXCLUDE_ODATA_FILTER;
}

/** True only when the row carries an explicit marker of true. */
export function isSyntheticReviewerRow(row) {
  return row?.[SYNTHETIC_REVIEWER_MARKER_FIELDS.marker] === true;
}

/**
 * Public read DTO fragment for a person row shown with its own identity
 * (Workbench Reviews tab, Reviewer Finder candidate hydration). Never exposes
 * the raw field; off mode returns an empty object so existing response shapes
 * stay byte-for-byte compatible, matching `testRequestVisibilityDto`.
 */
export function syntheticReviewerVisibilityDto(personSnapshot, env = process.env) {
  if (!syntheticReviewerIsolationEnabled(env)) return {};
  return { isSyntheticReviewer: isSyntheticReviewerRow(personSnapshot) };
}
...
export function assertTestRequestMarkerNotWritten(data, target = '') {
  ...
  // OData decodes the path before resolving the property and the URL parser
  // normalizes `\` to `/`, so split on both separators, decode each segment,
  // and split the decoded text again (an encoded `%2F` / `%5C` separator
  // cannot hide a marker). A malformed encoding is unclassifiable and refused.
  const separators = /[\/\\]/;
  // Canonicalize with the URL parser first so the query, the fragment (never
  // transmitted) and `\` separators are handled as the server will see them;
  // an unparsable target is unclassifiable and refused.
  let pathname;
  try {
    pathname = new URL(String(target ?? ''), 'https://marker-check.invalid/').pathname;
  } catch {
    found.add('(unparsable write target)');
    pathname = '';
  }
  for (const segment of pathname.split(separators)) {
    let decoded;
    try {
      decoded = decodeURIComponent(segment);
    } catch {
      found.add(`${segment} (malformed encoding)`);
      continue;
    }
    for (const part of decoded.split(separators)) {
