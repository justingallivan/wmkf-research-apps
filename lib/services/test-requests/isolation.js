...
export const TEST_REQUEST_ORDINARY_OData_FILTER =
  `(${TEST_REQUEST_ISOLATION_FIELDS.marker} eq false and ${TEST_REQUEST_ISOLATION_FIELDS.runId} eq null)`;
 ...
 *
 * `null` is not treated as ordinary. Existing legacy rows may only be
 * normalized after a separately verified projection/default contract; this
 * function deliberately has no caller-controlled legacy exemption.
 */
export function classifyTestRequestSnapshot(snapshot) {
  ...
  if (marker === null) {
    ...
    if (runId === null) return { kind: 'unknown', reason: 'legacy_null_marker_unverified' };
