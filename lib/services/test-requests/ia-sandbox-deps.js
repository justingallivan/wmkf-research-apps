...
import { resolveCanonicalInitialAssessment } from '../initial-assessment/artifact-reader.js';
...
// Re-exported for the runner's own sandbox-bound reads: this module is the
// registered non-facade Dataverse transport for the Factory, so callers take
// the annotation transform from here rather than importing a dynamics/
// submodule (check:dataverse-access-layer) or widening the facade allowlist.
export { processAnnotations };
...
/**
 * `resolveCanonical` (Stage C): `createInitialAssessmentBoardSnapshot` calls
 * `dependencies.resolveCanonical({ requestId, expectedArtifactId })` directly
 * (controls-service.js), so this must be a ONE-ARGUMENT function, not the raw
 * `resolveCanonicalInitialAssessment(args, { dependencies })` two-argument
 * export. `resolveCanonicalInitialAssessment`'s own `DEFAULT_DEPENDENCIES`
 * (artifact-reader.js) needs only `getRequest`/`findByRequest` -- both are
 * already sandbox-bound reads above -- so this closes over those two directly
 * rather than the full frozen `createIaSandboxDeps` return value (which would
 * require a circular self-reference).
 */
function sandboxResolveCanonical(svc) {
  const dependencies = {
  ...
  return (args) => resolveCanonicalInitialAssessment(args, { dependencies });
}
 ...
 *
 * Stage C: `controls-service.js`'s `createInitialAssessmentBoardSnapshot`
 * hardcodes `actorPolicy: REQUEST_DOCUMENT_ACTOR_POLICY.REQUIRED` on its own
 * create call (that file's registered writer-gate seam, unchanged) because
 * its production caller has a real staff actor. A sandbox rehearsal run
 * never has one, so this wrapper ALWAYS overrides whatever `actorPolicy` /
 * `actingUserSystemId` a caller supplies to `SANDBOX_REHEARSAL` / none —
 * this is a no-op for `stepSeedInitialAssessment`'s own call (already
 * SANDBOX_REHEARSAL, matching the writer-gate registration for
 * run-runner.js) and turns the snapshot function's REQUIRED into the same
 * no-system-user, no-operational-event policy for the sandbox path only;
 * `controls-service.js`'s own source text (and its writer-gate check) is
 * untouched, and every other, non-sandbox caller of that function still goes
 * through its DEFAULT_DEPENDENCIES.createDocument (the adapter's own create
 * export, called directly, never through this module) and keeps REQUIRED
 * for real.
 */
 ...
 * @param {object} [params.graph] - optional Graph dependency object already
 *   bound to a run's sharePointTarget/manifest (the `graph` runner
 *   dependency every Test Request Factory step receives) exposing
 *   `ensureFolderPath`/`uploadFile`/`getFileMetadataById`/`downloadFile`/
 *   `getFileMetadataByPath`/`getFileVersionMetadata`/`downloadFileVersion`.
 *   When supplied, those methods are forwarded verbatim under the same
 *   DEFAULT_DEPENDENCIES key names controls-service.js uses, so ONE sandbox
 *   deps object can serve a step needing both Dataverse and Graph deps
 *   (Stage B/C) rather than a caller mixing this object with `graph` calls
 *   directly. This module never derives a Graph target of its own -- the
 *   target always comes from the caller's `graph`, never the environment.
 *   `restoreFileVersion` is deliberately NOT forwarded:
 *   `createInitialAssessmentBoardSnapshot` (the only controls-service.js
 *   function the Factory's recipe calls) never reaches it --
 *   `restoreInitialAssessmentVersion` is the only caller, and that function
 *   is not part of the Factory recipe.
 * @returns {{findByGenerationKey: Function, findByRequest: Function, getRequest: Function, runChangeset: Function, createDocument: Function, updateDocument: Function, resolveCanonical: Function, hashDocx: Function, newClaimToken: Function, now: Function}}
 */
export function createIaSandboxDeps({ resourceUrl, graph = null } = {}) {
  ...
  return Object.freeze({
    ...
    resolveCanonical: sandboxResolveCanonical(svc),
    hashDocx: hashGovernedDocxContent,
    newClaimToken: () => crypto.randomUUID(),
    now: () => new Date(),
    ...(graph ? {
      ensureFolderPath: (...args) => graph.ensureFolderPath(...args),
      uploadFile: (...args) => graph.uploadFile(...args),
      getFileMetadataById: (...args) => graph.getFileMetadataById(...args),
      getFileMetadataByPath: (...args) => graph.getFileMetadataByPath(...args),
      getFileVersionMetadata: (...args) => graph.getFileVersionMetadata(...args),
      downloadFile: (...args) => graph.downloadFile(...args),
      downloadFileVersion: (...args) => graph.downloadFileVersion(...args),
    } : {}),
  });
}
