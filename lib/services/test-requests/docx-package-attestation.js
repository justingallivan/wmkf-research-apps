...
async function parseOpcXml(xml) {
  return parseStringPromise(String(xml), {
...
async function documentRoot(xml) {
  let doc;
  try {
    doc = await parseOpcXml(xml);
  } catch {
  ...
  if (!root?.$ns?.local) return null;
  return { uri: root.$ns.uri || '', local: root.$ns.local };
}
...
async function validateCustomXml(name, bytes, failures) {
  if (/^customXml\/itemProps\d+\.xml$/.test(name)) {
    const root = await documentRoot(bytes.toString('utf8'));
    if (!sameRoot(root, CUSTOMXML_PROPS_ROOT)) failures.push(`customXml part ${name} is not a SharePoint datastore item`);
  } else if (/^customXml\/_rels\/item\d+\.xml\.rels$/.test(name)) {
    const parsed = await relationships(bytes.toString('utf8'), name);
    ...
    const root = await documentRoot(bytes.toString('utf8'));
    if (!SHAREPOINT_CUSTOMXML_ITEM_ROOTS.some((known) => sameRoot(root, known))) {
