...
export async function attestDocxPackageAgainstSource(destinationBytes, sourceBytes, budget = DOCX_PACKAGE_BUDGET) {
  ...
  for (const [name, bytes] of actual) {
    ...
    if (name === CONTENT_TYPES_PART) {
      ...
      const expectedOverrides = uniqueMap(overridesFull(source.get(name) || ''), (o) => o.PartName, 'override');
      const foundOverrides = uniqueMap(overridesFull(bytes), (o) => o.PartName, 'override');
      ...
      const expectedDefaults = uniqueMap(defaultsFull(source.get(name) || ''), (d) => String(d.Extension || '').toLowerCase(), 'default extension');
      const foundDefaults = uniqueMap(defaultsFull(bytes), (d) => String(d.Extension || '').toLowerCase(), 'default extension');
