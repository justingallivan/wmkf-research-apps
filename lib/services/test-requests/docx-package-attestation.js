...
const TRASH = /^\[trash\]\//;
// Word/SharePoint `[trash]` entries are zero-filled padding left by in-place
// rewrites (`[trash]/0000.dat`, `[trash]/0001.dat` observed live 2026-09-24).
// A `[trash]` entry is normalized ONLY when it is canonically named and every
// byte is 0x00 (Codex slice review round 3): a blanket exemption accepted an
// arbitrary hidden payload under any `[trash]/` name. Zero bytes carry no
// information, so no cross-side comparison is needed.
...
function validateTrash(name, bytes, failures) {
  ...
  if (bytes.some((b) => b !== 0)) failures.push(`trash entry ${name} carries non-zero bytes`);
}
...
// A SharePoint-added customXml part is small (the live promoted IA package
// grew by ~7.4 KB across three items, three props parts, three rels parts
// and two trash entries, 2026-09-24). A characterized part carrying far more
// than that is a payload, whatever its root says.
const CUSTOMXML_PART_MAX_BYTES = 32 * 1024;
const XML_NS = 'http://www.w3.org/XML/1998/namespace';
const SITE_COLUMN_NS = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
// Namespaces every element and every prefixed attribute inside a customXml
// item may use, per recognized root (Codex reviewer-differs-from-author
// round: root recognition alone let an allowed root carry a 4 KiB
// foreign-namespace payload). SharePoint site-column namespaces are GUIDs.
const CUSTOMXML_ALLOWED_NS = {
  contentTypeSchema: new Set([
  ...
  FormTemplates: new Set(['http://schemas.microsoft.com/sharepoint/v3/contenttype/forms']),
  properties: new Set([
    'http://schemas.microsoft.com/office/2006/metadata/properties',
    'http://www.w3.org/2001/XMLSchema-instance',
    'http://schemas.microsoft.com/office/infopath/2007/PartnerControls',
    '', // the unprefixed <documentManagement> child
    XML_NS,
  ]),
  datastoreItem: new Set(['http://schemas.openxmlformats.org/officeDocument/2006/customXml']),
};
 ...
 * `Target` and `TargetMode`; any other element, a missing/duplicate `Id`, or
 * unparsable XML is a failure.
 */
async function relationships(xml, label = 'relationships part') {
  ...
  for (const child of root.kids || []) {
    ...
    const attrs = localAttributes(child, failures, label);
 ...
 * element or unparsable XML is a failure. (Duplicate PartName / Extension
 * detection stays in the comparators, which report them per side.)
 */
async function contentTypes(xml, label = '[Content_Types].xml') {
  ...
  for (const child of root.kids || []) {
    const attrs = localAttributes(child, failures, label);
    if (child.$ns?.uri === CONTENT_TYPES_NS && child.$ns?.local === 'Override') overrides.push({ PartName: attrs.PartName, ContentType: attrs.ContentType });
    else if (child.$ns?.uri === CONTENT_TYPES_NS && child.$ns?.local === 'Default') defaults.push({ Extension: attrs.Extension, ContentType: attrs.ContentType });
 ...
 */
/** The parsed document root as `{ uri, local }`, or null when the part is not well-formed XML. */
/** Parse a customXml part; returns `{ root: { uri, local }, node }` or null when not well-formed. */
...
/**
 * Walk every element and attribute under `node`; any namespace outside the
 * root's allowlist is a failure. Namespace declarations themselves are not
 * checked (declaring a namespace is harmless; using one is not).
 */
function assertAllowedNamespaces(node, allowed, name, failures, depth = 0) {
  if (depth > 64) { failures.push(`customXml part ${name} nests deeper than 64 elements`); return; }
  const uri = node.$ns?.uri || '';
  if (!allowed.has(uri) && !SITE_COLUMN_NS.test(uri)) {
    failures.push(`customXml part ${name} carries element ${uri}:${node.$ns?.local || node['#name']} outside the SharePoint namespace set`);
    return;
  }
  for (const attribute of Object.values(node.$ || {})) {
    if (attribute.prefix === 'xmlns' || attribute.name === 'xmlns') continue;
    const attrUri = attribute.uri || '';
    if (attribute.prefix && !allowed.has(attrUri) && !SITE_COLUMN_NS.test(attrUri)) {
      failures.push(`customXml part ${name} carries attribute ${attribute.name} outside the SharePoint namespace set`);
      return;
    }
  }
  for (const child of node.kids || []) {
    if (child['#name'] === '__text__') continue;
    assertAllowedNamespaces(child, allowed, name, failures, depth + 1);
  }
}

async function validateCustomXmlItem(name, bytes, roots, failures, notLabel) {
  if (bytes.length > CUSTOMXML_PART_MAX_BYTES) { failures.push(`customXml part ${name} exceeds the ${CUSTOMXML_PART_MAX_BYTES}-byte ceiling for a SharePoint-added part`); return; }
  const parsed = await parsedDocument(bytes);
  ...
  assertAllowedNamespaces(parsed.node, CUSTOMXML_ALLOWED_NS[known.local], name, failures);
}

async function validateCustomXml(name, bytes, failures) {
  if (/^customXml\/itemProps\d+\.xml$/.test(name)) {
    await validateCustomXmlItem(name, bytes, [CUSTOMXML_PROPS_ROOT], failures, 'datastore item');
  } else if (/^customXml\/_rels\/item\d+\.xml\.rels$/.test(name)) {
    const parsed = await relationships(bytes, name);
    failures.push(...parsed.failures);
    const rels = parsed.rels;
    if (!rels.length || rels.some((r) => r.Type !== CUSTOMXML_PROPS_REL_TYPE || (r.TargetMode || 'Internal') !== 'Internal')) {
      failures.push(`customXml relationships part ${name} carries a non-customXmlProps relationship`);
    }
  } else {
    await validateCustomXmlItem(name, bytes, SHAREPOINT_CUSTOMXML_ITEM_ROOTS, failures, 'property-promotion item');
  }
}
...
export async function attestDocxPackageAgainstRender(actualBytes, renderBytes, budget = DOCX_PACKAGE_BUDGET) {
  ...
  for (const [name, bytes] of actual) {
    ...
    if (SHAREPOINT_CUSTOMXML.test(name)) { await validateCustomXml(name, bytes, failures); normalizedParts.push(name); continue; }
    if (name === DOCUMENT_RELS_PART) {
      const expectedParsed = await relationships(render.get(name) || '', 'render document relationships');
      ...
      for (const rel of found) {
        ...
        else if (!known && String(rel.TargetMode || 'Internal') !== 'Internal') failures.push(`document relationship ${rel.Id} (${rel.Target}) is an external customXml relationship`);
      }
    ...
    if (name === CONTENT_TYPES_PART) {
      ...
      // source file.
      const expectedCt = await contentTypes(render.get(name) || '', 'render [Content_Types].xml');
      const foundCt = await contentTypes(bytes);
      failures.push(...expectedCt.failures, ...foundCt.failures);
      const expected = expectedCt.overrides.map((o) => o.PartName);
      const found = foundCt.overrides.map((o) => o.PartName);
...
export async function attestDocxPackageAgainstSource(destinationBytes, sourceBytes, budget = DOCX_PACKAGE_BUDGET) {
  ...
  for (const [name, bytes] of actual) {
    ...
    if (name === DOCUMENT_RELS_PART) {
      const expectedParsed = await relationships(source.get(name) || '', 'source document relationships');
      const foundParsed = await relationships(bytes, 'document relationships');
      failures.push(...expectedParsed.failures, ...foundParsed.failures);
      const expected = expectedParsed.rels;
      const found = foundParsed.rels;
    ...
    if (name === CONTENT_TYPES_PART) {
      ...
      // PartName / Extension entries (last wins), so a destination listing a
      // rogue override first and the original last would pass. Refuse
      // duplicates on either side; OPC Default extensions are case-insensitive.
      ...
      const expectedCt = await contentTypes(source.get(name) || '', 'source [Content_Types].xml');
      const foundCt = await contentTypes(bytes);
      failures.push(...expectedCt.failures, ...foundCt.failures);
