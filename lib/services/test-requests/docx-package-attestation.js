...
async function attestPromotion(actual, baseline, { baselineLabel, fullContentTypes }) {
  ...
  // Every baseline part must survive. `docProps/custom.xml` is the one part
  // SharePoint may ADD (so its absence from the baseline is fine); nothing
  // is ever removed by promotion, so a baseline part missing from the
  // destination -- core.xml included -- fails (Codex final re-review).
  for (const name of baseline.keys()) {
    if (!actual.has(name)) failures.push(`part ${name} is missing`);
  }
