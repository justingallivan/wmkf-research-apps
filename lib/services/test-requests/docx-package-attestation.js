...
export const CUSTOMXML_LIMITS = Object.freeze({
  ...
  elements: 1024, depth: 24, textChars: 256, columnValueChars: 1024, attributeChars: 256,
});
...
async function validateCustomXmlItem(name, bytes, roots, maxBytes, failures, notLabel) {
...
async function validateCustomXml(name, bytes, failures) {
  if (/^customXml\/itemProps\d+\.xml$/.test(name)) {
    ...
    if (rels.length !== 1 || rels.some((r) => r.Type !== CUSTOMXML_PROPS_REL_TYPE || (r.TargetMode || 'Internal') !== 'Internal' || !/^itemProps\d+\.xml$/.test(r.Target || ''))) {
      failures.push(`customXml relationships part ${name} carries a non-customXmlProps relationship`);
    }
  } else {
    await validateCustomXmlItem(name, bytes, SHAREPOINT_CUSTOMXML_ITEM_ROOTS, CUSTOMXML_LIMITS.itemBytes, failures, 'property-promotion item');
  }
}
