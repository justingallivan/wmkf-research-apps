...
import { planReviewFileCopies, REVIEW_FILE_COPY_POLICY, reviewFileCopyPolicyDigest, validateReviewFilePlan } from './review-file-copy.js';
...
import {
  isSyntheticNameDerived, syntheticPersonProjection,
...
function flattenFileCopyEntry(entry, index) {
  return compact({
    ...
    // The DESTINATION item's own post-write size (Opus round 1 P1-1):
    // distinct from `size` above (the source's pre-copy size), and
    // required because SharePoint's DOCX property promotion changes the
    // destination's byte count -- a DOCX folder census must compare
    // against what the destination actually reads back as, not the
    // source's size, and that value must survive a resume/out-of-process
    // verifier, not just live in bundle-file-copy.js's in-memory entry.
    itemSize: entry.item?.size,
    // The SOURCE's own eTag/versionId at copy time (P3-d, Opus round 1):
    // evidence of exactly which source revision was copied, reusing the
    // generic `eTagBefore`/`sourceVersionId` receipt keys already
    // allowlisted (6c-i) rather than adding new ones -- their grammar is
    // shape-only and does not encode which resource kind or semantic use
    // journals them.
    eTagBefore: entry.source?.eTag,
    sourceVersionId: entry.source?.versionId,
    // The source MIME type (Fable, Stage C pre-review check, Opus P3-1): the
    // verifier compares each journaled review file against the BUNDLE by
    // hash and MIME, so the receipt must carry it (ledger key `mimeType`).
...
function inflateFileCopyEntry(row) {
  ...
  return {
    ...
    source: {
      size: flat.size ?? null, contentHash: flat.contentHash ?? null, mimeType: flat.mimeType ?? null, graphItemId: flat.sourceGraphItemId ?? null,
      eTag: flat.eTagBefore ?? null, versionId: flat.sourceVersionId ?? null,
    },
    destinationDriveId: flat.driveId ?? null,
    sourceDriveId: flat.sourceDriveId ?? null,
    attestedDigest: flat.attestedDigest ?? null,
    item: flat.itemId ? { id: flat.itemId, eTag: flat.eTag ?? null, versionId: flat.versionId ?? null, size: flat.itemSize ?? null } : null,
  };
}

/**
 * Caller error: the supplied manifest/bundle no longer match the reserved
 * run. Never touches the run row. Runs BEFORE any lease is claimed (F2,
 * Codex slice 6c-ii Stage C round 1): for a `reviews` run this also (a)
 * requires the manifest's review-file policy digest to equal the LIVE
 * policy (ledger-independent -- catches a stale manifest even if the run
 * row itself were somehow unreadable), (b) recomputes the plan digest from
 * the manifest plus the ledger's own reviewer-assignment addresses via the
 * SAME shared helper the reservation script used, and requires it to equal
 * `run.planDigest`, and (c) re-runs `validateReviewFilePlan` for the run's
 * assignments against the CURRENT bundle/policy. `listRunReviewerAssignments`
 * is not otherwise read at this point in `advanceRun` (every existing read
 * happens inside a per-step handler, after the lease is already claimed),
 * so this is a genuinely new Postgres read, added deliberately rather than
 * reused -- gated to `reviews` runs only so basic/initial_assessment never
 * pay for it.
 */
async function assertRunMatchesManifestAndBundle(run, manifest, bundle, ledger) {
  const mismatches = [];
  ...
  const recipe = manifest.recipe ?? 'basic';
  if (recipe !== run.recipe) mismatches.push('recipe');
  ...
  if (recipe === 'reviews' && manifest.reviewFilePolicy?.digest !== reviewFileCopyPolicyDigest()) {
    mismatches.push('reviewFilePolicyDigest');
  }
  ...
  if (recipe === 'reviews') {
    ...
    // listRunReviewerAssignments never returns a plaintext address (D-R2 /
    // migration 054's "no plaintext" posture) -- only `addressSha256`,
    // which is exactly what computeRunPlanDigest needs.
    const recomputedPlanDigest = computeRunPlanDigest({ manifest, reviewerAddressDigests: assignments.map((a) => a.addressSha256) });
    if (recomputedPlanDigest !== run.planDigest) {
      throw new Error('Manifest/bundle does not match the reserved run (planDigest).');
    }
    validateReviewFilePlan(bundle, assignments.map((a) => a.sourcePersonId));
  }
}
...
async function verifyBasicAndInitialAssessment({
  ...
  let graphError = null;
  try {
    files = await listSharePointFiles(graph, observation, { maxDepth: censusMaxDepth });
  } catch (error) {
    graphError = error.message;
  }
...
async function resolveReviewerPerson({
  ...
  // The ONE ownership invariant for a destination person row, applied to
  // every path that resolves one (Codex slice review round 3): the reuse
  // path, the dispatch-marker recovery read, and the post-POST readback. A
  // marker-true row alone proved nothing about ownership -- a concurrent
  // edit, a Contact binding or a foreign marker-true row at the
  // preallocated GUID could be adopted and then receive a suggestion,
  // files and answers before terminal verification caught it.
  function assertOwnedSyntheticPerson(row, label) {
    if (!syntheticActiveContactless(row)) {
      throw codedError(`${label} destination person is not an active, Contact-less synthetic reviewer.`, 'reviewer_person_not_synthetic');
    }
    ...
    for (const [field, expectedValue] of Object.entries(expected)) {
      ...
      if (actual !== (expectedValue ?? null)) {
        throw codedError(`${label} destination person field ${field} does not match the bundle's synthetic-person projection.`, 'reviewer_person_projection_drift');
      }
    }
    if (!isSyntheticNameDerived(row)) {
      throw codedError(`${label} destination person's derived name does not carry the synthetic prefix.`, 'reviewer_person_projection_drift');
    }
  }
  ...
  if (assignment.reused) {
    ...
    assertOwnedSyntheticPerson(row, 'Reused');
  ...
  if (resource.outcome === 'verified' || resource.outcome === 'recovered') {
    // A terminal receipt proves the row WAS owned when it was written, not
    // that it still is: a resume between person creation and suggestion
    // creation must reread and re-assert before anything binds to the row
    // (Codex reviewer-differs-from-author round).
    const row = await deps.getPersonById(assignment.destinationPersonId, { select: personSelect });
    assertOwnedSyntheticPerson(row, 'Journaled');
    return { resource, destinationPersonId: assignment.destinationPersonId };
  }
  ...
  if (resource.readback?.personCreateAttemptedAt) {
    ...
    // The resource stays `dispatched` (marker intact) on a refusal: never
    // re-POST, and a corrected row can be recovered by a later advance.
    assertOwnedSyntheticPerson(row, 'Recovered');
    resource = await ledger.recordResourceReadback({
      resourceId: resource.resourceId, runId: run.runId, leaseToken: run.leaseToken, leaseGeneration: run.leaseGeneration,
      responseStatus: null, readback: compact({ destinationPersonId: assignment.destinationPersonId }), outcome: 'recovered',
    });
  ...
  assertOwnedSyntheticPerson(createdRow, 'Created');
  resource = await ledger.recordResourceReadback({
    resourceId: resource.resourceId, runId: run.runId, leaseToken: run.leaseToken, leaseGeneration: run.leaseGeneration,
...
// Folder levels below the Request folder that the reviews terminal census
// walks. Deepest folder this recipe creates is depth 3
// (`Reviewer_Uploads/<reviewer>/attempt_<id>`); 6 leaves headroom so any
// nested folder an outside writer adds under an attempt is still counted.
const REVIEWS_CENSUS_MAX_DEPTH = 6;
  ...
  if (next) {
    ...
    if (verifiedCount >= plannedFiles.length) {
      ...
      const dependencies = {
        ...
        // Magic-byte sniff on the verified source bytes, before any Graph
        // write (P1-2, Opus round 1): a renamed file (e.g. a `.docx` name
        // over PDF bytes) must never reach `ensureFolderPath`/upload. Only
        // the review-file copy path supplies this hook -- Basic `copy_file`
        // (line ~753) intentionally does not, since it never re-labels a
        // file's extension the way review uploads' synthetic naming does.
        validateBytes: (filename, buffer) => validateReviewFile(filename, buffer),
      };
      ...
      const journal = async (copies) => {
        ...
        const flat = flattenFileCopyEntry(entry, index);
        if (entry.status === 'failed') {
          // Prefer a caller-assigned reason code (e.g. `file_rejected` from
          // the magic-byte sniff, P1-2) over the generic `file_copy_failed`.
          const row = await ledger.recordResourceFailure({
            resourceId: fileResource.resourceId, runId: run.runId, leaseToken: run.leaseToken, leaseGeneration: run.leaseGeneration,
            outcome: 'failed', error: codedError(entry.error, entry.errorCode || 'file_copy_failed'),
          });
      ...
      // Defense-in-depth against a resumed attempt id diverging from the
      // journaled folder (P2-1, Opus round 1): attemptId above is always
      // recovered from folderResource.plannedIdentity.folder when resuming
      // (never freshly generated in that branch), so planReviewFileCopies
      // building a different folder for the same attemptId would mean a
      // planner regression, not operator error -- refuse rather than
      // silently uploading into an un-journaled folder.
      if (plannedFiles[index]?.destination?.folder !== folderResource.plannedIdentity?.folder) {
        throw codedError(
          `copy_review_file: planned folder ${plannedFiles[index]?.destination?.folder} disagrees with the journaled folder ${folderResource.plannedIdentity?.folder}.`,
          'file_journal_unverified',
        );
      }
  ...
  if (next) {
    ...
    if (bundleReviewer.reviewForm === 'unreceived') {
      ...
      if (resource.outcome === 'verified' || resource.outcome === 'recovered') {
        ...
        // F3 (Codex slice 6c-ii Stage C round 1): a readback with no eTag
        // must never record a terminal outcome -- the changeset already ran
        // (dispatch-marker rule forbids re-dispatch) and verify_reviews
        // refuses `uploaded`/`received_no_file` reviews with no journaled
        // eTagAfter, so recording `recovered` with none would strand the
        // run with no path to ready and no way to repeat the readback
        // (the resource would already be terminal). Leave the resource
        // `dispatched` (its attempt marker intact) and throw; the NEXT
        // advance re-enters this SAME branch and repeats the readback only.
        if (typeof row._etag !== 'string' || row._etag.length === 0) {
          throw codedError('seed_review_answers: the resumed readback returned no eTag; the changeset already ran and is never re-dispatched. Advance again to repeat the readback.', 'reviewer_answers_ambiguous');
        }
        resource = await ledger.recordResourceReadback({
          resourceId: resource.resourceId, runId: run.runId, leaseToken: run.leaseToken, leaseGeneration: run.leaseGeneration,
          responseStatus: null, readback: compact({ eTagAfter: row._etag }), outcome: 'recovered',
        });
        resources.push(resource);
      } else {
        ...
        // F3: same guard as the resume branch above -- the changeset already
        // ran, so a readback with no eTag must never be recorded as
        // `verified`. Leave the resource `dispatched` (attempt marker
        // intact) and throw; the next advance takes the `changesetAttemptedAt`
        // resume branch above, which repeats the readback (never the
        // changeset) and records `recovered` once it sees an eTag.
        if (typeof after?._etag !== 'string' || after._etag.length === 0) {
          throw codedError('seed_review_answers: the post-changeset readback returned no eTag; the changeset already ran and is never re-dispatched. Advance again to repeat the readback.', 'reviewer_answers_ambiguous');
        }
        resource = await ledger.recordResourceReadback({
          resourceId: resource.resourceId, runId: run.runId, leaseToken: run.leaseToken, leaseGeneration: run.leaseGeneration,
...
// Token/honorarium/proposal-access fields (P2-3, Opus round 1): a synthetic
// test suggestion must never carry a real external-access token, first-view
// timestamp, or a link to a real honorarium request -- all six field names
// [VERIFIED via lib/dataverse/core/entity-registry.js:161-165,200] on the
// live reviewer-suggestion entity registry.
// Fields a synthetic suggestion must never carry a LIVE value in. Five are
// nullable and must be null; `wmkf_externaltokenrevoked` is a Boolean that
// Dataverse defaults to `false` on create (live proof run 83c5da2e, sandbox
// Request 1000343, 2026-09-26: every seeded suggestion read back `false`
// although the seeder never writes it), so only `true` is a live value there.
const SUGGESTION_NO_LIVE_ACCESS_NULL_FIELDS = Object.freeze([
  'wmkf_externaltokenhash', 'wmkf_externaltokenissued', 'wmkf_externaltokenexpires',
  'wmkf_proposalfirstaccessed', '_wmkf_honorariumrequest_value',
]);
const SUGGESTION_NO_LIVE_ACCESS_FALSE_FIELDS = Object.freeze(['wmkf_externaltokenrevoked']);
const SUGGESTION_NO_LIVE_ACCESS_FIELDS = Object.freeze([...SUGGESTION_NO_LIVE_ACCESS_NULL_FIELDS, ...SUGGESTION_NO_LIVE_ACCESS_FALSE_FIELDS]);
const SUGGESTION_VERIFY_FIELDS = Object.freeze([
  ...
  ...SUGGESTION_NO_LIVE_ACCESS_FIELDS,
]);
...
async function verifyReviewerPerson(deps, assignment, bundleReviewer) {
  ...
  if (mismatches.length) {
    // The projection includes `wmkf_issyntheticreviewer: true`, so a false or
    // missing marker is refused HERE; a separate marker check after this line
    // could never fire (Fable, Stage C pre-review check: the Opus M8 mutation
    // survived because that redundant check was unreachable). The mutation that
    // pins the marker invariant is dropping the marker from the projection.
  ...
  if (row.statecode !== 0) {
    throw verificationFailed('destination person is not active.');
  }
  if (!isSyntheticNameDerived(row)) {
    throw verificationFailed('destination person\'s derived name does not carry the synthetic prefix.');
  }
}
...
async function verifyOneReview({
  ...
  const liveAccessField = SUGGESTION_NO_LIVE_ACCESS_NULL_FIELDS.find((field) => row[field] != null)
    || SUGGESTION_NO_LIVE_ACCESS_FALSE_FIELDS.find((field) => row[field] != null && row[field] !== false);
  if (liveAccessField) {
    throw verificationFailed(`destination suggestion carries a non-null ${liveAccessField}; a synthetic test suggestion must never have real external-access or honorarium fields set.`);
  }
  ...
  // A missing eTagAfter must refuse for 'uploaded'/'received_no_file' (P2-2,
  // Opus round 1) -- both forms issue a real seed_review_answers changeset,
  // so an absent post-changeset eTag means the answers step's own receipt
  // is incomplete, not that there was nothing to journal. 'unreceived'
  // reviews legitimately seed zero answers and can carry no eTagAfter.
  if (!journaledEtagAfter) {
    if (bundleReviewer.reviewForm === 'uploaded' || bundleReviewer.reviewForm === 'received_no_file') {
      throw verificationFailed('destination suggestion has no journaled post-changeset eTag to verify against.');
    }
  } else if (row._etag !== journaledEtagAfter) {
  ...
  if (bundleReviewer.reviewForm === 'uploaded') {
    ...
    // The combined folder census in verifyBasicAndInitialAssessment already
    // re-verifies every verified review-file resource against SharePoint
    // (identical reverifyCopiedItems logic), so repeating it here could never
    // be the first check to fail (Opus round 2, P3-1). What the census does
    // NOT do is compare the journaled copies against the BUNDLE: count,
    // ordinal filename, source hash and MIME type per file.
    const fileResources = existingResources
      .filter((r) => r.step === COPY_REVIEW_FILE_STEP && r.resourceKind === REVIEW_FILE_RESOURCE_KIND
        && r.plannedIdentity?.assignmentSequence === assignment.sequence && r.outcome === 'verified')
      .map((resource) => inflateFileCopyEntry(resource));
    const bundleFiles = bundleReviewer.files || [];
    // Count check: defense-in-depth for a journal inconsistency (more verified
    // resources than bundle files). A bundle with FEWER files than the source
    // never reaches here (the source fence refuses it as source_changed), and
    // a bundle file with no journaled copy is caught by the per-file lookup.
    if (fileResources.length !== bundleFiles.length) {
      throw verificationFailed(`uploaded review has ${fileResources.length} verified file(s) but the bundle lists ${bundleFiles.length}.`);
    }
    const byFilename = new Map(fileResources.map((entry) => [entry.destination?.filename, entry]));
    if (byFilename.size !== fileResources.length) throw verificationFailed('uploaded review journaled the same destination filename twice.');
    bundleFiles.forEach((file, index) => {
      const ext = String(file.name || '').toLowerCase().match(/\.([a-z0-9]+)$/)?.[1];
      const expectedFilename = `Review_${index + 1}.${ext}`;
      const entry = byFilename.get(expectedFilename);
      if (!entry) throw verificationFailed(`uploaded review is missing journaled file ${expectedFilename}.`);
      if (entry.source?.contentHash !== file.contentHash) throw verificationFailed(`${expectedFilename} journaled source hash does not match the bundle.`);
      if (entry.source?.mimeType !== file.mimeType) throw verificationFailed(`${expectedFilename} journaled MIME type does not match the bundle.`);
    });
...
async function stepVerifyReviews({ run, ledger, manifest, bundle, client, graph, sharePointTarget }) {
  ...
  // the attempt folders themselves but not anything nested INSIDE one, so
  // a rogue file at `Reviewer_Uploads/<reviewer>/attempt_<id>/x/y` was
  // invisible to the terminal verifier. The reviews census walks deeper
  // than any folder this recipe creates.
  const { observation } = await verifyBasicAndInitialAssessment({
    run, ledger, manifest, client, graph, sharePointTarget, preflight, existingResources, extraFileCopies: reviewFileCopies,
    censusMaxDepth: REVIEWS_CENSUS_MAX_DEPTH,
  });
...
export async function advanceRun({ runId, ledger, manifest, bundle, deps, options = {} }) {
  ...
  await assertRunMatchesManifestAndBundle(run, manifest, bundle, ledger);
