...
async function stepVerify({ run, ledger, manifest, client, graph, sharePointTarget }) {
  ...
  const priorBaselines = existingResources
    .filter((row) => row.resourceKind === 'foundation_baseline' && row.readback?.foundationBaselineSha256)
    .map((row) => row.readback.foundationBaselineSha256);
  if (priorBaselines.length > 0) {
    if (priorBaselines.some((prior) => prior !== digest)) {
      throw codedError('Foundation/Contact rows changed since this run recorded its baseline; resolve manually.', 'ia_verification_failed');
    }
  } else {
...
const STEP_HANDLERS = {
  ...
  observe: stepObserve,
  verify: stepVerify,
  seed_initial_assessment: notBuiltStep('seed_initial_assessment'),
