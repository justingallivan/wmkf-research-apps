...
async function getIaSeedRequest(client, requestId) {
  ...
  return DynamicsService.processAnnotations(bodyOrThrow('destination Request readback (seed_initial_assessment)', response));
}
...
async function stepSeedInitialAssessment({ run, ledger, client, graph, sharePointTarget }) {
  ...
  if (!row) {
    ...
    if (!row) {
      throw codedError(
        'The Initial Assessment registry create failed and the row is not readable; resolve manually, never re-POST.',
        'ambiguous_create_outcome',
      );
    }
