...
async function stepSeedInitialAssessment({ run, ledger, client, graph, sharePointTarget }) {
  ...
  if (row?.wmkf_operationstatus === REQUEST_DOCUMENT_OPERATION_STATUS.READY) {
    ...
    if (markers.itemId && !sameSandboxId(row.wmkf_sharepointitemid, markers.itemId)) {
      throw codedError(
        `The Ready Initial Assessment item ${row.wmkf_sharepointitemid} does not match the previously journaled item ${markers.itemId}; inspect before resuming.`,
        'ia_pointer_mismatch',
      );
    }
...
async function stepSeedInitialAssessmentSnapshot({ run, ledger, client, graph, sharePointTarget }) {
  ...
  async function wrappedUploadFile(library, folder, filename, content, contentType, options = {}) {
    await merge({ uploadAttemptedAt: new Date().toISOString(), filename });
    const onItemCreated = async (item) => {
      await merge({ itemId: item.id, driveId: item.driveId, siteId: item.siteId });
    };
...
async function stepVerifyInitialAssessment({ run, ledger, manifest, client, graph, sharePointTarget }) {
  ...
  if (seedResource.readback.contentHash && governedHashToHex(iaRow.wmkf_contenthash) !== seedResource.readback.contentHash) {
    throw codedError('The Initial Assessment content hash no longer matches the hash the seed step journaled.', 'ia_pointer_mismatch');
  }
  if (seedResource.readback.sourceVersionId && String(iaRow.wmkf_sharepointversionid) !== String(seedResource.readback.sourceVersionId)) {
    throw codedError('The Initial Assessment SharePoint version no longer matches the version the seed step journaled.', 'ia_pointer_mismatch');
  }
  ...
  if (snapshotResource.readback.contentHash && governedHashToHex(snapshotRow.wmkf_contenthash) !== snapshotResource.readback.contentHash) {
    throw codedError('The Board snapshot content hash no longer matches the hash the snapshot step journaled.', 'ia_pointer_mismatch');
  }
  if (snapshotResource.readback.versionId && String(snapshotRow.wmkf_sharepointversionid) !== String(snapshotResource.readback.versionId)) {
    throw codedError('The Board snapshot SharePoint version no longer matches the version the snapshot step journaled.', 'ia_pointer_mismatch');
  }
  ...
  const rawSha256Hex = (buffer) => crypto.createHash('sha256').update(buffer).digest('hex');
  ...
  // Only the recipe's final step may markReady; this is a defensive sanity
  // check, not a runtime condition -- RECIPE_STEP_ORDER already makes this
  // step the last entry for `initial_assessment`.
  if (nextStepFor(run.recipe, IA_VERIFY_STEP) !== null) {
    throw new Error('verify_initial_assessment must be the initial_assessment recipe\'s final step.');
  }
  const ready = await ledger.markReady({
    runId: run.runId, leaseToken: run.leaseToken, leaseGeneration: run.leaseGeneration, expectedVersion: run.version,
    destinationRequestNumber: observation.request.akoya_requestnum,
...
// Slice 6a's `recipe_step_not_built` stub (`notBuiltStep`) covered these two
// IA steps before their bodies landed (Stage C); both are now built above and
// wired into STEP_HANDLERS. `recipe_step_not_built` remains a valid
// LEDGER_REASON_CODES member (a future recipe's stub steps may still use it)
// but nothing in this file constructs it anymore.
