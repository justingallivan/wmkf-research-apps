...
async function stepSeedInitialAssessment({ run, ledger, client, graph }) {
  ...
  const dependencies = createIaSandboxDeps({ resourceUrl: iaSandboxResourceUrl(client) });
  ...
  if (!row) {
    ...
    await ensureResource();
    await merge({ generationKey, registryCreateAttemptedAt: new Date().toISOString() });
  ...
  await ensureResource();
  if (!receipt.requestDocumentId) await merge({ requestDocumentId: row.wmkf_requestdocumentid, generationKey });
  ...
  await graph.ensureFolderPath('akoya_request', row.wmkf_sharepointfolderpath);

  await merge({ uploadAttemptedAt: new Date().toISOString() });
  const onItemCreated = async (item) => {
    await merge({ itemId: item.id });
  };
