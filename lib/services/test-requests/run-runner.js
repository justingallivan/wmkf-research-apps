...
async function resolveReviewerPerson({
  ...
  const personSelect = [
    ...
    'statecode', '_wmkf_contact_value', SYNTHETIC_REVIEWER_MARKER_FIELDS.marker,
  ];
  ...
  try {
    ...
    if (!row || row[SYNTHETIC_REVIEWER_MARKER_FIELDS.marker] !== true) {
      ...
      throw Object.assign(codedError('The synthetic person create failed and the row is not readable/owned; resolve manually, never re-POST.', 'ambiguous_create_outcome'), { cause: error });
    }
  }
  resource = await ledger.recordResourceReadback({
    resourceId: resource.resourceId, runId: run.runId, leaseToken: run.leaseToken, leaseGeneration: run.leaseGeneration,
...
async function resolveReviewerSuggestion({
  ...
  try {
    ...
    if (!row) {
      ...
      throw Object.assign(codedError('The suggestion create failed and the row is not readable; resolve manually, never re-POST.', 'ambiguous_create_outcome'), { cause: error });
    }
    if (!owned(row)) {
      await ledger.recordResourceFailure({
        resourceId: resource.resourceId, runId: run.runId, leaseToken: run.leaseToken, leaseGeneration: run.leaseGeneration,
      ...
      throw Object.assign(codedError('A suggestion exists at this preallocated id but is not bound to this run\'s person/request; refusing to adopt it.', 'reviewer_suggestion_present_not_owned'), { cause: error });
    }
  }
  resource = await ledger.recordResourceReadback({
    resourceId: resource.resourceId, runId: run.runId, leaseToken: run.leaseToken, leaseGeneration: run.leaseGeneration,
 ...
 * Stage B always dispatches the completion write with `filePointers: null`
 * -- even for an `uploaded` review, since `copy_review_file` (Stage C) is
 * not built yet; Stage C adds the pointers to this SAME completion write,
 * it does not add a second write.
 */
  ...
  if (next) {
    ...
    if (bundleReviewer.reviewForm === 'unreceived') {
      ...
      if (resource.outcome === 'verified' || resource.outcome === 'recovered') {
        ...
        const write = buildCompletionWrite(bundleReviewer, { suggestionId, ifMatch: current._etag, filePointers: null });
        try {
...
// built above and wired into STEP_HANDLERS below. The same stub still covers
// `copy_review_file` and `verify_reviews` (Stage C); `seed_reviewers` and
// `seed_review_answers` are now built (slice 6c-ii Stage B).
