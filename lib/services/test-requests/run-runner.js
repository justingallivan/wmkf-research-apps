...
function flattenFileCopyEntry(entry, index) {
  return compact({
    ...
    // Reviews recipe only: the source drive id, needed by
    // reverifyCopiedItems/reconcileJournaledCopies to re-download a FRESH
    // source copy for DOCX package re-attestation (never journaled for the
    // Basic/IA recipes' exact-hash files, which don't need it).
    sourceDriveId: entry.kind === 'reviewerUpload' ? entry.sourceDriveId : undefined,
 ...
 * (`primaryFilename`) so `seed_review_answers` can read the destination
...
async function stepCopyReviewFile({
  ...
  const doneSequences = new Set(
    existingResources
      .filter((r) => r.step === COPY_REVIEW_FILE_STEP && r.resourceKind === REVIEW_FOLDER_RESOURCE_KIND && r.readback?.primaryFilename)
      .map((r) => r.plannedIdentity?.assignmentSequence),
  );
  ...
  if (next) {
    ...
    if (verifiedCount >= plannedFiles.length) {
      ...
      folderResource = await ledger.recordResourceReadback({
        ...
        responseStatus: null, readback: compact({ primaryFilename: plannedFiles[0].destination.filename }), outcome: 'verified',
      });
  ...
  if (next) {
    ...
    if (bundleReviewer.reviewForm === 'unreceived') {
      ...
      if (resource.outcome === 'verified' || resource.outcome === 'recovered') {
        ...
        // carries `folder` from plannedIdentity and `primaryFilename` set
        // once every file verified) -- never re-derived here. Refuse rather
        ...
        if (bundleReviewer.reviewForm === 'uploaded') {
          ...
          const filename = folderResource?.readback?.primaryFilename;
          if (!folder || !filename) {
