...
async function stepSeedInitialAssessment({ run, ledger, client, graph, sharePointTarget }) {
  ...
  if (row?.wmkf_operationstatus === REQUEST_DOCUMENT_OPERATION_STATUS.READY) {
    ...
    if (!markers.claimTokenSha256) {
      throw codedError(
        'A Ready Initial Assessment exists at this generation key but this run never journaled a create; refusing to adopt it.',
        'ia_pointer_mismatch',
      );
    }
    const rowClaimTokenSha256 = row.wmkf_claimtoken
      ...
      : null;
    if (rowClaimTokenSha256 !== markers.claimTokenSha256) {
      throw codedError(
        'The Ready Initial Assessment at this generation key does not carry this run\'s claim token; refusing to adopt it.',
        'ia_pointer_mismatch',
      );
    }
...
async function stepVerifyInitialAssessment({ run, ledger, manifest, client, graph, sharePointTarget }) {
  ...
  // the exact bytes this run uploaded, journaled before the PUT. Mandatory.
  if (!seedResource.readback.bytesSha256 || rawSha256Hex(downloadedIa.buffer) !== seedResource.readback.bytesSha256) {
    throw codedError('The downloaded Initial Assessment bytes are not the bytes the seed step uploaded.', 'ia_verification_failed');
  }
  ...
  if (!snapshotResource.readback.bytesSha256 || rawSha256Hex(downloadedSnapshot.buffer) !== snapshotResource.readback.bytesSha256) {
    throw codedError('The retained Board snapshot bytes are not the bytes the snapshot step uploaded.', 'ia_verification_failed');
  }
