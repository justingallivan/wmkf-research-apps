 ...
 * branch. Returns the review's own copy_review_file resources (for the
 * caller's combined folder census) or `[]` when the review has no files.
 */
async function verifyOneReview({
  ...
  if (bundleReviewer.reviewForm === 'uploaded') {
    ...
    return { destinationSuggestionId, fileResources, folder: filePointers.folder };
  }
  ...
  return { destinationSuggestionId, fileResources: [], folder: null };
}
