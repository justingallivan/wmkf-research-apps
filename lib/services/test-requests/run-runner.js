    ...
    if (bundleReviewer.reviewForm === 'unreceived') {
      ...
      if (resource.outcome === 'verified' || resource.outcome === 'recovered') {
        ...
        if (typeof row._etag !== 'string' || row._etag.length === 0) {
          throw codedError('seed_review_answers: the resumed readback returned no eTag; the changeset already ran and must never be re-dispatched -- resolve manually.', 'reviewer_answers_ambiguous');
        }
        resource = await ledger.recordResourceReadback({
          resourceId: resource.resourceId, runId: run.runId, leaseToken: run.leaseToken, leaseGeneration: run.leaseGeneration,
        ...
        if (typeof after?._etag !== 'string' || after._etag.length === 0) {
          throw codedError('seed_review_answers: the post-changeset readback returned no eTag; the changeset already ran and must never be re-dispatched -- resolve manually.', 'reviewer_answers_ambiguous');
        }
        resource = await ledger.recordResourceReadback({
          resourceId: resource.resourceId, runId: run.runId, leaseToken: run.leaseToken, leaseGeneration: run.leaseGeneration,
