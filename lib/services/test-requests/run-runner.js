...
async function stepVerifyInitialAssessment({ run, ledger, manifest, client, graph, sharePointTarget }) {
  ...
  const iaGovernedHash = await sandboxDeps.hashDocx(downloadedIa.buffer);
  ...
  const snapshotGovernedHash = await sandboxDeps.hashDocx(downloadedSnapshot.buffer);
  ...
  const basicFileCopies = existingResources
    .filter((row) => row.step === 'copy_file' && row.resourceKind === 'sharepoint_file' && row.readback)
    .map(inflateFileCopyEntry);
  const iaBytesSha256 = sha256(downloadedIa.buffer);
  const snapshotBytesSha256 = sha256(downloadedSnapshot.buffer);
  const iaEntry = {
  ...
  const verification = verifyClone(manifest, preflight, observation, files, foundationAfter, contactsAfter, allFileCopies);
  if (graphError) {
    verification.ok = false;
    verification.failures.push(`Graph folder inspection failed: ${graphError}`);
  }
