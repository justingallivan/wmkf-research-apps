...
export const SANDBOX_REHEARSAL_COPY_POLICY = Object.freeze({
  version: 'sandbox-rehearsal-2026-09-25',
  // Reviews recipe (slice 6c-i): the MIME types and version are updated now
  // so the copy policy recognizes uploaded review files, but maxFiles stays
  // at the pre-6c-i ceiling (Opus round 1, P3: 6c-i builds no copy step, so
  // there is nothing yet to bound per-reviewer). 6c-ii, which builds
  // copy_review_file, introduces a per-review cap (maxFilesPerReview: 5,
  // review-upload.js's own limit) together with a reviewer-count ceiling and
  // a reconciled maxTotalBytes, all bound into the copy-policy digest at
  // that point.
  maxFiles: 7,
