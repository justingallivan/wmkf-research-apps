...
export async function reverifyCopiedItems(copies, dependencies) {
  ...
  for (const entry of copies) {
    ...
    try {
      ...
      if (entry.attestedDigest && sha256(downloaded.buffer) !== entry.attestedDigest) {
...
export function verifyCopiedFiles(copies, listedFiles) {
  ...
  for (const entry of verified) {
    ...
    const expectedSize = integrityModeForMime(entry.source?.mimeType) === 'exact-hash' ? entry.source.size : entry.item?.size;
