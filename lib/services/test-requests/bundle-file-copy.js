...
export const SANDBOX_REHEARSAL_COPY_POLICY = Object.freeze({
  ...
  // Reviews recipe (slice 6c-i): each source review may carry up to 5
  // uploaded files (review-upload.js's own per-review cap), assuming up to
  // 10 reviewers per run; 7 (the prior Basic + Initial Assessment ceiling)
  // + 10 * 5 = 57. Per-file 25 MB is unchanged (also review-upload.js's own
  // ceiling); 6c-ii adds the copy step body and per-MIME integrity mode.
  maxFiles: 57,
