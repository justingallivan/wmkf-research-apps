 ...
 *
 * The resolver owns the `withDalContext` wrap for its Dataverse fetches
 * (service-layer concern, not the API route's), matching the Executor.
 */
...
export async function resolveReviewerPrompt(promptName, { userProfileId = null } = {}) {
  ...
  let dvError = null;
  try {
    dvRow = await withDalContext('reviewer-prompt-resolve', () =>
  ...
  const override = await withDalContext('reviewer-prompt-override-read', () =>
    readUserOverride(userProfileId, promptName));
