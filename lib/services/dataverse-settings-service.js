...
async function listSettingsWithMetaStrict(keyPrefix = '') {
  ...
  const filter = keyPrefix
    ...
    : '';
  const out = {};
  let next = `/wmkf_appsystemsettings?$select=wmkf_appsystemsettingid,wmkf_settingkey,wmkf_settingvalue,createdon,modifiedon,_wmkf_updatedby_value&$top=5000${filter ? `&$filter=${encodeURIComponent(filter)}` : ''}`;
  while (next) {
    const r = await client.get(next);
    if (!r.ok) throw new Error(`list settings with meta failed: ${r.status} ${r.text?.slice(0, 200)}`);
    for (const row of r.body?.value || []) {
      out[row.wmkf_settingkey] = {
        ...
        updatedByName: row._wmkf_updatedby_value_formatted ?? null,
      };
    }
