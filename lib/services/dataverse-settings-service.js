 ...
 *   getSettingStrict(key)                          → { found, value, revision }
 *   listSettings(keyPrefix)                        → { [key]: value }
 *   setSetting(key, value, updatedBy)              → best-effort upsert
 *   setSettingIfUnchanged(key, value, etag, actor) → strict optimistic upsert
...
function settingConflict(key) {
  const error = new Error(`setting changed since it was loaded: ${key}`);
  error.code = 'setting_conflict';
  error.status = 409;
  return error;
}
 ...
 * { found, value, revision } so callers can distinguish "key absent" (found: false)
...
async function getSettingStrict(key) {
  ...
  return {
    found: row != null,
    value: row ? row.wmkf_settingvalue : null,
    revision: row?.['@odata.etag'] || null,
  };
}
...
/**
 * Strict optimistic upsert for full-replacement settings editors.
 * Existing rows are PATCHed with the revision returned by getSettingStrict;
 * absent rows rely on the alternate key to make concurrent creates conflict.
 */
async function setSettingIfUnchanged(
  key,
  value,
  expectedRevision,
  ...
  const client = await getClient();
  const existing = await findRow(client, key);
  const actualRevision = existing?.['@odata.etag'] || null;
  if (existing && !actualRevision) {
    const error = new Error(`setting revision unavailable: ${key}`);
    error.code = 'setting_revision_unavailable';
    error.status = 503;
    throw error;
  }
  if (actualRevision !== expectedRevision) throw settingConflict(key);
  ...
  if (existing) {
    ...
    if (!r.ok) {
      if (r.status === 412) throw settingConflict(key);
      const error = new Error(`conditional patch setting failed: ${r.status} ${r.text?.slice(0, 200)}`);
  ...
  if (!r.ok) {
    if (r.status === 409 || r.status === 412) throw settingConflict(key);
    const error = new Error(`conditional create setting failed: ${r.status} ${r.text?.slice(0, 200)}`);
    error.status = r.status;
    throw error;
  }
  return true;
