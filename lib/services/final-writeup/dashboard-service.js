...
async function discoverCycles(dependencies) {
  const result = await dependencies.queryAllRequests({
    select: withTestRequestIsolationSelect(CYCLE_SCAN_SELECT),
...
function lifecycleStage(finalDocument) {
  ...
  if (finalDocument.wmkf_lifecyclestate === REQUEST_DOCUMENT_LIFECYCLE_STATE.FINAL
    && isLeadershipCheckpointComplete(finalDocument)) {
    return { key: 'leadership-review', label: 'Leadership review' };
  }
  // FINAL without the complete leadership checkpoint falls through: the same
  // reconciliation error as an unknown lifecycle, shared with the transition
  // and acknowledgement readers via leadership-checkpoint.js.
  throw dashboardError(
...
function projectRequestRow({ request, finalDocument, metadata, observation, acknowledgement }) {
  ...
  return {
    ...
    acknowledgedPublicationVersionId: acknowledgement.acknowledgedPublicationVersionId,
...
function navigationFor(selected, queues) {
  ...
  const compact = (row) => (row ? {
    ...
    title: row.title,
    ...(Object.prototype.hasOwnProperty.call(row, 'isTestRequest')
      ? { isTestRequest: row.isTestRequest }
      : {}),
  } : null);
  return {
...
function coordinatorMatrixRow(writeup, expectedReviewers) {
  ...
  return {
    ...
    stage: writeup.stage,
    ...(Object.prototype.hasOwnProperty.call(writeup, 'isTestRequest')
      ? { isTestRequest: writeup.isTestRequest }
      : {}),
...
function unconfiguredMatrixRow(writeup, request) {
  return {
    ...
    title: writeup.title,
    institution: writeup.institution,
    // Same PD object configured rows carry, so the client PD filter is total.
    responsibleProgramDirector: writeup.responsibleProgramDirector,
...
export async function loadFinalWriteupsDashboard(
  ...
  const countableRequestIds = new Set(requests
    .filter((request) => testRequestCountsAsOrdinary(request))
    .map((request) => String(request.akoya_requestid).toLowerCase()));
  const countable = (row) => countableRequestIds.has(String(row.requestId).toLowerCase());
  ...
  return {
    ...
    counts: {
      total: visibleProjected.filter(countable).length,
      open: queues.open.filter(countable).length,
      history: queues.history.filter(countable).length,
      stewardship: queues.stewardship.filter(countable).length,
    },
    queues,
