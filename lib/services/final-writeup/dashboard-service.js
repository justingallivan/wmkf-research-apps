 ...
 * and focused responses receive neither matrix nor configuration. Persona-team
 * membership is multi-valued but remains rollout-disabled until its separate
 * access proof.
 */
...
export async function loadFinalWriteupsDashboard(
  ...
  const [viewer, requests, personaProjection, matrixAudiences] = await Promise.all([
    ...
    includeCoordinatorMatrix ? dependencies.resolveMatrixAudiences() : Promise.resolve(null),
  ]);
  ...
  return {
    ...
    coordinatorMatrix: matrixAudiences
      ? coordinatorMatrix(projected, requests, matrixAudiences)
      : null,
    selected,
