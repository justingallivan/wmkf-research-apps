 ...
 *
 * Leadership review (Slice 4) is a second governed transition on the same Final
 * row: lifecycle Review -> Final, explicit leadership actor/time, and the row's
 * SharePoint observation fields refreshed to the verified current version. The
 * milestone triple and its Site Visit handoff actor are never written here.
 */
...
/**
 * A Final row in lifecycle FINAL is committed only with the complete leadership
 * checkpoint: explicit actor/time plus every SharePoint observation field the
 * transition refreshes. Anything less is a half-written transition and is
 * reported for reconciliation, never treated as committed.
 */
function leadershipCheckpointComplete(final) {
  return Boolean(final.wmkf_leadershipreviewstartedat)
    && Boolean(final._wmkf_leadershipreviewstartedby_value)
    && Boolean(final.wmkf_sharepointversionid)
    && Boolean(final.wmkf_sharepointetag)
    && Boolean(final.wmkf_sharepointlastmodified)
    && final.wmkf_filesize != null
    && Number.isFinite(Number(final.wmkf_filesize))
    && Boolean(final.wmkf_contenthash);
}

function committedLifecycleAccepted(final) {
  ...
  return final.wmkf_lifecyclestate === REQUEST_DOCUMENT_LIFECYCLE_STATE.FINAL
    && leadershipCheckpointComplete(final);
}

function stageOf(final) {
  return final.wmkf_lifecyclestate === REQUEST_DOCUMENT_LIFECYCLE_STATE.FINAL
    ? 'leadership-review'
    : 'group-review';
}
...
function committedFinal(state, sourceId, finalId) {
  ...
  if (!sameId(state.request._wmkf_currentpresitevisit_value, sourceId)
    ...
    || !committedLifecycleAccepted(final)
...
const SOURCE_VERIFY_ERRORS = Object.freeze({
  unverifiable: [
...
const LEADERSHIP_VERIFY_ERRORS = Object.freeze({
  unverifiable: [
...
/**
 * Metadata before, governed download/hash, metadata after. The row's stable
 * drive/item identity must match; the version must be unchanged across the hash.
 */
async function verifyDocument(row, dependencies, errors) {
...
function verifySource(source, dependencies) {
...
function leadershipResult(row, reused) {
  return { artifact: projectArtifact(row), phase: 'leadership-review', reused };
}

/**
 * The committed leadership row this call produced: lifecycle FINAL with the
 * complete checkpoint AND the observation fields equal to what this call
 * verified. Distinguishes "our write landed" from "someone else advanced it".
 */
function leadershipCommittedByThisCall(state, sourceId, finalId, verified) {
...
/**
 * Ready for leadership review: move the current Final row from Review to
 * Final. Authorization runs before any pointer, fence, or stage inspection so
 * an unauthorized caller learns nothing beyond the request's existence.
 */
