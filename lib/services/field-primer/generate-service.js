...
import { getProposalText } from '../workbench-proposal-documents';
...
export async function generateForRequest({ requestId, focus, regenerate }) {
  ...
  let proposal;
  try {
    proposal = await getProposalText(rec.akoya_requestid, rec.akoya_requestnum);
  } catch (e) {
  ...
  if (!proposal || !proposal.text || proposal.text.trim().length < 50) {
    ...
    throw new ServiceHttpError(
      `No readable canonical reviewer proposal was found at Reviewer Materials/Proposal_${rec.akoya_requestnum}.pdf.`,
      { httpStatus: 400 },
    );
  }
