...
// anything else unexpected keeps the client default rather than failing the run.
async function resolveGenerationTimeoutMs() {
  try {
    ...
    console.error('[field-primer/generate] executor budget read failed; using client default timeout:', e.message);
    return null;
  }
}
...
export async function generateForRequest({ requestId, focus, regenerate }) {
  ...
  const timeoutMs = await resolveGenerationTimeoutMs();
