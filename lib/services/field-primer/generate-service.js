...
// Below this the model cannot realistically answer; the pull already consumed
// the lease, so we still try rather than fail a run that may succeed.
const MIN_MODEL_TIMEOUT_MS = 30_000;
...
export function clampTimeoutToLease(timeoutMs, leaseDeadlineMs, nowMs = Date.now()) {
  ...
  return Math.max(MIN_MODEL_TIMEOUT_MS, Math.min(timeoutMs, available));
}
 ...
 *   (`field_primer_generation_timeout`); 502 provider HTTP error; 500 other generation failure;
...
export async function generateForRequest({ requestId, focus, regenerate }) {
  ...
  // After the persist step we may no longer own the lease, so restore only
  // when the stored lease still carries our nonce, conditionally on its ETag.
  const restorePriorIfStillOwned = async () => {
    try {
      ...
      console.error('[field-primer/generate] post-persist lease restore failed:', e.message);
    }
  };
  ...
  try {
    gen = await generateFieldPrimer({ proposalText: proposal.text, focus, runSource: 'Vercel Interactive', timeoutMs });
  } catch (e) {
  ...
  try {
    ...
    if (curLease && curLease.nonce === myNonce) {
      if (!cur._etag) {
        await restorePriorIfStillOwned();
