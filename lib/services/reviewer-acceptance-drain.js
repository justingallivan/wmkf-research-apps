...
async function verifyAcceptedOrCancel({ job, currentSuggestion, jobs }) {
  if (currentSuggestion?.wmkf_accepted === true && currentSuggestion?.wmkf_declined !== true) {
    return { ok: true };
  }
  const createdAt = new Date(job.created_at).getTime();
  const ageMs = Number.isFinite(createdAt) ? Date.now() - createdAt : ACCEPT_COMMIT_GRACE_MS + 1;
...
export async function processReviewerAcceptanceJob(job, deps = {}) {
  ...
  const acceptedCheck = await verifyAcceptedOrCancel({ job, currentSuggestion, jobs });
