...
export function sanitizeBlobFilename(input) {
  ...
  // Strip controls and separators.
  s = s.replace(CONTROL_CHARS, '').replace(PATH_SEPARATORS, '');

  // Trim whitespace and leading dots (hidden-file convention).
  s = s.trim().replace(LEADING_DOTS, '');

  if (!s) {
    throw new Error('sanitizeBlobFilename: input empty after sanitization');
  }

  // Length cap. Preserve the extension if one is present and reasonable
  // (≤ 16 chars including the dot) so a 250-char filename still ends
  // in `.pdf` and not in the middle of a name.
  const MAX_LEN = 200;
  if (s.length > MAX_LEN) {
    const extMatch = s.match(/\.[^.]{1,16}$/);
    if (extMatch) {
      const ext = extMatch[0];
      const base = s.slice(0, MAX_LEN - ext.length);
      s = base + ext;
    } else {
      s = s.slice(0, MAX_LEN);
    }
