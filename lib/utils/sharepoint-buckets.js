...
function assertCompleteLocationResult(label, result) {
  const records = Array.isArray(result?.records) ? result.records : [];
  const totalCount = Number.isSafeInteger(result?.totalCount) ? result.totalCount : records.length;
  if (result?.hasMore === true || records.length > LOCATION_PAGE_SIZE || totalCount > records.length) {
    throw new Error(`${label} returned more than ${LOCATION_PAGE_SIZE} rows; refusing a truncated result.`);
  }
}
 ...
 *   archive folder name).
 * @param {{requireResolvedParents?: boolean, requireCompleteResults?: boolean}} [options]
 *   Governed writers set `requireResolvedParents` so a best-effort library
 *   fallback cannot become a write destination. Snapshot/export callers set
 *   `requireCompleteResults` so the adapter's bounded page cannot be silently
 *   treated as a complete location inventory.
 ...
 */
export async function getRequestSharePointBuckets(
  requestId,
  requestNumber,
  { requireResolvedParents = false, requireCompleteResults = false } = {},
) {
  if (!requestId) {
  ...
  if (requireCompleteResults) {
    assertCompleteLocationResult('Request SharePoint location lookup', locResult);
  }
  ...
  if (parentIds.length > 0) {
    try {
      ...
      if (requireCompleteResults) {
        assertCompleteLocationResult('SharePoint parent location lookup', parentResult);
      }
      for (const p of parentResult.records) {
        if (p.relativeurl) parentToLibrary.set(p.sharepointdocumentlocationid, p.relativeurl);
      }
    } catch (error) {
      if (requireResolvedParents) {
        throw new Error(
          `Unable to verify the request SharePoint parent library: ${error.message}`,
          { cause: error },
        );
      }
      // Read-only callers retain the legacy best-effort fallback below.
    }
  }
  ...
  for (const loc of locResult.records) {
    ...
    const resolvedLibrary = parentToLibrary.get(loc._parentsiteorlocation_value);
    if (requireResolvedParents && !resolvedLibrary) {
      throw new Error(
        `Unable to verify the parent library for SharePoint location "${loc.relativeurl}".`,
      );
    }
    const lib = resolvedLibrary || 'akoya_request';
