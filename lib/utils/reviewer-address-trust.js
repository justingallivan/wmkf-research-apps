...
const STAFF_ADDRESS_CHOICE_REASON = 'Staff selected this address after reviewing the stored and found values';
...
function normalizeResolution(value) {
  ...
  const storedEmail = normalizeAddress(conflict?.storedEmail);
  const foundEmail = normalizeAddress(conflict?.foundEmail);
  const resolvedAt = value?.resolvedAt;
  if (!value || typeof value !== 'object' || !CONFLICT_REASONS.has(conflict?.reason)
    || !storedEmail || !foundEmail || storedEmail === foundEmail
    || !validIsoTimestamp(conflict?.detectedAt) || !validIsoTimestamp(resolvedAt)
    || Date.parse(resolvedAt) < Date.parse(conflict.detectedAt)
    ...
    throw new Error('Address conflict resolution is malformed or stale');
  }
  const selectedEmail = value.decision === 'keep_stored' ? storedEmail : foundEmail;
  return {
    conflict: {
      ...conflict,
      storedEmail,
    ...
    actorProfileId: boundedString(value.actorProfileId, 128),
    actorSystemUserId: boundedString(value.actorSystemUserId, 128),
    resolvedAt,
    selectedEmail,
  };
}
...
function createStaffVerifiedState({ email, resolution = null, ...attestation }) {
  ...
  const normalizedAttestation = buildAttestation(attestation);
  const normalizedResolution = normalizeResolution(resolution);
  if (normalizedAttestation.evidenceType === 'staff_address_choice') {
    if (!normalizedResolution) {
      throw new Error('A staff address choice requires an exact conflict resolution');
    }
    if (
      normalizedResolution.conflict.requestId !== normalizedAttestation.requestId
      || normalizedResolution.conflict.candidateKey !== normalizedAttestation.candidateKey
    ) {
      throw new Error('A staff address choice must resolve the same request and candidate');
    }
  }
...
function staffAddressChoiceProjection(state, {
  email = null,
  requestId = null,
  candidateKey = null,
  receipt = null,
  ...
  if (
    state?.status !== 'staff_verified'
    || state?.attestation?.evidenceType !== 'staff_address_choice'
    || !state?.resolution
  ) return null;
  const selectedEmail = normalizeAddress(state.resolution.selectedEmail);
  if (!selectedEmail || selectedEmail !== normalizeAddress(state.email)) return null;
  if (email && selectedEmail !== normalizeAddress(email)) return null;
  if (requestId && (
    state.attestation.requestId !== requestId
    || state.resolution.conflict?.requestId !== requestId
  )) return null;
  if (candidateKey && (
    state.attestation.candidateKey !== candidateKey
    || state.resolution.conflict?.candidateKey !== candidateKey
  )) return null;
  if (receipt && (
    receipt.personConfirmed !== true
    || receipt.evidenceType !== 'staff_address_choice'
    || normalizeAddress(receipt.email) !== selectedEmail
    || receipt.requestId !== state.attestation.requestId
    || receipt.candidateKey !== state.attestation.candidateKey
    || receipt.attestedAt !== state.attestation.attestedAt
  )) return null;
  return {
...
function sameAddressPair(leftStored, leftFound, rightStored, rightFound) {
  const left = [normalizeAddress(leftStored), normalizeAddress(leftFound)].sort();
  const right = [normalizeAddress(rightStored), normalizeAddress(rightFound)].sort();
  return left[0] && left[1] && left[0] === right[0] && left[1] === right[1];
}

/**
 * Classify a freshly rediscovered contradiction without destroying a prior
 * staff adjudication. A resolved A/B pair stays resolved even when the chosen
 * side became the person's stored address; a genuinely new C address replaces
 * the pending conflict so staff never adjudicate stale evidence.
 */
function addressConflictDisposition(currentState, {
  email,
  foundEmail,
  reason,
}) {
  const storedEmail = normalizeAddress(email);
  const normalizedFound = normalizeAddress(foundEmail);
  if (!storedEmail || !normalizedFound || storedEmail === normalizedFound) return 'none';

  if (currentState?.status === 'conflict_pending') {
    const conflict = currentState.conflict;
    if (conflict?.reason === reason && sameAddressPair(
      conflict.storedEmail,
      conflict.foundEmail,
      storedEmail,
      normalizedFound,
    )) return 'existing';
  ...
  const resolvedConflict = currentState?.status === 'staff_verified'
    ? currentState.resolution?.conflict
    : null;
  if (
    resolvedConflict?.reason === reason
    && sameAddressPair(
      resolvedConflict.storedEmail,
      resolvedConflict.foundEmail,
      storedEmail,
      normalizedFound,
    )
  ) return 'resolved';
...
function receiptCanResolveConflict(receipt, conflict) {
  if (!receipt?.personConfirmed || !validIsoTimestamp(receipt.attestedAt)
    || !validIsoTimestamp(conflict?.detectedAt)) return false;
  return Date.parse(receipt.attestedAt) >= Date.parse(conflict.detectedAt);
}
...
function parseAddressTrustState(raw, { storedEmail } = {}) {
  ...
  if (state.status === 'staff_verified') {
    try {
      ...
      if (resolution && resolution.selectedEmail !== bundleEmail) {
        return { valid: false, reason: 'invalid_resolution', state: null };
      }
      if (rebuilt.evidenceType === 'staff_address_choice' && (
        !resolution
        || resolution.conflict.requestId !== rebuilt.requestId
        || resolution.conflict.candidateKey !== rebuilt.candidateKey
      )) {
        return { valid: false, reason: 'invalid_staff_address_choice', state: null };
      }
      return {
        valid: true,
        reason: null,
        state: { ...state, email: bundleEmail, attestation: rebuilt, conflict: null, resolution },
      };
    } catch {
      return { valid: false, reason: 'invalid_attestation_or_resolution', state: null };
    }
  }
