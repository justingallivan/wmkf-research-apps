...
function emailConfidence(person) {
  ...
  let reason;
  // S387: a human confirmed a system-found address against an independent source
  // (my-candidates `verifyEmailAddress`). Distinct from 'manual' — nobody typed it —
  // and deliberately quick_check, NOT ready: a staff attestation without
  // address-specific first-party evidence does not earn the same tier as an
  // ORCID/institution-page address, so the per-recipient send acknowledgement stands.
