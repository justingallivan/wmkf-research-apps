...
function emailConfidence(person) {
  ...
  const { addressTrustDecision } = require('./reviewer-address-trust');
  ...
  if (source === 'staff_verified' && trust.status === 'staff_verified') {
    return { level: 'high', action: 'ready', reason: 'Exact address verified by staff against recorded evidence' };
  }
