...
// email). The hold ask is first contact too, so it's guarded like the invitation;
// materials/followup/thankyou/finalize are intentionally re-sendable.
const FIRST_CONTACT_TEMPLATES = new Set(['invitation', 'hold']);
...
const KNOWN_TEMPLATE_TYPES = new Set(['invitation', 'hold', 'finalize', 'materials', 'followup', 'thankyou']);
...
// wmkf_responsetype = held (the pre-accept "agree in principle" state).
const RESPONSE_TYPE_HELD = 100000004;

/**
 * SERVER-authoritative eligibility for the "proposal ready — please finalize" nudge:
 * it is valid ONLY for a reviewer who has HELD (agreed in principle). A finalize to a
 * fresh/declined/already-accepted row is a wrong target — the caller must skip it, so
 * a misdirected finalize can't (a) bypass the first-contact confidence gate on an
 * unproven address, nor (b) overwrite the outreach timestamp on a non-held row
 * (Codex chunk-6 #3/#7).
 *
 * @param {object} suggestion - the wmkf_appreviewersuggestion row
 * @returns {boolean}
 */
function mayReceiveFinalize(suggestion) {
  return suggestion?.wmkf_responsetype === RESPONSE_TYPE_HELD;
}
