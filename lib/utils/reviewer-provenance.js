...
/**
 * Does this candidate need an explicit staff identity attestation before it may be
 * promoted as a vetted reviewer?
 *
 * CANONICAL for the applicant-promotion gate: the server
 * (promote-applicant-reviewer-service) refuses the promotion when this is true and no
 * server-issued confirmation id is bound, and `provenanceGroupOf` routes the same row to
 * the read-only `needs_identity_review` group so the card offers "This is the right
 * person" instead of a checkbox that 422s on save. Client and server MUST read the same
 * predicate — S387 found a card selectable because the client tested three of these four
 * clauses (`institutionMismatch`, and any `identityStatus` outside
 * confirmed/probable/unresolved, were server-blocking but client-selectable).
 *
 * `identityStatus` is read from the top level first and the enrichment identity verdict
 * second, because a roster row persists the resolver status at either depth
 * (reviewer-search-logic pruneCandidateForRoster falls back to `identity?.status`).
 */
function requiresStaffIdentityConfirmation(candidate) {
  if (!candidate) return false;
  const identityStatus = candidate.identityStatus
  ...
  return candidate.needsIdentification === true
    ...
    || (identityStatus != null && identityStatus !== 'confirmed' && identityStatus !== 'probable');
}
...
function provenanceGroupOf(candidate) {
  ...
  // Applicant rows are the ONLY kind gated on the full server predicate here: the
  // promotion route (promote-applicant-reviewer) is what refuses them, and its gate is
  // `requiresStaffIdentityConfirmation`. Deliberately NOT applied to the other kinds —
  // save-candidates gates on the explicit unresolved markers instead
  // (save-candidates-service.js isUnresolvedIdentity), and rows with a resolver verdict
  // but no top-level identityStatus are legitimately saved there with field-level
  // gating, so widening this group for them would reject writes that path allows.
  // `pdIdentityConfirmed` is the client-visible escape (same convention as
  // isCandidateSelectable); the server re-verifies the opaque confirmation id and never
  // trusts this marker.
  if (provenance.kind === PROVENANCE_KINDS.APPLICANT_SUGGESTED) {
    return requiresStaffIdentityConfirmation(candidate) && candidate?.pdIdentityConfirmed !== true
...
// Institution-COI detail carries the current PI/reviewer institution pair plus
// bounded decision metadata. Strip any legacy `.historical` field so a roster row
// saved before the historical-COI retirement isn't reloaded/rendered as a current
// conflict. Canonical home (shared by the client merge/prune AND the server
// roster-store read path).
function sanitizeInstitutionCOIDetails(detail) {
  ...
  if (!piInstitution && !reviewerInstitution) return null;
  const out = { piInstitution, reviewerInstitution };
  const copyString = (key, allowed = null) => {
  ...
  copyString('dropDecision', new Set(['flagged', 'dropped']));
  copyString('corroborationReason');
  copyString('matchedAffiliationSource');
  copyString('contradictoryAffiliationSource');
  copyString('matchSource');
  copyString('dropStage');
  return out;
}

// ---------------------------------------------------------------------------
// Referral clause encode/decode (S249 D1: the referrer has no dedicated Dataverse
// field — it lives inside the human-readable match reason).
//
// The clause is `Referred by {name}.` and it OWNS LINE 1 of the match reason.
// A period cannot terminate the name on its own: titles and middle initials carry
// one ("Dr. Abby Doyle"), so a non-greedy parse stops at the first period and a
// greedy one swallows the trailing note. The newline is the terminator instead —
// it cannot occur inside a name, and every downstream annotation APPENDS
// (save-candidates' PD-confirmation and coauthor-COI suffixes), so it lands on a
// later line and can never re-enter the captured name.
//
// The trailing newline is emitted even with no remainder, precisely so those later
// appends stay off line 1. HTML collapses it, so the rendered "Why:" line is
// unchanged.
//
// LEGACY: rows written before this joined the clause and the note with a space,
// which is genuinely ambiguous. Those fall back to the original non-greedy parse —
// same (imperfect) result as before, no regression. One legacy shape is parsed
// worse than before: a multi-line note whose FIRST line ends in a period, which
// mimics the line-1 terminator. Both the old and new readings are wrong for that
// row; it is not recoverable from the stored string.
const REFERRED_BY_PREFIX = 'Referred by ';
const REFERRED_BY_LINE1 = /^Referred by (.+)\.\n/;
const REFERRED_BY_LEGACY = /^Referred by (.+?)\.(?:\s|$)/;

/**
 * Compose a match reason that round-trips `referredBy` exactly.
 * Returns `rest` unchanged when there is no referrer.
 */
function formatReferredByReason(referredBy, rest = '') {
  // A trailing period is dropped before the terminator is added, so a suffixed name
  // ("Abby Doyle Jr.") reads as "Referred by Abby Doyle Jr." rather than "Jr..", and
  // so re-encoding a parsed name is stable.
  const name = String(referredBy || '').replace(/\s+/g, ' ').trim().replace(/\.+$/, '').trim();
  const tail = String(rest || '').trim();
  if (!name) return tail;
  return `${REFERRED_BY_PREFIX}${name}.\n${tail}`;
}

/**
 * Split a match reason into its referrer and the rationale that follows.
 * Inverse of the above. `referredBy` is null when there is no clause, in which
 * case `rest` is the whole (trimmed) reason.
 *
 * `rest` exists so a surface that renders the referrer as its own labeled field
 * does not ALSO repeat it inside the rationale text.
 */
function splitReferredByReason(matchReason) {
  const text = String(matchReason || '');
  const match = REFERRED_BY_LINE1.exec(text) || REFERRED_BY_LEGACY.exec(text);
  if (!match) return { referredBy: null, rest: text.trim() };
  return {
    referredBy: match[1].trim() || null,
    rest: text.slice(match[0].length).trim(),
  };
}

/** Recover the referrer from a match reason, or null. */
function parseReferredByReason(matchReason) {
  return splitReferredByReason(matchReason).referredBy;
}
