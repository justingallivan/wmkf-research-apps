...
const { hasCandidateStaffIdentityConfirmation } = require('./reviewer-identity-authority');
...
function provenanceGroupOf(candidate) {
  ...
  if ((candidate?.needsIdentification || candidate?.identityStatus === 'unresolved' || candidate?.verificationStatus === 'unresolved')
    && !hasCandidateStaffIdentityConfirmation(candidate)) {
  ...
  // The current server-shaped staff confirmation is the client-visible escape
  // (same convention as isCandidateSelectable); the server still re-reads the
  // exact request-scoped record before any promotion.
  if (provenance.kind === PROVENANCE_KINDS.APPLICANT_SUGGESTED) {
    return requiresStaffIdentityConfirmation(candidate) && !hasCandidateStaffIdentityConfirmation(candidate)
