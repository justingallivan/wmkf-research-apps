...
export async function requireAppAccess(req, res, ...appKeys) {
  ...
  if (!cached || Date.now() - cached.loadedAt > APP_ACCESS_TTL_MS) {
    ...
    try {
      ...
      // Name the guarded app, and never say "application": at a grants
      // foundation that reads as a GRANT application (owner reports
      // 2026-08-06). This failure is the user's app-permission lookup.
      res.status(503).json({ error: `Could not verify your access to ${appDisplayName(appKeys)}; please retry` });
      return null;
    }
