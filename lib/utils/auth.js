...
export async function requireAppAccess(req, res, ...appKeys) {
  ...
  if (!cached || Date.now() - cached.loadedAt > APP_ACCESS_TTL_MS) {
    ...
    try {
      ...
      // anything about the user's account — the wording must blame the system
      // and say retry. Never say "application": at a grants foundation that
      // reads as a GRANT application (owner reports 2026-08-06).
      res.status(503).json({ error: `A temporary system error kept us from checking your access to ${appDisplayName(appKeys)}. Your account is unaffected — please retry in a moment` });
      return null;
    }
