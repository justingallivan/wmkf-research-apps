...
function validateOrigin(req) {
  ...
  // Production uses the canonical branded NEXTAUTH_URL. Preview intentionally
  // omits that fixed value because every deployment has its own hostname, so
  // derive the allowed origin from VERCEL_URL there. Never derive from a
  // request header: that would let the caller choose the CSRF allowlist.
  let allowedUrl = process.env.NEXTAUTH_URL;
  if (!allowedUrl && process.env.VERCEL_ENV === 'preview' && process.env.VERCEL_URL) {
    const previewUrl = process.env.VERCEL_URL;
    allowedUrl = previewUrl.includes('://') ? previewUrl : `https://${previewUrl}`;
  }

  if (!allowedUrl) {
    return process.env.NODE_ENV === 'production'
      ? { valid: false, reason: 'Allowed Origin not configured' }
      : { valid: true };
  }
  ...
  try {
    ...
    return process.env.NODE_ENV === 'production'
      ? { valid: false, reason: 'Allowed Origin invalid' }
      : { valid: true };
  }
