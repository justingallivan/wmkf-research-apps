...
export async function requireAppAccess(req, res, ...appKeys) {
  ...
  if (!cached || Date.now() - cached.loadedAt > APP_ACCESS_TTL_MS) {
    ...
    try {
      ...
      // "Permissions", not "application access": at a grants foundation
      // "application" reads as a GRANT application (owner report 2026-08-06);
      // this failure is the user's app-permission lookup, nothing else.
      res.status(503).json({ error: 'Could not verify your permissions; please retry' });
      return null;
    }
  ...
  if (!hasAccess) {
    res.status(403).json({ error: 'Your account does not have access to this app' });
    return null;
  }
