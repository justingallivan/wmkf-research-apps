 ...
 *
 * Prompt-cache note: by default a fresh random nonce is minted per call, which
 * puts unique bytes near the start of any prompt that leads with this block (or
 * with `buildUntrustedContentPreamble`), defeating Anthropic prefix caching. A
 * caller that reuses the SAME untrusted content across requests (e.g. multi-turn
 * Q&A against one proposal, or an identical Executor rerun) may pass a stable
 * `nonce` — see `deriveStableNonce` — so the wrapped block is byte-identical
 * across calls and a `cache_control` breakpoint over it can actually hit. The
 * nonce only needs to be UNPREDICTABLE to the author of the untrusted content,
 * not unique per call; `deriveStableNonce` keys an HMAC with a server secret to
 * preserve that property. Any caller-supplied value that isn't a 24-char hex
 * string is ignored and a fresh random nonce is used (fail-safe).
 *
 * @param {object} args
 ...
 * @param {string} [args.nonce]    - Optional stable 24-hex nonce (see deriveStableNonce).
...
export function wrapUntrustedContent({
  ...
  // Use the caller's stable nonce only when it is a well-formed 24-hex token
  // (the shape a forged sentinel would have to reproduce); otherwise mint a
  // fresh random one. Default behavior is unchanged and a malformed value fails
  // safe to random.
  const resolvedNonce =
    typeof nonce === 'string' && /^[0-9a-f]{24}$/.test(nonce)
      ? nonce
  ...
  if (inner.includes(resolvedNonce)) {
    inner = inner.split(resolvedNonce).join(STRIPPED_SENTINEL_PLACEHOLDER);
  ...
  const labelAttr = label
    ...
    : '';
  const open = `[[${UNTRUSTED_SENTINEL_KEYWORD} nonce=${resolvedNonce}${labelAttr}]]`;
  const close = `[[/${UNTRUSTED_SENTINEL_KEYWORD} nonce=${resolvedNonce}]]`;
...
/**
 * Derive a STABLE untrusted-content nonce from a set of identity parts, keyed by
 * a server-only secret so it stays unpredictable to the author of the untrusted
 * text. Pass the result as `wrapUntrustedContent({ nonce })` for call sites that
 * re-send the SAME untrusted content across requests (multi-turn Q&A over one
 * proposal, an identical Executor rerun): the wrapped block then hashes
 * identically across calls, so a `cache_control` breakpoint placed over it can
 * produce real Anthropic cache reads instead of a fresh write every time.
 *
 * Security: the nonce must be unguessable to whoever wrote the untrusted text
 * (they must not be able to forge a matching close sentinel). Keying an HMAC
 * with a server secret preserves that even though the OUTPUT is deterministic
 * for a given input. If NO server secret is configured we return `null` (never a
 * predictable unkeyed hash), and callers fall back to the per-call random nonce
 * — no caching, but never a forgeable nonce. `parts` should include enough to
 * scope the nonce to one logical document/session (e.g. a call-site tag plus the
 * content itself); different content therefore yields a different nonce.
 *
 * @param {...(string|number|null|undefined)} parts
 * @returns {string|null} 24-hex nonce, or null when no server secret is available.
 */
export function deriveStableNonce(...parts) {
  const secret = process.env.UNTRUSTED_NONCE_SECRET || process.env.NEXTAUTH_SECRET || null;
