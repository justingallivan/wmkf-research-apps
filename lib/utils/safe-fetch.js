 ...
 * @param {RequestInit & {onRequest?: Function}} [options] - Fetch options plus
 *   an optional server-only hook invoked before every validated outbound hop;
 *   the hook itself is never forwarded to global fetch.
...
export async function safeFetch(url, options = {}) {
  ...
  for (let i = 0; i <= MAX_REDIRECTS; i++) {
    ...
    onRequest?.({ url: currentUrl, redirectIndex: i });
