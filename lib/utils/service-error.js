...
export function buildServiceError(serviceName, resp, body, options = {}) {
  const status = resp?.status ?? null;
  const e = new Error(`${serviceName} failed (${status}): ${typeof body === 'string' ? body : ''}`);
  e.serviceName = serviceName;
  e.status = status;
  // Default transient classification: 408 (request timeout), 429 (throttle),
  // and 5xx. 408 was added round-11 — Dataverse/Graph occasionally surface
  // request-timeout as a real 408 rather than a no-response abort, and the
  // drain classifier treats it identically to 429/5xx.
  const autoTransient =
    status === 408 ||
    status === 429 ||
    (status >= 500 && status < 600);
  // Explicit `options.isTransient` override lets callers force a non-transient
  // classification on shapes the status alone misclassifies — most importantly,
  // the "missing env var" configuration-bug throws that use { status: 500 } as
  // a stub. Those are 5xx-class but NOT retryable; without the override the
  // drain would burn its max_attempts retrying a bug it can't recover from.
  e.isTransient = options.isTransient ?? autoTransient;
...
export function buildNoResponseError(serviceName, cause) {
  // Short-circuit: if cause is already a no-response structured error from
  // some nested wrapper, return it as-is rather than double-wrapping. Re-
  // wrapping would replace its specific .causeKind diagnostics with our
  // outer-layer 'unknown' and lose information.
  if (cause && cause.noResponse === true && cause.serviceName) return cause;
