...
class ContactParser {
   ...
   *
   * Also returns structured title/H1 text plus the legacy `identityText`
   * (<title> + <h1..h3>). The resolved-page selector uses the structured fields
   * for mailbox forms that need strong proof that this is the candidate's own
   * profile, while retaining `identityText` for the existing URL-slug fallback.
   *
   * @param {string} html
   * @returns {{
   *   text: string,
  ...
  static extractEmailsFromHtml(html) {
    if (!html || typeof html !== 'string') {
      return { text: '', identityText: '', titleText: '', h1Texts: [], emails: [] };
    }
    ...
    const cleanIdentityText = (value) => String(value || '')
      ...
      .replace(/\s+/g, ' ')
      .trim();
    const titleText = cleanIdentityText(titleMatch ? titleMatch[1] : '');
    const h1Texts = [...html.matchAll(/<h1[^>]*>([\s\S]*?)<\/h1>/gi)]
      .map((m) => cleanIdentityText(m[1]))
      .filter(Boolean);
    const headingMatches = [...html.matchAll(/<h[1-3][^>]*>([\s\S]*?)<\/h[1-3]>/gi)]
      .map((m) => cleanIdentityText(m[1]))
      .filter(Boolean);
    const identityText = [titleText, ...headingMatches].filter(Boolean).join(' ');
    ...
    return { text: s, identityText, titleText, h1Texts, emails };
  }
  ...
  /**
   * Normalize a name for DISPLAY in outbound copy (emails, vendor records):
   * trim the ends and collapse internal runs of whitespace to a single space.
   * Honorifics and casing are PRESERVED on purpose — this is presentation
   * cleanup, not match-normalization. Unlike `normalizeNameForMatch` (lowercases)
   * or `stripHonorifics` (drops "Dr."), "Dr. Jane  Smith " → "Dr. Jane Smith".
   *
   * Guards greetings like `Dear [Reviewer Name],` against whitespace artifacts in
   * the stored `wmkf_name` (a trailing space rendered "Dear Name ,").
   *
   * @param {*} name - Raw name (any type; non-strings/empty → null)
   * @returns {string|null} Cleaned display name, or null if nothing usable
   */
  static normalizeDisplayName(name) {
