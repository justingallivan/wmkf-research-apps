...
class ContactParser {
  ...
  static isNameConsistentEmail(email, name) {
    ...
    // Surname embedded in the local part — the strongest signal.
    if (surname.length >= 3 && local.includes(surname)) return true;
    // First-initial + surname ("jgallivan") or surname + first-initial.
    if (surname.length >= 3 && first.length >= 1) {
      const fi = first[0];
      if (local.includes(fi + surname) || local.includes(surname + fi)) return true;
    }
    return false;
  }
