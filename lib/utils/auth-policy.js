 ...
 *
 * NODE_ENV=production fails closed: returns true unconditionally unless
 * EMERGENCY_AUTH_BYPASS=true is set explicitly. This includes Vercel Preview
 * and Production runtimes. A misconfigured production-mode deploy
 ...
 *
 * Other NODE_ENV values keep the existing dev/test behavior: auth disabled if
...
export function isAuthRequired() {
  ...
  if (isProduction && emergencyBypass) {
    logBypassOnce('[auth-policy] EMERGENCY_AUTH_BYPASS=true in production — fail-closed override active; effective auth follows AUTH_REQUIRED and Azure credential configuration');
  }
