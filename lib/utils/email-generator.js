...
const { reflowAbstract } = require('./abstract-format');
 ...
 */
/**
 * Build the reviewer greeting line: "Dear Dr. Weeks".
 *
 * Single definition for every reviewer email. `parseRecipientName` supplies the
 * honorific (defaulting to "Dr." for academics), respects one the stored name
 * already carries (Prof./Mr./Ms./Mrs.), and strips generational/degree suffixes
 * so "Kevin Weeks Jr." greets "Dear Dr. Weeks".
 *
 * Falls back to a name-agnostic greeting when no surname is known, so a
 * blank/degenerate stored name never renders "Dear Dr. ,". Carries NO trailing
 * punctuation — templates supply their own comma or colon.
 *
 * @param {string} fullName
 * @returns {string}
 */
function buildReviewerGreeting(fullName) {
  const parsed = parseRecipientName(fullName);
...
function parseRecipientName(fullName) {
  ...
  if (honorificMatch) {
    const honorific = honorificMatch[1].toLowerCase().replace(/\.$/, '');
    switch (honorific) {
      case 'prof':
      case 'professor':
  ...
  // Strip the comma that separates credentials from the name ("Jane Roe, Ph.D.")
  // before matching suffixes, so the surname is "Roe" and not "Roe," — otherwise
  // a template ending in a comma renders "Dear Dr. Roe,,". lastName only; the
  // fullName/cleanName tokens keep the stored punctuation.
  const surnameParts = parts.map((p) => p.replace(/,+$/, '')).filter(Boolean);
...
function buildTemplateData(candidate, proposal, settings) {
  ...
  return {
    ...
    // Combined greeting for convenience — shared with every other reviewer
    // email so the honorific form cannot drift between them.
    greeting: buildReviewerGreeting(candidate.name),
    ...
    proposalAbstract: reflowAbstract(proposal.abstract || proposal.proposalAbstract || ''),
    piName: piName,
