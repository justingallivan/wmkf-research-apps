...
// Declared extension → canonical sniffed type. GIF is intentionally excluded for
// grantee award images (JPEG/PNG/WEBP only); sniffImageType still detects GIF so a
// GIF disguised with a png/jpg/webp extension is caught as a magic-byte mismatch.
 ...
 * @param {string} filename
 * @param {Buffer} buf
 * @returns {{ ok: true, type: 'png'|'jpeg'|'webp', ext: string }
...
export function validateGranteeImage(filename, buf) {
  ...
  if (!m) {
    return { ok: false, reason: 'unsupported image extension; allowed: .png, .jpg, .jpeg, .webp' };
  }
