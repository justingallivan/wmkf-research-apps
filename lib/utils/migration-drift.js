 ...
 * deduplicated `system_alerts` entry on drift; best-effort and never throws.
 *
 * Two distinct alert types:
 *   - `migration_tracker_missing` (severity error): the tracker table itself
 *     does not exist (42P01). This is the "fresh env / Phase 0 hasn't run"
 *     signal — operators need to see it as a separate, louder alert.
 *   - `migration_drift` (severity warning): tracker exists but manifest and
 *     tracker rows differ in either direction.
 *
