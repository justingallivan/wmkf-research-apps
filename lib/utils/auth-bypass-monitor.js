 ...
 *
 * `EMERGENCY_AUTH_BYPASS=true` disables the production fail-closed override and
 * lets the base AUTH_REQUIRED + credential predicate decide whether auth is
 * enforced (see `lib/utils/auth-policy.js`). That can let a misconfigured
 * production deploy fall open, so the lever must never be left set after an
 * incident even when the current base predicate still evaluates true. This
 * module raises a CRITICAL `system_alerts` row whenever the lever is found
 * active in production, and auto-resolves that alert once the lever is cleared.
 *
  ...
  try {
    if (active) {
      ...
      console.error(JSON.stringify({
        ...
        message: 'EMERGENCY_AUTH_BYPASS=true in production — fail-closed authentication protection is OVERRIDDEN',
        source,
      }));
      const alert = await NotificationService.notify({
        ...
        message:
          ...
          'fail-closed authentication protection is overridden and effective ' +
          'auth now follows AUTH_REQUIRED plus Azure credential configuration. ' +
          'Unset this variable immediately unless an active incident requires it.',
