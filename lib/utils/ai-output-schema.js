 ...
 *
 * Returns `{ ok: true, value }` or `{ ok: false, errors: string[] }`. Callers
 * that request `collectDroppedPaths` also receive the paths of undeclared keys
 * removed from the canonical value, without changing default result shapes.
 */
...
function validateNode(value, node, path, errors, options) {
  ...
  switch (node.type) {
    ...
    case 'array': {
      ...
      value.forEach((item, i) => {
        const v = validateNode(item, node.of, `${path}[${i}]`, errors, options);
    ...
    case 'object': {
      ...
      for (const [key, childNode] of Object.entries(node.fields)) {
        const v = validateNode(value[key], childNode, `${path}.${key}`, errors, options);
      ...
      for (const key of Object.keys(value)) {
        if (!(key in node.fields)) {
          options.droppedPaths?.push(`${path}.${key}`);
          if (node.allowExtra === 'error') fail(errors, path, `unexpected key "${key}".`);
        }
      }
      return out;
    }
    case 'record': {
      ...
      for (const [key, childValue] of entries) {
        ...
        const v = validateNode(childValue, node.of, keyPath, errors, options);
        if (v !== undefined) out[key] = v;
      }
 ...
 */
export function validateAiJson(parsed, schema, { path = '$', collectDroppedPaths = false } = {}) {
  const errors = [];
  const droppedPaths = collectDroppedPaths ? [] : null;
  const value = validateNode(parsed, schema, path, errors, { droppedPaths });
  if (errors.length > 0) return { ok: false, errors };
  return collectDroppedPaths ? { ok: true, value, droppedPaths } : { ok: true, value };
}
