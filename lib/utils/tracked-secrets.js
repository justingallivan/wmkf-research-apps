...
export const TRACKED_SECRETS = [
  { key: 'dossier_blob_read_write_token', name: 'Cycle Dossier private Blob store', tier: 'blob' },
  // Not yet provisioned (docs/plans/VIRTUAL_REVIEW_PANEL_PHASE_A_BUILD_PLAN_2026-09-12.md
  // D11) — tracked in anticipation so the cron reports 'not_tracked' rather
  // than the key being absent from rotation visibility once the owner
  // provisions the dedicated store.
  { key: 'review_panel_blob_read_write_token', name: 'Review Panel private Blob store', tier: 'forward' },
  ...
  { key: 'vercel_log_drain_secret',          name: 'Vercel Log Drain Secret (HMAC for /api/webhooks/vercel-log-drain)', tier: 'hmac' },

  // ─── API keys ───
  { key: 'claude_api_key',                   name: 'Anthropic Claude API Key',                                tier: 'vendor' },
  { key: 'openai_api_key',                   name: 'OpenAI API Key (Executor provider seam; opt-in)',          tier: 'vendor' },
  { key: 'openalex_api_key',                 name: 'OpenAlex API Key',                                        tier: 'vendor' },
  { key: 'cloudmersive_api_key',             name: 'Cloudmersive API Key (virus scan; gated by VIRUS_SCAN_ENABLED)', tier: 'vendor' },
  { key: 'perplexity_api_key',               name: 'Perplexity API Key (VRP sonar claim-verification + reviewer web discovery)', tier: 'vendor' },
