...
// akoya_requesttype — Akoya's NATIVE request-type picklist (distinct from
// wmkf_request_type above). Honoraria are "Scholarship" natively (verified 87/87 on
// the live cohort). On a bare create Dataverse auto-defaults this to 100000000 (the
// WRONG value), so the create body MUST set it explicitly. Global option-set int,
// stable across envs. Strategy doc §3a.
export const HONORARIUM_AKOYA_REQUEST_TYPE_SCHOLARSHIP = 100000001;
...
// Optional: transaction currency for the honorarium's money fields. When set, the
// create binds it explicitly (safest for a multi-currency org); when unset, Dataverse
// applies the org default currency (USD for this org). Env-specific GUID — resolve via
// scripts/probe-honorarium-discriminators.js if you choose to pin it. Strategy doc §3a.
export const HONORARIUM_CURRENCY_ID = orNull(process.env.HONORARIUM_CURRENCY_ID);

// Non-throwing companion to the assert below: true only when ALL three GUIDs are
// configured. The honorarium orchestrator uses this to decide capture-only
// (deferred) mode — when the discriminators aren't set, the akoya_request can't
// be minted, so we capture contact + address and defer the create rather than
// throw + alert per reviewer.
export function honorariumDiscriminatorsConfigured() {
  return Boolean(HONORARIUM_PROGRAM_ID && HONORARIUM_GRANTPROGRAM_ID && HONORARIUM_TYPE_ID);
}
