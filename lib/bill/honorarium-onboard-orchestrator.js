...
export async function ensureHonorariumOnboarding({ suggestion, request, reviewer, body }, deps = {}) {
  const {
  ...
  const contactId = await ensureContact({ reviewer, body, contacts, potentialReviewers });

  // ── 2. Address → contact.address1_* ──
...
async function ensureContact({ reviewer, body, contacts, potentialReviewers }) {
  ...
  const { id } = await contacts.findOrCreateByEmail({ firstName, lastName, email });
  ...
  if (reviewer?.wmkf_potentialreviewersid) {
    try {
      await potentialReviewers.setContactLink(reviewer.wmkf_potentialreviewersid, id);
    } catch (err) {
