...
async function ensureContact({ reviewer, body, contacts, potentialReviewers, actingUserSystemId }) {
  ...
  if (orcid) {
    ...
    if (orcidMatch?.ambiguous) {
      ...
      // staff to reconcile (owner decision: create + flag, never block). NOTE: this
      // flag is currently a server log only — a workbench-visible review surface is
      // a deferred follow-up (REVIEWER_CONTACT_BOUNDARY_GAP_FINDINGS, "flag for staff").
      console.warn(
