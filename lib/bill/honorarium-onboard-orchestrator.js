...
// method exists. `createRecord` stays raw DynamicsService: no akoya_requests
// create method exists on the (not-owned) grant-request adapter, so this one
// call site cannot convert (data-access-layer migration tail-2B skip). The
// `deps.dynamics` injection seam itself is unchanged, so
// tests/unit/honorarium-onboard-orchestrator.test.js's full fake override
// keeps working untouched.
const defaultDynamicsAdapter = {
  // Literal 'akoya_requests', 2-arg only — this file's single createRecord
  // call site (below) never passes an options 3rd arg.
  createRecord: (entity, data) => DynamicsService.createRecord('akoya_requests', data),
  getRecord: (entity, id, opts) => grantRequestAdapter.getById(id, { select: opts.select }),
  updateRecord: (entity, id, data) => contactAdapter.updateFields(id, data),
};
