...
// Default (production) `dynamics` shape backed by the DAL adapters — preserves
// the `deps.dynamics || <default>` injection seam (tests always inject a full
// fake `{ getRecord, updateRecord }`, so this default is exercised only in
// production). This file never passes an `updateRecord` options 4th arg or a
// `getRecord` select other than the fixed BILL pre-read, so both branches
// byte-mirror the prior raw calls exactly.
const defaultDynamicsAdapter = {
  getRecord: (entity, id) => contactAdapter.getBillingFieldsById(id),
  updateRecord: (entity, id, data) =>
    (entity === 'contacts' ? contactAdapter.updateFields(id, data) : grantRequestAdapter.updateById(id, data)),
};
import {
  BILLCOM_ACCOUNT_YES,
  BILLCOM_ACCOUNT_NO,
...
export async function onboardReviewer(input, deps = {}) {
  const {
    dynamics = defaultDynamicsAdapter,
  ...
  if (!reservation.reserved) {
    ...
    const resumePatch = await patchContactBillcomId(dynamics, input.reviewerContactId, stagedVendor);
    if (!resumePatch.ok) {
      await safeNotify(notifications, {
