 ...
 *   1. If BILL_ENABLED !== 'true' → dashboard alert + return status: 'alert_only'.
...
// ─────────────────────────── Input validation ───────────────────────────
// isGuid is the canonical branded guard from lib/utils/guid.js (imported
// above) — a local re-declaration here previously risked drifting from the
// trust-boundary contract (docs/TYPESCRIPT_OPTION_ASSESSMENT.md §5 step 4).
//
// The canonical guard TRIMS before testing, but this path uses the raw input
// id unchanged in reservation, contact reads, and akoya_request PATCHes — so a
// value with surrounding whitespace must still fail fast at validation (a 400
// before any BILL/Dataverse side effect), exactly as the prior untrimmed local
// guard did. isCanonicalGuid enforces canonical form (no surrounding
// whitespace) while keeping the shared branded guard as the base check.
/** @param {unknown} v @returns {boolean} */
function isCanonicalGuid(v) {
  return isGuid(v) && v === v.trim();
}
...
export function validateOnboardInput(input) {
  ...
  if (!isCanonicalGuid(input.honorariumRequestId)) return 'honorariumRequestId must be a GUID';
  if (!isCanonicalGuid(input.reviewerContactId)) return 'reviewerContactId must be a GUID';
