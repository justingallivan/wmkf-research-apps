...
export async function onboardReviewer(input, deps = {}) {
  ...
  if (!patchOutcome.ok) {
    ...
    await safeNotify(notifications, {
      type: 'bill_request_patch_failed',
      severity: 'warning',
      title: `BILL onboarding: honorarium request PATCH failed (${intendedStatus} path)`,
      message: patchOutcome.message,
      metadata: {
        honorariumRequestId: input.honorariumRequestId,
        vendorId,
  ...
  // Per Codex post-impl P1 #2: when the request PATCH failed after BILL-side
  // work, return 'partial' with the intended outcome preserved in
  // intendedStatus so the caller and alerts can see what we tried to do.
  if (patchFailed) {
    return {
