...
export async function onboardReviewer(input, deps = {}) {
  ...
  if (!reservation.reserved && !reservation.row?.vendor_id) {
    // Reserved by a concurrent — or abandoned — caller BEFORE a BILL vendor was
    // created. We can't safely create one now: an abandoned first caller may
    // have already created a vendor whose id we never persisted (→ duplicate
    // risk). Bail. If the row is genuinely abandoned the stuck-reconcile sweep
    // surfaces it for manual reconcile; if a concurrent caller is mid-create it
    // will finish. Either way a later re-accept resumes via the branch below
    // once vendor_id is present.
    return { ok: true, status: 'in_progress' };
  }
  // If reserved:false but the row already carries a vendor_id, a PRIOR attempt
  // got past vendor-create and persisted the id but didn't finish (contact
  // PATCH / writeback failed, process died, etc.). Fall through and RESUME: the
  // staged vendor_id is reused below (existingVendorId), so a re-accept
  // COMPLETES the stranded onboarding instead of looping on in_progress.
  // (Codex stop-time: "retries can be permanently stranded".)
  ...
  try {
    contact = await bypassDynamicsRestrictions('bill-onboard-pre-read', () =>
      dynamics.getRecord('contacts', input.reviewerContactId, {
  ...
  if (exactMatchCount === 1) {
    ...
    const patchOutcome = await patchAkoyaRequestSuccess(dynamics, input.honorariumRequestId, pni, sleep);
    if (!patchOutcome.ok) {
  ...
  const patchOutcome = await patchAkoyaRequestNoMatch(dynamics, input.honorariumRequestId, sleep);
...
async function patchAkoyaRequestSuccess(dynamics, requestId, pni, sleep) {
  const body = {
    wmkf_paymentnetworkidpni: pni,
    wmkf_exisitngbillcomaccount: BILLCOM_ACCOUNT_YES,
  };
  return patchAkoyaRequestWithRetry(dynamics, 'bill-onboard-request-patch-yes', requestId, body, sleep);
}

async function patchAkoyaRequestNoMatch(dynamics, requestId, sleep) {
  const body = {
    wmkf_exisitngbillcomaccount: BILLCOM_ACCOUNT_NO,
  };
  return patchAkoyaRequestWithRetry(dynamics, 'bill-onboard-request-patch-no', requestId, body, sleep);
}
 ...
 */
async function patchAkoyaRequestWithRetry(dynamics, context, requestId, body, sleep) {
  let lastMessage = 'unreachable';
  for (let attempt = 0; attempt < REQUEST_PATCH_ATTEMPTS; attempt++) {
    try {
      await bypassDynamicsRestrictions(context, () =>
        dynamics.updateRecord('akoya_requests', requestId, body),
      );
      return { ok: true };
    } catch (err) {
