...
jest.mock('../../lib/services/pre-rp-brief/reopen-service', () => ({
  reopenSentPreRpBrief: jest.fn(),
}));
...
import { reopenSentPreRpBrief } from '../../lib/services/pre-rp-brief/reopen-service';
import { isGuardedReopenSchemaReady } from '../../lib/utils/guarded-reopen-readiness';
import handler from '../../pages/api/workbench/pre-rp-brief/reopen';
...
beforeEach(() => {
  ...
  reopenSentPreRpBrief.mockResolvedValue({
...
test('requires an authenticated superuser with a linked Dynamics identity', async () => {
  ...
  expect(reopenSentPreRpBrief).not.toHaveBeenCalled();
});
...
test('fails closed before DAL access while guarded-reopen readiness is off', async () => {
...
test('runs guarded reopen in authenticated DAL context with the session actor', async () => {
  ...
  expect(withDalContext).toHaveBeenCalledWith('workbench-pre-rp-brief-reopen', expect.any(Function));
  expect(reopenSentPreRpBrief).toHaveBeenCalledWith(requestBody, { actingUserSystemId: USER_ID });
...
test('returns 202 for a safely leased in-progress operation', async () => {
  reopenSentPreRpBrief.mockResolvedValueOnce({
  ...
  expect(res.body).toMatchObject({ success: true, reused: true });
});

test('preserves governed conflict codes without exposing internal errors', async () => {
  reopenSentPreRpBrief.mockRejectedValueOnce(new ServiceHttpError('Reload and retry.', {
    httpStatus: 409,
    code: 'brief_reopen_stale',
    body: { error: 'Reload and retry.', code: 'brief_reopen_stale' },
  }));
  const res = mockRes();
  await handler({ method: 'POST', body: body() }, res);
