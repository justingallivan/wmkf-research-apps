...
function authorship(displayName, authorId = 'A100', orcid = null) {
  return {
    displayName,
...
function work(pmid, displayName = 'Alice Example', authorId = 'A100') {
  return {
    openAlexId: `https://openalex.org/W${pmid}`,
    title: `Bound work ${pmid}`,
    pmid: String(pmid),
    authorships: [authorship(displayName, authorId)],
  };
}
...
describe('independent-identity/v1', () => {
  ...
  test('the source work does not count unless PubMed supplies a full-forename byline', async () => {
    const deps = dependencies(
      { sourceWork: { pmid: '251' } },
      {
        searchPubmed: jest.fn(async () => [article('252'), article('253')]),
      },
    );
    const result = await evaluateIndependentReviewerIdentity({
      ...BINDING,
  ...
  test('PubMed returns sufficient only when three full-forename works bind to one author cluster', async () => {
    const deps = dependencies(
      { sourceWork: { pmid: '301' } },
      {
        searchPubmed: jest.fn(async () => [article('301'), article('302'), article('303')]),
      },
    );
    const result = await evaluateIndependentReviewerIdentity({
      ...BINDING,
    ...
    expect(deps.providers.searchPubmed).toHaveBeenCalledWith(
      expect.stringContaining('Alice Example[Author]'),
      30,
  ...
  test('same-name works split across author clusters are insufficient', async () => {
    const deps = dependencies(
      { sourceWork: { pmid: '401' } },
      {
        searchPubmed: jest.fn(async () => [article('401'), article('402'), article('403')]),
        getWorkByExternalId: jest.fn(async (_kind, value) => ({
          totalCount: 1,
  ...
  test('forename grounding succeeds only when the exact-work author occurs once in a complete pool', async () => {
    const deps = dependencies(
      { sourceWork: { pmid: '602' } },
      {
        searchAuthors: jest.fn(async () => ({
  ...
  test('stale server-held hard-ID evidence cannot return sufficient', async () => {
    const result = await evaluateIndependentReviewerIdentity({
      ...BINDING,
      method: METHODS.HARD_ID_JOIN,
    }, dependencies({
      providerObservedAt: new Date(NOW - MAX_TTL_MS - 1).toISOString(),
      sourceOrcid: '0000-0002-1825-0097',
      sourceOrcidLineage: 'byline_asserted',
      crmOrcid: '0000-0002-1825-0097',
      crmOrcidLineage: 'staff_entered',
  ...
  test('partial stored-provider evidence cannot authorize a no-call hard-ID join', async () => {
    const result = await evaluateIndependentReviewerIdentity({
      ...BINDING,
      method: METHODS.HARD_ID_JOIN,
    }, dependencies({
