...
describe('CandidateEditModal — merge mode', () => {
  ...
  // (c2) The S306 Swap-hint for a `loser_in_applicant_slot` block was removed when
  // that v1 block was lifted (the merge now repoints applicant slots to the keeper),
  // so the reason code is no longer produced and the hint no longer renders. The
  // general blocked-plan rendering is covered by the test above.
