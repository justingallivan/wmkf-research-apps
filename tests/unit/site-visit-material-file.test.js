...
test('slot decides the extensions; bytes must match the extension signature', () => {
  ...
  expect(validateSiteVisitMaterial('deck.pptx', ZIP, 'presentation_source').ok).toBe(true);
  ...
  expect(validateSiteVisitMaterial('bios.docx', ZIP, 'participant_bios').ok).toBe(true);
