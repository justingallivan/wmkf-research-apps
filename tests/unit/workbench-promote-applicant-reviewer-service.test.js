...
test('manual email collision: promotion stands, partialSuccess + email_conflict, backfill skipped', async () => {
  ...
  expect(findCandidateBySuggestion).not.toHaveBeenCalled(); // manual attempt gates the backfill
...
test('B1 idempotency: existing person email blocks the backfill; roster read failure is non-fatal', async () => {
  ...
  let body = await promoteApplicantReviewer(args());
  expect(body.savedFields).toEqual([]);
  expect(update).not.toHaveBeenCalled();

  findCandidateBySuggestion.mockRejectedValue(new Error('roster down'));
  body = await promoteApplicantReviewer(args());
  expect(body).toMatchObject({ success: true, partialSuccess: false, contactError: null });
});
