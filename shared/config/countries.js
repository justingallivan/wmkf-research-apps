/**
 * ISO 3166-1 country data + a tolerant normalizer to alpha-2.
 *
 * Used by the Stage 2a reviewer accept form (payment-address country picker)
 * and the context endpoint's address prefill. The downstream contract is
 * strict alpha-2: `respond.js` validateAddress and `lib/bill` createBillVendor
 * both reject any country that isn't a 2-char code. The picker therefore emits
 * codes directly, and prefill must coerce stored free-text (`contact
 * .address1_country` is historically a full name like "United States", and may
 * also be an alpha-3 code) into an alpha-2 code — or fall back to '' so the
 * reviewer picks one.
 *
 * COUNTRIES is the COMPLETE set of officially-assigned ISO 3166-1 alpha-2
 * codes (incl. territories like PR/GU/VI/MO/GL), so the closed <select> can
 * never hard-block a reviewer whose region is simply missing from a curated
 * subset — the failure mode a stop-time review flagged. Each entry also carries
 * its alpha-3 code so the normalizer can map alpha-3 prefill values.
 *
 * Plain ESM with no platform deps so it imports cleanly on both the client
 * (Stage2aView) and the Node API route (context.js).
 */

// code = ISO 3166-1 alpha-2 (uppercase); code3 = alpha-3; name = common
// English short name. Sorted by name. This is the full officially-assigned set.
export const COUNTRIES = [
  ...
  { code: 'CG', code3: 'COG', name: 'Congo (Republic)' },
  { code: 'CD', code3: 'COD', name: 'Congo (Democratic Republic)' },
  ...
  { code: 'VA', code3: 'VAT', name: 'Holy See (Vatican City)' },
...
// Set of valid uppercase alpha-2 codes for O(1) membership checks.
const CODE_SET = new Set(COUNTRIES.map((c) => c.code));

// name (lowercased) → alpha-2, for the common "stored as full name" case.
const NAME_TO_CODE = new Map(COUNTRIES.map((c) => [c.name.toLowerCase(), c.code]));

// alpha-3 (lowercased) → alpha-2, for stored ISO-3 values (CAN, GBR, …).
const ALPHA3_TO_CODE = new Map(COUNTRIES.map((c) => [c.code3.toLowerCase(), c.code]));

// Hand-curated aliases for stored values that won't match a canonical name or
// code exactly. Lowercased keys. Intentionally conservative — when in doubt we'd
// rather return '' (reviewer picks) than silently coerce to the wrong country.
...
/**
 * Coerce a free-text, alpha-2, or alpha-3 country value into an ISO 3166-1
 * alpha-2 code, or '' if it can't be confidently mapped.
 *
 *   normalizeCountryToIso2('US')             → 'US'
 *   normalizeCountryToIso2('us')             → 'US'
 *   normalizeCountryToIso2('United States')  → 'US'
 *   normalizeCountryToIso2('USA')            → 'US'   (alpha-3)
 *   normalizeCountryToIso2('GBR')            → 'GB'   (alpha-3)
 *   normalizeCountryToIso2('Puerto Rico')    → 'PR'
 *   normalizeCountryToIso2('Freedonia')      → ''     (unknown — caller defaults)
 *   normalizeCountryToIso2(undefined)        → ''
 *
 * @param {*} input
 * @returns {string} uppercase alpha-2 code, or '' when unmappable
 */
export function normalizeCountryToIso2(input) {
  if (typeof input !== 'string') return '';
  const trimmed = input.trim();
  if (!trimmed) return '';

  // Already a valid alpha-2 code (any case).
  ...
  // alpha-3 code.
  if (trimmed.length === 3 && ALPHA3_TO_CODE.has(lower)) {
    return ALPHA3_TO_CODE.get(lower);
  }

  if (NAME_TO_CODE.has(lower)) return NAME_TO_CODE.get(lower);
  if (ALIASES.has(lower)) return ALIASES.get(lower);

  return '';
}

/** True if `code` is a recognized alpha-2 country code (case-insensitive). */
export function isIso2Country(code) {
