      ...
      <form onSubmit={saveSession} className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <fieldset className="mt-6 border-t border-gray-100 pt-5">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {[['Staff', recipients.staff], ['Board', recipients.board]].map(([label, rows]) => <div key={label}><h2 className="text-sm font-semibold text-gray-700">{label}</h2><div className="mt-2 flex flex-wrap gap-2">{rows.map((person) => { const selected = form.attendees.some((ref) => sameRef(ref, person.ref)); const noEmail = !person.email; return <button key={`${person.ref.kind}-${person.ref.profileId || person.ref.rosterId}`} type="button" aria-pressed={selected} disabled={noEmail && !selected} title={noEmail ? NO_EMAIL_HINT : undefined} onClick={() => toggleAttendee(person.ref)} className={`rounded-full border px-3 py-1.5 text-sm font-medium disabled:cursor-not-allowed disabled:opacity-50 ${selected ? 'border-gray-900 bg-gray-900 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50'}`}>{person.name}{noEmail ? ' · no email' : ''}</button>; })}{rows.length === 0 && <p className="text-sm text-gray-500">No eligible people found.</p>}{rows.some((person) => !person.email) && <p className="w-full text-xs text-gray-500">{NO_EMAIL_HINT}</p>}</div></div>)}
          </div>
        </fieldset>
