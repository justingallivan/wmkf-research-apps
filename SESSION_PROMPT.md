...
- **Two-dashboard model (locked S206):** (a) **reviewer dashboard** = the mocked *post-shortlist* surface (request queue → per-request Workbench; Reviewers tab = v1) — replaces Reviewer Finder + Review Manager; (b) **J27 triage/cycle dashboard** = the *upstream* spreadsheet-replacement funnel (D26 ~200→32→28; J27 up to ~300), NOT yet mocked.
...
- **Reviewer tab structure: DECIDED S206 — 4-tab + status badges** (Find / Invite / Track / Approve & Pay), state-aware default landing (earliest funnel step with outstanding work: Invite if shortlisted-unsent, Track if invited, Approve & Pay if reviews back, Find if nothing). (Arc: briefly 3-tab w/ "Roster", then reconsidered and locked — "Roster" is a non-action noun, Invite vs Track are distinct modes, white-space worry minor at per-request scale; badges on the tab bar recover Roster's at-a-glance overview, counting work-remaining not totals.) "Closeout" disambiguated: reviewer-level step → "Approve & Pay" (PD closeout = `wmkf_reviewstatus=complete` + `wmkf_completedat`; honorarium is a SEPARATE staff-gated path, see scoping doc §3.4 + the payment-gate open item below); request-level endpoint → read-only "Status" (reflects Dynamics `akoya_requeststatus`, board-decided, not PD-editable). Still-open: PD dashboard row content; whether PD closeout should gate payment (the closeout fields `wmkf_reviewstatus=complete` + `wmkf_completedat` are already deployed S196 — a Justin/Steph policy call, NOT a missing field); J27 phase trigger (Connor). Scoping doc written: `docs/REQUEST_WORKBENCH_SCOPING.md`.
...
- **Next-build open items:** (1) **payment-gate policy** — "Approve & Pay" implies PD approval gates payment, but the deployed model keys eligibility on reviewer-submission (`wmkf_reviewreceivedat`) + Steph's remit gate, and `wmkf_completedat` is record-keeping only. Decide: rename the tab to "Review & Close" OR make closeout an actual payment gate (loop in Steph). (2) PD dashboard **row content / `isActionableForPD`** (reviewer-centric for v1). (3) **J27 phase trigger** (Connor).
...
| `docs/REQUEST_WORKBENCH_SCOPING.md` | S206 Connor/Sarah-shareable scoping doc: two-dashboard model, reviewer tab structure, D26 allowlist patch, dependencies, scope fences. |
