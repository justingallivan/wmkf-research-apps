...
- **Reviewer tab structure: DECIDED S206 — 4-tab + status badges** (Find / Invite / Track / Completed), state-aware default landing (earliest funnel step with outstanding work: Invite if shortlisted-unsent, Track if invited, Completed if reviews back awaiting sign-off, Find if nothing). (Arc: briefly 3-tab w/ "Roster", then reconsidered and locked.) Badge note: Track counts work-remaining; **Completed counts DONE (# completed)** — Justin's call, a deliberate exception. "Closeout" disambiguated: reviewer-level step → **"Completed"** (sets `wmkf_reviewstatus=complete` + `wmkf_completedat`; **record-keeping only — no trigger, no drop-off**; honorarium is a SEPARATE staff-gated path on the reviewer-submission signal); request-level endpoint → read-only "Status" (reflects Dynamics `akoya_requeststatus`, board-decided, not PD-editable). Scoping doc written: `docs/REQUEST_WORKBENCH_SCOPING.md`.
