...
| D2 | generate-emails draft generation mark | raw `patchFields` stamps `wmkf_invited`/`wmkf_emailsentat` unconditionally (`markInvitationGenerated` alias kept). **Usage checked S490: the route has zero client callers in the tracked tree; no runtime log exists for it.** | retire the mark, or retire the whole orphaned route (destructive; needs pre-flight) | none in-app |
