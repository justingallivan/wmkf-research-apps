...
- **Reviewer tab structure: S206 leaning 4-tab + status badges** (Find / Invite / Track / Approve & Pay), default landing Track — pending Justin's final confirm. (Arc: briefly 3-tab w/ "Roster", then reconsidered — "Roster" is a non-action noun, Invite vs Track are distinct modes, white-space worry minor at per-request scale; badges on the tab bar recover Roster's at-a-glance overview.) "Closeout" disambiguated: reviewer-level step → "Approve & Pay" (honorarium trigger); request-level endpoint → read-only "Status" (reflects Dynamics `akoya_requeststatus`, board-decided, not PD-editable). Still-open: confirm the 4-tab call; PD dashboard row content; the approve→payable status field name owed to Connor.
