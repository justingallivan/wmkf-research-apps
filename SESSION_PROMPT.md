...
0. **Stage 1d accepted by the owner (Session 536, 2026-09-23).** Six root-review defects fixed (`f06b5d3a8`, `7c8edcf45`, `6f2d56c48`, `008612629`), two Codex re-review findings fixed (`ee9901c59`, `c0cbf0dad`), and three further findings fixed by Codex rescue and reviewed by Claude (`6a293fe12`, `744bde88d`, `53260f1af`). Full record in the design doc's Stage 1d fix record. No fourth review round was run, by owner decision. Source-bundle exporter built and hardened to bundle v2 after a Codex adversarial review (strict discovery, drive identity, membership fence), then further hardened to enforce tracked Dataverse/SharePoint origins and distinguish request-folder misses from archive-library failure. The Session 536 v2 bundle for production Request 1003222 predates those final checks and must be re-exported before use. The sandbox clone must re-resolve each library drive on the registered site before copy; the stored drive ID is not routing authority. Next: Claude review, re-export, then the sandbox Basic clone reading the new bundle, with file copy.
