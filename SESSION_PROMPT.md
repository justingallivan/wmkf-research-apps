...
- **Reviewer tab structure: DECIDED S206 — 3-tab** (Find / Roster / Approve & Pay), default landing Roster. The 4-tab alternative was rejected. "Closeout" disambiguated: reviewer-level step → "Approve & Pay" (honorarium trigger); request-level endpoint → read-only "Status" (reflects Dynamics `akoya_requeststatus`, board-decided, not PD-editable). Still-open structural questions that feed the mockups: PD dashboard row content; the approve→payable status field name owed to Connor.
