...
function RunItem({ item, candidateById }) {
  ...
  return (
    <li className="flex flex-wrap items-center gap-3 border-b border-gray-100 py-3 last:border-0">
      ...
      {item.status === 'ready' && item.revisionId && (
        <span className="inline-flex items-center gap-1.5" data-testid="cycle-dossier-run-item-links">
          <a className="inline-flex items-center gap-1 rounded-md border border-gray-300 bg-white px-2 py-1 text-xs font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-500" href={`/api/cycle-dossier/download?entryId=${encodeURIComponent(item.revisionId)}&format=docx`} download><Icon name="download" className="h-3 w-3" /> Word</a>
          <a className="inline-flex items-center gap-1 rounded-md border border-gray-300 bg-white px-2 py-1 text-xs font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-500" href={`/api/cycle-dossier/download?entryId=${encodeURIComponent(item.revisionId)}&format=pdf`} download><Icon name="download" className="h-3 w-3" /> PDF</a>
        </span>
      )}
...
function EditionRow({ edition, onPreview }) {
  ...
  const requestNumbers = Array.isArray(edition.requestNumbers) ? edition.requestNumbers : [];
  ...
  return (
    <li className="rounded-xl border border-gray-200 bg-white p-4">
      ...
      <p className="mt-2 text-xs text-gray-700" data-testid="cycle-dossier-edition-contents">
        {requestNumbers.length > 0
          ? `${requestNumbers.length} entr${requestNumbers.length === 1 ? 'y' : 'ies'}: ${requestNumbers.map((n) => `#${n}`).join(', ')}`
...
export function CycleDossierWorkspace() {
  ...
  const load = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      if (!initialViewSet.current) {
        ...
        const latestRunStatus = String(body.runs?.[0]?.status || '').toLowerCase();
        if (['queued', 'running', 'paused'].includes(latestRunStatus)) setView('progress');
      }
  ...
  const runActive = latestRun && ['queued', 'running', 'paused'].includes(runStatus);
  ...
  const setAllIncluded = useCallback((included) => {
    const next = included ? new Set(candidates.map((candidate) => toId(candidate.requestId))) : new Set();
    selectedIdsRef.current = next;
    setSelectedIds(next);
    void saveSelection(next);
    setPreview(null);
  }, [candidates, saveSelection]);
        ...
        <section aria-labelledby="choose-requests-heading">
          <Card hover={false} padding="p-5 sm:p-6">
            <SectionTitle icon="file" title="Choose requests" detail="All eligible D26 requests start included. Exclude test cases here; the decision persists for your dossier.">
              <div className="flex items-center gap-2"><div className="inline-flex rounded-lg border border-gray-200 bg-gray-50 p-1" role="group" aria-label="Roster filter">{[['all', 'All'], ['included', 'Included'], ['excluded', 'Excluded']].map(([value, label]) => <button key={value} type="button" aria-pressed={rosterFilter === value} onClick={() => setRosterFilter(value)} className={`rounded-md px-2.5 py-1.5 text-xs font-semibold ${rosterFilter === value ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-800'}`}>{label}</button>)}</div><StatusPill tone="info">{selectedCandidates.length} of {candidates.length} included</StatusPill><span className="ml-auto inline-flex gap-2"><button type="button" onClick={() => setAllIncluded(true)} disabled={loading || candidates.length === 0 || selectedCandidates.length === candidates.length} className="rounded-md px-2.5 py-1.5 text-xs font-semibold text-gray-700 hover:bg-gray-100 disabled:opacity-40 focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-500">Include all</button><button type="button" onClick={() => setAllIncluded(false)} disabled={loading || selectedCandidates.length === 0} className="rounded-md px-2.5 py-1.5 text-xs font-semibold text-gray-700 hover:bg-gray-100 disabled:opacity-40 focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-500">Exclude all</button></span></div>
            </SectionTitle>
