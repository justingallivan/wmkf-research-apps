...
function BatchPhaseISummaries() {
  ...
  const exportAllAsMarkdown = () => {
    ...
    let content = `# Batch Phase I Summaries\n\n`;
    content += `Generated on: ${new Date().toLocaleDateString()}\n`;
    content += `Summary Length: ${summaryLength} paragraph${summaryLength > 1 ? 's' : ''}\n`;
    ...
    a.href = url;
    a.download = `batch_phase_i_summaries_${new Date().toISOString().split('T')[0]}.md`;
    a.click();
  ...
  return (
    <Layout
      ...
      description="Process multiple Phase I proposals at once with customizable summary length"
    >
      <PageHeader
        title="Batch Phase I Summaries"
        subtitle="Process multiple Phase I proposals simultaneously with customizable summary length and technical level"
        icon="📑"
      />
        ...
        <Card className="mb-6">
          <div className="mb-4">
            <h2 className="text-xl font-semibold text-gray-900 mb-2 flex items-center gap-2">
              <span>📁</span>
              <span>Upload Phase I Proposals</span>
            </h2>
          </div>
          <FileUploaderSimple
          ...
          <Card className="mb-6 bg-green-50 border-green-200">
            <div className="text-center">
              ...
              <p className="text-gray-700 mb-4">
                {selectedFiles.length} Phase I proposal{selectedFiles.length > 1 ? 's' : ''} uploaded and ready for batch processing
                <br />
                Summary: {summaryLength} paragraph{summaryLength > 1 ? 's' : ''} • Level: {summaryLevel ? summaryLevel.replace('-', ' ') : 'technical non expert'}
              </p>
              <Button
                variant="primary"
                size="lg"
