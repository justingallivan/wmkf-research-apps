/**
 * API: POST /api/workbench/pre-rp-brief/reopen
 *
 * Superuser-only guarded regeneration of a Pre-RP Brief already sent to the
 * Board (owner decision 2026-09-16). Copied from
 * pages/api/workbench/pre-site-visit/reopen.js line for line except the
 * service import, the DAL context name, and the error codes/messages named
 * below. Caller identities are expected-value and confirmation inputs only;
 * the service independently resolves all Dataverse authority before creating
 * a successor.
 */
...
function sendError(res, error) {
  if (error instanceof ServiceHttpError) {
    return res.status(error.httpStatus).json(error.body ?? {
      error: error.message,
      code: error.code || 'brief_reopen_failed',
    });
  }
  console.error('workbench pre-rp-brief reopen error:', error);
  return res.status(500).json({
    error: 'The Pre-RP Brief could not be regenerated.',
...
export default async function handler(req, res) {
  ...
  return withDalContext('workbench-pre-rp-brief-reopen', async () => {
    try {
      const result = await reopenSentPreRpBrief(req.body, { actingUserSystemId });
