...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (result.sent || errs) {
      console.log(`[cron:site-visit-materials-reminders] scanned=${result.scanned} eligible=${result.eligible} sent=${result.sent} nothingMissing=${result.skippedNothingMissing} noSender=${result.skippedNoSender} claimLost=${result.claimLost} sendFail=${result.sendFailed} sendUnconfirmed=${result.sendUnconfirmed} receiptFail=${result.receiptFailed} dryRun=${dryRun}`);
    }
