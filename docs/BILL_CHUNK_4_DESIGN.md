...
- **Stop-time: re-accept retries could be permanently stranded.** The `reserved:false → in_progress` short-circuit originally bailed for ALL lost-reservation cases, so a re-accept of a row whose first attempt created the vendor but failed the contact PATCH / writeback looped on `in_progress` forever (only the stuck-reconcile ops alert surfaced it). **Fix:** `in_progress` now applies ONLY when the reserved row has NO `vendor_id` (concurrent active create, or the rare abandoned pre-vendor case — can't safely re-create without a duplicate). When the reserved row already carries a `vendor_id`, the call FALLS THROUGH and RESUMES (reuses the staged vendor, completes search/invite/writeback), so a re-accept self-heals a stranded post-vendor onboarding. The `vendor_id`-null abandoned case remains a documented alert-only residual (safe auto-resume needs the unverified BILL vendor-lookup).
- **DIRECT_SERVICE_CALL_DRIFTS_FROM_ENDPOINT_CONTRACT (intentional deviation, documented).** The orchestrator calls `onboardReviewer()` **in-process** rather than HMAC-POSTing `/api/bill/onboard-reviewer`. This is deliberate: a Next.js route self-fetching its own API route on Vercel is an anti-pattern (extra cold start, base-URL fragility, doubled invocation) — the service module exists precisely so callers share the logic without the HTTP hop. The HMAC endpoint (`pages/api/bill/onboard-reviewer.js` + `internal-call-auth.js`) remains the boundary for any **external** caller (e.g. PowerAutomate); both paths share the same validated `onboardReviewer()` core. The design's "Call /api/bill/onboard-reviewer" step (Thread 1 §4) is satisfied by the in-process call.
