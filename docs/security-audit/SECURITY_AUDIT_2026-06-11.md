...
1. 🟡 Private Blob migration — pilot + file-loader cohort shipped, smoked, and **all three server-read consumers promoted to production** 2026-06-11 (`expense-reporter` + `phase-i-dynamics` + `grant-reporting`; grant-reporting prod-verified). The browser-render consumer (grant-cycle materials) + record-scoped download proxy are code-complete + Codex-reviewed (flag-gated, default public; **live e2e smoke pending** — `DOWNLOAD_PROXY_DESIGN_2026-06-11.md`).
