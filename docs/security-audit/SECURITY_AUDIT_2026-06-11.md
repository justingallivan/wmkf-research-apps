...
Status: VERIFIED → 🟡 PILOT SHIPPED + SMOKED 2026-06-11 (remediation Phase 1). `expense-reporter` can upload private blobs — **flag-gated** by `NEXT_PUBLIC_EXPENSE_REPORTER_PRIVATE_BLOB` (default `public`, so prod is unchanged until promoted) — and `process-expenses` reads them server-side by `pathname` via `lib/utils/uploaded-blob.js` (`get(pathname,{access:'private'})`); when enabled, no auth-free URL is created for receipts. Unit-tested + build green; **live smoke PASSED** (real upload→extract; receipts in the private store; URL returns HTTP 403). **Progress:** `file-loader.js` is now private-aware (2026-06-11); `phase-i-dynamics` **and** `grant-reporting` are migrated (flag-gated) and **read-path live-smoked** (2026-06-11, `scripts/smoke-private-file-loader.mjs`: a DOCX round-trips through the shared `loadFile` private read + text-extract; blob URL HTTP 403) — completing the file-loader consumer cohort. **Promoted to production 2026-06-11:** prod `UPLOADS_BLOB_RW_TOKEN` + both cohort flags set + deployed (`dpl_Cd6MGvsGvYgcqW8LHPNV4j7Wg5oA`); grant-reporting prod-verified (live upload → private store `wvodkxrlwniaujaj.private.blob…`, URL HTTP 403, extraction ran); phase-i-dynamics shares that read path. `expense-reporter` **also promoted** 2026-06-11 (`NEXT_PUBLIC_EXPENSE_REPORTER_PRIVATE_BLOB=true` set in Production + deployed `wmkfresearchapps-njdq4gr5y`); it shares the same prod-verified store/token/read path (browser spot-check optional). **All three Phase-1 server-read consumers are now private in production.** The browser-render consumer (reviewer-finder grant-cycle materials) + its record-scoped download proxy are **code-complete + Codex-reviewed** (flag-gated `NEXT_PUBLIC_REVIEWER_FINDER_PRIVATE_CYCLE_MATERIALS`, default public; PARKED — not smoked/promoted (low-risk, legacy-only consumer)) — see `docs/security-audit/DOWNLOAD_PROXY_DESIGN_2026-06-11.md`. Design + what-shipped: `docs/archive/PHASE_1_PRIVATE_BLOB_DESIGN_2026-06-11.md`.
...
- Prior checked-in scanner precedent exists at `docs/archive/SEMGREP_AUDIT_REPORT.md`, but this audit did not refresh those scan results.
...
1. 🟡 Private Blob migration — pilot + file-loader cohort shipped, smoked, and **all three server-read consumers promoted to production** 2026-06-11 (`expense-reporter` + `phase-i-dynamics` + `grant-reporting`; grant-reporting prod-verified). The browser-render consumer (grant-cycle materials) + record-scoped download proxy are code-complete + Codex-reviewed (flag-gated, default public; **PARKED — not smoked/promoted (low-risk, legacy-only consumer)** — `DOWNLOAD_PROXY_DESIGN_2026-06-11.md`).
