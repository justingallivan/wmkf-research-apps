...
1. 🟡 Private Blob migration — pilot + file-loader cohort shipped, smoked, and **all three consumers promoted to production** 2026-06-11 (`expense-reporter` + `phase-i-dynamics` + `grant-reporting`; grant-reporting prod-verified). Remaining open work: the remaining (browser-render) consumers and the download proxy.
