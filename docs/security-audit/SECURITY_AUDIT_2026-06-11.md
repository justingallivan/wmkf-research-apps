...
1. 🟡 Private Blob migration — pilot + file-loader cohort shipped, smoked, and **promoted to production** 2026-06-11 (`expense-reporter` pilot + `phase-i-dynamics` + `grant-reporting`, the latter prod-verified). Remaining open work: promote `expense-reporter` (Preview-only flag → still public in prod), the remaining (browser-render) consumers, and the download proxy.
