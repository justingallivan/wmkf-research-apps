...
**Status:** ✅ PILOT IMPLEMENTED + SMOKED 2026-06-11 (expense-reporter). SDK verified, design decisions resolved, pilot shipped server-read-only, Codex-reviewed (REVISE — folded), live smoke passed. **Follow-on (2026-06-11):** `lib/utils/file-loader.js` is now private-aware (delegates to `readUploadedBlobBuffer`); `phase-i-dynamics` and `grant-reporting` are migrated (flag-gated) — the full file-loader consumer cohort. **Read path live-smoked (2026-06-11):** `scripts/smoke-private-file-loader.mjs` round-trips a real DOCX through `loadFile({access:'private',pathname})` against the live store (text extracts; URL HTTP 403) — covers both cohort consumers, which share the chokepoint. **Cohort PROMOTED TO PRODUCTION 2026-06-11:** prod `UPLOADS_BLOB_RW_TOKEN` + both cohort flags set + deployed (`dpl_Cd6MGvsGvYgcqW8LHPNV4j7Wg5oA`); grant-reporting prod-verified (live upload → private store, URL 403, extraction ran); phase-i-dynamics shares that read path. `expense-reporter` also promoted (flag set in Production + deployed `wmkfresearchapps-njdq4gr5y`). **All three Phase-1 server-read consumers are now private in production.** The browser-render consumer (reviewer-finder grant-cycle materials) + its record-scoped download proxy are **code-complete + Codex-reviewed** (flag-gated, default public; PARKED — not smoked/promoted (low-risk, legacy-only consumer)) — see `docs/security-audit/DOWNLOAD_PROXY_DESIGN_2026-06-11.md`. See "Implementation notes" at the end.

> ✅ **Live smoke PASSED 2026-06-11; cohort promoted to production.** Private blobs use a **dedicated private store** (`wmkf-uploads-private`, `store_WvoDkxrlWniAuJAj`, iad1) with its own `UPLOADS_BLOB_RW_TOKEN` — NOT the public `BLOB_READ_WRITE_TOKEN` store. **Verified end-to-end** (run locally against the real store, flag on): `scripts/smoke-private-upload.mjs` passed; a live expense-reporter receipt upload → extraction worked; the receipts landed in the **private** store; and the receipt URL returns **HTTP 403** unauthenticated. **Production (2026-06-11):** `UPLOADS_BLOB_RW_TOKEN` set in dev + preview **+ production**, and `phase-i-dynamics` + `grant-reporting` flags set in Production + deployed (`dpl_Cd6MGvsGvYgcqW8LHPNV4j7Wg5oA`) — grant-reporting prod-verified (live upload → `wvodkxrlwniaujaj.private.blob…`, URL 403, extraction ran). `expense-reporter` flag (`NEXT_PUBLIC_EXPENSE_REPORTER_PRIVATE_BLOB=true`) also set in Production + deployed (`wmkfresearchapps-njdq4gr5y`) — **all three Phase-1 server-read consumers now upload private in prod.** The browser-render consumer (grant-cycle materials) + record-scoped download proxy are **code-complete, flag-gated, PARKED, not smoked/promoted (low-risk legacy consumer)** (`docs/security-audit/DOWNLOAD_PROXY_DESIGN_2026-06-11.md`). Where the token is unset the code fails closed — upload-handler 503, read throws.
...
The browser-facing **download proxy** for the **render** consumer (reviewer-finder
grant-cycle template + attachments, formerly via `proxifyBlobUrl`) has **since been built**
(2026-06-11, code-complete + Codex-reviewed, flag-gated, PARKED, not smoked/promoted (low-risk legacy consumer)) — see
