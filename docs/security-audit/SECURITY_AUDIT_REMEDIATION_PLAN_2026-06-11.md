...
> **Pilot shipped + smoked 2026-06-11** (`expense-reporter`); **file-loader cohort followed** (`file-loader.js` private read + `phase-i-dynamics` + `grant-reporting`, read-path live-smoked 2026-06-11). The steps below are the original full-slice plan; the pilot that shipped was narrower (server-read-only, no download proxy). The `file-loader.js` private read has since shipped. Authoritative what-shipped record: `docs/archive/PHASE_1_PRIVATE_BLOB_DESIGN_2026-06-11.md` § "Implementation notes". ✅ Live smokes passed (raw store + `loadFile` chokepoint; HTTP 403). ✅ **All three server-read consumers promoted to production 2026-06-11** (prod token + `expense-reporter` + `phase-i-dynamics` + `grant-reporting` flags + deploy; grant-reporting prod-verified). The **download proxy** + browser-render consumer (grant-cycle materials) are now code-complete + Codex-reviewed (flag-gated, default public; PARKED — not smoked/promoted (low-risk, legacy-only consumer)) — see `DOWNLOAD_PROXY_DESIGN_2026-06-11.md`.
