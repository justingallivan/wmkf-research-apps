...
|---|---|---|
| Generic uploader creates public Blob artifacts for sensitive document workflows | `docs/security-audit/SECURITY_AUDIT_2026-06-11.md` P2 | Pilot done + smoked (2026-06-11) — `expense-reporter` flag-gated private upload (default public) + server-side read via `lib/utils/uploaded-blob.js`; live smoke passed (private store + HTTP 403). `file-loader.js` now private-aware + `phase-i-dynamics` + `grant-reporting` migrated (2026-06-11), read-path live-smoked via `scripts/smoke-private-file-loader.mjs` (DOCX through `loadFile` private read + extract; URL 403). **All three consumers (`expense-reporter` + `phase-i-dynamics` + `grant-reporting`) PROMOTED TO PRODUCTION 2026-06-11** (prod token + all three flags set + deployed; grant-reporting prod-verified — live upload → private store, URL 403, extraction ran). The browser-render consumer (grant-cycle materials) + its record-scoped download proxy are now code-complete + Codex-reviewed (flag-gated `NEXT_PUBLIC_REVIEWER_FINDER_PRIVATE_CYCLE_MATERIALS`, default public; PARKED — not smoked/promoted (low-risk, legacy-only consumer)) — see `DOWNLOAD_PROXY_DESIGN_2026-06-11.md`. See `PHASE_1_PRIVATE_BLOB_DESIGN_2026-06-11.md` |
...
> **Pilot shipped + smoked 2026-06-11** (`expense-reporter`); **file-loader cohort followed** (`file-loader.js` private read + `phase-i-dynamics` + `grant-reporting`, read-path live-smoked 2026-06-11). The steps below are the original full-slice plan; the pilot that shipped was narrower (server-read-only, no download proxy). The `file-loader.js` private read has since shipped. Authoritative what-shipped record: `docs/security-audit/PHASE_1_PRIVATE_BLOB_DESIGN_2026-06-11.md` § "Implementation notes". ✅ Live smokes passed (raw store + `loadFile` chokepoint; HTTP 403). ✅ **All three server-read consumers promoted to production 2026-06-11** (prod token + `expense-reporter` + `phase-i-dynamics` + `grant-reporting` flags + deploy; grant-reporting prod-verified). The **download proxy** + browser-render consumer (grant-cycle materials) are now code-complete + Codex-reviewed (flag-gated, default public; PARKED — not smoked/promoted (low-risk, legacy-only consumer)) — see `DOWNLOAD_PROXY_DESIGN_2026-06-11.md`.
...
- [x] Phase 1 private Blob proof slice implemented + smoked (2026-06-11, `expense-reporter`) — unit-tested + build green; ✅ live upload→read smoke passed (receipts in private store; URL HTTP 403). ⚠️ production promotion (token + flag + deploy) still pending. Follow-on 2026-06-11: `file-loader.js` private read DONE + `phase-i-dynamics` + `grant-reporting` migrated (flag-gated) + read-path live-smoked (`scripts/smoke-private-file-loader.mjs`: DOCX through `loadFile` private read + extract; URL 403) + **PROMOTED TO PRODUCTION** (prod token + both flags `NEXT_PUBLIC_PHASE_I_DYNAMICS_PRIVATE_BLOB`/`NEXT_PUBLIC_GRANT_REPORTING_PRIVATE_BLOB` set + deployed `dpl_Cd6MGvsGvYgcqW8LHPNV4j7Wg5oA`; grant-reporting prod-verified: live upload → private store, URL 403, extraction ran). `expense-reporter` also promoted (`NEXT_PUBLIC_EXPENSE_REPORTER_PRIVATE_BLOB=true` set in Production + deployed `wmkfresearchapps-njdq4gr5y`; shares the prod-verified read path). All three server-read consumers now private in prod. The download proxy + browser-render consumer (grant-cycle materials) are code-complete + Codex-reviewed (flag-gated `NEXT_PUBLIC_REVIEWER_FINDER_PRIVATE_CYCLE_MATERIALS`, default public); PARKED — not smoked/promoted (low-risk, legacy-only consumer) — see `DOWNLOAD_PROXY_DESIGN_2026-06-11.md`.
