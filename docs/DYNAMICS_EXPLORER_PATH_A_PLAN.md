...
- `describeTable` (`pages/api/dynamics-explorer/chat.js:597-625`) returns **only** the hand-written `TABLE_ANNOTATIONS`. The dated schema-diff run measured **82 annotated of 579 total attributes** for `akoya_request` (`{total:579, interesting:364, annotated:82, missing:283}`; "283 missing" is the gap *within the 364 interesting subset*, not 579−82). Its reproducible raw receipt was disposed in the 2026-07-27 source-only cleanup; rerun `scripts/dynamics-schema-diff.js` for current state. The model is told "Do NOT guess field names" — so any real field a human didn't transcribe is invisible.
