...
|---|---|---|
| `codex/claude-bug-fixes` | Eight committed patches plus current uncommitted WIP in its linked worktree | Active Claude owner; finish and integrate before freeze |
...
`modules/expertise_matching` roster duplicate. It validates the rewrite
mechanics, PR-ref blast radius, and parity checks, but it is no longer a
scope-complete removal specification. Its counts are retained as a planning
baseline only.
...
`modules/expertise_matching` removal/redaction target, then be regenerated
from a fresh mirror after all approved work is integrated and the repository
is frozen.
...
1. Regenerate the private exact path/blob/message specification, including the
   ...
   earlier revision of its identifiable roster/assignment content.
...
6. approval to invalidate and replace all four local worktrees;
