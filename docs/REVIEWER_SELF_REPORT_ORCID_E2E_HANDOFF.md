...
| `pages/api/external/review/[token]/respond.js` + `lib/services/reviewer-acceptance-drain.js` | accept/decline handler and accept drain. `selfReportedOrcidOf(body, suggestion)` = `contactEdits.orcid ?? suggestion.wmkf_reviewerorcid`. On accept, `respond.js` freezes the raw value into `reviewer_acceptance_jobs`; the drain reflects a valid self-report onto the in-memory `reviewer` BEFORE honorarium, then calls the capture after. On decline, `respond.js` still calls the capture before the decline return. All NON-FATAL. |
