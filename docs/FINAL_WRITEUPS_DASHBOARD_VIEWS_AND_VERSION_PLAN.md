...
Built per §4–§8 with no deviation from the plan; merged to `main` via PR #175 as `44bdd240` (Production deployment `dpl_2UrsDnydRqudu95wJyLCK7A6FUFR`, Ready 2026-09-07 PT; all PR checks green including the full Jest run). **Owner-run signed-in Production smoke PASSED 2026-09-07 PT** (PD/superuser identity, read-only): the three views partitioned as expected with the sole D26 Final (Needs my review 0, Reviewed by me 0, All writeups 1; Your writeups section folded into All); the Needs my review empty state offered the other views with counts and switched on click; a bookmarked `view=all` URL reopened on that view, and `?view=bogus&pd=nonsense` opened on Needs my review with the address bar cleaned; the row and focused page showed "Version 5.0" for Request `1002788`; the PD dropdown correctly listed only the owner (the sole PD with a current D26 Final). The "You reviewed version X" copy cannot be exercised until a colleague acknowledges a D26 writeup that is later edited. Follow-ups landed directly on `main` as Tier 0: `dccb39b9` gave the view control the selects' 48px box, and a second commit opted both toolbar selects out of the native menu-button appearance (Safari ignores height and padding on a native `<select>`, so they rendered ~30px) with a drawn chevron, and moved the persona lens label out of the eyebrow slot above the title into the first line of the header's right-hand summary (owner chose this over a chip beside the title after a two-option mockup; the eyebrow pattern had no other instance across the suite). The toolbar select was then extracted to `shared/components/ToolbarSelect.js` (`314e9426`) and adopted by Reviewer follow-up, Request list, and Initial assessments (`e68bffe9`) so all four Workbench views share one control; a suite survey found 54 other native selects in 13 class variants, left for a smaller shared variant. Owner confirmed the aligned toolbars on all four views. Files: `lib/services/final-writeup/acknowledgement-service.js`
