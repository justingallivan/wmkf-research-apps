...
The "they never write the same field" rule has **one** narrow exception: three cached aggregate fields on `akoya_request` that the drain writes at submit time and a status-gated PA flow recomputes on post-submit edits to child `wmkf_proposalbudgetline` rows. Locked 2026-05-14 (Connor sync, Item 6). See `docs/INTAKE_PORTAL_BUDGET_AGGREGATE_RECOMPUTE_DECISION.md` § 0 for the full decision record.
...
> **Superseded 2026-05-18 (S163) — "Delete" in the "PA flow writes" column is obsolete.** Per Connor's S162 ruling (see `INTAKE_PORTAL_BUDGET_AGGREGATE_RECOMPUTE_DECISION.md` § 0 "Update 2026-05-18"), defunct child rows are **deactivated, not deleted**: the recompute fires on post-submit Create / **Update** (incl. `statecode`→Inactive) and sums **active children only**. There is no Delete trigger. Read the three "Create / Update / Delete" cells above as "Create / Update (incl. deactivation)."
...
**Failure fallback:** if P1-Update ultimately fails maker-portal validation, the exception is not operational and the path falls back to Option B alone (drain uses `$batch` change sets; PA never recomputes) — the slice-0 schema remains correct regardless. See `docs/INTAKE_PORTAL_BUDGET_AGGREGATE_RECOMPUTE_DECISION.md` § 5 Option B and § 0 "Update 2026-05-18 (S163)" for full reasoning + the corrected Option B drain lifecycle (deactivate, not "delete old / insert new").
...
| Scenario | Behavior |
|---|---|
| Scanner returns "infected" | **Delete the Blob immediately.** Write an audit row with `{attachmentId, filename, sha256, size, scanner, virusName, scannedAt, draftId, contactOid}`. Remove the pending entry from the draft. **Fire a `system_alerts` row + admin email** via `NotificationService.notify({ category: 'virus-detection', severity: 'error', ... })` — drafts are pre-submission so request-level PD routing isn't applicable; recipients resolve against the `virus-detection` category configured in `/admin → Alert Recipients` (admin must set this, falling back to the superuser roster if unconfigured). Return 422; the file-by-file three-call design means the rest of the applicant's draft (form state + other clean attachments) is preserved automatically — UI surfaces a banner emphasizing this: "We detected a virus in `filename`. Your form has been preserved — replace just this file with a clean copy (please run an antivirus scan on your computer first) and try again. The Foundation has been notified." The audit row is the durable evidence; the bytes are a liability with no audit value beyond what the metadata captures. **Server side wired S190**: `pages/api/intake/draft/attach.js`. |
