...
| `pre-site-visit/proposal-core-service.js` / `docx-renderer.js` | none | read-only `akoya_request`, applicant `account`, Co-PI junction, SharePoint `akoya_request`; governed execution writes append-only `wmkf_ai_run` only | **[VERIFIED LOCALLY 2026-08-17; HISTORICAL SINGLE-SOURCE LIVE EVIDENCE 2026-08-16]** integration producer requires exact separate `AI Materials/ProposalNarrative_{Request#}.pdf` and `ProposalBibliography_{Request#}.pdf`, passes separately bounded Claude inputs, and returns both identities/versions/hashes. Request `1002379` previously proved only the narrative source and prompt v3; no live bibliography probe or two-input run exists. Sole-current v3 is single-source, so a new immutable prompt version is required before promotion. The route returns DOCX bytes only: no SharePoint upload, registry row, pointer, or business-field write. |
