...
| `wmkf_requestdocument` | 4 (2026-08-17 inventory) | **LIVE PRODUCTION SCHEMA + CONTROLLED INITIAL-ASSESSMENT AND PRE-SITE PROOF.** Wave 19 adds the Pre-Site section/snapshot fields and current Pre-Site/Final pointers. Read-only post-generation inventory reports three Initial Assessments and one Pre Site Visit; all four are Ready/Draft. **[VERIFIED IN PRODUCTION 2026-08-17]** Request `1002379` row `aeb223a2-849a-f111-b8db-70a8a59cded0` uses only the exact Proposal Narrative plus Dataverse, owns stable Word item `01G4GVMS3Q5BJ65S7DDZDKFTSQLIQAIPER`, and is the request's current Pre-Site pointer. Exact retry reused the same row/run/item. Initial Assessment mechanics, lineage, recovery, editing, and current Graph metadata retain the production evidence described on the entity Atlas page. | [dataverse-wmkf-requestdocument.md](atlas/dataverse-wmkf-requestdocument.md) |
...
| Dynamics Explorer state | `dynamics_explorer_requests`, `dynamics_query_log`, `dynamics_feedback`, `dynamics_user_roles`, `dynamics_restrictions` — migration 033 source is complete; Production application/readback remains open | same |
...
| `llm-client.js` / `model-capabilities.js` | `api_usage_log` (direct best-effort write via `usage-logger.js` when `appName` supplied; migration 032 added nullable provider `stop_reason` to Production and was schema/tracker-read-back on 2026-08-21; signed-in Explorer smoke rows 5354/5355 then proved non-null `tool_use`/`end_turn` persistence; migration 033 source adds nullable Explorer request/round correlation, not yet Production-applied) | none | canonical Anthropic wrapper plus reviewed model capability registry for request shaping, structured-output eligibility, limits, refusal semantics, and per-call completion metadata |
| `dynamics-explorer-request-telemetry.js` | `dynamics_explorer_requests` (R/W; migration 033 source, not yet Production-applied) | none | awaited fail-soft request start/finalization; atomic running→terminal compare-and-set with terminal-row recovery; stores bounded metadata only |
