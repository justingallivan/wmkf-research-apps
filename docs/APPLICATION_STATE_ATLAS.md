...
| Review synthesis lifecycle | `review_synthesis_jobs` (generation queue/currentness ledger; no review text; Dataverse request memo remains content source) — **[VERIFIED 2026-07-28 via production schema probe] migration 028 applied; 0 rows before deployment** | [postgres-infra-tables.md](atlas/postgres-infra-tables.md) |
...
| `review-synthesis-job-service.js` / `review-synthesis-drain.js` | `review_synthesis_jobs` (R/W) | `wmkf_appreviewersuggestion`, `wmkf_appreviewanswer`, `akoya_request`, and `wmkf_ai_run` through adapters/Executor | Table live and empty; branch code not deployed as of 2026-07-28. Exact input fingerprint dedupes automatic work; leased claims revalidate readiness and fingerprint; the table stores state/hashes only, never reviewer content. Dataverse `wmkf_reviewsynthesisjson` remains the synthesis source of truth. |
