...
| `wmkf_finalwriteupreviewacknowledgement` | 0 | **[PRODUCTION RUNTIME LIVE; FIRST-ROW RETRY OPEN 2026-08-31.]** Wave 23 metadata is 11 exact / 0 absent / 0 divergent / 0 pending and the Final-document + reviewer alternate-key index is Active. Production readiness is exact `on` in Ready deployment `dpl_B9k3AprnYp5ExpkqpT3dUxCUZqWo`; Preview remains unset. Signed-in dashboard and Request `1002788` Final reads showed zero reviews, retained Word access, and correctly omitted responsible-PD self-review. An eligible colleague's first POST then failed with missing acknowledgement Create; the no-fallback path reread and persisted no partial row. The dedicated reviewer role is now directly assigned and its six requested Global privileges are effective for all 11 confirmed audience members. Dataverse attached nine standard App Opener baseline privileges when creating the role; none grants Delete, Assign, Share, or Request Document write. | [dataverse-wmkf-finalwriteupreviewacknowledgement.md](atlas/dataverse-wmkf-finalwriteupreviewacknowledgement.md) |
...
`grant-cycle.js`, `grant-request.js`,
...
| `final-writeup/acknowledgement-service.js` | none | `akoya_request`, `wmkf_requestdocument`, `systemusers`, and `wmkf_finalwriteupreviewacknowledgements`; Microsoft Graph separately | **[PRODUCTION-LIVE; FIRST-ROW RETRY OPEN 2026-08-31.]** Fail-closed current-Final/reviewer resolution, one publication observation, responsible-PD exclusion, same-version no-restamp, conditional later-version replacement, and ambiguous-write reread. Production readiness is exact `on` in Ready deployment `dpl_B9k3AprnYp5ExpkqpT3dUxCUZqWo`; signed-in Request `1002788` readback showed zero reviews and correct responsible-PD exclusion. An eligible colleague's missing-Create failure left no partial row; the remediated role is effective for all 11 audience members and their retry remains open. |
| `final-writeup/dashboard-service.js` | none | `akoya_request`, `wmkf_requestdocument`, `systemusers`, and `wmkf_finalwriteupreviewacknowledgements`; Microsoft Graph separately | **[PRODUCTION-LIVE; SIGNED-IN READ SMOKE PASSED 2026-08-31.]** Read-only, fail-closed ordinary staff projection over at most 100 requests with current-Final pointers. Exact Final rows and acknowledgement rows are batch-read in bounded chunks; stable Graph identities are deduplicated and refreshed with concurrency four. The service derives stage, lead-PD relationship, personal state, positive initials, open/history/stewardship queues, and optional focused navigation. It contains no PC/leadership persona inference, complete matrix, or supporting-material data reader. The Production dashboard/focused Final loaded under the signed-in owner session and the error scan was empty. |
