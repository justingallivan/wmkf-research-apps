...
| `wmkf_requestdocument` | 4 (2026-08-17 snapshot; re-probe for current count) | **LIVE PRODUCTION SCHEMA + CONTROLLED INITIAL-ASSESSMENT AND PRE-SITE PROOF.** Wave 19 adds the Pre-Site section/snapshot fields and current Pre-Site/Final pointers. The 2026-08-17 read-only post-generation snapshot contained three Initial Assessments and one Pre Site Visit, all Ready/Draft. That day, Request `1002379` row `aeb223a2-849a-f111-b8db-70a8a59cded0` used only the exact Proposal Narrative plus Dataverse, owned stable Word item `01G4GVMS3Q5BJ65S7DDZDKFTSQLIQAIPER`, and became the request's current Pre-Site pointer; exact retry reused the same row/run/item. **[PRODUCTION-PROVED 2026-08-21]** the current pointer later completed the signed-in Site Visit handoff to Ready/Review with a complete milestone, same-item Edit/Download readback, and regeneration lock. The browser proof did not expose the current row GUID or refresh the aggregate row count; re-probe before relying on either. **[PRODUCTION-DEPLOYED 2026-08-23; DURABLE REOPEN SMOKE OPEN]** Wave 20 typed metadata is 3 exact/0 divergent, the Production readiness flag is literal `on`, and guarded reopen is Ready in deployment `dpl_BbtmRghhSYa7EPiQkWxsmdkgRozp`. Signed-in Request `1002788` exercised the extended read-only status path without writes; no guarded-reopen business-row mutation has run. Failed/expired attempts do not block a distinct operation, retained copies become stable-identity cleanup work at claim expiry or distinct-operation supersession, live competing generation blocks reopen, stale generation activation must match the current correction cycle, actor attribution is reason-event-only, and correction audit details plus pending reopen attempts are withheld from non-superusers. | [dataverse-wmkf-requestdocument.md](atlas/dataverse-wmkf-requestdocument.md) |
...
| `pre-site-visit/proposal-core-service.js` / `artifact-service.js` / `docx-renderer.js` / `reopen-service.js` | none | reads `akoya_request`, applicant `account` (`akoya_aka`, `name`, city/state), Co-PI junction, exact Proposal Narrative, governed prompt, and Pre-Site registry/pointer state; `DV:InstitutionName` is AKA-first with account-name then formatted-lookup fallback and excludes Request Bill.com organization fields; POST writes `wmkf_ai_run`, `wmkf_requestdocument`, `akoya_request.wmkf_CurrentPreSiteVisit`, and one SharePoint Word item when generation succeeds; GET status writes nothing; the deployed reopen service reads the same registry/pointer plus exact handoff bytes and writes one successor row/item under literal-on Wave 20 readiness | **[VERIFIED IN PRODUCTION 2026-08-17.]** The durable writer requires the exact narrative identity before side effects, excludes the bibliography from prompt and generation identity, persists eight named fields plus immutable input/output snapshots, renders from Dataverse readback, uploads to `Artifacts/Pre-Site Visit/`, and atomically activates one current Ready Word row. Request `1002379` proved that flow and exact Ready retry. **[DEPLOYED TO PRODUCTION 2026-08-18; SIGNED-IN GENERATION SMOKE OPEN]** the resilience release uses 30,000-character sink ceilings, normalized envelope v3 plus v2 reads, durable edit-check warnings, strict prompt preflight, deterministic-failure retry blocking, typed support references, and render contract v5 over unchanged v4 bytes. Editorial deviations can reach Ready; empty/reserved/unreconciled content and authority/source/lineage/template/upload failures remain blocking. Sole-current prompt v4 and Ready deployment `dpl_HGogbJnprevoYKLaxevamxdajtC4` were exact-readback verified without generating a request artifact. **[DEPLOYED TO PRODUCTION 2026-08-18; SIGNED-IN GENERATION SMOKE OPEN]** AKA-first institution binding is covered by focused source tests and a rendered DOCX fixture. **[DEPLOYED TO PRODUCTION 2026-08-17; SIGNED-IN FEATURE SMOKE OPEN]** read-only current/pending status plus bounded GET polling recover a lost POST response without another generation request. Production template v2 added Recommendation-cell padding and generated a distinct Ready artifact; its Word Online render exposed a width-sensitive Word Online alignment defect. **[INFERRED FROM SCREENSHOT + OOXML WIDTH]** Implicit wrapping was the remaining layout variable. **[DEPLOYED TO PRODUCTION 2026-08-17; SIGNED-IN FEATURE SMOKE OPEN]** template v3 makes that label explicitly non-wrapping under a new generation identity. **[DEPLOYED TO PRODUCTION 2026-08-18; SIGNED-IN GENERATION SMOKE OPEN]** template v4 retains no-wrap and changes only the first-page metadata spacing to one blank line after Project Title, three compact single-spaced rows, and one blank line ending at the divider; v4 has a distinct generation identity. **[PRODUCTION-DEPLOYED 2026-08-23; DURABLE REOPEN SMOKE OPEN]** guarded reopen adds a superuser-only preserve-and-succeed path, append-only successor attempt audit with explicit outcomes, exact byte copy, final source/target identity rereads, ETag pointer transition, exact retry/recovery, failed/expired-attempt fallthrough with stable-identity cleanup tracking at expiry or distinct-operation supersession, live-lease competing-generation exclusion, final correction-cycle equality, reason-gated actor attribution, response-boundary audit redaction including pending-reopen omission, and a schema-readiness interlock across reads and creates. Wave 20 is 3 exact/0 divergent, the Production flag is literal `on`, deployment `dpl_BbtmRghhSYa7EPiQkWxsmdkgRozp` is Ready, and signed-in Request `1002788` passed the read-only extended-status smoke; no reopen business-row mutation has run. |
