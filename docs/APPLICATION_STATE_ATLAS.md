...
| `wmkf_ai_prompt` | probe required | staff-editable prompt rows for Executor; includes governed `initial-assessment.generate` v1, sole-current `pre-site-visit.proposal-core.generate` v5, and sole-current `review-synthesis.generate` v5 (six-key schema; representative quotation generation suspended 2026-09-16) | same page |
| `wmkf_appsystemsetting` | probe required | Shared Dataverse settings store. **[PRODUCTION-DEPLOYED 2026-08-30; REVISION v1 PUBLISHED 2026-09-07]** Executor standing/retry/timeout-only budgets use append-only keys `executor.budgets.vNNNNNN`; the highest valid revision is the mutable source, while code owns strict bounds and the outage fallback, and fills registered names missing from an older revision. `executor.budgets.v000001` (2026-09-07T20:33:23Z, Justin Gallivan) pins the reviewed code defaults for all three registered prompts; runtime and Admin reads both return `source: dataverse` at version 1 with zero storage warnings. **[PRODUCTION-LIVE V2; PERSONA CONSUMER ENABLED 2026-09-03 PT]** `final_writeup.matrix_audiences` stores explicit GUID-only persona/no-lens assignments plus broad Grant Program reviewer audiences under one ETag. The dry-run-first operator command upgraded once from `W/"96930393"`; exact readback proved v2 at `W/"96944113"`, all 11 current reviewer-role assignments, zero stale/unassigned rows, and unchanged nine-person Research/six-person Southern California audiences. A same-day preflight re-proved 11/11 assignments with zero stale/unassigned references before commit `213f6c34` enabled the consumer in Ready deployment `dpl_HGrbWUNPJMJunVevYLVEmtn7He6a`. Commit `84bf465b` remains the v2-capable rollback floor. Live Dataverse names and current reviewer-role/program eligibility remain authoritative. | [dataverse-wmkf-ai-run-and-prompt.md](atlas/dataverse-wmkf-ai-run-and-prompt.md) |
...
| `wmkf_deliberationsession` | Production row census not probed | **[PRODUCTION-LIVE; REVERIFIED 2026-09-15.]** Wave 28 is 22 exact / 0 absent / 0 divergent in Production. Grant-protected session CRUD, explicit actor attribution, reference-only attendees, and the Meeting Tracker pages are live behind exact-on readiness. | [dataverse-wmkf-deliberationsession.md](atlas/dataverse-wmkf-deliberationsession.md) |
| `wmkf_deliberationslot` | Production row census not probed | **[PRODUCTION-LIVE; REVERIFIED 2026-09-15.]** Wave 28 is 22 exact / 0 absent / 0 divergent in Production. ETag-fenced add/edit/move/delete operations, atomic full-order changesets, the ordered session UI, and the fail-open batched schedule reader are live behind exact-on readiness. | [dataverse-wmkf-deliberationslot.md](atlas/dataverse-wmkf-deliberationslot.md) |
...
| Meeting Tracker session agenda | `deliberation_agenda_sends` (exact email snapshot, send lease, Dynamics activity/transport receipt, one unresolved send per session, terminal failed-attempt audit) — **[DEPLOYED; migration 041 tracked in Production 2026-09-15; transport not independently production-smoked]** | [postgres-infra-tables.md](atlas/postgres-infra-tables.md) |
| Deliberation briefing links | `deliberation_briefing_links` (expiring, revocable per-request link to the read-only briefing page; token digest + sealed token, never the raw token) + `pre_site_distribution_attempts.briefing_link_id` — **[PLANNED: migration 038 in source on `feature/deliberation-briefing-page`; owner applies it and sets `DELIBERATION_BRIEFING_SCHEMA_READY=on`]** | [postgres-infra-tables.md](atlas/postgres-infra-tables.md) |
| Consultant Feedback | `consultant_feedback` (staff-recorded informal consultant feedback on a proposal, attributed to a roster consultant or a one-off name stored on the entry, shared on the briefing page by default; optional PDF/DOCX attachment bound through `requestdocument_id`) — **[PRODUCTION-LIVE 2026-09-14: Slices 1–3 shipped via PRs #293, #295, and #299; migrations 048 and 049 applied; PR #300 repaired append-only filename collisions with Graph `rename`; signed-in production smoke proved create, upload/finalize, staff open, and delete on request 1003222]** | [postgres-infra-tables.md](atlas/postgres-infra-tables.md) |
...
| Reviewer institution measurement | `reviewer_institution_measurement_events` — **[VERIFIED LIVE 2026-09-15: migration 051 applied to the shared Production/Preview database; exact schema readback passed; zero rows; exact-on flag absent/off]** pseudonymous, non-authoritative Find roster/staff/save observations with 90-day retention and a row cap | [postgres-reviewer-institution-measurement-events.md](atlas/postgres-reviewer-institution-measurement-events.md) |
| Cycle Dossier pilot | `cycle_dossiers`, `cycle_dossier_previews`, `cycle_dossier_entries`, `cycle_dossier_runs`, `cycle_dossier_control`, `cycle_dossier_editions` — **[VERIFIED 2026-09-15: canonical migration 045 is tracked; its byte-identical obsolete tracker alias `038_cycle_dossiers.sql` was removed after guarded readback]** private D26 selections, bounded previews, shared immutable request revisions, one global run lease with independently checkpointed items, a durable singleton operator stop, and owner-private editions; the dedicated `wmkf-cycle-dossier-private` Blob store (`store_W9WC1TLR7kpl9hty`) is connected under the custom `DOSSIER_BLOB` prefix, while generation, SharePoint publication, deployment, and promotion remain pending. | [postgres-cycle-dossiers.md](atlas/postgres-cycle-dossiers.md) |
| Virtual Review Panel Phase A foundation | `review_panels`, `review_panel_runs`, `review_panel_entries`, `review_panel_seat_attempts`, `review_panel_control` — **[VERIFIED LIVE 2026-09-15: migration 047 applied; four Production smoke panels completed 2026-09-13; Production rollout is now `access`]** private per-owner selections, one global run lease with a per-seat/chair paid-call ledger (two lease fences: run-state changes under the run's current lease, attempt finalization a CAS on the attempt's own dispatch token), a durable singleton operator stop, and a route→worker retry hand-off via `retry_requested_at`. The dedicated Blob store for per-entry DOCX/PDF editions (D11, `REVIEW_PANEL_BLOB_READ_WRITE_TOKEN`/`REVIEW_PANEL_BLOB_STORE_ID`) is `wmkf-review-panel-private` (`store_cwVLLxRMR3A8NFA2`), connected in Production and Preview 2026-09-13. The per-minute drain cron remains deployed; access is governed by superuser or the `review-panel` app grant rather than the old four-request allowlist. | [postgres-review-panel.md](atlas/postgres-review-panel.md) |
...
| `final-writeup/dashboard-service.js` | none | `akoya_request` including broad `wmkf_grantprogram`, `wmkf_requestdocument`, `systemusers`, current reviewer-role membership, `wmkf_appsystemsettings`, and `wmkf_finalwriteupreviewacknowledgements`; Microsoft Graph separately | **[PERSONA LENSES PRODUCTION-LIVE 2026-09-03 PT.]** Commit `213f6c34` in Ready deployment `dpl_HGrbWUNPJMJunVevYLVEmtn7He6a` enables explicit v2 PD/PC/Leadership unions. The matrix remains grouped by broad Grant Program and is included for superusers or configured PCs on the index only. A read-only production-data smoke verified PD, PC, Leadership, overlap, ineligible/unassigned, and superuser projections; stale stored identities only narrow runtime and surface warnings. **[OWNER-REPORTED 2026-09-04]** Program Coordinator Duncan Spore found Request `1002788` in History, saw the review-status matrix, and opened the Word document successfully. |
...
| `executor-budget-service.js` | none | `wmkf_appsystemsettings` (append-only `executor.budgets.vNNNNNN` rows) + `wmkf_ai_prompt` (current model reads at publication) | **[SOURCE-VERIFIED 2026-08-29.]** Superuser publication is create-only, version/idempotency guarded, model-ceiling checked, and reread-verified. Pre-Site, review synthesis, and the field primer (timeout-only, S493) consume the latest valid revision server-side; a settings failure uses bounded code fallback. |
...
| `execute-prompt.js` | none — dispatches by the reviewed model's `provider`: Anthropic through `llm-client.js` (default), OpenAI through `openai-client.js` only when the caller opts in via `allowedProviders` (2026-09-12); rejects unreviewed concrete prompt-row model ids, preserves joined text/stop metadata, and requires `end_turn` before persistence | `wmkf_ai_prompts` (read in `fetchCurrentPrompt()`), `akoya_requests` (read once up-front for the skip-if-populated guard in `preflightGuards()`; **coalesced write to the prompt's declared `target.field` via `persistOutputs()` → `DynamicsService.updateRecord`**), `wmkf_ai_runs` (one audit attempt per invocation in `writeRunRow()` with FKs to prompt + request) | Executor contract; **dynamically writes to `akoya_request` flat fields** (e.g. `wmkf_ai_summary`) only after terminal-response, parse, and local-schema validation. Prompt-level `generationMode:native-json-schema` is capability-gated. |
...
| `pre-site-visit/distribution-service.js` / `distribution-store.js` | `pre_site_distribution_attempts` (R/W; migrations 034–035 live with base and calendar/material `sent` proofs; migration 052 **[VERIFIED LIVE 2026-09-16 — migration 052 applied to the shared Production/Preview database by the owner via `node scripts/apply-migrations.js` (tracker `applied_at` 2026-09-17T03:59:31Z); readback exact: five nullable columns, both CHECK constraints, 14 pre-existing attempt rows all satisfy the constraints]**) | reads current `akoya_request`/`wmkf_requestdocument` and optional `wmkf_sitevisit`; **[BUILT on `claude/pre-rp-brief` — not merged, not deployed]** the distribution source is the current Pre-Research Presentation Brief (`_wmkf_currentprerpbrief_value`, artifact type `100000009`), no longer the Pre-Site writeup, and prepare fails closed on zero received reviews (`brief_reviews_required`) or unacknowledged input drift (`brief_inputs_stale`); creates retained snapshot rows and granular Dynamics email/attachment/send activity | **[PRODUCTION-PROVED BASE AND CALENDAR/MATERIAL EXTENSION 2026-08-25.]** Request `1002379`, PDF-only operation `85f52fc5-fb48-4ceb-84d6-0f246af0b6fb`, retained Ready/Board Ready DOCX and PDF rows and sent the selected 133,265-byte PDF to `jgallivan@wmkeck.org`. The ledger reached `sent`; Dynamics activity `33ce6346-d89f-f111-b8db-6045bd07a06d` read back Sent with actor attribution and exactly one attachment matching SHA-256 `574ac7b833801866c370a8056b7197933addfe3ea5dd535dcf4d29803c18f0c9`. Migration 035 adds exact material-link and informational-calendar snapshots, hashes, Site Visit ETag fencing, and a separate calendar-attachment receipt. Calendar/material operation `f497643a-2e9e-4032-a323-1e40874d16f1` reached `sent` with one material, the saved Site Visit, and no final error. Both receipts prove Dynamics transport acceptance; inbox/calendar-client delivery is unverified. |
| `meeting-tracker/agenda-service.js` / `agenda-store.js` | `deliberation_agenda_sends` (R/W; migration 041 applied/tracked) | reads the live deliberation session/slots and briefing links; creates or recovers one Dynamics email activity, issues SendEmail, and stores the exact snapshot plus transport state | **[DEPLOYED + UNIT-TESTED; migration verified tracked 2026-09-15; transport NOT LIVE-PROVED.]** GET reports the latest sent receipt and latest unresolved send independently. Prepare and send refuse a competing operation; a partial unique index is the concurrency backstop. Retry reuses the original Dynamics activity: accepted status closes the receipt, confirmed Draft may resume transport for the same sender, known closed status becomes terminal `failed`, and unknown/unreadable status remains unresolved and never sends. `sent` proves Dynamics transport acceptance only. |
| `deliberation-briefing/briefing-link-service.js` / `briefing-link-store.js` / `briefing-page-service.js` | `deliberation_briefing_links` (R/W; migration 038 **[PLANNED, not applied]**); `pre_site_distribution_attempts` (R: latest `sent` row for the pinned brief snapshot identity and, **[BUILT on `claude/pre-rp-brief` — not merged, not deployed]**, `stale_inputs_acknowledged_at` for the timestamp-only Board notice; W: `briefing_link_id` at prepare) | reads `akoya_request`, `account`, active `wmkf_sitevisit`, selected `wmkf_appreviewersuggestion` rows with a received review plus their `wmkf_appreviewanswer` snapshot; streams frozen snapshot items, review uploads, and the `AI Materials` narrative through Graph | **[SOURCE-BUILT 2026-09-09 (S502), flag-gated off until the owner enables it.]** `docs/DELIBERATION_BRIEFING_PAGE_PLAN.md`. Share's exact preview mints/reuses the live link and binds its id into `previewHash`; send refuses a replaced link. External routes verify the token digest against the row on every request. |
...
| `site-visit/logistics-service.js` / `recipient-directory-service.js` | `user_profiles` and `expertise_roster` (read); linked roster rows additionally read active Dataverse `contacts` | reads/writes one request-bound `wmkf_sitevisit`; reads `systemusers`, the request's status/triage/meeting date (write gate), current Pre-Site state (read path, materials only), and governed material rows | **[PRODUCTION-PROVED 2026-08-25; WRITE GATE CHANGED 2026-09-09; CONTACT-LINK RESOLUTION PRODUCTION-LIVE 2026-09-14, migration 050 applied.]** Writes require an advancing request in a cycle with a meeting date (`isVisibleRequestRow`; owner 2026-09-09, PC scheduling precedes sharing) — no longer Ready/Review Pre-Site state; reads are not gated on schedulability. Also literal schema readiness, server-resolved organizer/attendees, one active Site Visit, and ETag concurrency. Signed-in Request `1002379` save/readback confirmed one active request-bound row, persisted logistics fields, and five parties. Unchanged parties use parent PATCH; changed party roles use a sandbox-proved atomic delete/create of the same Activity GUID because direct ActivityParty writes are unsupported. External attendee identity remains `expertise_roster.id`; unlinked rows use manual `preferred_email`, while linked rows use only the active Contact's current primary email and fail visibly without fallback when that Contact is unavailable. |
...
| `expertise_roster.dataverse_contact_id` | `contacts.contactid` | optional reference for live Board/Consultant email resolution; roster `id` remains attendee identity |
