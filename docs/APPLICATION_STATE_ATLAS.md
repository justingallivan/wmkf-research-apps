...
| `wmkf_finalwriteupreviewacknowledgement` | 0 | **[PRODUCTION SCHEMA LIVE; ROUTE/UI SOURCE-BUILT, NOT DEPLOYED 2026-08-31.]** Wave 23 metadata is 11 exact / 0 absent / 0 divergent / 0 pending and the Final-document + reviewer alternate-key index is Active. The typed adapter, distinct fail-closed readiness guard, backend service, authenticated route, Final-tab consumer, and focused tests exist in source. Production readiness remains unset, so no runtime acknowledgement read/write or row exists. | [dataverse-wmkf-finalwriteupreviewacknowledgement.md](atlas/dataverse-wmkf-finalwriteupreviewacknowledgement.md) |
...
| `final-writeup/acknowledgement-service.js` | none | `akoya_request`, `wmkf_requestdocument`, `systemusers`, and `wmkf_finalwriteupreviewacknowledgements`; Microsoft Graph separately | **[ROUTE/UI SOURCE-BUILT, NOT DEPLOYED 2026-08-31.]** Fail-closed current-Final/reviewer resolution, one publication observation, responsible-PD exclusion, same-version no-restamp, conditional later-version replacement, and ambiguous-write reread. The authenticated route and Final-tab consumer exist; the live readiness value remains unset, so Production runtime access is still off. |
| `final-writeup/dashboard-service.js` | none | `akoya_request`, `wmkf_requestdocument`, `systemusers`, and `wmkf_finalwriteupreviewacknowledgements`; Microsoft Graph separately | **[SOURCE-BUILT, NOT DEPLOYED 2026-08-31.]** Read-only, fail-closed ordinary staff projection over at most 100 requests with current-Final pointers. Exact Final rows and acknowledgement rows are batch-read in bounded chunks; stable Graph identities are deduplicated and refreshed with concurrency four. The service derives stage, lead-PD relationship, personal state, positive initials, open/history/stewardship queues, and optional focused navigation. It contains no PC/leadership persona inference, complete matrix, or supporting-material data reader. The shared acknowledgement readiness value remains unset, so Production access is off. |
