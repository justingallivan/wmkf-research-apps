...
| `wmkf_requestdocument` | 0 | **LIVE PRODUCTION SCHEMA + APPLICATION (2026-07-30)** governed request-artifact registry for stable SharePoint identity, lifecycle, operation state, generation provenance, and operator-visible SharePoint cleanup storage (bounded primary queue plus durable exact-identity overflow that blocks further generation); paired with live `akoya_request.wmkf_CurrentInitialAssessment` canonical pointer/shared ETag fence. Verified through Wave 16 apply/readback, fresh zero-row probes, and Ready production deployment `dpl_AxxroabhpXLX1pz75MW6486fB4ci`. The controlled artifact pilot remains pending. | [dataverse-wmkf-requestdocument.md](atlas/dataverse-wmkf-requestdocument.md) |
...
| `initial-assessment/artifact-service.js` | none | `akoya_request`, `wmkf_requestdocument`, `wmkf_ai_prompt`, `wmkf_ai_run`, and SharePoint `akoya_request` | governed Initial Assessment producer/read model; exact retry convergence, Ready-row no-overwrite, atomic request-pointer/Ready/supersession activation, post-upload registry recovery, operator-visible retained-item cleanup work without silent eviction (durable overflow blocks further generation), and stable Graph identity. Production schema, governed prompt v1, and application are live and verified; the controlled pilot remains gated. Cleanup is manual (no drain). |
...
- **`wmkf_ai_prompt` and `wmkf_ai_run`**: per-entity page at [`atlas/dataverse-wmkf-ai-run-and-prompt.md`](atlas/dataverse-wmkf-ai-run-and-prompt.md). Both schemas documented from live code in `execute-prompt.js` — prompt read in `fetchCurrentPrompt()`, run-row write (with prompt + request FKs) in `writeRunRow()`.
