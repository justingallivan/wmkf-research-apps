...
| `wmkf_requestdocument` | 10 (2026-08-23 post-reopen probe: 3 Initial Assessments/7 Pre Site Visits; 9 Ready/1 Failed; 6 Draft/4 Superseded) | **LIVE PRODUCTION SCHEMA + CONTROLLED INITIAL-ASSESSMENT AND PRE-SITE PROOF.** The Production row count remains the 2026-08-23 snapshot. **[PRODUCTION-DEPLOYED + SIGNED-IN READ-SMOKED 2026-08-30; WRITE PATHS NOT PRODUCTION-EXERCISED]** Initial Assessment restore ETag-refreshes the canonical row after native Graph restore, while Board freeze uses existing source/version/hash, producer, lifecycle, generation, file-identity, and actor fields to create a distinct Ready/Board Ready row/item without schema change or pointer movement. Request `1003109` loaded the canonical artifact, snapshot control, and native versions `2.0` / `1.0`; no restore or snapshot write ran. Exact snapshot rows are excluded from editable activation/supersession/cycle discovery. Wave 19 adds the Pre-Site section/snapshot fields and current Pre-Site/Final pointers. Request `1002379` originally proved Ready generation/exact retry, then the 2026-08-21 signed-in Ready/Review Site Visit handoff. **[PRODUCTION-PROVED 2026-08-23]** Wave 20 is 3 exact/0 divergent, the readiness flag is literal `on`, and guarded reopen is Ready in deployment `dpl_BbtmRghhSYa7EPiQkWxsmdkgRozp`. After exact owner approval, signed-in Request `1002379` preserved and superseded source `76a0d4b2-8b9a-f111-b8db-7ced8d3d15a6`, created current Ready/Draft successor `888982b6-0a9f-f111-b8dc-7ced8d3d15a6` and one distinct SharePoint copy, and left no Final pointer or cleanup work. **[PRODUCTION SCHEMA VERIFIED 2026-08-30; RUNTIME NOT DEPLOYED]** Slice 1's Wave 22 transition-attribution fields are 4 exact / 0 divergent in Production, and the source branch contains the literal-on-gated same-item Final writer/read model. The schema apply created no Request Document rows, did not change this count, and did not enable or exercise the runtime. | [dataverse-wmkf-requestdocument.md](atlas/dataverse-wmkf-requestdocument.md) |
