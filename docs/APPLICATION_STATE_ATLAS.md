...
| `wmkf_appsystemsetting` | probe required | Shared Dataverse settings store. **[SOURCE-VERIFIED 2026-08-29; durable revision not production-probed]** Executor standing/retry budgets use append-only keys `executor.budgets.vNNNNNN`; the highest valid revision is the mutable source, while code owns strict bounds and the outage fallback. | [dataverse-wmkf-ai-run-and-prompt.md](atlas/dataverse-wmkf-ai-run-and-prompt.md) |
