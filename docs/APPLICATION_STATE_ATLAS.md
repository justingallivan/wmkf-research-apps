...
| Consultant Feedback | `consultant_feedback` (staff-recorded informal consultant feedback on a proposal, attributed to a roster consultant or a one-off name stored on the entry, shared on the briefing page by default; optional PDF/DOCX attachment bound through `requestdocument_id`) — **[VERIFIED 2026-09-14: Slice 1 production-live via PR #293 with migration 048 applied; Slice 2 attachment lifecycle merged to main via PR #295 (`bf6b41be`) with migration 049 not yet applied to production; Slice 3 staff attachment access and list/chooser polish source-built and fresh-review approved on `codex/consultant-feedback-slice-3`, awaiting owner merge]** | [postgres-infra-tables.md](atlas/postgres-infra-tables.md) |
