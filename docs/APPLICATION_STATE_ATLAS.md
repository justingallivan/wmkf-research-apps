...
| `wmkf_sitevisit` | 0 (Production and sandbox pre-release probe 2026-08-24) | Existing custom Activity now owns Site Visit schedule, format, IANA zone, location/link, organizer/attendee ActivityParty rows, and a server-owned immutable-reference map. Wave 21 is exact in sandbox and Production; reversible sandbox create/replace/delete proof passed. Production runtime is deployed; signed-in save proof remains open. | [dataverse-wmkf-sitevisit.md](atlas/dataverse-wmkf-sitevisit.md) |
| `wmkf_requestdocument` | 10 (2026-08-23 post-reopen probe: 3 Initial Assessments/7 Pre Site Visits; 9 Ready/1 Failed; 6 Draft/4 Superseded) | **LIVE PRODUCTION SCHEMA + CONTROLLED INITIAL-ASSESSMENT AND PRE-SITE PROOF.** Wave 19 adds the Pre-Site section/snapshot fields and current Pre-Site/Final pointers. Request `1002379` originally proved Ready generation/exact retry, then the 2026-08-21 signed-in Ready/Review Site Visit handoff. **[PRODUCTION-PROVED 2026-08-23]** Wave 20 is 3 exact/0 divergent, the readiness flag is literal `on`, and guarded reopen is Ready in deployment `dpl_BbtmRghhSYa7EPiQkWxsmdkgRozp`. After exact owner approval, signed-in Request `1002379` preserved and superseded source `76a0d4b2-8b9a-f111-b8db-7ced8d3d15a6`, created current Ready/Draft successor `888982b6-0a9f-f111-b8dc-7ced8d3d15a6` and one distinct SharePoint copy, and left no Final pointer or cleanup work. Exact unchanged retry returned the same row/item with `reused: true`; Dataverse/Graph postcheck found one cycle row, stable source/target reads, and identical governed bytes. Failed/expired attempts do not block a distinct operation, retained copies become stable-identity cleanup work at claim expiry or distinct-operation supersession, live competing generation blocks reopen, stale generation activation must match the current correction cycle, actor attribution is reason-event-only, and correction audit details plus pending reopen attempts are withheld from non-superusers. | [dataverse-wmkf-requestdocument.md](atlas/dataverse-wmkf-requestdocument.md) |
...
| Pre-Site informational distribution | `pre_site_distribution_attempts` (exact preview, cross-system send recovery, transport receipt; no attachment bytes) — **[VERIFIED LIVE 2026-08-24: migration 035 applied; 66 columns including 11 calendar/Site Visit/material additions; one `sent` base-proof row for Request `1002379`, zero calendar attempts]** | [postgres-infra-tables.md](atlas/postgres-infra-tables.md) |
...
**[VERIFIED 2026-08-24 via directory inventory]** The adapter layer contains
22 files:
...
| `pre-site-visit/distribution-service.js` / `distribution-store.js` | `pre_site_distribution_attempts` (R/W; migrations 034–035 live with one `sent` base-proof row and zero calendar attempts) | reads current `akoya_request`/`wmkf_requestdocument` and optional `wmkf_sitevisit`; creates retained snapshot rows and granular Dynamics email/attachment/send activity | **[PRODUCTION-PROVED BASE; CALENDAR/LINK EXTENSION PRODUCTION-DEPLOYED 2026-08-24.]** Request `1002379`, PDF-only operation `85f52fc5-fb48-4ceb-84d6-0f246af0b6fb`, retained Ready/Board Ready DOCX and PDF rows and sent the selected 133,265-byte PDF to `jgallivan@wmkeck.org`. The ledger reached `sent`; Dynamics activity `33ce6346-d89f-f111-b8db-6045bd07a06d` read back Sent with actor attribution and exactly one attachment matching SHA-256 `574ac7b833801866c370a8056b7197933addfe3ea5dd535dcf4d29803c18f0c9`; Workbench history showed the receipt and a bounded Production error-log scan was clean. Dynamics appended its CRM tracking token after preview acceptance; inbox delivery is unverified. Migration 035 adds exact material-link and informational-calendar snapshots, hashes, Site Visit ETag fencing, and a separate calendar-attachment receipt. The extension is deployed but has not prepared or sent a Production calendar attempt. |
| `site-visit/logistics-service.js` / `recipient-directory-service.js` | `user_profiles` and `expertise_roster` (read; roster preferred email written by the existing Expertise Finder editor) | reads/writes one request-bound `wmkf_sitevisit`; reads `systemusers`, current Pre-Site state, and governed material rows | **[PRODUCTION-DEPLOYED 2026-08-24; SIGNED-IN SAVE PROOF OPEN.]** Requires Ready/Review Pre-Site state, literal schema readiness, server-resolved organizer/attendees, one active Site Visit, and ETag concurrency. Unchanged parties use parent PATCH; changed party roles use a sandbox-proved atomic delete/create of the same Activity GUID because direct ActivityParty writes are unsupported. |
