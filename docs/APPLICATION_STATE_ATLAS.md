...
| `final-writeup/dashboard-service.js` | none | `akoya_request`, `wmkf_requestdocument`, `systemusers`, security-role/team membership, and `wmkf_finalwriteupreviewacknowledgements`; Microsoft Graph separately | **[PRODUCTION-LIVE ORDINARY FOUNDATION; SUPERUSER MATRIX SOURCE-BUILT + LOCAL SIGNED-IN VERIFIED AGAINST PRODUCTION READS 2026-08-31, NOT DEPLOYED.]** Read-only, fail-closed projection over at most 100 requests with current-Final pointers. Exact Final rows and acknowledgement rows are batch-read in bounded chunks; stable Graph identities are deduplicated and refreshed with concurrency four. The service derives stage, lead-PD relationship, personal state, positive initials, open/history/stewardship queues, and optional focused navigation. For a fresh superuser index request only, it resolves the bounded exact enabled `WMKF Final Writeup Reviewer` roster and returns the complete request × reviewer matrix; ordinary and focused responses return no matrix. The disabled persona resolver performs no Dataverse read. When later enabled it accepts multiple personas only from pinned team GUID membership and fails closed on missing/duplicate IDs or viewer ambiguity. The Production ordinary/focused surface is live; the matrix passed local signed-in desktop/mobile QA with actual Production read data but is not yet deployed. No supporting-material reader was added. |
