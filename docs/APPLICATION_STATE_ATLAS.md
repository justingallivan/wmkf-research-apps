...
| Cycle Dossier pilot | `cycle_dossiers`, `cycle_dossier_previews`, `cycle_dossier_entries`, `cycle_dossier_runs`, `cycle_dossier_control`, `cycle_dossier_editions` — **[SOURCE-BUILT 2026-09-07; migration 038 not live-provisioned]** private D26 selections, bounded previews, shared immutable request revisions, one global run lease with independently checkpointed items, a durable singleton operator stop, and owner-private editions; JSONB stores metadata/checkpoints while bytes use the dedicated private Blob store. | [postgres-cycle-dossiers.md](atlas/postgres-cycle-dossiers.md) |
