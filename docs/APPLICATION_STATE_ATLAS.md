...
| `initial-assessment/controls-service.js` | none | `akoya_request`, `wmkf_requestdocument`, and SharePoint `akoya_request` | **[SOURCE-BUILT 2026-08-30; NOT PRODUCTION-PROVED.]** Superuser restore resolves the canonical request pointer, promotes one native historical version, verifies governed content, and ETag-refreshes registry metadata with lost-response reconciliation. Board freeze copies exact current bytes to one deterministic distinct Ready/Board Ready row/item linked to source row/version/hash; create-only upload, byte-for-byte readback, claim recovery, actor attribution, and exact cleanup ownership fail closed. |
