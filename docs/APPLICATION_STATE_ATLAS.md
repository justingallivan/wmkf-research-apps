...
| `final-writeup/dashboard-service.js` | none | `akoya_request` including broad `wmkf_grantprogram`, `wmkf_requestdocument`, `systemusers`, security-role/team membership, `wmkf_appsystemsettings`, and `wmkf_finalwriteupreviewacknowledgements`; Microsoft Graph separately | **[PRODUCTION-LIVE FOUNDATION + SIGNED-IN READ SMOKE PASSED 2026-08-31; PROGRAM-AUDIENCE EXTENSION SOURCE-BUILT + UNIT-TESTED, NOT DEPLOYED.]** The deployed projection and its bounded reads remain unchanged for ordinary/focused consumers. The extension uses the request's broad Grant Program GUID to group only the superuser matrix. Missing configuration preserves the existing exact-role matrix; after a setting is published, configured groups use only their stable reviewer references, unconfigured request programs remain explicit, and stale saved identities fail closed. Ordinary/focused responses receive neither matrix nor configuration data. No setting value has been published. |
| `final-writeup/matrix-audience-service.js` | none | reads active `wmkf_grantprograms`, exact enabled reviewer-role `systemusers`, and `wmkf_appsystemsettings`; superuser Admin save writes only `final_writeup.matrix_audiences` | **[SOURCE-BUILT + UNIT-TESTED 2026-08-31; NOT DEPLOYED; NO VALUE PUBLISHED.]** Strict versioned reference configuration for separate program matrices. Stable GUIDs persist; names resolve live. Unknown/stale programs or reviewers, duplicate/empty audiences, and scope overflow fail closed. Admin reads preserve stale references for repair, while runtime resolution refuses them. The loaded setting ETag is required for publish and enforced with `If-Match`; stale editors fail with `409`. |
