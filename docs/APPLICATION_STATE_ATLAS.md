...
| `final-writeup/dashboard-service.js` | none | `akoya_request`, `wmkf_requestdocument`, `systemusers`, security-role/team membership, and `wmkf_finalwriteupreviewacknowledgements`; Microsoft Graph separately | **[PRODUCTION-LIVE + SIGNED-IN READ SMOKE PASSED 2026-08-31.]** Commit `52575761` and Ready deployment `dpl_Frc6fAonyFFYwiWyFJCzzE3UNune` ship the read-only, fail-closed projection over at most 100 requests with current-Final pointers. Exact Final rows and acknowledgement rows are batch-read in bounded chunks; stable Graph identities are deduplicated and refreshed with concurrency four. The service derives stage, lead-PD relationship, personal state, positive initials, open/history/stewardship queues, and optional focused navigation. For a fresh superuser index request only, it resolves the bounded exact enabled `WMKF Final Writeup Reviewer` roster and returns the complete request × reviewer matrix; ordinary and focused responses return no matrix. Signed-in Production DOM proof showed the exact 11-person roster and Request `1002788`, with Duncan Spore Reviewed, Justin Gallivan Responsible PD, all other cells Not reviewed, both direct actions, and zero browser-console errors; prior local desktop/mobile QA also passed. The disabled persona resolver performs no Dataverse read. When later enabled it accepts multiple personas only from pinned team GUID membership and fails closed on missing/duplicate IDs or viewer ambiguity. No supporting-material reader was added. |
