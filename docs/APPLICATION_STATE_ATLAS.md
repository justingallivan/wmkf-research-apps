...
| `wmkf_ai_prompt` | 22 (2026-08-16 PT snapshot) | staff-editable prompt rows for Executor; includes governed `initial-assessment.generate` v1 and sole-current `pre-site-visit.proposal-core.generate` v2 | same page |
...
| `wmkf_requestdocument` | 2 | **LIVE PRODUCTION SCHEMA + PARTIAL CONTROLLED PILOT (2026-07-30)** governed request-artifact registry for stable SharePoint identity, lifecycle, operation state, generation provenance, and operator-visible SharePoint cleanup storage. Request `1002788` preserves the mechanics-only Ready/Draft row produced from an old Phase I source. Request `1003109` adds one Ready/Draft row and matching `akoya_request.wmkf_CurrentInitialAssessment` pointer produced from the exact canonical reviewer proposal; its linked AI run carries the correct request lookup, exact-input retry created no duplicate, an interrupted-finalization retry recovered the same row/run/SharePoint item and version without another model call or upload, and an attributed substantive edit remained discoverable through both consumers. The registry correctly remains an upload/finalization snapshot after Graph advances the item. **[VERIFIED DEPLOYED 2026-07-30 via production deployment `dpl_HhiYXVFAtsGMwjU9UDcKz22AfvR2`, commit `68bcb4e8`, and signed-in Request `1003109` checks]** the Initial Assessment read model overlays Graph-current response metadata by stable drive/item identity without mutating that registry snapshot, and both consumers share current/missing/unavailable/unchecked display semantics. Both live consumers displayed current SharePoint version `2.0`. A controlled production-library probe also passed native version restore and signed-in first-stage recycle recovery; administrator policy/access evidence and Workbench history/milestone controls remain open. | [dataverse-wmkf-requestdocument.md](atlas/dataverse-wmkf-requestdocument.md) |
...
| `pre-site-visit/proposal-core-service.js` / `docx-renderer.js` | none | read-only `akoya_request`, applicant `account`, Co-PI junction, SharePoint `akoya_request`; governed execution writes append-only `wmkf_ai_run` only | **[VERIFIED IN SOURCE/TEST/LIVE CONTROLLED RUN 2026-08-16]** local guarded producer, tracked renderer, authenticated Workbench tab, and direct-download route. Request `1002379` resolved exact `AI Materials/ProposalNarrative_1002379.pdf` (33,011 extracted characters), PI/Co-PI roster, and every template metadata field. Sole-current prompt v2 `1d276948-ed99-f111-b8db-70a8a59cded0` uses reviewed `claude-sonnet-4-6`; controlled run `5bd65180-ed99-f111-b8db-7ced8d6e2f44` produced a four-page DOCX that passed rendered-page and structural QA. The route returns DOCX bytes only: no SharePoint upload, registry row, pointer, or business-field write. The branch and signed-in route remain unpromoted/unproven in Production. |
