...
| `wmkf_ai_prompt` | 20 | staff-editable prompt rows for Executor; includes governed `initial-assessment.generate` v1 provisioned 2026-07-30 | same page |
...
| `pre-site-visit/proposal-core-service.js` / `docx-renderer.js` | none | read-only `akoya_request`, applicant `account`, Co-PI junction, SharePoint `akoya_request`; governed execution attempts `wmkf_ai_run` only | **[VERIFIED IN SOURCE/TEST 2026-08-16; live source/prompt checks read-only]** local guarded producer, version-1 renderer, authenticated Workbench tab, and direct-download route. Request `1002379` resolved exact `AI Materials/ProposalNarrative_1002379.pdf` (33,011 extracted characters), PI/Co-PI roster, and every template metadata field. The route returns DOCX bytes only: no SharePoint upload, registry row, pointer, or business-field write. A live dry-run returned `action=create version=1`, so no current prompt row was seeded and no application model call has run. |
