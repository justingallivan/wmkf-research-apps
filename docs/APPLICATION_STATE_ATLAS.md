...
| `wmkf_appreviewanswer` | — | **[CREATED prod S301; R/W LIVE S302]** point-in-time answer-snapshot child of `wmkf_appreviewersuggestion` (one row per question per submitted review). WRITE: `/api/external/review/[token]/submit` and staff receipt paths upsert rows by alternate key. READ: `/api/review-manager/reviewers` GET attaches keyed child rows for the Workbench. **[SOURCE-BUILT, NOT DEPLOYED 2026-09-02]** Wave 25 adds a full categorical option snapshot for faithful individual/combined DOCX regeneration; read-only Production preflight reports that field absent, so schema must precede code. | [dataverse-wmkf-appreviewanswer.md](atlas/dataverse-wmkf-appreviewanswer.md) |
