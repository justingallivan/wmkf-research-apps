...
| `pre-site-visit/proposal-core-service.js` / `artifact-service.js` / `docx-renderer.js` | none | reads `akoya_request`, applicant `account`, Co-PI junction, exact SharePoint inputs, and governed prompt; writes `wmkf_ai_run`, `wmkf_requestdocument`, `akoya_request.wmkf_CurrentPreSiteVisit`, and one SharePoint Word item when generation succeeds | **[VERIFIED LOCALLY 2026-08-17; NOT DEPLOYED.]** The durable writer requires both exact source identities before side effects, persists eight named fields plus immutable input/output snapshots, renders from Dataverse readback, uploads to `Artifacts/Pre-Site Visit/`, and atomically activates one current Ready Word row. Exact Ready and partial-success retry behavior is focused-tested. Production Wave 19 is live but its inventory has no Pre-Site row. Sole-current prompt v3 is single-source, so a new immutable two-input prompt is required; the service blocks before claiming a row until then. Request `1002379` remains historical single-source evidence only. |
