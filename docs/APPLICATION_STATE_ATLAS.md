...
| `wmkf_requestdocument` | 4 (2026-08-17 snapshot; re-probe for current count) | **LIVE PRODUCTION SCHEMA + CONTROLLED INITIAL-ASSESSMENT AND PRE-SITE PROOF.** Wave 19 adds the Pre-Site section/snapshot fields and current Pre-Site/Final pointers. The 2026-08-17 read-only post-generation snapshot contained three Initial Assessments and one Pre Site Visit, all Ready/Draft. That day, Request `1002379` row `aeb223a2-849a-f111-b8db-70a8a59cded0` used only the exact Proposal Narrative plus Dataverse, owned stable Word item `01G4GVMS3Q5BJ65S7DDZDKFTSQLIQAIPER`, and became the request's current Pre-Site pointer; exact retry reused the same row/run/item. **[PRODUCTION-PROVED 2026-08-21]** the current pointer later completed the signed-in Site Visit handoff to Ready/Review with a complete milestone, same-item Edit/Download readback, and regeneration lock. The browser proof did not expose the current row GUID or refresh the aggregate row count; re-probe before relying on either. **[SOURCE-BUILT / NOT LIVE-VERIFIED 2026-08-22]** Wave 20 and guarded-reopen runtime/UI exist only on `codex/guarded-reopen`; target preflight/apply and runtime deployment have not run. A literal-on schema-readiness interlock keeps Wave 20 columns out of base reads and creates, and the route unavailable until exact provisioning; failed/expired attempts do not block a distinct operation, retained expired copies become stable-identity cleanup work, live competing generation blocks reopen, stale generation activation must match the current correction cycle, and all correction audit details are superuser-only. | [dataverse-wmkf-requestdocument.md](atlas/dataverse-wmkf-requestdocument.md) |
