...
| `wmkf_appsystemsetting` | probe required | Shared Dataverse settings store. **[PRODUCTION-DEPLOYED AND OWNER-VIEWED 2026-08-30; no durable budget revision published]** Executor standing/retry budgets use append-only keys `executor.budgets.vNNNNNN`; the highest valid revision is the mutable source, while code owns strict bounds and the outage fallback. The Production Admin read surface displayed the expected no-revision/fallback state. **[PRODUCTION-LIVE + SIGNED-IN ADMIN WRITE/READBACK PROVED 2026-08-31]** `final_writeup.matrix_audiences` stores one versioned set of broad Grant Program GUID → reviewer `systemuser` GUID references for the coordinator matrix. The published Research audience contains nine current role members and excludes Anneli Stone and Saskia Pallais; Southern California remains explicitly unconfigured pending its complete audience. Live Dataverse names remain authoritative. | [dataverse-wmkf-ai-run-and-prompt.md](atlas/dataverse-wmkf-ai-run-and-prompt.md) |
...
| `final-writeup/dashboard-service.js` | none | `akoya_request` including broad `wmkf_grantprogram`, `wmkf_requestdocument`, `systemusers`, security-role/team membership, `wmkf_appsystemsettings`, and `wmkf_finalwriteupreviewacknowledgements`; Microsoft Graph separately | **[PRODUCTION-LIVE + SIGNED-IN PROGRAM-AUDIENCE READ PROVED 2026-08-31.]** Commit `5573bca3` is live in Ready deployment `dpl_5DNuc2BV76RihwuWu8ZFYBgxBXE7`. The bounded projection uses each request's broad Grant Program GUID to group only the superuser matrix. Request `1002788` rendered under Research with exactly the nine published reviewer columns, excluding Anneli Stone and Saskia Pallais, and zero browser-console errors. Configured groups use only stable reviewer references; unconfigured request programs remain explicit and stale saved identities fail closed. Ordinary/focused responses receive neither matrix nor configuration data. |
| `final-writeup/matrix-audience-service.js` | none | reads active `wmkf_grantprograms`, exact enabled reviewer-role `systemusers`, and `wmkf_appsystemsettings`; superuser Admin save writes only `final_writeup.matrix_audiences` | **[PRODUCTION-LIVE + SIGNED-IN ADMIN WRITE/READBACK PROVED 2026-08-31.]** Strict versioned reference configuration for separate program matrices. The published Research audience contains nine current role members and excludes owner-confirmed Southern California staff Anneli Stone and Saskia Pallais; Southern California remains explicitly unconfigured pending its complete audience. Stable GUIDs persist and names resolve live. Unknown/stale programs or reviewers, duplicate/empty audiences, and scope overflow fail closed. Admin reads preserve stale references for repair, while runtime resolution refuses them. The loaded setting ETag is required for publish and enforced with `If-Match`; stale editors fail with `409`. |
