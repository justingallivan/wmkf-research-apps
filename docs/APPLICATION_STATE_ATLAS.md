...
| Review synthesis lifecycle | `review_synthesis_jobs` (generation queue/currentness ledger; no review text; Dataverse request memo remains content source) — **[VERIFIED 2026-07-28 via production schema and post-deployment probes] migration 028 applied; lifecycle deployed in `70956477`; automation disabled; 0 rows** | [postgres-infra-tables.md](atlas/postgres-infra-tables.md) |
...
| `review-synthesis-job-service.js` / `review-synthesis-drain.js` | `review_synthesis_jobs` (R/W) | `wmkf_appreviewersuggestion`, `wmkf_appreviewanswer`, `akoya_request`, and `wmkf_ai_run` through adapters/Executor | **[VERIFIED 2026-07-28 via Vercel deployment, authenticated disabled-path probe, and production Postgres probe]** Lifecycle code is production-deployed in merge `70956477` / deployment `dpl_2tgAYjUXFFx4nQo7FgE2Z3TBMqP9`; table live and empty; automation disabled. Exact input fingerprint dedupes automatic work; leased claims revalidate readiness and fingerprint; the table stores state/hashes only, never reviewer content. Dataverse `wmkf_reviewsynthesisjson` remains the synthesis source of truth. |
