...
| Personalized scheduled email | `scheduled_email_messages` (exact editable draft, PD action/version audit, internal review notification, send lease, Dynamics activity/transport receipt, Dataverse finalization repair) — **[SOURCE-BUILT 2026-08-25; migration 036 added but NOT applied or production-proved]** | [postgres-infra-tables.md](atlas/postgres-infra-tables.md) |
...
| `scheduled-email-service.js` / `scheduled-email-store.js` | `scheduled_email_messages` (R/W; migration 036 source-built, not applied) | reads fresh `wmkf_granteedeliverable`, mints grantee token only at send, creates/recovers Dynamics email activity, sends as the assigned PD, and finalizes Reminder Sent + `wmkf_remindeddate` | **[SOURCE-BUILT + UNIT-TESTED 2026-08-25; NOT LIVE.]** One row freezes the exact subject/body/signature/recipients and day-12 send time, opens an optional per-PD review window earlier, records edit/approve/stop actions with optimistic versioning, and stores notification/send leases and receipts. Due sends recheck source eligibility before transport; sent-but-unfinalized rows repair Dataverse without re-sending. Configured PDs enter this path; no-preference PDs retain the legacy automatic route as built. The rollout default was resolved 2026-08-26 by `docs/SCHEDULED_EMAIL_VIP_DIGEST_PLAN.md`, which supersedes this decision layer before promotion. |
