...
| `wmkf_requestdocument` | 10 (2026-08-23 post-reopen probe: 3 Initial Assessments/7 Pre Site Visits; 9 Ready/1 Failed; 6 Draft/4 Superseded) | **LIVE PRODUCTION SCHEMA + CONTROLLED INITIAL-ASSESSMENT AND PRE-SITE PROOF.** The Production row count remains the 2026-08-23 snapshot. **[SOURCE-BUILT 2026-08-30; NOT PRODUCTION-PROVED]** Initial Assessment restore ETag-refreshes the canonical row after native Graph restore, while Board freeze uses existing source/version/hash, producer, lifecycle, generation, file-identity, and actor fields to create a distinct Ready/Board Ready row/item without schema change or pointer movement. Exact snapshot rows are excluded from editable activation/supersession/cycle discovery. Wave 19 adds the Pre-Site section/snapshot fields and current Pre-Site/Final pointers. Request `1002379` originally proved Ready generation/exact retry, then the 2026-08-21 signed-in Ready/Review Site Visit handoff. **[PRODUCTION-PROVED 2026-08-23]** Wave 20 is 3 exact/0 divergent, the readiness flag is literal `on`, and guarded reopen is Ready in deployment `dpl_BbtmRghhSYa7EPiQkWxsmdkgRozp`. After exact owner approval, signed-in Request `1002379` preserved and superseded source `76a0d4b2-8b9a-f111-b8db-7ced8d3d15a6`, created current Ready/Draft successor `888982b6-0a9f-f111-b8dc-7ced8d3d15a6` and one distinct SharePoint copy, and left no Final pointer or cleanup work. | [dataverse-wmkf-requestdocument.md](atlas/dataverse-wmkf-requestdocument.md) |
...
| `initial-assessment/controls-service.js` | none | `akoya_request`, `wmkf_requestdocument`, and SharePoint `akoya_request` | **[SOURCE-BUILT 2026-08-30; NOT PRODUCTION-PROVED.]** Superuser restore resolves the canonical request pointer, promotes one native historical version, verifies governed content, and ETag-refreshes registry metadata with lost-response reconciliation. Board freeze sends the exact current source buffer through create-only upload to one deterministic distinct Ready/Board Ready row/item linked to source row/version/hash; post-ingestion readback verifies normalized governed Word content so SharePoint repackaging is accepted while changed/invalid content, claim recovery, actor attribution, and exact cleanup ownership fail closed. |
