...
| Pre-Site informational distribution | `pre_site_distribution_attempts` (exact preview, cross-system send recovery, transport receipt; no attachment bytes) — **[VERIFIED LIVE 2026-08-24: 55 columns and canonical constraints/indexes; one `sent` production-proof row for Request `1002379`, operation `85f52fc5-fb48-4ceb-84d6-0f246af0b6fb`]** | [postgres-infra-tables.md](atlas/postgres-infra-tables.md) |
...
| `pre-site-visit/distribution-service.js` / `distribution-store.js` | `pre_site_distribution_attempts` (R/W; migration 034 live with one `sent` production-proof row) | reads current `akoya_request`/`wmkf_requestdocument`; creates retained snapshot rows and granular Dynamics email/attachment/send activity | **[PRODUCTION-PROVED 2026-08-24.]** Request `1002379`, PDF-only operation `85f52fc5-fb48-4ceb-84d6-0f246af0b6fb`, retained Ready/Board Ready DOCX and PDF rows and sent the selected 133,265-byte PDF to `jgallivan@wmkeck.org`. The ledger reached `sent`; Dynamics activity `33ce6346-d89f-f111-b8db-6045bd07a06d` read back Sent with actor attribution and exactly one attachment matching SHA-256 `574ac7b833801866c370a8056b7197933addfe3ea5dd535dcf4d29803c18f0c9`; Workbench history showed the receipt and a bounded Production error-log scan was clean. Dynamics appended its CRM tracking token after preview acceptance; inbox delivery is unverified. The service retains exact Word first from stable-ID native publication metadata, fences Word version/eTag around PDF conversion, supports DOCX/PDF/both, and binds a content-hashed preview. Byte-identical Ready reuse may refresh settled Graph version/eTag metadata under Dataverse ETag concurrency only after stable read and exact hash proof. Send requires literal impersonation, revalidates the current pointer/source version under the lease, persists a created/recovered Dynamics activity ID before exact-content assertions, preserves correlation ambiguity, records attachment/send intent stepwise, and renews the lease before transport. Frozen snapshot rows are excluded from editable status/activation/supersession and guarded-reopen blockers; history still reports source-version drift. Recipient entry by known staff is authoritative; syntax/dedupe/To-Cc conflict checks remain fail-closed. |
