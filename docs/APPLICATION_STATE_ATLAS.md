...
| `pre-site-visit/proposal-core-service.js` / `docx-renderer.js` | none | read-only `akoya_request`, applicant `account`, Co-PI junction, SharePoint `akoya_request`; governed execution writes append-only `wmkf_ai_run` only | **[VERIFIED IN SOURCE/TEST/LIVE CONTROLLED RUN 2026-08-16]** local guarded producer, tracked renderer, authenticated Workbench tab, and direct-download route. Request `1002379` resolved exact `AI Materials/ProposalNarrative_1002379.pdf` (33,011 extracted characters), PI/Co-PI roster, and every template metadata field. Sole-current prompt v3 `f2c9ce97-f499-f111-b8db-7ced8d6e2f44` uses reviewed `claude-sonnet-4-6`; latest audited Executor run `5bd65180-ed99-f111-b8db-7ced8d6e2f44` remains the accepted v2 document. A direct exact-v3 model/render QA produced correct roster underlining and personnel style but spilled Methodology's final sentence to page 4; it created no `wmkf_ai_run`. The route returns DOCX bytes only: no SharePoint upload, registry row, pointer, or business-field write. The branch and signed-in route remain unpromoted/unproven in Production. |
