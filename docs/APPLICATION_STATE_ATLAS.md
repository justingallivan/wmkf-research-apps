...
| `pre-site-visit/proposal-core-service.js` / `docx-renderer.js` | none | read-only `akoya_request`, applicant `account`, Co-PI junction, SharePoint `akoya_request`; governed execution attempts `wmkf_ai_run` only | **[VERIFIED IN SOURCE/TEST 2026-08-16; live source read-only]** local guarded producer and version-1 template renderer. Request `1002379` resolved exact `AI Materials/ProposalNarrative_1002379.pdf` (33,011 extracted characters), PI/Co-PI roster, and every template metadata field. No prompt row was seeded or executed, no route/UI/upload/registry flow exists, and the generated local fixture explicitly states that Claude was not run. |
