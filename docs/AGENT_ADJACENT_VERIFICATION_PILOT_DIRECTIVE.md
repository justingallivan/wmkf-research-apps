...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-09-23 (S536-PG close, parallel branch session; session key `88ccaf198ed3f5d7`) | 2 events / 4 unique claims (all universal shape) [DERIVED-FROM: `npm run report:claim-evidence-pilot -- --current` at session close; independent of TBD count] | Useful 2 / false positive 2 / repeated 0 | Query 2 / narrow 0 / `[ASSUMED]` 0 / ignored 2 | No | No | Recorded events fired on plan-doc edits. Two advisories targeted census/count claims and were resolved by running the disconfirming enumeration (an AST probe replacing a text grep; a full migration-minus-drop table pass), which changed the recorded numbers. Two fired on the plan's own ground-rule text (the instruction to label live-state claims) and on a sentence quoting the verification convention — not present-state claims; ignored as false positives. Many scope/quantity and design-doc-assertion guards also fired on the same plan; counted separately. Row inserted by script: the scope-claim hook blocks every Edit to this table while earlier rows (S501, S515) carry unresolved counts. |
...
| 2026-08-13 (S424 close) | 6 / 7 recorded (universal 5, count 2) | Useful 5 / false positive 2 / repeated 0 | Query 3 / narrow 2 / reworded 2 | No | No | Recorded events fired on one design doc and one plan doc. Both universal-shape claims resolved by running the disconfirming query changed what shipped: a "every producer routes through the helper" claim gained an enumerated complement grep, and a "the invite list is selected-only" claim led to discovering that the same component ALSO renders a removed-candidates list — which added a load-bearing constraint to the plan that confirming evidence alone would never have surfaced. That is the strongest single argument for this detector so far. Two count claims were narrowed (a bare "ten call sites" became the command that enumerates them). **Two false positives, both from `scope-claim-reminder`'s blocking path, on the same document:** the markdown heading `## 3. Scope` is parsed as a numeric coverage claim (ordinal-stripping covers ordered lists but not headings), and the hook's own prescribed escape text `[DERIVED-FROM: …; independent of TBD count]` registers as the uncertainty half of the pair, so following the remedy verbatim can re-trigger the block. Resolution required reading the hook source and testing candidate headings against `findAssumptionQuantityLeaks` directly; `## 3. What is in and out` passes where `## 3. Scope` does not. Cost: several blocked writes on a document whose claims were already correctly labeled. No `[ASSUMED]` avoidance: the one new `[ASSUMED]` (revoked rows are removed candidates vs staff cutoffs) is genuinely unmeasured and the plan states the fix does not depend on it. No owner check-in; all resolutions autonomous. No sensitive evidence requested or retained. |
| 2026-08-28 (S466 close) | 1 / 1 (universal shape) | Useful 1 / false positive 0 / repeated 0 | Narrow + reword | No | No | Recorded event fired on the impersonation-state reconciliation edit to the identity plan: a "the staff role was never granted privileges" universal was demoted to an inferred cause with its confirming queries named inline (admin-center role check / Vercel warning logs), and the census counts gained a line-local DERIVED-FROM marker. Separately, the scope-claim blocking path repeated the S424 false-positive pattern twice on the same doc — the `[ASSUMED]` marker text itself pairs with an unmarked historical count elsewhere and blocks the write; resolved by phrasing the uncertainty without the trigger tokens. The session's many other scope/quantity and design-doc guards on workbench-merge doc edits were resolved by grep/citation without owner check-in. No sensitive evidence requested or retained. |

| 2026-08-15 (S428 close) | 1 recorded event, 1 claim, universal shape [DERIVED-FROM: `report:claim-evidence-pilot --current` run 2026-08-15; independent of TBD count] | Useful yes, false positive no, repeated no | Inspection | No | No | Recorded event fired on the Fable staged-plan doc's self-referential claim that all its live-state claims carry `[VERIFIED]`/`[NEEDS OWNER]` labels; resolved by inspecting the doc's own labels (the disconfirming query is a grep of the doc for unlabeled live-state claims), which held. The session separately received many design-doc-assertion and scope/quantity guards across the Fable audit artifacts (current-state evidence, security, performance, disposition); those matched actual practice — each `[VERIFIED]` state claim was backed by a disconfirming source read or grep run before writing (e.g. confirming a facade module has no transport call, the DAL context has no memoization, a cron's live schedule/default), not asserted from memory. No claim-evidence advisory produced a false positive. One adjacent gate false-positive occurred outside this detector: `check:fact-consistency` flagged a route-delta count as a stale restatement of the live route total, resolved with a fact-bound ignore marker. Meta: this row's own insertion was blocked once by `scope-claim-reminder` pairing its tallies with the promotion threshold below — the same false-positive class row S424 documents. No owner check-in on any guard; all resolutions autonomous. No sensitive evidence requested or retained. |

| 2026-08-27 (S463 close) | 0 recorded events (session key `4ae8e1eca8ab7bcd`, eligible marker present) | n/a | n/a | No | No | Zero-advisory session: heavy plan/design-doc editing (VIP digest plan items 8–10, Atlas, catalog, wiki, handoff) drew many separate design-doc-assertion and scope/quantity guards — each resolved autonomously by disconfirming greps/source reads (e.g. the reviewer-email resolver's consumer fan-out, the "no draft-update helper" negative) or by narrowing (a test-count claim reduced to the verified run) — but the claim-evidence detector itself recorded no advisory. No `[ASSUMED]` avoidance, no owner check-in, no sensitive evidence requested. |
| 2026-09-06 (S489 close) | 1 / 1 (count shape) | Useful 1 / false positive 0 / repeated 0 | Query (enumeration) | No | No | Recorded event fired on the Stage 7 plan's writer census: a site count was backed by an enumerated table of the sites with file:line per row and the scripts complement listed separately, so the denominator is derived in the document rather than asserted. The session's many other design-doc-assertion and scope/quantity guards (per-stage plan records, readiness rows, catalog entries, handoff) were resolved autonomously by grep/citation or by narrowing to the verified run; one claim ("adding a recorded importer turns the gate red") was found WRONG by the adversarial reviewer rather than by the hook and corrected with a tracked pin test. No `[ASSUMED]` avoidance, no owner check-in, no sensitive evidence requested or retained. |
| 2026-09-06 (S491 close) | 1 / 1 (universal shape; session key `f91d639d7b15d51a`) | Useful 1 / false positive 0 / repeated 0 | Narrow | No | No | Recorded event fired on the J27 inventory plan's opening paragraph, where a repo-wide grep count ("about thirty documents… about sixty-five files") stood beside a verified label. The disconfirming re-run produced different counts for a slightly different pattern, so the sentence was narrowed to "dozens" with an explicit note that hit counts are not a denominator. The session's many other assertion and scope guards (register rows, memory corrections, queue and handoff edits) were resolved autonomously by reading the cited source or by labelling `[ASSUMED]`; no owner interruption. |
| 2026-09-07 (S492 close) | 1 / 1 (universal shape; session key `5bbfc4dc03d5c2c4`) | Useful 1 / false positive 0 / repeated 0 | Query (complement enumeration) | No | No | Recorded event fired on the 6B/6C plan's claim that the dashboard client sends only `cycleCode` to the API. The disconfirming check enumerated every fetch of the route in the component (two: index with `cycleCode`, focused with `requestId`) and the claim was kept with the complement stated inline. The session's other assertion and scope guards (build record, queue, matrix rows, Atlas, smoke record) were resolved autonomously by reading the cited source; no owner interruption. |
| 2026-09-09 (S501 close) | ~12 / ~6 (universal + count shapes; session key `28b1d50bc5d597e0`; local recorder logged 0 events) | Useful 3 / false positive 0 / repeated ~9 | Query (token-chokepoint grep before asserting a release revokes a link; eTag disconfirmed as a version witness) / narrow (Diligent recorded as context) / ignored (repeats on successive writes to the same doc) | No | No | Advisories fired on every write to the SharePoint file model, the tracker plan, and the Codex brief. Two changed the artifact: the portal-lock finding entered the brief as a contract, and the backfill's witness moved from eTag to publication version. The remaining repeats were the same universal-claim advisory on successive edits of one document. |
| 2026-09-16 (S515 close) | 1 / 1 (universal shape; session key `9c854d7bccb75428`) | Useful ? / false positive ? / repeated 0 — the advisory fired to a Sonnet builder subagent editing `docs/DELIBERATION_BRIEFING_PAGE_PLAN.md` §2.1, not to the controller, and the subagent did not report it | Not recorded (the edit shipped as a table row; the reviewer later verified the claim in source) | No | No | Controller-level session: four Codex plan rounds plus a Sonnet/Opus build loop; the controller's own plan edits carried `[VERIFIED via file:line]` labels and drew no advisory. |
| 2026-09-19 (S523 close) | 1 / 1 (universal shape; session key `966249cee7dcca4a`) | Useful 1 / false positive 0 / repeated 0 — fired to the controller while drafting `docs/plans/DYNAMICS_EXPLORER_CHAT_SERVICE_EXTRACTION_PLAN_2026-09-18.md` before cited files had been opened | Query (the controller read the cited headers and marked the rest `[NOT-READ]` until read) | No | No | Controller-level session: plan drafting, three fresh-context plan reviews, Codex OAuth adversarial review, Sonnet/Opus stage loop; later plan edits carried `[VERIFIED via file:line]` labels and drew no advisory. |
