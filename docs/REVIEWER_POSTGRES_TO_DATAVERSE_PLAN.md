...
The `lib/dataverse/schema/wave2/` directory holds six schema-as-code files written in an earlier session that designed the original Wave 2 entities. Of the six, four are deployed today (`wmkf_appgrantcycle`, `wmkf_apppublication`, `wmkf_appresearcher`, `wmkf_appreviewersuggestion`) and `wmkf_appproposalsearch` is also deployed (S185, 0 rows, entity-set name uses the unconventional plural `wmkf_appproposalsearchs`; S188 audit re-sweep confirmed live). Only `wmkf_app_z_publication_author` (the junction) remains undeployed. All five deployed entities encode the **1:1 sidecar model** (not the pool model the Wave 1 design doc text implied):
...
| `proposal_searches` | 0 | **Reckon.** Writer is dead but `extract-summary.js` reads it as an IDOR ownership guard (`pages/api/reviewer-finder/extract-summary.js:57`). Empty table → guard always fails → extract-summary is functionally broken today. The Dataverse counterpart `wmkf_appproposalsearch` IS deployed (S185, 0 rows, entity-set `wmkf_appproposalsearchs`) but unused. **Decision is: retire `extract-summary` entirely**. UI removal of any callers required (Reviewer Finder picker calls `/api/reviewer-finder/extract-summary` from `pages/reviewer-finder.js:3538`). The deployed `wmkf_appproposalsearch` entity sits empty awaiting a future feature need. |
...
| `pages/api/reviewer-finder/grant-cycles.js` | Direct `sql\`\`` against `grant_cycles` (5+ sites) **plus** `proposal_searches` and `reviewer_suggestions` reads (verified 2026-05-12 grep + Codex S147 re-grep) | Rewrite all sites against `wmkf_appgrantcycle` (alt-key by short_code). **Full scope (Codex S147 Q3):** (a) per-cycle proposal counts (`proposal_searches` join, lines 64-69) → `akoya_request` filtered on `akoya_fiscalyear`; (b) per-cycle candidate counts (`reviewer_suggestions` join, lines 88-94) → `wmkf_appreviewersuggestion` filtered on `wmkf_grantcyclecode`; (c) **unassigned candidate count** (`reviewer_suggestions WHERE grant_cycle_id IS NULL`, lines 99-106) → `wmkf_appreviewersuggestion` filtered on null `wmkf_grantcyclecode`; (d) **duplicate-check read by active `short_code` before POST** (lines 162-166) → Dataverse query with alt-key collision check (or rely on alt-key to throw, but check first for a better UX message); (e) **soft-delete via `is_active=false`** (lines 355-368) → PATCH `wmkf_isactive=false`, NOT row delete. |
...
9. **`maintenance-service.js` blob orphan scanner.** Reads Postgres `proposal_searches`, `grant_cycles`, `reviewer_suggestions` for blob URLs. **Live-data note**: only `reviewer_suggestions.summary_blob_url` (55%) and `review_blob_url` (1%) carry real data. Rewrite to read Dataverse before cutover; bounded urgency.
