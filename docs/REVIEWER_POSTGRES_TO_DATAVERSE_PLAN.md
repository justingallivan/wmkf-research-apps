...
- ~~Post-pilot one-shot Postgres table drop~~ **DONE EARLY 2026-06-04 (S219)** via tracked migration `018_drop_reviewer_finder_postgres_tables.sql` (5 tables; `search_cache` kept). Was slip-eligible/post-pilot; shipped ahead of the ≥2026-07-01 trigger at Justin's direction. Not a cron. See the Post-pilot row of the schedule above + memory `[[w6-table-drop-pending]]` (CLOSED).
