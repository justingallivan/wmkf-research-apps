...
| `scripts/restore-reviewer-suggestion-cleanup-backup.js` | **BUILT 2026-05-18 (S164) — for the Rollback §3 / line-32 path ONLY** | ⚠️ **The plan _previously_ double-booked this filename for two unrelated restores (RESOLVED S164 — see below).** **(A) BUILT S164:** the Rollback §3 / line-32 restore — reverses the *Dataverse* `wmkf_appreviewersuggestion` unengaged-row cleanup; reads a JSON backup envelope (contract v1 defined in the script header), re-creates rows via `reviewerSuggestionAdapter.upsert` (re-runnable find-then-update/create — NOT a true alt-key PATCH, NOT concurrent-safe), dry-run default + non-zero exit on failure. Dry-run verified live read-only S164 (both WOULD-CREATE/WOULD-UPDATE branches). **(B) the Postgres drain-table restore — SUPERSEDED, never built under this name.** The line-801 checklist originally spec'd a *different* restore under this *same filename* (the double-booking) — "~30 lines, reads **JSONL → INSERT**" back into the *Postgres* drain-only tables. That spec'd `scripts/restore-postgres-drain-table-backup.js` was **never built**: the actual drop (2026-06-04, S219) went through tracked migration `018_drop_reviewer_finder_postgres_tables.sql` with backup/restore handled by `scripts/w6-drop-backup.js` / `scripts/w6-drop-restore.js` (+ Neon PITR), so the spec'd script was superseded. **RESOLUTION DONE (S164, Codex-recommended — distinct filenames, NOT multi-mode):** (A) = this script `scripts/restore-reviewer-suggestion-cleanup-backup.js` (built, Dataverse-suggestion restore); (B) = the Postgres drain-table restore, superseded by the migration-018 path (`w6-drop-restore.js`). S164 also fixed a `selected`-default BLOCKER (missing/non-boolean `selected` now hard-SKIPs instead of silently defaulting an engaged-signal to `true`) + corrected the idempotency wording (find-then-update/create, not a true alt-key PATCH); Codex-reviewed. NOTE: the S164-era claim that the W6 memory "does not exist (phantom ref)" is itself now stale — the memory exists at `.claude-memory/project-w6-table-drop-closed.md` and is CLOSED (drop DONE 2026-06-04). |
...
- Any of `wmkf_ReviewerImpact`, `wmkf_ReviewerRisk`, `wmkf_ReviewerOverallRating` populated (they submitted a review form) — **MOOT (S305):** this predicate is historical; migration 018 already dropped the `reviewer_suggestions` Postgres table it gated (2026-06-04). For the record, these 3 rating columns stopped being written at the epic's Phase E1; a live "submitted a review" signal would read `wmkf_reviewreceivedat` (co-set by every rating writer), as the reviewer-merge engagement predicate already does.
...
| `wmkf_fiscalyearcode` | `akoya_request.akoya_fiscalyear` for the matching cycle (e.g., `"June 2026"`) | Legacy compatibility/alt-key field; reviewer-finder proposal counts use `wmkf_meetingdate` month grouping because request fiscal-year text is incomplete. |

**Counts (derived, not stored)**: pre-W3 `grant-cycles.js` JOINed `proposal_searches` and `reviewer_suggestions` for per-cycle proposal/candidate counts. SHIPPED W3 cutover (2026-05-12), corrected 2026-09-07 — the endpoint groups proposal requests by persisted `wmkf_meetingdate` year/month and assigns unmatched/null dates to `unassigned.proposalCount`; candidate counts remain grouped by `wmkf_grantcyclecode`.
...
| `pages/api/reviewer-finder/grant-cycles.js` | Direct `sql\`\`` against `grant_cycles` (5+ sites) **plus** `proposal_searches` and `reviewer_suggestions` reads | **SHIPPED W3 cutover (2026-05-12), corrected 2026-09-07.** All sites rewritten against `wmkf_appgrantcycle` (alt-key by short_code). Proposal counts group proposal requests by persisted `wmkf_meetingdate`; unmatched/null dates are surfaced in `unassigned.proposalCount`. Candidate counts remain grouped on `wmkf_grantcyclecode`; duplicate-check and soft-delete behavior are unchanged. |
...
| **Per-cycle proposal count parity (W3 acceptance, corrected 2026-09-07)** | `SELECT COUNT(*) FROM proposal_searches WHERE grant_cycle_id=...` (the pre-cutover join) | Proposal-type `akoya_request` rows grouped by `wmkf_meetingdate` month/year; unmatched/null dates remain in an explicit bucket | Per-cycle count match plus conservation across cycle and unassigned buckets |
...
- ~~Post-pilot one-shot Postgres table drop~~ **DONE EARLY 2026-06-04 (S219)** via tracked migration `018_drop_reviewer_finder_postgres_tables.sql` (5 tables; `search_cache` kept). Was slip-eligible/post-pilot; shipped ahead of the ≥2026-07-01 trigger at Justin's direction. Not a cron. See the Post-pilot row of the schedule above + memory `[[project-w6-table-drop-closed]]` (CLOSED).
