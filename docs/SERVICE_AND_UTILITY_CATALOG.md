...
- **`review-documents/backfill-service.js`** — Operator-only manifest and execution coordinator for historical individual-review retention. It unions exact cycle stamps with request meeting-cycle fallback, emits a content-free hash-bound population/target manifest, blocks partial/divergent/unresolved state, rebuilds the complete manifest before write preflight, and binds every ensure call to the reviewed ETag, source fingerprint, semantic hash, and exact site/drive. Row failures are retained individually and do not stop later rows. **[WAVE 3 SOURCE + FOCUSED TEST VERIFIED 2026-09-03; ADVERSARIAL REVIEW, PRODUCTION DRY RUN, MANIFEST, AND EVERY WRITE REMAIN OPEN.]**
