...
- **`final-writeup/dashboard-service.js`** — **[PRODUCTION-LIVE 2026-08-31; SIGNED-IN READ SMOKE PASSED.]** Read-only ordinary-Workbench Final Writeups projection. It requires the same Wave 23 readiness boundary and an enabled session-derived `systemuser`, caps the current-Final request set at 100, batch-reads exact current Final rows and acknowledgement rows in chunks of 25, rejects capped/ambiguous/inconsistent state, and refreshes deduplicated stable Graph identities with concurrency four. It server-derives stage, lead-PD relationship, personal freshness, positive initials, open/history/stewardship membership, and optional focused previous/next navigation. Acknowledgement existence—not later Word edits—owns history membership. It deliberately contains no persona inference, leadership lens, full coordinator matrix, supporting-material data projection, or write path. Signed-in Production smoke loaded the dashboard and focused Final for Request `1002788`; the external Word action remained available and the deployment error scan was empty.
