...
- **`request-document-actor-service.js`** — **[SOURCE-BUILT 2026-08-31; WAVE 24 PRODUCTION SCHEMA EXACT; READINESS/RUNTIME NOT PROMOTED.]** Server-only Request Document business-actor resolver. It accepts only a session-derived Dynamics GUID, freshly rereads the exact enabled `systemuser`, enforces each writer's required vs availability-first policy, produces immutable create-time actor fields, and records a bounded `request_document_actor_not_captured` operational event for approved null-attribution outcomes. The accompanying read-only census fails on post-promotion partial pairs or gaps without an allowed matching event.
...
- **`request-document-explicit-actor-readiness.js`** — Wave 24 literal-on projection/write boundary. Off preserves the legacy Request Document shape; once this runtime reaches Vercel Production, off also reports an unhealthy `requestDocumentExplicitActorReadiness` health service until separately approved readiness activation is complete. The Production schema is exact 3/3 as of 2026-08-31.
