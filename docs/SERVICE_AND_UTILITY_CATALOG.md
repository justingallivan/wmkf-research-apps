...
- **`pre-site-visit/distribution-service.js` / `distribution-store.js`** — Source/test-verified frozen informational distribution (**Production-deployed 2026-08-23 in merge `76a93a41` / Ready deployment `dpl_A8naatyxM3vcXaG4vgt79GcL5TpR`; migration live and empty; full distribution path not live-proved**). Authenticated Production lifecycle reads passed without writes; Request `1002379` is Draft after guarded reopen, so the panel/history path was not exercised. Production read-only metadata and sandbox raw-recipient transport/repeat probes passed. It captures an exact current Pre-Site Word version under before/after metadata and governed/raw hashes, always retains a deterministic Word snapshot from stable-ID native publication version/eTag, fences that Word identity around optional PDF conversion, and lets staff select DOCX, PDF, or both. Known staff/consultant addresses are staff-authoritative but syntax-normalized, deduplicated, and rejected on invalid/To-Cc conflict. A content-hashed exact preview binds actor/sender, recipients, message, source, mode, and selected attachment identities/bytes. Every non-sent attempt requires literal Dynamics impersonation and current pointer/source-version revalidation under the lease. Creation or unique correlation recovery persists the activity ID before exact assertions; ambiguity remains a failure. Per-kind attachment receipts, durable `send_requested`, lease renewal immediately before transport, status-before-retry, and exact sent receipt prevent blind duplicate activity/send after partial failure. History compares the captured source with the current pointer/version and marks drift. SharePoint/Request Document remain file authority; Postgres stores identities/hashes only, never attachment bytes.
