...
- **`final-writeup/dashboard-service.js`** — **[PRODUCTION-LIVE FOUNDATION + SIGNED-IN READ SMOKE PASSED 2026-08-31; PROGRAM-AUDIENCE EXTENSION SOURCE-BUILT + UNIT-TESTED, NOT DEPLOYED.]** The bounded Final Writeups projection, personal queues, and pre-extension superuser matrix remain Production-live. The extension includes each request's broad `wmkf_grantprogram` only in matrix grouping, resolves the superuser audience through `matrix-audience-service.js`, preserves role-backed behavior while no setting exists, makes unconfigured programs explicit after first publication, and exposes neither matrix nor configuration to ordinary/focused responses. There is still no leadership lens, supporting-material projection, or dashboard write path.
- **`final-writeup/matrix-audience-service.js`** — Strict versioned `final_writeup.matrix_audiences` settings contract for the superuser coordinator matrix. It stores only broad Grant Program and exact enabled reviewer-role `systemuser` GUIDs, resolves names live, rejects empty/duplicate/over-cap/unknown references, fails runtime closed on stale programs or reviewers, and exposes stale references only to the superuser Admin repair surface. Full-replacement publishes use the loaded Dataverse revision as `If-Match`; stale Admin drafts receive `409` rather than overwriting newer changes. **[SOURCE-BUILT + UNIT-TESTED 2026-08-31; NOT DEPLOYED; NO VALUE PUBLISHED.]**
