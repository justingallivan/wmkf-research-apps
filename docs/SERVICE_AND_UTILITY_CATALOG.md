...
- **`institution-resolution/`** — Claim-oriented ROR API v2 candidate retrieval and local decision policy shared with the frozen v3 benchmark, plus the production request-scoped ROR→OpenAlex identity bridge. The API receives institution evidence only; provider rank/`chosen` never decides; vetoes and scoring select at most one ROR id before OpenAlex hydration; provider failure, ambiguity, multi-organization results, or ROR mismatch return `null` without poisoning settled cache entries.
...
- **`reviewer-identity-runtime.js`** — Server-owned W2 runtime seam for non-biomedical/PubMed-off Track-A verification and server-computed enrichment reconciliation. Discovery, Workbench applicant-recommended, and contact-enrichment modes are independently server-configured and fail closed to legacy; explicit owner-gated `combined` applies only to its selected entry point. Shadow/combined use one request-scoped claim-oriented ROR resolver and one 16-request OpenAlex budget per resolution; retries and redirects consume the same cap. Callers forward their parent deadline; W2 skips if its full allocation plus reserve does not fit, and comparison/error observation shares that allocation. Only one locally selected ROR id may be hydrated through OpenAlex. Batch evaluation emits one PII-free aggregate cache/provider-call metric log.
