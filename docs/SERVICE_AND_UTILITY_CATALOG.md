...
- **`pre-site-visit/proposal-core-service.js`** — Guarded Pre-Site proposal-core producer, consumed by the authenticated Workbench direct-download route. The integration branch requires exact active `AI Materials/ProposalNarrative_{Request#}.pdf` and `ProposalBibliography_{Request#}.pdf`, composes authoritative request/PI/Co-PI context from Dataverse, and calls `pre-site-visit.proposal-core.generate` with separate bounded untrusted overrides, exported system assertions, and `requireNoPersistence:true`. It returns eight validated narrative fields, document metadata, and both source identities; it does not write a business field or upload an artifact. **[VERIFIED 2026-08-16 via historical live prompt/readback and controlled Request `1002379` execution; two-file source VERIFIED LOCALLY 2026-08-17.]** Sole-current prompt v3 remains single-source and uses reviewed `claude-sonnet-4-6`; a new immutable two-input prompt version is required before promotion. The branch/direct-download route remains unpromoted and no durable artifact was written.
