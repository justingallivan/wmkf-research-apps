...
- **`pre-site-visit/proposal-core-service.js`** — Guarded Pre-Site proposal-core producer, consumed by the authenticated Workbench direct-download route. Loads the exact active `AI Materials/ProposalNarrative_{Request#}.pdf`, composes authoritative request/PI/Co-PI context from Dataverse, and calls `pre-site-visit.proposal-core.generate` with the exported system assertions plus `requireNoPersistence:true`. It returns eight validated narrative fields and document metadata; it does not write a business field or upload an artifact. A 2026-08-16 live read-only dry-run returned `action=create version=1`, so the prompt bootstrap remains unexecuted.
