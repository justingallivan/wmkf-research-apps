...
- **`review-documents/individual-review-builder.js`** — Shared answer-read → single-review composition → approved-template render seam. It returns the existing `Review-{requestNumber}.docx` attachment/file payload and keeps generation time caller-owned so thank-you attachments retain send-time metadata while a future retained copy can use the receipt timestamp. **[WAVE 1 SOURCE/TEST/RENDER VERIFIED 2026-09-03; NOT YET PRODUCTION-LIVE.]**
