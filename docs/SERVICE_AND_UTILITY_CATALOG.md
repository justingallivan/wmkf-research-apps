...
- **`reviewer-reminder-eligibility.js`** — SOURCE-BUILT on `feature/reviewer-cron-reminders-ledger` (not on main). Shared reviewer-reminder eligibility primitives extracted from the manual-reminder service: `readReminderSuggestion` (fresh `wmkf_reviewersuggestion` read, 404 → `row: null`), the shared/respond/review-due refusal predicates, and the review-status constants. Consumed by both the manual send path and the ledger workflow strategies so cron and manual sends can never diverge on what "refused" means.
- **`reviewer-reminder-workflows.js`** — SOURCE-BUILT on `feature/reviewer-cron-reminders-ledger` (not on main). Ledger strategy implementations for `reviewer_respond_reminder` / `reviewer_reviewdue_reminder`: send-time recompute from live per-request config (config drift → defer verdict; config off → stop), marker-without-CLAIM → permanent stop (a Dataverse reminded-marker stops the row only when the row never stamped `claim_committed_at`; an owned marker resumes the send), respond token-expiry stop (skipped when resuming an owned claim with no activity — the token is the row's own rotation, about to be re-minted), and `buildActivityInput` that fuses the Dataverse reminded-marker claim + fresh token mint into one If-Match `mintAndStore` PATCH inside activity creation (claim-before-send; never re-mints over an existing activity). PD send-now `force` overrides timing only, never hard eligibility. [RECHECKED after lib/services/reviewer-reminder-workflows.js change: claim-ownership discrimination added 2026-08-27 from the Codex adversarial-review high finding]
