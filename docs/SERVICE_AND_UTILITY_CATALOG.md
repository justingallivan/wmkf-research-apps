...
- **`review-documents/individual-review-builder.js`** — Shared answer-read → single-review composition → approved-template render seam. It returns the existing `Review-{requestNumber}.docx` attachment/file payload and keeps generation time caller-owned: thank-you attachments retain send-time metadata, while the Wave 2 retained-file service supplies the receipt timestamp. **[WAVE 1 SOURCE/TEST/RENDER VERIFIED AND PRODUCTION-LIVE 2026-09-03 AT `83da197f` / `dpl_F3oZ9MDbnyFox7S8Ekdos7423ece`.]**
- **`review-documents/individual-file-service.js`** — Guarded create-only retention and repair service for structured individual-review DOCX files. It freshly revalidates suggestion/request/reviewer/answer state, uses receipt time for stable generation, reconciles target content with the governed Word-part hash, conditionally commits both existing suggestion pointer fields, bounds pointer retry/cleanup, and returns per-suggestion statuses without logging review content. Scheduled discovery is exact-cycle-stamp-only and newest-first; mutation additionally requires the literal-on rollout flag, caller-specific deployment guard, enforcing Dataverse interlock, and canonical pre-resolved akoyaGO site/drive. The flag-off route creates no maintenance row. **[WAVE 2 SOURCE/FOCUSED-TEST/ADVERSARIAL-REVIEW VERIFIED AND PRODUCTION-DEPLOYED INERT 2026-09-03 AT `83da197f` / `dpl_F3oZ9MDbnyFox7S8Ekdos7423ece`; BOTH PRODUCTION ROLLOUT VARIABLES ABSENT, AUTHENTICATED DISABLED-ROUTE PROOF PASSED. THE LOCAL OPERATOR PATH IS PRODUCTION-PROVED FOR ONE EXACT OWNER-APPROVED REQUEST `1002874` ROW; SCHEDULED EXECUTION AND THE REMAINING D26 SET ARE UNPROVED.]**
