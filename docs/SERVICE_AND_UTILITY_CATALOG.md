...
- **`final-writeup/acknowledgement-service.js`** — **[ROUTE/UI SOURCE-BUILT, NOT DEPLOYED 2026-08-31.]** Wave 23 mark/read coordinator behind a distinct literal-on readiness interlock. It resolves the current Final and responsible PD server-side, validates the session-derived enabled `systemuser`, rejects PD self-review, records one stable-ID Graph publication observation, preserves same-version acknowledgement time, conditionally replaces later versions with `If-Match`/`noFallback`, and rereads ambiguous outcomes. Publication version owns freshness; eTag and last-modified remain diagnostic. The authenticated route supplies only session identity and exact request/artifact fences; the Final tab consumes its projection without blocking the external Word action. The live flag remains unset.
- **`final-writeup/dashboard-service.js`** — **[SOURCE-BUILT, NOT DEPLOYED 2026-08-31; LIVE ACKNOWLEDGEMENT FLAG UNSET.]** Read-only ordinary-Workbench Final Writeups projection. It requires the same Wave 23 readiness boundary and an enabled session-derived `systemuser`, caps the current-Final request set at 100, batch-reads exact current Final rows and acknowledgement rows in chunks of 25, rejects capped/ambiguous/inconsistent state, and refreshes deduplicated stable Graph identities with concurrency four. It server-derives stage, lead-PD relationship, personal freshness, positive initials, open/history/stewardship membership, and optional focused previous/next navigation. Acknowledgement existence—not later Word edits—owns history membership. It deliberately contains no persona inference, leadership lens, full coordinator matrix, supporting-material data projection, or write path.
