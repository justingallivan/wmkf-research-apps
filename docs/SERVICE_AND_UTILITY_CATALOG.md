...
- **`workbench-proposal-documents.js`** — Workbench document listing plus strict AI-input resolvers. Initial Assessment and request-bound Field Primer retain the exact active `AI Materials/ProposalNarrative_{Request#}.pdf` contract; the Pre-Site integration resolver requires that file plus the separate exact `ProposalBibliography_{Request#}.pdf`, extracting and hashing both independently. Reviewer packages, Phase I files, archives, neighbors, and ambiguous active buckets do not substitute.
...
- **`pre-site-visit/proposal-core-service.js`** — Guarded Pre-Site proposal-core input/execution helper. It requires exact active `AI Materials/ProposalNarrative_{Request#}.pdf` and `ProposalBibliography_{Request#}.pdf`, composes authoritative request/PI/Co-PI context from Dataverse, and calls `pre-site-visit.proposal-core.generate` with separate bounded untrusted overrides and exported system assertions. It returns eight validated narrative fields, document metadata, and both source identities to the durable artifact writer. **[VERIFIED 2026-08-16 via historical live prompt/readback and controlled Request `1002379` execution; two-file source VERIFIED LOCALLY 2026-08-17.]**
- **`pre-site-visit/artifact-service.js`** — Durable Pre-Site Request Document producer. **[VERIFIED LOCALLY 2026-08-17 via focused tests; NOT DEPLOYED.]** It resolves and fingerprints both exact sources before side effects, requires a governed prompt declaring all three server variables, claims/reclaims one deterministic Wave 19 row, persists all eight named fields plus immutable generated/input snapshots and prompt/run provenance, rereads Dataverse before rendering, uploads to `Artifacts/Pre-Site Visit/`, and atomically activates the Ready Word row with `akoya_request.wmkf_CurrentPreSiteVisit`. Exact Ready retries avoid Claude/render/upload; persisted-output retries avoid another Claude call; matching post-upload recovery reuses the same item; prompt/claim/cleanup races fail closed. Sole-current Production prompt v3 remains single-source, so a new immutable two-input prompt version is required before promotion and the writer currently blocks before claiming a row.
