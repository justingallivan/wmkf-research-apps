...
- **`final-writeup/acknowledgement-service.js`** — **[PRODUCTION-LIVE 2026-08-31; CROSS-USER WRITE PROOF PENDING.]** Wave 23 mark/read coordinator behind a distinct literal-on readiness interlock. It resolves the current Final and responsible PD server-side, validates the session-derived enabled `systemuser`, rejects PD self-review, records one stable-ID Graph publication observation, preserves same-version acknowledgement time, conditionally replaces later versions with `If-Match`/`noFallback`, and rereads ambiguous outcomes. Publication version owns freshness; eTag and last-modified remain diagnostic. Production readiness is exact `on` in Ready deployment `dpl_B9k3AprnYp5ExpkqpT3dUxCUZqWo`; signed-in Request `1002788` GET/Final-tab proof confirmed zero reviews and correct responsible-PD exclusion, so no write was attempted.
