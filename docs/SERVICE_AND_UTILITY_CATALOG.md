...
- **`request-document-explicit-actor-readiness.js`** — Wave 24 literal-on projection/write boundary. Off preserves the legacy Request Document shape; once this runtime reaches Vercel Production, off also reports an unhealthy `requestDocumentExplicitActorReadiness` health service until separately approved schema promotion is complete.
