...
- **`final-writeup/dashboard-service.js`** — **[PRODUCTION-DEPLOYED V2-CAPABLE / STORED V1 / PERSONAS DISABLED 2026-08-31.]** The bounded Final Writeups projection preserves the proved broad-Grant-Program superuser matrix. Commit `84bf465b` in Ready deployment `dpl_41SybgPYfJXGarf7UqcMGCLMy4KS` can add PD, PC, Leadership, and overlapping queues from explicit v2 assignments, but the tracked flag remains false and rollout-off performs no staffing read or response-shape change. Signed-in post-deploy smoke proved the Research matrix still renders. Flag-on v1/missing/unassigned/removed-role cases return no persona rows with bounded warnings rather than broadening access.
- **`final-writeup/matrix-audience-service.js`** — **[PRODUCTION-DEPLOYED V2-CAPABLE / STORED V1 2026-08-31.]** Strict v1/v2 `final_writeup.matrix_audiences` contract. V1 remains matrix-readable; only complete canonical v2 staffing replacements are writable. V2 combines explicit GUID-only persona/no-lens rows with broad Grant Program reviewer audiences under one ETag. Publication requires every current enabled reviewer-role member and rejects unknown/stale/incomplete references; runtime intersects stored references with current roles/programs, omits stale identities with warnings, and never treats program audience as persona authority. The Admin read produces the v1→v2 draft; dry-run-first ETag upgrade/repair/downgrade tooling supplies the out-of-band malformed-value path. Signed-in Production readback proved the stored v1 value has nine Research and six Southern California reviewers; no migration or PUT ran.
