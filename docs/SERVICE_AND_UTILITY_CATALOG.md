...
- **`review-manager/materials-send-repair-receipt.js`** — Fifteen-minute `NEXTAUTH_SECRET`-signed proof of a successful materials dispatch, binding request, suggestion, rendered due date, dispatch timestamp, nonce, and source ETag. The exact HMAC-verified `(suggestionId, materialsSentAt, effectiveReviewDueDate)` tuple is the repair idempotency key.
- **`review-manager/repair-materials-send-service.js`** — No-resend recovery for a dispatched materials email whose inline lifecycle stamp failed. Accepts only the signed dispatch receipt, re-checks fresh-row lifecycle/request state, returns `already_recorded` for the exact signed tuple, rejects older receipts, and applies a newer signed dispatch with the row's fresh ETag while preserving set-once `atSend` and advancing `lastSent`.
...
- **`review-upload.js`** — Shared `writeReviewFiles` core for staff + reviewer-self upload paths; shared receipt authorization, unique per-attempt SharePoint folders persisted on the winning row, ETag-bound Dataverse write, and conflict rollback limited to the losing attempt (with winning-item identity exclusion).
