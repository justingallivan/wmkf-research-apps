...
- **`final-writeup/dashboard-service.js`** — **[PRODUCTION-LIVE V1 + SOURCE-BUILT PERSONAS DISABLED 2026-08-31.]** The bounded Final Writeups projection preserves the proved broad-Grant-Program superuser matrix. The current branch can add PD, PC, Leadership, and overlapping queues from explicit v2 assignments, but the tracked flag remains false and rollout-off performs no staffing read or response-shape change. Flag-on v1/missing/unassigned/removed-role cases return no persona rows with bounded warnings rather than broadening access.
- **`final-writeup/matrix-audience-service.js`** — **[PRODUCTION-LIVE V1 + SOURCE-BUILT V2 2026-08-31.]** Strict v1/v2 `final_writeup.matrix_audiences` contract. V1 remains matrix-readable; only complete canonical v2 staffing replacements are writable. V2 combines explicit GUID-only persona/no-lens rows with broad Grant Program reviewer audiences under one ETag. Publication requires every current enabled reviewer-role member and rejects unknown/stale/incomplete references; runtime intersects stored references with current roles/programs, omits stale identities with warnings, and never treats program audience as persona authority. The Admin read produces the v1→v2 draft; dry-run-first ETag upgrade/repair/downgrade tooling supplies the out-of-band malformed-value path. Production has not yet deployed or stored v2.
- **`final-writeup/persona-service.js`** — **[SOURCE-BUILT / ROLLOUT DISABLED 2026-08-31.]** Server-only viewer resolution from the published v2 assignment and current direct reviewer-role membership. Disabled mode returns the legacy empty contract without reading the setting or roster; explicit no-lens, missing row, removed-role, flag-on v1, and overlap semantics are separately tested.
