...
- **`final-writeup/dashboard-service.js`** — **[PRODUCTION-LIVE ORDINARY FOUNDATION 2026-08-31; SUPERUSER MATRIX SOURCE-BUILT + LOCAL SIGNED-IN VERIFIED AGAINST PRODUCTION READS, NOT DEPLOYED.]** Read-only Final Writeups projection. It requires the same Wave 23 readiness boundary and an enabled session-derived `systemuser`, caps the current-Final request set at 100, batch-reads exact current Final rows and acknowledgement rows in chunks of 25, rejects capped/ambiguous/inconsistent state, and refreshes deduplicated stable Graph identities with concurrency four. It server-derives stage, lead-PD relationship, personal freshness, positive initials, open/history/stewardship membership, and optional focused previous/next navigation. Acknowledgement existence—not later Word edits—owns history membership. A fresh-superuser index branch derives the bounded exact enabled `WMKF Final Writeup Reviewer` roster and returns the complete neutral matrix; ordinary/focused responses return none. The GUID-only multi-persona team resolver is rollout-disabled and performs no read. There is still no leadership lens, supporting-material data projection, or write path. Signed-in Production smoke covers the ordinary/focused foundation; signed-in local desktop/mobile QA against Production reads covers the not-yet-deployed matrix.
