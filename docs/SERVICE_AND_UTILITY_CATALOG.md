...
- **`pre-site-visit/proposal-core-service.js`** — Guarded Pre-Site proposal-core input/execution helper. It requires the exact active `AI Materials/ProposalNarrative_{Request#}.pdf`, composes authoritative request/PI/Co-PI context from Dataverse, and calls `pre-site-visit.proposal-core.generate` with bounded context and narrative overrides plus exported system assertions. It returns eight validated narrative fields, document metadata, and the narrative source identity to the durable artifact writer. The bibliography is deliberately excluded and remains reserved for next-cycle Reviewer Finder.
