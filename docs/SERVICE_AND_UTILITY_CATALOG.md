...
- **`pre-site-visit/artifact-service.js`** — Durable Pre-Site Request Document producer. **[VERIFIED IN SOURCE/FOCUSED TESTS AND READ-ONLY LIVE PROMPT COMPARISON 2026-08-17; PRODUCTION GENERATION NOT YET PROVED.]** It resolves and fingerprints the exact narrative before side effects, requires a governed prompt declaring exactly the context and `proposal_text` variables, claims/reclaims one deterministic Wave 19 row, persists all eight named fields plus immutable generated/input snapshots and prompt/run provenance, rereads Dataverse before rendering, uploads to `Artifacts/Pre-Site Visit/`, and atomically activates the Ready Word row with `akoya_request.wmkf_CurrentPreSiteVisit`. Exact Ready retries avoid Claude/render/upload; bibliography changes do not affect the generation identity; persisted-output retries avoid another Claude call; matching post-upload recovery reuses the same item; prompt/claim/cleanup races fail closed. Sole-current prompt v3 already matches the narrative-only runtime contract.
