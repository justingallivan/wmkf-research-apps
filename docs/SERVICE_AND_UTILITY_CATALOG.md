...
- **`institution-affiliation-assessment.js`** — Shadow-only source/currentness-aware organization relationship and total per-consumer policy contract. It preserves additional affiliations, distinguishes same/parent-child/sibling/related/distinct/unresolved, applies conditional neutrality only with an explicit affiliation-excluding identity result, and fails closed for unknown high-authority inputs. No production caller consumes it.
...
- **`ror-affiliation-assertion-resolver.js`** — Shadow-only assertion resolver that reuses verdict-free ROR candidates and the local decision contract while retaining source/canonical ids and one result per explicit organization segment. Partial provider failure preserves successful peers and never becomes `distinct`; it has benchmark/test callers only.
