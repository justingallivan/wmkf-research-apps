...
- **`email-automation-preferences.js`** — Strict reader for the reserved per-PD `email_automation` preference. Absence remains an explicit unconfigured rollout state; malformed persisted JSON fails closed instead of silently becoming automatic send.

### Storage / persistence

- **`database-service.js`** — Vercel Postgres operations; Wave 1 dispatch lives here.
- **`scheduled-email-store.js`** / **`scheduled-email-service.js`** — Source-built P0 personalized-email ledger/coordinator (migration 036 not applied). Freezes exact recipient/sender/draft/schedule state, versions PD edit/approve/stop actions, leases notification/send work, mints secure links only at real send, persists or correlation-recovers the Dynamics activity before transport, and repairs sent-but-unfinalized Dataverse state without re-sending.
