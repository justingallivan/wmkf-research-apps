...
- **`scheduled-email-store.js`** / **`scheduled-email-service.js`** — Source-built personalized-email ledger/coordinator plus per-(PD, contact) VIP flags (migration 036 applied 2026-08-26, branch code not yet deployed; VIP/digest decision layer per `docs/SCHEDULED_EMAIL_VIP_DIGEST_PLAN.md`). Freezes exact recipient/sender/draft/schedule/approval state at row creation, versions PD edit/approve/stop actions, refuses due-send claims on unapproved approval-required rows (send_now is the only bypass), builds the per-PD daily digest with receipted FYIs (claimed through the `scheduled_email_digest_runs` lease/membership ledger since the 2026-08-26 adversarial-review fixes), rebuilds unsent rows under the request's current PD on handoff, mints secure links only at real send, persists or correlation-recovers the Dynamics activity before transport, and repairs sent-but-unfinalized Dataverse state without re-sending. The store also holds the per-(lead PD, reviewer person) invitation VIP flags (`scheduled_email_reviewer_vip_flags`, person-keyed; synchronous Invite-Reviewers routing aid only, no ledger consumer). [RECHECKED after lib/services/scheduled-email-store.js change: reviewer VIP flag helpers added 2026-08-26] [RECHECKED after lib/services/scheduled-email-store.js change: claimDigestRun/reassignScheduledEmail added 2026-08-26] [RECHECKED after lib/services/scheduled-email-store.js change: cancelScheduledEmailBySource/deferScheduledEmailSend/reviveStoppedScheduledEmail/refreshUntouchedScheduledEmail added on `feature/reviewer-cron-reminders-ledger` 2026-08-27] [RECHECKED after lib/services/scheduled-email-service.js change: per-workflow `strategyFor` dispatch (grantee inline + reviewer reminder strategies) with defer/stop eligibility verdicts on `feature/reviewer-cron-reminders-ledger` 2026-08-27]
- **`reviewer-reminder-eligibility.js`** — SOURCE-BUILT on `feature/reviewer-cron-reminders-ledger` (not on main). Shared reviewer-reminder eligibility primitives extracted from the manual-reminder service: `readReminderSuggestion` (fresh `wmkf_reviewersuggestion` read, 404 → `row: null`), the shared/respond/review-due refusal predicates, and the review-status constants. Also home to `loadReminderReviewer`, the single reviewer-email resolver shared by the sweeps, the manual path, and send-time recipient revalidation. Consumed by both the manual send path and the ledger workflow strategies so cron and manual sends can never diverge on what "refused" means. [RECHECKED after lib/services/reviewer-reminder-eligibility.js change: loadReminderReviewer extracted 2026-08-27] [RECHECKED after lib/services/reviewer-reminder-workflows.js change: marker-gated expiry exemption + recipient revalidation 2026-08-27] [RECHECKED after lib/services/reviewer-reminder-sweep.js change: resolver re-export only 2026-08-27]
- **`reviewer-reminder-workflows.js`** — SOURCE-BUILT on `feature/reviewer-cron-reminders-ledger` (not on main). Ledger strategy implementations for `reviewer_respond_reminder` / `reviewer_reviewdue_reminder`: send-time recompute from live per-request config (config drift → defer verdict; config off → stop), marker-without-CLAIM → permanent stop (a Dataverse reminded-marker stops the row only when the row never stamped `claim_committed_at`; an owned marker resumes the send), respond token-expiry stop (skipped only when the fresh Dataverse marker AND `claim_committed_at` are both present with no activity — the marker proves the claim PATCH landed and the token is the row's own rotation; a claim alone is pre-PATCH intent), send-time recipient revalidation for rows with no activity (frozen `to_recipients` vs the reviewer's current email via the shared `loadReminderReviewer`; mismatch/unresolvable fails closed to stop `recipient_changed`, healed by sweep revive with fresh recipients), and `buildActivityInput` that fuses the Dataverse reminded-marker claim + fresh token mint into one If-Match `mintAndStore` PATCH inside activity creation (claim-before-send; never re-mints over an existing activity). PD send-now `force` overrides timing only, never hard eligibility. [RECHECKED after lib/services/reviewer-reminder-workflows.js change: claim-ownership discrimination added 2026-08-27 from the Codex adversarial-review high finding]
- **`graph-service.js`** — Microsoft Graph (SharePoint files, listing/download, exact historical-version download, current-item PDF conversion, content search, idempotent folder-path creation, stable path-to-item metadata, stable drive/item-ID metadata readback, and upload responses carrying site/drive/item/eTag/version metadata). Historical-version and conversion downloads manually follow Graph's pre-authenticated redirect without forwarding the bearer token.
...
- **`pre-site-visit/docx-renderer.js`** — Template-preserving OOXML renderer for the tracked Phase II Pre-Site Word template. Replaces split-run Dataverse/AI placeholders across body, tables, and headers while retaining the complete package, page structure, PAGE field, and PD-owned graphical-abstract/recommendation/review/funding-history slots. Production template v2 added the same 0.1-inch left cell padding to Recommendation used by the amount values and created a distinct Ready artifact. Its Word Online render exposed a width-sensitive Recommendation-label alignment defect. **[INFERRED FROM SCREENSHOT + OOXML WIDTH]** implicit wrapping was the remaining layout variable. **[DEPLOYED TO PRODUCTION 2026-08-17; SIGNED-IN FEATURE SMOKE OPEN]** template v3 makes that label explicitly non-wrapping. **[DEPLOYED TO PRODUCTION 2026-08-18; SIGNED-IN GENERATION SMOKE OPEN]** template v4 retains the no-wrap contract and changes only the first-page metadata rhythm: one blank line after Project Title, compact single-spaced Meeting Date/Staff Lead/Recommendation rows, and one blank line ending at the divider. **[DEPLOYED TO PRODUCTION 2026-08-18; SIGNED-IN GENERATION SMOKE OPEN]** render contract v5 keeps the exact v4 template bytes but renders any non-empty long-form paragraph count and treats exact roster-name underlining as best effort. Each contract version participates in the generation key, so later generation cannot collide with earlier Ready documents.
