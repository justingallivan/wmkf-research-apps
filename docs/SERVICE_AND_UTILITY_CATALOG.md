...
- **`pre-site-visit/reopen-service.js`** — Superuser-only guarded correction service (source-built on `codex/guarded-reopen`; Wave 20 not yet applied and runtime not deployed). It verifies the current Ready/Review pointer and exact unchanged handoff bytes, blocks a Final or any derived Request Document, creates/reclaims one client-UUID-bound Ready/Draft successor and unique SharePoint copy, then atomically supersedes the preserved Review row and moves the request pointer under ETags. Successor audit fields plus existing lineage and standard actor/time form the append-only event; exact retry and post-upload recovery reuse the same row/item. Later Pre-Site generation is salted with the correction cycle so it cannot reactivate the preserved generation.
