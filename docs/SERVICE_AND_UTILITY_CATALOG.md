...
- **`final-writeup/dashboard-service.js`** — **[PRODUCTION-LIVE + SIGNED-IN READ SMOKE PASSED 2026-08-31.]** Commit `52575761` and Ready deployment `dpl_Frc6fAonyFFYwiWyFJCzzE3UNune` ship the read-only Final Writeups projection and superuser matrix. It requires the same Wave 23 readiness boundary and an enabled session-derived `systemuser`, caps the current-Final request set at 100, batch-reads exact current Final rows and acknowledgement rows in chunks of 25, rejects capped/ambiguous/inconsistent state, and refreshes deduplicated stable Graph identities with concurrency four. It server-derives stage, lead-PD relationship, personal freshness, positive initials, open/history/stewardship membership, and optional focused previous/next navigation. Acknowledgement existence—not later Word edits—owns history membership. A fresh-superuser index branch derives the bounded exact enabled `WMKF Final Writeup Reviewer` roster and returns the complete neutral matrix; ordinary/focused responses return none. Signed-in Production DOM proof showed the exact 11-person roster and Request `1002788`, with Duncan Spore Reviewed, Justin Gallivan Responsible PD, all other cells Not reviewed, both direct actions, and zero browser-console errors; prior local desktop/mobile QA also passed. The GUID-only multi-persona team resolver is rollout-disabled and performs no read. There is still no leadership lens, supporting-material data projection, or write path.
