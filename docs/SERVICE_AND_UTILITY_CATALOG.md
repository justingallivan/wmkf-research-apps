...
- **`pre-site-visit/artifact-service.js`** — Durable Pre-Site Request Document producer. **[VERIFIED IN PRODUCTION 2026-08-17.]** It resolves and fingerprints the exact narrative before side effects, requires a governed prompt declaring exactly the context and `proposal_text` variables, claims/reclaims one deterministic Wave 19 row, persists all eight named fields plus immutable generated/input snapshots and prompt/run provenance, rereads Dataverse before rendering, uploads to `Artifacts/Pre-Site Visit/`, and atomically activates the Ready Word row with `akoya_request.wmkf_CurrentPreSiteVisit`. Request `1002379` created Ready row `aeb223a2-849a-f111-b8db-70a8a59cded0`, completed governed v3 run `ba0f42b9-849a-f111-b8db-6045bd008868`, and registered stable item `01G4GVMS3Q5BJ65S7DDZDKFTSQLIQAIPER`; exact Ready retry reused those identities. Bibliography changes do not affect generation identity; persisted-output retries avoid another Claude call; matching post-upload recovery reuses the same item; prompt/claim/cleanup races fail closed.
- **`pre-site-visit/docx-renderer.js`** — Template-preserving OOXML renderer for the tracked Phase II Pre-Site Word template. Replaces split-run Dataverse/AI placeholders across body, tables, and headers while retaining the complete package, page structure, PAGE field, and PD-owned graphical-abstract/recommendation/review/funding-history slots.
