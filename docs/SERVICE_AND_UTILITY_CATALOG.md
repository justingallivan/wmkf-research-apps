...
- **`final-writeup/dashboard-service.js`** — **[PRODUCTION-LIVE + SIGNED-IN PROGRAM-AUDIENCE READ PROVED 2026-08-31.]** Commit `5573bca3` is live in Ready deployment `dpl_5DNuc2BV76RihwuWu8ZFYBgxBXE7`. The bounded Final Writeups projection includes each request's broad `wmkf_grantprogram` only in matrix grouping and resolves the superuser audience through `matrix-audience-service.js`. Request `1002788` rendered under Research with the exact nine published reviewer columns, excluding Anneli Stone and Saskia Pallais, and zero browser-console errors. Unconfigured programs are explicit; ordinary/focused responses receive neither matrix nor configuration. There is still no leadership lens, supporting-material projection, or dashboard write path.
- **`final-writeup/matrix-audience-service.js`** — **[PRODUCTION-LIVE + SIGNED-IN ADMIN WRITE/READBACK PROVED 2026-08-31.]** Strict versioned `final_writeup.matrix_audiences` settings contract for the superuser coordinator matrix. The published Research audience contains nine current reviewer-role members and excludes owner-confirmed Southern California staff Anneli Stone and Saskia Pallais; Southern California remains unconfigured pending its complete audience. The Admin publication/readback survived a full reload. The service stores only broad Grant Program and exact enabled reviewer-role `systemuser` GUIDs, resolves names live, rejects empty/duplicate/over-cap/unknown references, fails runtime closed on stale programs or reviewers, and exposes stale references only to the superuser Admin repair surface. Full-replacement publishes use the loaded Dataverse revision as `If-Match`; stale Admin drafts receive `409` rather than overwriting newer changes.
