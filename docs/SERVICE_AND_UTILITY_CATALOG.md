...
- **`workbench-proposal-documents.js`** — Workbench document listing plus the strict shared AI-input resolver. Initial Assessment and request-bound Field Primer accept only the exact active `AI Materials/ProposalNarrative_{Request#}.pdf` under a positively resolved Dynamics-tracked request folder; reviewer packages, Phase I files, archives, neighbors, and ambiguous active buckets do not substitute.
...
- **`pre-site-visit/proposal-core-service.js`** — Guarded Pre-Site proposal-core producer, consumed by the authenticated Workbench direct-download route. Loads the exact active `AI Materials/ProposalNarrative_{Request#}.pdf`, composes authoritative request/PI/Co-PI context from Dataverse, and calls `pre-site-visit.proposal-core.generate` with the exported system assertions plus `requireNoPersistence:true`. It returns eight validated narrative fields and document metadata; it does not write a business field or upload an artifact. **[VERIFIED 2026-08-16 via live exact prompt readback and controlled Request `1002379` execution]** sole-current prompt v3 uses reviewed `claude-sonnet-4-6`; the latest audited Executor run remains accepted v2 run `5bd65180-ed99-f111-b8db-7ced8d6e2f44`. A direct exact-v3 model/render QA produced a four-page DOCX with correct underlined roster names and personnel rules, but Methodology's final sentence spilled to page 4, so the one-page Background/Methodology target remains a prompt-tuning follow-up. The branch/direct-download route remains unpromoted and no durable artifact was written.
