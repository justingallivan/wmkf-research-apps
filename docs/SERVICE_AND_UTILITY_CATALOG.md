...
- **`pre-site-visit/docx-renderer.js`** — Template-preserving OOXML renderer for the tracked Phase II Pre-Site Word template. Replaces split-run Dataverse/AI placeholders across body, tables, and headers while retaining the complete package, page structure, PAGE field, and PD-owned graphical-abstract/recommendation/review/funding-history slots. Production template v2 adds the same 0.1-inch left cell padding to Recommendation used by the amount values; its version participates in the generation key so it cannot overwrite a v1 Ready document. Signed-in v2 generation proof remains open.
