...
- **`reviewer-identity-runtime.js`** — Server-owned W2 runtime seam for non-biomedical/PubMed-off Track-A verification. Default and unknown modes return the legacy identity result exactly; `shadow` settles the full legacy batch first, then runs hard-bounded, redacted comparisons with non-throwing observers; the default observers also persist each comparison durably (per-batch run id) via `reviewer-identity-shadow-log.js`. No cutover mode exists.
- **`reviewer-identity-shadow-log.js`** — Best-effort Postgres persistence for shadow comparisons (`reviewer_identity_shadow_log`, migration 026). Whitelisted redacted scalars only (hashed candidate key, decisions, reason, anchors-agree, error code); never awaited on the request path, never throws, circuit-breaks on repeated insert failure. Non-authoritative: no application reader; retention/cap cleanup runs in the daily maintenance cron.
- **`reviewer-works-first.js`** — Shared works-first resolver and overlay-free anchor canonicalizer used by the frozen W2 evaluator and runtime shadow arm. Uses byline-filtered works plus W0 institution identities; same-ORCID fragments may collapse, while every distinct-ORCID set goes to review.
