...
- **`initial-assessment/controls-service.js`** — Source-built superuser restore and Board-freeze coordinator (2026-08-30; not Production-proved). Restore resolves the request pointer/stable item server-side, narrows stale races with exact version/metadata fences, invokes native Graph restore, requires byte-for-byte agreement with the selected version, ETag-patches the registry, and reconciles a lost response without a duplicate restore. Board freeze creates/reclaims a deterministic distinct Ready/Board Ready row/item, uses create-only path upload for the exact selected source buffer, then verifies normalized governed Word content after SharePoint ingestion (including concurrent-winner reuse), links exact source row/version/hash, never moves the canonical pointer, and recovers interrupted governed-equivalent uploads. Different governed content and invalid DOCX packages fail closed; only items uploaded by the owned attempt can be recorded for cleanup.
