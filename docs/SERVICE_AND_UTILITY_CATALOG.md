...
- **`final-writeup/acknowledgement-service.js`** — **[PRODUCTION-LIVE 2026-08-31; FIRST-ROW RETRY PENDING.]** Wave 23 mark/read coordinator behind a distinct literal-on readiness interlock. It resolves the current Final and responsible PD server-side, validates the session-derived enabled `systemuser`, rejects PD self-review, records one stable-ID Graph publication observation, preserves same-version acknowledgement time, conditionally replaces later versions with `If-Match`/`noFallback`, and rereads ambiguous outcomes. Publication version owns freshness; eTag and last-modified remain diagnostic. Production readiness is exact `on` in Ready deployment `dpl_B9k3AprnYp5ExpkqpT3dUxCUZqWo`. An eligible colleague's first POST reached Dataverse but failed on missing acknowledgement Create; the reread confirmed no partial row. The dedicated role is now assigned/effective for the complete 11-person audience, and the colleague retry/readback remains open.
