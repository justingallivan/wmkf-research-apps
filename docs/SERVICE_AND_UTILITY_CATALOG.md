...
- **`final-writeup/transition-service.js`** — **[BUILT IN SOURCE 2026-08-30; WAVE 22 EXACT IN PRODUCTION, FLAG OFF, NOT DEPLOYED.]** Governed same-item Site Visit→Final Writeup group-review transition. It requires a session-derived actor and server-derived lead-PD/superuser authorization, verifies the current Ready/Review Pre-Site pointer plus one stable current Graph publication version and governed DOCX hash, and claims/reuses one deterministic Final row with the same drive/item identity. One three-ETag changeset moves the source to lifecycle Final, readies the Final row in Review with explicit group-review actor/time, retains Current Pre-Site, and sets Current Final. Exact retry, live lease, source/version races, atomic rejection, and ambiguous-response recovery are focused-test covered. It has no Graph upload/copy, acknowledgement, dashboard, leadership transition, or backward-stage action.
