...
- **`pre-site-visit/proposal-core-service.js`** — Guarded Pre-Site proposal-core producer, consumed by the authenticated Workbench direct-download route. Loads the exact active `AI Materials/ProposalNarrative_{Request#}.pdf`, composes authoritative request/PI/Co-PI context from Dataverse, and calls `pre-site-visit.proposal-core.generate` with the exported system assertions plus `requireNoPersistence:true`. It returns eight validated narrative fields and document metadata; it does not write a business field or upload an artifact. **[VERIFIED 2026-08-16 via live exact prompt readback and controlled Request `1002379` execution]** sole-current prompt v2 uses reviewed `claude-sonnet-4-6`; completed run `5bd65180-ed99-f111-b8db-7ced8d6e2f44` produced a structurally and visually accepted four-page DOCX. The branch/direct-download route remains unpromoted and no durable artifact was written.
