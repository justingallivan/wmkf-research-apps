...
|---|---|---|
| **2 — narrow shared policy** | **COMPLETE 2026-09-05 (PR #155 → `716bc558`; Opus PASS, Codex approve; plan `../REVIEWER_LIFECYCLE_STAGE2_BUILD_PLAN.md`).** `shared/utils/reviewer-engagement-policy.js` owns the correction-source / closed-engagement predicates; the other `REVIEW_STATUS_MAP` sets (materials, receipt, terminal, rollup) deliberately stay separate. | Raw correction source/closed sets at `lib/services/reviewer-finder/my-candidates-service.js:51` and `lib/dataverse/adapters/reviewer-suggestion.js:77`; exact guards at service `:697` and adapter `:1890` passed the 30-case comparison below. Preserve raw/DTO separation, missing versus explicit null, completion markers and each caller's error contract. |
| **3 — closeout command pilot** | **COMPLETE 2026-09-05 (3A, PR #154 → `b7a04cd6`; Opus PASS, Codex two rounds; plan `../REVIEWER_LIFECYCLE_STAGE3_BUILD_PLAN.md`).** | Auth/request binding already reaches `close-review-service.js:101` from `pages/api/review-manager/close-review.js:49`. Composed version/repeat/correction/invalid-input tests exist at `tests/integration/reviewer-engagement-contract.test.js:226`. The historical report sequences Stage 2 first, but this move has no hard technical dependency on a new policy module. Preserve original exports/error identity and prove both import paths reach the moved implementation. |
| **3 — command expansion** | **COMPLETE 2026-09-06 (all slices merged): 3B terminal transition COMPLETE (PR #156 → `4e7c378c`); 3C status correction COMPLETE (PR #158 → `1c24e56f`, 2026-09-06); 3D response correction COMPLETE (PR #160 → `081d558b`, 2026-09-06); 3E expire/post-send bookkeeping COMPLETE (PR #161 → `01072571`, 2026-09-06). Expansion slices 3F–3K are planned in `../REVIEWER_LIFECYCLE_STAGE7_BUILD_PLAN.md`; 3G COMPLETE (PR #162 → `ec74c0d4`); 3H COMPLETE (PR #163 → `81fdac43`); 3I COMPLETE (PR #164 → `d47b07be`); 3F COMPLETE (PR #165 → `68198b2f`); 3J COMPLETE (PR #166 → `3b8dca2b`); 3K COMPLETE (PR #167 → `19955148`). All Stage 3 slices 3A–3K are merged; Stage 7 is complete (PR #168).** | Withdrawal/delete and release differ (`lib/services/review-manager/terminal-transition-service.js:95`); correction retains write → nonfatal token follow-up → person edits (`my-candidates-service.js:708`, `:729`, `:743`). Fixed expiry and post-send bookkeeping exist (`lib/services/reviewer-suggestion-sweep.js:124`; `lib/services/review-manager/send-emails-service.js:127`). Add the caller-boundary census before command two. |
...
| **5 — narrow document-pointer and thank-you operations** | **COMPLETE 2026-09-05 (PR #157 → `21cc221b`; Opus PASS with one required shim, Codex two rounds → approve; plan `../REVIEWER_LIFECYCLE_STAGE5_BUILD_PLAN.md`).** `attachReviewDocumentPointer` and `claimThankYou` on the suggestion adapter require a concrete ETag (`lib/utils/etag.js` `isConcreteEtag`, 400 `missing_version`); `individual-file-service.js` `commitPointers` and `reviewer-thankyou-sweep.js` no longer call the generic receipt patch, pinned by caller wiring tests. Four inline ETag regexes remain as a consolidation follow-up. | Pointer conditional write/readback/retry is defined (`lib/services/review-documents/individual-file-service.js:652`); thank-you builds then claims then sends (`lib/services/reviewer-thankyou-sweep.js:58`). The real-adapter completed-review test covers both plus second-receipt rejection (`reviewer-engagement-contract.test.js:351`). These operations must remain legal after receipt/Complete. No dependency on the optional adapter split or receipt helper. |
| **5 — shared receipt-persistence helper** | **SKIPPED 2026-09-05 (recorded in the Stage 5 plan §5B): the four receipt sinks differ in version optionality and payload shape; a shared helper would collapse those differences.** | External legacy setVersion remains optional (`lib/services/external-review/submit-service.js:134`), staff version mandatory (`lib/services/review-manager/manual-review-entry-service.js:141`), no-file data partial/empty (`lib/services/review-manager/mark-received-no-file-service.js:81`), upload outcomes file-specific (`lib/services/review-upload.js:263`). Define receipt kind, allowed fields/snapshots, authorized version and error ownership; retain `lib/services/review-receipt-guard.js:32`. |
...
| **6C — UI extraction** | **COMPLETE 2026-09-05 (PR #153 → `3b2b34d5`).** | `TokenActionsMenu`/`TokenStateBadge` → `TokenActionsMenu.js`; `ReviewReminderAction` → `ReviewReminderAction.js`; `ReleaseMaterialsModal` + `PREVIEW_RENDER_TIMEOUT_MS` → `ReleaseMaterialsModal.js`; `membershipKeyFor`/`proposalKeyFor` → `reviewer-draft-keys.js`; every named export re-exported from `ReviewerManagePanel.js`. Pure move verified by Opus (PASS) and Codex; plan `docs/REVIEWER_LIFECYCLE_STAGE6C_BUILD_PLAN.md`. Historical routing text below (`ReviewerManagePanel.js:126`, `:531`) is pre-6C. |
| **7 — close arbitrary bypasses** | **COMPLETE 2026-09-06 (PR #168 → `790ba3a1`; Opus PASS with one required item fixed; Codex two rounds, remaining shapes closed under the cap and architect-verified; plan `../REVIEWER_LIFECYCLE_STAGE7_BUILD_PLAN.md`).** `check:reviewer-engagement-boundary` (LAW mode over `lib/pages/shared/modules`) fails any generic suggestion-writer binding outside `lib/services/reviewer-engagement/`, the adapter, or the two recorded receipt sinks (pinned by a tracked test); `bulkUpdateByRequest` deleted; expiry writes through the named `expireInvitationResponse` op. Scripts are recorded, not gated (owner decision D5). Owner review 2026-09-06 (S490): D0 and D3 taken (PR #170), D1 preserved, D4(a) chosen but its route branch has no client caller, D2/D5 open — see the Stage 7 plan decisions table. | Generic aliases are still live at `reviewer-suggestion.js:1370`; the census below is a lower bound. Narrow only after caller/descriptor/script coverage is complete. Keep legitimate specialized persistence operations and live compatibility wrappers. Stage 4 may be skipped. |
