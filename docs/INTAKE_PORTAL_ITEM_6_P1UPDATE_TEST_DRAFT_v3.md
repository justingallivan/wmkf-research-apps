...
| Reactivation path (`statecode`->Active) | IN | §5.9 | It proves the same trigger/filter path handles the reverse state transition and reveals whether the production flow needs direction-aware logic. |
| Pre-submit / drain-window gate-false case | IN | §5.7, §5.12 | The parent-status filter is only useful if it suppresses child updates before `Phase II Pending`. |
| Multiple children under one parent, partial deactivation, active-only recompute input | IN | §5.10 | The flow must recompute over active child rows only after one obsolete row is deactivated. |
| Bulk / batch deactivation | IN, limited | §5.11 | The gate needs a multi-row stress check. This runbook uses rapid deterministic PATCHes, not a drain `$batch` implementation test. |
| Concurrent / rapid edits | IN, limited | §5.11 | The gate needs child-GUID attribution and duplicate/retry disambiguation under rapid adjacent updates. Full concurrency correctness belongs to the production flow design. |
| Parent status transitions into/out of `Phase II Pending` | IN | §5.12 | The trigger filter must evaluate the parent's status at child-update time, not merely save a syntactically valid expression. |
| `SdkMessage` identity for deactivation and reactivation | IN | §5.8, §5.9 | The production flow can only make deterministic decisions if the literal is exactly `Update`. |
| Trigger `Filter rows` vs `Select columns` semantics for statecode-only change | IN | §5.13 | A statecode-only deactivation can be accidentally filtered out by Select columns. |
...
| Full production aggregate-field math and parent PATCH | OUT; deferred | N/A | This runbook proves the trigger path and active-child input set; the production flow's final arithmetic/PATCH belongs to flow implementation testing. |
...
| `[BUSINESS_SELECT_COLUMNS]` | Comma-separated business columns for §5.13, for example `wmkf_amount,wmkf_category,wmkf_year,wmkf_description`. |
...
## 5.9 Reactivation symmetry under Phase-II parent

Purpose: prove the reverse state transition also binds as a child Update under the same parent-status gate.
...
Pass criterion: `[REACTIVATE_RUN_COUNT]` = 1, `[REACTIVATE_SDK_MESSAGE_LITERAL]` is exactly `Update`, and `[REACTIVATE_PARENT_LOOKUP_MATCH]` = yes.

Fail criterion: `[REACTIVATE_RUN_COUNT]` != 1, `[REACTIVATE_SDK_MESSAGE_LITERAL]` is not exactly `Update`, or parent lookup does not match `[PARENT_B_GUID]`.

---

## 5.10 Active-subset recompute input after partial deactivation

Purpose: prove that after one obsolete child is deactivated, the flow can recompute from active siblings only.
...
20. Record `[ACTIVE_SUBSET_OBSERVED_AMOUNT_SUM]` from returned active rows if `[AMOUNT_COLUMN]` is available; otherwise record `N/A -- amount column not available on proxy`.

Pass criterion: `[ACTIVE_SUBSET_RUN_COUNT]` = 1, `[ACTIVE_SUBSET_SDK_MESSAGE_LITERAL]` is exactly `Update`, returned active GUIDs are exactly `[CHILD_C1_GUID]` and `[CHILD_C3_GUID]`, `[CHILD_C2_GUID]` is absent from the active set, and `[ACTIVE_SUBSET_OBSERVED_AMOUNT_SUM]` equals `[ACTIVE_SUBSET_EXPECTED_AMOUNT_SUM]` when `[AMOUNT_COLUMN]` is available.

Fail criterion: deactivated `[CHILD_C2_GUID]` appears in the active set, an active sibling is missing, the run count is not 1, `SdkMessage` is not exactly `Update`, or the observed sum is wrong when amount evidence is available.

---

## 5.11 Rapid multi-child deactivation stress check

Purpose: prove child-GUID attribution and duplicate/retry handling under rapid adjacent statecode-only Updates. This is a limited pre-deploy stress check, not a true `$batch` drain implementation test.
...
Pass criterion: `[RAPID_D1_RUN_COUNT]` = 1, `[RAPID_D2_RUN_COUNT]` = 1, both `SdkMessage` literals are exactly `Update`, and each run's trigger child ID matches the expected child.

Fail criterion: either child has 0 runs, either child has an unresolved duplicate true fire, any `SdkMessage` literal is not exactly `Update`, or child-GUID attribution is ambiguous.

---

## 5.12 Parent status transition timing checks

Purpose: prove the parent-status filter evaluates at child-update time for both transition directions around the drain boundary.
...
Pass criterion: `[TRANSITION_IN_FALSE_RUN_COUNT]` = 0, `[TRANSITION_IN_TRUE_RUN_COUNT]` = 1, and `[TRANSITION_IN_TRUE_SDK_MESSAGE_LITERAL]` is exactly `Update`.

Fail criterion: the child update fires before the parent enters Phase II Pending, does not fire after the parent enters Phase II Pending, or returns any `SdkMessage` literal other than exactly `Update`.
...
Pass criterion: `[TRANSITION_OUT_FALSE_RUN_COUNT]` = 0.

Fail criterion: the child update fires after the parent has left Phase II Pending.

---

## 5.13 Select-columns interaction for statecode-only changes

Purpose: prove the production Select columns setting will not accidentally suppress statecode/statuscode-only Updates.
...
Run this section only after §5.7 through §5.13 pass. Treat this section as secondary evidence only; it cannot rescue a failed deterministic Web API gate.
...
13. §5.10 evidence: `[ACTIVE_SUBSET_RUN_COUNT]`, `[ACTIVE_SUBSET_SDK_MESSAGE_LITERAL]`, `[ACTIVE_SUBSET_RETURNED_CHILD_GUIDS]`, `[ACTIVE_SUBSET_RETURNED_STATES]`, `[ACTIVE_SUBSET_RETURNED_AMOUNTS]`, `[ACTIVE_SUBSET_EXPECTED_ACTIVE_GUIDS]`, `[ACTIVE_SUBSET_EXPECTED_AMOUNT_SUM]`, `[ACTIVE_SUBSET_OBSERVED_AMOUNT_SUM]`.
14. §5.11 evidence: `[RAPID_D1_RUN_COUNT]`, `[RAPID_D2_RUN_COUNT]`, `[RAPID_D1_RUN_ID]`, `[RAPID_D2_RUN_ID]`, `[RAPID_D1_SDK_MESSAGE_LITERAL]`, `[RAPID_D2_SDK_MESSAGE_LITERAL]`, `[RAPID_RETRY_OR_DUPLICATE_OBSERVATION]`.
...
16. §5.13 evidence: `[SELECT_BUSINESS_RUN_COUNT]`, `[SELECT_STATE_RUN_COUNT]`, `[SELECT_STATE_SDK_MESSAGE_LITERAL]` if present, `[SELECT_STATE_STATUS_RUN_COUNT]`, `[SELECT_STATE_STATUS_SDK_MESSAGE_LITERAL]`.
...
|---|---|
| VERIFIED | Real path; selected candidate saves; §5.7 pass; §5.8 pass; §5.9 pass; §5.10 pass; §5.11 pass; §5.12 pass; §5.13 pass; every required `SdkMessage` literal is exactly `Update`; every counted run's child ID and parent lookup attribution match the expected GUIDs. Clears the S163 P1-Update pre-deploy gate. |
| partial(proxy) | Proxy path; all VERIFIED runtime conditions pass on the proxy pair; no real `wmkf_proposalbudgetline` run has been completed. Result is `[partially verified -- Connor tests this]`; real repeat = P4 post-deploy. |
| FAIL(->Option B, no schema rework) | No candidate saves; any IN-scope executable section fails; any required `SdkMessage` literal is anything other than exactly `Update`; trigger output child row ID or parent lookup attribution is missing/ambiguous; duplicate fire cannot be disambiguated; or Select columns cannot be configured so statecode/statuscode-only updates fire. |

Hard rules:

1. Proxy-path evidence never becomes VERIFIED.
2. A UI-only pass never rescues a Web API failure.
3. Any non-`Update` `SdkMessage` literal in an IN-scope state-change test is FAIL, recorded verbatim.
...
1. Build one flow.
2. Collect metadata in §5.2.
3. Run one gate-FALSE Web API deactivation.
4. Run one gate-TRUE Web API deactivation.
5. Run one reactivation check.
6. Run one active-subset check.
7. Run one rapid two-child stress check.
8. Run two parent-status transition checks.
9. Run three Select-columns subtests.
10. Run the optional UI realism check only after deterministic Web API checks.
11. Expected effort: 75-120 minutes including run-history latency.
