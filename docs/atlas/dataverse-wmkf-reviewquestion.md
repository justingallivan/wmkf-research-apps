...
**Phase status:** Original staff-editable-question epic complete through S305. The multiselect code expansion is deployed (`main` `5282cee8`; production `dpl_7sfTLrMafYPKp7mnYdrEVjs9HmW5`), and the sibling `wmkf_appreviewanswer.wmkf_answervalues` production schema expansion completed first on 2026-07-26. The backward-compatible `review-synthesis.generate` v2 and exact target question-set publications also completed 2026-07-26. The first controlled portal smoke passed question/context/draft/submit/readback and cleanup. A 2026-07-27 follow-up proved the structured staff Manual Review Entry path and exact restoration, but synthesis failed three controlled current-v2 executions on incomplete JSON. The reliability code for terminal stop enforcement, native schema, retained diagnostics, and one bounded retry merged through PR #92 (`ab1d2943`) and reached production on 2026-07-28. Governed prompt publication, the controlled post-fix smoke, the two legacy staff-writer success rehearsals, rollback/republish readiness, and reviewer exposure remain pending.
