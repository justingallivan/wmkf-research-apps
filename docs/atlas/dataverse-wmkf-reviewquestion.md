...
- **The four parent-bound rows (`affiliation`/`impact`/`risk`/`overallRating`) can't be removed via the editor** (Codex Phase C P1-3, fixed): the save route rejects (400) any submitted set missing one of them, derived from `reviewParentColumnByKey`. Soft-deleting one would otherwise drop it from the form AND silently stop its parent-column dual-write until Phase E. The guard lifts when Phase E retires the parent columns.
