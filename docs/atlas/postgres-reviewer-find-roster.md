...
diagnosis built from it are not current proof. The owner-directed Session 393
Fable pass must independently test the observations, authority model, and
proposed repair before implementation.
...
- Applicant-suggested restore is keyed on the complete expected canonical suggestion-key set plus `candidate.enrichedProposalKey` and the current `candidate.applicantEnrichmentCacheVersion`, not the proposal Blob URL. Canonical excluded/saved suggestion keys are subtracted as terminal staff actions; unrelated/non-canonical keys cannot hide a missing expected row. Unversioned and older-version rows refresh once; the successful refresh persists the current version. `load-proposal` uses `addRandomSuffix:true`, so `blobUrl` changes across reloads and is not a stable cache key. A completed batch exposes **Update applicant suggestions** even when its cache is valid, allowing a staff-requested rerun after source data or resolver behavior changes; reruns preserve actor-bound staff confirmations and manual contact. **OPEN stabilization hypothesis (source-supported plus Request `1002912` observation on 2026-07-31):** the roster-only terminal calculation does not cover an older `saved` row stored under a noncanonical `candidate:` key while a later applicant-enrichment run writes a canonical `active` twin, and it does not consult Dataverse invitation/response lifecycle. The July 31 probe associated that shape with Ralph Isberg and Rotem Sorek resurfacing. Their current live state is unknown. Dataverse engagement as an independent terminal input is the proposed repair, not an accepted invariant; Fable must test the authority and complement cases. Do not claim the current canonical-key cache prevents all re-display.
- **OPEN missing-suggestion hypothesis (observed on Request `1002912`, 2026-07-31):** active roster key `suggestion:bb81d1f6-fc68-f111-a826-000d3a306da2` then embedded a Dataverse suggestion id that returned 404 after the earlier Sorek merge. Current source appears able to restore an applicant-origin active row that is outside the request's expected suggestion set, but the live row and causal path must be re-checked. Restricting restore to current server-derived anchors plus later dry-run cleanup is the proposed remedy under Fable review, not an authorized patch.
