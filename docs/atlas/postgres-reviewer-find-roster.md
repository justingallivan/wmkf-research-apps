...
migration 029 and is not a deployment claim. **Incident supplement verified
2026-07-31 PT / 2026-08-01 UTC:** Request `1002912` disproved the unqualified
...
- Applicant-suggested restore is keyed on the complete expected canonical suggestion-key set plus `candidate.enrichedProposalKey` and the current `candidate.applicantEnrichmentCacheVersion`, not the proposal Blob URL. Canonical excluded/saved suggestion keys are subtracted as terminal staff actions; unrelated/non-canonical keys cannot hide a missing expected row. Unversioned and older-version rows refresh once; the successful refresh persists the current version. `load-proposal` uses `addRandomSuffix:true`, so `blobUrl` changes across reloads and is not a stable cache key. A completed batch exposes **Update applicant suggestions** even when its cache is valid, allowing a staff-requested rerun after source data or resolver behavior changes; reruns preserve actor-bound staff confirmations and manual contact. **OPEN stabilization defect (verified Request `1002912`, 2026-07-31):** this roster-only terminal calculation does not cover an older `saved` row stored under a noncanonical `candidate:` key while a later applicant-enrichment run writes a canonical `active` twin. It also does not consult Dataverse invitation/response lifecycle. Ralph Isberg therefore resurfaced despite being selected+invited, while Rotem Sorek resurfaced despite being invited+declined. Dataverse engagement must become an independent terminal input; do not claim the current canonical-key cache prevents all re-display until the golden workflows in `docs/REVIEWER_WORKFLOW_STABILIZATION_DIRECTIVE.md` pass.
- **OPEN missing-suggestion roster row (verified Request `1002912`, 2026-07-31):** active roster key `suggestion:bb81d1f6-fc68-f111-a826-000d3a306da2` embeds a Dataverse suggestion id that returns 404 after the earlier Sorek merge. Current display restoration can still render an applicant-origin active row that is no longer in the request's expected suggestion set. The stabilization patch must restrict restored applicant rows to current server-derived suggestion anchors, then a dry-run/backup repair may remove the orphan.
