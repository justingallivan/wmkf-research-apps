...
- `wmkf_reviewsynthesisjson` (Memo, JSON, 20000) — **PROVISIONED IN PROD 2026-07-03 (workbench Reviews tab Phase 4; live-probed selectable). Governed `review-synthesis.generate` v3 became sole-current on 2026-07-28 with the tracked native JSON schema. Version 4 (seven keys: adds `writeupThemes`, `writeupQuotations`) became sole-current on 2026-09-15 UTC via owner-run `--force` republish (row `1349652f-aeb0-f111-aaac-6045bd04539e`), after PR #296 deployed; a 2026-09-15 read-only Production dry-run confirmed v4 remains the sole current row and the live/tracked schema is seven-key. Both a direct controlled smoke and the later controlled automatic lifecycle smoke on Request `1002788` persisted valid five-key syntheses on their first semantic attempts. The automatic result remains visible as Stale after exact review cleanup, which proves the read surface preserves a stored synthesis when no submitted reviews remain.** AI synthesis of a proposal's submitted peer reviews. Shape as of Slice 2 (Reviews Tab Phase II, 2026-09-14, docs/plans/REVIEWS_TAB_WRITEUP_PARAGRAPHS_PLAN_2026-09-14.md §4.3) is seven keys: `{consensus, disagreements, keyConcerns, ratingSummaries, overall, writeupThemes, writeupQuotations}` — `writeupThemes` (string) and `writeupQuotations` (`{questionKey, quote}[]`, quote provenance verified against submitted reviewers' answer text at the `reviewers-service.js` read boundary, never trusted from the model) are read by `parseReviewSynthesis` and rendered by the Reviews tab's Writeup paragraphs card and `shared/utils/review-report.js`'s Word-export synthesis section. The read path remains backward-compatible with stored five-key syntheses. Written by the Executor via the `review-synthesis.generate` prompt (`guard: always-overwrite`; regeneration gated at the calling route, not the Dataverse guard). Schema-as-code: `lib/dataverse/schema/wave11-review-synthesis/` (applied to prod 2026-07-03). Written directly by `POST /api/review-manager/synthesize-reviews` or automatically by the durable `review_synthesis_jobs` drain; read (fail-soft JSON parse) by `GET /api/review-manager/reviewers` → `proposal.reviewSynthesis` → `ReviewsTab`'s Synthesis card. Deploy-order note (historical, satisfied 2026-07-03): the wave had to be applied before deploying the reading code — selecting a not-yet-created column hard-400s (wave10 precedent).
...
> **Reviewer Follow-up picker supersession (Production-live 2026-09-02):** the
> S261 PD-scoped cycle-picker statement above is historical. The implementation
> derives cycles from every organization-wide meeting-dated eligible
> request, labels active and set-aside counts separately, and defaults first to
> the caller's newest active assigned cycle and then the newest organization-wide
> active cycle. A capped discovery read fails closed; the approved Production
> read probe returned 236 eligible rows with `capped:false`. Authenticated
> Production proof showed D26 My 10 → All 44 and J26 My 0 → All 5; no write was
> exercised. Runtime merge `acf40fb8` is deployed.

**Reviewer-engagement campaign config (S275, 2026-06-21).** Per-request config backing the reviewer-engagement build (`docs/REVIEWER_ENGAGEMENT_SPEC.md`). **[LIVE in prod — provisioned `apply-dataverse-schema --target=prod --wave=7-reviewer-engagement --execute`, published + verified in live metadata 2026-06-21; only newly-created columns, no existing field touched, no Power Automate trigger.]** Discrete columns (NOT a JSON blob) so reminder and quota code can OData `$filter` server-side. **Phase 1 LIVE (S275):** `wmkf_respondoffsetdays` + `wmkf_reviewduedate` are written on first invite and read/edited through campaign config. **Phase 2 LIVE (S275):** `wmkf_reviewduedate` anchors token TTL. **Phase 3 mechanism IMPLEMENTED (S275), Vercel schedule PAUSED (2026-09-01):** `/api/cron/reviewer-reminders` consumes the reminder enabled/lead pairs, but the route is absent from the Vercel cron registry under an incident hold. The Campaign settings modal omits those controls, the schema defaults both flags true, and a 2026-09-01 read-only probe found 92 current-cycle active requests with both flags true; 8 also had the sweep's date/offset inputs. These are armed rows, so reactivation requires an owner-approved backfill/disarm decision plus the canonical spec's remaining prerequisites. **Phase 4 LIVE (S275; PD email + quota write path S352):** `wmkf_desiredcount` + `wmkf_quotanotifiedat` are consumed by `lib/services/reviewer-quota.js`; `wmkf_desiredcount` is seeded from the admin default and editable through campaign config. All 8 columns are consumed by implemented code, although the Phase-3 scheduler is paused:
