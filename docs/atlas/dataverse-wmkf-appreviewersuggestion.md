...
- `pages/api/review-manager/reviewers.js` — **[VERIFIED in local source at `5b9964c8`, Stage 6A; deployment pending]** existing generic single/batch status writer through `reviewers-service.patchReviewers` → `updateLifecycle` → `wmkf_reviewstatus`. Every target receives lead-PD/superuser authorization before any write. Nonempty batches canonicalize GUIDs and deduplicate in first-occurrence order, then await sequential writes and stop at the first failure. HTTP 200 confirms every target in `savedIds`; an attempted failure returns HTTP 500 with the confirmed `savedIds` prefix, one uncertain `failedIds` target, and the `notAttemptedIds` suffix. Failure does not prove the attempted row was unwritten; earlier saves remain applied, with no automatic retry or rollback. Pre-write validation/auth errors stay error-only. Complete/withdrew/released remain dedicated operations; no schema or storage change is introduced.
