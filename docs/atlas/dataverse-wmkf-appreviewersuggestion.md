...
- `pages/api/review-manager/send-emails.js` — adapter `updateLifecycle`; materials sends conditionally stamp `wmkf_materialssentat` + set-once `wmkf_reviewduedateatsend` inline after dispatch. A failed stamp returns `sent_but_unrecorded`; `/api/review-manager/repair-materials-send` retries only the write, never the email.
