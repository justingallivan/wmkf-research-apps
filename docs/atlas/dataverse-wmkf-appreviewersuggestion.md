...
- `complete` (S196 claim; dedicated closeout source-built 2026-09-04): a lead PD or superuser closes an already-received review through `/api/review-manager/close-review`. One ETag-conditional suggestion write sets Complete, `wmkf_completedat`, and one recognized eligibility value; duplicate repeats write nothing and explicit corrections update only eligibility without restamping completion. Completed rows remain visible.
...
- `pages/api/review-manager/close-review.js` — dedicated lead-PD/superuser human closeout. The service freshly requires selected + accepted + non-excluded + `review_received` + existing `wmkf_reviewreceivedat`; disposition-specific guards check opt-out/linkage. First closeout atomically writes status/completed-at/eligibility under the fresh ETag. Same-value repeats are no-ops; corrections write only eligibility. It never writes the linked honorarium request or payment authorization.
