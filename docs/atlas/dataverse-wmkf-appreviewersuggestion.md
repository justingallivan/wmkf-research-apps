...
**Last verified:** 2026-07-13 via `node scripts/preflight-reviewer-identity-binding-fields.mjs --target=prod --include-population`; prior 2026-05-31 (S208 — `wmkf_applicantdisposition` deployed + live metadata probe: 77 `wmkf_`-prefixed attrs, 108 total). Row count re-probed 2026-07-21 via `scripts/reconcile-memory-claims.js`.
**Live row count:** 710
