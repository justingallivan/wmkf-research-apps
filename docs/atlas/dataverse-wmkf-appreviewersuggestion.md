...
- `pages/api/review-manager/send-emails.js` — adapter `updateLifecycle`; materials sends retain the existing best-effort `wmkf_materialssentat` / `materials_sent` lifecycle update after dispatch. Thank-you sends do not move a terminal row to `complete`. **Send-time reader/authority (S404):** `send-emails-service.js` also READS `wmkf_externaltokenhash`/`wmkf_externaltokenrevoked`/`wmkf_externaltokenexpires` — via `verifySuggestionToken` (`lib/external/verify-suggestion-token.js`), not a direct field read — immediately before dispatch, whenever the final edited draft's `externalLinkExpected` stamp (from `render-emails.js`) or an extracted reviewer JWT calls for verification. It performs no new write to these fields; `render-emails.js` remains the sole writer/rotator. This closes the render↔send ordering gap: two overlapping renders (or a stale still-open preview) can rotate the hash between preview and send, and this read-time check fails that recipient closed (`email_failed`, code `external_link_superseded`/`external_link_invalid`/etc.) rather than dispatching a dead or foreign link.
