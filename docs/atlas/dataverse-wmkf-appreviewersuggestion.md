...
**Extension manifests:** base entity in `lib/dataverse/schema/wave2/wmkf_app_reviewer_suggestion.json`; extensions in `lib/dataverse/schema/wave2-existing/wmkf_appreviewersuggestion-extensions.json` (S128–S130 additions) + `lib/dataverse/schema/wave3/04_wmkf_appreviewersuggestion_stage2a.json` (S143 Stage 2a slice 1 additions) + `lib/dataverse/schema/wave5/01_wmkf_appreviewersuggestion_workbench.json` (S196 Workbench prep) + `lib/dataverse/schema/wave6/01_wmkf_appreviewersuggestion_applicant_disposition.json` (S208 applicant disposition) + `lib/dataverse/schema/wave13-reviewer-identity-binding/02_wmkf_appreviewersuggestion_identity_coi.json` + `lib/dataverse/schema/wave18-reviewer-due-date-override/01_wmkf_appreviewersuggestion_due_date_override.json` (**production run and verified 2026-08-11 / 2026-08-12 UTC**) + `lib/dataverse/schema/wave26-reviewer-closeout/01_wmkf_appreviewersuggestion_honorarium_eligibility.json` (source baseline for the manually created Production field; runtime deployment pending) + `lib/dataverse/schema/wave27-reviewer-due-date-extension-event/01_wmkf_appreviewersuggestion_due_date_extension_event.json` (source-built; deployment pending). Existing-picklist extension artifact: `scripts/extend-reviewstatus-picklist-terminal.mjs` (**production run and verified 2026-07-23**). Relevance-score range widen artifact: `scripts/widen-relevancescore-max.mjs`.
...
- `wmkf_grantcyclecode` (String, e.g. `J26`) — stamped at save time from the request's `wmkf_meetingdate` (`meetingDateToCycleCode`); since 2026-09-08 (S499) no write path accepts it from a client, and `my-candidates` reads the meeting-date-derived code first, using the stored value only for a request with no meeting date
...
- [VERIFIED via source/tests on `codex/reviewer-ui-surfacing`] `wmkf_reviewduedateextensiongrantedat`
  ...
  event. Historical extensions written before this field exists require an
  exact owner-authorized backfill.
...
- `complete` (S196 claim; dedicated closeout source-built 2026-09-04): a lead PD or superuser closes an already-received review through `/api/review-manager/close-review`. One ETag-conditional suggestion write sets Complete, `wmkf_completedat`, one recognized eligibility value, and request-scoped closeout notes. Notes are optional for Eligible/Not applicable and required/nonblank for Not eligible. Exact repeats write nothing; explicit corrections update only changed eligibility/notes fields without restamping completion. Completed rows remain visible.
...
- `wmkf_honorariumeligibility` (local Picklist, nullable; Production values `Eligible=100000000 | Not eligible=100000001 | Not applicable=100000002`) — engagement-level human closeout disposition. [VERIFIED via Production typed metadata and runtime `$select` 2026-09-04] the field is published/readable with the exact values, no default, and optional required level. The tracked schema preflight remains intentionally divergent because the live description does not yet distinguish this disposition from Operations/Finance authority. Runtime writer/UI are source-built but not deployed. No application closeout path writes `akoya_request.wmkf_authorizationtoremitpaymentflag`.
...
**Generated individual-review retention (Waves 1–4 Production-live; Wave 5
active for exact D26):** `main` commit `3ba2a6ad` / Ready deployment
`dpl_22wUzC1cCi4nhKTSFQftfaycucSh` introduced the dedicated CRON-secret route;
Ready activation-proof deployment `dpl_E6VKW5Wi8zDTfU1ZRhNsbH9yg9oM`
includes exact Production `WRITE=on` and `CYCLE=D26`. The route
...
The owner-directed path/name simplification and approved v4 Word-web template
produced fresh population manifest hash
...
candidates, zero blockers, and only the visible test exclusion. An authenticated
enabled route probe returned HTTP 200 with exact cycle `D26`, scanned only known
test Request `1003223`, classified it `invalid_snapshot`, and attempted no
filing. Bounded logs showed only successful Graph/Dataverse reads, no Graph
mutation or Dataverse PATCH, and no runtime error. Enabled runs do write
maintenance telemetry and deduplicated actionable events. The local operator
backfill write path is Production-proved for the exact D26 set; the scheduled
route is active but its first natural create/pointer mutation remains unproved.
The `1003223` test suggestion is now `wmkf_selected=false`, with its received
review history and empty pointers preserved; the immediate enabled follow-up
sweep returned zero candidates and zero attempts.
...
- `wmkf_withdrawnsufficientat` (DateTime) — the sufficient-reviews timestamp. **Pending writer (production-live, reviewer-engagement Phase 4, S275):** `POST /api/review-manager/withdraw-sufficient` sets this + `wmkf_responsetype=withdrawn_sufficient` and clears `wmkf_respondremindersentat`, only on still-pending rows. The same endpoint's `no_response` reason instead writes `wmkf_responsetype=no_response` + `wmkf_responsereceivedat`, clears the reminder stamp, and revokes the token. **Accepted writer (source-built 2026-09-16; promotion pending):** `POST /api/review-manager/terminal-transition` sets this when a PD releases an accepted, not-yet-submitted reviewer because enough reviews were received. That path also writes `reviewStatus=released`, revokes the token, derives the outcome “Review not received” from the guarded absence of receipt/completion, and may update the existing `wmkf_notes` after an expected-value check.
...
- `ensureStaffManualCandidate({ potentialReviewerId, requestId, … })` — **Workbench manual reviewer add**. Idempotently materializes/reselects a non-excluded staff-added row, **unions** `staff_manual` into existing `wmkf_sources` (no clobber), preserves applicant recommendation state when an existing row already has it, honors "excluded wins" (`{ skippedExcluded: true }`), and catches a lost alternate-key create race. Unlike lazy applicant ingestion, this is an explicit staff action, so an existing soft-deleted non-excluded row is re-selected. **Re-add fresh start (S343):** when the re-selected row was *removed* (`wmkf_selected===false`), `ENGAGEMENT_STAMP_RESET` clears the stale engagement stamps (`wmkf_invited=false`; `wmkf_emailsentat`, `wmkf_respondremindersentat`, `wmkf_remindersentat`, `wmkf_remindercount`, `wmkf_materialssentat`, `wmkf_reviewreceivedat`, `wmkf_responsereceivedat`, `wmkf_thankyousentat`, `wmkf_completedat`, `wmkf_honorariumeligibility`, `wmkf_withdrawnsufficientat`, `wmkf_proposalfirstaccessed`, `wmkf_reviewduedateoverride` = `null`) so the row returns to a clean engagement lifecycle; an already-active row's stamps are left untouched. The re-select PATCHes are ETag-guarded from the fetched row; on 412 they re-fetch, source-union without reset if the row is now active, or retry the reset if it is still removed.
...
- `updateLifecycle(id, updates, { actingUserSystemId, ifMatch })` — partial update with picklist mapping for `responseType`/`reviewStatus`/`applicantDisposition`/`honorariumEligibility`; supports `completedAt` and applies the sink-level today-or-future Foundation-Pacific validation to non-null `reviewDueDateOverride`. The dedicated extension service adds the stricter accepted-row and after-original-date contract. Reads the row once per write to reject applicant exclusion, refuse status changes out of `complete`/`withdrew`/`released`, and require existing receipt evidence plus a recognized eligibility disposition before Complete while stamping only `wmkf_completedat`. Eligibility cannot be cleared or written on a non-complete row; dedicated closeout corrections may change eligibility/notes on a complete row without restamping completion.
  **[VERIFIED in source at `c51fa34d`, Stage 1D; shipped in Production at `c19a16d8` per the release receipt above]** any payload containing `invited`, `accepted`, `declined`, `emailSentAt`, `responseType`, or `responseReceivedAt` additionally rejects a completed-at marker or closed source status, and permits only explicit null/accepted/materials-sent/under-review/review-received source status. Missing/unknown source status fails closed. These six fields require a concrete ETag: a supplied value always wins, including a stale one; an invalid supplied value is rejected rather than replaced with the newer guard-read version. When omitted, the guard-read ETag is required. Other fields retain their existing writer contracts; status writes still use the guard-read ETag when no caller version is supplied.
...
- `applyStaffReviewerRelease(id, { ifMatch, actingUserSystemId, notes, cancelHonorarium })` — PD-recorded sufficient-reviews release of an accepted reviewer. Requires the fresh row ETag and writes `reviewStatus=released`, token revocation, `wmkf_withdrawnsufficientat`, and optional notes. When the server-read linked honorarium is `Pending`, unpaid, unauthorized, and ETag-bearing, the same changeset retains it and writes `akoya_requeststatus=Withdrawn` + `wmkf_datewithdrawalreceived`; paid/authorized/unexpected states fail before this adapter is called.
- `withdrawLinkedHonorariumForReleasedSuggestion(...)` — acceptance-worker race compensation. ETag-asserts that the suggestion remains released while atomically marking the exact late-linked honorarium `Withdrawn`; unlike reviewer withdrawal, Foundation release never deletes that request.
...
- `pages/api/reviewer-finder/save-candidates.js` — canonical contact projection
  ...
  saved.
- `pages/api/reviewer-finder/my-candidates.js` — **[VERIFIED in source at `c51fa34d`, Stage 1D; shipped in Production at `c19a16d8` per the release receipt above]** generic single-suggestion PATCH supports only `invited`, `accepted`, `declined`, `emailSentAt`, `responseType`, and `responseReceivedAt` lifecycle corrections. The route passes the Request resolved by its lead-PD/superuser authorization separately from the body; the service freshly requires the same Request binding, a non-excluded row, no completed-at marker, explicit null or recognized open review status, and a concrete ETag before calling `updateLifecycle` with that exact version. Closed/unknown/missing state and version races fail before token follow-up or person edits; numeric 412 becomes `correction_conflict` and is never automatically retried. Only a successful `accepted:true` lifecycle write reaches the nonfatal `ensureToken` follow-up, before any person edits. Dedicated closeout owns eligibility/notes; `reviewDueDateOverride` remains outside this PATCH. Named manual-invite recording, restore, and person-only edits retain their separate contracts; the per-proposal cycle/program-area assignment branch was removed 2026-09-06 (owner decision D4) — `wmkf_grantcyclecode`/`wmkf_programarea` are written at save time (`save-candidates`; the cycle code is derived server-side from the request's `wmkf_meetingdate` since 2026-09-08, never taken from the body) and have no post-save correction path. DELETE uses `softDelete`; `DELETE {mode:'hard'}` dispatches to `removeCandidateEntirely` (see Permanent removal above).
- `pages/api/review-manager/reviewers.js` — **[VERIFIED in source at `5b9964c8`, Stage 6A; shipped in Production at `c19a16d8` per the release receipt above]** existing generic single/batch status writer through `reviewers-service.patchReviewers` → `updateLifecycle` → `wmkf_reviewstatus`. Every target receives lead-PD/superuser authorization before any write. Nonempty batches trim/lowercase GUIDs and deduplicate in first-occurrence order, then await adapter operations sequentially and stop at the first failure. The single path preserves the submitted ID and lifecycle object; the UI canonicalizes IDs for outcome comparison. HTTP 200 confirms every target in `savedIds`; any failure from an invoked adapter returns HTTP 500 with the confirmed `savedIds` prefix, one unconfirmed adapter operation in `failedIds`, and the `notAttemptedIds` suffix. This includes `mapPicklist` rejection before the lifecycle guard read/write, so `failedIds` does not prove a Dataverse HTTP write began or committed. Earlier saves remain applied, with no automatic retry or rollback. Route validation/authorization and the service's dedicated-target prechecks stay error-only. Complete/withdrew/released remain dedicated operations. Existing null/empty-string status clearing on open rows remains supported with closed-source protection; no new validation policy, schema or storage change is introduced.
...
- `pages/api/review-manager/send-emails.js` — adapter `updateLifecycle`; materials sends retain the existing best-effort `wmkf_materialssentat` / `materials_sent` lifecycle update after dispatch. Thank-you sends write only `wmkf_thankyousentat`; they never set Complete/completion/eligibility, and retained `{{honorariumNote}}` tokens resolve to blank. **Send-time writer/authority (S404 v4):** `send-emails-service.js` extracts the final edited subject/body link, defense-in-depth verifies any real legacy/edited JWT, computes the unchanged per-recipient expiry, calls `mintAndStore`, and substitutes only the JWT path segment immediately before invoking Dynamics dispatch. Mint failures remain per-row `email_failed {code,error}` and do not stop healthy siblings.
- `pages/api/review-manager/close-review.js` — dedicated lead-PD/superuser human closeout. The service freshly requires selected + accepted + non-excluded + `review_received` + existing `wmkf_reviewreceivedat`; disposition-specific guards check opt-out/linkage. First closeout atomically writes status/completed-at/eligibility and bounded `wmkf_notes` under the fresh ETag; Not eligible requires a nonblank reason, while Eligible/Not applicable keep notes optional. Exact repeats are no-ops; corrections write only changed eligibility/notes fields. Track Reviewers no longer gives notes a dedicated table column or a separate write path; the closeout form preloads and owns the request-scoped note. The service never writes the linked honorarium request or payment authorization.
- `pages/api/review-manager/terminal-transition.js` — dedicated fresh-read/ETag service writes only `withdrew` or `released` from accepted/materials-sent/under-review rows with no received/completed stamp. For `withdrew`, it corrects accepted/declined response state and deletes the exact linked honorarium. For `released`, it requires the fixed sufficient-reviews reason, records the dated release and optional internal note, retains and marks a safely open honorarium `Withdrawn`, cancels acceptance follow-up, and only then sends optional reviewed thank-you copy. Generic `/reviewers` PATCH explicitly refuses both terminal values.
...
- `pages/api/cron/file-review-docx.js` — Production-active dedicated exact-cycle
