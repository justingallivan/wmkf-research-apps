...
- `pages/api/review-manager/close-review.js` — dedicated lead-PD/superuser human closeout. The service freshly requires selected + accepted + non-excluded + `review_received` + existing `wmkf_reviewreceivedat`; disposition-specific guards check opt-out/linkage. First closeout atomically writes status/completed-at/eligibility and optional bounded `wmkf_notes` under the fresh ETag. Exact repeats are no-ops; corrections write only changed eligibility/notes fields. Track Reviewers no longer gives notes a dedicated table column or a separate write path; the closeout form preloads and owns the request-scoped note. The service never writes the linked honorarium request or payment authorization.
