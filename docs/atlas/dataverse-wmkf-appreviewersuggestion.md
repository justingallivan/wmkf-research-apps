...
- `updateLifecycle(id, updates, { actingUserSystemId, ifMatch })` — partial update with picklist mapping for `responseType`/`reviewStatus`/`applicantDisposition`/`honorariumEligibility`; supports `completedAt` and applies the sink-level today-or-future Foundation-Pacific validation to non-null `reviewDueDateOverride`. The dedicated extension service adds the stricter accepted-row and after-original-date contract. Reads the row once per write to (a) THROW on an applicant-excluded row, (b) refuse status changes out of `withdrew`/`released`, and (c) require existing receipt evidence plus a recognized eligibility disposition before Complete while stamping only `wmkf_completedat`. Eligibility cannot be cleared or written on a non-complete row. Status-changing writes bind to the guard read's ETag when the caller does not supply a stricter one.
