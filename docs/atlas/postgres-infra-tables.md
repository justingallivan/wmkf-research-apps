...
One row per reviewer acceptance timestamp (`UNIQUE (suggestion_id, accepted_at)`). `/api/external/review/[token]/respond` stages the row before a fresh Dataverse accept PATCH and returns after the PATCH commits; repeat accepts reuse/requeue the same logical job. `/api/cron/drain-reviewer-acceptances` claims ready rows with `FOR UPDATE SKIP LOCKED` + `lease_token`, re-reads `wmkf_appreviewersuggestion`, and runs the formerly-inline accept tail: honorarium/contact capture, self-reported ORCID, board identity, contact name/title sync, mismatch alerts, acceptance confirmation email, and quota notification. Dataverse `wmkf_appreviewersuggestion` remains the authoritative accepted/declined state; this table records retryable side-effect progress only. Stale `accept_pending` rows are cancelled if the Dataverse accept never landed.
