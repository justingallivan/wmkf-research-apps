...
| `review_panel_entries` | Shared request entry, `created_by` attribution | Per-request-revision JSONB `data` (`input`: the frozen narrative DTO — see deviation note below — plus `chairResult`/`files`/`error` once settled; each `data.files.{docx,pdf}` ref carries a `savedAt` (ISO) stamp for the moment that edition landed; on a report-save failure `reportError: { name, message (≤300 chars), at }` also carries the raw throw for diagnosis, alongside the generic user-facing `error` copy; `rerender: { requestedAt, requestedBy, previousFiles }` is present only while an operator re-render request is outstanding on a `completed` entry — see re-render note below); `winners_json` maps `{ seatKey: attemptId }`; `retry_requested_at` is the route→worker hand-off marker, reused for BOTH a failed-entry retry and a completed-entry re-render request | `status` enum: `pending → running → completed \| failed`; unique `(request_id, request_revision)`; `retry_requested_at` is set only by `requestReviewPanelRetry`/`requestReviewPanelRerender` (route, no lease) and cleared only by `mutateReviewPanelEntry` (worker, under lease) — for a retry, in the SAME update that changes status back to `running`; for a re-render, in its OWN update (moved to `data.lastRerender`) ahead of the separate write that later changes `data.files`/status — see hand-off note below |
...
`requestReviewPanelRerender` (`review-panel-store.js`) lets an operator re-derive a `completed` entry's DOCX/PDF editions from its already-saved seat/chair reviews, with NO model calls — built so a report-template fix can reach editions that were rendered and saved before the fix landed, which are otherwise never re-rendered. It shares the SAME `retry_requested_at` route→worker marker as `requestReviewPanelRetry` (no new column, no migration): the settle fence is identical (no live lease, status not `queued`/`running`/`cancelled`), and eligibility is a `completed` entry with a chair winner (`winners_json.chair`). On acceptance it clears `data.files` to `null` and stamps `data.rerender = { requestedAt, requestedBy, previousFiles }` — the CURRENT `data.files` preserved as `previousFiles` (the superseded refs are never deleted, only superseded, per the Blob cleanup rule: cleanup deletes only exact persisted pathnames). The worker's `drainReviewPanels` reads `listRetryRequestedEntries` exactly as it does for retry, then splits the ids by each entry's own status/`data.rerender` (the marker alone cannot tell retry and re-render apart): `rerenderCompletedEntries` moves the marker to `data.lastRerender` (keeping `previousFiles` reachable, never simply dropped) under its OWN lease-fenced write — a separate update from the one that later changes status/files — then calls the SAME `completeEntryWithReport` used at normal completion, but forces a NEW Blob pathname per render (`report-<stamp>.<format>`, never the original `report.<format>`) so `storeReviewPanelFile`'s create-only put (`allowOverwrite:false`) can never collide with the edition it supersedes. If that render/save fails, the entry is restored to `completed` with the OLD `data.files` intact (never left `failed` with no report and no attempt/`runSeat`/`runChair` call ever made). The service projects `entries[].rerender = { requestedAt, previousFiles: <count> }` (a count only — the superseded refs themselves never reach the client) while the marker is outstanding, and the timeline shows "Re-render requested" (not "Retry requested") for the same marker, plus "Report saved" timed from the newer file ref's own `savedAt` when present.
