...
- **Pre-deployment expand gate.** Apply wave 15 to production and verify metadata readback **before this branch is merged/promoted to any auto-deploying production branch**. The compatible readers always select `wmkf_answervalues`, and every answer writer emits the property (null for non-multiselect rows), so merely keeping the old question set active does not make the code safe against absent metadata. Row-level compatibility begins only after the nullable property exists: then old null/absent values remain readable, the compatible code may deploy, and only afterward may a `multiselect` question be activated.
