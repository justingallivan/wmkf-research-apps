...
- **Pre-deployment expand gate — cleared 2026-07-26.** Wave 15 is applied and exact metadata plus entity-set select readback passed. The compatible code may now deploy against the still-old active question set. A `multiselect` question must remain inactive until that deployment is verified.
