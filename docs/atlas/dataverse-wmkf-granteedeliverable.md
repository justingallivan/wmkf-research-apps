...
- `pages/api/cron/grantee-deliverable-reminders.js` — paged query of all Invited packages with an invite date; no `$expand`. **[SOURCE-BUILT 2026-08-26 (VIP/digest decision layer); migration 036 not applied]** every Invited row gets a Postgres `scheduled_email_messages` ledger entry on first sight, with `approval_required` frozen from the PD's review-all override and per-(PD, contact) VIP flags; there is no per-PD timing gate or unconfigured-PD path.
