...
**Reviews Tab writeup paragraphs read model (Reviews Tab Phase II Slice 1, 2026-09-14):** the Review Manager reviewers read model (`lib/services/review-manager/reviewers-service.js` `fetchPotentialReviewers`) now selects `wmkf_lastname`, `wmkf_academicrank`, `wmkf_maininstitution`, `wmkf_areaofexpertise`, and `wmkf_keywords` alongside the fields already selected there, projecting them onto each reviewer as `lastName`, `academicRank`, `mainInstitution`, `areaOfExpertise`, `keywords` for `shared/utils/review-writeup-paragraphs.js` to compose the Reviews tab's deterministic writeup sentences. Read-time only; not added to the review-synthesis digest, so stored syntheses do not go stale from this change.
...
- `lib/services/review-manager/reviewers-service.js` `fetchPotentialReviewers` — chunked OR-chain on `wmkf_potentialreviewersid` to hydrate the Review Manager reviewer list; select widened 2026-09-14 to add the board-writeup identity + expertise fields (see "Reviews Tab writeup paragraphs read model" above)
