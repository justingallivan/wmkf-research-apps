...
The cycle table's `wmkf_meetingdate` is currently not populated/selected by the grant-cycle service. Reviewer-finder proposal counts therefore parse the validated month/year from `wmkf_displayname`/`wmkf_fiscalyearcode` as a temporary cycle-side key, while request rows are grouped by persisted `akoya_request.wmkf_meetingdate`; unmatched and null dates are surfaced in the API's unassigned proposal bucket.

**`wmkf_summarypages` is deployed but unused since 2026-09-06:** `grant-cycles-dataverse.js` no longer selects, maps or writes it (the create-time default `'2'` is gone too) because the summary-page extraction it configured was removed from `/api/reviewer-finder/analyze`. Existing values are inert history; dropping the attribute would be a separate schema change.
...
| `wmkf_appreviewersuggestion` | (extension manifest only) | ✅ | ✅ (793 rows; re-probed 2026-08-15 PT) |
