...
| `REVIEWER_IDENTITY_DISCOVERY_RESOLVER_MODE` | Reviewer-discovery Track-A W2 seam. `shadow` compares while returning exact legacy results; `combined` is authoritative only for this caller. | Unset/`legacy`/unknown values are safe. Do not set `combined` without that entry point's offline gate and owner-approved cutover. |
| `REVIEWER_IDENTITY_WORKBENCH_RESOLVER_MODE` | Workbench applicant-recommended Track-A W2 seam, independent of reviewer discovery. | Same fail-closed modes and separate gate as above. |
| `REVIEWER_IDENTITY_CONTACT_ENRICHMENT_RESOLVER_MODE` | Contact-enrichment reconciliation seam, independent of both batch callers. | Keep `legacy` until contact enrichment has its own representative evidence and owner gate. |
...
The former generic `REVIEWER_IDENTITY_RESOLVER_MODE` remains a compatibility
input for the unscoped internal/single-candidate seam. It does not select the
three live entry-point modes above. A 2026-08-07 redacted probe established only
that this former global variable normalized to legacy at that time; it is not
evidence for the new scoped variables. Probe each scoped variable before any
live observation or mode change, without printing its raw value.
