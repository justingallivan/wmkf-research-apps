...
| `REVIEWER_PORTAL_BASE_URL` | Public base URL used in external-reviewer invitation links | `https://wmkfresearch.vercel.app` now; later `https://reviews.wmkeck.org` | Non-secret. Defaults to `NEXTAUTH_URL` if unset; set explicitly before reviewer invitations go out so the future domain switch is one env-var change. |
| `GRANTEE_PORTAL_BASE_URL` | Public base URL used in grantee deliverables magic-links (invite + reminder) | `https://wmkfresearch.vercel.app` now (set S271, non-sensitive); later `https://grantees.wmkeck.org` | Non-secret. Code resolves `GRANTEE_PORTAL_BASE_URL ‖ NEXTAUTH_URL ‖ ''` — **MUST be set** or grantee links are hostless/broken (NEXTAUTH_URL is empty in prod). Branded-domain switch is one env-var change (no code change; nothing hardcodes the domain). |
