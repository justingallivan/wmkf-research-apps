...
| `NOTIFICATION_EMAIL_FROM` (pending change) | Owner selected `alerts@wmkeck.org` to replace the individual staff mailbox so program directors stop receiving system alerts that appear to come from a person [VERIFIED via owner instruction, session 2026-07-27]. **Not yet applied**, and the address is **not yet confirmed** to be a Dynamics systemuser with outgoing Server-Side Sync — see `docs/TODO_EMAIL_NOTIFICATIONS.md` for the verification steps and the silent-failure mode if it is not. |
