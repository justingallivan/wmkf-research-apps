...
| `DOSSIER_BLOB_STORE_ID` | Managed store identity paired with the dedicated Cycle Dossier Blob token | **Source-built 2026-09-07; not provisioned.** Must be the managed private store ID (`store_<id>`); rollout preflight performs an authenticated read-only list and checks token/store identity. |
...
| `DOSSIER_BLOB_READ_WRITE_TOKEN` | Dedicated Cycle Dossier private store | **not provisioned** | record at rollout |
...
set only `DOSSIER_BLOB_READ_WRITE_TOKEN` in development, preview, and production. Do
