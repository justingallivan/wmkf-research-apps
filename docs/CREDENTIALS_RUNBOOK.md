...
| `SITE_VISIT_MATERIALS_SCHEMA_READY` | Non-sensitive migration-042 readiness interlock for the applicant materials collection. Only literal `on` enables `/api/meeting-tracker/visits/[requestId]/materials` and the contributor link; unset fails 503 after auth. Owner sets it after applying migration 042. | Manual, per environment |
