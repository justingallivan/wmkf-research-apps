...
| `NOTIFICATION_EMAIL_FROM` (pending change) | Owner selected `alerts@wmkeck.org` to replace the individual staff mailbox so program directors stop receiving system alerts that appear to come from a person [VERIFIED via owner instruction, session 2026-07-27]. It is a licensed, enabled Dynamics systemuser (`d57ddb27-c8db-ee11-904d-000d3a310f67`, `fullname` `# Alerts`, owner-accepted as the visible sender name) [VERIFIED via read-only Dataverse probe of `wmkf.crm.dynamics.com`, 2026-07-27], so the sender lookup resolves. **Not yet applied** to Vercel, and outgoing Server-Side Sync on its mailbox row is **still unverified** — see `docs/TODO_EMAIL_NOTIFICATIONS.md` for that check and the silent-failure mode. |
