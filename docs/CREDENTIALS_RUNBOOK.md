...
| `MEETING_TRACKER_SCHEMA_READY` | Non-sensitive Wave 28 readiness interlock. Only literal `on` may let Meeting Tracker routes, adapters, and services read or write `wmkf_deliberationsession` / `wmkf_deliberationslot`. Unset or any other value makes every tracker route return 503 before Dataverse work and makes the shared schedule reader return all-null. Set only after `scripts/preflight-meeting-tracker-schema.mjs --target=<target>` reports 22 exact / 0 absent / 0 divergent following a separately approved additive apply. Retain the schema and flag during runtime rollback once session or slot rows exist. | **Default: unset.** Sandbox Wave 28 was applied and read back exact on 2026-09-09; the flag remains unset pending runtime implementation. Production is unapplied and unverified. |
