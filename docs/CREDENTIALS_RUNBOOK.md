...
| `UPLOADS_BLOB_RW_TOKEN` | Shared document-uploader private store (`wmkf-uploads-private`, `store_WvoDkxrlWniAuJAj`, `iad1`) RW token — Phase 1 private-blob migration of `FileUploaderSimple` | Manual — set in **dev + preview + production** (2026-06-11); live smoke **passed**; **cohort promoted to prod** (`phase-i-dynamics` + `grant-reporting` flags set + deployed; grant-reporting prod-verified). `expense-reporter` prod flag still unset (Preview-only) → public in prod. Private uploads fail closed where the token is unset. See "Private Blob store provisioning" below |
