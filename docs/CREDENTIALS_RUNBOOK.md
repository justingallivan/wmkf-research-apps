...
| `REVIEWER_INSTITUTION_PHASE2` | Dormant server-only gate for independent-identity evidence projection, typed affiliation assertions, request/candidate-bound roster receipts, and fail-closed additional-affiliation COI recomputation at both reviewer save boundaries. Only the literal `on` enables it; any other value preserves the incumbent response and write path. This is separate from the public Stage 2 presentation flag and from measurement migration 048. | **[PLANNED 2026-09-14.]** Keep unset/`off` in every deployed environment until the dated-currentness policy, card/remedy projection, full flag-off equality suite, and owner-approved high-authority rollout are complete. |
