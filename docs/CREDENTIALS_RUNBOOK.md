...
| `REVIEW_DOCX_SHAREPOINT_WRITE` | Non-sensitive master write gate for `/api/cron/file-review-docx`. Only literal `on` permits generated individual-review DOCX uploads and pointer commits. The writer additionally requires an enforcing Dataverse target interlock, the exact canonical akoyaGO SharePoint site, and a Production scheduled runtime; all checks run before Graph mutation. | **Source-built 2026-09-03; unset in deployed environments.** Unset/empty/any other value disables the sweep before candidate or Graph reads. Keep off through adversarial review and Preview/mock verification. |
| `REVIEW_DOCX_SHAREPOINT_CYCLE` | Exact automatic-filing cohort (`JYY` or `DYY`) for `/api/cron/file-review-docx`. The service prefers the suggestion's stamped cycle and otherwise derives it from the request meeting date; it never infers a current cycle globally. | **Source-built 2026-09-03; unset in deployed environments.** Must be configured together with deliberate write activation; planned first cohort is `D26`. |
