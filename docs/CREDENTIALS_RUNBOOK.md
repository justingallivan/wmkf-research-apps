...
| `NEXT_PUBLIC_INSTITUTION_STAGE2_PRESENTATION` | Exact `on` enables source-aware post-acceptance institution notifications and explanatory reviewer-card copy. It does not change candidate selectability, identity weighting, or Dataverse-write gates. Unset/`off` restores the incumbent boolean presentation. Because the client reads this flag, changes require a new build/deployment. | unset/`off`; enable in Preview for signed-in acceptance before Production |
