...
| `REVIEWER_PORTAL_BASE_URL` | Public base URL used in external-reviewer invitation links | Current Vercel domain; later `https://reviews.wmkeck.org` | Non-secret. Defaults to `NEXTAUTH_URL` if unset; set explicitly before reviewer invitations go out so the future domain switch is one env-var change. |
