...
| `REVIEW_PANEL_BLOB_READ_WRITE_TOKEN` | Dedicated private Blob RW token for the Virtual Review Panel Phase A foundation's per-entry DOCX/PDF report editions (D11) | **Source-built 2026-09-12; store NOT yet provisioned.** No store id has been created or connected. `lib/services/review-panel-storage.js` returns a 503 with plain copy when this is unset — never invent a store id or token value here. |
| `REVIEW_PANEL_BLOB_STORE_ID` | Managed store identity paired with the dedicated Review Panel Blob token | **Source-built 2026-09-12; not provisioned.** Will be set once the owner creates the dedicated store, mirroring `DOSSIER_BLOB_STORE_ID`'s provisioning shape. |
| `REVIEW_PANEL_ENABLED` | Review Panel Phase A activation flag (readable config, not a secret) | **Source-built 2026-09-12; disabled until rollout.** Set literal `true` only after migration 047, the dedicated private Blob token, and governed prompts (`review-panel.seat`, `review-panel.chair`) are verified. |
| `REVIEW_PANEL_ROLLOUT_MODE` | Cohort mode: `pilot` or `smoke` (readable config, not a secret) | **Source-built 2026-09-12; defaults to `pilot`.** An unrecognised value fails closed (503, `assertReviewPanelModeValid`). |
| `REVIEW_PANEL_REQUEST_ALLOWLIST` | Server-owned comma-separated request IDs or request numbers admitted to the controlled cohort (readable config, not a secret) | **Source-built 2026-09-12; not configured.** Required before activation (`assertReviewPanelCohortConfigured`); `smoke` mode additionally caps the cohort at four requests. |
...
## Private Blob store provisioning (`DVX_BLOB_RW_TOKEN`, `INTAKE_BLOB_RW_TOKEN`, `UPLOADS_BLOB_RW_TOKEN`, `DOSSIER_BLOB_READ_WRITE_TOKEN`)
...
| `REVIEW_PANEL_BLOB_READ_WRITE_TOKEN` | Dedicated Review Panel private store (D11) | **NOT yet provisioned — no store id exists.** | Follows the same one-store-per-app pattern once the owner creates and connects the store; do not invent a store id or token value ahead of that. |
