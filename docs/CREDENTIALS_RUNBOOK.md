...
| `NEXTAUTH_URL` | Production URL for OAuth callbacks and staff API Origin/Referer checks | Held for `https://applications.wmkeck.org` after Azure redirect URI validation | **Production is currently empty** (2026-06-23). Do not set to `applications.wmkeck.org` until the staff Azure app registration includes `https://applications.wmkeck.org/api/auth/callback/azure-ad` and staff sign-in + a state-changing staff API action have been smoke-tested. |
