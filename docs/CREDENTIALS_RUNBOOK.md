...
| `REVIEW_SYNTHESIS_AUTOMATION_ENABLED` | Master rollout gate for `/api/cron/drain-review-syntheses`. The route authenticates and returns an inert `automation_disabled` response unless the value is exactly `true`; set only after migration 028 is applied and manual/read-only verification passes. | unset/anything other than exact `true` → disabled |
