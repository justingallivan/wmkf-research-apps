...
|---|---|
| VERIFIED | Real path; selected candidate saves; §5.7 pass; §5.8 pass; §5.9 pass; §5.10 pass; §5.11 is either clean evidence or record-and-escalate with no §5.11 gate-fail condition; §5.12 pass; §5.13 pass; every required `SdkMessage` literal is exactly `Update`; every counted run's child ID and parent lookup attribution match the expected GUIDs. Clears the S163 P1-Update pre-deploy gate, with any §5.11 record-and-escalate item tracked separately. |
...
| FAIL(->Option B, no schema rework) | No candidate saves; §5.7, §5.8, §5.9, §5.10, §5.12, or §5.13 fails; §5.11 has a gate-fail condition; any required `SdkMessage` literal is anything other than exactly `Update`; trigger output child row ID or parent lookup attribution is missing/ambiguous; or Select columns cannot be configured so statecode/statuscode-only updates fire. |
