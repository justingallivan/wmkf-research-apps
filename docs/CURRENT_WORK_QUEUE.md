...
| 4 | Final Writeup persona rollout and complete dashboards | **[PERSONA LENSES PRODUCTION-LIVE; NATURAL STAFF OBSERVATION COMPLETE 2026-09-04 PT.]** Commit `213f6c34` is live in Ready deployment `dpl_HGrbWUNPJMJunVevYLVEmtn7He6a`; the tracked source flag is true. Read-only production-data smoke verified PD, PC, Leadership, overlap, ineligible/unassigned, and superuser projections. **[OWNER-REPORTED]** President Allison Keller and Program Coordinators Duncan Spore and Sarah Hibler proved canonical Word access and acknowledgement; Duncan later found Request `1002788` in History, saw the review matrix, and opened the Word document successfully. Duncan, Allison, and Sarah initially appeared Reviewed and later Updated, consistent with publication-version freshness. No team exists or is required. | **Met:** runtime projection, representative Word access, and signed-in non-superuser History/matrix/Word observation are complete. Owner decisions 2026-09-06: **Leadership stage transition is a PD action in the Workbench** (a new lifecycle-state write from Review to Final). Planned S493 in `docs/FINAL_WRITEUP_LEADERSHIP_TRANSITION_PLAN.md` (six Codex passes; D2 owner-confirmed 2026-09-07) and **[PRODUCTION-LIVE 2026-09-07: PR #176 merged as `25dc8645`, Ready deployment `dpl_22eyAD8S4yPmx16iv3Nng2u9sw5T`, owner-run signed-in smoke passed on Request `1002788`.]** **PC backup is not a separate feature:** PCs already see every Final [VERIFIED via `dashboard-service.js:355`, PC persona returns all rows]; a retiring PD's requests transfer by changing the program director in Dataverse, which the dashboards key on. Broader supporting-material projection remains open. Dashboard filters, cycle scoping, and the retired-preview decision are tracked as item 5 below. |
| 5 | Final writeups dashboard completion: cycle scoping, filters, and residuals (Editor Dashboard contract) | **[OWNER-DIRECTED 2026-09-06; 6A PRODUCTION-LIVE AND SMOKED; 6B/6C PRODUCTION-LIVE AND OWNER-SMOKED 2026-09-07 (TWO CODEX DIFF PASSES, FINAL APPROVE; PR #175 MERGED `44bdd240`, DEPLOYMENT `dpl_2UrsDnydRqudu95wJyLCK7A6FUFR`; SIGNED-IN SMOKE PASSED ON VIEWS, EMPTY STATE, BOOKMARKED AND INVALID URLS, VERSION LABEL, PD FILTER); 6D/6E CLOSED; PRIORITIZED ABOVE ITEM 6.]** The owner reviewed the Editor Dashboard contract against the Production-live Final writeups dashboard and decided: keep and build the contract filters (cycle, program/PD, artifact stage, editing/review state) as navigation-only controls; retire inline preview (Word opens outside the Workbench); keep the residuals. Boundary [VERIFIED S491 via source]: the dashboard lists every current Final across all cycles under one hard 100-row cap that fails the whole page (503) when exceeded, renders no cycle or publication version, and has no filter controls or "has edits" hint. Build order and contracts are Slice 6 of `docs/FINAL_WRITEUP_REVIEW_IMPLEMENTATION_PLAN.md`: 6A server-side cycle scoping (dated: before D26 Final writeups exist; **built S491 on branch `claude/final-writeups-cycle-scoping` per `docs/FINAL_WRITEUPS_DASHBOARD_CYCLE_SCOPING_PLAN.md` after three Codex plan passes and the owner's 2026-09-06 decision that lenses focus rather than conceal (PD lens now retains leadership-stage rows); two Codex diff passes, merged `842c9f13`, owner-run signed-in smoke passed 2026-09-06; the PD lens now retains leadership-stage rows with a warn-not-lock notice**), 6B views and Program director filter persisted in the URL (**owner-shaped 2026-09-06; built 2026-09-07 per `docs/FINAL_WRITEUPS_DASHBOARD_VIEWS_AND_VERSION_PLAN.md` after three Codex plan passes:** the page opens on "Needs my review" for every role, with "Reviewed by me" and "All writeups" as alternatives and a Program director dropdown that also filters every coordinator-matrix row; the stage filter is dropped because the dashboard has only two stages and the label is already on each row; `view`/`pd` are page-URL state sanitized on mount and never sent to the API), 6C current-version context in place of preview (**built 2026-09-07, same branch:** rows and the focused page show the verbatim publication version and, when Updated since review, the version the viewer acknowledged; `acknowledgedPublicationVersionId` is additive on the dashboard row and both acknowledgement responses, null for the responsible PD), 6D "has edits" hint and 6E other-stage lists (**owner 2026-09-06: closed as unnecessary for D26; do not build**). No approval gates, denominators, program-taxonomy grouping, or authorization changes. | **Met when:** a signed-in user opens the dashboard on the writeups that need their review, can switch to reviewed-by-me or all, can narrow by PD, and the view is bookmarkable; the publication version acknowledgements key to is visible per row; two cycles of Finals load without the scope-exceeded error. 6D/6E closed by owner decision 2026-09-06. |
| 6 | Automated materials-on-acceptance email (reviewer lifecycle) | **[OWNER-DIRECTED 2026-09-06; NOT STARTED; PLAN-FIRST; J27 ITEM.]** Owner correction 2026-09-06: this is a **J27** (June 2027 cycle) flow, not D26. D26 invitations and acceptances have already happened; J27 proposals arrive in early December 2026 and J27 reviewer invitations follow in 2027. Desired J27 flow: every review material the reviewer needs is in hand at the time of the request to review, and upon acceptance **and** onboarding the system automatically emails the reviewer a link to the review materials (the proposal) with no PD action. Nothing does this today [VERIFIED S490: the acceptance job/drain handles honorarium onboarding and bookkeeping only; materials go out solely through the PD-driven `ReleaseMaterialsModal` → send-emails `materials` template]. Build when the current campaigns settle; the plan must exist before J27 reviewer invitations go out (2027; exact window depends on the J27 calendar in `docs/J27_TRANSITION_REGISTER.md`). Plan-first triggers (`/contract-reconcile`): new background/durable work in the acceptance drain, a live email send, idempotency across drain retries (never two materials emails), the "materials in hand" precondition the drain can verify, accept-before-materials-attached handling, the PD's visibility/override, and how the manual modal becomes the fallback. No preview step, so the 6D draft fingerprint does not apply; render and send are one server step. | Production-live automated send observed on the first real J27 acceptance, recorded in the Stage 6B3 receipt; the manual modal documented as the exception path. |
| 7 | Per-reminder audit trail for reviewer follow-up (Activity history) | **[SURFACED 2026-09-06 BY THE CODEX POLISH HANDOFF; NOT STARTED; PLAN-FIRST WITH `/contract-reconcile`; J27 ITEM (owner 2026-09-06: "can wait till J27"). Reminders stay manual for D26.]** The reviewer record stores only cumulative `reminderCount` and the latest `reminderSentAt`, so the Activity history drawer (reachable since the 2026-09-06 polish from status, token, and reminder pills) shows one dated reminder event plus "N reminders recorded in total". Build a durable per-reminder record and extend the reviewer activity API/DTO so each reminder is its own dated event. Do not read `reminderCount` as evidence of individual timestamps. Handoff: `docs/plans/REVIEWER_FOLLOW_UP_POLISH_HANDOFF_2026-09-06.md`. | **Met when:** each reminder sent appears as its own dated event in the drawer, backed by durable rows, with the existing count preserved for older reviewers. |
...
| 9 | PC Meeting Tracker: deliberation sessions and site visits as the schedule of record, read by the Staff Deliberations rail and cycle view | **[SLICES 0–3 MERGED TO MAIN 2026-09-10 (S502): #217 gate, #218 rail, #222 Codex slices 1–2 with adversarial-review fixes; INERT until the owner applies Wave 28 to production and sets `MEETING_TRACKER_SCHEMA_READY=on` (sandbox 22/0/0). Slice 2b (site-visit editor) was built S503 (2026-09-10) and the briefing page's session seam reads the tracker's schedule reader; the applicant-materials collection (PR 1–2 S503, PR 3 S506) is the visit page's card plus list-row cue.]** All 23 advancing D26 requests hold an AI-generated Pre-Site draft (owner-run batch 2026-09-09). The owner described two meetings per proposal — a weekly internal deliberation session (staff + select Board, ~15 min per proposal, linked access to the shared document, reviews, and proposal) and the PC-scheduled site visit — and directed a PC-owned app as the schedule of record. Decisions D1–D12 and the plan are in `docs/PC_MEETING_TRACKER_PLAN.md` (D10–D12 on 2026-09-09 evening: fixed staff attendees plus per-session Board; each slot links the external briefing room, not the Workbench; name Meeting Tracker). First deliberation session is the week of 2026-09-14; slices 1–2 briefed to Codex in `docs/plans/MEETING_TRACKER_CODEX_BRIEF_2026-09-09.md`; the Staff Deliberations tab redesign (Share = lock + send, session slot) is shaped in `docs/plans/STAFF_DELIBERATIONS_TAB_SHAPE_BRIEF_2026-09-09.md`: own app (grant means edit, org-open posture), site visits stay on `wmkf_sitevisit`, sessions are a new table with ordered slots and no uniqueness on request, "Shared" = locked, stage labels admin-editable behind stable keys. Slice 0 (built 2026-09-09, Opus-reviewed): `saveSiteVisitLogistics` used to refuse writes until the draft was in Review (`assertActiveStage`), contradicting a PC scheduling in advance; it now gates writes on the Request-list visibility predicate plus a meeting date (`isVisibleRequestRow`), and reads are not gated. Supersedes in part the 2026-08-28 removal of scheduling from the Workbench (it returns on a PC surface) and the Codex materials plan's "no scheduling" exclusion (not in that release, not never). "Visited" is date-derived, every advancing D26 request is visited (J27 will allow reviewed-but-not-visited, behind one predicate), and there is no confirmation step (D7–D9, 2026-09-09); session attendance defaults and the bundle-vs-links question are open (plan §9). Form work on the rail waits for slice 3. | **Met when:** the PC can attach a deliberation slot and a site visit to every advancing request in a cycle from one app, moving either updates the rail and cycle view without PD action, the materials composer's calendar invite still comes from the Activity, and the stage labels can be renamed in Admin without a deploy. Companion (S502): the deliberation briefing page (D13–D16, `docs/DELIBERATION_BRIEFING_PAGE_PLAN.md`) is source-built on `feature/deliberation-briefing-page`, flag-gated off; its session line reads the tracker's `getDeliberationScheduleByRequests` once slice 2 lands (`lib/services/deliberation-briefing/session-reader.js` is the null seam). |
| 10 | Email send feedback and consistency audit (system-wide) | **[OWNER-DIRECTED 2026-09-11 (S507); NOT STARTED; SUBAGENT SWEEP FIRST.]** Owner ask: immediate feedback everywhere the app sends email, and one consistent voice. Two triggers: (a) the 2026-09-10 deliberation send on ZZTEST-03 failed 403 in Dataverse but the sender saw nothing at send time; the red "last send failed" line only appears on the next Staff Deliberations load; (b) send confirmations carry a clumsy disclaimer: "Sent — Dynamics accepted this exact email for transport. This receipt does not assert inbox delivery." (`PreSiteDistributionPanel.js`, `SessionAgendaPanel.js`) and "Recorded in the record; delivery not confirmed." (`reviewer-activity-history.js`); ledger `sent` = transport accepted. Scope: fan subagents across every send path (`lib/services/dynamics/email.js` callers: reviewer invitations/reminders, materials invitations, deliberation distribution, agenda email, grantee deliverables, admin test email, scheduled emails) and every surface that reports a send; inventory what each shows at click time, on failure, and after; then propose one contract (immediate success/failure at click, one "sent" definition, one wording) before any edit. `/contract-reconcile` on the proposal; no copy changes without the inventory. | Owner accepts one send-feedback contract and the inventory; edits then land per surface as Tier 1 PRs. |
| 11 | Cycle Dossier superuser pilot (D26): land, smoke, then pilot-mode hardening | **[SMOKE SUCCEEDED 2026-09-12 (S508): run completed, item ready, edition 7b656b74 assembled 11:14 PDT, PDF reviewed by the owner; DOCX+PDF in the request's `AI Artifacts/Cycle Dossier/D26/<revision>/` folder. Four production fixes landed on the way: #260 destinationHash (JSONB key order), #261 structural DOCX verification, #262 customXml relationships + differing-part logging, #264 `[trash]` slots; #263 api_usage_log per paid stage. Total spend $0.135 (research + entry once; every retry was free).]** Prior state: `claude/cycle-dossier-merge-main` = the S499 reconcile branch merged onto main with migration renumbered to 045, all gates and 888 jest suites green, `/contract-reconcile` READY FOR SMOKE, Codex adversarial review fixed on-branch (roster-resolved smoke selection, resume cap floor, Applicant-lookup institution, 60 s checkpoint window). Production env set: smoke mode, operator profile 2, allowlist 1002852, enabled=false. Owner sequence in the PR body (migration 045 → seed prompts → preflight → enable → launch). PR #179 to be closed with a pointer. **Before pilot mode (more than one request/operator):** stop re-read inside LLMClient retries and between folder POSTs; preview/Blob retention job (owner decision open since S494); drain-cron cadence (ops 2026-09-14); UI retry affordance and dead `pausing`/`cancelling` strings; `Content-Disposition` filename escaping; round the page's USD estimate to two decimals (smoke 2026-09-12 showed `$2.87775–$14.38875`; owner wants `$2.88–$14.39`); rename the "Retry failed" button (reads as a status; owner 2026-09-12) to "Retry failed entries"; move or duplicate the error banner next to the launch control (it renders above the fold and the owner missed the 409 message); the Progress tab should be the default view when the latest run is not settled; SharePoint copies must not be opened in Word Online before the item is ready (Word Online autosaves a re-serialised package that fails verification; recovered by restoring version 1.0), so the page should say so or the verification should run before any human can touch the file. **2026-09-12 (S509):** pilot-mode hardening built on `claude/dossier-pilot-hardening` (Sonnet agents, Fable review): Content-Disposition escaping; two-decimal USD; "Retry failed entries"; inline error beside Launch and the run controls; Progress tab default while a run is unsettled; dead `pausing`/`cancelling` removed; Word Online warning; operator stop re-read inside in-flight LLM calls and `ensureFolderPath` (Executor `signal` argument). Still open: preview/Blob retention (owner), drain-cron cadence (ops). **2026-09-12 (S508):** `CYCLE_DOSSIER_REQUEST_ALLOWLIST` had never been saved in Vercel (preflight reads the shell env, so it did not prove Vercel state); owner added it as a readable config var and redeployed; smoke preview then returned a known estimate. | Owner smoke on 1002852 succeeds end to end (entry DOCX/PDF in the request's AI Artifacts folder); then a pilot-hardening slice is scoped from the accepted-for-smoke list. |

## Audit follow-ups — verified open, not silently prioritized
- **Two Dependabot alerts need major upgrades (triaged 2026-09-12, S509).** The
  patch-level fixes (Next 16.3.5, xmldom 0.8.15, sharp 0.35.4, qs 6.16.0) shipped
  in the same session. Left open: `@tiptap/core` 2.27.x prototype pollution via
  `mergeAttributes` (GHSA-cp6q-959q-f8rh) has no 2.x patch and needs the tiptap 3
  editor migration; low exposure (staff-facing editor, attacker-controlled
  extension attributes required). `csv-parse` 6.2.1 prototype replacement via the
  columns path (GHSA-8cw4-87c7-c6xx) needs csv-parse 7, a major bump used only by
  the IRS BMF importer on a trusted download; run the importer once after the bump.
...
- **Public/onboarding reviewer-token documentation reconciliation.**
  **[CLOSED BY RETIREMENT 2026-09-08.]** The `docs/onboarding/` Workbench decks
  ...
  parity-check follow-up is moot. The two residual follow-ups surfaced by that
  pass — `docs/REVIEWER_ENGAGEMENT_SPEC.md:110` (contradicted the same spec's
  `:64`/`:79` on the lifted freeze and respond-by re-minting) and
  `docs/agent-wiki/topics/finance-honoraria.md:41` (said the closeout honorarium
  decision was "deployment pending" although it is on `main`) — were both
  **resolved 2026-09-08 on branch `claude/j27-gate-tighten`, commit `c1f4893d`.**
