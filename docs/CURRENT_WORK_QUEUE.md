...
| 2 | Final Writeup persona rollout and complete dashboards | **[PRODUCTION-LIVE PROGRAM-AUDIENCE MATRIX / REVIEWED REPLACEMENT PLAN / PERSONAS DISABLED 2026-08-31.]** Commit `5573bca3` is live in Ready deployment `dpl_5DNuc2BV76RihwuWu8ZFYBgxBXE7`. The neutral acknowledgement role remains live for all 11 role-eligible members, while the published Research matrix contains nine current role members and excludes owner-confirmed Southern California staff Anneli Stone and Saskia Pallais. Signed-in Admin publication/readback survived a full reload; Request `1002788` rendered under Research with exactly those nine columns and zero browser-console errors. Allison Keller is owner-confirmed President. Beth Pruitt is owner-confirmed CSO and also has responsible-PD requests. John Sader is owner-confirmed as a Program Director. The owner rejected the unshipped three-team mechanism after its authorized apply made zero writes. `docs/FINAL_WRITEUP_PERSONA_CONFIGURATION_PLAN.md`, independently reviewed by Claude as **READY WITH NAMED CHANGES**, replaces it with explicit GUID-only, multi-valued responsibilities in the existing versioned Final Writeup Admin setting and the same one-panel/one-Publish surface. The disabled source branch still contains the superseded team prototype; all team IDs remain null and persona lenses remain hard-disabled. | **Next gate:** implement the reviewed v2 configuration contract while disabled, including v1 compatibility, safe stale-reference pruning, explicit no-lens assignments, ETag-guarded repair/downgrade, and prototype removal. Then migrate/read back the unchanged Research audience, prove representative PC/leadership Word access, enable and smoke-test the personas, and only then return to the complete Southern California audience. No elevated team privilege, outside Dataverse administrator, or runtime inference from names, titles, email, or program labels is required. |
