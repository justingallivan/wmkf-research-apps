...
| 3 | Retain structured individual review DOCX files in SharePoint and backfill D26 | **[WAVES 1–2 PRODUCTION-DEPLOYED INERT; WAVE 3 STORAGE/POINTER PROOF PASSED WITH A WORD-WEB LAYOUT DEFECT FOUND 2026-09-03.]** `main` commit `83da197f` is Ready in deployment `dpl_F3oZ9MDbnyFox7S8Ekdos7423ece`; both Production rollout variables remain absent. The exact approved Request `1002874` backfill created Agnes Karasik's DOCX and committed its pointer pair; independent Graph/Dataverse verification passed, and the owner confirmed the Workbench download succeeds and looks correct. Opening the retained item through akoyaGO/Word for the web split the tab-positioned `Proposal Review` title. The branch preserves the Production-used v1 templates and selects new v2 templates that directly right-align both titles without positioning tabs. The owner also simplified generated retention to request-level `Reviews/Review-<request>-<reviewer name>.docx`, with no intermediate generated or suggestion-GUID folders. Focused tests, package isolation checks, rendered inspection, and a fresh Production read-only manifest pass, but the fixes are not deployed and the existing item is unchanged at its legacy path/name. Replacement manifest hash `25f10fcd0347d7de02abcc8a744357d4e215fe56efa462ff8cc0a6eef99650ff` has 22 eligible missing files with unique destinations, one visible Request `1003223` test exclusion, zero blockers, and excludes completed Request `1002874`. The active contract is `docs/REVIEW_DOCX_SHAREPOINT_RETENTION_PLAN.md`. | **Next gate:** review and promote the template/path compatibility fixes, then explicitly repair Request `1002874` into `Reviews/Review-1002874-Agnes Karasik.docx`; only afterward consider approving the refreshed 22-file Wave 4 set. No additional write or forward cron activation is approved. |
...
- Review DOCX template implementation: **[PRODUCTION-LIVE + SIGNED-IN EXPORT
  ...
  Exact-item repair, remaining writes, and activation remain current work, not
  part of this completed template-output item.
