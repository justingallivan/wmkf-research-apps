...
| 3 | Retain structured individual review DOCX files in SharePoint and backfill D26 | **[WAVES 1–4 COMPLETE 2026-09-04 UTC.]** The v4 text-table templates, request-level `Reviews/Review-<request>-<reviewer name>.docx` target, guarded repair service, and backfill runtime are live on `main` at `3ba2a6ad` in Ready deployment `dpl_22wUzC1cCi4nhKTSFQftfaycucSh`. Both Production rollout variables remain absent, so the scheduled route is inert. Exact manifest `9254df9e5e504c79007391efc85d189e89e8b8b2ff80b8e4f11990baca08f4f8` executed 22 created / zero failed; row-by-row reconciliation matched all expected identities, destinations, governed hashes, and unique SharePoint items. Fresh post-write manifest `f17eae89d00bf1c85301ae7a0b7ad6f3de37ff394030fbb0d66de793d10b1b64` has zero eligible missing files, zero reconcile candidates, zero blockers, and only the visible Request `1003223` test exclusion. Request `1002874` remains the owner-confirmed Word Online proof at version `4.0`; its separate legacy-path item remains. | **Met for D26:** exact backfill and clean rerun are complete. Forward cron activation and legacy-file cleanup remain separate owner-gated work. |
...
- Review DOCX template implementation: **[PRODUCTION-LIVE + SIGNED-IN EXPORT
  ...
  SharePoint filing/repair route, now at `3ba2a6ad` /
  `dpl_22wUzC1cCi4nhKTSFQftfaycucSh`, but it is inert with both rollout
