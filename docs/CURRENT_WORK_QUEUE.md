...
| 3 | Retain structured individual review DOCX files in SharePoint and backfill D26 | **[WAVES 1–2 PRODUCTION-DEPLOYED INERT; REQUEST 1002874 NEW-PATH + V3 CONTENT REPAIRS PROVED 2026-09-03.]** `main` commit `83da197f` is Ready in deployment `dpl_F3oZ9MDbnyFox7S8Ekdos7423ece`; both Production rollout variables remain absent. The first Request `1002874` proof exposed Word-web title wrapping. v2 directly right-aligned the title and the first exact repair moved the file to request-level `Reviews/Review-<request>-<reviewer name>.docx`, but owner inspection showed Word Online still pushed the title below the logo. The source branch now selects v3 templates whose floating behind-text logo explicitly uses `wrapNone`. Exact content-repair manifest `ab98b779b660c77719c317f73b8f1004b08a898f7159971d6f5c97f9bfb2295d` versioned the same SharePoint item `01G4GVMSZZ25YPTP3RGFEK6LCT64W3JPX2` and filename from `1.0` to `2.0`; the prior version was retained and verified, and independent readback matched v3 hash `gdc1:E3KvF7rvlOaGoxps6DHihILCQlyDlSheguneB0F0ojw` with unchanged Dataverse pointers. The separate old legacy-path item also remains for later cleanup. The runtime fixes are not deployed and owner Word Online visual confirmation of version `2.0` is pending. Historical replacement population manifest hash `25f10fcd0347d7de02abcc8a744357d4e215fe56efa462ff8cc0a6eef99650ff` had 22 eligible missing files with unique destinations, one visible Request `1003223` test exclusion, and zero blockers, but is superseded by the v3 governed hash and must be regenerated. The active contract is `docs/REVIEW_DOCX_SHAREPOINT_RETENTION_PLAN.md`. | **Next gate:** owner visually confirms version `2.0` in Word for the web, then review and promote the template/path/repair safeguards and generate a fresh v3 Wave 4 manifest. No additional backfill write, old-file cleanup, or forward cron activation is approved. |
...
- Review DOCX template implementation: **[PRODUCTION-LIVE + SIGNED-IN EXPORT
  ...
  The exact path repair and subsequent v3 same-item content repair are complete
  and independently verified, with both the prior SharePoint version and old
  legacy-path item retained. Owner Word Online visual confirmation of version
  `2.0`, a fresh v3 population manifest, remaining writes, old-file cleanup,
