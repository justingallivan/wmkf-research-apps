...
| 2 | Final Writeup persona rollout and complete dashboards | **[PRODUCTION-DEPLOYED V2-CAPABLE / STORED V1 / PERSONAS DISABLED 2026-08-31.]** Commit `84bf465b` is live in Ready deployment `dpl_41SybgPYfJXGarf7UqcMGCLMy4KS`, establishing the reviewed v2-capable floor: strict v1/v2 reads, v2-only atomic publication, explicit multi-valued/no-lens assignments, safe stale pruning, dry-run-first ETag upgrade/repair/downgrade tooling, and one consolidated Admin editor. Signed-in Production Admin readback proved the stored v1 setting already contains a nine-person Research audience and a six-person Southern California audience; the Research dashboard still renders its expected nine reviewer columns. The draft contains complete explicit responsibility rows for all 11 current reviewer-role members, including Beth Pruitt's PD + Leadership overlap, and was not published. The superseded team source is removed and persona lenses remain hard-disabled. | **Next gate:** after explicit write authorization, dry-run and publish/read back both unchanged program audiences as v2, prove representative PC/leadership Word access, then enable and smoke-test the personas. No elevated team privilege, outside Dataverse administrator, or runtime inference from names, titles, email, or program labels is required. |
