...
| 13 | Client request layer: fix unguarded response handling (D1) after each file migrates onto `shared/utils/api-request.js` | **[OWNER PRIORITY 2026-09-20; BRANCH-BUILT 2026-09-20 on `codex/d1-response-handling`; fresh review pending.]** The roles, providers, scheduled-email posture GETs, and remaining D9 `ErrorAlert` props are implemented with focused tests. This is not yet a release or deployment claim. Source plan and execution details remain in `docs/plans/CLIENT_REQUEST_LAYER_D1_UNGUARDED_RESPONSES_2026-09-20.md` and `docs/plans/CLIENT_REQUEST_LAYER_EXECUTION_2026-09-19.md`; owner controls commit and promotion. | Every confirmed unguarded site shows the server error on non-2xx, each with a test; execution log carries a "D1 fixes" section; campaign-critical rows released under Tier 2. |
