...
| 3 | Retain structured individual review DOCX files in SharePoint and backfill D26 | **[WAVES 1–2 PRODUCTION-DEPLOYED INERT; WAVE 3 SOURCE-BUILT 2026-09-03.]** `main` commit `83da197f` is Ready in deployment `dpl_F3oZ9MDbnyFox7S8Ekdos7423ece`. The shared builder plus guarded exact-cycle filing/repair cron are deployed; both rollout variables are absent. An authenticated Production call returned `enabled:false`, and the job's maintenance-run population remained zero before/after. The Wave 3 branch now adds the dry-run-default operator script, redacted hash-bound manifest, complete pre-write population/source drift check, local same-day acknowledgement gate, and per-row manifest binding to the existing ensure service. Focused tests pass; external adversarial review and every Production run remain open. The active contract is `docs/REVIEW_DOCX_SHAREPOINT_RETENTION_PLAN.md`. | **Next gate:** adversarially review the Wave 3 build, then run a read-only D26 dry run to create the first fresh manifest. Stop for separate explicit approval before executing even one exact SharePoint/Dataverse write smoke. Forward cron activation remains Wave 5. |
...
- Review DOCX template implementation: **[PRODUCTION-LIVE + SIGNED-IN EXPORT
  ...
  source-built and focused-test verified but has not been externally reviewed
  or run against Production; live manifest creation, writes, and activation
