...
| 2 | Final Writeup persona rollout and complete dashboards | **[PRODUCTION-LIVE V1 PROGRAM MATRIX / SOURCE-BUILT V2 / PERSONAS DISABLED 2026-08-31.]** Commit `5573bca3` is live in Ready deployment `dpl_5DNuc2BV76RihwuWu8ZFYBgxBXE7`. The published Research matrix contains nine current reviewer-role members and excludes owner-confirmed Southern California staff Anneli Stone and Saskia Pallais. Allison Keller is owner-confirmed President. Beth Pruitt is owner-confirmed CSO and also has responsible-PD requests. John Sader is owner-confirmed as a Program Director. The reviewed v2 replacement is now source-built: strict v1/v2 reads, v2-only atomic publication, explicit multi-valued/no-lens assignments, safe stale pruning, dry-run-first ETag upgrade/repair/downgrade tooling, and one consolidated Admin editor. The superseded team source is removed. Production still stores v1 and persona lenses remain hard-disabled. | **Next gate:** complete release gates and Claude implementation review, then deploy the v2-capable rollback floor. After explicit write authorization, dry-run and publish/read back the unchanged Research audience as v2, prove representative PC/leadership Word access, enable and smoke-test the personas, and only then return to the complete Southern California audience. No elevated team privilege, outside Dataverse administrator, or runtime inference from names, titles, email, or program labels is required. |
