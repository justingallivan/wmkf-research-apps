...
| 2 | Final Writeup persona rollout and complete dashboards | **[PRODUCTION-LIVE MATRIX / PROGRAM-AUDIENCE CONFIG SOURCE-BUILT / ROLLOUT PARTIAL 2026-08-31.]** The neutral acknowledgement role is live for all 11 role-eligible members, and the deployed superuser matrix still uses that role-wide roster. The owner clarified that per-request matrix assignment is narrower: Anneli Stone and Saskia Pallais are Southern California staff and should not appear on Research requests. A source-built, unit-tested Admin editor now supports separate broad Grant Program GUID → reviewer GUID audiences using each request's existing `wmkf_grantprogram`; it is not deployed and no live setting has been published. Missing configuration retains the deployed role-wide matrix; after publication, unconfigured request programs are explicit and stale references fail closed. Allison Keller is owner-confirmed President. Beth Pruitt is owner-confirmed CSO and also has responsible-PD requests. John Sader is owner-confirmed as a Program Director. The three no-privilege persona teams remain unprovisioned and persona lenses hard-disabled. | **Next gate:** review, deploy, and publish the Research/Southern California matrix audiences deliberately; then provision/verify the exact no-privilege PD, PC, and leadership teams, pin their GUIDs, and prove representative PC/leadership Word access before enabling persona lenses. No runtime inference from names, titles, email, or program labels. |
