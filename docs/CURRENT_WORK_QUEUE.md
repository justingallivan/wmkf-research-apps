...
| 8 | J27 single-phase transition inventory (register of what retires, persists, changes, or must be built) | **[OWNER-DIRECTED 2026-09-06; SWEEP RUN; REGISTER BUILT; CODEX REVIEW AND OWNER ANSWERS PENDING.]** The owner asked to collect every J27-sensitive site in one place and plan the June 2027 build and change work. `docs/J27_TRANSITION_REGISTER.md` now holds the sweep output: 76 rows across Retire / Persist / Change / Build / Scale (Codex-reviewed 2026-09-06, four findings fixed), seven recorded contradictions, 25 owner questions, and the owner-run Dataverse checklist. The `J27:` marker convention and check script are not yet built. `docs/J27_SINGLE_PHASE_TRANSITION_INVENTORY_PLAN.md` defines the register schema (category, site, excerpt, cycle dependency, evidence label, owner decision, disposition), a six-slice read-only agent sweep plus an owner-run Dataverse slice, a Codex verification pass, a `J27:` marker convention with a check script, and five owner questions starting with the J27 calendar. Retire rows are candidates, not authorizations. Owner answered the work-shaping questions on 2026-09-06 (phase trigger, staff review surface, triage states, GOApply intake, D26 residue deletion, cycle-boundary notes); seven J27 questions are filed in the register's §7 as important-not-urgent, and Connor's early decision on the J27 proposal file location heads the slice G checklist. Next: the marker check script, then the separate J27 build plan once Connor's file-location decision lands. | **Met when:** every register row is labelled and verified, the owner questions are answered or queued, and the marker check script is referenced from `docs/CI_GATES_REFERENCE.md`. |
