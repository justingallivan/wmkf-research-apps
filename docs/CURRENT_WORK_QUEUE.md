...
| 5 | Final writeups dashboard completion: cycle scoping, filters, and residuals (Editor Dashboard contract) | **[OWNER-DIRECTED 2026-09-06; PLANNED, NOT BUILT; PRIORITIZED ABOVE ITEM 6.]** The owner reviewed the Editor Dashboard contract against the Production-live Final writeups dashboard and decided: keep and build the contract filters (cycle, program/PD, artifact stage, editing/review state) as navigation-only controls; retire inline preview (Word opens outside the Workbench); keep the residuals. Boundary [VERIFIED S491 via source]: the dashboard lists every current Final across all cycles under one hard 100-row cap that fails the whole page (503) when exceeded, renders no cycle or publication version, and has no filter controls or "has edits" hint. Build order and contracts are Slice 6 of `docs/FINAL_WRITEUP_REVIEW_IMPLEMENTATION_PLAN.md`: 6A server-side cycle scoping (dated: before D26 Final writeups exist; **planned S491 in `docs/FINAL_WRITEUPS_DASHBOARD_CYCLE_SCOPING_PLAN.md` with the `/contract-reconcile` trace and invariant table; Codex plan review next; not built**), 6B client-side filters persisted in the URL, 6C current-version context in place of preview, 6D "has edits" hint and 6E other-stage lists as unscheduled product calls. No approval gates, denominators, program-taxonomy grouping, or authorization changes. | **Met when:** a signed-in user can select a cycle and filter the open/history/stewardship queues and the coordinator matrix by PD, stage, and review state with the filtered view bookmarkable; the publication version acknowledgements key to is visible per row; two cycles of Finals load without the scope-exceeded error. 6D/6E close by owner decision, not by build. |
