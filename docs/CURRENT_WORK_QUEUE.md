...
| --- | --- | --- | --- |
| 1 | Remaining lifecycle design freeze | Deliver the shared governed-artifact foundation and a human-in-the-loop Initial Assessment pilot by 2026-08-10, before proposal intake begins around 2026-08-18. The owner chose a controlled production rehearsal using colleague-created representative dummy requests rather than building the existing Dataverse sandbox organization into an integrated application/file test environment. The dummy request IDs, exact content shape, authorized testers, and schedule remain pending. The pilot exercises the intended Workbench generation entry point, representative proposal/Dataverse inputs, governed prompt execution, canonical SharePoint Word creation and human editing, typed Dataverse registry/provenance, Workbench discovery/Open in Word, cycle-wide pilot-locator discovery/Open in Word, and one safe failure/retry path. It is a draft-functional gate, not a broad production-readiness claim, and does not require the later lifecycle tabs. Finish the contracts for Pre Site Visit Writeup, the Site Visit dossier, Final Writeup, J27 Initial Assessment scale-out, and the staff-wide Editor Dashboard that preserves Allison's single-list editing workflow while supporting all PDs and designated staff proofreaders. SharePoint Word is the decided narrative authority; Dataverse is the typed registry/workflow authority. The three writeups are separate documents, Final is copied from the latest Pre-Site version at action time with a rare safe regeneration option, and Site Visit is a dossier rather than a fourth writeup. Pre-Site inputs are decided: full-proposal factual material through an iterated governed `phase-ii.summarize`, all currently submitted reviews through `review-synthesis.generate`, valid zero-review distribution, independent review refresh, and a versioned replaceable Word template. Site Visit logistics and its six material categories are also decided, with one paste-friendly staff-observations area and no separate visit-status field. Applicant uploads accept PDF/PPTX, allow additional uploads while access remains active, and are capped at 1 GB per file and 20 current applicant files per request. Files land in the request's governed SharePoint folder under `Site Visit/Applicant Materials/Slides` or `Other`. Applicants see current files and operation confirmations only; staff sees action/file/category/size/time/request/link metadata and uses native SharePoint recovery, with no custom restore control initially. Successful file changes are batched into a short email digest to the lead PD and a still-to-be-defined staff audience; do not hard-code a program coordinator as the only additional recipient. The request is manually staff-triggered, never sent automatically from a visit-date change. Site Visits are scheduled promptly after advancement, around reviewer invitations; once the date is recorded, staff may send without waiting for reviews, synthesis, or a Pre-Site Writeup. Any staff member with Workbench Site Visit access may send, resend, or reissue. Recipient choices are the Dataverse-linked liaison and PI: normally liaison in To, or PI in To with liaison optionally copied. Missing, invalid, or duplicate selected addresses block sending until staff corrects Dataverse; there is no free-form bypass. To and CC share one request-scoped link and manage the same file list; person-level action attribution is not promised. Expiration is automatically 60 days after successful send and is unaffected by visit rescheduling. Resend preserves the active link and expiry; Reissue/restart stages a replacement and revokes the old link only after the new invitation is accepted for sending, so failure does not destroy a still-active link. No standalone Revoke action is needed in the minimum product. SharePoint is the applicant-file byte store; Dataverse holds the artifact registry and Postgres only expiring-link/resumable-session workflow state. A new resumable Graph upload-session path is required because the current buffered helper stops at 60 MB. The Initial Assessment pilot schema/read model, provisional v1 prompt/template pair, exact `Artifacts/Initial Assessment/` destination, and `reviewers`-grant pilot-locator access are implemented in source. The complete production registry/pointer schema and governed prompt v1 were provisioned and verified on 2026-07-30; application promotion and the controlled artifact pilot remain pending. Target-library protection and SharePoint permission verification, later-stage inputs, sender/reply-to and lead-PD copy behavior after owner/staff coordination, large-file scanner implementation, exact additional notification audience/digest window, transcript contracts after PC coordination, and Reviewed-marker granularity remain open. Historically, non-PD staff usually sent these requests without PD involvement; that is discussion context, not the future contract. | The August 10 controlled-production pilot succeeds end to end with representative dummy source data, human review/editing, the same registered artifact visible in Workbench and the pilot locator, and safe retry without false success; each later tab has approved user, inputs, producer, persistence, consumer, search, access, version/recovery/retention, milestone, and deadline contracts. |
| 2 | First deadline-bound writeup slice | Promote and exercise the source-built governed-artifact spine through the J27 Initial Assessment first. The production Dataverse schema and governed prompt gates are complete; application promotion and the controlled rehearsal remain. Pre Site Visit becomes the next reuse after the pilot rather than the default first slice. | One dedicated production dummy request produces a durable, searchable, editable and recoverable Word artifact; authorized staff review/edit it and find the same registered file from Workbench and the staff-wide pilot locator; one failure/retry path proves write-on-success. The full Editor Dashboard contract remains a later reuse. |
