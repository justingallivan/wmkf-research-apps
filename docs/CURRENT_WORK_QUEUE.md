...
| 3 | Retain structured individual review DOCX files in SharePoint and backfill D26 | **[WAVES 1–2 PRODUCTION-DEPLOYED INERT; REQUEST 1002874 NEW-PATH REPAIR PROVED 2026-09-03.]** `main` commit `83da197f` is Ready in deployment `dpl_F3oZ9MDbnyFox7S8Ekdos7423ece`; both Production rollout variables remain absent. The first Request `1002874` proof exposed Word-web title wrapping. The branch preserves v1, selects v2 templates that directly right-align both titles, and simplifies generated retention to request-level `Reviews/Review-<request>-<reviewer name>.docx`. The owner authorized an exact manifest-bound repair: it created SharePoint item `01G4GVMSZZ25YPTP3RGFEK6LCT64W3JPX2` as `Review-1002874-Agnes Karasik.docx`, ETag-conditionally repointed Dataverse, and independent readback matched the corrected semantic hash. The old item remains present for later cleanup. The runtime fixes are not deployed and owner Word-web visual confirmation of the repaired file is pending. Replacement population manifest hash `25f10fcd0347d7de02abcc8a744357d4e215fe56efa462ff8cc0a6eef99650ff` has 22 eligible missing files with unique destinations, one visible Request `1003223` test exclusion, zero blockers, and excludes completed Request `1002874`. The active contract is `docs/REVIEW_DOCX_SHAREPOINT_RETENTION_PLAN.md`. | **Next gate:** owner visually confirms the repaired file in Word for the web, then review and promote the template/path/repair safeguards before considering the refreshed 22-file Wave 4 set. No additional backfill write, old-file cleanup, or forward cron activation is approved. |
...
- Review DOCX template implementation: **[PRODUCTION-LIVE + SIGNED-IN EXPORT
  ...
  The exact-item repair is now complete and independently verified, with the old
  item retained. Owner Word-web visual confirmation, remaining writes, old-file
  cleanup, and activation remain current work, not part of this completed
