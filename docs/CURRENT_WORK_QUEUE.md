...
| 2 | Final Writeup persona rollout and complete dashboards | **[PRODUCTION-LIVE PROGRAM-AUDIENCE MATRIX / SOURCE-BUILT PERSONA PARTIAL 2026-08-31.]** Commit `5573bca3` is live in Ready deployment `dpl_5DNuc2BV76RihwuWu8ZFYBgxBXE7`. The neutral acknowledgement role remains live for all 11 role-eligible members, while the published Research matrix contains nine current role members and excludes owner-confirmed Southern California staff Anneli Stone and Saskia Pallais. Signed-in Admin publication/readback survived a full reload; Request `1002788` rendered under Research with exactly those nine columns and zero browser-console errors. The owner has now prioritized the global Research personas before completing the separate Southern California audience. Allison Keller is owner-confirmed President. Beth Pruitt is owner-confirmed CSO and also has responsible-PD requests. John Sader is owner-confirmed as a Program Director. Commit `2f064351` tracks a validated, dry-run-by-default three-team provisioning manifest. The disabled source branch implements PD group/own queues, PC all-row access plus the neutral matrix, leadership-stage queues, multi-persona union, and fail-closed unassigned behavior; 32 focused tests pass. An authorized Production team apply made zero writes because the application user lacks `prvCreateTeam`, so all three GUIDs remain null and persona lenses remain hard-disabled. | **Next gate:** a Dataverse administrator creates the exact zero-role PD, PC, and leadership owner teams or runs the tracked apply under an appropriately privileged operator identity. Engineering then verifies exact membership and zero-role state, pins the three GUIDs, proves representative PC/leadership Word access, enables and smoke-tests the personas, and only then returns to the complete Southern California audience. No runtime inference from names, titles, email, or program labels. |
