...
| 2 | Release template-backed review DOCX outputs | **[SCHEMA EXACT; PRE-RELEASE VERIFIED; NOT DEPLOYED 2026-09-03.]** The approved scope keeps the combined Reviews-tab action as an on-demand download and makes the individual review a template-backed courtesy attachment; this phase performs no SharePoint upload. The guarded combined-export route rereads Dataverse authoritatively. New categorical answer rows preserve the complete submitted option set in Wave 25 `wmkf_questionoptions`; historical rows use an explicit selected-only fallback. The owner-approved Production apply and exact readback are complete. The branch is current with `origin/main`; 306 affected-workflow tests, relevant gates/self-tests, types, lint (0 errors), production build, and template bundle traces passed. | **Next gate:** explicit owner approval to merge/push the reviewed five-commit branch to auto-deploying `main`, followed by Ready-deployment and signed-in export verification. |
...
- Review DOCX template implementation: **[SCHEMA EXACT; SOURCE-BUILT, NOT DEPLOYED
  ...
  current branch then passed 306 affected-workflow tests, relevant gates and
  self-tests, TypeScript, lint with zero errors, production build, and template
  bundle-trace checks. Explicit owner approval for merge/push and post-deploy
  signed-in verification remain. No SharePoint review-export workflow was built.
