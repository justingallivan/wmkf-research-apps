...
| 10 | Email send feedback and consistency audit (system-wide) | **[IMPLEMENTED ON BRANCH `codex/email-send-feedback` 2026-09-15; FULL REPOSITORY SUITE AND RELEVANT GATES GREEN; PRODUCTION PROOF PENDING.]** `docs/EMAIL_SEND_FEEDBACK_AUDIT_2026-09-15.md` records the complete send-path inventory and the implemented contract. Human-triggered surfaces now share accessible confirmed/failed/uncertain/partial/draft feedback; `sent` means Dynamics accepted for delivery, never inbox delivery. Pre-dispatch errors are definite failures, send-stage errors retain the activity id and stay uncertain unless a ledger reconciles them, batch outcomes retain recipient identities, and refresh failures cannot relabel accepted sends. Background at-most-once jobs now count failed versus unconfirmed separately. No migration or new retry loop was introduced. | **Met in source when merged:** inventory accepted, shared contract implemented, full tests and relevant gates green. **Operational close:** after promotion, smoke one ledger-backed send and one ordinary `createAndSendEmail` flow while observing the immediate result. |
