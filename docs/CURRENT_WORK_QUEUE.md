...
| --- | --- | --- | --- |
| 1 | Reviews/synthesis contract closure | Reliability is production-proven. The approved participating-invitation state machine, exact-fingerprint currentness ledger, automatic all-in drain, explicit manual early-run, stored-output visibility, and observability are implemented and focused-test-proven on `codex/review-synthesis-lifecycle`. **Release pending:** migration 028 is live-applied and verified empty; the branch is not deployed, automation remains disabled, and signed-in Preview/Production verification remains. | Deploy with automation disabled, verify manual/read-only behavior, then enable and smoke the bounded automatic path before declaring completion. |
