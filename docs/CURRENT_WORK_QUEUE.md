...
| 12 | Pre-Research Presentation Brief: fourth governed artifact replacing the Pre-Site writeup as the Board distribution source | **[SCHEMA LIVE 2026-09-16; CODE BUILT ON `claude/pre-rp-brief`, NOT MERGED, NOT DEPLOYED.]** Owner decisions B1–B14 and four Codex rounds in `docs/plans/PRE_RESEARCH_PRESENTATION_BRIEF_PLAN_2026-09-16.md`. Picklist `100000009` and pointer `wmkf_CurrentPreRPBrief` applied and re-read in Production. Slices 1–6 built and Opus-reviewed on the branch (slice 4: 2 rounds; slice 5: 3 rounds). Migration 052 applied to the shared database 2026-09-16 (readback exact) ahead of merge, expand-first. Codex adversarial round (gpt-5.6-sol) returned four findings; three code fixes landed in `7f0e0794` and were controller-reviewed with a mutation pass. | **Not met until** merge of PR #307 and a ZZTEST Production smoke (generate, lock, share, briefing page serves the brief, drift-acknowledgement path). Follow-ups in plan §8. |
