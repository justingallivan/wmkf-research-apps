...
| 12 | Pre-Research Presentation Brief: fourth governed artifact replacing the Pre-Site writeup as the Board distribution source | **[SCHEMA LIVE 2026-09-16; CODE BUILT ON `claude/pre-rp-brief`, NOT MERGED, NOT DEPLOYED.]** Owner decisions B1–B14 and four Codex rounds in `docs/plans/PRE_RESEARCH_PRESENTATION_BRIEF_PLAN_2026-09-16.md`. Picklist `100000009` and pointer `wmkf_CurrentPreRPBrief` applied and re-read in Production. Slices 1–6 built and Opus-reviewed on the branch (slice 4: 2 rounds; slice 5: 3 rounds). Migration 052 is on the branch and applied nowhere. | **Not met until** controller review + full gates/jest/lint/build, PR, owner-run Codex adversarial review, merge, `node scripts/apply-migrations.js`, and a ZZTEST Production smoke (generate, lock, share, briefing page serves the brief, drift-acknowledgement path). Follow-ups in plan §8. |
