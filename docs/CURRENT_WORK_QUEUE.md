...
| 3 | Retain structured individual review DOCX files in SharePoint and backfill D26 | **[WAVES 1–2 PRODUCTION-DEPLOYED INERT; REQUEST 1002874 NEW-PATH + V4 CONTENT REPAIRS PROVED AND OWNER-APPROVED 2026-09-03.]** `main` commit `83da197f` is Ready in deployment `dpl_F3oZ9MDbnyFox7S8Ekdos7423ece`; both Production rollout variables remain absent. The first Request `1002874` proof exposed Word-web title wrapping. v2 removed tabs, and v3 removed shape wrapping, but owner inspection still rejected the floating-logo alignment. The owner-edited v4 header is a fixed 9360-DXA two-column Times New Roman table with no image/drawing/anchor/tab geometry. Focused tests and full individual/combined renders pass. Exact content-repair manifest `18007d495f52ab7abb88c03e6d3099eadb953157e69051b0c4c96898881ef09e` versioned the same SharePoint item `01G4GVMSZZ25YPTP3RGFEK6LCT64W3JPX2` and filename from `2.0` to `3.0`; prior versions remain in SharePoint, and independent readback matched v4 hash `gdc1:fbIC8o5aWoe_rOjXNK6mAKR6kbNRQ_I6MU60R28Chi4` with unchanged Dataverse pointers. At the owner's explicit request, a further guarded upload wrote the same generated v4 output to the stable item as version `4.0` at `2026-09-04T00:59:51Z`; versions `1.0`–`3.0` remain in history, and independent post-write readback again matched the v4 hash with unchanged pointers. The owner visually confirmed version `4.0` in Word Online and approved the v4 header. The separate old legacy-path item also remains for later cleanup. The runtime fixes are not deployed. Fresh v4 population manifest hash `9254df9e5e504c79007391efc85d189e89e8b8b2ff80b8e4f11990baca08f4f8` has 22 eligible missing files with unique destinations, one visible Request `1003223` test exclusion, zero blockers, and no added or removed suggestion IDs versus the prior survey. The active contract is `docs/REVIEW_DOCX_SHAREPOINT_RETENTION_PLAN.md`. | **Next gate:** review and promote the template/path/repair safeguards, then obtain explicit approval for the bounded 22-file write set. No additional backfill write, old-file cleanup, or forward cron activation is approved. |
...
- Review DOCX template implementation: **[PRODUCTION-LIVE + SIGNED-IN EXPORT
  ...
  22 eligible missing files and no blockers; remaining writes, old-file cleanup,
