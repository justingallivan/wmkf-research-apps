...
| 2 | First deadline-bound writeup slice | **[VERIFIED IN SOURCE/TEST 2026-08-16]** The Workbench Pre Site Visit tab now has an interim local-download path: it resolves the exact AI Materials narrative and authoritative Dataverse metadata, executes governed `pre-site-visit.proposal-core.generate`, renders the tracked Word template, and returns the DOCX without a SharePoint or registry write. A read-only live prompt probe returned `action=create version=1`, so the current prompt row is not yet provisioned and no model call has run. The durable governed-artifact reuse still waits on the remaining library/readback gate above. | First provision and verify the governed prompt, then prove one dedicated request can download a correct draft. The later durable milestone remains a searchable, editable, recoverable Word artifact with complete prompt/run/request lineage and verified human review; the full Editor Dashboard contract remains later reuse. |
