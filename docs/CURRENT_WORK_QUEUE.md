...
| --- | --- | --- | --- |
| 1 | Close the remaining Initial Assessment pilot gates | Request `1002788` preserves mechanics-only evidence because it used an old Phase I proposal. PR #103 / merge `84155a5a` deployed the exact active `Reviewer Materials/Proposal_{Request#}.pdf` contract, and Request `1003109` then production-proved canonical-input generation, a newly linked AI run, Ready/Draft request-pointer lineage, exact-input no-duplicate reuse, interrupted-finalization recovery, and an attributed substantive edit including Foundation Opportunity. Both consumers still resolve the same stable item. **[VERIFIED 2026-07-30]** Graph-current metadata display is live; a disposable production-library probe also proved previous-version inspection/restore and signed-in first-stage recycle recovery. Before declaring the August 10 gate complete, obtain administrator evidence for second-stage recovery, Purview retention, and editor least privilege (version limits are now answered), then implement Workbench history/admin restore and milestone snapshots. **The August 10 date is a deliberately early internal buffer, NOT an external commitment (owner, 2026-08-10 / S412).** It has passed without being met — and that is expected, not a missed deadline. **Connor replied 2026-08-10 (S413) and three of the four checks are still not closed:** the version policy is **ANSWERED** — read from the signed-in Versioning Settings page for `akoya_request`: major versions only, no time limit, keep 500 major versions, check-out not required. The residual is administrative: an admin can lower the limit, after which SharePoint gradually trims excess old versions on later edits; the policy can be monitored with authorized SharePoint/PnP read-only cmdlets. No second-stage recycle bin was reported (unusual for SharePoint Online, and Microsoft documents that view as site-collection-admin-only — confirm with a site-collection admin before relying on it); Purview is unanswered and rerouted to an M365 compliance admin; the actual Members permission assignment/definition and `akoya_request` inheritance are unresolved. See `docs/INITIAL_ASSESSMENT_CONTROLLED_PILOT_2026-07-30.md` item 5 and, for owner routing, exact questions, closing criteria, and send-ready messages, `outputs/sharepoint-storage-policy-question-brief.md` (S415). Do not treat the date as a live deadline or raise it as slippage; the completion decision below is the real gate. | Current metadata, native version restore, and first-stage recovery are proven; administrator policy/access evidence and product history/milestone controls are recorded or completed. |
