...
| --- | --- | --- | --- |
| 1 | Close the remaining Initial Assessment pilot gates | Request `1002788` preserves mechanics-only evidence because it used an old Phase I proposal. PR #103 / merge `84155a5a` deployed the exact active `Reviewer Materials/Proposal_{Request#}.pdf` contract, and Request `1003109` then production-proved canonical-input generation, a newly linked AI run, Ready/Draft request-pointer lineage, exact-input no-duplicate reuse, interrupted-finalization recovery, and an attributed substantive edit including Foundation Opportunity. Both consumers still resolve the same stable item. **[VERIFIED 2026-07-30]** Graph-current metadata display is live; a disposable production-library probe also proved previous-version inspection/restore and signed-in first-stage recycle recovery. Before declaring the August 10 gate complete, obtain the remaining administrator evidence — **`Request` library permission inheritance**, and Purview retention at low priority — then implement Workbench history/admin restore and milestone snapshots (**scope reduced**: Diligent holds the Board record, so confirm with the owner that snapshots are still worth building). **The August 10 date is a deliberately early internal buffer, NOT an external commitment (owner, 2026-08-10 / S412).** It has passed without being met — and that is expected, not a missed deadline. **Administrator evidence closed three of the four checks across two rounds (2026-08-10 / S413 and 2026-08-13 / S425).** Version limits: major only, no time limit, keep 500 — the platform default, so untouched configuration. Second-stage recovery: **a bin exists**, reversing the 2026-08-10 "none"; 93 days from the original deletion, shared across both stages, `dftadmin`-only restore, and interruptible by quota purge or a manual empty. Editor least privilege: **answered unfavourably** — "limited control" was a pane caption, the Members group holds `Edit`, and ordinary editors can delete documents and purge version history; `Manage Lists` also puts the version limit within their reach. **Purview retention is the one original question still unanswered, but it is low priority** — no regulatory retention obligation applies to these documents (owner, 2026-08-13), so a policy would be an unclaimed safety net rather than a gate; needs an M365 compliance admin if ever wanted. **New and open:** Members contains `Everyone except external users`, so `Edit` reaches every licensed internal account at *site* scope — whether it reaches the `Request` library depends on unread `HasUniqueRoleAssignments`, and that qualifier must travel with the finding. See `docs/INITIAL_ASSESSMENT_CONTROLLED_PILOT_2026-07-30.md` item 5 and `docs/DATAVERSE_SHAREPOINT_FILE_MODEL.md`. Do not treat the date as a live deadline or raise it as slippage; the completion decision below is the real gate. | Current metadata, native version restore, and first-stage recovery are proven; administrator policy/access evidence and product history/milestone controls are recorded or completed. |
...
- Initial Assessment substantive human edit: Request `1003109`'s canonical
  ...
  passed in the production Request library. Library-limit, second-stage, and
  editor-permission evidence is now **in** (2026-08-10 / 2026-08-13). Purview
  retention, `Request` library permission inheritance, Workbench
  history/admin-restore, and milestone controls remain in the open
  library/readback gate.
