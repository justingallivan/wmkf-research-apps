...
| 3 | Retain structured individual review DOCX files in SharePoint and backfill D26 | **[WAVES 1–2 PRODUCTION-DEPLOYED INERT; REQUEST 1002874 NEW-PATH + V4 CONTENT REPAIRS PROVED 2026-09-03.]** `main` commit `83da197f` is Ready in deployment `dpl_F3oZ9MDbnyFox7S8Ekdos7423ece`; both Production rollout variables remain absent. The first Request `1002874` proof exposed Word-web title wrapping. v2 removed tabs, and v3 removed shape wrapping, but owner inspection still rejected the floating-logo alignment. The owner-edited v4 header is a fixed 9360-DXA two-column Times New Roman table with no image/drawing/anchor/tab geometry. Focused tests and full individual/combined renders pass. Exact content-repair manifest `18007d495f52ab7abb88c03e6d3099eadb953157e69051b0c4c96898881ef09e` versioned the same SharePoint item `01G4GVMSZZ25YPTP3RGFEK6LCT64W3JPX2` and filename from `2.0` to `3.0`; prior versions remain in SharePoint, and independent readback matched v4 hash `gdc1:fbIC8o5aWoe_rOjXNK6mAKR6kbNRQ_I6MU60R28Chi4` with unchanged Dataverse pointers. At the owner's explicit request, a further guarded upload wrote the same generated v4 output to the stable item as version `4.0` at `2026-09-04T00:59:51Z`; versions `1.0`–`3.0` remain in history, and independent post-write readback again matched the v4 hash with unchanged pointers. The separate old legacy-path item also remains for later cleanup. The runtime fixes are not deployed and owner Word Online visual confirmation of version `4.0` is pending. Historical replacement population manifest hash `25f10fcd0347d7de02abcc8a744357d4e215fe56efa462ff8cc0a6eef99650ff` had 22 eligible missing files with unique destinations, one visible Request `1003223` test exclusion, and zero blockers, but is superseded by the v4 governed hash and must be regenerated. The active contract is `docs/REVIEW_DOCX_SHAREPOINT_RETENTION_PLAN.md`. | **Next gate:** owner visually confirms version `4.0` in Word for the web, then review and promote the template/path/repair safeguards and generate a fresh v4 Wave 4 manifest. No additional backfill write, old-file cleanup, or forward cron activation is approved. |
| 4 | Final Writeup persona rollout and complete dashboards | **[PRODUCTION-LIVE V2 / PERSONAS DISABLED 2026-09-01 UTC.]** Commit `84bf465b` is live in Ready deployment `dpl_41SybgPYfJXGarf7UqcMGCLMy4KS`, establishing the reviewed v2-capable floor. The dry-run-first operator command then upgraded `final_writeup.matrix_audiences` once under ETag `W/"96930393"`; exact readback proved version 2 at `W/"96944113"`, all 11 complete explicit assignments including Beth Pruitt's PD + Leadership overlap, zero stale/unassigned rows, and unchanged nine-person Research and six-person Southern California audiences. Signed-in Admin reloaded as **Published revision loaded**, and Request `1002788` retained the expected nine-column Research matrix. The superseded team source is removed and persona lenses remain hard-disabled. **[OWNER-REPORTED 2026-09-03]** Representative Word access is proved: President Allison Keller (Leadership) opened the canonical Word item through the signed-in Final Writeups experience and marked it reviewed on 2026-09-03, and Program Coordinators Duncan Spore and Sarah Hibler did the same (dates not recorded). The owner saw Allison's acknowledgement on the dashboard; no independent Dataverse readback was run. | **Next gate after the review-DOCX retention work:** deliberately enable (source flag flip, own commit and deployment) and smoke-test PD, PC, Leadership, overlap, unassigned/ineligible, and superuser behavior. No elevated team privilege, outside Dataverse administrator, or runtime inference from names, titles, email, or program labels is required. |
...
- Review DOCX template implementation: **[PRODUCTION-LIVE + SIGNED-IN EXPORT
  ...
  SharePoint filing/repair route at `83da197f` /
  `dpl_F3oZ9MDbnyFox7S8Ekdos7423ece`, but it is inert with both rollout
  variables absent. The Wave 3 backfill is now
  ...
  complete and independently verified, with prior SharePoint versions and the
  old legacy-path item retained. The owner-requested repeat generation/upload
  advanced the stable item to version `4.0`, with exact v4 readback and versions
  `1.0`–`3.0` retained. Owner Word Online visual confirmation of version `4.0`,
  a fresh v4 population manifest, remaining writes, old-file cleanup,
  and activation remain current work, not part of this completed
