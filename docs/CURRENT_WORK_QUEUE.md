...
| 8 | J27 single-phase transition inventory (register of what retires, persists, changes, or must be built) | **[OWNER-DIRECTED 2026-09-06; SWEEP RUN; REGISTER BUILT; GATE BASELINE CURRENTLY RED ON BRANCH `claude/j27-gate-tighten` PENDING RECONCILIATION.]** The owner asked to collect every J27-sensitive site in one place and plan the June 2027 build and change work. `docs/J27_TRANSITION_REGISTER.md` holds the sweep output: 76 rows across Retire / Persist / Change / Build / Scale, seven recorded contradictions, 25 owner questions, and the owner-run Dataverse checklist. The `J27:` marker convention and advisory check script `scripts/check-j27-register.js` were built 2026-09-07 and merged to `main` (PR #180/#181). **2026-09-08 (unmerged, branch `claude/j27-gate-tighten`):** the owner decided to keep the strict per-site excerpt rule and reconcile the register with subagents rather than weaken the gate; Codex's adversarial review of that plan found the gate still had no site-to-fragment binding (a reconciled row could pass on a coincidental match), so Phase 0 (schema fix) landed first — binding syntax (`` `path` → `fragment` ``), per-file bound/bag isolation, no more directory-existence pass, and disposition-vocabulary enforcement. An Opus review of that Phase 0 commit found the binding schema itself still had four gaps (silent orphan bindings, a vacuous ASCII-arrow self-test, silently-dropped too-short bindings, and a bag fragment that could vacuously match a file quoting its own path) — all fixed same day, plus the owner's STRICT UNBOUND decision ("we don't need drift": an unbound file in a multi-site row is now itself a failure, not an info flag). Applying the fully-fixed gate to the real register: 12 ok, **47 stale**, 6 unverifiable, 11 closed; 47 multi-site rows flagged `unbound` (now itself a stale reason, not informational). **Next action is `docs/plans/J27_REGISTER_PER_SITE_RECONCILIATION_PLAN_2026-09-08.md` Phase 1** — four parallel Sonnet subagents reconcile the 47 stale rows against the new binding syntax (the plan's slice assignment must be re-run against this count before assigning), then two Opus reviewers and a Codex adversarial pass before a PR to `main`. The separate J27 build plan still waits on Connor's file-location decision (parked, he is out). | **Met when:** every register row is labelled and verified, the owner questions are answered or queued, the gate is green on `main` (0 stale, 0 unbound, 0 vocabulary warnings), and the marker check script is referenced from `docs/CI_GATES_REFERENCE.md`. |
