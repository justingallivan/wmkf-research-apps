...
| 9 | PC Meeting Tracker: deliberation sessions and site visits as the schedule of record, read by the Staff Deliberations rail and cycle view | **[OWNER-DIRECTED 2026-09-09 (S501); PLANNED, NOT BUILT.]** All 23 advancing D26 requests hold an AI-generated Pre-Site draft (owner-run batch 2026-09-09). The owner described two meetings per proposal — a weekly internal deliberation session (staff + select Board, ~15 min per proposal, linked access to the shared document, reviews, and proposal) and the PC-scheduled site visit — and directed a PC-owned app as the schedule of record. Decisions D1–D6 and the plan are in `docs/PC_MEETING_TRACKER_PLAN.md`: own app (grant means edit, org-open posture), site visits stay on `wmkf_sitevisit`, sessions are a new table with ordered slots and no uniqueness on request, "Shared" = locked, stage labels admin-editable behind stable keys. Blocking finding: `saveSiteVisitLogistics` refuses writes until the draft is in Review (`assertActiveStage`), which contradicts a PC scheduling in advance; slice 0 replaces it with the Request-list visibility predicate. Supersedes in part the 2026-08-28 removal of scheduling from the Workbench (it returns on a PC surface) and the Codex materials plan's "no scheduling" exclusion (not in that release, not never). "Visited" is date-derived, every advancing D26 request is visited (J27 will allow reviewed-but-not-visited, behind one predicate), and there is no confirmation step (D7–D9, 2026-09-09); session attendance defaults and the bundle-vs-links question are open (plan §9). Form work on the rail waits for slice 3. | **Met when:** the PC can attach a deliberation slot and a site visit to every advancing request in a cycle from one app, moving either updates the rail and cycle view without PD action, the materials composer's calendar invite still comes from the Activity, and the stage labels can be renamed in Admin without a deploy. |
