...
| 4 | Final Writeup persona rollout and complete dashboards | **[PERSONA LENSES PRODUCTION-LIVE; NATURAL STAFF OBSERVATION COMPLETE 2026-09-04 PT.]** Commit `213f6c34` is live in Ready deployment `dpl_HGrbWUNPJMJunVevYLVEmtn7He6a`; the tracked source flag is true. Read-only production-data smoke verified PD, PC, Leadership, overlap, ineligible/unassigned, and superuser projections. **[OWNER-REPORTED]** President Allison Keller and Program Coordinators Duncan Spore and Sarah Hibler proved canonical Word access and acknowledgement; Duncan later found Request `1002788` in History, saw the review matrix, and opened the Word document successfully. Duncan, Allison, and Sarah initially appeared Reviewed and later Updated, consistent with publication-version freshness. No team exists or is required. | **Met:** runtime projection, representative Word access, and signed-in non-superuser History/matrix/Word observation are complete. PC backup transitions, Leadership stage transitions, and any broader supporting-material projection remain separate implementation decisions. Dashboard filters, cycle scoping, and the retired-preview decision are tracked as item 5 below. |
| 5 | Final writeups dashboard completion: cycle scoping, filters, and residuals (Editor Dashboard contract) | **[OWNER-DIRECTED 2026-09-06; 6A PRODUCTION-LIVE AND SMOKED; 6B–6E NOT BUILT; PRIORITIZED ABOVE ITEM 6.]** The owner reviewed the Editor Dashboard contract against the Production-live Final writeups dashboard and decided: keep and build the contract filters (cycle, program/PD, artifact stage, editing/review state) as navigation-only controls; retire inline preview (Word opens outside the Workbench); keep the residuals. Boundary [VERIFIED S491 via source]: the dashboard lists every current Final across all cycles under one hard 100-row cap that fails the whole page (503) when exceeded, renders no cycle or publication version, and has no filter controls or "has edits" hint. Build order and contracts are Slice 6 of `docs/FINAL_WRITEUP_REVIEW_IMPLEMENTATION_PLAN.md`: 6A server-side cycle scoping (dated: before D26 Final writeups exist; **built S491 on branch `claude/final-writeups-cycle-scoping` per `docs/FINAL_WRITEUPS_DASHBOARD_CYCLE_SCOPING_PLAN.md` after three Codex plan passes and the owner's 2026-09-06 decision that lenses focus rather than conceal (PD lens now retains leadership-stage rows); two Codex diff passes, merged `842c9f13`, owner-run signed-in smoke passed 2026-09-06; the PD lens now retains leadership-stage rows with a warn-not-lock notice**), 6B client-side filters persisted in the URL (**owner-shaped 2026-09-06, planning item:** the page opens on "Needs my review" for every role, with "Reviewed by me" and "All writeups" as alternatives and a Program director dropdown; the stage filter is dropped because the dashboard has only two stages and the label is already on each row), 6C current-version context in place of preview, 6D "has edits" hint and 6E other-stage lists as unscheduled product calls. No approval gates, denominators, program-taxonomy grouping, or authorization changes. | **Met when:** a signed-in user opens the dashboard on the writeups that need their review, can switch to reviewed-by-me or all, can narrow by PD, and the view is bookmarkable; the publication version acknowledgements key to is visible per row; two cycles of Finals load without the scope-exceeded error. 6D/6E close by owner decision, not by build. |
