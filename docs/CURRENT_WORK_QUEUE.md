...
| 2 | First deadline-bound writeup slice | **[VERIFIED IN SOURCE/TEST/LIVE CONTROLLED RUN 2026-08-16]** The Workbench Pre Site Visit tab has an interim local-download path: it resolves the exact AI Materials narrative and authoritative Dataverse metadata, executes governed `pre-site-visit.proposal-core.generate`, renders the tracked Word template, and returns the DOCX without a SharePoint or registry write. Dataverse now has sole-current prompt v2 `1d276948-ed99-f111-b8db-70a8a59cded0` on reviewed `claude-sonnet-4-6`. A controlled local invocation against live Request `1002379` completed AI run `5bd65180-ed99-f111-b8db-7ced8d6e2f44`; its four-page DOCX passed structural and rendered-page QA. The earlier v1 run completed at the model boundary but was rejected during render QA for page overflow and remains append-only history. The branch is not deployed, the signed-in Workbench route has not been production-smoked, and no SharePoint artifact, registry row, request pointer, or business field was written. The durable governed-artifact reuse still waits on the remaining library/readback gate above. | Promote the direct-download slice deliberately, then prove the signed-in Workbench download for a dedicated request. The later durable milestone remains a searchable, editable, recoverable Word artifact with complete prompt/run/request lineage and verified human review; the full Editor Dashboard contract remains later reuse. |
