...
| 8 | J27 single-phase transition inventory (register of what retires, persists, changes, or must be built) | **[OWNER-DIRECTED 2026-09-06; PLAN WRITTEN; SWEEP NOT RUN.]** The owner asked to collect every J27-sensitive site in one place and plan the June 2027 build and change work. `docs/J27_SINGLE_PHASE_TRANSITION_INVENTORY_PLAN.md` defines the register schema (category, site, excerpt, cycle dependency, evidence label, owner decision, disposition), a six-slice read-only agent sweep plus an owner-run Dataverse slice, a Codex verification pass, a `J27:` marker convention with a check script, and five owner questions starting with the J27 calendar. Retire rows are candidates, not authorizations. Run the sweep on the owner's go. | **Met when:** `docs/J27_TRANSITION_REGISTER.md` exists with every row labelled and verified, the owner questions are answered or queued, and the marker check script is referenced from `docs/CI_GATES_REFERENCE.md`. |
