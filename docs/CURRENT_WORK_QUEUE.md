...
- **Public/onboarding reviewer-token documentation reconciliation.**
  ...
  **Resolved 2026-09-08 on branch `claude/j27-gate-tighten` (commit `c1f4893d`):**
  `docs/REVIEWER_ENGAGEMENT_SPEC.md:110` was reworded — it no longer contradicts
  the same spec's `:64` and `:79`; it now states the manual "Send reminder"
  action on Invite Reviewers is the sanctioned nudge and re-mints a link the
  same way the (still-held) automated respond-by reminder would.
