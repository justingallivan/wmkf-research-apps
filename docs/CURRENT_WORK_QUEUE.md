...
- Pre-Site durable-generation production proof: commit `abfe5529` reached Ready
  ...
  the server but displayed `Failed to fetch` in the browser. A local follow-up
  now adds read-only status loading/polling and template-v2 Recommendation
  padding; promotion and signed-in smoke remain open.
