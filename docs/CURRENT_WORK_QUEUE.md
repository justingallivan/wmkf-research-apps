...
| 3 | Retain structured individual review DOCX files in SharePoint and backfill D26 | **[WAVES 1–4 COMPLETE; WAVE 5 ACTIVE 2026-09-04 UTC.]** The v4 text-table templates, request-level `Reviews/Review-<request>-<reviewer name>.docx` target, guarded repair service, and exact D26 backfill are Production-proved. Exact manifest `9254df9e5e504c79007391efc85d189e89e8b8b2ff80b8e4f11990baca08f4f8` executed 22 created / zero failed; row-by-row reconciliation matched all expected identities, destinations, governed hashes, and unique SharePoint items. Production now has exact `REVIEW_DOCX_SHAREPOINT_WRITE=on` and `REVIEW_DOCX_SHAREPOINT_CYCLE=D26`; Ready activation-proof deployment `dpl_E6VKW5Wi8zDTfU1ZRhNsbH9yg9oM` returned HTTP 200 for an authenticated enabled sweep. It attempted no filing and made no SharePoint document or Dataverse pointer mutation because the sole candidate was known test Request `1003223`, classified `invalid_snapshot`; bounded logs had no runtime errors. The test row was then removed from the filing population by an ETag-protected, one-field `wmkf_selected=false` correction that preserved its received-review history; the immediate enabled follow-up sweep returned zero candidates and zero attempts. Request `1002874` remains the owner-confirmed Word Online proof at version `4.0`; its obsolete `Reviewer_Uploads/Generated` tree has been owner-deleted, Graph returns 404 for the old path and 200 for the current file, and Dataverse retains the current `Reviews` pointer. | **Met for backfill, activation boundary, test-row disposition, and legacy cleanup:** the next natural automatic filing remains the final end-to-end proof. |
| 4 | Final Writeup persona rollout and complete dashboards | **[PERSONA LENSES PRODUCTION-LIVE 2026-09-03 PT.]** Commit `213f6c34` is live in Ready deployment `dpl_HGrbWUNPJMJunVevYLVEmtn7He6a`; the tracked source flag is true. A same-day read-only preflight re-proved version 2, all 11 current reviewer-role members assigned, zero stale/unassigned references, and role counts of seven Program Directors, three Program Coordinators, and two Leadership assignments (including Beth Pruitt's explicit PD + Leadership overlap). A production-data service smoke from the deployed source verified PD, PC, Leadership, overlap, ineligible/unassigned, and superuser projections: PC and superuser received the complete matrix, ineligible received zero rows plus the bounded warning, and all persona resolutions were enabled without warnings for mapped users. **[OWNER-REPORTED 2026-09-03]** President Allison Keller and Program Coordinators Duncan Spore and Sarah Hibler had already proved canonical Word access and acknowledgement; the owner saw Allison's acknowledgement on the dashboard. No team exists or is required. | **Runtime gate met.** The isolated automated browser reached the expected sign-in boundary, so record one signed-in non-superuser dashboard observation on the next natural staff visit. PC backup transitions, Leadership stage transitions, and any broader supporting-material projection remain separate implementation decisions; this rollout changes read-only dashboard lenses only. |
