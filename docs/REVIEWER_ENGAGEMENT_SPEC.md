...
**Operational hold:** `/api/cron/reviewer-reminders` remains implemented and cron-secret protected, but it is not registered in the production Vercel cron schedule. `scripts/check-reviewer-reminder-hold.js` fails the Vercel prebuild and CI if that exact route is re-registered or if the cron registry becomes unreadable. The production manual-send freeze is **procedural only**: the staff reminder endpoint, link-bearing email sends, token regeneration, and review-due resend remain callable in code. Staff must not invoke them during the hold.

The hold covers every action that can issue or replace reviewer token authority: `POST /api/review-manager/send-review-reminder` (`respond` or `reviewdue`), link-bearing templates through `POST /api/review-manager/send-emails`, `POST /api/review-manager/regenerate-token`, and review-due extension/resend. `regenerate-token` also deletes the matching Postgres `review_drafts` row, so it is not a harmless link refresh.
...
- The manual review-due reminder remains implemented and shares the review-due marker; it is procedurally frozen with the other token-issuing manual actions during the incident hold. `[verified pages/api/review-manager/send-review-reminder.js]`
...
- **"Accepted but never released" window (Codex P1):** an accepted-pre-materials reviewer holds only the **invite** token (review-due cap). If the PD never sends materials, that link dies at review-due with no self-serve path. The existing **regenerate-token** staff endpoint can reissue authority but also deletes the reviewer's saved Postgres draft; it is suspended during the incident hold and must not be treated as a routine recovery action.
- **Cap is "going forward" only (Codex P2):** `mintToken` fixes `exp` at mint time; changing `reviewDueDate` in the config later does NOT re-cap already-minted tokens. Re-invite, materials send, and regenerate-token all re-mint, but each is suspended as a production recovery tactic until the token policy and draft-preservation contract are settled.
...
- **Incident risk — latest-link-wins:** every dispatched link-bearing reminder/materials email supersedes prior links and can invalidate a reviewer's active session. This is the reason the automatic schedule and manual token-issuing actions are held; the follow-up template's "use the link in this email" copy does not make that behavior safe.
...
- **Manual freeze is not enforced:** the reminder, link-bearing send, regenerate, and due-date resend surfaces remain callable. A staff process lapse can still rotate authority; a code/UI gate is an explicit unresolved owner decision.
