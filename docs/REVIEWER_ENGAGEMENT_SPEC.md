...
**Remediated runtime contract (pending deployment verification):** review-due reminders and ordinary follow-up/thank-you templates are link-free and cannot mint or carry a reviewer URL. Respond-by reminders may still mint the short pre-acceptance link; invitation and materials delivery remain the only ordinary link-bearing templates. `POST /api/review-manager/regenerate-token` is the explicit replacement-link recovery action and now preserves the matching Postgres `review_drafts` row. Review-due extension/resend does not rotate token authority; an extension beyond the current signed expiry fails before the date is saved and requires explicit recovery.

**Reactivation prerequisites:** do not restore the schedule until all of the following are complete and verified together: (1) reminder sends no longer invalidate an active reviewer link/session; (2) per-request enabled and lead-day settings are visible and editable in the staff UI; (3) new-request defaults fail closed; (4) an owner-approved value is applied to the existing armed rows before the first run; (5) the incident hold guard is deliberately retired in the same reviewed change that restores the schedule; and (6) the restored sweep first runs with `?dryRun=1` and its `maintenance_runs` evidence is reviewed before a live send. Whether to add a code/UI gate for the manual-send freeze is a separate owner decision; the current prose freeze must not be mistaken for enforcement.
...
- **Manual freeze is not enforced:** the surfaces remain callable while the fix awaits deployment verification. Review-due follow-up and due-date resend are now non-rotating in source; respond reminders, invitation/materials delivery, and explicit regeneration still replace token authority by design.
