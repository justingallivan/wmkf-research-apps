...
- The manual review-due reminder remains implemented and shares the review-due marker. Both manual and scheduled review-due paths claim the marker with `If-Match` before sending and preserve the existing review token. `[verified lib/services/reviewer-reminder-sweep.js::sendOneReminder]`
- **Implemented (Phase 3):** fire-once via the existing `wmkf_remindersentat`; the email instructs reviewers to use the secure link in their original materials email or contact the Foundation for an explicit replacement.
