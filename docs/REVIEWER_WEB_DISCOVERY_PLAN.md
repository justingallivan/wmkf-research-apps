...
**Relationship to prior work:** distinct from the *identity-disambiguation* Perplexity use spec'd in `docs/REVIEWER_IDENTITY_RESOLVER_PHASE2_DESIGN.md` §7 (a deferred backstop). The earlier strategy's adopted principle—Perplexity may orchestrate discovery and evidence gathering but must not be the final identity authority—is retained in that tracked design; the redundant ignored strategy note was disposed under `docs/audits/local-operational-data-retention-audit-2026-07-27.md`.
