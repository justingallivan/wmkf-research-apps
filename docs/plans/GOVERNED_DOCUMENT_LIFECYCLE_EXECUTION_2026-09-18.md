...
The isolated process used `NEXTAUTH_URL=http://localhost:3000`; this is suitable only for a local process actually listening on port 3000 and cannot produce a usable external rehearsal link. A supported target deployment must provide its public `NEXTAUTH_URL` (production uses the documented branded origin); no email was sent. The backend and proxy were stopped cleanly with SIGINT (exit 130); Final and leadership were intentionally deferred to preserve the reopen order.
