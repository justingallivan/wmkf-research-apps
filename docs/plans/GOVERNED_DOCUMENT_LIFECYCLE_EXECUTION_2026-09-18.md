...
Root's independent sanitized readback confirms 28 Request Document rows, exactly one new Ready row, only the prior IA lifecycle/modified fields changed among 27 prior rows, current PSV `205da1cd-b7b3-f111-aaac-6045bd04539e` unchanged in Review, Final null, and all 14 distribution attempts unchanged. The provider run details, end reason and retry reuse remain handoff-attributed rather than independently rerun. The decomposed candidate branch remains prompt version 1 and is not production evidence; its generation path has not been rerun against PR #314. IA-4 remains historical for this current row: `7d00fffd` has only its v1 history; the prior restore and Board snapshot `3732f140` belong to superseded IA `a6876ad6`, and no new restore or Board snapshot test ran.

The semantic integration point is the still-facade `generateInitialAssessment` in `lib/services/initial-assessment/artifact-service.js`: add the server-owned `getExecutorBudget('initial-assessment.generate')` lookup immediately before `executePrompt`, pass its token/timeout overrides, and preserve the prompt-version mismatch failure code. The companion production changes are `shared/config/executorBudgets.js`, `lib/services/execute-prompt.js`, `lib/services/llm-client.js`, and `shared/config/requestDocument.js` (production contract v2); the IA service test must mock `executor-budget-service` and update hardcoded prompt-version fixtures. No runtime cherry-pick or candidate change was made here.
