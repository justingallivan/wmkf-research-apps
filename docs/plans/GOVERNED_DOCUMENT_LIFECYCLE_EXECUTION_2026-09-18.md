...
service reconciliation are recorded in the smoke runbook; S3–S4 were unrun at that historical point; later results are recorded below.
...
[VERIFIED via `/tmp/wmkf-reopen-smoke-review.json`, `/tmp/wmkf-reopen-smoke-reconciliation.json`, `/tmp/wmkf-reopen-smoke-graph.json` and sanitized process logs] With the guarded schema and explicit-actor readiness checks enabled for this process, the authorized ZZTEST-03 reopen returned HTTP 200. Source artifact `4bcceb18-5cad-f111-aaab-7ced8d3c3a59` was superseded while preserving its original SharePoint item and milestone. Successor `205da1cd-b7b3-f111-aaac-6045bd04539e` used a distinct SharePoint item on the same drive, preserved the source version/hash, and was returned to Review by Start Site Visit. The successor milestone version `1.0`, governed hash, created time `2026-09-18T23:25:52Z`, and session-derived actor `29b0de0d-4ff7-ee11-a1fd-000d3a3621c7` were recorded.

Independent Graph verification read both items twice, downloaded both in memory, and found stable metadata, equal governed hashes `gdc1:0a4Ub05Eq7Oi7fOv55zDUIdkYLRTJXWZW26ZJnBCfQ4`, equal 38,161-byte raw packages, and distinct item IDs; no persistence call was made by the probe. The request current Pre-Site pointer changed to the successor and the source lifecycle/modified time changed; the 25 other pre-existing Request Document rows and all 14 distribution attempts were unchanged, with one successor row added (27 rows total); Final remained null. No AI regeneration ran. The exact reopen retry, stale-pointer and fault matrix remains unrun, and Final/leadership remain deferred/unrun. Backend and proxy stopped with SIGINT (exit 130); process-only readiness used `GUARDED_REOPEN_SCHEMA_READY=on`, `REQUEST_DOCUMENT_EXPLICIT_ACTOR_SCHEMA_READY=on`, and the interlock-on dated ACK. The exact schema probes passed at `/tmp/wmkf-guarded-reopen-prod-preflight.log`, `/tmp/wmkf-site-visit-logistics-prod-preflight.log`, and `/tmp/wmkf-request-document-explicit-actor-prod-preflight.log`; no env-file, migration, schema, code or deployment change was made. Future correction cycles require the guarded-reopen readiness setting to remain verified.

## Production IA budget-fix handoff — 2026-09-18

[CLAUDE-HANDOFF PRODUCTION EVIDENCE, independently confirmed for pointer/reconciliation fields via `/tmp/wmkf-claude-handoff-verified.json` and `/tmp/wmkf-claude-handoff-reconciliation.json`] PR #314 merge `0b240f0a` deployed the thinking-budget fix. Claude reports the signed-in production rerun on request `1003222 / ZZTEST-03` passed at 23:38Z: run `7770d508-bab3-f111-aaac-7ced8d3c3a59` ended `end_turn`, produced 1,065 output tokens with `thinkingTokens=0` and `maxTokens=12000`, and created Ready IA row `7d00fffd-b9b3-f111-aaac-000d3a361c1f`, superseding prior Ready row `a6876ad6-3b94-f111-8075-70a8a59cded0`. Claude also reports the exact retry reused that row with no new run.

Root's independent sanitized readback confirms 28 Request Document rows, exactly one new Ready row, only the prior IA lifecycle/modified fields changed among 27 prior rows, current PSV `205da1cd-b7b3-f111-aaac-6045bd04539e` unchanged in Review, Final null, and all 14 distribution attempts unchanged. The provider run details, end reason and retry reuse remain handoff-attributed rather than independently rerun. The isolated candidate now integrates prompt version 2 and is not production evidence; its generation path has passed local validation but has not been live-rerun. IA-4 remains historical for this current row: `7d00fffd` has only its v1 history; the prior restore and Board snapshot `3732f140` belong to superseded IA `a6876ad6`, and no new restore or Board snapshot test ran.

The semantic integration is now present in the still-facade `generateInitialAssessment` in `lib/services/initial-assessment/artifact-service.js`: it reads the server-owned `getExecutorBudget('initial-assessment.generate')` immediately before `executePrompt`, passes token/timeout overrides, and preserves the prompt-version mismatch failure code. The companion integrated files are `shared/config/executorBudgets.js`, `lib/services/execute-prompt.js`, `lib/services/llm-client.js`, `shared/config/requestDocument.js` (production contract v2), and `shared/components/admin/PromptTemplatesSection.js`; the IA service tests mock `executor-budget-service` and use the contract prompt version. The reviewed integration joins parent `c054dc52` with incoming `7a335e27`; focused validation passed 9 suites / 250 tests, full Jest passed 958 suites / 14,195 tests, types/lint/import census/build and applicable gate pairs passed. Logs are `/tmp/wmkf-pr314-focused.log`, `/tmp/wmkf-pr314-full-test.log`, `/tmp/wmkf-pr314-build.log`, `/tmp/wmkf-pr314-types.log`, `/tmp/wmkf-pr314-imports.log` and `/tmp/wmkf-pr314-*.log`; no live AI calls, data mutations, servers, deployment or push occurred. Sol produced no verdict after two model-capacity failures. The owner supplied Claude's replacement **READY** integration review: Claude disclosed authorship of PR #314, so this is an integration self-review rather than an independent review of the original fix. Claude reports rerunning the nine focused suites (250 tests), inspecting the full validation logs, and confirming unchanged working-tree fingerprints. No introduced defects were found. Direct streaming-census test coverage and the reported Opus 4.8 registry-label limitation remain pre-existing limitations; neither is represented as fixed here. Root final review accepted the preserved retry-before-provider ordering, failure/run linkage, claim checks, and untouched restore/public-link fixes. No runtime changes followed validation; only this review receipt changed before the local merge commit.
...
The first send failed closed with 503 `distribution_impersonation_required` before ledger claim. After the approved process-only `DYNAMICS_IMPERSONATION_ENABLED=true` setting was present, the same prepared preview was retried once and returned HTTP 200; the UI showed `Sent for delivery`. Ledger readback shows `sent`, attempt count 1, Dynamics email `f78b4b7c-b6b3-f111-aaac-7ced8d3c3a59`, initial status 6 and later status 3 with `senton` (`/tmp/wmkf-email-smoke-delivery-check.json`), one correlation match, approved sole recipient, no Cc/Bcc, and sender `jgallivan@wmkeck.org`. This proves transport acceptance. The user confirmed inbox receipt on 2026-09-18; this does not infer that the user clicked the public briefing link.

Request and 26 Request Document rows were unchanged; 13 prior distribution attempts remained unchanged and the total became 14. The public briefing link rendered correctly over HTTPS. Process-only briefing readiness, public-base override, and impersonation settings were used; local `NEXTAUTH_URL` remained unchanged. Migration 038 was already verified and no migration ran. Backend and proxy stopped with SIGINT (exit 130). The later PSV reopen and return-to-Review happy path passed; the exact retry/stale/fault matrix remains unrun. No AI regeneration ran, and Final and leadership were deferred at that run; the later S4 result is recorded below.


## S4 Final and leadership continuation — 2026-09-18 local / 2026-09-19 UTC

[VERIFIED via /tmp/wmkf-final-smoke receipts, sanitized HTTP captures and browser evidence] The authorized S4 rehearsal ran local candidate `aff3049d` and used PSV successor 205da1cd-b7b3-f111-aaac-6045bd04539e in Ready/Review. Final row 473c9160-c0b3-f111-aaac-000d3a361c1f was created on the same SharePoint drive/item; group review was recorded at 2026-09-19T00:23:51Z and leadership at 2026-09-19T00:24:37Z with session actor 29b0de0d-4ff7-ee11-a1fd-000d3a3621c7. Same-item, version, eTag and governed-hash checks passed; four expected Final metadata fields changed during leadership.

The identical start and leadership payloads each returned HTTP 200 with reused:true and equal artifact DTOs. Independent reconciliation showed 28 to 29 Request Document rows, only the prior source lifecycle/modified fields plus the four Final leadership fields changed, and all 14 distribution attempts unchanged. No AI, email, or SharePoint upload/copy calls were observed in the bounded dependency capture; no schema change occurred. Fault injection was not exercised. Each of the three Dataverse writes first rejected impersonation and then succeeded through the supported service-principal fallback; native CreatedBy impersonation is not claimed. The browser showed leadership after reload, and five signed-out route checks returned 307. The temporary proxy passed eight checks and replayed each successful 200 once in memory without persisting authentication headers. Wrong-app access and review-bundle rebuild remain mock-only; other Final fault cases remain unrun.
