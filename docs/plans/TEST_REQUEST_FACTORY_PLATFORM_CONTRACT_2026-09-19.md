...
[VERIFIED via source and CLI validation, 2026-09-23] The `2099` values above were deliberately artificial rehearsal inputs. The sandbox rehearsal operator now requires `--source-request-number`; it resolves exactly one Grant Request and defaults fiscal year and meeting date from that source. Each missing source value requires its own explicit override. This remains a local sandbox-only script and is not a deployed Admin Create capability.

[SOURCE-BUILT on `codex/test-request-preview-integration`, not executed against Dataverse] Manifest v3 stores the preallocated destination identities, minimal source identity/revision provenance, and the digest of the compiled create body in a private exclusive file. Execute accepts only v3, re-reads the exact source GUID and copied purpose/amount before the sole Request POST (including immediately after an explicitly requested GoVerify bypass), and records exact destination IDs on failure for read-only inspection. Receipt milestones use same-directory atomic replacement with file and directory fsync; inspect can derive the exact expected folder if location creation is absent. Bypass calls and the Request POST are bounded. SIGINT/SIGTERM cancels client-side fetch work and awaits the client promise, but does not prove that the remote server canceled or rolled back an in-flight PATCH or POST. If GoVerify deactivation was attempted without a verified inactive readback, the receipt records `restoreVerified:false`, the exact workflow ID, and a manual-recheck reason; the command stops without treating an active readback as proof or continuing post-create steps. A Request POST timeout can leave an ambiguous create; use `--inspect` to reconcile the preallocated GUID and never retry that manifest. Verified deactivation follows the existing bounded restoration path. A hard process kill or power loss remains outside signal handling and requires manual inspection/reconciliation. Final verification checks copied purpose/amount, cycle fields, marker/run, applicant, app-suite creator/owner, exact empty location/folder, and zero payments/emails. No new Request was created for this implementation; historical v1/v2 manifests remain inspectable but cannot execute.
