...
| **1002874 (all fixes + rendering)** | **Opus: completed, 85 s, 14.5k in / 5.6k out, $0.22** | **completed, 32 s, 8.8k / 1.4k, $0.06** | **chair Opus completed, 110 s, 19.8k / 8.3k, $0.31; editions saved first time. Panel cost $0.59, ~4 min wall clock.** |

Findings (2026-09-13, in order fixed):
- **Complete panels: 1002852 $0.61, 1002874 $0.59** (seats $0.21 + $0.07, chair $0.33), ~4 minutes wall clock including a one-minute cron wait. D6 posture: two data points so far, not a typical figure; two more requests queued for the same subset.
...
- **Report rendering**: the first PDF printed the rating matrix as raw JSON and disagreements as `[object Object]` (chair fields are structured: matrix keyed by question, disagreements as topic/positions/significance). Fixed in PR #288 (labels from the projected question set, seat display labels for positions); 1002874 rendered with it.
