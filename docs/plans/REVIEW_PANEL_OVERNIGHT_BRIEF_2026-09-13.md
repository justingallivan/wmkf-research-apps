...
> **End of day (2026-09-13): all ten PRs #280–#290 merged; the panel is live in production smoke mode; four D26 requests completed at ~$0.60 each.** Snapshot log, not ship state. Branch: `feature/review-panel-foundation` (worktree
...
| **1002874 (all fixes; OLD report template, see note)** | **Opus: completed, 85 s, 14.5k in / 5.6k out, $0.22** | **completed, 32 s, 8.8k / 1.4k, $0.06** | **chair Opus completed, 110 s, 19.8k / 8.3k, $0.31; editions saved first time. Panel cost $0.59, ~4 min wall clock. Its PDF/DOCX carry the old template: the cron tick that processed it started before the PR #288 build was promoted (Fable misread the alias as already updated).** |
| **1002903** | **Opus: completed, 91 s, 16.7k in / 5.9k out, $0.23** | **completed, 31 s, 10.8k / 1.3k, $0.07** | **chair Opus completed, 121 s, 22.0k / 9.5k, $0.35; editions saved first time with the new report template. Panel cost $0.65.** |
| **1002912** | **Opus: completed, 78 s, 15.5k in / 5.4k out, $0.22** | **completed, 27 s, 10.1k / 1.4k, $0.07** | **chair Opus completed, 98 s, 21.3k / 7.5k, $0.30; new template. Panel cost $0.59.** |

Findings (2026-09-13, in order fixed):
- **Smoke subset complete: 1002852 $0.61, 1002874 $0.59, 1002903 $0.65, 1002912 $0.59** (seats $0.21 + $0.07, chair $0.33), ~4 minutes wall clock including a one-minute cron wait. D6 posture: four data points, $0.59–$0.65 per request (~$0.60) at ~4 minutes wall clock; the Opus chair is ~half the cost. Seat-failure runs before the fixes added ~$1.50 of spend. Enough to decide whether the page shows a typical figure.
...
- **Retry-state UX**: while a retry is queued the row showed both `failed` and `Retry queued` with stale copy and no timeline events; fixed in PR #287; a worker-stamped retry marker and edition `savedAt` remain follow-ups.
- **Report rendering**: the first PDF printed the rating matrix as raw JSON and disagreements as `[object Object]` (chair fields are structured: matrix keyed by question, disagreements as topic/positions/significance). Fixed in PR #288 (labels from the projected question set, seat display labels for positions); 1002903 rendered with it. 1002852 and 1002874 keep old-template editions until a re-render action exists.
- **"Re-render report" action built (PR #289, owner reversed the deferral once two of four reports carried the old template).** Opus caught that the first version nulled the saved editions at request time; fixed to keep the previous pair live until a new pair lands, with history and orphan refs kept. First production click failed: the page sent the LATEST run's id while the entry belonged to an older run, and eligibility keyed on `winners_json.chair`, which the normal completion path never records (only the retry path does). Fixed in PR #290 (server resolves the run from the entry ids, eligibility by completed chair attempt, worker records the chair winner on normal completion). Verified on 1002912 (re-rendered once, `savedAt` stamped). **1002852 and 1002874 still carry old-template editions at session end: one click each on "Re-render report" refreshes them at no cost.**
