...
| `lib/bill/honorarium-onboard-orchestrator.js` | Honorarium/request creation and reminder defaults | Reject marked source requests from ordinary honorarium flow; no payment fixture in V1. Covered by the Stage 1b-ii acceptance-job gate: the orchestrator is reached only from that job |
