...
| J27-023 | `scripts/audit-grant-cycle-shortcode-domain.js` | C (re-decided 2026-09-08, matrix review) | `wmkf_grantcyclecode distinct values` — kept in `site`, not the row's fact | Matrix review flagged the original A-case reasoning below as wrong: this file is not "the same subject the pattern-based files group under" in a way that supports SV — a full-file read found no cycle-derivation or validation pattern and no D26 assumption anywhere (generic W3 shortcode-collision preflight over the Postgres/Dataverse union). Re-decided case C per plan §3: citation kept in `site` (still bound, verbatim, at :7) with a dated drift note on the register row and `Ev` downgraded SV→AS for the whole row, since this citation was not independently pattern-verified like the other four. |
