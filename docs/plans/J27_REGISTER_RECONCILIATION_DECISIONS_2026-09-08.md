...
| J27-011 | `tests/unit/intake-draft-endpoint.test.js` | (excluded, does not carry the fact) | none — left out of `site` | 2026-09-08 matrix review: uses the placeholder `'phase-ii-2026-06'` (lines 46, 67, 284, 355), not the D26 literal `'phase-ii-research-2026-06'`. It pins a formKey-is-required invariant that survives J27 unchanged, not this row's D26-form-key fact — moved from case A to excluded. |
| J27-011 | `tests/unit/intake-draft-service.test.js` | (excluded, does not carry the fact) | none — left out of `site` | 2026-09-08 matrix review: uses the placeholder `'phase-ii-2026-06'` (line 28), not the D26 literal `'phase-ii-research-2026-06'`. Same reasoning as intake-draft-endpoint.test.js — moved from case A to excluded. |
...
| J27-023 | `scripts/audit-grant-cycle-shortcode-domain.js` | C (re-decided 2026-09-08, matrix review; unbound 2026-09-08, Codex adversarial review of `65eb4bfe`) | none — kept in `site` at :7, not bound to any fragment | Matrix review flagged the original A-case reasoning below as wrong: this file is not "the same subject the pattern-based files group under" in a way that supports SV — a full-file read found no cycle-derivation or validation pattern and no D26 assumption anywhere (generic W3 shortcode-collision preflight over the Postgres/Dataverse union). Re-decided case C per plan §3, initially by keeping the citation bound to its verbatim `wmkf_grantcyclecode distinct values` fragment — but a Codex adversarial review correctly flagged that as green-on-a-non-fact-match: the fragment being verbatim doesn't make it this row's fact. The binding was removed (finding 2, `docs/J27_TRANSITION_REGISTER.md` §10); the citation stays in `site` with the dated drift note and `Ev` downgraded SV→AS, and the file is left deliberately unbound so the row is honestly STALE (`unbound: scripts/audit-grant-cycle-shortcode-domain.js`) pending an owner ruling on whether to drop the citation or split it into its own row. Owner ruling 2026-09-08 (option 1): citation dropped from `site`; not a J27-sensitive site. |
...
### J27-032 (added 2026-09-08, matrix review — fell between slices A and B on first pass)

`shared/components/reviewers/` holds exactly 30 files. Grepping `Phase [0-9]` across all 30
found exactly 9 that carry a build-phase comment (reviewer-verified count); the register's
"21 of 30 carry no Phase N comment" note is therefore correct (30 − 9 = 21). The 9 bound
components plus `docs/REVIEWER_ENGAGEMENT_SPEC.md` make the register row's 10 site citations.

| register id | file | case | fragment or note | reason |
|---|---|---|---|---|
| J27-032 | `docs/REVIEWER_ENGAGEMENT_SPEC.md` | A | `Phases 1, 2, and 4 remain live. Phase 3 reminder code remains implemented, but its Vercel schedule was **paused on 2026-09-01**` | Verbatim at L21 (the spec's own `**Status:**` line); the row's canonical citation for "these are build phases, not grant phases." |
| J27-032 | `shared/components/reviewers/CampaignConfigModal.js` | A | `Phase 1 exposes "days to respond"` | Verbatim at L7. |
| J27-032 | `shared/components/reviewers/InviteEmailModal.js` | A | `Reviewer-engagement Phase 1: respond-by is now a "days to respond" OFFSET` | Verbatim at L49. |
| J27-032 | `shared/components/reviewers/ReviewerActivityDrawer.js` | A | `Phase 1 of reviewer activity history (owner decisions 2026-08-12` | Verbatim at L4. |
| J27-032 | `shared/components/reviewers/reviewer-activity-history.js` | A | `Reviewer activity history — Phase 1, derived from current record state.` | Verbatim at L2. |
| J27-032 | `shared/components/reviewers/reviewer-modes.js` | A | `docs/REQUEST_WORKBENCH_BUILD_PLAN.md § Phase 2.` | Verbatim at L8. |
| J27-032 | `shared/components/reviewers/ReviewerInvitePanel.js` | A | `Reviewer-engagement Phase 4: a set responseType` | Verbatim at L58. |
| J27-032 | `shared/components/reviewers/ReviewerManagePanel.js` | A | `ProposalDetailTab (Phase 2 of the` | Verbatim at L4. |
| J27-032 | `shared/components/reviewers/ReviewerFindPanel.js` | A | `(tier-3), Phase 3.` | Verbatim at L3. |
| J27-032 | `shared/components/reviewers/ReviewersTab.js` | A | `ReviewerFindPanel (Phase 3) — applicant-reviewer ingestion` | Verbatim at L5. |
| J27-032 | `shared/components/reviewers/CandidateEditModal.js`, `ContactLeads.js`, `email-template-store.js`, `EmailTemplatesModal.js`, `prompt-override-store.js`, `ReleaseEmailModal.js`, `ReleaseMaterialsModal.js`, `remove-reviewer-confirm.js`, `RemoveEntirelyModal.js`, `render-preview-failure.js`, `RespondReminderModal.js`, `reviewer-draft-keys.js`, `reviewer-search-logic.js`, `ReviewerCloseoutModal.js`, `ReviewerDueDateEditor.js`, `ReviewerPromptOverridePanel.js`, `ReviewerSearchSection.js`, `ReviewReminderAction.js`, `sse.js`, `SubTabBadges.js`, `TokenActionsMenu.js` | excluded — no Phase N comment | none — left out of `site` | 21 files, grep-confirmed clean of any `Phase [0-9]` comment; none carry the row's "build phase, not grant phase" fact. |
...
| J27-028 | `dataverse-wmkf-requestdocument.md` (bare Atlas token, matches `site`) | A | `The entity contains 12 rows: three Initial Assessments, eight Pre Site Visits, and one Final Writeup.` | Atlas restates the 12-Production-rows fact the row's note claims, at the `#Status` VERIFIED-2026-08-31 census line (:40-41, not :27-29 — 2026-09-08 matrix review corrected the line cite; :27 is only the section heading). |
...
| J27-036 | `lib/services/reviewer-finder/my-proposals-service.js` | A | `Always exclude concept and Phase I-declined — they never need outside` | Rationale comment verbatim only at L120 (not L23 — 2026-09-08 matrix review corrected the line cite); still states the exclusion this row's excerpt names. |
...
| J27-038 | `lib/services/reviewer-finder/load-proposal-service.js` | A (rebound 2026-09-08, matrix review) | `file.name === 'ProjectDescription.pdf'` | `isCurrentCycleFallback` still hard-codes the `Phase I` folder match at L84-86, but the original fragment (`/(^|\/)Phase I$/i.test(String(file.folder || ''))`) has three bare `\|` inside the excerpt cell, which breaks Markdown table parsing (the gate tolerates it only because `splitRow` is backtick-aware) — never write a literal `|` inside a register cell. Rebound to the pipe-free verbatim sibling line (L85) inside the same function, which pins the same fact (the fallback still requires the exact legacy `Phase I` filename). |
...
| J27-039 | `shared/components/workbench/ProposalTab.js` | C (line drift, fact intact) | `Phase I documents` (L443) / `Phase II documents` (L459) | Two separate section headings, still present verbatim, but at L443 and L459 respectively, not the cited 336/352 — 2026-09-08 matrix review split the two facts to their own lines; no fact changed. |
...
| register id | file | case | fragment or note | reason |
|---|---|---|---|---|
| J27-041 | `shared/config/prompts/proposal-summarizer.js` | A | `Used for creating Phase II writeup drafts from research proposals` | Header docblock states the Phase II input fact directly (line 3, not line 6 — 2026-09-08 matrix review corrected the line cite). |
...
| J27-056 | `dataverse-wmkf-requestdocument.md` (Atlas) | A | `Phase I display documents … do not satisfy the producer contract.` | Matches the row's own bag fragment, present only under `#Initial Assessment pilot contract` (L303-304); `#Status` (L27-245) does not contain this phrase — 2026-09-08 matrix review corrected the cited heading. |
...
| J27-062 | `.claude-memory/project-grant-phasing-evolution.md` | A | bound (via arrow): `Plan an upstream per-PD triage/cycle dashboard for J27`; unbound bag: `up to ~300 full proposals, most never sent for outside review` | **Reason corrected 2026-09-08 (matrix review):** the original entry said "both original bag-fragment halves match this file verbatim" — wrong, because the two spans are joined by an ellipsis (`…`), not an arrow, so only the first span is actually bound to this file; the second is a plain shared bag fragment (available to any unbound file in the row, not specifically tied to this one). Both halves ARE verbatim in this file, but only the first is load-bearing for THIS file's binding — the second's presence elsewhere is coincidental, not evidence. |
...
| J27-062 | `pages/workbench/[requestId].js` | A (rebound 2026-09-08, matrix review) | `LIVE tabs: Reviewers` | **Original binding (`{ key: 'awardee', label: 'Awardee' }`, the TABS array's last entry) does not detect the row's fact** — an array literal has no "this is the complete list" assertion a matcher can verify; a future added tab would leave that exact string unchanged and the row would stay silently "verified." Rebound to the file's own docblock (L1-4), which explicitly enumerates "LIVE tabs: Reviewers ... Proposal ... Reviews ... Overview ... Status, Awardee ... Initial Assessment ... Staff Deliberations ... Final Writeup" with no triage tab named — this IS a claim about completeness, and is the fact-bearing evidence for "no triage tab today." |
