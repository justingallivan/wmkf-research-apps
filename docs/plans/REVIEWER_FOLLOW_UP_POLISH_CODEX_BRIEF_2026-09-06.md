...
- To-do: add a durable per-reminder audit trail (one timestamped event per reminder) and extend the reviewer activity API/DTO so the activity drawer can show each reminder separately; the current record exposes only `reminderSentAt` plus cumulative `reminderCount`.
