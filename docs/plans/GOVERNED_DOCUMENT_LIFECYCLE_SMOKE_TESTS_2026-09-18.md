...
below. S3–S4 were NOT RUN at that historical point; later results are recorded below. No production deployment was changed.
...
The recovery process and wrapper exited 0. Limits: this was not a global database audit, and the browser restore was not rerun after the service fix. S2 therefore remains incomplete; S3–S4 were NOT RUN at that historical point; later results are recorded below.
...
**PASS — approved happy-path prepare/send; user confirmed inbox receipt.** [VERIFIED via `/tmp/wmkf-email-smoke-send-blocked.json`, `/tmp/wmkf-email-smoke-sent.json`, `/tmp/wmkf-email-smoke-before.json`, `/tmp/wmkf-email-smoke-after.json`, browser UI and sanitized process logs] On code `cbad8a8d`, the same approved ZZTEST-03 request used operation `871e75b4-e7b1-4fdf-92a7-62c83abc7c3b`, source artifact `c5d81d74-b7b2-f111-aaac-002248086b29`, sole To recipient `justingallivan@me.com`, empty Cc/Bcc, and calendar disabled. Prepare succeeded and reused existing briefing link `63431ae5-7fad-4eab-9e5b-5e6f89e64520`; no new document was created.

The first send attempt failed closed with 503 `distribution_impersonation_required` before claiming the ledger row. The same prepared preview was retried once after the already-approved process-only `DYNAMICS_IMPERSONATION_ENABLED=true` setting was present. Send then returned HTTP 200 and the UI showed **Sent for delivery**. Independent ledger readback shows state `sent`, `attempt_count: 1`, Dynamics email `f78b4b7c-b6b3-f111-aaac-7ced8d3c3a59`, initial status code 6 and later status code 3 with `senton` (`/tmp/wmkf-email-smoke-delivery-check.json`), one correlation match, the approved sole recipient, no Cc/Bcc, and sender `jgallivan@wmkeck.org`. The result proves Dynamics transport acceptance and the sent ledger contract. The user confirmed inbox receipt on 2026-09-18; this does not infer that the user clicked the public briefing link.

Before/after reconciliation shows the request and 26 Request Document rows unchanged, 13 prior distribution attempts unchanged, and 14 attempts total afterward. The existing briefing link rendered correctly in the public browser page; the link used HTTPS and was not localhost. Process-only configuration used the briefing readiness flag, `DELIBERATION_BRIEFING_PUBLIC_BASE_URL`, and impersonation setting; local `NEXTAUTH_URL` remained unchanged. Migration 038 was already verified and no migration was run. Backend and proxy were stopped cleanly with SIGINT (exit 130). The later PSV reopen and return-to-Review happy path passed; the exact retry/stale/fault matrix remains unrun. No AI regeneration ran, and Final and leadership were deferred at that run; the later S4 result is recorded below.

## Controlled Pre-Site correction continuation — 2026-09-18

Candidate runtime: `6707ab51` on `codex/document-lifecycle-decomposition`.

**PASS — guarded reopen and return-to-Review happy path; matrix incomplete.** [VERIFIED via `/tmp/wmkf-reopen-smoke-review.json`, `/tmp/wmkf-reopen-smoke-reconciliation.json`, `/tmp/wmkf-reopen-smoke-graph.json` and sanitized process logs] After the documented Dataverse schema and explicit-actor readiness checks were enabled for the process, the authorized ZZTEST-03 reopen returned HTTP 200. Source artifact `4bcceb18-5cad-f111-aaab-7ced8d3c3a59` was superseded while its original SharePoint item and milestone were preserved. Successor `205da1cd-b7b3-f111-aaac-6045bd04539e` used a distinct item on the same drive, preserved source version/hash, and Start Site Visit returned it to Review with milestone version `1.0`, governed hash, created time `2026-09-18T23:25:52Z`, and session-derived actor `29b0de0d-4ff7-ee11-a1fd-000d3a3621c7`.

The independent Graph probe read both items twice, downloaded both in memory, and found stable metadata, equal governed hashes `gdc1:0a4Ub05Eq7Oi7fOv55zDUIdkYLRTJXWZW26ZJnBCfQ4`, equal 38,161-byte raw packages, and distinct item IDs; it made no persistence calls. The request current Pre-Site pointer changed to the successor and the source lifecycle/modified time changed; the 25 other pre-existing Request Document rows and all 14 distribution attempts were unchanged, with one successor row added (27 rows total); Final remained null. No AI regeneration ran. The exact retry, stale-pointer and fault matrix remains unrun; Final and leadership were deferred at that run; the later S4 result is recorded below. Backend and proxy stopped with SIGINT (exit 130); process-only readiness used `GUARDED_REOPEN_SCHEMA_READY=on`, `REQUEST_DOCUMENT_EXPLICIT_ACTOR_SCHEMA_READY=on`, and the interlock-on dated ACK. The exact schema probes passed at `/tmp/wmkf-guarded-reopen-prod-preflight.log`, `/tmp/wmkf-site-visit-logistics-prod-preflight.log`, and `/tmp/wmkf-request-document-explicit-actor-prod-preflight.log`; no env-file, migration, schema, code or deployment change was made. Future correction cycles require the guarded-reopen readiness setting to remain verified.

## Production IA budget-fix handoff — 2026-09-18
...
|---|---|---|---|---|
| IA-1 generation | `pages/api/workbench/initial-assessment.js` POST; `shared/components/workbench/InitialAssessmentTab.js` | Valid request returns Ready artifact, stable SharePoint identity, registry lineage, and UI Word/SharePoint action. | `MOCKED SUITES PASS` | `PASS — production handoff reports Ready row 7d00fffd; candidate branch not rerun` |
| IA-2 exact retry | Same POST and `lib/services/initial-assessment/artifact-service.js` retry path | Repeating the same request reuses the generation/registry identity and does not call AI, upload, or create a second row. Capture row identity, generation key, call counts, and response. | `MOCKED SUITES PASS` | `PASS — production handoff reports exact retry reused the row with no new run; candidate branch not rerun` |
...
| IA-4 versions/restore | `pages/api/workbench/initial-assessment/versions.js`, `restore-version.js`, `board-snapshot.js`; `tests/unit/initial-assessment-artifact-versions.test.js` | Version list, selected-version restore, and Board snapshot preserve stable identity, conditional writes, actor policy, and cleanup ownership. | `MOCKED SUITES PASS` | `PARTIAL — current IA 7d00fffd has only v1 history; prior restore and Board snapshot 3732f140 remain historical on superseded IA a6876ad6; no new restore/Board test` |
...
| FINAL-1 same-item transition | `pages/api/workbench/final-writeup.js`; `shared/components/workbench/FinalWriteupTab.js` | Start requires session actor and lead-PD/superuser authorization, verifies the current stable document, creates/reuses one Final row, and changes no SharePoint item. | `MOCKED SUITES PASS` | `PASS — Final 473c9160 created on the same item; HTTP 200 exact retry reused the row` |
| FINAL-2 leadership transition | `pages/api/workbench/final-writeup/leadership-review.js`; Final Writeup tab | Leadership transition verifies current pointer/version, applies the ordered ETag changeset, records actor/time, and replays a landed response without a second write. | `MOCKED SUITES PASS; operator case PASS` | `PASS — leadership actor/time and same-item metadata verified; HTTP 200 retry reused the DTO` |
| AUTH-1 negative access | Every named Workbench route plus external briefing routes | Unauthenticated, wrong-app, non-reviewer, missing actor, invalid GUID/body, stale expected identity, and unauthorized leadership requests fail before service writes. | `MOCKED SUITES PASS; operator case NOT RUN` | `PARTIAL — five signed-out reads reverified on aff3049d; wrong-app case mock-only by owner decision` |
...
## Operator checklist (S1 partial; S2 incomplete; S3 happy-path email passed with inbox receipt confirmed; S4 Final and leadership passed; fault matrix remains unrun)
...
**Recorded run — PASS, candidate `aff3049d`.** On the then-current Ready/Review ZZTEST-03 PSV successor `205da1cd-b7b3-f111-aaac-6045bd04539e`, Final row `473c9160-c0b3-f111-aaac-000d3a361c1f` was created on the existing SharePoint item. Group review and leadership were recorded at `2026-09-19T00:23:51Z` and `2026-09-19T00:24:37Z`; the session-derived actor was `29b0de0d-4ff7-ee11-a1fd-000d3a3621c7`. Both exact payload retries returned HTTP 200 with `reused:true` and equal DTOs. The request-document census moved from 28 to 29: the source lifecycle/modified fields and four expected Final leadership fields changed, while all 14 distribution attempts stayed unchanged. No AI, email, or SharePoint upload/copy calls were observed in the bounded dependency capture; no schema action occurred. Fault injection was not exercised. Dataverse impersonation was rejected for each of three writes and the supported service-principal fallback succeeded; native CreatedBy impersonation is not claimed. The UI showed leadership after reload, and five signed-out route checks returned 307. Evidence is in the `/tmp/wmkf-final-smoke-*` JSON receipts and sanitized HTTP captures. Wrong-app access and review-bundle rebuild remain mock-only; other fault/retry cases remain unrun.
