...
## PR #332 later adversarial review fixes

[VERIFIED via branch source and local commands on 2026-09-24] A later Claude Opus diff review identified five P3 gaps. An authorized manual preview now returns the shared Admin copy when the sender preference is corrupt or unavailable; staff can edit it and obtain a new proof without writing a preference. An explicit one-send template skips the failing preference read, while an automatic sweep still skips before its claim. An absent personal row now validates the Admin fallback before automatic use. A stale review-due preview calls the parent list refresh. The isolated rehearsal now rejects invalid Save, Preview, and Send templates with the same validation results as the server on representative cases. The composer translates template validation errors into plain language. A subsequent independent read-only review found a race where a late preference response could overwrite the recovery edit; an edit generation guard and delayed-response test now preserve the staff draft. No auth guard, schema, reminder schedule, or email transport changed.

[VERIFIED via local commands] Focused Jest passed 7 suites/176 tests. Lint and build passed; build retained two unrelated filesystem trace warnings. API route, route lifecycle, route service, reviewer engagement, reminder hold, fact consistency, secret scan, and scaffolding gates and their self-tests passed sequentially. The synthetic rehearsal compiled and started on loopback port 3133, then was stopped. No Dataverse, Graph, SharePoint, Vercel, Postgres, or email probe was run. Staff walkthrough and live behavior remain unverified.
