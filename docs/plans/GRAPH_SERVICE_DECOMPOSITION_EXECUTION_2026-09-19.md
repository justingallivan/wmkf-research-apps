...
| External environment, campaign window, last-known-good deployment, rollback operator | The S11 closure recorded these as unknown. Subsequent release evidence verifies the dedicated `graph-rehearsal` environment and synthetic request; campaign window and named rollback operator remain **UNKNOWN**. |
...
| Approved rehearsal mode | **Verified after the S11 checkpoint across three contexts:** external upload, route fence, and one explicitly authorized clean scan in the dedicated Vercel `graph-rehearsal` deployment; standalone disposable-file conditional PUT probe; authenticated localhost candidate staff restore. No production promotion. |
| Approved environment and fixtures | **Verified for the completed rehearsal:** request `1003220` / GUID `4bfb6e40-678f-f111-8076-7ced8d3d15a6`, dedicated `graph-rehearsal` deployment, synthetic 917-byte PDF, existing SharePoint item/version readback, and bounded staging/registry effects. This evidence does not authorize reuse of production fixtures. |
| Expected side effects | **Verified by context:** the cloud external upload produced SharePoint version `3.0`, one Ready/Draft Request Document row, predecessor superseded, staging consumed, and Blob deletion; the localhost staff restore preserved the IA pointer and version history; the standalone probe cleaned its disposable file. Receipts are in the release packet; no blanket claim is made for unexercised journeys. |
