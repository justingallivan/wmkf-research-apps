...
kind: plan
status: active
summary: "Independent source review of the 2026-08-08 ROR strategic assessment. A later evaluator-only fix closes the C2 ROR provider-failure gap; C3 production-pair policy and one real-pair label remain unresolved."
...
**Phase 0 verdict: NEEDS REWORK before the measurements are used as promotion gates.** Phase 0 was a read-only review of the assessment's §§1–5. The later evaluator-only fix described below closes one gate-coverage gap; no evaluation run, provider request, resolver-mode change, or Phase 1 work has been performed. The two bounded runs remain useful as *diagnostic measurements*; C3 pair-policy and label limits remain. Owner decisions 1, 3, and 4 remain open.
...
**Proposed changes to the measurement plan:** retain the two C2 arm runs and check the now-extended provider-failure gate before interpreting either output. Separate C3 single-string resolution from pair-consistency evidence. Treat the fifth production pair as unlabeled for correctness rather than scoring it as a proven positive or negative.

### §3 Go/no-go criteria — DISAGREE materially with operationalization

**C2 safety/recall thresholds:** [VERIFIED via `lib/services/reviewer-works-first.js:572-621`] `evaluatePromotion` encodes `falseBinds = 0`, `rightPersonPolicyBinds ≤ spine`, `correctBindGain ≥ 3`, `misses ≤ 8`, and `providerFailures = 0`. I agree that any false bind or provider failure disqualifies a promotion run, and I agree that the two recall thresholds are owner policy rather than newly ratified constants. The original `evaluatePromotion` function counts only `spine` and `works` row reasons (`:580-588`). The production ROR decision layer converts provider errors to `review` with reason `provider_failure` (`lib/services/ror-institution-decision.js:359-368`); the identity bridge converts that to null (`lib/services/ror-institution-identity-resolver.js:63-75`), and Works-first then reports `claimed_institution_unresolved` (`lib/services/reviewer-works-first.js:337-352`). **Follow-up fix:** the evaluation script now folds the ROR resolver's provider-failure metric into the promotion gate, records row and resolver counts separately, and rejects missing metrics (`scripts/evaluate-reviewer-works-first.js:96-116,470-500`; `tests/unit/reviewer-works-first-evaluator-cli.test.js`). The gate's `actual` sums failure *signals*, which may overlap; zero in both components is the validity condition. The evaluation's OpenAlex request metric still explicitly excludes current-spine internal requests (`scripts/evaluate-reviewer-works-first.js:495-499`), so it is not a total-provider-request count.
...
**Proposed gate change:** the C2 evaluator now makes row-level and ROR resolver provider failures a void-run precondition; its numerical thresholds remain visible and unchanged. Keep C3 zero-wrong/veto gates; distinguish which can be measured from decision outputs, bridge outputs, and current pair-consumer outputs. Do not call the C3 comparative and five-pair gates pass/fail until the owner defines the target pair policy and labels. This is a measurement-contract correction, not a request to tune production code.
...
The owner authorized the two bounded measurements in the September brief, but has not chosen production authority, a C2 recall budget, or a shadow window. C3's v3 falsification result is real; [VERIFIED via `versions/v3/results/ror-claim-resolver-2026-08-07-v14.summary.json` and `lib/services/workbench/enrich-recommended-service.js:273-276`] it does not establish a production ROR pair policy or representative benefit to the current pair consumer. Therefore “C3 first” remains a hypothesis, not a source-backed promotion recommendation. No alternative promotion target or recall/shadow policy is recommended here. A clean C2 run can now satisfy the provider-health precondition; C3 pair-policy and label questions remain.

## Contract reconciliation and stop boundary

**Surface:** Phase 0 review plus a subsequent evaluator-only gate fix for the proposed C2 run; C3 comparator/replay remains unbuilt. **Entry points:** `scripts/evaluate-reviewer-works-first.js` and proposed `benchmarks/fuzzy-matching-falsification/run-comparator.js`. **Persistence:** only planned local JSON/JSONL measurement artifacts; no database or application write. **Consumers:** `evaluatePromotion`, comparator `judge`, the tracked aggregate report, and eventual owner decisions. **Prior finding:** the August assessment's §§1–5. [VERIFIED via `scripts/evaluate-reviewer-works-first.js:521-531` and `benchmarks/fuzzy-matching-falsification/run-comparator.js:34-77`] Both drivers write local artifacts, but neither was run in this review or its follow-up.

The whole-flow audit found the C2 ROR-error-to-null-to-review path and the C3 decision-to-identity-to-pair-policy gap described above. Partial-success audit: an ROR failure can look like an ordinary review row, so the evaluator now checks the ROR metric; a C3 comparator error must not be counted as safe abstention (`run.js:150-171`). Async and stale-state audit: no UI state or background task changed in this follow-up. Helper-extraction audit: the new evaluator-only helper preserves the runtime `evaluatePromotion` result and adds ROR health; pair policy must not be silently supplied by a comparator. Durable-surface and symbol-fan-out audits: no schema, route, enum, or runtime symbol changed. Historical August documents remain historical evidence.

**Recommendation evidence:** the prerequisite for a C2 validity claim is an observed zero across row-level and ROR resolver provider-failure metrics; the follow-up script now checks both without changing the runtime `evaluatePromotion` signature. The prerequisite for a C3 pair gate is a specified, production-equivalent pair policy and settled labels; neither is available from the single-identity bridge. A disconfirming check would show a production ROR pair consumer with an enforceable policy; the cited export and call paths did not show one.

**Phase 1 remains stopped.** The owner-requested evaluator fix closes the C2 provider-failure gap, while the C3 pair and label disagreements remain material under the September brief. Reconcile those before using C3 results as promotion evidence; do not treat this review as authorization to edit frozen cases, tune resolvers, or change production authority.
