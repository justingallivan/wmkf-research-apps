...
| C9 | `:1006–1085,1259–1503` | Folder 409 recovery reads exact cumulative path. Upload defaults to replace; fail/rename are distinct. Stable replacement requires If-Match. Large upload sends contiguous ranges without Authorization, cancels only on its existing non-ok response branch, and retains version-readback behavior. No automatic write retry. |
...
| C12 | `:1533–1542` | `clearCaches()` stays synchronous: search state, auth state with generation increment, then site/drive state. No hidden second state store; no facade import from any internal module. |
...
| `graph-service-write-contract.test.js` + extend versions/folders | All conflict modes + invalid value; Buffer/size/filename/pinned-pair checks; successful PUT with version; absent publication causes readback; HTTP-failed/mismatched-ID readback versus fetch/JSON throw after successful PUT; replacement exact If-Match/412/cTag behavior; delete 204/404/error; no extra PUT/DELETE retry. | S9 |
| Extend upload-large | Exact simple threshold and delegation; multi-chunk final short range; no auth on session PUT/DELETE; missing upload URL; HTTP-failed chunk with successful/failed cleanup; network-thrown chunk's distinct cleanup behavior; all chunks 202/no final ID; final item then readback HTTP/malformed/identity/transport failures; publication fallback and no hidden retry. | S10 |
| `graph-service-boundary.test.js` | Source import graph: no internal→facade dependency, no external runtime direct imports of internals, no duplicate implementation/state owners, no cycle through a helper, no surviving `this.` in extracted operations, no raw fetch outside http. Test matcher with red fixtures for ESM import/re-export, require/dynamic import and extensionless paths. Only dedicated test fixtures may import internal scanner helpers. | All; tighten staged inventory each move |

Mutation proof in S0: temporarily remove token generation fencing, forward bearer auth to a CDN request, change continuation 404 to `null`, replace `svc` dispatch with direct sibling dispatch in an isolated fixture, remove If-Match, and suppress the chunk failure. Each applicable test must fail for the intended reason. Restore all mutations; run tests green. Tests asserting “not called” must populate the excluded path so they prove exclusion. No runtime mutation is committed.

Boundary checker implementation is deliberately scoped: reuse the repo's existing AST tooling if suitable, but do not generalize or rewrite unrelated gates. Store any new helper under `tests/helpers/`; test its matcher fixtures. Static assertions supplement public contract tests; they do not establish behavior parity alone.
...
- `dynamics-explorer-search-documents`, `dynamics-explorer-chat-characterization`, `workbench-proposal-document-listing`, `workbench-download-proposal-document-service`.
...
1. Run all Graph suites, the stage's listed consumer suites, and the boundary checker. S0 runs only tests that exist after its additions. Use `npx jest --runInBand --silent --runTestsByPath <explicit files>`; preserve an exact command in the receipt.
2. `npm run check:types`, `npm run lint`, then `npm run build`, sequentially. Build runs the existing prebuild; inspect the manifest/worktree afterward. Follow the sandbox retry guidance in `docs/CI_GATES_REFERENCE.md` if Turbopack fails on process/port permissions. A webpack fallback is supplemental, not a green canonical build.
...
4. Inspect the complete diff. Prove body parity with a normalized comparison against the previous accepted source: allow only §3 transformations and import/reset seams. Record every exception; any semantic difference blocks acceptance. Do not update snapshots to make an unexplained difference green.
...
Use the existing browser harness if it covers a journey; otherwise add a bounded test in S0, before the affected extraction. Record exact harness files and commands in the S0 receipt. API mocks alone do not replace the real-facade bridges in §4.
...
Final planning verdict: **READY TO IMPLEMENT after separate implementation authorization and S0 prerequisites**. All three planning checkpoints completed; their named corrections are incorporated. Current build health beyond the startup checks and five Graph suites remains an S0 verification obligation, not a planning-session claim. Implementation remains unauthorized by this planning-only request.
