...
| 3 | Admin surface | `shared/components/admin/*` sections with sites (`PromptTemplatesSection` 5, `SiteVisitRecipientsSection` 4, `OperationalEventsSection` 3, `ReviewQuestionsSection` 3, `DynamicsExplorerRestrictionsSection` 3, `PoliciesSection` 2, `EmailDefaultsSection` 2, `FinalWriteupMatrixAudiencesSection` 2, `SiteVisitMaterialsDefaultsSection` 2, `MeetingTrackerDefaultsSection` 2, `ReviewerRepairAlertDetails` 1, `AdminOverviewSection` 1 [added at the Stage 3 review; the first draft omitted it]), then `pages/admin.js` (32) section by section |
...
| T2 | Stage 2, per file | **Per-call-site contract matrix, not per-endpoint sampling** (Codex cycle 2: `RespondReminderModal.js:52-75` and `:105-140` hit the same endpoint with different failure semantics, preview → `loadError`, send → `uncertain` receipt). For EVERY migrated site: (a) 2xx JSON produces today's visible state, (b) non-2xx `{error:'X'}` produces today's state/throw/toast verbatim, (c) network rejection likewise, (d) for bare-`.json()` sites a malformed 2xx produces today's outcome (for `AwardeeTab.js` send-invite: the `uncertain` receipt and step), and for tolerant sites a malformed and an empty 2xx produce today's fallback value; (e) for every bare-`.json()` site, a NON-2xx whose body is unparseable (a 502 HTML gateway page): the helper parses non-2xx tolerantly, so this input moves from the site's catch block to its `!ok` branch; the test pins the visible outcome there and it must never be silent (Stage 2 review finding at `AwardeeTab` send-invite and the expertise-finder roster sites); and for every non-GET site the test also asserts the mocked `fetch` call's URL, method, headers, and exact body string are unchanged before and after migration. Sites that branch on status or body-level flags add one case per branch. Existing RTL tests (`tests/unit/awardee-tab.test.js`, `tests/unit/staff-deliberations-tab.test.js`, `tests/unit/consultant-feedback-section.test.js`, `tests/unit/expertise-finder-batch-cycle.test.js`, `tests/unit/final-writeup-tab.test.js`) are extended, not replaced [VERIFIED P-B: each renders the component and mocks `global.fetch` per scenario]. Fixtures that mock `{ ok: true, json }` without a `status` field (e.g. `expertise-finder-batch-cycle.test.js:31`) must gain `status` before any site in that file moves to `requestEnvelope`. |
...
error surface. Two rules learned in Stage 2 (apply in every later stage):
(i) where a bare-`.json()` site's `!ok` branch reads `data.error` with no
fallback, write `data.error || envelope.error.message` so an unparseable
non-2xx body never yields an empty message that a `{error && ...}` guard hides;
(ii) where the site's catch path carries a distinct meaning from its `!ok`
path (the send-invite `uncertain` receipt), map `envelope.error?.parseError`
to that meaning inside the `!ok` branch so an unparseable non-2xx keeps its
honest reading. Verify: per-file tests, then Gate G after the group. Exit: group
...
files in this plan. Verify: per-workspace tests, Gate G. Exit: 62 admin sites
migrated (30 in sections, 32 in the page). Rollback: per-commit.
...
| D1 | Pre-existing: sites trust the body without checking `ok` (census: 38 unwrapped JSON candidates, heuristic, to be confirmed by reading) | Preserved during migration. Stage 5a/5b and the scoped remainder are now branch-built on `codex/d1-response-handling`; see `docs/plans/CLIENT_REQUEST_LAYER_D1_UNGUARDED_RESPONSES_2026-09-20.md` for accepted implementation evidence. Promotion remains owner-controlled. |
...
| D9 | Pre-existing, user-facing: `shared/components/ErrorAlert.js:45` reads prop `error`; six client call sites passed `message=` (`pages/expertise-finder.js:250,388,851`, `pages/dataverse-bulk-export.js:471`, `pages/virtual-review-panel.js:1296`, `pages/phase-i-dynamics.js:152`), so those error banners rendered blank | Expertise-finder and the three scoped remainder call sites are branch-built with `error=` and real render coverage; promotion remains owner-controlled. |
...
| Codex adversarial 4 — built code, full `main...HEAD` diff (222 files) | gpt-5.6-sol | `4007d420e` / thread `01a0c111-2419-7190-980b-c10404dd6915` | **approve**, no material findings ("request bytes, body-level flags, error precedence, stale-response guards, partial failures, and the ratchet were preserved or explicitly covered") | Owner-requested 2026-09-20 after Stage 6 acceptance; first Codex pass over code (cycles 1–3 reviewed the plan only). |
