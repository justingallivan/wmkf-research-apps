...
### Decisions — LOCKED 2026-06-02 (user accepted all recommended options)

> **D-AFF = (a)** canonical `wmkf_primaryaffiliation` + migrate the adapter & 7 readers + drop the 100-clamp · **D-BACKFILL = (a)** light backfill all 339 (link-audit gate + JSONL export) · **D-DEPT = String(255)** on the person · **D-NOTES = skip** (0 rows) · **D-DENORM = skip** (`wmkf_potentialreviewername`). Detail below.
- **D-AFF (affiliation canonical):** person `wmkf_organizationname` is **String(100)** — too short for richer affiliation; sidecar `wmkf_primaryaffiliation` is **String(500)**, populated on 330. **Recommend: adopt `wmkf_primaryaffiliation` (500) as canonical on the person, retire `wmkf_organizationname`.**
  ...
  - **Scope (enumerated 2026-06-02 — narrows Codex #7):** `wmkf_organizationname` exists on TWO entities — `akoya_request` (the proposal's applicant institution) AND `wmkf_potentialreviewers` (the reviewer's affiliation). **D-AFF only touches the reviewer-person field.** Codex #7 over-scoped by conflating them. Actual person-field scope:
    - **Writer + clamp:** `lib/dataverse/adapters/potential-reviewer.js` (FIELD_SELECT L18, 100-clamp L46, upsert L96, update L140).
    - **7 reviewer-affiliation readers to migrate:** `review-manager/reviewers.js` (194,330 — *already* `primaryaffiliation || organizationname`), `reviewer-finder/my-candidates.js` (176,279 — *already* the same fallback), `workbench/enrich-recommended.js` (154), `review-manager/render-emails.js` (91,169), `external/review/[token]/context.js` (209,224,358), `dynamics-explorer/chat.js` (885,1481,1497), `external/verify-suggestion-token.js` (81).
    - **NOT touched (read `akoya_request.wmkf_organizationname` = the proposal institution):** `render-emails.js` (94,178), `workbench/dashboard.js` (45,273), `workbench/resolve-request.js` (22,80), `reviewer-finder/generate-emails.js` (78,119), `review-manager/reviewers.js` (41,90,141,176), `reviewer-suggestion.js` (548,570).
    - **Decision:** (a) make `wmkf_primaryaffiliation` canonical on the person, migrate the 7 readers + adapter, drop the 100-clamp — **recommended** (bounded, kills truncation; 2 readers are already half-migrated); or (b) keep `organizationname` as a short display field with `primaryaffiliation` as the long companion (smaller, perpetuates the 100-cap + two-field ambiguity).
...
**Decisions: all locked 2026-06-02** (see the Decisions section). The plan is execution-ready pending the pre-flight re-verify and an explicit go-ahead to touch prod (staged: schema add → backfill → adapter → caller switchover → drop).
