...
3. `npm run check:memory-drift` and its compatibility alias
   `check:memory-drift:no-write` are read-only: they evaluate the committed
   report and never probe or write. Only `npm run refresh:memory-drift` runs
   live probes and may update the tracked JSON; that remains an owner-approved
   action.
...
npm run check:memory-drift             # note report age from the warning line
...
npm run check:memory-drift                                                   # advisory close-out reading
```
...
| 2026-09-17 | routine (§6 + §10 diet) | 8,239→6,921 | 70→67 | 67→52 | 269 | 240/269 | 21→9 | stale (read-only) | `docs/audits/memory-routine-audit-2026-09-17.md` |
...
- If an explicit drift refresh was run unintentionally: first inspect
