...
| `check:route-service-boundary:self-test` | Temp-root fixture tree proving adapter-source detection inherits the hardened classes (direct import, in-file alias, wrapper re-export, dynamic import, inline require, dynamics-service, root-level route) plus greens (clean shell, service-only route, exempt dirs), the unclassifiable-route hard error, and the ratchet firing on a divergent count. | Silent regressions in boundary detection, wave classification, or the ratchet. |
...
| `check:route-service-boundary` | `check:route-service-boundary:self-test` — red fixtures (direct/alias/wrapper-reexport/dynamic-import/inline-require adapter imports, dynamics-service import, root-level route) + greens (clean shell, service-only route, exempt dirs) + unclassifiable-route hard error + ratchet divergence. |
