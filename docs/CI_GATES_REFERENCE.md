...
| `check:route-service-boundary` | Babel-AST census of `pages/api` route files that reach the Dataverse layer directly — importing `lib/dataverse/adapters/*` or `lib/services/dynamics-service` (static import, ESM re-export, dynamic `import()`, inline `require()`, or a thin re-export wrapper), outside the exempt dir `pages/api/dataverse-export/` (S9 of the chat-service-extraction plan retired the `pages/api/dynamics-explorer/` exemption, 2026-09-19). Shares the hardened scanner core (`scripts/lib/ast-scan-core.js`) with the dataverse gate. LAW MODE since Stage 7 of `docs/ROUTE_SERVICE_CONSOLIDATION_PLAN.md`: fails on ANY in-scope boundary-importing route — no allowlist, no baseline, no ratchet for the Dataverse family. Since Stage 1 of `docs/plans/POSTGRES_ACCESS_LAYER_MIGRATION_PLAN_2026-09-23.md` (S537-PG) the same detection covers a second **`postgres` family** (`@vercel/postgres`, `pg`, `lib/postgres/**`, through the same re-export propagation) with an in-script, shrink-only `POSTGRES_CARRYOVER` array of the 17 routes that reached Postgres directly at the freeze, pinned exact-set by `tests/unit/route-service-boundary-postgres-carryover.test.js`; a carry-over entry never excuses a Dataverse reason in the same route. `--json` rows carry `boundaryFamily`. | Any `pages/api` route importing adapters or `dynamics-service` directly (law); any route outside `POSTGRES_CARRYOVER` reaching Postgres directly (law); any carry-over entry whose route no longer reaches Postgres or no longer exists (stale — remove it); `--postgres-carryover` passed without a non-default `--root` (self-test-only override). |
| `check:route-service-boundary:self-test` | Temp-root fixture tree proving adapter-source detection inherits the hardened classes (direct import, in-file alias, wrapper re-export incl. ESM import-then-export and CJS binding re-export, dynamic import, inline require, dynamics-service, root-level route) plus greens (clean shell, service-only route, service exporting its own adapter-calling functions, exempt dirs), the fail-closed hard error on non-literal `require()`/`import()` sources in routes or re-export positions, and law mode failing closed on every red class (each offending route named) while a green-only tree exits 0; plus the Postgres family (S537-PG): red direct `@vercel/postgres`/`pg`/`lib/postgres/*`/thin re-export wrapper routes when unlisted, red stale carry-over entries (clean route, missing file), a mixed wrapper re-exporting both families still failing on Dataverse for a listed route, and greens for a listed route, a route consuming a service that merely uses Postgres, and `pg-copy-streams`. Every fixture run passes an explicit scenario carry-over file. | Silent regressions in boundary detection, fail-closed handling, the law gate, or the carry-over semantics. |
| `check:postgres-access-layer` | Babel-AST census + **count ratchet** (on `scripts/lib/ast-scan-core.js`) of direct Postgres access under `pages/`, `lib/`, `shared/`, `modules/`: driver imports (`@vercel/postgres`, `pg`, `@neondatabase/serverless` — static/ESM re-export/dynamic import/inline require, incl. a same-file string-const alias), `sql` tagged templates (counted wherever they appear, incl. renamed/destructured/namespace tags bound to an import), `sql.query`/`client.query`/`pool.query`, `db.connect`/`pool.connect`, `new Pool(` (incl. `new pg.Pool(` and renamed `Pool`), `BEGIN` literals, computed `require()`/`import()` sources (`unresolved-import`: a non-literal specifier that does not resolve one hop through a same-file string const — the gate cannot see what it loads, so new ones are red), and raw-driver exports (`driver-export`: `export { sql }`, `module.exports.pool = pool`, or an exported function returning a driver-bound binding — the laundering shape), plus best-effort table names. Comment-only mentions never count. `scripts/postgres-access-allowlist.json` is the Stage 0 census frozen 2026-09-23 as line-tolerant `{file, kind, count}` keys (156 entries incl. the 9 live deliberate bundler-bypass `unresolved-import` sites; no regenerate flag — hand-edited downward only). Q5 exemptions (`lib/utils/migration-drift.js`, `lib/utils/health-checker.js`) and the seam `lib/postgres/client.js` are exempt for every kind: reported, never ratcheted, never allowlisted. Any other `lib/postgres/**` file is exempt only for statement kinds (`sql` tags, query/connect calls, `BEGIN`) and is ratcheted for `driver-import`, `driver-export`, `unresolved-import` and `new-Pool`, so a store cannot bypass the seam. Q6 cast-lint is a warn-only report section (per-kind and per-file×kind totals; rows behind `--cast-lint-detail` / `--json`). Stage 1 of `docs/plans/POSTGRES_ACCESS_LAYER_MIGRATION_PLAN_2026-09-23.md` (S537-PG). | (a) any `(file, kind)` key not in the allowlist, or a census count above its allowed count (a new raw call in a legacy file); (b) an allowlist count above the current census or a vanished key (stale entry — shrink it); (c) any `pages/api` file importing `lib/postgres/**` (no-op until the seam exists); an allowlist entry for a Q5-exempt file, a duplicate key, or a malformed entry. `--report` and `--json` apply the same exit code (package.json runs `--report`). Exit 2 only on a parse failure or a bad allowlist file. |
| `check:postgres-access-layer:self-test` | Temp-root fixture tree under `os.tmpdir()` (never under scanned repo paths) with per-scenario `--allowlist` files proving every recognised kind incl. the Stage 1 binding forms (renamed/destructured/namespace `sql`, `new pg.Pool`, renamed `Pool`, nested member `this.pool.query`/`.connect`, string-const require alias), a `sql` tag counted without any driver import (barrel consumer), neon/subpath driver sources, `ON CONFLICT … DO UPDATE SET` / `SKIP LOCKED` / quoted-identifier table extraction, comment-only exclusion, and every ratchet outcome: green exact match; red unknown key, count above allowed, allowlist above census, vanished key, exempt file listed; red route→`lib/postgres` import when the dir exists and green no-op when it does not; green Q5 exemptions for both files; cast-lint warned vs `::cast`-suppressed; the Codex-found shapes — computed specifier (`['@vercel','postgres'].join('/')`) red as `unresolved-import`, ESM and CommonJS raw-driver accessors (`export function getRawSql() { return sql }`, `module.exports.getRawSql = () => sql`, `exports.X`, function-valued `module.exports = {…}` properties) red as `driver-export` with an internal-use control green, aliased `require` (`const load = require; load('pg')`) and `module.require('pg')` counted, a legitimate allowlist-backed shrink green, and a >64 KiB `--json` pipe deep-equal to the in-process payload. | Silent regressions in recognition (an evasion of the ratchet) or in any ratchet failure mode. |
| `check:reviewer-engagement-boundary` | Babel-AST census (shares source/export detection with `scripts/lib/ast-scan-core.js`; alias/method-extraction/barrel resolution is gate-specific) of `lib/`, `pages/`, `shared/`, `modules/` files that BIND one of the reviewer-suggestion adapter's four generic writers (`updateLifecycle`, `patchFields`, `patchReviewReceipt`, `bulkUpdateByRequest`) via a MONOTONIC FIXPOINT: named import/require-destructure; namespace import (or CJS whole-require, or awaited dynamic `import()`) plus a member access naming the writer; a same-file alias chain of any length; an extracted-method reference or destructure off an object identifier; a whole-barrel wrapper (`export * from`, CJS whole `require(...)`, or CJS spread `{ ...adapter }`), transitively through another such barrel, with a NAMED import from it classified exactly like a named import from the adapter; a named one-hop ESM re-export/CJS re-publish through a wrapper, transitively through a wrapper-of-a-wrapper; a computed member access on a namespace binding (fails CLOSED unless the key is a string literal resolving to a non-writer name); a class-held adapter (`this.<field>` bound by a class-field initializer or constructor assignment, flowing through the same alias fixpoint — file-scoped, so a second class's unrelated `this.adapter` is over-approximated closed); a renamed member re-export (`module.exports = { mutate: adapter.updateLifecycle }`, `exports.mutate = …`, `export const mutate = …`) classified at the consumer like the writer itself; a direct `(await import('<adapter>')).writer` access; and a generic catch-all for any other unresolvable object shape whose outer property names a writer (or is non-literal over a subtree holding a literal adapter source). A non-literal require()/import() source fails CLOSED only when it could be laundering a writer binding — a writer-named or non-literal property, destructure or identity re-export — and that check follows the same alias chain; a bare lazy-backend `require(path)` or `(await import(path)).default` with no writer-shaped use stays green (documented limit). LAW MODE since Reviewer Lifecycle Stage 7: allowed only from `lib/services/reviewer-engagement/*`, the adapter itself, or the tracked `RECORDED_IMPORTERS` map in the gate script — a stale entry (file gone, or no longer binding its writer) fails the gate; GROWING the map (a new entry) is guarded separately by `tests/unit/reviewer-engagement-boundary-recorded-set.test.js`, which pins it to its exact contents. `scripts/` is out of scope (D5: recorded, not gated). | Any generic suggestion-writer bypass of the named `reviewer-engagement/` commands, an undetected alias/barrel/computed-member evasion, or a recorded-importer entry that has silently rotted or silently grown. |
| `check:reviewer-engagement-boundary:self-test` | Temp-root fixture tree proving every binding form (named import, namespace + member access, destructured require, dynamic import + member access, same-file alias chains, extracted-method/destructure-off-identifier references, `export *`/CJS-whole/CJS-spread barrels incl. transitively through a wrapper-of-a-wrapper, named ESM/CJS wrapper re-export/re-publish, computed member access, class-held adapter via `this.<field>` incl. aliased-to-local and passed-through-a-helper forms, renamed CJS/ESM member re-exports, direct dynamic-import member, `shared/`/`modules/` roots), the recorded-importer exemption (valid + stale + missing-file scenarios), narrow non-generic-writer ops staying green, the catch-all's greens (dynamic-keyed constant lookup, chained non-writer call, inline literal `require('<adapter>').findById`), `scripts/` staying out of scope, the narrowed non-literal-source fail-closed rule across every documented shape (direct and aliased: destructure, member access, identity re-export, dynamic-import writer member/computed property) with lazy-backend `require(p)` and `(await import(p)).default` green counterparts, and law mode (every red named, green-only tree exits 0). | Silent regressions in binding detection (incl. the alias/barrel/computed-member classes from the Stage 7 correction round and the class-held/renamed-export/dynamic-import/catch-all classes from the second round and post-merge Opus fixes), the recorded-importer map's integrity check, or the law gate. |
| `check:script-suggestion-writers` | Plain-text, entity-scoped scan of `scripts/**/*.{js,mjs,cjs}` (excluding `check-*` gate scripts by name) for writes to `wmkf_appreviewersuggestions` outside the adapter's named ops: `DynamicsService.updateRecord/deleteRecord/createRecord` with the suggestion entity as a literal or a same-file alias (string literal or `resolveEntitySetName(...)`); the same write with an UNRESOLVED first argument in a file that names the entity (fails closed); an own-`fetch` PATCH/DELETE against a `wmkf_appreviewersuggestions(<id>)` URL (bypasses the target interlock); and the adapter's generic writers called from a file that imports the adapter. Every writer must appear in the tracked `RECORDED_SCRIPT_WRITERS` map with its exact shapes; a stale or drifted entry fails; GROWTH is pinned by `tests/unit/script-suggestion-writers-recorded-set.test.js`. Owner decision D5 (2026-09-06) — the `scripts/` complement of `check:reviewer-engagement-boundary`. Documented limit: a raw POST create via fetch is not detected (every script's OAuth token call is a POST). | A new or silently re-shaped operational script writing the suggestion entity raw, or a recorded entry that has rotted. |
| `check:script-suggestion-writers:self-test` | `--root` temp fixture tree: every red shape (literal/alias/logical-name `updateRecord`, `deleteRecord`, `createRecord`, unresolved helper param, raw-rest PATCH and DELETE, `updateLifecycle` and destructured `patchReviewReceipt`), every green (read-only, adapter named ops, other-entity literal/alias/`resolveEntitySetName`, GET-only with token POST, `@odata.bind` fragment with no nearby write, `check-*` out of scope), stale-missing, stale-not-writing, shape drift, empty tree, and green-only exit 0. | Silent regressions in shape detection or the recorded-set integrity check. |
...
- Exemptions: `lib/dataverse/core/odata.js` itself (the primitive necessarily
  contains the pattern it forbids elsewhere); a single-file exemption for
  `lib/services/dynamics-explorer/tools/get-entity.js` (one legacy escape
  carried over from the former `pages/api/dynamics-explorer/chat.js`); no
  directory-level exemption remains — S9 of the chat-service-extraction plan
  retired the `pages/api/dynamics-explorer/` exempt dir (2026-09-19);
...
- Self-test: `npm run check:odata-escape:self-test` — fixture-based, proves
  red (mechanical + `String(x)`/single-quoted-replacement variants + a raw
  escape under `pages/api/dynamics-explorer/`, now non-exempt), green
  (after removal), and that the HTML/XML/comment-only/canonical-file/
  single-file-exempt-file shapes are never flagged.
...
| `check:postgres-access-layer` | `check:postgres-access-layer:self-test` — every kind fixture incl. Stage 1 binding forms, sql-tag-without-driver (barrel consumer counts), neon/subpath sources, table-extraction edge cases, comment-only exclusion, all ratchet red/green outcomes via per-scenario `--allowlist` files, Q5 exemptions, cast-lint warn/suppress, computed-specifier and raw-driver-accessor (ESM + CommonJS) reds, aliased/`module.require` loads, shrink green, >64 KiB JSON pipe; fixtures under `os.tmpdir()`, cleaned in `finally`. |
| `check:reviewer-engagement-boundary` | `check:reviewer-engagement-boundary:self-test` — law-mode red fixtures (named import, namespace + member access, destructured require, dynamic import + member access, same-file alias chains, extracted-method/destructure-off-identifier references, `export *`/CJS-whole/CJS-spread barrels incl. transitively, named ESM/CJS wrapper re-export/re-publish, computed member access, class-held `this.<field>` adapter incl. alias-local and helper-arg forms, renamed member re-exports, direct dynamic-import member, `shared/`/`modules/` roots), the recorded-importer exemption's valid/stale/missing-file scenarios, narrow-ops, catch-all and `scripts/`-out-of-scope greens, the narrowed non-literal-source fail-closed rule across every documented shape (direct and aliased) with lazy `require(p)`/`import(p).default` greens, and green-only tree exits 0. |
| `check:script-suggestion-writers` | `check:script-suggestion-writers:self-test` — red/green shape fixtures, stale/drift/empty-tree scenarios, green-only exit 0. |
