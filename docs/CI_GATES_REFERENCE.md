...
| `check:postgres-access-layer` | Babel-AST census (on `scripts/lib/ast-scan-core.js`) of direct Postgres access under `pages/`, `lib/`, `shared/`, `modules/`: driver imports (`@vercel/postgres`, `pg` — static/ESM re-export/dynamic import/inline require), `sql` tagged templates, `sql.query`/`client.query`/`pool.query`, `db.connect`/`pool.connect`, `new Pool(`, `BEGIN` literals, plus best-effort table names. Comment-only mentions never count. Stage 0 of `docs/plans/POSTGRES_ACCESS_LAYER_MIGRATION_PLAN_2026-09-23.md` (S536). | **Nothing yet — REPORT-ONLY.** Exits 0 on any census result; exits 2 only when a scanned file fails to parse. The ratchet (raw access outside `lib/postgres/**` and the Q5 exemptions) arrives in Stage 1 of the plan. |
| `check:postgres-access-layer:self-test` | Temp-root fixture tree under `os.tmpdir()` (never under scanned repo paths) proving every recognised kind, alias/namespace/CJS/dynamic import forms, the barrel no-propagation choice, `ON CONFLICT … DO UPDATE SET` / `SKIP LOCKED` / quoted-identifier table extraction, and that a comment-only driver mention yields zero records. | Silent regressions in the census that Stage 1's ratchet will inherit. |
...
| `check:postgres-access-layer` | `check:postgres-access-layer:self-test` — every kind fixture, alias/namespace/CJS/dynamic import forms, barrel no-propagation, table-extraction edge cases, comment-only exclusion; fixtures under `os.tmpdir()`, cleaned in `finally`. |
