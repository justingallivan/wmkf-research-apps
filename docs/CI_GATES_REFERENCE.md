...
| `check:route-service-boundary` | Babel-AST census of `pages/api` route files that reach the Dataverse layer directly — importing `lib/dataverse/adapters/*` or `lib/services/dynamics-service` (static import, ESM re-export, dynamic `import()`, inline `require()`, or a thin re-export wrapper), outside exempt dirs `pages/api/dynamics-explorer/` + `pages/api/dataverse-export/`. Shares the hardened scanner core (`scripts/lib/ast-scan-core.js`) with the dataverse gate. RATCHET MODE: fails if the count rises above `scripts/route-service-boundary-baseline.json`; a falling count must update the baseline same commit. Drives `docs/ROUTE_SERVICE_CONSOLIDATION_PLAN.md`. | New/regressed `pages/api` routes importing adapters or `dynamics-service` directly instead of going through a `lib/services/<domain>/` service. |
| `check:route-service-boundary:self-test` | Temp-root fixture tree proving adapter-source detection inherits the hardened classes (direct import, in-file alias, wrapper re-export incl. ESM import-then-export and CJS binding re-export, dynamic import, inline require, dynamics-service, root-level route) plus greens (clean shell, service-only route, service exporting its own adapter-calling functions, exempt dirs), the fail-closed hard error on non-literal `require()`/`import()` sources in routes or re-export positions, the unclassifiable-route hard error, and the ratchet firing on a divergent count. | Silent regressions in boundary detection, wave classification, fail-closed handling, or the ratchet. |
...
| `check:route-service-boundary` | `check:route-service-boundary:self-test` — red fixtures (direct/alias/wrapper-reexport incl. import-then-export + CJS binding re-export/dynamic-import/inline-require adapter imports, dynamics-service import, root-level route, non-literal-source hard-fails) + greens (clean shell, service-only route, own-function-exporting service, exempt dirs) + unclassifiable-route hard error + ratchet divergence. |
