...
| `check:reviewer-engagement-boundary` | `check:reviewer-engagement-boundary:self-test` — law-mode red fixtures (named import, namespace + member access, destructured require, dynamic import + member access, same-file alias chains, extracted-method/destructure-off-identifier references, `export *`/CJS-whole/CJS-spread barrels incl. transitively, named ESM/CJS wrapper re-export/re-publish, computed member access, `shared/`/`modules/` roots), the recorded-importer exemption's valid/stale/missing-file scenarios, narrow-ops and `scripts/`-out-of-scope greens, the narrowed non-literal-source fail-closed rule across every documented shape (direct and aliased), and green-only tree exits 0. |
