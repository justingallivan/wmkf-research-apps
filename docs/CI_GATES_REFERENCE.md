...
| `check:reviewer-engagement-boundary` | Babel-AST census (shares `scripts/lib/ast-scan-core.js`) of `lib/`, `pages/`, `shared/`, `modules/` files that BIND one of the reviewer-suggestion adapter's four generic writers (`updateLifecycle`, `patchFields`, `patchReviewReceipt`, `bulkUpdateByRequest`) — named import/require-destructure, namespace import (or CJS whole-require, or awaited dynamic `import()`) plus a member access naming the writer, or one hop of ESM re-export/CJS re-publish through a wrapper. LAW MODE since Reviewer Lifecycle Stage 7: allowed only from `lib/services/reviewer-engagement/*`, the adapter itself, or the tracked `RECORDED_IMPORTERS` map in the gate script — an entry whose file no longer exists or no longer binds its recorded writer is ALSO a failure ("stale recorded importer"), so the map can only shrink honestly. `scripts/` is out of scope (D5: recorded, not gated). | Any generic suggestion-writer bypass of the named `reviewer-engagement/` commands, or a recorded-importer entry that has silently rotted. |
| `check:reviewer-engagement-boundary:self-test` | Temp-root fixture tree proving every binding form (named import, namespace + member access, destructured require, dynamic import + member access, ESM re-export wrapper, CJS re-publish wrapper, `shared/`/`modules/` roots), the recorded-importer exemption (valid + stale + missing-file scenarios), narrow non-generic-writer ops staying green, `scripts/` staying out of scope, the narrowed non-literal-source fail-closed rule (a writer-shaped destructure hard-fails; a lazy-backend shape does not), and law mode (every red named, green-only tree exits 0). | Silent regressions in binding detection, the recorded-importer map's integrity check, or the law gate. |
...
| `check:reviewer-engagement-boundary` | `check:reviewer-engagement-boundary:self-test` — law-mode red fixtures (named import, namespace + member access, destructured require, dynamic import + member access, ESM re-export wrapper, CJS re-publish wrapper, `shared/`/`modules/` roots), the recorded-importer exemption's valid/stale/missing-file scenarios, narrow-ops and `scripts/`-out-of-scope greens, the narrowed non-literal-source fail-closed rule, and green-only tree exits 0. |
