...
- [VERIFIED via `shared/components/reviewers/ReviewerCloseoutModal.js`] Request-scoped notes now live beside the PD's plain-language payment decision: optional before a choice and for Yes, required for No.
...
- [VERIFIED] Closeout notes remain optional for ordinary closeout and become required only when the PD says no honorarium should be paid, so the exception carries its rationale.
...
- [VERIFIED via `pages/api/review-manager/close-review.js`] The closeout route accepts bounded notes, requires a nonblank reason for Not eligible, and retains the existing lead-PD/superuser authorization boundary.
...
- [VERIFIED] Request-scoped notes are saved as part of closeout and are required only for a No payment decision.
...
4. [PARTIAL] Request-scoped PD notes now live in closeout and are required for a No payment decision; a global "would ask again?" flag and any required global rationale remain unbuilt decisions.
