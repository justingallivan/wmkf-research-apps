...
**Status: ✅ DEPLOYED to prod 2026-06-03** via `apply-dataverse-schema.js --target=prod --wave=6 --execute`; all 6 fields confirmed present in `wmkf_potentialreviewers` metadata. The deploy was initially blocked for ~15 min by a 429 `0x80071151` — "another [Import] running" — which a live `importjobs` probe confirmed was a **rolling Microsoft managed-solution update wave** (`msdyn_URModernAdminBase` → `msdynce_EmailTemplateConfigSolution` → …), NOT WMKF work; it landed cleanly once the wave cleared. (Gotcha logged: the metadata `Attributes` endpoint 501s on `$filter startswith` — verify field presence by fetching all + filtering in JS.) PR1 threading/write-gates are now unblocked.
