...
related:
  - scripts/probe-dataverse-batch-changeset.mjs
  - lib/dataverse/schema/wave8-review-answer-snapshot/01_wmkf_appreviewanswer.json
...
- `pages/api/external/review/[token]/context.js` GET assembles proposal/reviewer/files/prefill, computes `engagementState`, and returns an `_etag` for optimistic locking (round-tripped as `If-Match` by `/respond`). **No draft/autosave** — one atomic submit. (This is the pre-build S300 baseline; Phase 1 added the draft route, and Phase 2 removed the then-returned `formSchema` field — the client now imports the schema statically. See §8.)
...
- **Dependency:** add `@tiptap/react`, `@tiptap/starter-kit`, `@tiptap/pm`. New `shared/components/external/RichReviewEditor.js` — controlled tiptap with one compact toolbar for reviewer and staff manual-entry surfaces (bold, italic, subscript, superscript, undo, redo); raw-HTML paste is stripped. Both surfaces share one schema so historical structural formatting remains readable.
- **Formatting allowlist [DECISION #A, standard set]:** bold, italic, subscript, superscript, bullet/numbered lists, H2/H3, blockquote, links. Both live authoring surfaces expose the compact inline subset; structural tags remain accepted for historical content. **No images, no tables.**
- **Sanitizer — executable contract, not prose (Codex P1-2).** `lib/external/sanitize-review-html.js` exporting `sanitizeReviewHtml(html)` using **`sanitize-html`** (DOM-free — **never** DOMPurify+jsdom, which does not load in Vercel/Turbopack serverless; memory `project-jsdom-serverless-esm-incompat`). Enumerated config:
  - `allowedTags`: `p, br, strong, b, em, i, sub, sup, ul, ol, li, h2, h3, blockquote, a` (nothing else).
...
**[Originally VERIFIED S300; NOW BUILT S302]** At S300 `DynamicsService` exposed only single-row writes — `createRecord`, `updateRecord` (supports `ifMatch`), `updateIfEmpty`, `deleteRecord` (supports `ifMatch`) — with no `$batch`/changeset/multipart method anywhere in `lib/`, so the "all-or-nothing submit" the snapshot model needs had to be built. **It now exists: [VERIFIED via `lib/services/dynamics-service.js` `executeChangeset`, S302]** alongside those single-row helpers. **[VERIFIED via `pages/api/admin/prompts/[name].js:12`]** the prompt/policy publish flows carry a comment that "Dataverse has no $batch transaction" and deliberately use a non-atomic mirror — that belief was refuted by the S301 spike and the working helper; if that flow is ever reworked, `$batch` is a real option.

Plan (owner chose to **build the helper**, not redesign around non-atomic writes):
1. **Feasibility spike — ✅ DONE (S301, 2026-06-28): GO.** `scripts/probe-dataverse-batch-changeset.mjs --execute` against **prod** (test suggestion under request 1002788) proved all three properties [VERIFIED via probe run]: (a) `$batch` accepts a `multipart/mixed` single changeset and **commits** a multi-op changeset (two creates → HTTP 200, ops `204,204`); (b) the changeset is **atomic** — a create + a 404-delete returned HTTP 400 and the created row did **not** persist (rolled back); (c) **per-op `If-Match` is honored** (stale etag → 412). The probe self-cleaned (0 rows left). **This refutes the prior "Dataverse has no `$batch` transaction" belief** (`pages/api/admin/prompts/[name].js`). → The non-atomic fallback below is **NOT needed**; build `executeChangeset` on the atomic path.
2. **`DynamicsService.executeChangeset(operations, { actingUserSystemId })` — ✅ DONE (S302).** Builds one `multipart/mixed` `$batch` request wrapping a single changeset of create/PATCH/delete operations (all-or-nothing), supports **per-operation `If-Match`**, parses the nested multipart response, and surfaces per-operation failures with the same structured-error shape as the single-row helpers (`.status` = the failing op's HTTP status, so callers branch on 412 as with `updateRecord`). Reuses the existing token/headers/`MSCRMCallerID` plumbing. Op shape: `{ method, url, body?, ifMatch? }` (url is a v9.2-relative path, no leading slash). [VERIFIED via `lib/services/dynamics-service.js` `executeChangeset`.]
3. **Isolated tests — ✅ DONE (S302):** `tests/unit/dynamics-service-changeset.test.js` (17 tests) covers changeset body construction, multipart-response parsing, per-op `If-Match`, all-or-nothing failure surfacing (embedded 412/400 + bare-JSON-error fallback), **fail-closed on an unconfirmable commit** (outer-200 under-count / unparseable embedded status), **wire-format tolerance** (LF endings + case-insensitive MIME), and input validation — landed **before** `/submit` consumes it.
...
- **Phase 1 (data layer + sanitizer, no UI) — ✅ DONE (S301, 2026-06-28).** `lib/external/sanitize-review-html.js` (enumerated `sanitize-html` allowlist + `htmlToPlainText` + `isEffectivelyEmptyHtml`) with its full bypass suite (`tests/unit/sanitize-review-html.test.js`, 36 cases — every §4 vector); migration `021_review_drafts.sql` + manifest; `lib/services/review-draft-service.js` (`tests/unit/review-draft-service.test.js`); draft GET/PUT route `pages/api/external/review/[token]/draft.js` (server-sanitize on write, schema-key whitelist, finality 409 / materials-not-sent 409 via reused `computeEngagementState`, 2 MB body cap) with `tests/integration/external-review-draft-route.test.js`. Atlas: new PG page `docs/atlas/postgres-review-drafts.md` + master tables; API matrix row added; `CANONICAL_COUNTS` route count refreshed (134→135). **`check:prompt-injection-tagging` DECISION:** does NOT apply to these reviewer answers in v1 — they are stored-and-rendered-to-staff HTML, not an LLM-prompt surface. The untrusted-content markers apply only if/when the future document assembler or VRP feeds these answers to a model (revisit then). **Migration applied:** `node scripts/apply-migrations.js` ran 2026-06-28 (S301); `review_drafts` is live in the DB (`[apply ok] 021_review_drafts.sql`, 0 rows).
- **Phase 2 (editor) — ✅ DONE (S301, 2026-06-28; toolbar contract updated 2026-08-14).** **2a (Codex P0/P2):** added the 8 rich-text questions (Q2/Q4–Q9/Q11) to `lib/external/review-form-schema.js` (`richtext` type, stable keys, `order` 1–11, `maxLength` 50000); `validateReviewForm` skips richtext (parent-only, legacy upload path intact); `ReviewFormFields` filters richtext; schema test + the route test (`q2: '<img onerror=…>'` persists only as sanitized HTML); `SERVICE_AND_UTILITY_CATALOG.md` updated. **2b (full cutover, owner-approved S301):** added tiptap deps; `shared/components/external/RichReviewEditor.js` (one compact toolbar for reviewer and staff manual-entry surfaces; one shared sanitizer-compatible schema that retains historical headings/lists; SSR-safe `immediatelyRender:false`); `shared/components/external/ReviewAuthoringForm.js` (controlled, autosave debounced to the Phase 1 draft route, initial = draft overlaid on prefill); `MaterialsView` now renders the authoring form instead of the file-upload card (upload route/infra retained server-side, hidden per §7); submitted view is read-only. **At Phase 2 the Submit button was disabled (final submit was Phase 3 — now wired, see Phase 3 below).** `npm run build` green; full suite green (expected-red only). **Browser-verified** (Playwright/Chromium, real prod build): no file input, 8 editors render, autosave PUT on edit, reload rehydrates. **Codex Phase-2 review folded in:** P0 — draft-load race fixed (the form now gates render on the draft GET so no editable surface exists before the saved draft loads; a returning reviewer's keystrokes can't be clobbered; delayed-GET E2E proves it); P1 — richtext `maxLength` (50000) now enforced server-side on the draft PUT (`400 answer_too_long`). Codex re-review of the P0/P1 fixes: **SIGN-OFF**. All Codex P2s also resolved (S301): token-change remount (`key={token}` on the dispatched `MaterialsView`); picklist value normalization (`''`/non-finite/out-of-range → `null` in `buildInitialValues`); removed the dead `data.formSchema` response field from `context.js` (no client consumer — both renderers import the schema statically); submitted-view E2E added (read-only notice, no editors, no draft fetch). Phase 2 is fully clean against the review.
- **Phase 2.5 (changeset helper — Codex P0-N1, prerequisite for Phase 3): ✅ DONE.** Feasibility spike GO (S301, `scripts/probe-dataverse-batch-changeset.mjs`; §5a). `DynamicsService.executeChangeset` built S302 with per-op `If-Match` + all-or-nothing semantics + isolated tests (`tests/unit/dynamics-service-changeset.test.js`: body construction, multipart-response parse, per-op `If-Match`, failure surfacing, validation). The §5a non-atomic fallback is not needed.
- **Phase 3 (submit + lifecycle) — ✅ DONE (S302, 2026-06-28).** `lib/external/build-review-submission.js` — `validateReviewSubmission` (full-form, live picklist domain, richtext emptiness-after-strip + maxLength, strict integer rating parse) + the single producer `buildReviewSubmission()` (P1-N4) with the snapshot-fidelity / exactly-3-ratings / rating-domain / parent-child-equality backstops asserted in code (Codex S301 P1; 21 unit tests). `pages/api/external/review/[token]/submit.js` — finality precheck (409) → server-sanitize → validate → `executeChangeset` (N answer rows upserted by the **`_wmkf_appreviewersuggestion_value=<guid>`** alt-key form — prod-verified S302, NOT the bare logical name; memory `reference-dataverse-altkey-lookup-upsert-url`) + parent PATCH **fail-closed on `If-Match`** (re-reads for a fresh etag, refuses to write unguarded — Codex S302 P1) → draft deleted post-commit (best-effort); 13 integration tests; API matrix row added; route count 135→136. `upload.js` reviewer-token path 409s post-submit (P0-1). `ReviewAuthoringForm` Submit button wired → POST `/submit`, locks read-only on success (stage2b E2E: submit → read-only). Codex post-impl review folded (P1 etag fail-closed + 2×P2). **Deferred to Phase 4:** `context.js` post-submit prefill **from child rows** (the submitted view shows a read-only receipt now; full answer read-back is the Phase 4 keyed child read). Token-window tightening deferred (the finality precheck + If-Match are the live guards). **Snapshot-fidelity backstop** is enforced in `buildReviewSubmission()` (hard-asserts order/text/type per row + tested), so the missing-`requiredLevel` on those columns is covered in code.
- **Phase 4 (workbench + fan-out) — ✅ DONE (S302, 2026-06-28).** `/api/review-manager/reviewers` GET attaches `answers[]` per submitted reviewer via the keyed child read (§6: `queryAllRecords('wmkf_appreviewanswers', { filter: '_wmkf_appreviewersuggestion_value eq <id>', orderby: 'wmkf_questionorder' })`, chunked + paginated, **re-sanitized on read**). `ReviewsTab` renders the narrative rich-text answers (questionText → sanitized HTML) below the existing rating cells; picklist rows stay as cells (no duplication). Tests: `tests/integration/review-manager-reviewers-answers.test.js` (ordering, read-time re-sanitization, keyed filter, no-submit short-circuit) + `tests/unit/reviews-tab.test.js` (narrative render). The P1-1 consumer fan-out is complete (schema → keyed read → reviewers DTO → ReviewsTab).
- **Phase 5 (lifecycle integration + cleanup + gates) — ✅ DONE (S302, 2026-06-28).** Review draft deleted on token **revoke** and **regenerate** (Codex P1-4 — wired into `revoke-token.js` + `regenerate-token.js` after the token action, best-effort; **not** `mintAndStore`, which also runs on benign reminder/email resends [VERIFIED via grep of `mintAndStore` callers]); draft GC added to the maintenance cron (`ReviewDraftService.deleteExpired`, 90d); upload UI hidden since Phase 2 + dormant infra documented (memory `project-reviewer-upload-dormant-not-deleted`, agent-wiki); `stage2b` E2E (5 specs incl. submit → read-only → conflict-lock). Tests: token-route draft-delete (+ non-fatal) cases.
...
**Top risks:**
1. **Changeset feasibility (RESOLVED S301)** — the atomic submit depends on a `$batch` changeset helper that does **not** exist yet, and prior code believed `$batch` was unavailable here ([prompts/[name].js:12]). **Spike result: GO** — the probe confirmed `$batch` atomic changeset + per-op `If-Match` work in prod (§5a), refuting that belief. The helper still has to be built, but on the sound atomic path; the non-atomic fallback is off the table.
...
- Migration mechanism: `node scripts/apply-migrations.js` + `lib/db/migrations-manifest.json` (manifest ends at `021_review_drafts.sql`, applied S301).
...
- **Changeset helper — ✅ BUILT (§5a, P0-N1, S302):** `DynamicsService.executeChangeset` ([dynamics-service.js `executeChangeset`]) — atomic `$batch` changeset, per-op `If-Match`, structured per-op failures. The prompt/policy publish flows ([prompts/[name].js:12]) still use a non-atomic mirror written before `$batch` was verified available here.
