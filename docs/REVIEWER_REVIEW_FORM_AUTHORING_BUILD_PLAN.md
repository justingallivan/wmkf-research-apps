...
- **Phase 2.5 (changeset helper — Codex P0-N1, prerequisite for Phase 3):** feasibility spike **✅ DONE (S301): GO** (`scripts/probe-dataverse-batch-changeset.mjs`; §5a) — `$batch` atomic changeset + per-op `If-Match` confirmed working in prod. **Remaining:** build `DynamicsService.executeChangeset` with per-op `If-Match` + all-or-nothing semantics + isolated tests (changeset body construction, multipart-response parse, per-op `If-Match`, rollback). The §5a non-atomic fallback is not needed.
