...
3. **Isolated tests — ✅ DONE (S302):** `tests/unit/dynamics-service-changeset.test.js` (13 tests) covers changeset body construction, multipart-response parsing, per-op `If-Match`, all-or-nothing failure surfacing (embedded 412/400 + bare-JSON-error fallback), and input validation — landed **before** `/submit` consumes it.
