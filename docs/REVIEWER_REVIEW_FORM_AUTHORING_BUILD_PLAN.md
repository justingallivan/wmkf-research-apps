...
- **Dependency:** add `@tiptap/react`, `@tiptap/starter-kit`, `@tiptap/pm`. New `shared/components/external/RichReviewEditor.js` — controlled tiptap with a compact reviewer toolbar (bold, italic, subscript, superscript, undo, redo) and a full staff toolbar for manual rescue entry; raw-HTML paste is stripped. Both variants share one schema so historical structural formatting remains readable.
- **Formatting allowlist [DECISION #A, standard set]:** bold, italic, subscript, superscript, bullet/numbered lists, H2/H3, blockquote, links. The reviewer toolbar exposes the compact subset; the staff rescue toolbar exposes the structural controls. **No images, no tables.**
