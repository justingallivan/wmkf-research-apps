...
> **Current routing (2026-07-26):** Historical discussion and rationale record. Use
> `docs/audits/AUDIT_REQUEST_WORKBENCH_TRUTH_2026-07-26.md` for source-backed status and
> `docs/REQUEST_WORKBENCH_NEAR_TERM_EXECUTION_PLAN.md` for current scope. The former build
> plan is now historical implementation chronology.
...
>
> **BUILD UPDATE (2026-06-11, refreshed 2026-07-26):** the D26 reviewer dashboard shipped as `/workbench` (tier 2) + `/workbench/[requestId]` (tier 3). The Reviewers tab shipped with five sub-tabs and later collapsed to three (Find · Invite Reviewers · Track Reviewers, S280). Proposal, Overview, Status, Reviews, and Awardee/grantee deliverables also shipped; four lifecycle tabs remain placeholders. This document preserves the original design rationale, including superseded assumptions. Use `docs/audits/AUDIT_REQUEST_WORKBENCH_TRUTH_2026-07-26.md` for current evidence and `docs/REQUEST_WORKBENCH_NEAR_TERM_EXECUTION_PLAN.md` for forward scope. The former build plan is historical implementation chronology.
>
...
- **Grant Program** selector (owner direction 2026-09-08): load options live, default to the signed-in PD's program, and allow the user to select other programs. The hard-coded Medical Research + Science and Engineering Research scope on `codex/compact-controls` is a temporary implementation that must be reconciled before release. The live option source, non-PD fallback, and program-change refresh contract remain to be verified in design; do not infer a permanent allowlist from that branch.
...
**Access:** both the Tools menu and the per-request Workbench tabs filter by the **same per-user app-access grants** the app uses today (Dataverse `wmkf_appuserappaccesses`, admin-managed in `/admin`; `Layout.js` → `hasAccess()`). **A Workbench tab can map to more than one existing grant** — the **Reviewers** tab consolidated *both* `reviewer-finder` (Find) and `review-manager` (Invite/Track/Completed). **Decided (S206): collapse the two into a single new `reviewers` grant** — one tab = one grant. The separate `reviewer-finder` / `review-manager` grants were only an artifact of the two apps being built at different times; since the Workbench merges the apps, the access merges too. **Status (S261):** local probe found no legacy-only users; the standalone pages and appRegistry keys were retired, while API routes intentionally keep accepting the legacy grant strings variadically with `reviewers` as deferred cleanup. (The mockup ignores access entirely and shows everything, for legibility.)

### 3.3 The Reviewers tab — locked structure
**Four sub-tabs, with status badges on the bar** (decided S206, after comparing a 3-tab "Roster" variant). *(Built with a 5th sub-tab — a **Candidates** saved-roster tab between Find and Invite, added S211 — then **collapsed to 3 LIVE sub-tabs: Find · Invite Reviewers · Track Reviewers (S280)**; "Candidates"→"Invite Reviewers", "Invite"/"Completed" folded into "Track Reviewers". The four below remain the S206 design funnel/rationale, not the current structure.)*
...
**In scope, now (D26):** the reviewer dashboard (request queue + Workbench Reviewers tab, designed 4-tab / built 5-tab / now 3-tab — see §3.3) + Reviewer Pool + the allowlist pre-population. This is what Phase II peer review and honoraria need by mid-June. *(Status: the dashboard + Reviewers tab SHIPPED; Reviewer Pool is still pending — see the build-plan doc.)*
