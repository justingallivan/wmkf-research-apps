...
**Access:** both the Tools menu and the per-request Workbench tabs filter by the **same per-user app-access grants** the app uses today (Dataverse `wmkf_appuserappaccesses`, admin-managed in `/admin`; `Layout.js` → `hasAccess()`). **A Workbench tab can map to more than one existing grant** — the **Reviewers** tab consolidates *both* `reviewer-finder` (Find) and `review-manager` (Invite/Track/Completed). So tab-level gating is an **open design choice**: show the tab on *any* contributing grant and gate its sub-tabs by their specific grant, **or** collapse the two into a single new `reviewers` grant — not a one-grant rule. (The mockup ignores access entirely and shows everything, for legibility.)
