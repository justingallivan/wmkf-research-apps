...
When materials become available, the reviewer is notified via a fresh email containing a NEW secure link. **The link is NOT stable across the journey** (corrected 2026-08-06 to match the code): preview rendering is read-only, then the send path (`lib/services/review-manager/send-emails-service.js`) re-mints the JWT, overwrites the stored hash, and substitutes only the JWT path segment immediately before dispatch for every email that carries `{{externalLink}}`. Any earlier link (the original invitation, a prior reminder) therefore **stops verifying** once a newer email is dispatched. The backend still resolves the same engagement (the `wmkf_appreviewersuggestion` row) regardless of which current token is presented, and flips the page from Stage 2a to Stage 2b state — but reviewers must use the link in the **most recent** email. (Any future calendar-invite feature must therefore embed the link from whichever email is current, not a one-time URL.)
