...
**Built S304** exactly as specced below, with these concretions: optimistic-lock token = `questionSetVersion` (reused, not a new etag); diff/changeset logic extracted to the pure, unit-tested `lib/admin/review-question-save.js`; updates/deletes go by **row-id URL** (the editor round-trips ids) so no alt-key escaping; reorder = native HTML5 drag (owner choice). Audit table = `lib/db/migrations/022_review_question_audit.sql` (pending→final, hard-abort). Tests: `tests/unit/review-question-save.test.js` (14), `tests/integration/admin-review-questions-route.test.js` (7), `tests/unit/review-questions-section.test.js` (4, RTL incl. drag-reorder). **Known gap (flagged for review):** no server guard against soft-deleting the four parent-bound rows (affiliation/impact/risk/overallRating) before Phase E retires the parent columns — see the atlas gotcha. Migration 022 is committed + in the manifest but **not yet applied to prod Postgres** (`node scripts/apply-migrations.js` is the gated apply step).
