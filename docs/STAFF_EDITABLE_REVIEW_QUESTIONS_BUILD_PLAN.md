...
- **Route:** `pages/api/admin/review-questions.js`, `requireSuperuser`.
  ...
  - Validation: unique keys, key-format (`^[a-z][a-z0-9_]*$`), key-immutability (an existing row's key can't change — it's the snapshot join), picklist option shape, length caps, ≥1 question, exactly-the-types-we-support.
