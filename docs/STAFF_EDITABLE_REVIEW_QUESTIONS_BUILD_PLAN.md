...
**✅ All readers below were migrated to the snapshot in Phase D (S305); Phase E1 (S305) stopped the writes.** This table is the historical pre-migration map. No code reads OR writes the rating VALUES anymore — the 3 columns remain in the Dataverse schema (dormant) only until E2 drops them.
...
- **6d-E2. Drop the parent columns. ← next, destructive + gated.** After E1 bakes in prod (verify no parent-rating PATCHes, real review still works): retire the 3 attrs from schema-as-code (`lib/dataverse/schema/wave2-existing/wmkf_appreviewersuggestion-extensions.json`) FIRST so the create-only applier can't resurrect them (Codex E2 P0-1), then drop via a dedicated metadata-delete script (no such tooling exists yet). Never roll back past E1 (P0-2). The Postgres engagement/drop predicate is moot — migration 018 already dropped the `reviewer_suggestions` table (2026-06-04); the predicate docs are stale, not a live blocker (P2-2).
...
- **Phase E — retire parent columns (§6d). E1 ✅ DONE (S305); E2 next.** **E1** stopped the dual-write (all 3 writers; backstop re-anchored on `CORE_RATING_KEYS`; equality assert dropped; admin guard decoupled; backfill frozen) — Codex-reviewed, full suite green. **E2** (drop the 3 columns) is the separate destructive/gated step: retire from schema-as-code first (create-only applier would otherwise resurrect them — Codex P0-1), dedicated metadata-delete script, after E1 bakes. The Postgres engagement/drop predicate referenced in `docs/atlas/postgres-reviewer-suggestions.md` / `docs/REVIEWER_POSTGRES_TO_DATAVERSE_PLAN.md` is **moot** — migration 018 already dropped the `reviewer_suggestions` table (2026-06-04); those predicate notes are stale doc-debt, not a live requirement.
