...
[RECHECKED after `lib/services/review-upload.js` / `lib/services/review-manager/mark-received-no-file-service.js` changed, 2026-07-26: the Phase-B/Phase-D decisions recorded above are unaffected and remain accurate as history. Those two write paths now resolve the question set through `getAuthoritativeQuestionSet()` instead of `getActiveQuestionSet()` — an uncached resolve at the write boundary, per `docs/REVIEW_FORM_MULTISELECT_BUILD_PLAN.md` §0.2. Which SET they receive is unchanged; only its freshness guarantee is. VERIFIED via `lib/services/review-upload.js:134` and `lib/services/review-manager/mark-received-no-file-service.js:80`.]
