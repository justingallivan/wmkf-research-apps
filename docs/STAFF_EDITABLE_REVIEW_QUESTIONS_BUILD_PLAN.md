...
**✅ All readers below were migrated to the snapshot in Phase D (S305)** — this table is the historical pre-migration map. The columns are still dual-written until Phase E, but no reader sources rating VALUES from them anymore (only `build-review-submission.js` writes them + asserts parent/child equality).
...
- **6a. Dual-write (no reader breaks). ✅** Submit writes ALL ratings to the snapshot (already does) **and** continues to PATCH the canonical-3 parent columns *when those three keys are present in the set*. Still in force through Phase D (the safety net during the reader migration); Phase E stops it. New (4th+) rating questions are snapshot-only from day one.
...
- **6d. Stop writing / drop the parent columns. ← Phase E (next).** 6a–6b + 6b-legacy are complete (all writers emit snapshot rows; all readers read the snapshot). Remaining: stop the `build-review-submission.js` dual-write + relax the parent/child equality assert, then optionally drop the columns (destructive Dataverse schema work — separate, explicitly-gated; keep the columns unwritten if in doubt).
...
- **Phase E — retire parent columns (§6d).** External gate resolved (§7); Phase D complete (all readers AND writers on the snapshot). Remaining: stop the dual-write in `build-review-submission.js` (relax the parent/child equality assert), then optionally drop the 3 columns. NB the Postgres drain-table drop predicate + the legacy "engaged" predicate (`docs/atlas/postgres-reviewer-suggestions.md`, `docs/REVIEWER_POSTGRES_TO_DATAVERSE_PLAN.md`) still read the rating columns as engagement signals — switch them to `wmkf_reviewreceivedat` (as the merge predicate already did in D) before dropping.
...
- **P1-3 — parent-bound rows removable before Phase E.** Soft-deleting affiliation/impact/risk/overallRating silently stopped its `reviewParentColumnByKey` dual-write. Fixed: the route rejects (400) any set missing one of the four (`missingParentBoundKeys`, derived from the binding map) until Phase E.
