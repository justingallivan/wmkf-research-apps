...
| `codex-verbatim-reminder.js` | **Mildly harmful as wired** — matcher is `Task\|Agent`, so it fires on *every* subagent call, not just Codex; this session it fired on a plain inventory agent, which then appended an irrelevant compliance disclaimer to its report. The instruction itself encodes owner feedback (`feedback-share-codex-verbatim`) and must survive — narrow the matcher or move the check into the hook's own script logic | [OBSERVED] |
...
- **Redundant blocks (trim, not delete):** generic git narration a competent agent does unprompted — start L21-35 (fetch/pull/status), stop L15-26 (log/status/diff + commit flow), parallel-agent-worktree L49-65/99-125/140-154 (worktree command sequences) [VERIFIED scan].
...
4. **`codex-verbatim-reminder.js` as currently scoped** — not a deletion but a re-scope: narrow to Codex-flavored delegations. Cutting it outright touches an owner-feedback contract — needs owner sign-off.
...
1. **`codex-verbatim-reminder.js`:** the underlying instruction exists because relayed Codex findings were previously paraphrased into wrongness (S221 lineage, `feedback-share-codex-verbatim`). The mis-scope is provable; the fix direction (narrow vs remove) is an owner call because the memory expresses durable owner feedback. If narrowed, verify the matcher still catches the `codex:codex-rescue` agent type and Codex-flavored `general-purpose` prompts — the S322 scan noted the "looksReview"-style scoping used by the delegation-trace guard as a working pattern to copy.
