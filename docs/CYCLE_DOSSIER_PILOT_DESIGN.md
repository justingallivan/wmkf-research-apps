...
The five source-built Postgres tables are documented in docs/atlas/postgres-cycle-dossiers.md. JSONB stores bounded previews, immutable entry revisions, one global run lease with independently checkpointed items, and private edition metadata. The dedicated private Blob store owns bytes and accepts only create-only exact pathnames with SHA-256/size verification. Retries create a new run budget while preserving prior charges.

## Branch-local implementation status

**[VERIFIED 2026-09-07 via source and focused tests]** The generation/document modules, guarded historical prompt support, prompt seed definitions, prompt seeder, five-table migration, setup-database fresh-install shape, API routes, worker/service/store, and UI are source-built on branch codex/cycle-dossier-pilot-build. Focused generation/document/Executor tests and targeted lint pass in the local checkout. The earlier design decisions remain preserved on codex/cycle-dossier-pilot-design.

**[VERIFIED 2026-09-07 via seed dry-run]** Both prompt families currently have zero Dataverse rows; the governed bootstrap would create version 1. The dry-run completed without prompt publication or LLM calls.
...
**[VERIFIED 2026-09-07 via GitHub/Vercel and protected HTTP probes]** The owner-approved feature branch is pushed with draft [PR #179](https://github.com/justingallivan/wmkf-research-apps/pull/179). Commit `8aa90f08` has a successful Vercel status and READY preview deployment `dpl_8d46FbxyzyywRWVUma9qbcfa6Ffn` at [the dossier preview](https://wmkfresearchapps-4ac5x5035-justin-gallivans-projects.vercel.app/cycle-dossier). Signed-out page and API requests redirect to application sign-in. The Preview cron returns HTTP 500 with `Cron secret not configured`, before worker execution; the intended worker environment needs this configuration before integration testing. These probes establish deployment and access guards, not live generation.

**Not claimed live:** no live Postgres migration 038, private DOSSIER_BLOB_READ_WRITE_TOKEN provisioning, CYCLE_DOSSIER_ENABLED=true activation, Cycle Dossier prompt publication, paid generation, unattended cron processing, or production promotion has been performed or verified by this branch work. Existing populated databases must use the governed migration process; do not run fresh-install setup against them.

## Controlled rollout remaining

1. Provision a dedicated private Blob store and configure DOSSIER_BLOB_READ_WRITE_TOKEN for the intended environment; keep CYCLE_DOSSIER_ENABLED disabled during setup.
2. Review pending migrations and apply them with scripts/apply-migrations.js against the approved existing database. Publish the two prompts with the governed seeder after reviewing its dry-run, using the extensionless loader shown in the script's usage.
