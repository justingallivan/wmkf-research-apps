...
**Not claimed live:** no live Postgres migration 038, private DOSSIER_BLOB_RW_TOKEN provisioning, CYCLE_DOSSIER_ENABLED=true activation, Cycle Dossier prompt publication, paid generation, unattended cron processing, or production promotion has been performed or verified by this branch work. Existing populated databases must use the governed migration process; do not run fresh-install setup against them.

## Controlled rollout remaining

1. Provision a dedicated private Blob store and configure DOSSIER_BLOB_RW_TOKEN for the intended environment; keep CYCLE_DOSSIER_ENABLED disabled during setup.
