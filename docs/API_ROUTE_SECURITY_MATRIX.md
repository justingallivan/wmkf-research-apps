...
| `/api/reviewer-finder/generate-emails` | POST | App | `requireAppAccess('reviewer-finder')` | `user_profile_id` scoped | Writes/updates `wmkf_appreviewersuggestion` (DV) via `reviewer-suggestion` adapter (W5 cutover 2026-05-12); `api_usage_log` (PG) | Low | Existing cross-user tests cover representative behavior. |
| `/api/reviewer-finder/grant-cycles` | GET, POST, PATCH, DELETE | App | `requireAppAccess('reviewer-finder')` | Shared grant cycles | Reads/writes Dataverse `wmkf_appgrantcycle` via `lib/services/grant-cycles-dataverse` (W3 cutover 2026-05-12); admin UI for cycle management | Medium | Shared mutable records; confirm app-level access is enough. |
...
| `/api/reviewer-finder/my-proposals` | GET | App | `requireAppAccess('reviewer-finder')` | Caller Azure email -> PD | Reads Dataverse (akoya_request + wmkf_appreviewersuggestion) | Low | Good identity-derived scope pattern. Postgres reader retired in W5 step 4 (2026-05-12). |
