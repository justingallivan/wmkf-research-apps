...
| `/api/cron/drain-submissions` | GET, POST | Cron | Inline strict `CRON_SECRET` (no dev bypass — Codex round-12 Q5) | Up to `DRAIN_BATCH_SIZE` non-terminal jobs (claim-leased) | Claims + advances rows in `submission_jobs` (PG); writes `intake_audit` (PG); writes `system_alerts` (PG) on corruption/anomaly/build-pending; writes Dataverse `akoya_requests` (Create with pre-gen GUID, `Prefer: return=representation`); writes SharePoint files to `akoya_request/{requestnum}_{guidUpper}/Applicant_Uploads/` via Graph; reads private Blob (`INTAKE_BLOB_RW_TOKEN`) | Medium | Applicant intake portal drain. Two-phase claim (`FOR UPDATE SKIP LOCKED` + `lease_token`), transactional state-transition+audit, classifier-driven retry taxonomy. Wired: `queued → scanning → request_created → files_moved` with duplicate-PK recovery (read-back via GET) + per-file idempotency via `sharepoint_paths` JSONB skip + post-download sha256 verification. BUILD-PENDING: `dynamics_patched`/`status_flipped`/`completed` (park next_attempt_at +1h + alert). Schedule every 2 min (≪ `DRAIN_LOCK_TTL_SECONDS=600s`). |
