...
| Workbench version-history display | `View version history` disclosure on the Initial Assessment tab, lazy-fetched per staff click | Read-only. Lists native SharePoint versions for the exact displayed artifact via Graph; a replacement race returns 409 rather than another artifact's editors; no Dataverse write and no snapshot row | Workbench shows editor attribution per version, current-first, with honest truncation reporting | BUILT AND DEPLOYED S413 (2026-08-10), merge `147d3e49` — unit-verified only. All three test files covering this path stub their outbound boundary — `global.fetch` [VERIFIED via `tests/unit/graph-service-versions.test.js:45`], the service [VERIFIED via `tests/unit/workbench-initial-assessment-versions-route.test.js:11`], the adapters [VERIFIED via `tests/unit/initial-assessment-artifact-versions.test.js:5`] — and the 2026-08-10 probe called Graph directly rather than through the route, so the route→service→Graph chain has NOT executed end to end. Signed-in smoke on Request `1003109` still outstanding; until it runs this is built, not verified |
