...
- **Wave 2 entity list.** This doc mentions `wmkf_app_proposal_search` and `wmkf_app_publication_author` as Wave 2 deliverables. Live state (S188 audit re-sweep 2026-05-25): `wmkf_appproposalsearch` IS deployed — entity-set name uses the unconventional plural `wmkf_appproposalsearchs` (no `e` before `s`), currently 0 rows. `wmkf_app_z_publication_author` is still **not deployed** (decision per `docs/REVIEWER_POSTGRES_TO_DATAVERSE_PLAN.md:113` — zero data on both sides + consuming `researchers.js` admin UI was retired).
