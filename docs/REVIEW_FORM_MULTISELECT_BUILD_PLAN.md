...
`347a37e820f73890`. The first controlled Request #1002788 production smoke then
passed the external context/draft/submit path, canonical 11-row answer snapshot,
workbench DTO/matrix, panel DOCX/PDF, courtesy DOCX, finality locks, and atomic
cleanup. It did **not** pass synthesis: two independent
`review-synthesis.generate` v2 executions returned incomplete JSON
(`Unexpected end of JSON input`) and created failed append-only AI runs
...
5. On a dedicated internal test suggestion, complete an end-to-end review through
   ...
   the new prompt. The Request #1002788 smoke completed the external-authoring,
...
7. Run a final controlled smoke against the republished target, then remove/reset
   ...
   Attach the evidence to the release record. The failed primary smoke's 11
   answer rows and parent finality fields were already reset in one verified
   12-operation changeset; its two failed AI audit rows remain append-only.
   The post-republish final smoke is still pending. [PRIMARY CLEANUP COMPLETED
   2026-07-26; FINAL SMOKE PLANNED]
...
3. From independently routed requests, verify the same new
   ...
   the exact target version/set. The controlled smoke then verified the external
   context, exposed exactly `Proposal_1002788.pdf`, cleared the prior draft on
   token regeneration, and proved draft sanitize/reload/finality behavior against
   the same version. [COMPLETED 2026-07-26]
4. Run a controlled production smoke using a dedicated internal test suggestion,
   ...
   acceptance surface in §7. Request #1002788 passed the portal submit and all
   deterministic consumers, including categorical storage/tallies and generated
   artifacts, with no email. Both synthesis attempts failed before writeback with
