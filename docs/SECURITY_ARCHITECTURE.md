...
| `RequireAppAccess` | Page-level wrapper on all [16](CANONICAL_COUNTS.md#app-definition-count) app pages; shows "Access Not Available" if denied. |
...
| Attribute | Detail |
|-----------|--------|
| **Registry** | `shared/config/appRegistry.js` — single source of truth for all [16](CANONICAL_COUNTS.md#app-definition-count) app definitions (keys, names, routes, icons, categories) |
...
**Finding:** `api_usage_log` grows with every Claude API call across all apps ([16](CANONICAL_COUNTS.md#app-definition-count) in current registry) and has no archival or cleanup mechanism.
