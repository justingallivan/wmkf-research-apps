...
| `excerpt` | Verbatim snippet, at most two lines, enough to find the site again after line drift. **2026-09-08 amendment:** a fragment may be BOUND to one of the `site` cell's cited paths/globs by prefixing it: `` `path/or/glob` → `fragment` `` (arrow is `→` or `->`). Unprefixed fragments remain a shared bag. A row with several cited sites should bind a fragment per site once reconciled (see `docs/plans/J27_REGISTER_PER_SITE_RECONCILIATION_PLAN_2026-09-08.md`). **STRICT UNBOUND (owner decision 2026-09-08, "we don't need drift"):** in a multi-site row, an unbound file is itself a FAILURE — the shared bag is never consulted for it, even if a bag fragment happens to appear in it. Only a single-file row still falls back to the bag. |
...
- **Marker convention.** Any new code comment, doc callout, or memory line that records a J27-sensitive fact carries the tag `J27:` followed by the register ID once one exists (`// J27: J27-014 D26-only hide, retire after D26 close`). Untagged mentions found later are sweep misses, not the author's fault, and get added. **Refined 2026-09-08:** an id-less `J27:` counts as an unregistered marker only when it sits in a comment — the line, after leading whitespace, begins with a code-comment leader (`//`, `/*`, `*`, `#`, `--`, `<!--`) with `J27:` following it (optionally after whitespace). A bare `J27:` in prose, inside a bold label (`**J27:**`), or mid-sentence is not a marker; it is the term used in ordinary writing, not a site tag.
- **Check script.** [RECHECKED after scripts/check-j27-register.js change: §3/§7 rewritten 2026-09-08 for binding syntax, STRICT UNBOUND, comment-led markers, row-detection; verified against the script header at 440143d8] `scripts/check-j27-register.js` lists every `J27:` tagged site in the tracked tree and fails when a tag names a register ID that does not exist, or when a register row's `site` no longer resolves to a file that contains its excerpt. Advisory at first, promoted to a blocking gate only if the owner wants it. Register it in `docs/CI_GATES_REFERENCE.md` and the `/start` gate list when it lands; a gate and its self-test run sequentially like every other gate. **Refined 2026-09-08:** excerpt resolution is per site, not per row — a multi-site row (several backticked files, or a glob with several matches) is stale unless EVERY resolved file/match contains at least one excerpt fragment; a row is no longer "ok" merely because some one of its several cited files still matches.
- **Site-to-fragment binding, Phase 0 of `docs/plans/J27_REGISTER_PER_SITE_RECONCILIATION_PLAN_2026-09-08.md` (2026-09-08):** per-site resolution alone still let a reconciled row pass on a coincidental match (a fragment could appear in the wrong cited file and the gate would not know). The `excerpt` cell now supports binding a fragment to the exact path/glob it belongs to (§3 amendment, `` `path` → `fragment` ``); a bound file must match one of ITS bound fragments and never falls back to the shared bag, even if a bag fragment happens to appear in it. **STRICT UNBOUND (owner decision 2026-09-08, "we don't need drift"):** a file with no binding in a multi-site row (more than one resolved file) is itself a FAILURE — the bag is not consulted for it at all, and the row is STALE (`unbound: <files>`) even when the bag would have matched. Only a single-file row still falls back to the bag. The `N multi-site rows with unbound files` summary line stays as an info count of that (now-stale) condition. A directory site no longer passes on existence alone; it must cite a specific file or the row is stale. The disposition vocabulary (`open` · `scheduled` · `done` · `rejected`, §3) is enforced the same way: any other leading word (e.g. a bare `closed.`) is reported, not silently treated as closed.
...
3. Should the Initial assessments view remain hidden for D26 in Production until D26 closes, or be shown read-only so PDs can preview the J27 workflow? (Answered 2026-09-08 hidden; unhidden and re-hidden 2026-09-09 — the working view the owner needed was a new Staff deliberations list of Pre-Site drafts — register Q3.)
...
- 2026-09-07: §7 marker convention and `scripts/check-j27-register.js` built as an advisory gate with a `--root`-isolated self-test; registered in `docs/CI_GATES_REFERENCE.md` and `/start`. That exit criterion is met; the memory/wiki pointer criterion and the §8 answers remain open.
- 2026-09-07 (plan closeout, branch `claude/j27-plan-closeout`): the memory/wiki pointer exit criterion is now met — `project-grant-phasing-evolution.md` and `docs/agent-wiki/topics/strategy-roadmap.md` already pointed at `docs/J27_TRANSITION_REGISTER.md`; `project-j27-doc-capture-evolution.md` now does too. All three `J27:` markers found without a register id (two in the prior baseline, plus a third discovered this session in `.claude/skills/start/SKILL.md`) are resolved; `npm run check:j27-register` reports 0 unregistered markers. Q18's "copy item" (annotating the pilot passages that couple the J27 Initial Assessment to 2026-08-18, register J27-051) is done for three of the four cited sites; the fourth (`project-reviewer-apps-redesign-direction.md` L82) was outside this session's assigned scope and remains unannotated. §8 owner questions beyond Q0 remain open and were not attempted; this plan's exit criteria are therefore still not fully met.
- 2026-09-08 (gate tightening and per-site reconciliation, branch `claude/j27-gate-tighten`): the check script's two documented weaknesses (prose-colon false positives on id-less `J27:` markers; any-of-N rather than per-site excerpt matching) were fixed at the rule level, which surfaced 43 (then 47, after the owner's STRICT UNBOUND decision) stale rows against the tightened gate. A site-to-fragment binding schema (`` `path` → `fragment` ``) was added to the register format so a reconciled row cannot pass on a coincidental match; four parallel slice agents reconciled every stale row to the binding syntax, followed by two exhaustive Opus matrix reviews whose CONDITIONS were applied on top. Before this reconciliation effort: 12 ok / 47 stale / 6 unverifiable / 11 closed, 47 multi-site rows with unbound files. After: 59 ok / 0 stale / 6 unverifiable / 11 closed, 0 unbound rows, 0 disposition-vocabulary warnings; self-test fully green (72/72). A subsequent Codex adversarial review (`65eb4bfe`) found a row-detection parsing gap (fixed, now whitespace-tolerant and fails closed on a malformed id) and that J27-023 was bound to a fragment that is not its fact (fixed by unbinding it, leaving the row honestly STALE pending an owner ruling). The owner ruled 2026-09-08 to drop that citation (not a J27 site). Final: 59 ok / 0 stale / 6 unverifiable / 11 closed, self-test 80/80 with `KNOWN_OWNER_PENDING` empty. See `docs/J27_TRANSITION_REGISTER.md` §9-10 for the full gate-fix and reconciliation record.
