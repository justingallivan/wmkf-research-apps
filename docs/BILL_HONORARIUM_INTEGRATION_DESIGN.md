...
- **Reviewer address + phone are REQUIRED and SERVER-ENFORCED** on a non-opted-out accept (`422 payment_contact_required`, via `missingRequiredAddressFields` in `respond.js`; state/province is required for US/Canada addresses and optional elsewhere) — this **supersedes** the "server treats address as optional / client is the primary gate" framing in the failure-modes table and the "provisional" (S200) notes below. Ground truth in memory: `project-bill-honorarium-integration`, `project-reviewer-address-collection-provisional`.
