...
- **Expiry** [ASSUMED default, owner may adjust]: site visit scheduled end + 7 days when an active visit exists; else `wmkf_meetingdate` + 7 days when set; else mint + 60 days. Expiry is fixed in the JWT; extending it means reissuing.
- **Mint** (`ensureLiveBriefingLink`): returns the live row when one exists and is not expired; otherwise mints, revoking any expired live row first. Runs inside `preparePreSiteDistribution` after the source resolves, so the link appears in the exact preview; its id joins `previewHash` (which today covers attachments, links, calendar, recipients, subject, and body [VERIFIED via lib/services/pre-site-visit/distribution-service.js:1140-1178]) and is stored on the attempt (`pre_site_distribution_attempts.briefing_link_id`, migration 038 [PLANNED]). Send re-checks that the same link is still live; a revoke between preview and send fails the send with `distribution_briefing_stale` and asks for a new preview, matching the existing stale-source and stale-material guards [VERIFIED via lib/services/pre-site-visit/distribution-service.js:1259-1310].
- **Reissue** (`reissueBriefingLink`): a single transaction marks the live row revoked and inserts the replacement, recording `superseded_by`. The tab shows the new link. Old emails now point at a dead link; the PD resends.
...
- Raw token never logged. The ciphertext column is decrypted only inside the staff route and the distribution prepare path.
