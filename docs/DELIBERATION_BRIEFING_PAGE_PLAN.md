...
| Consultant feedback | Postgres `consultant_feedback` rows for the request with `status = 'active' AND shared = true` (`loadSharedConsultantFeedbackForBriefing` in `lib/services/consultant-feedback-service.js` [RECHECKED after lib/services/consultant-feedback-service.js change: lib/services/consultant-feedback-service.js:321]), consultant name/affiliation live-joined from `expertise_roster` (not filtered on `is_active`) or the entry's own one-off name/affiliation. Context carries `consultantFeedback: { status: 'ok' or 'unavailable', items }`; a read failure on this table alone degrades to `unavailable` with a visible notice rather than omitting the section or failing the whole context. Zero shared items with `status: 'ok'` omits the section. No attachment/`document` member in slice 1 (text-only). [SOURCE-BUILT 2026-09-14, Consultant Feedback slice 1, `docs/plans/CONSULTANT_FEEDBACK_PLAN_2026-09-14.md`] | live |
