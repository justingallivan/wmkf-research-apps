...
| D17 (revised 2026-09-15) | Every newly minted or deliberately reissued briefing link expires exactly 60 days after issuance. Existing live links retain their embedded expiry until they expire or staff deliberately reissue them; there is no silent revocation or extension. Site-visit and meeting dates do not affect link expiry. |
...
Carried from the Codex Site Visit Materials plan §2 and §9, unchanged: one shared link per proposal, read-only, no copies of files, exact pinned writeup version, external labels are institution-led and carry no request number, any member id outside the model's own set fails closed, no folder listing.
...
| Research Presentation line | active `wmkf_sitevisit` for the request via `findActiveByRequest` [VERIFIED via lib/dataverse/adapters/site-visit.js:46-58], reduced to one row by `selectActiveSiteVisit` (earliest scheduled end wins, ties on activity id) [ASSUMED rule, owner may adjust] so display never depends on Dataverse's ordering | live |
...
| Research presentation materials | `wmkf_requestdocument` rows for the request of artifact type Applicant Slides, Other Applicant Materials, Recording, Transcript, or Transcript Summary that are Ready, not Superseded, carry SharePoint drive/item ids, and are not distribution snapshots (`loadMaterialRows` in `briefing-page-service.js`; the staff brief row is excluded on purpose, it is served from the ledger above; unchanged by the Consultant Feedback slice 2 `feedback:` member below — the `material:` artifact-type filter is not widened). Member `material:<requestdocumentid>`, membership re-proved on every download. Files over `MAX_MATERIAL_BYTES` (50 MB) list with size and no link. [BUILT 2026-09-10, D19] | live |
| Consultant feedback | Postgres `consultant_feedback` rows for the request with `status = 'active' AND shared = true` (`loadSharedConsultantFeedbackForBriefing` in `lib/services/consultant-feedback-service.js` [RECHECKED after lib/services/consultant-feedback-service.js change: lib/services/consultant-feedback-service.js:580, slice 2 adds `attachment.member`/`inline` to each item]), consultant name/affiliation live-joined from `expertise_roster` (not filtered on `is_active`) or the entry's own one-off name/affiliation. Context carries `consultantFeedback: { status: 'ok' or 'unavailable', items }`; a read failure on this table alone degrades to `unavailable` with a visible notice rather than omitting the section or failing the whole context. Zero shared items with `status: 'ok'` omits the section. Slice 2: each item's `attachment` is `{ member: 'feedback:<requestdocumentid>', filename, inline }` when the row carries a bound attachment that is Ready and not Superseded, else `null`; PDF (`inline: true`) opens in a new tab, everything else downloads (D23/D28, same rule as materials). [SOURCE-BUILT 2026-09-14, Consultant Feedback slices 1-2, `docs/plans/CONSULTANT_FEEDBACK_PLAN_2026-09-14.md`] | live |
...
- **Expiry** [OWNER REVISED 2026-09-15, D17]: every newly minted or deliberately reissued link expires exactly 60 days after issuance, independent of the site-visit and meeting dates. The same timestamp is fixed in the JWT and persisted row. `ensureLiveBriefingLink` continues to return an already-issued readable live link without changing its embedded expiry; extending or shortening that link requires deliberate reissue, which revokes the old link immediately.
- **Mint** (`ensureLiveBriefingLink`): returns the live row when one exists and is not expired; otherwise mints, revoking any expired live row first. Runs inside `preparePreSiteDistribution` after the source resolves, so the link appears in the exact preview; its id joins `previewHash`, which covers attachments, links, calendar, recipients, subject, staff-edited body, and resolved Admin briefing copy, and is stored on the attempt (`pre_site_distribution_attempts.briefing_link_id`, migration 038 [PLANNED]). The stored email body carries a placeholder, never the token; the live URL is rendered into the Dynamics activity at creation (and re-rendered for recovery matching), so `pre_site_distribution_attempts.body_html` never holds a usable bearer. Once the flag is on, an unsent preview prepared before it (no bound link) is refused with `distribution_briefing_stale` so the PD re-prepares and the email carries the link; a preview whose send was already requested only reconciles. Send re-checks that the same link is still live at claim time and again in the final rechecks, which run before send intent is stamped so a pre-transport failure leaves the attempt provably unsent; a retry of a send-requested attempt reconciles Dynamics transport status before any liveness check; a revoke in between fails the send with `distribution_briefing_stale` and asks for a new preview, matching the existing stale-source and stale-material guards [VERIFIED via lib/services/pre-site-visit/distribution-service.js].
...
- **Feature gate**: `DELIBERATION_BRIEFING_SCHEMA_READY === 'on'`, owner-set after applying migration 038, the same convention as `SITE_VISIT_LOGISTICS_SCHEMA_READY` [VERIFIED via lib/utils/site-visit-logistics-readiness.js:10-12]. When off, the staff route returns 503 and the external routes return 404. Since 2026-09-10 (S503, the email carries no attachment) a Share preview also refuses to prepare with `distribution_briefing_required` (503) while the flag is off, so unsetting the flag pauses deliberation sends rather than reverting them to attachments. The migration is additive.
...
| `/api/external/briefing/[token]/document` | GET | External token | `?member=` one of `writeup-docx`, `proposal`, `review:<suggestionId>`, `material:<requestdocumentid>`, or `feedback:<requestdocumentid>` (Consultant Feedback slice 2) (D19, D22, D23). The retired `writeup-pdf` member returns 404 before any Graph call. The server resolves every member against the request's model — for `feedback:`, the registry row must belong to the token's request, be referenced by a `shared = true AND status = 'active'` `consultant_feedback` row for that request, be Ready, and not Superseded; anything else is 404. Streams bytes with `nosniff`, `private, no-store`, and a bounded `Content-Disposition`. PDFs are inline; the DOCX staff brief and non-PDF materials/attachments are downloads. [RECHECKED after lib/services/deliberation-briefing/briefing-page-service.js change: lib/services/deliberation-briefing/briefing-page-service.js:378, the `feedback:` branch now also requires artifact type Consultant Feedback and non-empty drive/item ids before any Graph call] |
...
Inside `PreSiteDistributionPanel` (rendered by the tab once the writeup is shared [VERIFIED via shared/components/workbench/StaffDeliberationsTab.js:830-843]): the composer shows "Briefing page link — included in this email, expires {date}" once the preview has minted it. Its initial subject and message come from the Admin-editable `email.deliberation_share.subject` / `.body` settings in the Internal emails group beside `email.deliberation_agenda.*`; the subject supports `{{requestNumber}}`, and both fields remain editable per send. The same Share-for-deliberation Admin card owns four additional static-copy settings for the generated briefing section: heading, clickable link text, description, and expiration lead-in. The destination URL and formatted expiration date are derived from the preview's bound live link, cannot be edited in Admin, and are inserted only by the server. Prepare rereads and escapes the copy, stores the rendered section in the frozen attempt body, and includes the resolved copy in both preview hashes. Blank/unset or unavailable settings retain the former built-in wording and the composer visibly reports fallback state. After a send, the panel header shows the live link with Copy and "Issue new link" (a confirm dialog names the consequence: earlier emails stop working, resend required).
...
- Mint: a new link and a deliberate reissue both expire at issuance + 60 days; ensure twice → same row id and original expiry; a live link issued under an earlier policy is reused unchanged; reissue → old row revoked with `superseded_by`, new row live.
...
Rehearsed in production 2026-09-10 (S503) on test request 1003222: Share minted the link, the PDF-only send bound it (`briefing_link_id` on the ledger row, state `sent`), the emailed link opened in a private window with every material downloadable (page/context/document all 200), and Issue new link revoked the old row with `superseded_by` set in the same transaction; the old token then answered 401 from `context`. Watched read-only via `deliberation_briefing_links` and `vercel logs --query "external/briefing"`.
