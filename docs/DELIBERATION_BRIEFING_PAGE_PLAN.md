...
- **Reissue** (`reissueBriefingLink`): a single transaction locks the live row and every unsent distribution attempt bound to it, refuses with 409 `briefing_send_in_progress` when one of those attempts holds an unexpired send lease, otherwise marks the live row revoked and inserts the replacement, recording `superseded_by`. A send that claims its lease after that commit sees the link revoked and fails `distribution_briefing_stale`. The tab shows the new link. Old emails now point at a dead link; the PD resends. A live row whose sealed token can no longer be read (key rotation, damaged ciphertext) is reported as `unreadable` to the tab, which still offers "Issue new link"; Share's `ensure` replaces such a row itself.
