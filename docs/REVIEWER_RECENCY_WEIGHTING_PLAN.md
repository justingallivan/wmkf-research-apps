...
- `save-candidates` now persists the 0–100 `relevanceScore` (not the 0–1 `verificationConfidence`) to `wmkf_relevancescore` — recency rank reaches the stored Track-A score; scale-mix fixed (Justin S223). → Files.
