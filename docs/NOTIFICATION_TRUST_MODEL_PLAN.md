...
| 10 | `lib/bill/onboard-reviewer-service.js` — reaches the email branch via THREE distinct error/critical paths: the invalid-input guard (severity `error`) at line ~108, `billFailure()` (severity `error`/`critical` depending on error type) at ~481-503 (called from ~209, ~248, ~259), and `unhandled()` (severity `error`) at ~511-529. `notifyAlertOnly()` (`bill_manual_onboarding`) is a warning dashboard alert only; it no longer sets `emailAdmins: true` as of 2026-07-06 to avoid bulk manual-onboarding emails. | `[VERIFIED via :105-118 (invalid-input guard), :481-503 (billFailure), :511-529 (unhandled), :531-548 (notifyAlertOnly); :209,:248,:259 (billFailure call sites)]` | Callers of `onboardReviewer()`: `pages/api/bill/onboard-reviewer.js` (route) — **also independently REACHES, see #14** — and `lib/bill/honorarium-onboard-orchestrator.js` (already NEVER-REACHES itself, but calls `onboardReviewer` which can) | 2 (via `onboardReviewer`) |
