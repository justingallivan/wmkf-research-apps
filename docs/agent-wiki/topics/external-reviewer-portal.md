...
- **Legacy declined-row repair (Stage 3J, PR #166, 2026-09-06).** When a repeat decline finds a
...
- **Board-writeup identity is REQUIRED at accept (S308).** The Stage 2a accept form collects three
  ...
  (request 1002852, 2026-09-21) ONLY when the enrichment value still
  `looksLikeByline` (Codex round 4 finding, 2026-09-21: an echoed email, a
  geographic segment, or a department lead) — so a PubMed-enriched reviewer's
  raw byline in `wmkf_primaryaffiliation` seeds a clean institution name, not
  the whole byline, while a clean enrichment affiliation that merely contains
  its own commas ("Weill Cornell Medicine, Cornell University") seeds
  unchanged, since reducing it would drop a real co-affiliation
...
- **Materials/release email is portal-link-only by default (S327).** The materials
  ...
  `ReleaseMaterialsModal` (`shared/components/reviewers/ReleaseMaterialsModal.js`, extracted from the panel in Stage 6C) *additionally* offers the SharePoint
...
**Consultant feedback card (slices 1-2, 2026-09-14):** the context adds
...
otherwise lists shared entries. Slice 2: an item's `attachment` is
`{ member: 'feedback:<requestdocumentid>', filename, inline }` when it carries
a bound, Ready, not-Superseded attachment; the page links it through
`/api/external/briefing/[token]/document?member=feedback:<id>` (new member
kind, `resolveBriefingMember` in `briefing-page-service.js`), inline for PDF
and download otherwise, same as the materials section. See
