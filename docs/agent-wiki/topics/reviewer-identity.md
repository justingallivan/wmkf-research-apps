...
3. **Invitation address-action gate** — `send-emails.js` recomputes `emailConfidence` per recipient: `ready` sends without added review, `quick_check` requires that recipient in `confirmedLowConfidenceIds`, `research_only` is always refused, and `missing` has no sendable address. Scoped to `templateType==='invitation'`.
