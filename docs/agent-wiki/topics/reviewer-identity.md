...
- **ROR institution resolution integration (deployed 2026-08-08; authority unchanged).** `reviewer-identity-runtime.js` constructs one request-scoped ROR candidate/decision/OpenAlex-bridge resolver for W2 shadow/combined evaluation. The candidate adapter sends only affiliation text, returns no verdict, and treats provider rank/`chosen` as evidence; trusted country/domain evidence stays local and hard vetoes precede scoring. Only a unique local ROR decision may hydrate that exact ROR through OpenAlex. Provider/bridge failure abstains, aggregate metrics retain no raw affiliation/name, and legacy/default/unknown modes remain exact legacy with no ROR work. [VERIFIED via source, focused contract/runtime tests, and production deployment `dpl_8J167uKtsFi5ej5uS9pgmXTxLjKu`.] Production remains `legacy-default`. Promotion and query-level tuning are paused because the first comparison conflated reviewer relevance, person identity, and institution normalization; current sequencing is `docs/ROR_REVIEWER_FINDING_STRATEGIC_RESET_BRIEF.md`. **PR #116 (`codex/ror-api-production-shadow`, tip `a847b730`, 2026-08-07) was closed 2026-09-14 as superseded:** `444bd781` landed the same ROR candidate/decision core, OpenAlex bridge, and shadow/combined wiring on main under flat `lib/services/ror-institution-*.js` paths. The only remainder main lacks is the benchmark dedupe (the branch collapsed `benchmarks/fuzzy-matching-falsification/versions/v3/adapters-ror-api.js` and siblings into thin wrappers over the production core; main still carries full copies). Cleanup candidate only; no bridge build is pending. [VERIFIED S512 via `git diff origin/main...a847b730` and source.]
...
- Current strategic-reset handoff and implementation boundaries:
  `docs/ROR_REVIEWER_FINDING_STRATEGIC_RESET_BRIEF.md`.
...
**The reconciler RETRACTS its own alerts (S414, hardened after the Opus
review).** Any row reaching a non-alert outcome auto-resolves its prior alert
...
retraction branch. `3872d97c` moved gone/deselected checks ahead of contact
vetting. The follow-up implementation reads the open alert keys once: an
unvetted row with no standing keyed alert skips Dataverse, while an unvetted row
with an open alert is classified for gone/deselected/excluded lifecycle evidence.
...
An `ALREADY_RESOLVED` verdict means the *reconciler's* work item is stale; the
read-only probe does not clear the alert or establish that the duplicate
`wmkf_potentialreviewers` records were merged.
