...
- **Wave 13 first binding-writer caller (built 2026-07-13; promotion pending).** The owner-approved first caller is limited to acceptance-drain self-report with the durable job `accepted_at`. `capture-self-reported-orcid.js` sends clean/already-bound rows through `reviewer-identity-binding-writer.js`, uses the transitional person writes only for typed `legacy_classification_required`, and rethrows every other writer failure before contact fill. `reviewer-acceptance-drain.js` runs this before honorarium/back-propagation and lets failure retry the durable job; it marks the in-memory reviewer confirmed only after `{persisted:true}`. Decline/no-stable-timestamp calls remain on the transitional path. Automated writers, backfill, merge/action policy, and the four suggestion COI fields are unchanged.
