...
- **Reviewer ↔ CRM-contact boundary gap (S290 findings, NOT BUILT).** Origination/save does NO CRM-contact match (`save-candidates.js` keys on potentialreviewer email only), reviewer/PD corrections don't sync back to the CRM contact, and honorarium `ensureContact` find-or-creates by email ONLY — a corrected email that differs from the contact's spawns a DUPLICATE contact. Distinct from the shipped potentialreviewer↔potentialreviewer merge (which blocks `loser_has_contact`). Full trace + design stub: `docs/REVIEWER_CONTACT_BOUNDARY_GAP_FINDINGS.md`.
