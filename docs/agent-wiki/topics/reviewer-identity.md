...
3. **Invitation address-action gate** — `send-emails.js` recomputes `emailConfidence` per recipient: `ready` sends without added review, `quick_check` requires that recipient in `confirmedLowConfidenceIds`, `research_only` is always refused, and `missing` has no sendable address. Scoped to `templateType==='invitation'`. A `research_only` address is escapable only by changing its stored provenance — a different address via the contact editor (`manual`), or a staff attestation for the same address (`verifyEmailAddress` → `staff_verified` → `quick_check`, S387; see the enforcement contracts doc). The send gate itself is unchanged: it still refuses `research_only` outright and never trusts a client label.
