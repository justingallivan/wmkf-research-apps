...
The "X" on an Invite Reviewers panel (`ReviewerInvitePanel`) card is a **soft-delete**, not a UI-only dismiss: `DELETE /api/reviewer-finder/my-candidates` → `suggestionAdapter.softDelete(id, {alsoRevokeToken:true})` flips `wmkf_selected=false`, clears accepted/declined/responsetype/reviewstatus/heldat, and (if invited) sets `wmkf_externaltokenrevoked=true` — all in one atomic PATCH. It refuses (throws → DELETE route 500) any **closed engagement row**: a closed `wmkf_reviewstatus` OR a stamped `wmkf_completedat` (`isClosedEngagementRow`; the completedat half was added 2026-09-06 under owner decision D0 — before that a completion-stamped row with a non-closed status could be removed). The row is NOT hard-deleted; the person/contact is untouched. Because that reset also erases an ACCEPTANCE (and the withdrawal path then becomes unreachable — terminal-transition requires `wmkf_accepted`), both Remove confirms (`ReviewerInvitePanel.removeCandidate`, `ReviewerManagePanel.handleRemoveReviewer`) prepend a warning for accepted reviewers pointing to "Record reviewer withdrawal" (`shared/components/reviewers/remove-reviewer-confirm.js`, added 2026-09-02 after an accepted reviewer with materials sent was removed instead of withdrawn on Request 1002833). It is a warning, not a block. Removed candidates do **not** reappear in the Find tab (Find is ephemeral discovery + roster dedup; it never reads back persisted suggestion rows).
...
fresh authorization read, and the final shared sender; revoked, not-minted,
invalid, expired, insufficient-window, or missing-due-date state refuses
without a write or email. The review-due email is link-free and never mints or rotates token
...
`30 10 * * *`; the reviewer reminder cron that formerly ran at 10:00 UTC is paused) →
...
token regeneration is independently fail-closed to accepted/held nonterminal rows
with a concrete ETag; it does not reselect a removed row or undo
