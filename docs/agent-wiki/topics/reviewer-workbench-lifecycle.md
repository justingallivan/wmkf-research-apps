...
**Contact evidence and action policy (S267–S321; revised 2026-07-18):** `shared/components/reviewers/ContactLeads.js` renders quarantined `contactEnrichment.contactLeads` in `ReviewerSearchSection`'s `CandidateCard`, gated on `!identityUnverified` and deduped against the email + website chips. **`identityUnverified` no longer means "show nothing" (S388):** the verified TREATMENT is still suppressed on those rows — no mailto chip, no readiness verdict, no green "known in Dataverse" claim, no contact leads — but the underlying EVIDENCE is now offered in a collapsed "Review evidence before confirming" disclosure (affiliation + provenance label, Dataverse match and institution records, the address as plain text with its source, recent papers, a Scholar name search). Staff had to answer "is this the right person?" with only a publication COUNT visible, and the evidence appeared only AFTER "✓ This is the right person → edit & add" — which commits a durable request-scoped attestation, so it arrived after the decision. When editing this area, keep the treatment/evidence split: restoring the verified chips is the thing the original suppression was protecting against. **The paper list inside that disclosure is LOAD-BEARING — do not truncate, collapse, or drop it as clutter.** An S388 adversarial review (Codex `gpt-5.6-sol`) argued the disclosure invites circular confirmation: affiliation, address, and the Dataverse match all descend from the SAME retrieval, so they agree with one another whether or not the right person was retrieved, and a bundle of mutually-consistent facts reads as corroboration when it is one piece of evidence shown four ways. The owner's answer (accepted): his workflow is to scan the top line, then open the papers and check them against the proposal he has read — evidence the retrieval did not produce, which is what breaks the circle. So the papers are the identity control on these rows, not a detail. The Dataverse line is therefore worded to state what the match does NOT prove (the key came from the same search result); it must not be "restored" to the confident `✓ Known in Dataverse` phrasing used on resolved cards. Residual risk the paper-relevance check does NOT cover, per that review: a same-field namesake and a fragmented cluster of the right person (`reviewer-identity.md:97`). Merely reviewing a paper confirms the PERSON, not automatically the ADDRESS. The owner clarified in S390 that staff may also open a linked paper and locate the corresponding author's exact address; that is valid address-specific evidence when staff explicitly attest the value. The current confirm modal does not capture that separate act and nonetheless stamps `emailSource: 'manual'`; the draft replacement contract is `docs/REVIEWER_ADDRESS_TRUST_AND_CONFLICT_RESOLUTION_PLAN.md`. A manage-only "Use this email/page" currently promotes a lead as `manual`; manual addresses are `quick_check` and require the exact recipient's checkbox in `InviteEmailModal`. Search-derived addresses (`serp_search`, `claude_search`, `search_contested`) are `research_only`: visible for investigation, but render and send both refuse them for first-contact invitations, even if a caller supplies `confirmedLowConfidenceIds`. `InviteEmailModal` offers those rows a staff attestation — "✓ I verified &lt;address&gt; is correct" → `PATCH my-candidates { requestId, suggestionId, verifyEmailAddress:true, verifiedEmail }` → `emailSource='staff_verified'` → `quick_check` (S387), which then re-renders the preview and mints a fresh secure link. Hardened after adversarial review: request-scoped (the person row is shared across requests), lifecycle-gated (selected, not invited, not responded), and ETag-conditional on the person row so a concurrent address swap 409s instead of attesting a substituted address. Full precondition list in `docs/REVIEWER_FINDER_ENFORCEMENT_CONTRACTS.md`. The confirm dialog shows the server's withholding reason, so a `search_contested` override (address contradicts verified identity evidence) is visibly different from a plain unproven search lead. Editing to a DIFFERENT address is still the current `manual` path; the draft replacement makes explicit evidence-backed attestation—not changed/unchanged text—the trust boundary. Identity-anchored NCBI PubMed + Europe PMC evidence runs before paid search: the same address on two distinct recent works is `ready` (`scholarly_multi`), one work is `quick_check` (`scholarly_single`), and tied addresses abstain. Candidate cards show the bounded publication evidence; `pruneCandidateForRoster` keeps the action/reason and a bounded evidence subset across reloads. The saved-candidate editor still routes duplicate-email 409s into `POST /api/reviewer-finder/merge-candidates`; conflict-safe fields can persist with `partialSuccess` before the isolated email write, and merge execution repoints or clears applicant reviewer slots before moving the address. See `docs/REVIEWER_MERGE_DESIGN.md` and `scripts/probe-akoya-potentialreviewer-slot-navprops.mjs`; hand-entered replacements are stamped `manual`. Contracts 3/7 in `docs/REVIEWER_FINDER_ENFORCEMENT_CONTRACTS.md` own the current send and page-fetch gates.
...
request. The complete paper list and Scholar name-search rule remain load-bearing.
