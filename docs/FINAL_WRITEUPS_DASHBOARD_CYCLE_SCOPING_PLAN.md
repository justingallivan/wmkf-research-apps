...
- 2026-09-06 (later): owner shaped Slice 6B as a planning item. Opening view for every role is "Needs my review"; alternatives "Reviewed by me" and "All writeups"; a Program director dropdown; stage filter dropped (two stages only, label already per row). Both controls persist in the URL beside the cycle select. Planned and built (with 6C) in `docs/FINAL_WRITEUPS_DASHBOARD_VIEWS_AND_VERSION_PLAN.md` (2026-09-07, PR #175 merge `44bdd240`, Production-live).
