...
| D2 | generate-emails `:501` | draft generation stamps `wmkf_invited`/`wmkf_emailsentat` raw, unconditionally | none until the legacy mark-as-sent is separately approved (historical row 6) | owner decision; preserve. **Usage check 2026-09-06 (S490) [VERIFIED via `git grep` of the tracked tree]:** `POST /api/reviewer-finder/generate-emails` has ZERO client callers — no page or shared component fetches it (the standalone Reviewer Finder page that did is retired); `api_usage_log` is LLM-keyed and the route never writes it, so there is no runtime usage record either. In-app use is therefore nil; the remaining choice is retire the mark vs. retire the whole route (destructive — needs its own decision and a live-caller pre-flight). |
...
| D4 | `bulkUpdateByRequest` (`reviewer-suggestion.js:2320–2325`) | sequential unconditional `updateLifecycle` per selected row with no try/catch: a middle-row failure throws out of the loop, so the picker route returns an error (not `success:true`) but earlier rows stay written and the caller learns neither which rows nor how to retry | (a) per-row results returned to the picker (additive response field), (b) atomic changeset, (c) per-row `ifMatch` | **DECIDED 2026-09-06: (a) per-row results.** Not yet built. **Pre-build finding (S490) [VERIFIED via `git grep` of every `my-candidates` fetch body in `shared/components/`]:** no client sends the proposal-wide PATCH (`proposalId` without `suggestionId`, `grantCycleCode`/`programArea`) — `ReviewerSearchSection` writes cycle/program-area through `save-candidates`, and the other five my-candidates callers all send `suggestionId`. The branch is reachable only by hand-crafted request, so (a) would improve a response no UI reads. Owner to confirm build-anyway vs. park vs. verify-then-remove before it proceeds. |
