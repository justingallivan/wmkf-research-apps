...
npm run check:reviewer-engagement-boundary && npm run check:reviewer-engagement-boundary:self-test # lib/pages/shared/modules binding a reviewer-suggestion generic writer outside reviewer-engagement/ (LAW mode since Reviewer Lifecycle Stage 7; recorded-importer map may only shrink)
