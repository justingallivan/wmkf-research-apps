...
npm run check:postgres-access-layer && npm run check:postgres-access-layer:self-test # Stage 0 REPORT-ONLY census of direct Postgres access (driver imports, sql tags, query/connect calls); exits 0 on any census result, 2 only on a parse failure; ratchet arrives in Stage 1 of docs/plans/POSTGRES_ACCESS_LAYER_MIGRATION_PLAN_2026-09-23.md
