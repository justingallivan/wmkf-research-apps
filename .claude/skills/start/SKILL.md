...
npm run check:route-service-boundary && npm run check:route-service-boundary:self-test # pages/api routes reaching Dataverse adapters/dynamics-service directly (LAW mode since Route→Service Stage 7; no baseline) + routes reaching Postgres drivers/lib/postgres outside the shrink-only POSTGRES_CARRYOVER (17 at S537-PG freeze; pinned by tests/unit/route-service-boundary-postgres-carryover.test.js)
npm run check:postgres-access-layer && npm run check:postgres-access-layer:self-test # count ratchet over direct Postgres access (driver imports, sql tags, query/connect calls) against scripts/postgres-access-allowlist.json (Stage 0 census frozen S537-PG; shrink-only, no regenerate flag) + routes importing lib/postgres/**; Q6 cast-lint is warn-only; law mode arrives in Stage 7 of docs/plans/POSTGRES_ACCESS_LAYER_MIGRATION_PLAN_2026-09-23.md
npm run check:reviewer-engagement-boundary && npm run check:reviewer-engagement-boundary:self-test # lib/pages/shared/modules binding a reviewer-suggestion generic writer outside reviewer-engagement/ (LAW mode since Reviewer Lifecycle Stage 7; recorded-importer map: stale entries fail, growth requires a deliberate edit to tests/unit/reviewer-engagement-boundary-recorded-set.test.js)
npm run check:script-suggestion-writers && npm run check:script-suggestion-writers:self-test # scripts/ files writing wmkf_appreviewersuggestions outside adapter named ops must be in the tracked RECORDED_SCRIPT_WRITERS set (D5; growth pinned by tests/unit/script-suggestion-writers-recorded-set.test.js)
