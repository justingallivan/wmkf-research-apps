...
5. **Durable-surface** — for a new/changed durable surface, confirm each that applies: migration file · migration manifest (`npm run prebuild`) · Atlas page (`docs/atlas/`, `check:atlas`) · API route security matrix (`check:api-routes`) · `CANONICAL_COUNTS` + `check:fact-consistency` (route/endpoint counts shift) · source-header / service-catalog entry · cap/cleanup strategy · tests for the new contract · the gate that would catch the omission. (Gate names here are a snapshot — `grep '"check:' package.json` for the current set before relying on this list.)
