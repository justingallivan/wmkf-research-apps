...
/**
 * Read-only metadata preflight for wave18-reviewer-due-date-override.
 * Absent and exact are safe; a divergent existing column blocks the
 * creation-only schema applier.
 *
 * Usage:
 *   node scripts/preflight-reviewer-due-date-override-field.mjs --target=prod
 *   node scripts/preflight-reviewer-due-date-override-field.mjs --target=sandbox
 *   node scripts/preflight-reviewer-due-date-override-field.mjs --self-test
 */
...
const FIELD = 'wmkf_reviewduedateoverride';
...
function classify(body) {
  ...
  const notes = [];
  if (body.AttributeType !== EXPECTED.AttributeType) {
    notes.push(`AttributeType ${body.AttributeType} != ${EXPECTED.AttributeType}`);
  }
  ...
  if (body.DateTimeBehavior?.Value !== EXPECTED.DateTimeBehavior) {
    notes.push(`DateTimeBehavior ${body.DateTimeBehavior?.Value} != ${EXPECTED.DateTimeBehavior}`);
  }
...
if (process.argv.includes('--self-test')) {
  ...
  console.log('PASS: reviewer due-date override field preflight self-test.');
  process.exit(0);
}
...
const response = await fetch(
  `${resourceUrl}/api/data/v9.2/EntityDefinitions(LogicalName='${ENTITY}')`
    + `/Attributes(LogicalName='${FIELD}')/Microsoft.Dynamics.CRM.DateTimeAttributeMetadata`
    + '?$select=AttributeType,RequiredLevel,Format,DateTimeBehavior',
...
console.log(`PROCEED: node scripts/apply-dataverse-schema.js --target=${target} --wave=18-reviewer-due-date-override --execute`);
