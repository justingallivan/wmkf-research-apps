 ...
 *
 * Consultant Feedback slice 2 prerequisite (owner decision CF7,
 * docs/plans/CONSULTANT_FEEDBACK_PLAN_2026-09-14.md §4). Same shape as
 * scripts/extend-responsetype-picklist.mjs: read the live option set, no-op if
 * the value exists, dry-run by default, InsertOptionValue inside the app
 * solution only with --execute, then re-read and verify.
 *
 * Wave 16 options (lib/dataverse/schema/wave16-request-document-registry/
 * wmkf_requestdocument.json): 100000000..100000007. Next free integer = 100000008.
 *
 * Usage:
 *   node scripts/extend-requestdocument-artifacttype.mjs            # dry run (reads only)
 *   node scripts/extend-requestdocument-artifacttype.mjs --execute  # insert + verify
 */
import fs from 'fs';
...
const NEW_VALUE = 100000008;
const NEW_LABEL = 'Consultant Feedback';
...
// The metadata read is cache-served and lagged the insert by several seconds on
// 2026-09-14 (first verify showed eight values; a re-read moments later showed
// nine), so poll briefly instead of trusting one immediate read.
let found = null;
let verifyOptions = [];
