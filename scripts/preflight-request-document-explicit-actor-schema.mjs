...
/**
 * Read-only preflight for wave24-request-document-explicit-actor.
 *
 * Classifies one UserLocal DateTime attribute and two optional systemuser
 * relationships as absent, exact, or divergent. This script never writes.
 */
...
const WAVE = '24-request-document-explicit-actor';
const SPEC_PATH = 'lib/dataverse/schema/wave24-request-document-explicit-actor/'
  + 'wmkf_requestdocument_explicit_actor.json';
...
function validateSpec() {
  if (spec.kind !== 'extensions-on-existing'
    ...
    throw new Error('The wave24 spec must extend wmkf_requestdocument.');
  }
  const attributes = new Map((spec.attributes || []).map((attribute) => [
    attribute.schemaName.toLowerCase(),
    attribute,
  ]));
  const initiatedAt = attributes.get('wmkf_initiatedat');
  if (!initiatedAt
      || initiatedAt.type !== 'DateTime'
      || initiatedAt.format !== 'DateAndTime'
      || initiatedAt.behavior !== 'UserLocal') {
    throw new Error('Unexpected Wave 24 InitiatedAt shape.');
  }
  attributes.delete('wmkf_initiatedat');
  if (attributes.size) throw new Error('Unexpected attributes in the wave24 spec.');

  const relationships = new Map((spec.relationships || []).map((relationship) => [
    relationship.lookupSchemaName,
    relationship,
  ]));
  for (const name of ['wmkf_InitiatedBy', 'wmkf_MilestoneCreatedBy']) {
    ...
    if (!relationship
      ...
      throw new Error(`Unexpected Wave 24 lookup shape: ${name}.`);
    }
    relationships.delete(name);
  }
  if (relationships.size) throw new Error('Unexpected relationships in the wave24 spec.');
}
...
async function runSelfTest() {
  ...
  console.log('PASS: Wave 24 explicit-actor spec and metadata projections are valid.');
}
