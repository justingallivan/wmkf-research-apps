...
- **Reviewer search time budget** is the same pattern — Dataverse setting `reviewer.time_budget_seconds` (admin-editable at `/admin` → "Reviewer Search Time Budget", default 600s, clamped [120,800]). The reviewer search routes pin Vercel `maxDuration` at the plan cap (800s) and enforce this live, admin-editable budget in app code via an AbortSignal deadline, because `maxDuration` is build-time-static and can't be runtime-configurable (S223). See `lib/services/reviewer-time-budget.js` + `docs/REVIEWER_TIMEOUT_BUDGET_PLAN.md`.
...
| `prompt_publish_audit` | Append-only audit of `wmkf_ai_prompt` versioned publishes via `/api/admin/prompts/[name]` (the `/admin` Prompt Templates editor). Same pending/final + `request_id` pattern as `policy_publish_audit`, `UNIQUE(request_id, phase)`. Migration `019` (S222). See `docs/atlas/dataverse-wmkf-ai-run-and-prompt.md`. |
...
| `reviewer_find_roster` | Workbench Reviewers→Find durable per-request candidate roster (S224). One row per `(request_id, normalized_name)` — the cross-run dedup key — status `active`\|`excluded`\|`saved`, pruned JSONB render DTO. Makes surfaced search candidates durable + suppresses already-surfaced names from future searches (enforced server-side in `/discover` before the Claude reasoning). **Operational/pre-save state, NOT canonical reviewer identity** (that's Dataverse `wmkf_appreviewersuggestion`) — same class as the retained `search_cache`. Written by `lib/services/reviewer-roster-store.js`. Per-request row cap (300); TTL cron deferred. Migration `020_reviewer_find_roster.sql`. See `docs/atlas/postgres-reviewer-find-roster.md`. |
...
The full route catalogue lives in **`docs/API_ROUTE_SECURITY_MATRIX.md`** ([105](docs/CANONICAL_COUNTS.md#api-route-file-count) route files, CI-gated via `npm run check:api-routes` — PRs touching `pages/api/**` fail without a matrix update). Source files in `pages/api/<app>/` are authoritative for behavior.
...
- **`docs/API_ROUTE_SECURITY_MATRIX.md`** — [105](docs/CANONICAL_COUNTS.md#api-route-file-count)-route catalogue, CI-gated.
