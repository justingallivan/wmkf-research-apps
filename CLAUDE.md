...
| Request Workbench (Reviewers) | `workbench.js` + `workbench/[requestId].js` | `/api/workbench/{dashboard,resolve-request}` | App key `reviewers`. Tier-2 cycle dashboard → tier-3 per-request shell; consolidating Reviewer Finder + Review Manager. Building in phases (S208: Phase 0 grant+`wmkf_applicantdisposition` foundation, Phase 1 dashboard+D26 allowlist, per-request shell stub). Additive `reviewers` grant; the 18 reviewer-finder/review-manager routes accept it via variadic `requireAppAccess`. Not in default grants. See `docs/REQUEST_WORKBENCH_BUILD_PLAN.md`. |
