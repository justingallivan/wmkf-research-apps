...
| Request Workbench (Reviewers) | `workbench.js` + `workbench/[requestId].js` | `/api/workbench/{dashboard,resolve-request}` | App key `reviewers`. Tier-2 cycle dashboard → tier-3 per-request shell; consolidating Reviewer Finder + Review Manager. Phases: S208 Phase 0 (grant + `wmkf_applicantdisposition`) + Phase 1 (dashboard + D26 allowlist); **S209 Phase 2** — Reviewers tab live (Invite/Track/Completed via shared `shared/components/reviewers/ReviewerManagePanel.js` + `ReviewersTab.js`; pure logic in `reviewer-modes.js`; Find tab still a placeholder pending Phase 3). Additive `reviewers` grant; the 18 reviewer-finder/review-manager routes accept it via variadic `requireAppAccess`. Not in default grants. See `docs/REQUEST_WORKBENCH_BUILD_PLAN.md`. |
...
The full route catalogue lives in **`docs/API_ROUTE_SECURITY_MATRIX.md`** ([99](docs/CANONICAL_COUNTS.md#api-route-file-count) route files, CI-gated via `npm run check:api-routes` — PRs touching `pages/api/**` fail without a matrix update). Source files in `pages/api/<app>/` are authoritative for behavior.
...
- **`docs/API_ROUTE_SECURITY_MATRIX.md`** — [99](docs/CANONICAL_COUNTS.md#api-route-file-count)-route catalogue, CI-gated.
