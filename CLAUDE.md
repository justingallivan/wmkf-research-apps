...
The full route catalogue lives in **`docs/API_ROUTE_SECURITY_MATRIX.md`** ([88](docs/CANONICAL_COUNTS.md#api-route-file-count) route files, CI-gated via `npm run check:api-routes` — PRs touching `pages/api/**` fail without a matrix update). Source files in `pages/api/<app>/` are authoritative for behavior.
...
- **`docs/API_ROUTE_SECURITY_MATRIX.md`** — [88](docs/CANONICAL_COUNTS.md#api-route-file-count)-route catalogue, CI-gated.
