...
**Sessions:** 503 (Fable built, reviewed, and merged; Codex built the agenda on its own brief and implemented six adversarial-review findings on the upload path under Claude's review).
**Ship state:** PRs #223–#232 merged (`adcc5f0a` … `cba1c4a9`); migrations 039–044 applied by the owner; `MEETING_TRACKER_SCHEMA_READY` and `SITE_VISIT_MATERIALS_SCHEMA_READY` on in Production; external materials route fails closed from outside; first agenda send and first applicant upload on real requests still pending; PR 3 (PD visibility, auto-close, reminder cron) planned.
**Why it matters:** The first deliberation session (week of 2026-09-14) runs on one link per proposal and one agenda per session, and the first site visits collect slides and bios through the app instead of email attachments.
**Pointers:** `docs/APPLICANT_ADDITIONAL_MATERIALS_PLAN.md` §16, `docs/plans/SESSION_AGENDA_EMAIL_CODEX_BRIEF_2026-09-10.md`, `docs/plans/STAFF_DELIBERATIONS_TAB_SHAPE_BRIEF_2026-09-09.md`, `docs/PC_MEETING_TRACKER_PLAN.md` §9, `docs/API_ROUTE_SECURITY_MATRIX.md` (materials and agenda rows).
...
**Sessions:** 492.
**Ship state:** Plan with three Codex passes; build on `claude/final-writeups-views-and-version` with two Codex diff passes; PR #175 merged `44bdd240`, Production `dpl_2UrsDnydRqudu95wJyLCK7A6FUFR`; owner-run signed-in smoke passed 2026-09-07 (views, empty state, bookmarked and invalid URLs, "Version 5.0", PD filter). Tier 0 follow-ups `dccb39b9` `02bc6a8b` `314e9426` `e68bffe9`. Persona lens label moved from an eyebrow above the title into the header summary. 6D/6E closed by owner. 54 other native selects remain across the suite for a smaller shared variant.
**Why it matters:** The first D26 reviewers meet the intended dashboard shape before any D26 Final exists, and the version label makes "Updated since review" explainable without opening Word.
**Pointers:** `docs/FINAL_WRITEUPS_DASHBOARD_VIEWS_AND_VERSION_PLAN.md` §10–§14, `docs/CURRENT_WORK_QUEUE.md` item 5, `shared/components/ToolbarSelect.js`.
...
browser smoke of the 6B surfaces ran before promotion; owner decision. Session 489 smoked
closeout modal and "Send reminder" on production; release-materials modal still unsmoked
(no `accepted` reviewer to target).
