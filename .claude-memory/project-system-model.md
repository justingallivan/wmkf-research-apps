...
- **Prompt migration to Dataverse is demand-driven** (a prompt moves when it becomes *shared*, esp. a PA caller) and **barely started — ~1 of ~19 LLM surfaces** read from Dataverse today; the rest use bundled in-repo prompts. It's a named workstream, not a background detail. *(Open fork: does staff-editability also force a Dataverse home, or only cross-surface sharing?)*
