...
Do not:
- Use a round "~19/~24" prompt denominator (canonical app count=18; A7 input-surface registry=24; both differ from prompt-bearing-route count).
...
- **Prompt migration to Dataverse is demand-driven** (a prompt moves when it becomes *shared*, esp. a PA caller) and **barely started — exactly one live route** (`/api/phase-i-dynamics/summarize-v2`) reads from Dataverse via the Executor today; the rest use bundled in-repo prompts living in three places per the A7 surface taxonomy (`shared/config/prompts/`, route-local, service-local). It's a named workstream, not a background detail. *(Open fork: does staff-editability also force a Dataverse home, or only cross-surface sharing?)* Don't use a round "~19/~24" denominator — those collide (canonical app count=16; A7 injection input-surface registry=24; both differ from the prompt-bearing-route count).
