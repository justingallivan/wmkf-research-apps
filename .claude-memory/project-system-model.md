...
metadata:
  ...
  status: active
  scope: strategy
  last_verified: 2026-07-08 (S348 audit) — corrected stale hardcoded counts (app count is 12 per CANONICAL_COUNTS, not 18/16; A7 = 27 not 24) and the "one Executor route" claim (peer-review shipped S344); counts now point to live sources
---
...
Do not:
- Use a round "~19/~24" prompt denominator, or hardcode an app count here (it drifts). The app-definition count and the A7 input-surface registry both differ from the prompt-bearing-route count — read the live values from `docs/CANONICAL_COUNTS.md` (app-definition-count) and `docs/CLOSEABLE_CLASS_INVARIANT_MAP.md` (A7 registered surfaces), not from this file.
...
- **An app = (a prompt in Dataverse) × (a thin adapter).** App surfaces → N prompts + a few adapter shapes (Workbench tab · PA trigger · ad hoc standalone).
...
- **Prompt migration to Dataverse is demand-driven** (a prompt moves when it becomes *shared*, esp. a PA caller) and **early — a small, growing set of routes** read from Dataverse via the Executor today (e.g. `/api/phase-i-dynamics/summarize-v2`, and `process-peer-reviews.js` since S344 [VERIFIED via pages/api/process-peer-reviews.js:11]); the rest use bundled in-repo prompts living in three places per the A7 surface taxonomy (`shared/config/prompts/`, route-local, service-local). It's a named workstream, not a background detail. *(Open fork: does staff-editability also force a Dataverse home, or only cross-surface sharing?)* Don't hardcode an app or surface count here — the app-definition count and the A7 input-surface registry both differ from the prompt-bearing-route count; read live values from `docs/CANONICAL_COUNTS.md` and `docs/CLOSEABLE_CLASS_INVARIANT_MAP.md`.
- **The three-tier prompt-resolution *ordering* (Postgres override → Dataverse → bundled) is TARGET, not current code.** *Current reality:* the Executor (`execute-prompt.js:215`) throws on a missing Dataverse prompt row (NO bundled fallback); the bundled fallback lives only in legacy `prompt-resolver.js` (gated by `PROMPT_RESOLVER_STRICT`); the Postgres per-user tier is unbuilt.
...
- The **intake "mid-June 2026 Phase II Research pilot" is DEFUNCT** (Phase II submissions are going away; intake is a **Phase I** build for the *next* cycle). **Reconciled S197** across the high-impact current-state surfaces (CLAUDE.md, MEMORY.md, STRATEGY.md, SESSION_PROMPT.md, intake memories, atlas, BUDGET_FORM_SPEC, REVIEWER_POSTGRES plan); `INTAKE_PORTAL_DESIGN.md` got a SUPERSEDED banner (body left as pilot record pending next-form redesign). Hazard respected — the live J26 reviewer "mid-June 2026" deadline + the 142 live `Phase II Pending` status refs + the `phase-ii-research-2026-06` form-module paths were left untouched. Residual = dated/historical snapshots only.
