...
- **0 / 35 have any `wmkf_appreviewersuggestion` rows** — no candidates, none selected, none accepted. So during a Workbench browser smoke the Reviewers-tab **Invite/Track/Completed panels are all EMPTY** for every D26 request and the state-aware default lands on **Find** (the Phase-2 placeholder). The Phase 2 Manage panel therefore has **no live D26 data to render** — meaningful Manage-panel validation must use an older Phase II cycle via the standalone Review Manager (which does have accepted reviewers), not D26.
