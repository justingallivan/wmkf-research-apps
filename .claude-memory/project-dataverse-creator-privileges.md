...
- **Still summarize after the fact.** Maintain a running audit document at `docs/INTAKE_PORTAL_SCHEMA_CHANGES.md` (live; landed 2026-05-14) listing every entity / field / choice change for the pilot, so Connor has one place to review post-hoc.
...
**What this unblocked for pilot** (as deployed; the in-meeting sketch names were refined pre-deploy — see [[project-slice0-scope]] for current truth):
- `wmkf_portalmembership` entity (Decision 1; no underscore in deployed name).
- Structured-tables for budget + roster (Decision 6, narrowed 2026-05-13): NEW `wmkf_proposalbudgetline` + EXTENSIONS on existing `wmkf_apprequestperson` + `akoya_request.wmkf_totalothersources`. The 2026-05-06 sketch names (`wmkf_budgetline`/`wmkf_personnel`/`wmkf_priorsupport`/`wmkf_milestone`) were superseded; prior-support + milestones DEFERRED out of slice-0 (pilot uses narrative + PDF).
