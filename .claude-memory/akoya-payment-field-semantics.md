...
**Stronger org-wide form [VERIFIED via full scan 2026-06-27]:** across **all 9,151** `akoya_folio="PAID"` disbursements ever recorded, **100% went to an `account` (institution); 0 to an individual `contact`.** The payment engine is rail-agnostic (ACH/check/BILL all just channels — pre-BILL grant #997034 paid by ACH through the same machinery) but **structurally payee-bound to institutions** — it has never paid a person. Approvals are two-stage: a human advances the `akoya_folio` state machine (Ready To Send → Ready to Pay, on `akoya_requestpayment`), then money-out executes in BILL/offline. **The `wmkf_*approval` gate fields are NOT dead** — full-table scan 2026-06-28 (`scripts/probe-akoya-approval-flags-deadness.js`) found `wmkf_authorizationtoremitpaymentflag`=Yes on **303** `akoya_request` rows, `wmkf_executivedirectorapproval` (DateTime) dated on **323**, `wmkf_directorofoperationsapproved` (DateTime) on **611**; only `wmkf_controllerapproved` (Picklist) is unused (0/25,584). They fire on **grants** (Discretionary/Program/Special), **never on Individual/honorarium** requests — consistent with the "Real gates" note above and with honoraria paying offline. (Supersedes an earlier S298 "all null/false" read that over-generalized from grant #1002238.) Full reverse-engineered current-state + the onboarding chain: [[project-honorarium-payment-landscape]].
