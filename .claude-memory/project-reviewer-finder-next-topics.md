...
**Increment 2 SHIPPED S227:** route `pages/api/reviewer-finder/web-suggestions.js` (POST, `requireAppAccess('reviewer-finder','reviewers')`, rate-limited; derives ≤3 web queries + proposalContext from the held `analysisResult` server-side; fail-soft) + security-matrix row + the read-only "Web suggestions" panel + capability-gated `searchWeb` toggle (default-on, hidden when `api-capabilities.reviewerWebSearch` false) in the SHARED `ReviewerSearchSection.js` — which both surfaces render, so one integration covers standalone + Workbench Find tab. The web call runs in `runSearch` as a genRef-guarded fire-and-forget IIFE, fully OFF `/discover`'s error/abort boundary. Tests: `web-suggestions-endpoint.test.js` (11) + `reviewer-web-suggestions-toggle.test.js` (2, jsdom). `WebDiscoveryService` added to the service catalog. **Live Perplexity Search contract VERIFIED 2026-06-05 (S227)** via `scripts/probe-perplexity-search.mjs`: HTTP 200 (account entitled to `/search`, not just sonar chat), `search_after_date_filter` M/D/YYYY accepted + honored, `results[].{title,url,snippet,date,last_updated}` 10/10. **`PERPLEXITY_API_KEY` now live in PROD** (Justin, S227) → capability true → feature activates on deploy (NOT inert anymore). Now in tracked-secrets.js (`perplexity_api_key`, vendor) + runbook. Snippet-budget tuned S227: `WEB_RESULTS_MAX_CHARS` 20K→100K + `EXTRACTION_MAX_TOKENS` 1024→4096 + new `PER_SNIPPET_MAX_CHARS` 6K guard (live probe showed ~8KB faculty-page snippets were truncating all but ~2-3 of 24 results; the constants were untuned defaults, not an API/cost wall — Sonnet 200K window). The VRP-coupling consequence of the always-present key is parked in [[Virtual Review Panel]] — settle it during VRP work, NOT each reviewer session. Add-as-candidate / manual-add path is deferred-v2 (NOT built).
