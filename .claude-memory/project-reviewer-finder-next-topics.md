...
description: "Three reviewer-finder improvement topics Justin flagged EOD S222. #1 (timeout) SHIPPED S223; #2 (recency-weighted identification + current-affiliation pinning) SHIPPED S224; #3 (Perplexity's role) read-only web-suggestions panel — backend increment-1 S225, route+UI increment-2 SHIPPED S227 (inert in prod until PERPLEXITY_API_KEY set + live contract test passes)."
metadata: 
  node_type: memory
  type: project
  status: active
...
## Recall Rule
Read when picking up reviewer-finder improvement work. Justin raised these EOD S222. **#1 + #2 SHIPPED; #3 (Perplexity) read-only panel: backend S225 + route/UI S227 SHIPPED, inert in prod until a key + live contract test** — see §3 for exact state before continuing.
...
**Increment 2 SHIPPED S227:** route `pages/api/reviewer-finder/web-suggestions.js` (POST, `requireAppAccess('reviewer-finder','reviewers')`, rate-limited; derives ≤3 web queries + proposalContext from the held `analysisResult` server-side; fail-soft) + security-matrix row + the read-only "Web suggestions" panel + capability-gated `searchWeb` toggle (default-on, hidden when `api-capabilities.reviewerWebSearch` false) in the SHARED `ReviewerSearchSection.js` — which both surfaces render, so one integration covers standalone + Workbench Find tab. The web call runs in `runSearch` as a genRef-guarded fire-and-forget IIFE, fully OFF `/discover`'s error/abort boundary. Tests: `web-suggestions-endpoint.test.js` (11) + `reviewer-web-suggestions-toggle.test.js` (2, jsdom). `WebDiscoveryService` added to the service catalog. **STILL GATING prod-enable:** the live Perplexity contract test — no `PERPLEXITY_API_KEY` set yet, so `search_after_date_filter` (M/D/YYYY) is unconfirmed; feature is inert in prod (capability false → toggle hidden) until the key is set + the contract test passes. Add-as-candidate / manual-add path is deferred-v2 (NOT built).
