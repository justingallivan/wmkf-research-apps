...
Do:
- Treat the winddown as strategic priority/UI-investment, NOT a development freeze — `/phase-i-dynamics` + `summarize{,-v2}` are still actively iterated as a prompt-tuning surface.
- Keep forward investment on Dynamics-tied user apps (reviewer finder, Phase II writeup/Q&A, Expertise Finder, Grant Reporting, Review Manager).
