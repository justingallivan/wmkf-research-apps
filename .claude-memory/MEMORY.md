...
- New Dataverse WRITE path "done" on green MOCKS (contract lives in the service — verify w/ a real write or metadata+seed-parity probe): feedback-verify-write-paths-against-live-service.md
...
- Branded domains / portal base URLs / wmkeck.org vs vercel.app / *_PORTAL_BASE_URL / NEXTAUTH_URL (verify via /api/health, NOT env pull): project-branded-domains.md
- jsdom / DOMPurify / markdown sanitization on a server route (won't load in Vercel serverless; grantee/app-markdown still latent): project-jsdom-serverless-esm-incompat.md
...
- Delegating a BUILD to codex:codex-rescue (read-only sandbox fixed at launch; can't write sibling worktree dirs — spawn a FRESH build-framed agent, or run it in-checkout): reference-codex-rescue-plan-task-runs-readonly.md
...
- Nomenclature sweep / legacy app sunset / stale memory+wiki cleanup (DEFERRED TODO; overloaded "candidate" vs "Invite Reviewers"; 3 API namespaces for 1 app): project-nomenclature-and-app-sunset-sweep.md
...
- Reviewer invite→accept collapse (S279: onboard at one final Accept; no hold step; capture-only honorarium; .ics on accept): project-reviewer-hold-step-decouple.md
- Reviewer review-form in-browser authoring (Phases 0–5 DONE, S302; tiptap, /submit atomic snapshot changeset, draft lifecycle): ../docs/REVIEWER_REVIEW_FORM_AUTHORING_BUILD_PLAN.md
- Staff-editable review questions epic (S303–S305 COMPLETE; ratings live only in wmkf_appreviewanswer snapshot; don't redeploy pre-E1 code): ../docs/STAFF_EDITABLE_REVIEW_QUESTIONS_BUILD_PLAN.md
...
- Atomic multi-row Dataverse write / `$batch` changeset (WORKS in prod; helper = `DynamicsService.executeChangeset`): project-dataverse-batch-changeset-available.md
- Dataverse alt-key upsert URL whose key includes a LOOKUP (address by `_<lookup>_value=<guid>`, else 400 0x80060888): reference-dataverse-altkey-lookup-upsert-url.md
...
- Prompt / Executor / document processing: ../docs/agent-wiki/topics/prompt-executor.md; adding a prompt / editing a seed / prompt-store governance: project-prompt-governance.md
- BILL / honoraria / payment semantics: ../docs/agent-wiki/topics/finance-honoraria.md; reviewer honorarium onboarding→payment current-state / why no individual paid (0/9,151): project-honorarium-payment-landscape.md
