...
- [Reviewer identity resolution Phase 1 SHIPPED S214](project-reviewer-identity-resolution-phase1.md) — Scholar displayed-name guard + ORCID name-scoring + persistence gate (commit 40d7327). Audit ground truth: persisted-Scholar footprint is 8 pinned profiles (not "~330"); 1 confirmed wrong match (Frank Noe's URL → Cecilia Clementi's profile) + Sigman malformed id. Remediation pending.
