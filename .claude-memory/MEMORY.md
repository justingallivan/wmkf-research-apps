...
- [Slice-0 deactivate-not-delete recalc](slice0-deactivate-not-delete-recalc.md) — Connor's ruling: roster rollup uses `statecode` deactivation, never hard delete. P1-Update (statecode-Update trigger-filter binding) is the single open pre-deploy gate. S165: core-gate test emailed to Connor, AWAITING his Step 11 evidence + Step 12 verdict; next session = verdict-checker, motivated-reasoning guard.
...
- [Slice-0 wmkf_role pre-deploy probe](project-slice0-role-probe.md) — use `scripts/probe-apprequestperson-role-data.js` NOT `dynamics-schema-diff.js`; CLEAR 2026-05-15 (5,561 rows, none in 100000002–4); re-run at deploy time
