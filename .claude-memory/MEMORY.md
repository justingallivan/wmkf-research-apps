...
- [Appresearcher collapse — ✅ SHIPPED 2026-06-02](project-appresearcher-collapse-post-pilot.md) — sidecar collapsed into `wmkf_potentialreviewers` (17 fields folded on the person) + `wmkf_appresearcher`/`apppublication`/`apppublicationauthor` DROPPED. As-executed: `docs/APPRESEARCHER_COLLAPSE_PLAN_V2.md`. Deferred tail: 5 secondary organizationname readers + long-tail prose docs.
...
- [Match reviewer affiliation against existing accounts](project-reviewer-institution-match.md) — `wmkf_appresearcher.wmkf_primaryaffiliation` is uncurated free text; promotion to `contact.parentcustomerid` is the load-bearing join. Reuse intake fuzzy-match.
