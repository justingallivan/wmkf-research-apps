...
- Reviewer address trust / remedies: ../docs/REVIEWER_ADDRESS_TRUST_AND_CONFLICT_RESOLUTION_PLAN.md; ../docs/REVIEWER_EMAIL_CONFLICT_SELF_SERVICE_PLAN.md (S391 fail-closed state machine plus direct stored/found Workbench choice and structural AkoyaGO retry are Production-live as of 2026-08-20; combined identity/email cards now present one Review and confirm action and keep legacy repair-alert state internal; signed-in smoke verified Neville's card and the exact-choice dialog through neutral Cancel; one active legacy repair alert remains for compatibility.)
