...
- [Dataverse OData null-filter trap](project-dataverse-odata-null-filter.md) — `$filter` drops rows evaluating to null; `field ne X` silently EXCLUDES null rows. Use `(field eq null or field ne X)` for any nullable field. Bit S208's exclusion filter.
...
- [ProfileContext refactor runtime bugs (S203)](feedback-profile-context-runtime-bugs.md) — refactor introduced (1) infinite loop: `loadSession` (callback) depended on `state.profiles` which it also caused to change via dispatch → stable identity needed via `useRef`; (2) silent data loss: `fetch()` doesn't throw on HTTP errors (500/403); must check `response.ok` before irreversible actions like `localStorage.removeItem`. CI (lint/tests/build) missed both; manual smoke tests are mandatory for load-bearing async logic.
...
- [D26 reviewer-inputs ground truth (S209 probe)](project-d26-reviewer-inputs-probe.md) — all 35 D26 reqs: 0 existing candidates (Workbench Manage tabs empty → smoke via Review Manager), 5/5 legacy slots populated (~175 recs for Phase 3 ingest), excluded free-text heterogeneous + mostly N/A noise.
