...
1. Claimed the reviewer portal shows the "agree-in-principle / hold" view this cycle. WRONG: the old hold/readiness path was later removed (`lib/external/proposal-readiness.js` and `shared/components/external/HoldView.js` no longer exist); the current producer is `pages/api/external/review/[token]/context.js::computeEngagementState()`, which falls through accepted reviewers to `stage2a`. I read a **consumer** and never re-read the current **producer**.
