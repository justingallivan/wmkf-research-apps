...
description: "Owner principle for global reviewer merge and staff-wide document reads: org-open by design. Narrow exception decided 2026-09-02: request-bound Reviewer Follow-up mutations will be lead-PD/superuser-only at the server boundary; planned, not implemented."
metadata:
  type: project
  status: active
  scope: security
  last_verified: 2026-09-02 — T1 merge and D4 reads remain org-open; request-bound Reviewer Follow-up mutation exception decided
---
...
those actions on 2026-09-02. That target is planned, not yet implemented.
...
is planned in `docs/REVIEWER_FOLLOW_UP_ORG_CYCLE_VISIBILITY_PLAN.md` and is not
live until its implementation and verification complete.
...
- **Request-bound Reviewer Follow-up mutations — planned exception (owner,
  2026-09-02):** server-authoritative lead-PD/superuser writes; other authorized
  staff remain organization-wide readers. This does not alter T1 or D4.
...
- Do not use the older general "reviewer APIs stay org-open" wording to block or
  weaken the planned request-bound mutation hardening.
