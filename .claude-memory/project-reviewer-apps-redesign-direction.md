...
- **Reviewer-tab structure = 4-tab + status badges, DECIDED S206** (Find / Invite / Track / Approve & Pay). The arc within S206: first landed on 3-tab (Find/Roster/Approve & Pay) as simpler, then Justin reconsidered and locked 4-tab — "Roster" is a noun that breaks the all-actions label pattern and hides the work; Invite (compose+send) vs Track (monitor+chase) are genuinely different modes worth separating; the white-space worry was inherited from the old standalone Manager and is minor at per-request scale. Resolution = 4-tab with count/status badges on the tab bar (e.g. Track "1 pending · ⚠1", Approve & Pay "1 to approve"), so the bar doubles as the at-a-glance overview that Roster provided. Badges count work-REMAINING, not totals (Approve & Pay = returned − already-approved; Track = pending + overdue), and every sub-panel's rows are generated from the same per-request counts so row-count and badge never drift. Default landing is **state-aware** — the earliest funnel step with outstanding PD work (Invite if shortlisted-but-unsent, Track if invited/in-flight, Approve & Pay if reviews are back awaiting approval, Find if nothing started); a fixed "always Track" would skip the actionable Invite state. 3-tab kept behind a compare toggle in the mockup only.
...
**Honorarium is NOT a PD-facing tab.** Approve & Pay maps to **existing, deployed** fields on `wmkf_appreviewersuggestion`: PD closeout sets `wmkf_reviewstatus = complete (100000004)` + stamps `wmkf_completedat` (S196, prod 2026-05-28). Per `docs/INTAKE_PORTAL_SCHEMA_CHANGES.md`, `wmkf_completedat` is **record-keeping only — NO PA/automation reacts to it** (row just drops off the PD dashboard). Honorarium provenance = the `wmkf_HonorariumRequest` lookup on `wmkf_appreviewersuggestion` (**SHIPPED 2026-05-28** per [[project-bill-honorarium-integration]] — NOT still owed, and NOT on `wmkf_potentialreviewer`). Payment-eligibility keys on `wmkf_reviewreceivedat` (reviewer submitted), with staff holding the remit gate `wmkf_authorizationtoremitpaymentflag`. **OPEN POLICY Q:** should PD closeout actually gate payment (a behavior change — loop in Steph), or stay a "PD done" marker with eligibility on reviewer-submission? Justin/Steph call.
...
S206 settled the reviewer tab-structure fork — DECIDED 4-tab + status badges (briefly 3-tab, then reconsidered and locked) — and disambiguated "Closeout" (reviewer-level → "Approve & Pay"; request-level → read-only "Status"). **Scoping doc written 2026-05-31: `docs/REQUEST_WORKBENCH_SCOPING.md`.** The old "approve→payable field owed to Connor" item was stale — the closeout fields are already deployed (see Honorarium note above). Remaining open: the PD-closeout-gates-payment policy question (Justin/Steph); PD dashboard row content; J27 phase trigger (Connor).
