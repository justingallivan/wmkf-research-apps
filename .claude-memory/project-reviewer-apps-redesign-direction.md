...
- **Request endpoint = read-only "Status" tab, NOT a PD decision.** Justin: staff only *recommend*; the BOARD decides approve/decline and it is entered into Dynamics by someone else. So the top-level final tab is a read-only reflection of the proposal's own lifecycle string `akoya_requeststatus` on `akoya_request`, not an editable PD field. Confirmed values ([[project-grant-lifecycle-states-confirmed]], prod 2026-05-01) are the pending stages (Concept Pending → Phase I Pending → Phase II Pending); later funded/declined states exist in the same field but were NOT surveyed, so the exact final-disposition strings are unverified. This is **distinct from** (a) the reviewer-level approve→payable field (S206 question Q&A 2026-05-31 — that field is reviewer-scoped, lives on the suggestion/contact side per [[project-bill-honorarium-integration]], owed to Connor, and does NOT drive this tab) and (b) `wmkf_phaseiistatus` (a separate Phase-II-specific field, often null). Still tentative — Justin isn't sure yet what else belongs at the request endpoint.
