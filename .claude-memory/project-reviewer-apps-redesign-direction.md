...
- **Request endpoint = read-only "Status" tab, NOT a PD decision.** Justin: staff only *recommend*; the BOARD decides approve/decline and it is entered into Dynamics by someone else. So the top-level final tab is a read-only reflection of the proposal's own lifecycle string `akoya_requeststatus` on `akoya_request`, not an editable PD field. The full value set IS documented (don't re-flag as "unverified"): `docs/DATAVERSE_POWER_TOOLS_DESIGN.md` carries the probe-derived value→class map (`scripts/probe-akoya-status-predicate.js`, 2026-05-16, ambiguous-middle user-confirmed S158) — in-flight = Pending-family (Concept / Phase I / Phase II Pending, Pending); decided-terminal = Approved / Active / Denied / *Declined / *Ineligible / Closed / *Done / Withdrawn / Proposal Not Invited. ([[project-grant-lifecycle-states-confirmed]] only surveyed the pending stages on 2026-05-01; the terminal values were enumerated later in the Power Tools probes — the two are consistent, not contradictory.) This is **distinct from** (a) the reviewer-level approve→payable field (S206 question Q&A 2026-05-31 — that field is reviewer-scoped, lives on the suggestion/contact side per [[project-bill-honorarium-integration]], owed to Connor, and does NOT drive this tab) and (b) `wmkf_phaseiistatus` (a separate Phase-II-specific field, often null). Still tentative — Justin isn't sure yet what else belongs at the request endpoint.
...
**Tabs — DECIDED S206: 4-tab + status badges.** Default landing is Track.
