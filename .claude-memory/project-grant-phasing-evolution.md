...
- **Document capture/storage is itself evolving (S258).** D26 resolves request docs by listing the SharePoint `Phase I` subfolder + matching consistent filenames — an INTERIM BRIDGE. J27 collects documents differently (new naming at least) and the converging target is **direct Dataverse-table doc references** (e.g. `wmkf_requestdocument`), not folder-walking. Single-submission may also un-scaffold the reviewer hold step. Full picture + sequencing: [[project-j27-doc-capture-evolution]].
