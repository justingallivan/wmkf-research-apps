...
- Admin publish = adapt `pages/api/admin/policies.js` protocol (no Dataverse `$batch`): `prompt_publish_audit` pending row, If-Match ETag flip, resume-repair, invariant = exactly one `iscurrent`. The original S222 body-hash idempotency was superseded locally on 2026-08-16 by a payload-bound v2 fingerprint covering body, system, variables, output schema, model, temperature, and max tokens, plus expected-version rejection. PUT still clones/publishes a full immutable row.
